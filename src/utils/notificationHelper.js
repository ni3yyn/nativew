...
const PRODUCT_SPECIFIC_MESSAGES = {
  ...
  sunscreen: {
    ...
    evening: (name, pName) => [
      ...
      `تذكرة مسائية: لا تنسي ${pName} غدا صباحا ☀️`
    ]
  },
  serum: {
    morning: (name, pName) => [
...
const MESSAGES = {
  ...
  evening: {
    empty: (name) => [
      `مساء الخير يا ${name} 🌙.. لا تتركي رفّك فارغا.`,
    ...
    acne: (name) => [
      `يا ${name}، غلاف الوسادة نظيف = وجه نظيف.. غيرتيه مؤخرا؟`,
      `عالجي الحبوب الآن لتختفي غدا.. التزامك يصنع الفرق.`,
    ],
    anti_aging: (name) => [
