export const combinedOilsDB = {
  ingredients: [
{
        id: 'jojoba',
        name: 'زيت الجوجوبا',
        scientific_name: 'Simmondsia Chinensis Seed Oil',
        searchKeywords: ['jojoba', 'jojoba oil', 'simmondsia chinensis', 'jojoba seed oil', 'جوجوبا', 'زيت الجوجوبا', 'هوهوبا'],
        usageScope: ['skin', 'hair'],
        functionalCategory: 'مرطب / مطري',
        chemicalType: 'زيت ناقل',
        benefits: { 'ترطيب': 0.9, 'تنظيم الإفرازات الدهنية': 0.85, 'ملطف ومرمم': 0.75, 'دعم حاجز البشرة': 0.8 },
        synergy: { 'argan': 1.05, 'tea-tree-essential': 1.1 },
        negativeSynergy: {},
        warnings: [
          { level: 'info', text: 'يشبه في تركيبه دهون البشرة الطبيعية، مما يجعله مناسبا لجميع أنواع البشرة.' }
        ],
      },
{
        id: 'argan',
        name: 'زيت الأرغان',
        scientific_name: 'Argania Spinosa Kernel Oil',
        searchKeywords: ['argan', 'argan oil', 'argania spinosa', 'argan kernel oil', 'moroccan argan oil', 'أرغان', 'ارغان', 'زيت الأرغان', 'الأركان', 'اركان'],
        usageScope: ['skin', 'hair'],
        functionalCategory: 'مرطب / مطري',
        chemicalType: 'زيت ناقل',
        benefits: { 'ترطيب': 0.88, 'مضاد أكسدة': 0.8, 'تنعيم وإعطاء لمعان': 0.9, 'دعم حاجز البشرة': 0.75 },
        synergy: { 'jojoba': 1.1, 'rosemary-essential': 1.05 },
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'sweet-almond',
        name: 'زيت اللوز الحلو',
        scientific_name: 'Prunus Amygdalus Dulcis Oil',
        searchKeywords: ['sweet almond oil', 'almond oil', 'prunus amygdalus dulcis', 'لوز حلو', 'زيت اللوز الحلو', 'زيت اللوز'],
        usageScope: ['skin', 'hair'],
        functionalCategory: 'مرطب / مطري',
        chemicalType: 'زيت ناقل',
        benefits: { 'ترطيب': 0.85, 'ملطف ومرمم': 0.8, 'تنعيم وإعطاء لمعان': 0.75 },
        synergy: { 'lavender-essential': 1.05 },
        negativeSynergy: {},
        warnings: [
          { level: 'risk', text: 'يجب تجنبه من قبل الأشخاص الذين يعانون من حساسية المكسرات.' }
        ],
        
      },
{
        id: 'rosehip',
        name: 'زيت بذور الورد',
        scientific_name: 'Rosa Canina Seed Oil',
        searchKeywords: ['rosehip oil', 'rosehip seed oil', 'rosa canina', 'rose hip', 'روزهيب', 'زيت بذور الورد', 'زيت ثمر الورد'],
        usageScope: ['skin'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'زيت ناقل',
        benefits: { 'تفتيح وتوحيد اللون': 0.85, 'مكافحة الشيخوخة': 0.88, 'ملطف ومرمم': 0.8, 'دعم حاجز البشرة': 0.75 },
        synergy: { 'vitamin-c-serum': 1.1, 'frankincense-essential': 1.08 },
        negativeSynergy: {},
        warnings: [
          { level: 'info', text: 'غني بفيتامين A الطبيعي (حمض الريتينويك الطبيعي)، مما يساعد على تجديد الخلايا.' },
          { level: 'caution', text: 'قد يسبب تهيجا لحب الشباب النشط والملتهب بسبب خصائصه المحفزة لتجدد الخلايا.' }
        ],
      },
{
        id: 'squalane',
        name: 'السكوالان',
        scientific_name: 'Squalane',
        searchKeywords: ['squalane', 'phytosqualane', 'olive squalane', 'sugarcane squalane', 'سكوالان', 'سكوالين مهدرج'],
        usageScope: ['skin', 'hair'],
        functionalCategory: 'مرطب / مطري',
        chemicalType: 'مرطب',
        benefits: { 'ترطيب': 0.95, 'دعم حاجز البشرة': 0.8, 'تنعيم وإعطاء لمعان': 0.85 },
        synergy: { 'hyaluronic-acid-serum': 1.1, 'niacinamide': 1.05 },
        negativeSynergy: {},
        warnings: [
          { level: 'info', text: 'زيت خفيف جدا، غير كوميدوجينيك، ومناسب لجميع أنواع البشرة بما في ذلك الدهنية.' }
        ],
      },
{
        id: 'coconut',
        name: 'زيت جوز الهند',
        scientific_name: 'Cocos Nucifera Oil',
        searchKeywords: ['coconut oil', 'cocos nucifera', 'fractionated coconut oil', 'virgin coconut oil', 'جوز الهند', 'زيت جوز الهند', 'زيت النارجيل'],
        usageScope: ['skin', 'hair'],
        functionalCategory: 'مرطب / مطري',
        chemicalType: 'زيت ناقل',
        benefits: { 'ترطيب': 0.9, 'تقوية الشعر': 0.88, 'مضاد للميكروبات': 0.75 },
        synergy: { 'castor': 1.05 },
        negativeSynergy: {},
        warnings: [
          { level: 'caution', text: 'قد يكون كوميدوجينيك (يسد المسام) وغير مناسب لبشرة الوجه المعرضة لحب الشباب.' }
        ],
      },
{
        id: 'castor',
        name: 'زيت الخروع',
        scientific_name: 'Ricinus Communis Seed Oil',
        searchKeywords: ['castor oil', 'ricinus communis', 'jamaican black castor oil', 'خروع', 'زيت الخروع'],
        usageScope: ['skin', 'hair'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'زيت ناقل',
        benefits: { 'تحفيز نمو الشعر': 0.9, 'ترطيب': 0.75, 'مضاد للالتهابات': 0.7 },
        synergy: { 'rosemary-essential': 1.15, 'argan': 1.05 },
        negativeSynergy: {},
        warnings: [
          { level: 'caution', text: 'زيت ثقيل ولزج، قد يكون من الصعب غسله من الشعر.' }
        ],
      },
{
        id: 'grapeseed',
        name: 'زيت بذور العنب',
        scientific_name: 'Vitis Vinifera Seed Oil',
        searchKeywords: ['grapeseed oil', 'vitis vinifera', 'grape seed oil', 'بذور العنب', 'زيت بذور العنب', 'زيت العنب'],
        usageScope: ['skin', 'hair'],
        functionalCategory: 'مرطب / مطري',
        chemicalType: 'زيت ناقل',
        benefits: { 'ترطيب': 0.8, 'مضاد أكسدة': 0.75, 'تنظيم الإفرازات الدهنية': 0.7 },
        synergy: { 'lemongrass-essential': 1.05 },
        negativeSynergy: {},
        warnings: [
          { level: 'info', text: 'زيت خفيف وسريع الامتصاص، مناسب للبشرة الدهنية والمختلطة.' }
        ],
      },
{
        id: 'avocado',
        name: 'زيت الأفوكادو',
        scientific_name: 'Persea Gratissima Oil',
        searchKeywords: ['avocado oil', 'persea gratissima', 'أفوكادو', 'زيت الأفوكادو', 'زيت لافوكا'],
        usageScope: ['skin', 'hair'],
        functionalCategory: 'مرطب / مطري',
        chemicalType: 'زيت ناقل',
        benefits: { 'ترطيب': 0.88, 'تغذية عميقة': 0.9, 'دعم حاجز البشرة': 0.8 },
        synergy: { 'vitamin-e': 1.05 },
        negativeSynergy: {},
        warnings: [
          { level: 'info', text: 'زيت غني ومغذي، مثالي للبشرة الجافة والناضجة.' }
        ],
      },
{
        id: 'olive',
        name: 'زيت الزيتون',
        scientific_name: 'Olea Europaea Fruit Oil',
        searchKeywords: ['olive oil', 'olea europaea', 'extra virgin olive oil', 'زيتون', 'زيت الزيتون'],
        usageScope: ['skin', 'hair'],
        functionalCategory: 'مرطب / مطري',
        chemicalType: 'زيت ناقل',
        benefits: { 'ترطيب': 0.85, 'مضاد أكسدة': 0.75, 'تغذية عميقة': 0.8 },
        synergy: {},
        negativeSynergy: {},
        warnings: [
          { level: 'caution', text: 'زيت ثقيل وقد يسد المسام للبشرة المعرضة لحب الشباب.' }
        ],
      },
{
        id: 'sunflower',
        name: 'زيت دوار الشمس',
        scientific_name: 'Helianthus Annuus Seed Oil',
        searchKeywords: ['sunflower oil', 'helianthus annuus', 'دوار الشمس', 'زيت دوار الشمس', 'عباد الشمس'],
        usageScope: ['skin', 'hair'],
        functionalCategory: 'مرطب / مطري',
        chemicalType: 'زيت ناقل',
        benefits: { 'ترطيب': 0.8, 'دعم حاجز البشرة': 0.85, 'ملطف ومرمم': 0.75 },
        synergy: { 'oat-extract': 1.05 },
        negativeSynergy: {},
        warnings: [
          { level: 'info', text: 'غني بحمض اللينوليك، مما يجعله خيارا جيدا لدعم حاجز البشرة دون أن يكون ثقيلا.' }
        ],
      },
{
        id: 'hemp-seed',
        name: 'زيت بذور القنب',
        scientific_name: 'Cannabis Sativa Seed Oil',
        searchKeywords: ['hemp seed oil', 'cannabis sativa seed oil', 'hemp oil', 'قنب', 'زيت بذور القنب', 'زيت الحشيش'],
        usageScope: ['skin', 'hair'],
        functionalCategory: 'ملطف ومرمم',
        chemicalType: 'زيت ناقل',
        benefits: { 'ملطف ومرمم': 0.88, 'مضاد للالتهابات': 0.85, 'تنظيم الإفرازات الدهنية': 0.8, 'ترطيب': 0.75 },
        synergy: {},
        negativeSynergy: {},
        warnings: [
          { level: 'info', text: 'لا يحتوي على مركبات نفسية (THC). ممتاز لتهدئة البشرة المتهيجة والمعرضة للأكزيما.' }
        ],
      },
{
        id: 'shea-butter',
        name: 'زبدة الشيا',
        scientific_name: 'Butyrospermum Parkii Butter',
        searchKeywords: ['shea butter', 'butyrospermum parkii', 'شيا', 'زبدة الشيا', 'زبدة الكاريتي'],
        usageScope: ['skin', 'hair'],
        functionalCategory: 'مرطب / مطري',
        chemicalType: 'زبدة',
        benefits: { 'ترطيب': 0.95, 'دعم حاجز البشرة': 0.92, 'ملطف ومرمم': 0.85 },
        synergy: { 'coconut': 1.05 },
        negativeSynergy: {},
        warnings: [
          { level: 'caution', text: 'قد تكون ثقيلة على بشرة الوجه الدهنية أو المعرضة لحب الشباب.' }
        ],
      },
{
        id: 'cocoa-butter',
        name: 'زبدة الكاكاو',
        scientific_name: 'Theobroma Cacao Seed Butter',
        searchKeywords: ['cocoa butter', 'theobroma cacao', 'كاكاو', 'زبدة الكاكاو'],
        usageScope: ['skin'],
        functionalCategory: 'مرطب / مطري',
        chemicalType: 'زبدة',
        benefits: { 'ترطيب': 0.9, 'دعم حاجز البشرة': 0.88, 'مضاد أكسدة': 0.75 },
        synergy: {},
        negativeSynergy: {},
        warnings: [
          { level: 'caution', text: 'كوميدوجينيك (قد تسد المسام) وغير مناسبة للوجه إذا كانت البشرة دهنية.' }
        ],
      },
{
        id: 'mango-butter',
        name: 'زبدة المانجو',
        scientific_name: 'Mangifera Indica Seed Butter',
        searchKeywords: ['mango butter', 'mangifera indica', 'مانجو', 'زبدة المانجو'],
        usageScope: ['skin', 'hair'],
        functionalCategory: 'مرطب / مطري',
        chemicalType: 'زبدة',
        benefits: { 'ترطيب': 0.88, 'تنعيم وإعطاء لمعان': 0.8, 'مضاد أكسدة': 0.7 },
        synergy: {},
        negativeSynergy: {},
        warnings: [
          { level: 'info', text: 'أخف قواما من زبدة الشيا والكاكاو، مما يجعلها خيارا جيدا للكريمات الأقل دهنية.' }
        ],
      },
{
        id: 'tamanu',
        name: 'زيت التامانو',
        scientific_name: 'Calophyllum Inophyllum Seed Oil',
        searchKeywords: ['tamanu oil', 'calophyllum inophyllum', 'tamanu', 'تامانو', 'زيت التامانو'],
        usageScope: ['skin'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'زيت ناقل',
        benefits: { 'ملطف ومرمم': 0.9, 'مكافحة حب الشباب': 0.85, 'مضاد للميكروبات': 0.8, 'تفتيح وتوحيد اللون': 0.75 },
        synergy: { 'tea-tree-essential': 1.05 },
        negativeSynergy: {},
        warnings: [
          { level: 'caution', text: 'له رائحة عشبية قوية قد لا تكون مقبولة للجميع.' }
        ],
      },
{
        id: 'neem',
        name: 'زيت النيم',
        scientific_name: 'Azadirachta Indica Seed Oil',
        searchKeywords: ['neem oil', 'azadirachta indica', 'neem', 'نيم', 'زيت النيم'],
        usageScope: ['skin', 'hair'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'زيت ناقل',
        benefits: { 'مضاد للميكروبات': 0.9, 'مكافحة حب الشباب': 0.85, 'مضاد للفطريات': 0.8, 'طارد للحشرات': 0.75 },
        synergy: { 'tea-tree-essential': 1.1 },
        negativeSynergy: {},
        warnings: [
          { level: 'caution', text: 'له رائحة قوية جدا وغير مستساغة.' }
        ],
      },
{
        id: 'black-seed',
        name: 'زيت حبة البركة',
        scientific_name: 'Nigella Sativa Seed Oil',
        searchKeywords: ['black seed oil', 'nigella sativa', 'black cumin seed oil', 'حبة البركة', 'زيت حبة البركة', 'الحبة السوداء', 'كمون أسود'],
        usageScope: ['skin', 'hair'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'زيت ناقل',
        benefits: { 'مضاد للالتهابات': 0.9, 'مضاد أكسدة': 0.85, 'مضاد للميكروبات': 0.8, 'تقوية الشعر': 0.75 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'pomegranate-seed',
        name: 'زيت بذور الرمان',
        scientific_name: 'Punica Granatum Seed Oil',
        searchKeywords: ['pomegranate seed oil', 'punica granatum', 'رمان', 'زيت بذور الرمان'],
        usageScope: ['skin'],
        functionalCategory: 'مضاد أكسدة',
        chemicalType: 'زيت ناقل',
        benefits: { 'مضاد أكسدة': 0.95, 'مكافحة الشيخوخة': 0.9, 'مضاد للالتهابات': 0.85, 'ملطف ومرمم': 0.8 },
        synergy: { 'vitamin-c-serum': 1.1 },
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'prickly-pear-seed',
        name: 'زيت بذور التين الشوكي',
        scientific_name: 'Opuntia Ficus-Indica Seed Oil',
        searchKeywords: ['prickly pear seed oil', 'barbary fig seed oil', 'opuntia ficus-indica', 'تين شوكي', 'زيت بذور التين الشوكي'],
        usageScope: ['skin'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'زيت ناقل',
        benefits: { 'مكافحة الشيخوخة': 0.95, 'تفتيح وتوحيد اللون': 0.9, 'ترطيب': 0.88, 'دعم حاجز البشرة': 0.85 },
        synergy: { 'rosehip': 1.1 },
        negativeSynergy: {},
        warnings: [
          { level: 'info', text: 'زيت فاخر وغني بفيتامين K، ممتاز للمساعدة في تقليل مظهر الهالات السوداء.' }
        ],
      },
{
        id: 'seabuckthorn',
        name: 'زيت نبق البحر',
        scientific_name: 'Hippophae Rhamnoides Fruit Oil',
        searchKeywords: ['sea buckthorn oil', 'hippophae rhamnoides', 'seabuckthorn', 'نبق البحر', 'زيت نبق البحر'],
        usageScope: ['skin'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'زيت ناقل',
        benefits: { 'ملطف ومرمم': 0.9, 'مضاد أكسدة': 0.88, 'مضاد للالتهابات': 0.85, 'تفتيح وتوحيد اللون': 0.8 },
        synergy: { 'rosehip': 1.08 },
        negativeSynergy: {},
        warnings: [
          { level: 'caution', text: 'لونه برتقالي غامق وقد يصبغ البشرة والملابس مؤقتا. يفضل استخدامه ليلا.' }
        ],
      },
{
        id: 'borage-oil',
        name: 'زيت لسان الثور',
        scientific_name: 'Borago Officinalis Seed Oil',
        searchKeywords: ['borage oil', 'borago officinalis', 'لسان الثور', 'زيت لسان الثور'],
        usageScope: ['skin'],
        functionalCategory: 'ملطف ومرمم',
        chemicalType: 'زيت ناقل',
        benefits: { 'مضاد للالتهابات': 0.92, 'دعم حاجز البشرة': 0.88, 'ملطف ومرمم': 0.9, 'ترطيب': 0.8 },
        synergy: { 'evening-primrose': 1.05 },
        negativeSynergy: {},
        warnings: [
          { level: 'info', text: 'غني جدا بحمض جاما لينولينيك (GLA)، ممتاز للبشرة التي تعاني من الإكزيما والتهيج.' }
        ],
      },
{
        id: 'evening-primrose',
        name: 'زيت زهرة الربيع المسائية',
        scientific_name: 'Oenothera Biennis Oil',
        searchKeywords: ['evening primrose oil', 'oenothera biennis', 'زهرة الربيع المسائية', 'زيت زهرة الربيع'],
        usageScope: ['skin'],
        functionalCategory: 'ملطف ومرمم',
        chemicalType: 'زيت ناقل',
        benefits: { 'مضاد للالتهابات': 0.88, 'دعم حاجز البشرة': 0.85, 'ملطف ومرمم': 0.85, 'ترطيب': 0.8 },
        synergy: { 'borage-oil': 1.05 },
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'marula',
        name: 'زيت المارولا',
        scientific_name: 'Sclerocarya Birrea Seed Oil',
        searchKeywords: ['marula oil', 'sclerocarya birrea', 'marula', 'مارولا', 'زيت المارولا'],
        usageScope: ['skin'],
        functionalCategory: 'مرطب / مطري',
        chemicalType: 'زيت ناقل',
        benefits: { 'ترطيب': 0.9, 'مضاد أكسدة': 0.85, 'تنعيم وإعطاء لمعان': 0.8, 'سريع الامتصاص': 0.88 },
        synergy: { 'squalane': 1.05 },
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'moringa',
        name: 'زيت المورينجا',
        scientific_name: 'Moringa Oleifera Seed Oil',
        searchKeywords: ['moringa oil', 'moringa oleifera', 'moringa', 'مورينجا', 'زيت المورينجا'],
        usageScope: ['skin', 'hair'],
        functionalCategory: 'مرطب / مطري',
        chemicalType: 'زيت ناقل',
        benefits: { 'مضاد أكسدة': 0.88, 'تطهير': 0.8, 'مضاد للالتهابات': 0.8, 'تقوية الشعر': 0.75 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'broccoli-seed',
        name: 'زيت بذور البروكلي',
        scientific_name: 'Brassica Oleracea Italica Seed Oil',
        searchKeywords: ['broccoli seed oil', 'brassica oleracea', 'بروكلي', 'زيت بذور البروكلي'],
        usageScope: ['hair'],
        functionalCategory: 'محسن قوام',
        chemicalType: 'زيت ناقل',
        benefits: { 'تنعيم وإعطاء لمعان': 0.95, 'بديل طبيعي للسيليكون': 0.9, 'تقليل الهيشان': 0.85 },
        synergy: { 'abyssinian': 1.05 },
        negativeSynergy: {},
        warnings: [
          { level: 'info', text: 'يعطي انزلاقا ولمعانا للشعر يشبه تأثير السيليكون.' }
        ],
      },
{
        id: 'abyssinian',
        name: 'الزيت الحبشي',
        scientific_name: 'Crambe Abyssinica Seed Oil',
        searchKeywords: ['abyssinian oil', 'crambe abyssinica', 'الزيت الحبشي', 'زيت الحبشة'],
        usageScope: ['hair'],
        functionalCategory: 'محسن قوام',
        chemicalType: 'زيت ناقل',
        benefits: { 'تنعيم وإعطاء لمعان': 0.92, 'بديل طبيعي للسيليكون': 0.88, 'فك تشابك الشعر': 0.85 },
        synergy: { 'broccoli-seed': 1.1 },
        negativeSynergy: {},
        warnings: [
          { level: 'info', text: 'خفيف الوزن للغاية ومقاوم للحرارة، بديل ممتاز للسيليكونات المتطايرة.' }
        ],
      },
{
        id: 'amla-oil',
        name: 'زيت الأملا',
        scientific_name: 'Phyllanthus Emblica Fruit Extract',
        searchKeywords: ['amla oil', 'phyllanthus emblica', 'indian gooseberry', 'amla', 'أملج', 'زيت الأملا'],
        usageScope: ['hair'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'زيت ناقل (مُنقوع)',
        benefits: { 'تقوية الشعر': 0.9, 'تحفيز نمو الشعر': 0.88, 'تقليل الشيب المبكر': 0.8 },
        synergy: { 'coconut': 1.08, 'castor': 1.05 },
        negativeSynergy: {},
        warnings: [
          { level: 'caution', text: 'قد يغمق لون الشعر الفاتح مع الاستخدام المتكرر.' }
        ],
      },
{
        id: 'fenugreek-oil',
        name: 'زيت الحلبة',
        scientific_name: 'Trigonella Foenum-Graecum Seed Oil',
        searchKeywords: ['fenugreek oil', 'trigonella foenum-graecum', 'methi oil', 'حلبة', 'زيت الحلبة'],
        usageScope: ['hair'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'زيت ناقل (مُنقوع)',
        benefits: { 'تقوية الشعر': 0.88, 'تقليل تساقط الشعر': 0.85, 'علاج القشرة': 0.78 },
        synergy: { 'coconut': 1.05, 'rosemary-essential': 1.1 },
        negativeSynergy: {},
        warnings: [
          { level: 'caution', text: 'له رائحة قوية تشبه شراب القيقب قد تبقى في الشعر.' }
        ],
      },
{
        id: 'pumpkin-seed',
        name: 'زيت بذور اليقطين',
        scientific_name: 'Cucurbita Pepo Seed Oil',
        searchKeywords: ['pumpkin seed oil', 'cucurbita pepo', 'يقطين', 'زيت بذور اليقطين', 'Green Pumpkin Seeds', 'بذور القرع الأخضر'],
        usageScope: ['hair', 'skin'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'زيت ناقل',
        benefits: { 'تحفيز نمو الشعر': 0.85, 'تقوية الشعر': 0.8, 'مضاد أكسدة': 0.8 },
        synergy: { 'rosemary-essential': 1.1 },
        negativeSynergy: {},
        warnings: [
          { level: 'info', text: 'يُعتقد أنه يساعد في منع تساقط الشعر الوراثي عن طريق تثبيط إنزيم 5-alpha-reductase.' }
        ],
      },
{
        id: 'flaxseed',
        name: 'زيت بذور الكتان',
        scientific_name: 'Linum Usitatissimum Seed Oil',
        searchKeywords: ['flaxseed oil', 'linseed oil', 'flax seed', 'linum usitatissimum', 'كتان', 'زيت بذور الكتان', 'زيت الكتان', "زيت بذر الكتان", 'بذور الكتان'],
        usageScope: ['hair'],
        functionalCategory: 'مرطب / مطري',
        chemicalType: 'زيت ناقل',
        benefits: { 'تنعيم وإعطاء لمعان': 0.8, 'تحديد تموجات الشعر': 0.85, 'تغذية': 0.75 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'macadamia',
        name: 'زيت المكاديميا',
        scientific_name: 'Macadamia Ternifolia Seed Oil',
        searchKeywords: ['macadamia oil', 'macadamia ternifolia', 'macadamia nut oil', 'مكاديميا', 'زيت المكاديميا'],
        usageScope: ['skin', 'hair'],
        functionalCategory: 'مرطب / مطري',
        chemicalType: 'زيت ناقل',
        benefits: { 'ترطيب': 0.85, 'تنعيم وإعطاء لمعان': 0.8, 'دعم حاجز البشرة': 0.8 },
        synergy: { 'argan': 1.05 },
        negativeSynergy: {},
        warnings: [
          { level: 'risk', text: 'يجب تجنبه من قبل الأشخاص الذين يعانون من حساسية المكسرات.' }
        ],
      },
{
        id: 'apricot-kernel',
        name: 'زيت بذور المشمش',
        scientific_name: 'Prunus Armeniaca Kernel Oil',
        searchKeywords: ['apricot kernel oil', 'prunus armeniaca', 'مشمش', 'زيت بذور المشمش', 'زيت نواة المشمش'],
        usageScope: ['skin', 'hair'],
        functionalCategory: 'مرطب / مطري',
        chemicalType: 'زيت ناقل',
        benefits: { 'ترطيب': 0.8, 'ملطف ومرمم': 0.75, 'سريع الامتصاص': 0.85 },
        synergy: { 'sweet-almond': 1.05 },
        negativeSynergy: {},
        warnings: [
          { level: 'info', text: 'مشابه لزيت اللوز الحلو في القوام والفوائد.' }
        ],
      },
{
        id: 'safflower',
        name: 'زيت القرطم',
        scientific_name: 'Carthamus Tinctorius Seed Oil',
        searchKeywords: ['safflower oil', 'carthamus tinctorius', 'قرطم', 'زيت القرطم', 'العصفر'],
        usageScope: ['skin'],
        functionalCategory: 'مرطب / مطري',
        chemicalType: 'زيت ناقل',
        benefits: { 'ترطيب': 0.85, 'غير كوميدوجينيك': 0.9, 'مضاد للالتهابات': 0.75, 'تنظيم الإفرازات الدهنية': 0.8 },
        synergy: { 'grapeseed': 1.05 },
        negativeSynergy: {},
        warnings: [
          { level: 'info', text: 'خيار ممتاز للبشرة المعرضة لحب الشباب بسبب محتواه العالي من حمض اللينوليك.' }
        ],
      },
{
        id: 'sesame',
        name: 'زيت السمسم',
        scientific_name: 'Sesamum Indicum Seed Oil',
        searchKeywords: ['sesame oil', 'sesamum indicum', 'sesame', 'سمسم', 'زيت السمسم', 'جلجلان'],
        usageScope: ['skin', 'hair'],
        functionalCategory: 'مرطب / مطري',
        chemicalType: 'زيت ناقل',
        benefits: { 'ترطيب': 0.75, 'مضاد أكسدة': 0.7, 'مضاد للميكروبات': 0.65 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'camellia-seed',
        name: 'زيت بذور الكاميليا',
        scientific_name: 'Camellia Oleifera Seed Oil',
        searchKeywords: ['camellia seed oil', 'camellia oleifera', 'tea seed oil', 'كاميليا', 'زيت بذور الكاميليا'],
        usageScope: ['skin', 'hair'],
        functionalCategory: 'مرطب / مطري',
        chemicalType: 'زيت ناقل',
        benefits: { 'ترطيب': 0.85, 'تنعيم وإعطاء لمعان': 0.8, 'مضاد أكسدة': 0.75 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'baobab',
        name: 'زيت الباوباب',
        scientific_name: 'Adansonia Digitata Seed Oil',
        searchKeywords: ['baobab oil', 'adansonia digitata', 'baobab', 'باوباب', 'زيت الباوباب'],
        usageScope: ['skin', 'hair'],
        functionalCategory: 'مرطب / مطري',
        chemicalType: 'زيت ناقل',
        benefits: { 'ترطيب': 0.85, 'مكافحة الشيخوخة': 0.8, 'تغذية': 0.8, 'تقوية الشعر': 0.75 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'red-raspberry-seed',
        name: 'زيت بذور التوت الأحمر',
        scientific_name: 'Rubus Idaeus Seed Oil',
        searchKeywords: ['red raspberry seed oil', 'rubus idaeus', 'raspberry oil', 'توت أحمر', 'زيت بذور التوت الأحمر'],
        usageScope: ['skin'],
        functionalCategory: 'مضاد أكسدة',
        chemicalType: 'زيت ناقل',
        benefits: { 'مضاد أكسدة': 0.9, 'حماية من أضرار الشمس': 0.7, 'مضاد للالتهابات': 0.85 },
        synergy: { 'zinc-oxide': 1.1 },
        negativeSynergy: {},
        warnings: [
          { level: 'caution', text: 'لا يمكن الاعتماد عليه كواقي شمسي منفرد، ولكنه يعزز الحماية عند دمجه مع فلاتر الشمس.' }
        ],
      },
{
        id: 'cranberry-seed',
        name: 'زيت بذور التوت البري',
        scientific_name: 'Vaccinium Macrocarpon Seed Oil',
        searchKeywords: ['cranberry seed oil', 'vaccinium macrocarpon', 'توت بري', 'زيت بذور التوت البري'],
        usageScope: ['skin'],
        functionalCategory: 'مضاد أكسدة',
        chemicalType: 'زيت ناقل',
        benefits: { 'مضاد أكسدة': 0.88, 'ترطيب': 0.8, 'دعم حاجز البشرة': 0.8 },
        synergy: { 'vitamin-e': 1.05 },
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'wheat-germ',
        name: 'زيت جنين القمح',
        scientific_name: 'Triticum Vulgare Germ Oil',
        searchKeywords: ['wheat germ oil', 'triticum vulgare', 'جنين القمح', 'زيت جنين القمح'],
        usageScope: ['skin', 'hair'],
        functionalCategory: 'مرطب / مطري',
        chemicalType: 'زيت ناقل',
        benefits: { 'مضاد أكسدة': 0.9, 'ترطيب': 0.85, 'ملطف ومرمم': 0.8, 'تغذية': 0.82 },
        synergy: { 'vitamin-e': 1.1, 'avocado': 1.05 },
        negativeSynergy: {},
        warnings: [
          { level: 'caution', text: 'زيت ثقيل وقد يسد المسام للبشرة الدهنية.' },
          { level: 'risk', text: 'يجب تجنبه من قبل الأشخاص الذين يعانون من حساسية القمح أو الغلوتين.' }
        ],
      },
{
        id: 'kukui-nut',
        name: 'زيت جوز الكوكوي',
        scientific_name: 'Aleurites Moluccanus Seed Oil',
        searchKeywords: ['kukui nut oil', 'aleurites moluccanus', 'kukui', 'كوكوي', 'زيت الكوكوي'],
        usageScope: ['skin', 'hair'],
        functionalCategory: 'مرطب / مطري',
        chemicalType: 'زيت ناقل',
        benefits: { 'سريع الامتصاص': 0.9, 'ترطيب': 0.85, 'ملطف ومرمم': 0.8 },
        synergy: { 'macadamia': 1.05 },
        negativeSynergy: {},
        warnings: [
          { level: 'risk', text: 'يجب تجنبه من قبل الأشخاص الذين يعانون من حساسية المكسرات.' }
        ],
      },
{
        id: 'meadowfoam-seed',
        name: 'زيت بذور الميدوفوم',
        scientific_name: 'Limnanthes Alba Seed Oil',
        searchKeywords: ['meadowfoam seed oil', 'limnanthes alba', 'meadowfoam', 'ميدوفوم', 'زيت بذور الميدوفوم'],
        usageScope: ['skin', 'hair'],
        functionalCategory: 'مرطب / مطري',
        chemicalType: 'زيت ناقل',
        benefits: { 'مقاومة عالية للأكسدة': 0.9, 'ترطيب': 0.88, 'تنعيم وإعطاء لمعان': 0.85 },
        synergy: {},
        negativeSynergy: {},
        warnings: [
          { level: 'info', text: 'مستقر جدا ويساعد على إطالة عمر الزيوت الأخرى في التركيبة.' }
        ],
      },
{
        id: 'calendula',
        name: 'زيت الكاليندولا (الآذريون)',
        scientific_name: 'Calendula Officinalis Flower Extract',
        searchKeywords: ['calendula oil', 'calendula officinalis', 'marigold oil', 'كاليندولا', 'زيت الكاليندولا', 'آذريون'],
        usageScope: ['skin'],
        functionalCategory: 'ملطف ومرمم',
        chemicalType: 'زيت ناقل (مُنقوع)',
        benefits: { 'ملطف ومرمم': 0.95, 'مضاد للالتهابات': 0.9, 'علاج الجروح الطفيفة': 0.85 },
        synergy: { 'chamomile-essential': 1.05 },
        negativeSynergy: {},
        warnings: [
          { level: 'info', text: 'عادة ما يكون مستخلص زهرة الآذريون منقوعا في زيت ناقل مثل زيت دوار الشمس.' },
          { level: 'risk', text: 'يجب تجنبه من قبل الأشخاص الذين يعانون من حساسية نباتات عائلة الأقحوان (مثل الرجيد والأقحوان).' }
        ],
      },
{
        id: 'cress-seed-oil',
        name: 'زيت حب الرشاد',
        scientific_name: 'Lepidium Sativum Seed Oil',
        searchKeywords: ['cress seed oil', 'lepidium sativum', 'garden cress', 'حب الرشاد', 'زيت حب الرشاد', 'ثفاء'],
        usageScope: ['hair'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'زيت ناقل',
        benefits: { 'تقوية الشعر': 0.85, 'تقليل تساقط الشعر': 0.8, 'تغذية': 0.78 },
        synergy: { 'black-seed': 1.1, 'castor': 1.05 },
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'lupine-oil',
        name: 'زيت الترمس',
        scientific_name: 'Lupinus Albus Seed Oil',
        searchKeywords: ['lupine oil', 'lupinus albus', 'lupin oil', 'ترمس', 'زيت الترمس'],
        usageScope: ['skin'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'زيت ناقل',
        benefits: { 'تفتيح وتوحيد اللون': 0.8, 'مكافحة الشيخوخة': 0.75, 'ترطيب': 0.78 },
        synergy: { 'rosehip': 1.05, 'licorice-extract': 1.1 },
        negativeSynergy: {},
        warnings: [
          { level: 'risk', text: 'يجب تجنبه من قبل الأشخاص الذين يعانون من حساسية الترمس أو البقوليات.' }
        ],
      },
{
        id: 'costus-root-oil',
        name: 'زيت القسط الهندي',
        scientific_name: 'Saussurea Costus Root Oil',
        searchKeywords: ['costus root oil', 'saussurea costus', 'kuth oil', 'قسط هندي', 'زيت القسط الهندي'],
        usageScope: ['skin'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'زيت ناقل',
        benefits: { 'مضاد للميكروبات': 0.9, 'مضاد للالتهابات': 0.88, 'علاج مشاكل البشرة': 0.85 },
        synergy: { 'neem': 1.08, 'black-seed': 1.05 },
        negativeSynergy: {},
        warnings: [
          { level: 'caution', text: 'رائحة قوية جدا ونفاذة.' },
          { level: 'caution', text: 'قد يسبب حساسية لبعض الأشخاص.' }
        ],
      },
{
        id: 'harmal-oil',
        name: 'زيت الحرمل',
        scientific_name: 'Peganum Harmala Seed Oil',
        searchKeywords: ['harmal oil', 'peganum harmala', 'syrian rue oil', 'حرمل', 'زيت الحرمل'],
        usageScope: ['hair'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'زيت ناقل',
        benefits: { 'مضاد للطفيليات': 0.85, 'تقليل تساقط الشعر': 0.8, 'مضاد للميكروبات': 0.75 },
        synergy: { 'neem': 1.1 },
        negativeSynergy: {},
        warnings: [
          { level: 'caution', text: 'زيت قوي يجب استخدامه بحذر شديد ولأغراض علاجية محددة.' },
          { level: 'risk', text: 'لا ينصح باستخدامه داخليا أو بتركيزات عالية دون إشراف متخصص.' }
        ],
      },
{
        id: 'lettuce-seed-oil',
        name: 'زيت بذور الخس',
        scientific_name: 'Lactuca Sativa Seed Oil',
        searchKeywords: ['lettuce seed oil', 'lactuca sativa', 'بذور الخس', 'زيت الخس'],
        usageScope: ['skin', 'hair'],
        functionalCategory: 'مرطب / مطري',
        chemicalType: 'زيت ناقل',
        benefits: { 'ترطيب': 0.85, 'ملطف ومرمم': 0.8, 'تغذية': 0.7 },
        synergy: { 'lavender-essential': 1.08, 'chamomile-essential': 1.1 },
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'garlic-oil',
        name: 'زيت الثوم',
        scientific_name: 'Allium Sativum Bulb Oil',
        searchKeywords: ['garlic oil', 'allium sativum', 'ثوم', 'زيت الثوم'],
        usageScope: ['hair'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'زيت ناقل (مُنقوع)',
        benefits: { 'مضاد للميكروبات': 0.85, 'تحفيز نمو الشعر': 0.75, 'صحة فروة الرأس': 0.8 },
        synergy: { 'rosemary-essential': 1.05, 'castor': 1.08 },
        negativeSynergy: {},
        warnings: [
          { level: 'caution', text: 'له رائحة قوية جدا ونفاذة قد تبقى حتى بعد الغسيل.' },
          { level: 'caution', text: 'قد يسبب تهيجا للفروة الحساسة إذا استخدم بتركيز عالٍ.' }
        ],
      },
{
        id: 'emu',
        name: 'زيت الإيمو',
        scientific_name: 'Dromaius Novaehollandiae Oil',
        searchKeywords: ['emu oil', 'dromaius novaehollandiae', 'emu', 'ايمو', 'زيت الإيمو', 'زيت النعام'],
        usageScope: ['skin', 'hair'],
        functionalCategory: 'مرطب / مطري',
        chemicalType: 'زيت حيواني',
        benefits: { 'اختراق عميق': 0.9, 'مضاد للالتهابات': 0.85, 'ترطيب': 0.8 },
        synergy: {},
        negativeSynergy: {},
        warnings: [
          { level: 'info', text: 'منتج حيواني قد لا يناسب الجميع.' }
        ],
      },
{
        id: 'tea-tree-essential',
        name: 'زيت شجرة الشاي العطري',
        scientific_name: 'Melaleuca Alternifolia Leaf Oil',
        searchKeywords: ['tea tree oil', 'melaleuca alternifolia', 'tea tree', 'شجرة الشاي', 'زيت شجرة الشاي', 'تي تري'],
        usageScope: ['skin', 'hair'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'زيت عطري',
        benefits: { 'مضاد للميكروبات': 0.95, 'مكافحة حب الشباب': 0.9, 'مضاد للفطريات': 0.88, 'تطهير': 0.85 },
        synergy: { 'jojoba': 1.1, 'lavender-essential': 1.05 },
        negativeSynergy: {},
        warnings: [
          { level: 'info', text: 'زيت عطري قوي، يتم استخدامه بتركيزات منخفضة جدا (عادة أقل من 1%) في المنتجات النهائية.' },
          { level: 'caution', text: 'قد يسبب تهيجا للبشرة الحساسة جدا.' }
        ],
      },
{
        id: 'lavender-essential',
        name: 'زيت اللافندر العطري',
        scientific_name: 'Lavandula Angustifolia Oil',
        searchKeywords: ['lavender oil', 'lavandula angustifolia', 'lavender', 'لافندر', 'زيت اللافندر', 'خزامى'],
        usageScope: ['skin', 'hair'],
        functionalCategory: 'ملطف ومرمم',
        chemicalType: 'زيت عطري',
        benefits: { 'ملطف ومرمم': 0.9, 'مضاد للالتهابات': 0.8, 'تحفيز نمو الشعر': 0.7, 'مضاد للميكروبات': 0.75 },
        synergy: { 'sweet-almond': 1.05, 'chamomile-essential': 1.08 },
        negativeSynergy: {},
        warnings: [
          { level: 'info', text: 'يتم استخدامه بتركيزات منخفضة في المنتجات النهائية.' },
          { level: 'risk', text: 'يحتوي على مسببات حساسية طبيعية مثل لينالول وجيرانيول. يجب تجنبه من قبل الأشخاص الذين يعانون من حساسية اللافندر.' }
        ],
      },
{
        id: 'rosemary-essential',
        name: 'زيت إكليل الجبل العطري',
        scientific_name: 'Rosmarinus Officinalis Leaf Oil',
        searchKeywords: ['rosemary oil', 'rosmarinus officinalis', 'rosemary', 'إكليل الجبل', 'زيت إكليل الجبل', 'روزماري'],
        usageScope: ['hair'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'زيت عطري',
        benefits: { 'تحفيز نمو الشعر': 0.92, 'تقوية الشعر': 0.85, 'صحة فروة الرأس': 0.8, 'مضاد للقشرة': 0.75 },
        synergy: { 'castor': 1.15, 'pumpkin-seed': 1.1 },
        negativeSynergy: {},
        warnings: [
          { level: 'info', text: 'يتم استخدامه بتركيزات منخفضة في المنتجات النهائية.' },
          { level: 'risk', text: 'لا ينصح به أثناء الحمل، أو لمن يعانون من ضغط دم مرتفع أو الصرع دون استشارة طبيب.' }
        ],
      },
{
        id: 'peppermint-essential',
        name: 'زيت النعناع الفلفلي العطري',
        scientific_name: 'Mentha Piperita Oil',
        searchKeywords: ['peppermint oil', 'mentha piperita', 'peppermint', 'نعناع', 'زيت النعناع الفلفلي'],
        usageScope: ['skin', 'hair'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'زيت عطري',
        benefits: { 'تنشيط فروة الرأس': 0.9, 'تخفيف الحكة': 0.85, 'مضاد للميكروبات': 0.75, 'إحساس بالبرودة': 0.92 },
        synergy: { 'eucalyptus-essential': 1.05 },
        negativeSynergy: {},
        warnings: [
          { level: 'info', text: 'زيت عطري قوي جدا، يستخدم بتركيز منخفض جدا لإعطاء إحساس بالبرودة والانتعاش.' },
          { level: 'risk', text: 'لا ينصح باستخدامه للأطفال الصغار أو أثناء الحمل.' }
        ],
      },
{
        id: 'frankincense-essential',
        name: 'زيت اللبان العطري',
        scientific_name: 'Boswellia Carterii Oil',
        searchKeywords: ['frankincense oil', 'boswellia carterii', 'olibanum', 'frankincense', 'لبان', 'زيت اللبان'],
        usageScope: ['skin'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'زيت عطري',
        benefits: { 'مكافحة الشيخوخة': 0.88, 'ملطف ومرمم': 0.8, 'مضاد للالتهابات': 0.8, 'توحيد لون البشرة': 0.75 },
        synergy: { 'rosehip': 1.08, 'myrrh-essential': 1.05 },
        negativeSynergy: {},
        warnings: [
          { level: 'info', text: 'يتم استخدامه بتركيزات منخفضة في المنتجات النهائية.' }
        ],
      },
{
        id: 'chamomile-essential',
        name: 'زيت البابونج العطري',
        scientific_name: 'Chamomilla Recutita (German) / Anthemis Nobilis (Roman) Flower Oil',
        searchKeywords: ['chamomile oil', 'matricaria recutita', 'anthemis nobilis', 'german chamomile', 'roman chamomile', 'بابونج', 'زيت البابونج'],
        usageScope: ['skin'],
        functionalCategory: 'ملطف ومرمم',
        chemicalType: 'زيت عطري',
        benefits: { 'ملطف ومرمم': 0.95, 'مضاد للالتهابات': 0.92, 'تقليل الاحمرار': 0.9 },
        synergy: { 'lavender-essential': 1.08, 'blue-tansy-essential': 1.1 },
        negativeSynergy: {},
        warnings: [
          { level: 'info', text: 'البابونج الألماني (الأزرق) أقوى كمضاد للالتهابات.' },
          { level: 'risk', text: 'يجب تجنبه من قبل الأشخاص الذين يعانون من حساسية نباتات عائلة الأقحوان.' }
        ],
      },
{
        id: 'lemon-essential',
        name: 'زيت الليمون العطري',
        scientific_name: 'Citrus Limon Peel Oil',
        searchKeywords: ['lemon oil', 'citrus limon oil', 'زيت الليمون'],
        usageScope: ['skin'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'زيت عطري',
        benefits: { 'تفتيح وتوحيد اللون': 0.8, 'تطهير': 0.75, 'مضاد للميكروبات': 0.7 },
        synergy: { 'grapeseed': 1.05 },
        negativeSynergy: {},
        warnings: [
          { level: 'risk', text: 'يسبب حساسية للضوء (Phototoxic). يجب تجنب التعرض المباشر لأشعة الشمس لمدة 12-24 ساعة بعد استخدام المنتجات التي تترك على الجلد وتحتوي عليه.' },
          { level: 'caution', text: 'قد يكون مهيجا للبشرة الحساسة جدا.' }
        ],
      },
{
        id: 'orange-essential',
        name: 'زيت البرتقال العطري',
        scientific_name: 'Citrus Sinensis Peel Oil',
        searchKeywords: ['orange oil', 'citrus sinensis', 'sweet orange oil', 'برتقال', 'زيت البرتقال'],
        usageScope: ['skin'],
        functionalCategory: 'عطر',
        chemicalType: 'زيت عطري',
        benefits: { 'تفتيح وتوحيد اللون': 0.75, 'مضاد أكسدة': 0.7, 'رائحة منعشة': 0.95 },
        synergy: {},
        negativeSynergy: {},
        warnings: [
          { level: 'risk', text: 'قد يسبب حساسية للضوء. يفضل تجنب التعرض للشمس بعد استخدامه في المنتجات التي تترك على الجلد.' }
        ],
      },
{
        id: 'eucalyptus-essential',
        name: 'زيت الأوكالبتوس العطري',
        scientific_name: 'Eucalyptus Globulus Leaf Oil',
        searchKeywords: ['eucalyptus oil', 'eucalyptus globulus', 'eucalyptus', 'أوكالبتوس', 'زيت الأوكالبتوس', 'كاليتوس'],
        usageScope: ['skin', 'hair'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'زيت عطري',
        benefits: { 'تطهير': 0.85, 'صحة فروة الرأس': 0.8, 'مضاد للميكروبات': 0.8, 'إحساس بالبرودة': 0.88 },
        synergy: { 'peppermint-essential': 1.05 },
        negativeSynergy: {},
        warnings: [
          { level: 'info', text: 'زيت قوي، يستخدم بتركيزات منخفضة.' },
          { level: 'risk', text: 'لا ينصح باستخدامه للأطفال الصغار أو بالقرب من الوجه، أو من قبل مرضى الربو.' }
        ],
      },
{
        id: 'geranium-essential',
        name: 'زيت الجيرانيوم العطري',
        scientific_name: 'Pelargonium Graveolens Oil',
        searchKeywords: ['geranium oil', 'pelargonium graveolens', 'geranium', 'جيرانيوم', 'زيت الجيرانيوم', 'عطرشة'],
        usageScope: ['skin'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'زيت عطري',
        benefits: { 'تنظيم الإفرازات الدهنية': 0.85, 'مكافحة الشيخوخة': 0.8, 'مضاد للالتهابات': 0.75 },
        synergy: { 'lavender-essential': 1.05 },
        negativeSynergy: {},
        warnings: [
          { level: 'info', text: 'يتم استخدامه بتركيزات منخفضة في المنتجات النهائية.' }
        ],
      },
{
        id: 'clary-sage-essential',
        name: 'زيت المريمية العطري',
        scientific_name: 'Salvia Sclarea Oil',
        searchKeywords: ['clary sage oil', 'salvia sclarea', 'clary sage', 'مريمية', 'زيت المريمية'],
        usageScope: ['skin', 'hair'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'زيت عطري',
        benefits: { 'تنظيم الإفرازات الدهنية': 0.8, 'تقليل تساقط الشعر': 0.75, 'ملطف ومرمم': 0.7 },
        synergy: { 'lavender-essential': 1.05 },
        negativeSynergy: {},
        warnings: [
          { level: 'info', text: 'يتم استخدامه بتركيزات منخفضة في المنتجات النهائية.' },
          { level: 'risk', text: 'لا يستخدم أثناء الحمل أو الرضاعة. لا يستخدم مع الكحول لأنه قد يزيد من تأثيره.' }
        ],
      },
{
        id: 'cedarwood-essential',
        name: 'زيت خشب الأرز العطري',
        scientific_name: 'Cedrus Atlantica Wood Oil',
        searchKeywords: ['cedarwood oil', 'cedrus atlantica', 'cedarwood', 'خشب الأرز', 'زيت خشب الأرز'],
        usageScope: ['hair'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'زيت عطري',
        benefits: { 'تحفيز نمو الشعر': 0.8, 'علاج القشرة': 0.78, 'تنظيم الإفرازات الدهنية': 0.75 },
        synergy: { 'rosemary-essential': 1.05 },
        negativeSynergy: {},
        warnings: [
          { level: 'info', text: 'يتم استخدامه بتركيزات منخفضة في المنتجات النهائية.' }
        ],
      },
{
        id: 'ylang-ylang-essential',
        name: 'زيت الإيلنغ العطري',
        scientific_name: 'Cananga Odorata Flower Oil',
        searchKeywords: ['ylang ylang oil', 'cananga odorata', 'ylang ylang', 'إيلنغ إيلنغ', 'يلانغ يلانغ'],
        usageScope: ['hair'],
        functionalCategory: 'عطر',
        chemicalType: 'زيت عطري',
        benefits: { 'تنعيم وإعطاء لمعان': 0.8, 'تنظيم الإفرازات الدهنية': 0.75, 'رائحة عطرية': 0.95 },
        synergy: { 'jojoba': 1.05 },
        negativeSynergy: {},
        warnings: [
          { level: 'caution', text: 'رائحته قوية جدا، قد تسبب الصداع للبعض.' }
        ],
      },
{
        id: 'murumuru-butter',
        name: 'زبدة المورومورو',
        scientific_name: 'Astrocaryum Murumuru Seed Butter',
        searchKeywords: ['murumuru butter', 'astrocaryum murumuru', 'مورومورو', 'زبدة المورومورو'],
        usageScope: ['hair', 'skin'],
        functionalCategory: 'مرطب / مطري',
        chemicalType: 'زبدة',
        benefits: { 'ترطيب': 0.92, 'دعم حاجز البشرة': 0.9, 'تنعيم وإعطاء لمعان': 0.88, 'تحسين مرونة الشعر': 0.85 },
        synergy: { 'shea-butter': 1.05 },
        negativeSynergy: {},
        warnings: [
          { level: 'info', text: 'بديل ممتاز للسيليكون في منتجات الشعر، حيث يغلف الشعر بطبقة خفيفة وواقية دون إثقاله.' }
        ],
      },
{
        id: 'cupuacu-butter',
        name: 'زبدة الكوبواسو',
        scientific_name: 'Theobroma Grandiflorum Seed Butter',
        searchKeywords: ['cupuacu butter', 'theobroma grandiflorum', 'كوبواسو', 'زبدة الكوبواسو'],
        usageScope: ['skin', 'hair'],
        functionalCategory: 'مرطب / مطري',
        chemicalType: 'زبدة',
        benefits: { 'قدرة فائقة على حبس الماء': 0.98, 'ترطيب': 0.95, 'تحسين مرونة الجلد': 0.9, 'دعم حاجز البشرة': 0.92 },
        synergy: {},
        negativeSynergy: {},
        warnings: [
          { level: 'info', text: 'يمكنها امتصاص ما يصل إلى 4 أضعاف وزنها من الماء، مما يجعلها مرطبا فائقا للبشرة والشعر شديدي الجفاف.' }
        ],
      },
{
        id: 'kokum-butter',
        name: 'زبدة الكوكوم',
        scientific_name: 'Garcinia Indica Seed Butter',
        searchKeywords: ['kokum butter', 'garcinia indica', 'كوكوم', 'زبدة الكوكوم'],
        usageScope: ['skin'],
        functionalCategory: 'مرطب / مطري',
        chemicalType: 'زبدة',
        benefits: { 'غير كوميدوجينيك': 0.9, 'ترطيب': 0.88, 'ملطف ومرمم': 0.85, 'صلابة عالية': 0.8 },
        synergy: {},
        negativeSynergy: {},
        warnings: [
          { level: 'info', text: 'تتميز بقوامها الصلب الذي يذوب عند ملامسة الجلد، وهي خيار ممتاز للبشرة المعرضة لحب الشباب لأنها لا تسد المسام.' }
        ],
      },
{
        id: 'buriti-oil',
        name: 'زيت البوريتي',
        scientific_name: 'Mauritia Flexuosa Fruit Oil',
        searchKeywords: ['buriti oil', 'mauritia flexuosa', 'بوريتي', 'زيت البوريتي'],
        usageScope: ['skin', 'hair'],
        functionalCategory: 'مضاد أكسدة',
        chemicalType: 'زيت ناقل',
        benefits: { 'مضاد أكسدة': 0.95, 'حماية من أضرار الشمس': 0.8, 'مكافحة الشيخوخة': 0.88, 'ترطيب': 0.85 },
        synergy: {},
        negativeSynergy: {},
        warnings: [
          { level: 'info', text: 'أغنى مصدر طبيعي للبيتا كاروتين (برو-فيتامين A)، وهو مضاد أكسدة قوي جدا.' },
          { level: 'caution', text: 'لونه أحمر غامق وقد يصبغ البشرة مؤقتا.' }
        ],
      },
{
        id: 'acai-oil',
        name: 'زيت الآساي',
        scientific_name: 'Euterpe Oleracea Fruit Oil',
        searchKeywords: ['acai oil', 'euterpe oleracea', 'acai berry oil', 'آساي', 'زيت الآساي'],
        usageScope: ['skin'],
        functionalCategory: 'مضاد أكسدة',
        chemicalType: 'زيت ناقل',
        benefits: { 'مضاد أكسدة': 0.92, 'مكافحة الشيخوخة': 0.85, 'مضاد للالتهابات': 0.8, 'ترطيب': 0.8 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'brazil-nut-oil',
        name: 'زيت الجوز البرازيلي',
        scientific_name: 'Bertholletia Excelsa Seed Oil',
        searchKeywords: ['brazil nut oil', 'bertholletia excelsa', 'الجوز البرازيلي', 'زيت الجوز البرازيلي'],
        usageScope: ['skin', 'hair'],
        functionalCategory: 'مرطب / مطري',
        chemicalType: 'زيت ناقل',
        benefits: { 'مصدر غني بالسيلينيوم': 0.9, 'مضاد أكسدة': 0.88, 'ترطيب': 0.9, 'تنعيم وإعطاء لمعان': 0.85 },
        synergy: {},
        negativeSynergy: {},
        warnings: [
          { level: 'risk', text: 'يجب تجنبه من قبل الأشخاص الذين يعانون من حساسية المكسرات.' }
        ],
      },
{
        id: 'andiroba-oil',
        name: 'زيت الأنديروبا',
        scientific_name: 'Carapa Guaianensis Seed Oil',
        searchKeywords: ['andiroba oil', 'carapa guaianensis', 'أنديروبا', 'زيت الأنديروبا'],
        usageScope: ['skin'],
        functionalCategory: 'ملطف ومرمم',
        chemicalType: 'زيت ناقل',
        benefits: { 'مضاد للالتهابات': 0.9, 'تسكين الألم الموضعي': 0.85, 'ملطف ومرمم': 0.8, 'طارد للحشرات': 0.75 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'pequi-oil',
        name: 'زيت البيكي',
        scientific_name: 'Caryocar Brasiliense Fruit Oil',
        searchKeywords: ['pequi oil', 'caryocar brasiliense', 'بيكي', 'زيت البيكي'],
        usageScope: ['hair'],
        functionalCategory: 'مكيف للشعر',
        chemicalType: 'زيت ناقل',
        benefits: { 'تحديد تموجات الشعر': 0.9, 'تقليل الهيشان': 0.88, 'تنعيم وإعطاء لمعان': 0.85 },
        synergy: {},
        negativeSynergy: {},
        warnings: [
          { level: 'info', text: 'زيت شائع في البرازيل، ممتاز للشعر المجعد لتعزيز التحديد والتحكم في الهيشان.' }
        ],
      },
{
        id: 'mongongo-oil',
        name: 'زيت المونجونجو',
        scientific_name: 'Schinziophyton Rautanenii Kernel Oil',
        searchKeywords: ['mongongo oil', 'manketti oil', 'schinziophyton rautanenii', 'مونجونجو', 'زيت المونجونجو'],
        usageScope: ['hair', 'skin'],
        functionalCategory: 'حماية من الشمس',
        chemicalType: 'زيت ناقل',
        benefits: { 'حماية من أضرار الشمس': 0.8, 'ترطيب': 0.85, 'حماية من الحرارة': 0.82 },
        synergy: {},
        negativeSynergy: {},
        warnings: [
          { level: 'info', text: 'يكون طبقة واقية على الشعر والجلد عند تعرضه للأشعة فوق البنفسجية، مما يوفر حماية طبيعية.' }
        ],
      },
{
        id: 'kalahari-melon-oil',
        name: 'زيت بطيخ كالاهاري',
        scientific_name: 'Citrullus Lanatus (Watermelon) Seed Oil',
        searchKeywords: ['kalahari melon seed oil', 'watermelon seed oil', 'citrullus lanatus', 'بطيخ كالاهاري', 'زيت بذور البطيخ'],
        usageScope: ['skin'],
        functionalCategory: 'مرطب / مطري',
        chemicalType: 'زيت ناقل',
        benefits: { 'تنظيم الإفرازات الدهنية': 0.88, 'ترطيب خفيف': 0.9, 'سريع الامتصاص': 0.92, 'غير كوميدوجينيك': 0.9 },
        synergy: {},
        negativeSynergy: {},
        warnings: [
          { level: 'info', text: 'زيت خفيف جدا وممتاز للبشرة الدهنية والمعرضة لحب الشباب.' }
        ],
      },
{
        id: 'cherry-kernel-oil',
        name: 'زيت نواة الكرز',
        scientific_name: 'Prunus Avium Seed Oil',
        searchKeywords: ['cherry kernel oil', 'prunus avium', 'نواة الكرز', 'زيت نواة الكرز'],
        usageScope: ['skin'],
        functionalCategory: 'مرطب / مطري',
        chemicalType: 'زيت ناقل',
        benefits: { 'ترطيب': 0.85, 'مضاد أكسدة': 0.8, 'ملطف ومرمم': 0.75 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'plum-kernel-oil',
        name: 'زيت نواة البرقوق',
        scientific_name: 'Prunus Domestica Seed Oil',
        searchKeywords: ['plum kernel oil', 'prunus domestica', 'نواة البرقوق', 'زيت نواة البرقوق'],
        usageScope: ['skin'],
        functionalCategory: 'مرطب / مطري',
        chemicalType: 'زيت ناقل',
        benefits: { 'مضاد أكسدة': 0.85, 'ترطيب': 0.9, 'دعم حاجز البشرة': 0.88 },
        synergy: {},
        negativeSynergy: {},
        warnings: [
          { level: 'info', text: 'له رائحة لوزية طبيعية جميلة.' }
        ],
      },
{
        id: 'chia-seed-oil',
        name: 'زيت بذور الشيا',
        scientific_name: 'Salvia Hispanica Seed Oil',
        searchKeywords: ['chia seed oil', 'salvia hispanica', 'بذور الشيا', 'زيت بذور الشيا'],
        usageScope: ['skin'],
        functionalCategory: 'ملطف ومرمم',
        chemicalType: 'زيت ناقل',
        benefits: { 'مصدر غني بالأوميغا 3': 0.95, 'مضاد للالتهابات': 0.9, 'ترطيب': 0.88, 'دعم حاجز البشرة': 0.92 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'hazelnut-oil',
        name: 'زيت البندق',
        scientific_name: 'Corylus Avellana Seed Oil',
        searchKeywords: ['hazelnut oil', 'corylus avellana', 'بندق', 'زيت البندق'],
        usageScope: ['skin'],
        functionalCategory: 'مرطب / مطري',
        chemicalType: 'زيت ناقل',
        benefits: { 'مادة قابضة طبيعية': 0.85, 'تنظيم الإفرازات الدهنية': 0.8, 'ترطيب خفيف': 0.82 },
        synergy: {},
        negativeSynergy: {},
        warnings: [
          { level: 'risk', text: 'يجب تجنبه من قبل الأشخاص الذين يعانون من حساسية المكسرات.' }
        ],
      },
{
        id: 'walnut-oil',
        name: 'زيت الجوز',
        scientific_name: 'Juglans Regia Seed Oil',
        searchKeywords: ['walnut oil', 'juglans regia', 'جوز', 'زيت الجوز', 'عين الجمل'],
        usageScope: ['skin'],
        functionalCategory: 'ملطف ومرمم',
        chemicalType: 'زيت ناقل',
        benefits: { 'مضاد للالتهابات': 0.8, 'مضاد أكسدة': 0.85, 'ترطيب': 0.75 },
        synergy: {},
        negativeSynergy: {},
        warnings: [
          { level: 'risk', text: 'يجب تجنبه من قبل الأشخاص الذين يعانون من حساسية المكسرات.' }
        ],
      },
{
        id: 'kiwi-seed-oil',
        name: 'زيت بذور الكيوي',
        scientific_name: 'Actinidia Chinensis Seed Oil',
        searchKeywords: ['kiwi seed oil', 'actinidia chinensis', 'كيوي', 'زيت بذور الكيوي'],
        usageScope: ['skin'],
        functionalCategory: 'مضاد أكسدة',
        chemicalType: 'زيت ناقل',
        benefits: { 'مصدر غني بالأوميغا 3': 0.9, 'مضاد أكسدة': 0.85, 'مكافحة الشيخوخة': 0.8 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'poppy-seed-oil',
        name: 'زيت بذور الخشخاش',
        scientific_name: 'Papaver Somniferum Seed Oil',
        searchKeywords: ['poppy seed oil', 'papaver somniferum', 'خشخاش', 'زيت بذور الخشخاش'],
        usageScope: ['skin'],
        functionalCategory: 'مرطب / مطري',
        chemicalType: 'زيت ناقل',
        benefits: { 'ترطيب': 0.8, 'دعم حاجز البشرة': 0.85, 'تنعيم البشرة': 0.75 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'tomato-seed-oil',
        name: 'زيت بذور الطماطم',
        scientific_name: 'Solanum Lycopersicum Seed Oil',
        searchKeywords: ['tomato seed oil', 'solanum lycopersicum', 'طماطم', 'زيت بذور الطماطم'],
        usageScope: ['skin'],
        functionalCategory: 'مضاد أكسدة',
        chemicalType: 'زيت ناقل',
        benefits: { 'غني بالليكوبين': 0.9, 'مضاد أكسدة': 0.92, 'حماية من أضرار الشمس': 0.75 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'cucumber-seed-oil',
        name: 'زيت بذور الخيار',
        scientific_name: 'Cucumis Sativus Seed Oil',
        searchKeywords: ['cucumber seed oil', 'cucumis sativus', 'خيار', 'زيت بذور الخيار'],
        usageScope: ['skin'],
        functionalCategory: 'ملطف ومرمم',
        chemicalType: 'زيت ناقل',
        benefits: { 'ملطف ومرمم': 0.88, 'ترطيب': 0.85, 'إحساس بالبرودة': 0.8 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'blueberry-seed-oil',
        name: 'زيت بذور التوت الأزرق',
        scientific_name: 'Vaccinium Corymbosum Seed Oil',
        searchKeywords: ['blueberry seed oil', 'vaccinium corymbosum', 'توت أزرق', 'زيت بذور التوت الأزرق'],
        usageScope: ['skin'],
        functionalCategory: 'مضاد أكسدة',
        chemicalType: 'زيت ناقل',
        benefits: { 'مضاد أكسدة': 0.9, 'حماية من الضوء الأزرق': 0.8, 'مكافحة الشيخوخة': 0.85 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'oat-kernel-oil',
        name: 'زيت نواة الشوفان',
        scientific_name: 'Avena Sativa Kernel Oil',
        searchKeywords: ['oat kernel oil', 'avena sativa oil', 'oat oil', 'شوفان', 'زيت نواة الشوفان'],
        usageScope: ['skin'],
        functionalCategory: 'ملطف ومرمم',
        chemicalType: 'زيت ناقل',
        benefits: { 'ملطف ومرمم': 0.95, 'دعم حاجز البشرة': 0.92, 'مضاد للالتهابات': 0.9, 'غني بالسيراميد': 0.88 },
        synergy: { 'beta-glucan': 1.05 },
        negativeSynergy: {},
        warnings: [
          { level: 'info', text: 'ممتاز للبشرة الحساسة جدا والمعرضة للأكزيما.' }
        ],
      },
{
        id: 'coffee-bean-oil',
        name: 'زيت حبوب البن',
        scientific_name: 'Coffea Arabica Seed Oil',
        searchKeywords: ['coffee bean oil', 'coffea arabica', 'green coffee oil', 'بن', 'زيت حبوب البن', 'زيت القهوة'],
        usageScope: ['skin'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'زيت ناقل',
        benefits: { 'مضاد أكسدة': 0.9, 'تحفيز الدورة الدموية': 0.85, 'مكافحة الشيخوخة': 0.8, 'تقليل الانتفاخ': 0.82 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'chaulmoogra-oil',
        name: 'زيت الشولموجرا',
        scientific_name: 'Hydnocarpus Wightianus Seed Oil',
        searchKeywords: ['chaulmoogra oil', 'hydnocarpus wightianus', 'شولموجرا', 'زيت الشولموجرا'],
        usageScope: ['skin'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'زيت ناقل',
        benefits: { 'مضاد للميكروبات': 0.88, 'علاج مشاكل البشرة': 0.85, 'مضاد للالتهابات': 0.8 },
        synergy: {},
        negativeSynergy: {},
        warnings: [
          { level: 'info', text: 'يستخدم تقليديا في طب الأيورفيدا لعلاج الأمراض الجلدية المزمنة.' }
        ],
      },
{
        id: 'perilla-seed-oil',
        name: 'زيت بذور البريلا',
        scientific_name: 'Perilla Ocymoides Seed Oil',
        searchKeywords: ['perilla seed oil', 'perilla ocymoides', 'بريلا', 'زيت بذور البريلا'],
        usageScope: ['skin'],
        functionalCategory: 'ملطف ومرمم',
        chemicalType: 'زيت ناقل',
        benefits: { 'مصدر غني بالأوميغا 3': 0.92, 'مضاد للالتهابات': 0.9, 'ملطف ومرمم': 0.88 },
        synergy: {},
        negativeSynergy: {},
        warnings: [
          { level: 'info', text: 'زيت جاف (سريع الامتصاص)، ممتاز للبشرة الحساسة والمتهيجة.' }
        ],
      },
{
        id: 'elemi-essential',
        name: 'زيت الإليمي العطري',
        scientific_name: 'Canarium Luzonicum Gum Oil',
        searchKeywords: ['elemi oil', 'canarium luzonicum', 'إليمي', 'زيت الإليمي'],
        usageScope: ['skin'],
        functionalCategory: 'ملطف ومرمم',
        chemicalType: 'زيت عطري',
        benefits: { 'ملطف ومرمم': 0.85, 'مكافحة الشيخوخة': 0.8, 'تجديد الجلد': 0.82 },
        synergy: { 'frankincense-essential': 1.05 },
        negativeSynergy: {},
        warnings: [
          { level: 'info', text: 'من نفس عائلة اللبان والمر، له رائحة ليمونية راتنجية.' }
        ],
      },
{
        id: 'galbanum-essential',
        name: 'زيت الجلبانوم العطري',
        scientific_name: 'Ferula Galbaniflua Resin Oil',
        searchKeywords: ['galbanum oil', 'ferula galbaniflua', 'جلبانوم', 'زيت الجلبانوم', 'قنة'],
        usageScope: ['skin'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'زيت عطري',
        benefits: { 'تجديد الجلد': 0.8, 'علاج الندوب': 0.75, 'مضاد للالتهابات': 0.7 },
        synergy: {},
        negativeSynergy: {},
        warnings: [
          { level: 'info', text: 'له رائحة خضراء ترابية قوية، ويستخدم في العطور الفاخرة.' }
        ],
      },
{
        id: 'palo-santo-essential',
        name: 'زيت بالو سانتو العطري',
        scientific_name: 'Bursera Graveolens Wood Oil',
        searchKeywords: ['palo santo oil', 'bursera graveolens', 'بالو سانتو', 'الخشب المقدس'],
        usageScope: ['skin'],
        functionalCategory: 'ملطف ومرمم',
        chemicalType: 'زيت عطري',
        benefits: { 'مضاد للالتهابات': 0.8, 'مهدئ': 0.85, 'مضاد أكسدة': 0.75 },
        synergy: {},
        negativeSynergy: {},
        warnings: [
          { level: 'info', text: 'يجب التأكد من أنه من مصادر مستدامة.' }
        ],
      },
{
        id: 'hinoki-essential',
        name: 'زيت الهينوكي العطري',
        scientific_name: 'Chamaecyparis Obtusa Wood Oil',
        searchKeywords: ['hinoki oil', 'chamaecyparis obtusa', 'japanese cypress', 'هينوكي', 'زيت السرو الياباني'],
        usageScope: ['skin', 'hair'],
        functionalCategory: 'ملطف ومرمم',
        chemicalType: 'زيت عطري',
        benefits: { 'مضاد للميكروبات': 0.8, 'مهدئ': 0.85, 'صحة فروة الرأس': 0.75 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'davana-essential',
        name: 'زيت الدافانا العطري',
        scientific_name: 'Artemisia Pallens Flower Oil',
        searchKeywords: ['davana oil', 'artemisia pallens', 'دافانا', 'زيت الدافانا'],
        usageScope: ['skin'],
        functionalCategory: 'عطر',
        chemicalType: 'زيت عطري',
        benefits: { 'رائحة فريدة': 0.9, 'مضاد للميكروبات': 0.7 },
        synergy: {},
        negativeSynergy: {},
        warnings: [
          { level: 'info', text: 'له رائحة فاكهية معقدة تتغير على جلد كل شخص.' }
        ],
      },
{
        id: 'osmanthus-essential',
        name: 'زيت الأوسمانثوس العطري',
        scientific_name: 'Osmanthus Fragrans Flower Extract',
        searchKeywords: ['osmanthus absolute', 'osmanthus fragrans', 'أوسمانثوس', 'زيت الأوسمانثوس'],
        usageScope: ['skin'],
        functionalCategory: 'عطر',
        chemicalType: 'زيت عطري (مطلق)',
        benefits: { 'رائحة فاخرة': 0.95, 'مضاد أكسدة': 0.7 },
        synergy: {},
        negativeSynergy: {},
        warnings: [
          { level: 'info', text: 'مكلف جدا، يستخدم في العطور الفاخرة.' }
        ],
      },
{
        id: 'petitgrain-essential',
        name: 'زيت البيتيتغرين العطري',
        scientific_name: 'Citrus Aurantium Amara Leaf/Twig Oil',
        searchKeywords: ['petitgrain oil', 'citrus aurantium', 'بيتيتغرين', 'زيت أوراق البرتقال المر'],
        usageScope: ['skin'],
        functionalCategory: 'ملطف ومرمم',
        chemicalType: 'زيت عطري',
        benefits: { 'تنظيم الإفرازات الدهنية': 0.8, 'مهدئ': 0.85, 'مضاد للميكروبات': 0.75 },
        synergy: {},
        negativeSynergy: {},
        warnings: [
          { level: 'info', text: 'يُستخرج من أوراق وأغصان شجرة البرتقال المر، وهو أقل حساسية للضوء من زيوت الحمضيات القشرية.' }
        ],
      },
{
        id: 'green-mandarin-essential',
        name: 'زيت المندرين الأخضر العطري',
        scientific_name: 'Citrus Nobilis Peel Oil',
        searchKeywords: ['green mandarin oil', 'citrus nobilis', 'ماندرين أخضر', 'يوسفي أخضر', 'زيت المندرين الأخضر'],
        usageScope: ['skin'],
        functionalCategory: 'عطر',
        chemicalType: 'زيت عطري',
        benefits: { 'رائحة منعشة': 0.9, 'مضاد للميكروبات': 0.7 },
        synergy: {},
        negativeSynergy: {},
        warnings: [
          { level: 'caution', text: 'قد يسبب حساسية للضوء.' }
        ],
      },
{
        id: 'yuzu-essential',
        name: 'زيت اليوزو العطري',
        scientific_name: 'Citrus Junos Peel Oil',
        searchKeywords: ['yuzu oil', 'citrus junos', 'يوزو', 'زيت اليوزو'],
        usageScope: ['skin'],
        functionalCategory: 'عطر',
        chemicalType: 'زيت عطري',
        benefits: { 'تفتيح وتوحيد اللون': 0.75, 'مضاد أكسدة': 0.8, 'رائحة معقدة': 0.92 },
        synergy: {},
        negativeSynergy: {},
        warnings: [
          { level: 'caution', text: 'قد يسبب حساسية للضوء.' }
        ],
      },
{
        id: 'spearmint-essential',
        name: 'زيت النعناع السنبلي العطري',
        scientific_name: 'Mentha Spicata Herb Oil',
        searchKeywords: ['spearmint oil', 'mentha spicata', 'spearmint', 'نعناع سنبلي', 'زيت النعناع'],
        usageScope: ['skin', 'hair'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'زيت عطري',
        benefits: { 'مضاد للالتهابات': 0.8, 'مضاد للميكروبات': 0.75, 'رائحة منعشة': 0.9 },
        synergy: {},
        negativeSynergy: {},
        warnings: [
          { level: 'info', text: 'ألطف من زيت النعناع الفلفلي لأنه يحتوي على كمية أقل بكثير من المنثول.' }
        ],
      },
{
        id: 'basil-essential',
        name: 'زيت الريحان العطري',
        scientific_name: 'Ocimum Basilicum Oil',
        searchKeywords: ['basil oil', 'ocimum basilicum', 'sweet basil', 'ريحان', 'زيت الريحان', 'حبق'],
        usageScope: ['skin', 'hair'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'زيت عطري',
        benefits: { 'مضاد للالتهابات': 0.8, 'تنشيط فروة الرأس': 0.75, 'مضاد أكسدة': 0.7 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'marjoram-essential',
        name: 'زيت المردقوش العطري',
        scientific_name: 'Origanum Majorana Leaf Oil',
        searchKeywords: ['marjoram oil', 'origanum majorana', 'sweet marjoram', 'مردقوش', 'زيت المردقوش'],
        usageScope: ['skin'],
        functionalCategory: 'ملطف ومرمم',
        chemicalType: 'زيت عطري',
        benefits: { 'مهدئ': 0.85, 'تخفيف آلام العضلات': 0.8, 'مضاد للميكروبات': 0.7 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'fennel-essential',
        name: 'زيت الشمر العطري',
        scientific_name: 'Foeniculum Vulgare Oil',
        searchKeywords: ['fennel oil', 'foeniculum vulgare', 'شمر', 'زيت الشمر', 'بسباس'],
        usageScope: ['skin'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'زيت عطري',
        benefits: { 'تطهير': 0.8, 'توحيد لون البشرة': 0.75, 'مضاد للالتهابات': 0.7 },
        synergy: {},
        negativeSynergy: {},
        warnings: [
          { level: 'risk', text: 'لا ينصح به أثناء الحمل أو لمن يعانون من الصرع.' }
        ],
      },
{
        id: 'spikenard-essential',
        name: 'زيت السنبل العطري',
        scientific_name: 'Nardostachys Jatamansi Oil',
        searchKeywords: ['spikenard oil', 'nardostachys jatamansi', 'jatamansi', 'سنبل', 'زيت السنبل'],
        usageScope: ['skin'],
        functionalCategory: 'ملطف ومرمم',
        chemicalType: 'زيت عطري',
        benefits: { 'مهدئ قوي': 0.9, 'مضاد للالتهابات': 0.85, 'تجديد الجلد': 0.8 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'niaouli-essential',
        name: 'زيت النياولي العطري',
        scientific_name: 'Melaleuca Viridiflora Leaf Oil',
        searchKeywords: ['niaouli oil', 'melaleuca viridiflora', 'niaouli', 'نياولي'],
        usageScope: ['skin'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'زيت عطري',
        benefits: { 'مضاد للميكروبات': 0.88, 'تطهير': 0.85, 'مضاد للفيروسات': 0.75 },
        synergy: { 'eucalyptus-essential': 1.05 },
        negativeSynergy: {},
        warnings: [
          { level: 'info', text: 'من نفس عائلة شجرة الشاي والأوكالبتوس، ولكنه ألطف على الجلد.' }
        ],
      },
{
        id: 'ravensara-essential',
        name: 'زيت الرافنسارا العطري',
        scientific_name: 'Ravensara Aromatica Leaf Oil',
        searchKeywords: ['ravensara oil', 'ravensara aromatica', 'ravensara', 'رافنسارا'],
        usageScope: ['skin'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'زيت عطري',
        benefits: { 'مضاد للميكروبات': 0.85, 'مضاد للفيروسات': 0.8, 'منشط': 0.75 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'lime-essential',
        name: 'زيت اللايم العطري',
        scientific_name: 'Citrus Aurantifolia Oil',
        searchKeywords: ['lime oil', 'citrus aurantifolia', 'lime', 'لايم', 'زيت اللايم', 'ليمون أخضر'],
        usageScope: ['skin'],
        functionalCategory: 'عطر',
        chemicalType: 'زيت عطري',
        benefits: { 'تطهير': 0.8, 'رائحة منعشة': 0.92, 'مادة قابضة': 0.75 },
        synergy: {},
        negativeSynergy: {},
        warnings: [
          { level: 'risk', text: 'الزيت المعصور على البارد (Expressed) يسبب حساسية شديدة للضوء. الزيت المقطر بالبخار (Distilled) آمن للاستخدام.' }
        ],
      },
{
        id: 'blue-cypress-essential',
        name: 'زيت السرو الأزرق العطري',
        scientific_name: 'Callitris Intratropica Wood Oil',
        searchKeywords: ['blue cypress oil', 'callitris intratropica', 'blue cypress', 'سرو أزرق'],
        usageScope: ['skin'],
        functionalCategory: 'ملطف ومرمم',
        chemicalType: 'زيت عطري',
        benefits: { 'مضاد للالتهابات': 0.85, 'مهدئ': 0.8, 'مضاد للميكروبات': 0.75 },
        synergy: {},
        negativeSynergy: {},
        warnings: [
          { level: 'info', text: 'يحتوي على مركب الغوايازولين الذي يعطيه لونا أزرقا وخصائص مهدئة.' }
        ],
      },
{
        id: 'lentisk-oil',
        name: 'زيت الضرو (المستكة)',
        scientific_name: 'Pistacia Lentiscus Fruit Oil',
        searchKeywords: ['lentisk oil', 'mastic oil', 'pistacia lentiscus', 'زيت الضرو', 'الضرو', 'زيت المستكة', 'المستكة'],
        usageScope: ['skin', 'hair'],
        functionalCategory: 'ملطف ومرمم',
        chemicalType: 'زيت ناقل',
        benefits: { 'ملطف ومرمم': 0.92, 'مضاد للميكروبات': 0.9, 'مضاد للالتهابات': 0.88, 'علاج مشاكل البشرة': 0.85 },
        synergy: { 'chamomile-essential': 1.05 },
        negativeSynergy: {},
        warnings: [
          { level: 'info', text: 'زيت تقليدي ثمين في الجزائر، يستخدم لعلاج الحروق، الأكزيما، والتهابات الجلد بفضل خصائصه المرممة القوية.' }
        ],
      },
{
        id: 'cade-oil',
        name: 'زيت القطران (العرعار)',
        scientific_name: 'Juniperus Oxycedrus Wood Tar',
        searchKeywords: ['cade oil', 'juniper tar', 'juniperus oxycedrus', 'زيت القطران', 'قطران', 'العرعار', 'زيت العرعار'],
        usageScope: ['skin', 'hair'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'زيت عطري (تقطير جاف)',
        benefits: { 'علاج مشاكل البشرة': 0.95, 'مضاد للالتهابات': 0.9, 'مضاد للفطريات': 0.88, 'علاج الصدفية والأكزيما': 0.92 },
        synergy: {},
        negativeSynergy: {},
        warnings: [
          { level: 'risk', text: 'مادة قوية جدا؛ يجب استخدامها بتركيزات منخفضة جدا (1-5%) ومخففة دائما.' },
          { level: 'risk', text: 'له رائحة دخانية قوية جدا ونفاذة.' },
          { level: 'risk', text: 'قد يكون مسرطنا عند استخدامه بتركيزات عالية أو لفترات طويلة بسبب احتوائه على الهيدروكربونات العطرية متعددة الحلقات (PAHs).' },
          { level: 'risk', text: 'لا يستخدم أثناء الحمل أو الرضاعة أو على الجروح المفتوحة.' }
        ],
      },
{
        id: 'henna-oil',
        name: 'زيت الحناء',
        scientific_name: 'Lawsonia Inermis Extract',
        searchKeywords: ['henna oil', 'lawsonia inermis', 'زيت الحناء', 'حناء', 'الحنة'],
        usageScope: ['hair'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'زيت ناقل (مُنقوع)',
        benefits: { 'تقوية الشعر': 0.88, 'تنعيم وإعطاء لمعان': 0.85, 'صحة فروة الرأس': 0.8 },
        synergy: { 'amla-oil': 1.05 },
        negativeSynergy: {},
        warnings: [
          { level: 'info', text: 'عادة ما يكون زيتا منقوعا (أوراق الحناء في زيت ناقل) وليس زيتا معصورا.' },
          { level: 'caution', text: 'قد يضفي لونا نحاسيا خفيفا على الشعر الفاتح جدا مع الاستخدام المتكرر.' }
        ],
      },
{
        id: 'arugula-seed-oil',
        name: 'زيت الجرجير',
        scientific_name: 'Eruca Sativa Seed Oil',
        searchKeywords: ['arugula oil', 'rocket seed oil', 'eruca sativa', 'jarjeer oil', 'زيت الجرجير', 'جرجير', 'الجرجير'],
        usageScope: ['hair'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'زيت ناقل',
        benefits: { 'تحفيز نمو الشعر': 0.9, 'تقوية الشعر': 0.88, 'تقليل تساقط الشعر': 0.85, 'تغذية': 0.8 },
        synergy: { 'castor': 1.08, 'rosemary-essential': 1.05 },
        negativeSynergy: {},
        warnings: [
          { level: 'info', text: 'زيت مشهور جدا في منطقة الخليج والشرق الأوسط لفوائده في تعزيز نمو الشعر وكثافته.' }
        ],
      },
{
        id: 'mustard-oil',
        name: 'زيت الخردل',
        scientific_name: 'Brassica Nigra (Mustard) Seed Oil',
        searchKeywords: ['mustard oil', 'brassica nigra', 'black mustard oil', 'زيت الخردل', 'خردل'],
        usageScope: ['hair', 'skin'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'زيت ناقل',
        benefits: { 'تحفيز الدورة الدموية': 0.9, 'تحفيز نمو الشعر': 0.85, 'مضاد للميكروبات': 0.8, 'تخفيف آلام العضلات': 0.75 },
        synergy: {},
        negativeSynergy: {},
        warnings: [
          { level: 'caution', text: 'قد يسبب تهيجا وإحساسا بالحرارة على الجلد بسبب طبيعته المنشطة. يجب إجراء اختبار حساسية.' },
          { level: 'risk', text: 'يحتوي على نسبة عالية من حمض الإيروسيك. استخدامه الموضعي على نطاق واسع مقيد في بعض البلدان (مثل الاتحاد الأوروبي) بسبب مخاوف من الامتصاص الجلدي.' }
        ],
      },
{
        id: 'date-seed-oil',
        name: 'زيت نواة التمر',
        scientific_name: 'Phoenix Dactylifera Seed Oil',
        searchKeywords: ['date seed oil', 'phoenix dactylifera', 'زيت نواة التمر', 'زيت بذور التمر', 'تمر'],
        usageScope: ['skin'],
        functionalCategory: 'مضاد أكسدة',
        chemicalType: 'زيت ناقل',
        benefits: { 'مضاد أكسدة': 0.9, 'مكافحة الشيخوخة': 0.88, 'ترطيب': 0.85, 'دعم حاجز البشرة': 0.8 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'pistachio-oil',
        name: 'زيت الفستق',
        scientific_name: 'Pistacia Vera Seed Oil',
        searchKeywords: ['pistachio oil', 'pistacia vera', 'زيت الفستق', 'فستق'],
        usageScope: ['skin', 'hair'],
        functionalCategory: 'مرطب / مطري',
        chemicalType: 'زيت ناقل',
        benefits: { 'ترطيب': 0.9, 'مضاد أكسدة': 0.85, 'تنعيم وإعطاء لمعان': 0.8, 'غني بالبيوتين': 0.75 },
        synergy: {},
        negativeSynergy: {},
        warnings: [
          { level: 'risk', text: 'يجب تجنبه من قبل الأشخاص الذين يعانون من حساسية المكسرات.' }
        ],
      },
{
        id: 'watercress-oil',
        name: 'زيت الجرجير المائي',
        scientific_name: 'Nasturtium Officinale Seed Oil',
        searchKeywords: ['watercress oil', 'nasturtium officinale', 'زيت الجرجير المائي', 'جرجير الماء'],
        usageScope: ['hair'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'زيت ناقل',
        benefits: { 'تقوية الشعر': 0.85, 'صحة فروة الرأس': 0.8, 'غني بالمعادن': 0.82 },
        synergy: { 'arugula-seed-oil': 1.05 },
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'carob-oil',
        name: 'زيت الخروب',
        scientific_name: 'Ceratonia Siliqua Seed Oil',
        searchKeywords: ['carob oil', 'ceratonia siliqua', 'locust bean', 'زيت الخروب', 'خروب'],
        usageScope: ['skin'],
        functionalCategory: 'مرطب / مطري',
        chemicalType: 'زيت ناقل',
        benefits: { 'ترطيب': 0.8, 'مضاد أكسدة': 0.75, 'ملطف ومرمم': 0.7 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'coriander-seed-oil',
        name: 'زيت بذور الكزبرة',
        scientific_name: 'Coriandrum Sativum Seed Oil',
        searchKeywords: ['coriander seed oil', 'coriandrum sativum', 'كزبرة', 'زيت بذور الكزبرة', 'قصبر'],
        usageScope: ['skin', 'hair'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'زيت ناقل',
        benefits: { 'مضاد للميكروبات': 0.8, 'صحة فروة الرأس': 0.75, 'مضاد للالتهابات': 0.7 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'bitter-almond-oil',
        name: 'زيت اللوز المر',
        scientific_name: 'Prunus Amygdalus Amara Kernel Oil',
        searchKeywords: ['bitter almond oil', 'prunus amygdalus amara', 'لوز مر', 'زيت اللوز المر'],
        usageScope: ['skin'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'زيت ناقل',
        benefits: { 'تفتيح وتوحيد اللون': 0.7, 'مضاد للفطريات': 0.65 },
        synergy: {},
        negativeSynergy: {},
        warnings: [
          { level: 'risk', text: 'يحتوي على مركبات سامة (amygdalin) بشكل طبيعي. يجب استخدام الزيت "المعالج" فقط الذي تمت إزالة هذه المركبات منه. استخدامه الخام خطير.' },
          { level: 'risk', text: 'يجب تجنبه من قبل الأشخاص الذين يعانون من حساسية المكسرات.' }
        ],
      },
{
        id: 'cumin-seed-oil',
        name: 'زيت بذور الكمون',
        scientific_name: 'Cuminum Cyminum Seed Oil',
        searchKeywords: ['cumin seed oil', 'cuminum cyminum', 'كمون', 'زيت بذور الكمون'],
        usageScope: ['skin'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'زيت ناقل',
        benefits: { 'مضاد أكسدة': 0.8, 'مضاد للميكروبات': 0.75, 'تنقية البشرة': 0.7 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'parsley-seed-oil',
        name: 'زيت بذور البقدونس',
        scientific_name: 'Carum Petroselinum Seed Oil',
        searchKeywords: ['parsley seed oil', 'carum petroselinum', 'بقدونس', 'زيت بذور البقدونس', 'معدنوس'],
        usageScope: ['skin'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'زيت ناقل',
        benefits: { 'تنقية البشرة': 0.8, 'مضاد أكسدة': 0.75, 'مضاد للميكروبات': 0.7 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'radish-seed-oil',
        name: 'زيت بذور الفجل',
        scientific_name: 'Raphanus Sativus Seed Oil',
        searchKeywords: ['radish seed oil', 'raphanus sativus', 'فجل', 'زيت بذور الفجل'],
        usageScope: ['hair'],
        functionalCategory: 'محسن قوام',
        chemicalType: 'زيت ناقل',
        benefits: { 'بديل طبيعي للسيليكون': 0.85, 'تنعيم وإعطاء لمعان': 0.8, 'انزلاق ممتاز': 0.88 },
        synergy: {},
        negativeSynergy: {},
        warnings: [
          { level: 'info', text: 'يعطي ملمسا انزلاقيا مشابها للسيليكونات دون أن يكون سيليكونا.' }
        ],
      },
{
        id: 'papaya-seed-oil',
        name: 'زيت بذور البابايا',
        scientific_name: 'Carica Papaya Seed Oil',
        searchKeywords: ['papaya seed oil', 'carica papaya', 'بابايا', 'زيت بذور البابايا'],
        usageScope: ['skin'],
        functionalCategory: 'مقشر',
        chemicalType: 'زيت ناقل',
        benefits: { 'تقشير لطيف': 0.85, 'تفتيح وتوحيد اللون': 0.8, 'تنظيف المسام': 0.75 },
        synergy: {},
        negativeSynergy: {},
        warnings: [
          { level: 'info', text: 'يحتوي على إنزيم الباباين الذي يساعد على تقشير خلايا الجلد الميتة بلطف.' }
        ],
      },
{
        id: 'guava-seed-oil',
        name: 'زيت بذور الجوافة',
        scientific_name: 'Psidium Guajava Seed Oil',
        searchKeywords: ['guava seed oil', 'psidium guajava', 'جوافة', 'زيت بذور الجوافة'],
        usageScope: ['skin'],
        functionalCategory: 'مضاد أكسدة',
        chemicalType: 'زيت ناقل',
        benefits: { 'مضاد أكسدة': 0.8, 'مكافحة الشيخوخة': 0.75, 'تحسين مرونة الجلد': 0.7 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'passionfruit-seed-oil',
        name: 'زيت بذور فاكهة العاطفة (ماراكوجا)',
        scientific_name: 'Passiflora Edulis Seed Oil',
        searchKeywords: ['passionfruit seed oil', 'maracuja oil', 'passiflora edulis', 'ماراكوجا', 'زيت الماراكوجا', 'فاكهة العاطفة'],
        usageScope: ['skin'],
        functionalCategory: 'مرطب / مطري',
        chemicalType: 'زيت ناقل',
        benefits: { 'دعم حاجز البشرة': 0.9, 'ترطيب': 0.88, 'ملطف ومرمم': 0.85, 'تنظيم الإفرازات الدهنية': 0.8 },
        synergy: {},
        negativeSynergy: {},
        warnings: [
          { level: 'info', text: 'غني جدا بحمض اللينوليك، مما يجعله ممتازا للبشرة المعرضة لحب الشباب والجافة.' }
        ],
      },
{
        id: 'pecan-oil',
        name: 'زيت البقان',
        scientific_name: 'Carya Illinoinensis Seed Oil',
        searchKeywords: ['pecan oil', 'carya illinoinensis', 'بقان', 'زيت البقان'],
        usageScope: ['skin'],
        functionalCategory: 'مرطب / مطري',
        chemicalType: 'زيت ناقل',
        benefits: { 'ترطيب': 0.85, 'مضاد أكسدة': 0.8, 'مكافحة الشيخوخة': 0.75 },
        synergy: {},
        negativeSynergy: {},
        warnings: [
          { level: 'risk', text: 'يجب تجنبه من قبل الأشخاص الذين يعانون من حساسية المكسرات.' }
        ],
      },
{
        id: 'rice-bran-oil',
        name: 'زيت نخالة الأرز',
        scientific_name: 'Oryza Sativa Bran Oil',
        searchKeywords: ['rice bran oil', 'oryza sativa oil', 'نخالة الأرز', 'زيت نخالة الأرز'],
        usageScope: ['skin', 'hair'],
        functionalCategory: 'مضاد أكسدة',
        chemicalType: 'زيت ناقل',
        benefits: { 'مضاد أكسدة': 0.9, 'تفتيح وتوحيد اللون': 0.8, 'ترطيب': 0.85 },
        synergy: {},
        negativeSynergy: {},
        warnings: [
          { level: 'info', text: 'غني بمركب جاما أوريزانول، وهو مضاد أكسدة قوي.' }
        ],
      },
{
        id: 'cotton-seed-oil',
        name: 'زيت بذور القطن',
        scientific_name: 'Gossypium Herbaceum Seed Oil',
        searchKeywords: ['cottonseed oil', 'gossypium herbaceum', 'قطن', 'زيت بذور القطن'],
        usageScope: ['skin'],
        functionalCategory: 'مرطب / مطري',
        chemicalType: 'زيت ناقل',
        benefits: { 'ترطيب': 0.75, 'دعم حاجز البشرة': 0.8 },
        synergy: {},
        negativeSynergy: {},
        warnings: [
          { level: 'caution', text: 'قد يكون كوميدوجينيك.' }
        ],
      },
{
        id: 'elderberry-seed-oil',
        name: 'زيت بذور البلسان',
        scientific_name: 'Sambucus Nigra Seed Oil',
        searchKeywords: ['elderberry seed oil', 'sambucus nigra oil', 'بلسان', 'زيت بذور البلسان'],
        usageScope: ['skin'],
        functionalCategory: 'مضاد أكسدة',
        chemicalType: 'زيت ناقل',
        benefits: { 'مضاد أكسدة': 0.88, 'ملطف ومرمم': 0.85, 'دعم حاجز البشرة': 0.8 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'cloudberry-seed-oil',
        name: 'زيت بذور التوت السحابي',
        scientific_name: 'Rubus Chamaemorus Seed Oil',
        searchKeywords: ['cloudberry seed oil', 'rubus chamaemorus', 'توت سحابي', 'زيت بذور التوت السحابي'],
        usageScope: ['skin'],
        functionalCategory: 'مضاد أكسدة',
        chemicalType: 'زيت ناقل',
        benefits: { 'مضاد أكسدة': 0.9, 'تفتيح وتوحيد اللون': 0.8, 'حماية من أضرار البيئة': 0.85 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'lingonberry-seed-oil',
        name: 'زيت بذور عنب الثور',
        scientific_name: 'Vaccinium Vitis-Idaea Seed Oil',
        searchKeywords: ['lingonberry seed oil', 'vaccinium vitis-idaea', 'عنب الثور', 'زيت بذور عنب الثور'],
        usageScope: ['skin'],
        functionalCategory: 'مضاد أكسدة',
        chemicalType: 'زيت ناقل',
        benefits: { 'مضاد أكسدة': 0.9, 'تفتيح وتوحيد اللون': 0.85, 'حماية من الضوء الأزرق': 0.8 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'blackcurrant-seed-oil',
        name: 'زيت بذور الكشمش الأسود',
        scientific_name: 'Ribes Nigrum Seed Oil',
        searchKeywords: ['blackcurrant seed oil', 'ribes nigrum', 'كشمش أسود', 'زيت بذور الكشمش الأسود'],
        usageScope: ['skin'],
        functionalCategory: 'ملطف ومرمم',
        chemicalType: 'زيت ناقل',
        benefits: { 'مضاد للالتهابات': 0.9, 'دعم حاجز البشرة': 0.88, 'ملطف ومرمم': 0.85 },
        synergy: {},
        negativeSynergy: {},
        warnings: [
          { level: 'info', text: 'غني بكل من أحماض أوميغا 3 (ALA) وأوميغا 6 (GLA)، مما يجعله ممتازا لتهدئة الالتهابات.' }
        ],
      },
{
        id: 'celery-seed-oil-essential',
        name: 'زيت بذور الكرفس العطري',
        scientific_name: 'Apium Graveolens Seed Oil',
        searchKeywords: ['celery seed oil', 'apium graveolens', 'كرفس', 'زيت بذور الكرفس'],
        usageScope: ['skin'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'زيت عطري',
        benefits: { 'إزالة السموم': 0.8, 'مضاد للالتهابات': 0.75 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'dill-essential-oil',
        name: 'زيت الشبت العطري',
        scientific_name: 'Anethum Graveolens Herb Oil',
        searchKeywords: ['dill oil', 'anethum graveolens', 'شبت', 'زيت الشبت'],
        usageScope: ['skin'],
        functionalCategory: 'ملطف ومرمم',
        chemicalType: 'زيت عطري',
        benefits: { 'مهدئ': 0.8, 'مضاد للالتهابات': 0.75 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'anise-essential-oil',
        name: 'زيت اليانسون العطري',
        scientific_name: 'Pimpinella Anisum Seed Oil',
        searchKeywords: ['anise oil', 'pimpinella anisum', 'aniseed', 'يانسون', 'زيت اليانسون'],
        usageScope: ['skin'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'زيت عطري',
        benefits: { 'مضاد للميكروبات': 0.8, 'مضاد أكسدة': 0.7 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'star-anise-essential-oil',
        name: 'زيت اليانسون النجمي العطري',
        scientific_name: 'Illicium Verum Fruit/Seed Oil',
        searchKeywords: ['star anise oil', 'illicium verum', 'يانسون نجمي', 'زيت اليانسون النجمي'],
        usageScope: ['skin'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'زيت عطري',
        benefits: { 'مضاد للميكروبات': 0.85, 'مضاد أكسدة': 0.8 },
        synergy: {},
        negativeSynergy: {},
        warnings: [
          { level: 'info', text: 'مصدر حمض الشيكيميك، الذي يستخدم في صناعة الأدوية المضادة للفيروسات.' }
        ],
      },
{
        id: 'laurel-berry-oil',
        name: 'زيت غار حلب',
        scientific_name: 'Laurus Nobilis Fruit Oil',
        searchKeywords: ['laurel berry oil', 'laurus nobilis', 'aleppo soap', 'زيت الغار', 'غار حلب'],
        usageScope: ['skin', 'hair'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'زيت ناقل',
        benefits: { 'مضاد للميكروبات': 0.9, 'مضاد للالتهابات': 0.85, 'علاج القشرة': 0.88, 'مكافحة حب الشباب': 0.8},
        synergy: {},
        negativeSynergy: {},
        warnings: [
          { level: 'info', text: 'المكون الرئيسي في صابون حلب التقليدي. له خصائص تطهيرية وعلاجية قوية.' }
        ],
      },
{
        id: 'spearmint-oil',
        name: 'زيت النعناع السنبلي',
        scientific_name: 'Mentha Spicata Herb Oil',
        searchKeywords: ['spearmint oil', 'mentha viridis', 'نعناع', 'زيت النعناع'],
        usageScope: ['skin', 'hair'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'زيت عطري',
        benefits: { 'مضاد للالتهابات': 0.8, 'منعش': 0.9, 'ألطف من النعناع الفلفلي': 0.9 },
        synergy: {},
        negativeSynergy: {},
        warnings: [
          { level: 'info', text: 'ألطف بكثير من النعناع الفلفلي لأنه يحتوي على كمية قليلة من المنثول.' }
        ],
      },
{
        id: 'caraway-seed-oil',
        name: 'زيت بذور الكراوية',
        scientific_name: 'Carum Carvi Seed Oil',
        searchKeywords: ['caraway seed oil', 'carum carvi', 'كراوية', 'زيت بذور الكراوية'],
        usageScope: ['skin'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'زيت عطري',
        benefits: { 'مضاد للميكروبات': 0.75, 'قابض للمسام': 0.7 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'magnolia-bark-oil',
        name: 'زيت لحاء الماغنوليا',
        scientific_name: 'Magnolia Officinalis Bark Extract',
        searchKeywords: ['magnolia bark oil', 'magnolia officinalis', 'ماغنوليا', 'زيت لحاء الماغنوليا'],
        usageScope: ['skin'],
        functionalCategory: 'ملطف ومرمم',
        chemicalType: 'زيت ناقل (مُنقوع)',
        benefits: { 'مضاد للالتهابات': 0.88, 'مضاد أكسدة': 0.85, 'مضاد للميكروبات': 0.8 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'ambrette-seed-oil',
        name: 'زيت بذور المسك',
        scientific_name: 'Abelmoschus Moschatus Seed Oil',
        searchKeywords: ['ambrette seed oil', 'abelmoschus moschatus', 'musk mallow', 'مسك', 'زيت بذور المسك'],
        usageScope: ['skin'],
        functionalCategory: 'عطر',
        chemicalType: 'زيت عطري',
        benefits: { 'بديل نباتي للمسك': 0.95, 'رائحة معقدة': 0.9 },
        synergy: {},
        negativeSynergy: {},
        warnings: [
          { level: 'info', text: 'يستخدم في العطور الفاخرة كبديل نباتي لرائحة المسك الحيواني.' }
        ],
      },
{
        id: 'cassia-essential-oil',
        name: 'زيت الكاسيا العطري',
        scientific_name: 'Cinnamomum Cassia Leaf Oil',
        searchKeywords: ['cassia oil', 'cinnamomum cassia', 'كاسيا', 'زيت الكاسيا'],
        usageScope: ['skin'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'زيت عطري',
        benefits: { 'مضاد للميكروبات': 0.85, 'رائحة شبيهة بالقرفة': 0.9 },
        synergy: {},
        negativeSynergy: {},
        warnings: [
          { level: 'risk', text: 'قوي جدا ومهيج للجلد. يجب استخدامه بتركيزات منخفضة للغاية وبحذر شديد.' }
        ],
      },
{
        id: 'litsea-cubeba-essential-oil',
        name: 'زيت ليتسيا كوبيبا العطري',
        scientific_name: 'Litsea Cubeba Fruit Oil',
        searchKeywords: ['litsea cubeba oil', 'may chang', 'ليتسيا كوبيبا', 'ماي تشانغ'],
        usageScope: ['skin'],
        functionalCategory: 'عطر',
        chemicalType: 'زيت عطري',
        benefits: { 'رائحة ليمونية منعشة': 0.9, 'مضاد للميكروبات': 0.8, 'مضاد للفطريات': 0.75 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'tuberose-oil',
        name: 'زيت مسك الروم',
        scientific_name: 'Polianthes Tuberosa Flower Oil',
        searchKeywords: ['tuberose absolute', 'polianthes tuberosa', 'مسك الروم', 'زيت مسك الروم'],
        usageScope: ['skin'],
        functionalCategory: 'عطر',
        chemicalType: 'زيت عطري (مطلق)',
        benefits: { 'رائحة زهرية قوية وفاخرة': 0.98 },
        synergy: {},
        negativeSynergy: {},
        warnings: [
          { level: 'info', text: 'مكلف جدا، يستخدم بشكل أساسي في صناعة العطور الراقية.' }
        ],
      },
{
        id: 'violet-leaf-oil',
        name: 'زيت ورق البنفسج',
        scientific_name: 'Viola Odorata Leaf Extract',
        searchKeywords: ['violet leaf absolute', 'viola odorata', 'بنفسج', 'زيت ورق البنفسج'],
        usageScope: ['skin'],
        functionalCategory: 'عطر',
        chemicalType: 'زيت عطري (مطلق)',
        benefits: { 'رائحة خضراء ترابية': 0.9, 'ملطف ومرمم': 0.75 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'fir-needle-essential-oil',
        name: 'زيت إبر التنوب العطري',
        scientific_name: 'Abies Sibirica Oil',
        searchKeywords: ['fir needle oil', 'abies sibirica', 'siberian fir', 'تنوب', 'زيت إبر التنوب'],
        usageScope: ['skin'],
        functionalCategory: 'عطر',
        chemicalType: 'زيت عطري',
        benefits: { 'رائحة منعشة ونظيفة': 0.9, 'مضاد للميكروبات': 0.75 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'pine-needle-essential-oil',
        name: 'زيت إبر الصنوبر العطري',
        scientific_name: 'Pinus Sylvestris Leaf Oil',
        searchKeywords: ['pine needle oil', 'pinus sylvestris', 'scots pine', 'صنوبر', 'زيت إبر الصنوبر'],
        usageScope: ['skin'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'زيت عطري',
        benefits: { 'مضاد للميكروبات': 0.8, 'منشط': 0.85, 'مضاد للالتهابات': 0.75 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'cajeput-essential-oil',
        name: 'زيت الكاجيبوت العطري',
        scientific_name: 'Melaleuca Leucadendron Cajaputi Oil',
        searchKeywords: ['cajeput oil', 'melaleuca leucadendron', 'كاجيبوت', 'زيت الكاجيبوت'],
        usageScope: ['skin'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'زيت عطري',
        benefits: { 'مضاد للميكروبات': 0.85, 'تسكين الألم الموضعي': 0.8 },
        synergy: {},
        negativeSynergy: {},
        warnings: [
          { level: 'info', text: 'من نفس عائلة شجرة الشاي، له رائحة طبية قوية.' }
        ],
      },
{
        id: 'palmarosa-essential-oil',
        name: 'زيت بالماروزا العطري',
        scientific_name: 'Cymbopogon Martini Oil',
        searchKeywords: ['palmarosa oil', 'cymbopogon martini', 'بالماروزا', 'زيت بالماروزا'],
        usageScope: ['skin'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'زيت عطري',
        benefits: { 'مضاد للميكروبات': 0.85, 'تنظيم الإفرازات الدهنية': 0.8, 'ترطيب': 0.75 },
        synergy: {},
        negativeSynergy: {},
        warnings: [
          { level: 'info', text: 'له رائحة وردية ليمونية، وهو ممتاز للبشرة الدهنية والمعرضة لحب الشباب.' }
        ],
      },
{
        id: 'thuja-essential-oil',
        name: 'زيت العفص العطري',
        scientific_name: 'Thuja Occidentalis Leaf Oil',
        searchKeywords: ['thuja oil', 'arborvitae', 'cedar leaf oil', 'عفص', 'زيت العفص'],
        usageScope: ['skin'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'زيت عطري',
        benefits: { 'مضاد للفيروسات': 0.8, 'مضاد للميكروبات': 0.75 },
        synergy: {},
        negativeSynergy: {},
        warnings: [
          { level: 'risk', text: 'يحتوي على مركب الثوجون الذي قد يكون ساما للأعصاب بتركيزات عالية. يجب استخدامه بحذر شديد وبتركيزات منخفضة جدا.' }
        ],
      },
{
        id: 'sodium-lauryl-sulfate',
        name: 'صوديوم لوريل سلفات (SLS)',
        scientific_name: 'Sodium Lauryl Sulfate',
        searchKeywords: ['sodium lauryl sulfate', 'sls', 'sds', 'sodium dodecyl sulfate', 'صوديوم لوريل سلفات', 'sodium dodecyl sulfate', 'صوديوم دوديسيل سلفات'],
        usageScope: ['hair', 'skin'],
        functionalCategory: 'منظف',
        chemicalType: 'منظف قاسي',
        benefits: { 'تنظيف عميق': 0.95, 'رغوة غنية': 0.9, 'إزالة الزيوت الثقيلة': 0.92 },
        synergy: {},
        negativeSynergy: {},
        warnings: [
          { level: 'risk', text: 'منظف قوي جدا يمكن أن يجرد الشعر وفروة الرأس من الزيوت الطبيعية، مما قد يسبب الجفاف والتهيج.' },
          { level: 'risk', text: 'لا ينصح به للشعر الجاف، المصبوغ، أو فروة الرأس الحساسة والمعرضة للأكزيما.' }
        ],
      },
{
        id: 'sodium-laureth-sulfate',
        name: 'صوديوم لوريث سلفات (SLES)',
        scientific_name: 'Sodium Laureth Sulfate',
        searchKeywords: ['sodium laureth sulfate', 'sles', 'صوديوم لوريث سلفات', 'صوديوم لوراث سلفات', 'sodium laureth ether sulfate', 'صوديوم لوريث إيثر سلفات', 'صوديوم لوراث إيثر سلفات'],
        usageScope: ['hair', 'skin'],
        functionalCategory: 'منظف',
        chemicalType: 'منظف قوي',
        benefits: { 'تنظيف فعال': 0.9, 'رغوة غنية': 0.95 },
        synergy: { 'cocamidopropyl-betaine': 1.1 },
        negativeSynergy: {},
        warnings: [
          { level: 'caution', text: 'ألطف قليلا من SLS بسبب عملية الإيثوكسيل، ولكنه لا يزال منظفا قويا وقد يسبب جفافا للبعض.' },
          { level: 'info', text: 'مخاوف تلوثه بـ 1,4-dioxane تم التعامل معها إلى حد كبير في الصناعة الحديثة من خلال عمليات التنقية.' }
        ],
      },
{
        id: 'ammonium-lauryl-sulfate',
        name: 'كبريتات لوريل الأمونيوم (ALS)',
        scientific_name: 'Ammonium Lauryl Sulfate',
        searchKeywords: ['ammonium lauryl sulfate', 'als', 'كبريتات لوريل الأمونيوم', 'كبريتات الأمونيوم لوريل', 'أمونيوم لوريل سلفات'],
        usageScope: ['hair'],
        functionalCategory: 'منظف',
        chemicalType: 'منظف قاسي',
        benefits: { 'تنظيف عميق': 0.9, 'رغوة غنية': 0.88 },
        synergy: {},
        negativeSynergy: {},
        warnings: [
          { level: 'risk', text: 'مشابه لـ SLS في قوته وقد يكون مجففا ومهيجا، خاصة لفروة الرأس الحساسة.' }
        ],
      },
{
        id: 'ammonium-laureth-sulfate',
        name: 'كبريتات لوريث الأمونيوم (ALES)',
        scientific_name: 'Ammonium Laureth Sulfate',
        searchKeywords: ['ammonium laureth sulfate', 'ales', 'كبريتات لوريث الأمونيوم'],
        usageScope: ['hair'],
        functionalCategory: 'منظف',
        chemicalType: 'منظف قوي',
        benefits: { 'تنظيف فعال': 0.88, 'رغوة غنية': 0.9 },
        synergy: {},
        negativeSynergy: {},
        warnings: [
          { level: 'caution', text: 'مشابه لـ SLES، وهو ألطف من ALS ولكنه لا يزال منظفا قويا.' }
        ],
      },
{
        id: 'sodium-c14-16-olefin-sulfonate',
        name: 'صوديوم C14-16 أوليفين سلفونات',
        scientific_name: 'Sodium C14-16 Olefin Sulfonate',
        searchKeywords: ['sodium c14-16 olefin sulfonate', 'صوديوم أوليفين سلفونات'],
        usageScope: ['hair'],
        functionalCategory: 'منظف',
        chemicalType: 'منظف قوي',
        benefits: { 'تنظيف عميق': 0.9, 'رغوة غنية': 0.88, 'بديل للسلفات': 0.9 },
        synergy: {},
        negativeSynergy: {},
        warnings: [
          { level: 'caution', text: 'على الرغم من أنه "خالٍ من السلفات"، إلا أنه منظف قوي جدا ويمكن أن يكون مجففا تقريبا مثل SLES.' }
        ],
      },
{
        id: 'sodium-cocoyl-isethionate',
        name: 'صوديوم كوكويل إيزثيونات (SCI)',
        scientific_name: 'Sodium Cocoyl Isethionate',
        searchKeywords: ['sodium cocoyl isethionate', 'sci', 'صوديوم كوكويل إيزثيونات'],
        usageScope: ['hair', 'skin'],
        functionalCategory: 'منظف',
        chemicalType: 'منظف لطيف',
        benefits: { 'تنظيف لطيف جدا': 0.95, 'رغوة كريمية': 0.9, 'آمن على البشرة الحساسة': 0.92 },
        synergy: { 'cocamidopropyl-betaine': 1.05 },
        negativeSynergy: {},
        warnings: [
          { level: 'info', text: 'منظف لطيف جدا مشتق من جوز الهند، مثالي للشامبو الصلب والمنتجات المخصصة للشعر الجاف أو المصبوغ.' }
        ],
      },
{
        id: 'sodium-lauroyl-methyl-isethionate',
        name: 'صوديوم لورويل ميثيل إيزثيونات (SLMI)',
        scientific_name: 'Sodium Lauroyl Methyl Isethionate',
        searchKeywords: ['sodium lauroyl methyl isethionate', 'slmi', 'صوديوم لورويل ميثيل إيزثيونات'],
        usageScope: ['hair'],
        functionalCategory: 'منظف',
        chemicalType: 'منظف لطيف',
        benefits: { 'تنظيف لطيف جدا': 0.95, 'بديل آمن للسلفات': 0.92, 'رغوة غنية وكريمية': 0.9, 'آمن على الشعر المصبوغ': 0.88 },
        synergy: { 'cocamidopropyl-betaine': 1.05 },
        negativeSynergy: {},
        warnings: [
          { level: 'info', text: 'أحد أفضل بدائل السلفات، يوفر رغوة ممتازة مع الحفاظ على لطفه وقابل للتحلل بيولوجيا.' }
        ],
      },
{
        id: 'disodium-laureth-sulfosuccinate',
        name: 'ثنائي الصوديوم لوريث سلفوساكسينات',
        scientific_name: 'Disodium Laureth Sulfosuccinate',
        searchKeywords: ['disodium laureth sulfosuccinate', 'سلفوساكسينات'],
        usageScope: ['hair', 'skin'],
        functionalCategory: 'منظف',
        chemicalType: 'منظف لطيف',
        benefits: { 'تنظيف فائق اللطف': 0.9, 'آمن على البشرة الحساسة': 0.9, 'رغوة جيدة': 0.8 },
        synergy: {},
        negativeSynergy: {},
        warnings: [
          { level: 'info', text: 'على الرغم من اسمه، فهو ليس سلفات. جزيئاته كبيرة ولا تخترق الجلد، مما يجعله لطيفا جدا وغير مهيج.' }
        ],
      },
{
        id: 'sodium-lauroyl-sarcosinate',
        name: 'صوديوم لورويل ساركوسينات',
        scientific_name: 'Sodium Lauroyl Sarcosinate',
        searchKeywords: ['sodium lauroyl sarcosinate', 'صوديوم لورويل ساركوسينات'],
        usageScope: ['hair'],
        functionalCategory: 'منظف',
        chemicalType: 'منظف لطيف',
        benefits: { 'تنظيف لطيف': 0.9, 'رغوة جيدة': 0.8, 'آمن على الشعر المصبوغ': 0.85 },
        synergy: { 'cocamidopropyl-betaine': 1.05 },
        negativeSynergy: {},
        warnings: [
          { level: 'info', text: 'مشتق من حمض أميني، وهو لطيف على الشعر والجلد.' }
        ],
      },
{
        id: 'sodium-methyl-cocoyl-taurate',
        name: 'صوديوم ميثيل كوكويل توريت',
        scientific_name: 'Sodium Methyl Cocoyl Taurate',
        searchKeywords: ['sodium methyl cocoyl taurate', 'smct', 'كوكويل توريت'],
        usageScope: ['hair'],
        functionalCategory: 'منظف',
        chemicalType: 'منظف لطيف',
        benefits: { 'تنظيف لطيف': 0.9, 'رغوة كثيفة ومستقرة': 0.92, 'يترك الشعر ناعما': 0.9 },
        synergy: {},
        negativeSynergy: {},
        warnings: [
          { level: 'info', text: 'منظف ممتاز مشتق من حمض أميني، فعال حتى في وجود الماء العسر والزيوت.' }
        ],
      },
{
        id: 'cocamidopropyl-betaine',
        name: 'كوكاميدوبروبيل بيتين (CAPB)',
        scientific_name: 'Cocamidopropyl Betaine',
        searchKeywords: ['cocamidopropyl betaine', 'capb', 'كوكاميدوبروبيل بيتين', 'كوكو أميدوبروبيل', 'كوكاميدوپروبيل بيتايين'],
        usageScope: ['hair', 'skin'],
        functionalCategory: 'منظف',
        chemicalType: 'منظف معتدل',
        benefits: { 'منظف ثانوي لطيف': 0.9, 'يعزز الرغوة': 0.92, 'يقلل تهيج المنظفات القاسية': 0.95 },
        synergy: { 'sodium-laureth-sulfate': 1.1 },
        negativeSynergy: {},
        warnings: [
          { level: 'info', text: 'يُستخدم دائما مع منظفات أخرى لجعل التركيبة أكثر لطفا ورغوة.' },
          { level: 'caution', text: 'قد يسبب حساسية تلامسية لدى بعض الأفراد بسبب الشوائب الناتجة عن عملية التصنيع، على الرغم من أن هذا أصبح نادرا.' }
        ],
      },
{
        id: 'cocamidopropyl-hydroxysultaine',
        name: 'كوكاميدوبروبيل هيدروكسي سولتين (CAHS)',
        scientific_name: 'Cocamidopropyl Hydroxysultaine',
        searchKeywords: ['cocamidopropyl hydroxysultaine', 'cahs', 'هيدروكسي سولتين'],
        usageScope: ['hair'],
        functionalCategory: 'منظف',
        chemicalType: 'منظف معتدل',
        benefits: { 'تنظيف لطيف': 0.9, 'رغوة كثيفة ومستقرة': 0.92, 'مكيف خفيف': 0.8 },
        synergy: {},
        negativeSynergy: {},
        warnings: [
          { level: 'info', text: 'يعتبر نسخة محسنة وأكثر لطفا من Cocamidopropyl Betaine مع رغوة أفضل.' }
        ],
      },
{
        id: 'lauramidopropyl-betaine',
        name: 'لوراميدوبروبيل بيتين',
        scientific_name: 'Lauramidopropyl Betaine',
        searchKeywords: ['lauramidopropyl betaine', 'لوراميدوبروبيل بيتين'],
        usageScope: ['hair'],
        functionalCategory: 'منظف',
        chemicalType: 'منظف معتدل',
        benefits: { 'منظف لطيف': 0.88, 'معزز للرغوة': 0.9, 'مكثف للقوام': 0.8 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'disodium-cocoamphodiacetate',
        name: 'ثنائي الصوديوم كوكو أمفو ثنائي الأسيتات',
        scientific_name: 'Disodium Cocoamphodiacetate',
        searchKeywords: ['disodium cocoamphodiacetate', 'كوكو أمفو ثنائي الأسيتات',  'sodium cocoamphodiacetate', 'ثنائي الصوديوم كوكو أمفو دي أسيتات', 'صوديوم كوكو أمفو دي أسيتات', 'صوديوم كوكوامفودياسيتات'],
        usageScope: ['hair', 'skin'],
        functionalCategory: 'منظف',
        chemicalType: 'منظف لطيف',
        benefits: { 'تنظيف فائق اللطف': 0.95, 'مناسب للأطفال': 0.92, 'لا يسبب تهيج للعين': 0.9 },
        synergy: {},
        negativeSynergy: {},
        warnings: [
          { level: 'info', text: 'لطيف جدا، يستخدم في شامبوهات الأطفال والمنظفات المخصصة للبشرة الحساسة.' }
        ],
      },
{
        id: 'sodium-lauroamphoacetate',
        name: 'صوديوم لورو أمفو أسيتات',
        scientific_name: 'Sodium Lauroamphoacetate',
        searchKeywords: ['sodium lauroamphoacetate', 'صوديوم لورو أمفو أسيتات'],
        usageScope: ['hair'],
        functionalCategory: 'منظف',
        chemicalType: 'منظف لطيف',
        benefits: { 'تنظيف لطيف': 0.9, 'معزز للرغوة': 0.88, 'مكيف خفيف': 0.8 },
        synergy: { 'decyl-glucoside': 1.05 },
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'babassuamidopropyl-betaine',
        name: 'باباسو أميدوبروبيل بيتين',
        scientific_name: 'Babassuamidopropyl Betaine',
        searchKeywords: ['babassuamidopropyl betaine', 'باباسو بيتين'],
        usageScope: ['hair'],
        functionalCategory: 'منظف',
        chemicalType: 'منظف معتدل',
        benefits: { 'بديل نباتي لـ CAPB': 0.9, 'تنظيف لطيف': 0.88, 'رغوة جيدة': 0.85 },
        synergy: {},
        negativeSynergy: {},
        warnings: [
          { level: 'info', text: 'مشتق من زيت الباباسو، وهو بديل شائع لـ Cocamidopropyl Betaine في التركيبات الطبيعية.' }
        ],
      },
{
        id: 'decyl-glucoside',
        name: 'ديسيل جلوكوسايد',
        scientific_name: 'Decyl Glucoside',
        searchKeywords: ['decyl glucoside', 'ديسيل جلوكوسايد'],
        usageScope: ['hair', 'skin'],
        functionalCategory: 'منظف',
        chemicalType: 'منظف لطيف جدا',
        benefits: { 'تنظيف لطيف جدا': 0.98, 'مشتق من النباتات': 0.95, 'مناسب للأطفال': 0.92, 'قابل للتحلل': 0.95 },
        synergy: {},
        negativeSynergy: {},
        warnings: [
          { level: 'info', text: 'لطيف للغاية ومثالي للبشرة الحساسة جدا. رغوته قد تكون أقل كثافة من المنظفات التقليدية.' }
        ],
      },
{
        id: 'coco-glucoside',
        name: 'كوكو جلوكوسايد',
        scientific_name: 'Coco-Glucoside',
        searchKeywords: ['coco-glucoside', 'كوكو جلوكوسايد'],
        usageScope: ['hair', 'skin'],
        functionalCategory: 'منظف',
        chemicalType: 'منظف لطيف جدا',
        benefits: { 'تنظيف لطيف جدا': 0.95, 'مشتق من جوز الهند والسكر': 0.92, 'قابل للتحلل': 0.9 },
        synergy: { 'decyl-glucoside': 1.05 },
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'lauryl-glucoside',
        name: 'لوريل جلوكوسايد',
        scientific_name: 'Lauryl Glucoside',
        searchKeywords: ['lauryl glucoside', 'لوريل جلوكوسايد'],
        usageScope: ['hair', 'skin'],
        functionalCategory: 'منظف',
        chemicalType: 'منظف لطيف جدا',
        benefits: { 'تنظيف لطيف': 0.92, 'مشتق من النباتات': 0.9, 'يعزز كثافة الرغوة': 0.85 },
        synergy: { 'decyl-glucoside': 1.05 },
        negativeSynergy: {},
        warnings: [
          { level: 'info', text: 'من نفس عائلة الجلوكوسيدات، لطيف ومثالي للتركيبات الطبيعية.' }
        ],
      },
{
        id: 'caprylyl-capryl-glucoside',
        name: 'كابريليل/كابريل جلوكوسايد',
        scientific_name: 'Caprylyl/Capryl Glucoside',
        searchKeywords: ['caprylyl/capryl glucoside', 'كابريليل جلوكوسايد'],
        usageScope: ['hair', 'skin'],
        functionalCategory: 'منظف',
        chemicalType: 'منظف لطيف جدا',
        benefits: { 'تنظيف لطيف': 0.9, 'مذوب للزيوت العطرية': 0.88, 'قابل للتحلل': 0.9 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'peg-7-glyceryl-cocoate',
        name: 'PEG-7 غليسيريل كوكوات',
        scientific_name: 'PEG-7 Glyceryl Cocoate',
        searchKeywords: ['peg-7 glyceryl cocoate', 'peg 7', 'غليسيريل كوكوات', 'بيج 7', 'بيغ 7', 'بيج-7', 'بيغ-7', 'peg7', 'glyceryl cocoate', 'PEG7 Glyceryl Cocoate', 'كوكو غليسيريل إيثر', 'كوكو غليسيريل إيثير', 'كوكو غليسيريل إيثرات', 'PEG 7 Glyceryl Cocoate'],
        usageScope: ['hair', 'skin'],
        functionalCategory: 'مرطب / مطري',
        chemicalType: 'عامل إعادة ترطيب',
        benefits: { 'إعادة ترطيب الشعر': 0.88, 'مستحلب': 0.85, 'منظف لطيف ثانوي': 0.75 },
        synergy: {},
        negativeSynergy: {},
        warnings: [
          { level: 'info', text: 'يعمل كمطري قابل للذوبان في الماء، يساعد على تقليل التأثير المجفف للمنظفات القاسية ويترك الشعر ناعما.' }
        ],
      },
{
        id: 'behentrimonium-chloride',
        name: 'كلوريد بيهينتريمونيوم',
        scientific_name: 'Behentrimonium Chloride',
        searchKeywords: ['behentrimonium chloride', 'btac', 'بيهينتريمونيوم كلوريد'],
        usageScope: ['hair'],
        functionalCategory: 'مكيف للشعر',
        chemicalType: 'مركب أمونيوم رباعي',
        benefits: { 'فك تشابك الشعر': 0.95, 'تنعيم الشعر': 0.9, 'مضاد للكهرباء الساكنة': 0.88, 'ترطيب': 0.8 },
        synergy: { 'cetearyl-alcohol': 1.1 },
        negativeSynergy: {},
        warnings: [
          { level: 'caution', text: 'مكيف قوي جدا وفعال، ولكنه قد يسبب تراكما على الشعر الناعم مع الاستخدام المفرط.' }
        ],
      },
{
        id: 'cetrimonium-chloride',
        name: 'كلوريد السيتريمونيوم',
        scientific_name: 'Cetrimonium Chloride',
        searchKeywords: ['cetrimonium chloride', 'ctac', 'سيتريمونيوم كلوريد'],
        usageScope: ['hair'],
        functionalCategory: 'مكيف للشعر',
        chemicalType: 'مركب أمونيوم رباعي',
        benefits: { 'مضاد للكهرباء الساكنة': 0.9, 'فك تشابك خفيف': 0.8, 'تنعيم الشعر': 0.85, 'مستحلب خفيف': 0.7 },
        synergy: {},
        negativeSynergy: {},
        warnings: [
          { level: 'info', text: 'أخف من BTMS و Behentrimonium Chloride، جيد للشعر العادي إلى الناعم.' }
        ],
      },
{
        id: 'behentrimonium-methosulfate',
        name: 'ميثوسلفات البيهينتريمونيوم (BTMS)',
        scientific_name: 'Behentrimonium Methosulfate',
        searchKeywords: ['behentrimonium methosulfate', 'btms', 'ميثوسلفات البيهينتريمونيوم'],
        usageScope: ['hair'],
        functionalCategory: 'مكيف للشعر',
        chemicalType: 'مركب أمونيوم رباعي',
        benefits: { 'فك تشابك فائق': 0.98, 'تكييف عميق للشعر': 0.95, 'لطيف جدا ومستحلب': 0.92, 'يترك الشعر ناعما': 0.95 },
        synergy: { 'cetearyl-alcohol': 1.1 },
        negativeSynergy: {},
        warnings: [
          { level: 'info', text: 'يعتبر من أفضل مكيفات الشعر وأكثرها لطفا. على الرغم من اسمه، فهو **ليس سلفات** مجفف.' }
        ],
      },
{
        id: 'stearamidopropyl-dimethylamine',
        name: 'ستيراميدوبروبيل ثنائي ميثيل أمين',
        scientific_name: 'Stearamidopropyl Dimethylamine',
        searchKeywords: ['stearamidopropyl dimethylamine'],
        usageScope: ['hair'],
        functionalCategory: 'مكيف للشعر',
        chemicalType: 'أمين دهني',
        benefits: { 'بديل للسيليكون': 0.9, 'فك تشابك الشعر': 0.88, 'مضاد للكهرباء الساكنة': 0.85, 'يتحلل بيولوجيا': 0.9 },
        synergy: {},
        negativeSynergy: {},
        warnings: [
          { level: 'info', text: 'يصبح موجب الشحنة في البيئة الحمضية للشامبو والبلسم، فيعمل كمكيف فعال. بديل جيد للكواتس التقليدية.' }
        ],
      },
{
        id: 'dicetyldimonium-chloride',
        name: 'كلوريد ثنائي سيتيل دايمونيوم',
        scientific_name: 'Dicetyldimonium Chloride',
        searchKeywords: ['dicetyldimonium chloride', 'كلوريد ثنائي سيتيل'],
        usageScope: ['hair'],
        functionalCategory: 'مكيف للشعر',
        chemicalType: 'مركب أمونيوم رباعي',
        benefits: { 'تكييف قوي': 0.9, 'مضاد للكهرباء الساكنة': 0.88, 'تنعيم الشعر': 0.85 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'steartrimonium-chloride',
        name: 'كلوريد ستيرتريمونيوم',
        scientific_name: 'Steartrimonium Chloride',
        searchKeywords: ['steartrimonium chloride', 'كلوريد ستيرتريمونيوم'],
        usageScope: ['hair'],
        functionalCategory: 'مكيف للشعر',
        chemicalType: 'مركب أمونيوم رباعي',
        benefits: { 'مضاد للكهرباء الساكنة': 0.92, 'تكييف': 0.85 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'polyquaternium-7',
        name: 'بولي كواتيرنيوم-7',
        scientific_name: 'Polyquaternium-7',
        searchKeywords: ['polyquaternium-7', 'pq-7', 'بولي كواتيرنيوم 7'],
        usageScope: ['hair', 'skin'],
        functionalCategory: 'مكيف للشعر',
        chemicalType: 'بوليمر',
        benefits: { 'فك تشابك الشعر': 0.9, 'تنعيم الشعر': 0.88, 'يعطي إحساسا غنيا للرغوة': 0.85 },
        synergy: {},
        negativeSynergy: {},
        warnings: [
          { level: 'caution', text: 'يشكل طبقة رقيقة على الشعر لتنعيمه. قد يتراكم على الشعر الناعم مع الاستخدام المتكرر بدون تنظيف عميق.' }
        ],
      },
{
        id: 'polyquaternium-10',
        name: 'بولي كواتيرنيوم-10',
        scientific_name: 'Polyquaternium-10',
        searchKeywords: ['polyquaternium-10', 'pq-10', 'بوليكواتيرنيوم-10', 'بولي كواتيرنيوم 10', 'بولي كواتيرنيوم-10',  ],
        usageScope: ['hair'],
        functionalCategory: 'مكيف للشعر',
        chemicalType: 'بوليمر',
        benefits: { 'تكوين طبقة حماية': 0.9, 'تقليل الهيشان': 0.88, 'تحسين تموجات الشعر': 0.8, 'تسهيل التصفيف': 0.85 },
        synergy: {},
        negativeSynergy: {},
        warnings: [
          { level: 'caution', text: 'مشتق من السليلوز، وهو بوليمر شائع جدا في منتجات الشعر. قد يتراكم على الشعر.' }
        ],
      },
{
        id: 'polyquaternium-39',
        name: 'بولي كواتيرنيوم-39',
        scientific_name: 'Polyquaternium-39',
        searchKeywords: ['polyquaternium-39', 'pq-39', 'بولي كواتيرنيوم 39'],
        usageScope: ['hair'],
        functionalCategory: 'مكيف للشعر',
        chemicalType: 'بوليمر',
        benefits: { 'تكييف ممتاز للشعر الجاف': 0.92, 'يعطي ملمسا ناعما': 0.9, 'فك تشابك الشعر': 0.88 },
        synergy: {},
        negativeSynergy: {},
        warnings: [
          { level: 'caution', text: 'قد يتراكم على الشعر مع الاستخدام المتكرر.' }
        ],
      },
{
        id: 'polyquaternium-11',
        name: 'بولي كواتيرنيوم-11',
        scientific_name: 'Polyquaternium-11',
        searchKeywords: ['polyquaternium-11', 'pq-11', 'بولي كواتيرنيوم 11'],
        usageScope: ['hair'],
        functionalCategory: 'مكيف للشعر',
        chemicalType: 'بوليمر',
        benefits: { 'تثبيت خفيف لتصفيفة الشعر': 0.88, 'إضافة حجم وكثافة': 0.85, 'تكوين طبقة واقية': 0.8 },
        synergy: {},
        negativeSynergy: {},
        warnings: [
          { level: 'info', text: 'يستخدم غالبا في رغوة الشعر (الموس) ومنتجات التصفيف لتوفير ثبات مرن.' }
        ],
      },
{
        id: 'guar-hydroxypropyltrimonium-chloride',
        name: 'كلوريد غوار هيدروكسي بروبيل تريمونيوم',
        scientific_name: 'Guar Hydroxypropyltrimonium Chloride',
        searchKeywords: ['guar hydroxypropyltrimonium chloride', 'guar gum'],
        usageScope: ['hair'],
        functionalCategory: 'مكيف للشعر',
        chemicalType: 'بوليمر طبيعي',
        benefits: { 'تكييف لطيف': 0.88, 'تقليل الهيشان': 0.85, 'مشتق نباتي': 0.9, 'زيادة كثافة القوام': 0.8 },
        synergy: {},
        negativeSynergy: {},
        warnings: [
          { level: 'caution', text: 'مشتق من صمغ الغوار، وهو مكيف ومكثف طبيعي. قد يتراكم على الشعر.' }
        ],
      },
{
        id: 'dimethicone',
        name: 'دايميثيكون',
        scientific_name: 'Dimethicone',
        searchKeywords: ['dimethicone', 'polydimethylsiloxane', 'pdms', 'دايميثيكون', 'سيليكون'],
        usageScope: ['hair', 'skin'],
        functionalCategory: 'مكيف للشعر',
        chemicalType: 'سيليكون',
        benefits: { 'تنعيم وإعطاء لمعان': 0.95, 'تكوين طبقة حماية': 0.9, 'حماية من الحرارة': 0.85, 'تقليل الهيشان': 0.92 },
        synergy: {},
        negativeSynergy: {},
        warnings: [
          { level: 'caution', text: 'سيليكون غير قابل للذوبان في الماء. يتراكم على الشعر بمرور الوقت، مما يتطلب استخدام شامبو منظف (clarifying) لإزالته.' }
        ],
      },
{
        id: 'amodimethicone',
        name: 'أمodimethicone',
        scientific_name: 'Amodimethicone',
        searchKeywords: ['amodimethicone', 'أمodimethicone'],
        usageScope: ['hair'],
        functionalCategory: 'مكيف للشعر',
        chemicalType: 'سيليكون',
        benefits: { 'تكييف مستهدف وذكي': 0.92, 'حماية اللون': 0.85, 'لمعان ونعومة': 0.9, 'لا يتراكم بكثرة': 0.8 },
        synergy: {},
        negativeSynergy: {},
        warnings: [
          { level: 'info', text: 'سيليكون "ذكي" يلتصق بالمناطق الأكثر تلفا في الشعر. من الصعب إزالته بالمنظفات اللطيفة ولكنه لا يتراكم على نفسه مثل الدايميثيكون.' }
        ],
      },
{
        id: 'cyclopentasiloxane',
        name: 'سيكلوبنتاسيلوكسان (D5)',
        scientific_name: 'Cyclopentasiloxane',
        searchKeywords: ['cyclopentasiloxane', 'd5', 'dekamethylcyclopentasiloxane', 'سيكلوبنتاسيلوكسان'],
        usageScope: ['hair', 'skin'],
        functionalCategory: 'محسن قوام',
        chemicalType: 'سيليكون متطاير',
        benefits: { 'ملمس حريري': 0.95, 'تبخر سريع': 0.92, 'تسهيل انتشار المنتج': 0.9 },
        synergy: {},
        negativeSynergy: {},
        warnings: [
          { level: 'info', text: 'سيليكون خفيف جدا يتبخر بعد التطبيق، تاركا وراءه المكونات الأخرى. يستخدم لإعطاء ملمس خفيف وغير دهني.' }
        ],
      },
{
        id: 'dimethiconol',
        name: 'دايميثيكونول',
        scientific_name: 'Dimethiconol',
        searchKeywords: ['dimethiconol', 'دايميثيكونول'],
        usageScope: ['hair', 'skin'],
        functionalCategory: 'مكيف للشعر',
        chemicalType: 'سيليكون',
        benefits: { 'تنعيم فائق': 0.9, 'تكوين طبقة حماية': 0.88, 'لمعان': 0.85, 'قوام غني': 0.8 },
        synergy: {},
        negativeSynergy: {},
        warnings: [
          { level: 'caution', text: 'سيليكون عالي اللزوجة، غالبا ما يتم مزجه مع سيليكونات متطايرة لتسهيل انتشاره. يتراكم على الشعر.' }
        ],
      },
{
        id: 'peg-12-dimethicone',
        name: 'PEG-12 دايميثيكون',
        scientific_name: 'PEG-12 Dimethicone',
        searchKeywords: ['peg-12 dimethicone', 'peg 12', 'سيليكون مائي'],
        usageScope: ['hair'],
        functionalCategory: 'مكيف للشعر',
        chemicalType: 'سيليكون قابل للذوبان في الماء',
        benefits: { 'تكييف خفيف': 0.85, 'ترطيب': 0.8, 'لا يتراكم': 0.9 },
        synergy: {},
        negativeSynergy: {},
        warnings: [
          { level: 'info', text: 'بما أنه قابل للذوبان في الماء، فإنه لا يتراكم على الشعر ويمكن غسله بسهولة.' }
        ],
      },
{
        id: 'cetyl-alcohol',
        name: 'كحول السيتيل',
        scientific_name: 'Cetyl Alcohol',
        searchKeywords: ['cetyl alcohol', 'كحول السيتيل'],
        usageScope: ['hair', 'skin'],
        functionalCategory: 'محسن قوام',
        chemicalType: 'كحول دهني',
        benefits: { 'مطري ومنعم': 0.9, 'مكثف للقوام': 0.88, 'مستحلب مساعد': 0.8 },
        synergy: { 'stearyl-alcohol': 1.05 },
        negativeSynergy: {},
        warnings: [
          { level: 'info', text: 'كحول دهني "جيد" يساعد على ترطيب وتنعيم الشعر، وليس من الكحوليات المجففة.' }
        ],
      },
{
        id: 'stearyl-alcohol',
        name: 'كحول الستياريل',
        scientific_name: 'Stearyl Alcohol',
        searchKeywords: ['stearyl alcohol', 'كحول الستياريل', 'ستياريل كحول', 'alcool stéarylique', ' Alcohol Stéarylique', 'Alcohol Stéarylic', 'alcolol stearyl'],
        usageScope: ['hair', 'skin'],
        functionalCategory: 'محسن قوام',
        chemicalType: 'كحول دهني',
        benefits: { 'مطري ومنعم': 0.92, 'مكثف للقوام': 0.9, 'مستحلب مساعد': 0.88 },
        synergy: { 'cetyl-alcohol': 1.05 },
        negativeSynergy: {},
        warnings: [
          { level: 'info', text: 'كحول دهني "جيد"، أثقل قليلا من كحول السيتيل ويعطي ملمسا أكثر ثراء.' }
        ],
      },
{
        id: 'cetearyl-alcohol',
        name: 'كحول السيتريل',
        scientific_name: 'Cetearyl Alcohol',
        searchKeywords: ['cetearyl alcohol', 'كحول السيتريل', 'alcool cetéarylique', ' Alcohol Cetearylique', 'Alcohol Cetearylic'],
        usageScope: ['hair', 'skin'],
        functionalCategory: 'محسن قوام',
        chemicalType: 'كحول دهني',
        benefits: { 'مطري ومنعم': 0.95, 'مكثف للقوام': 0.92, 'مستحلب فعال': 0.9 },
        synergy: { 'behentrimonium-methosulfate': 1.1 },
        negativeSynergy: {},
        warnings: [
          { level: 'info', text: 'مزيج من كحول السيتيل والستياريل، وهو مكون شائع جدا في البلسمات والكريمات.' }
        ],
      },
{
        id: 'salicylic-acid',
        name: 'حمض الساليسيليك',
        scientific_name: 'Salicylic Acid',
        searchKeywords: ['salicylic acid'],
        usageScope: ['hair', 'skin'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'حمض بيتا هيدروكسي (BHA)',
        benefits: { 'تقشير فروة الرأس': 0.9, 'تنظيف المسام': 0.92, 'علاج القشرة': 0.88 },
        synergy: { 'piroctone-olamine': 1.05 },
        negativeSynergy: {},
        warnings: [
          { level: 'info', text: 'ممتاز لفروة الرأس الدهنية والمعرضة للقشرة لأنه يذوب في الزيت وينظف بصيلات الشعر بعمق.' },
        ],
      },
{
        id: 'aloe-vera-juice',
        name: 'عصير الألوفيرا',
        scientific_name: 'Aloe Barbadensis Leaf Juice',
        searchKeywords: ['aloe vera juice', 'aloe barbadensis', 'aloe', 'ألوفيرا', 'صبار'],
        usageScope: ['hair', 'skin'],
        functionalCategory: 'ملطف ومرمم',
        chemicalType: 'مستخلص نباتي',
        benefits: { 'ترطيب': 0.9, 'ملطف ومرمم': 0.95, 'صحة فروة الرأس': 0.88 },
        synergy: { 'panthenol': 1.05 },
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'hydrolyzed-oat-protein',
        name: 'بروتين الشوفان المهدرج',
        scientific_name: 'Hydrolyzed Oat Protein',
        searchKeywords: ['hydrolyzed oat protein', 'oat protein'],
        usageScope: ['hair'],
        functionalCategory: 'تقوية الشعر',
        chemicalType: 'بروتين',
        benefits: { 'إضافة نعومة': 0.85, 'ترطيب': 0.8, 'تقليل التقصف': 0.75 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'bamboo-extract',
        name: 'مستخلص الخيزران',
        scientific_name: 'Bambusa Vulgaris Extract',
        searchKeywords: ['bamboo extract', 'bambusa vulgaris', 'خيزران'],
        usageScope: ['hair'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'مستخلص نباتي',
        benefits: { 'تقوية الشعر': 0.85, 'مصدر للسيليكا': 0.88, 'تحسين مرونة الشعر': 0.8 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'hibiscus-extract',
        name: 'مستخلص الكركديه',
        scientific_name: 'Hibiscus Sabdariffa Flower Extract',
        searchKeywords: ['hibiscus extract', 'hibiscus sabdariffa', 'كركديه'],
        usageScope: ['hair'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'مستخلص نباتي',
        benefits: { 'مضاد أكسدة': 0.85, 'تقوية الشعر': 0.8, 'إضافة لمعان': 0.82 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'ginseng-root-extract',
        name: 'مستخلص جذور الجينسنغ',
        scientific_name: 'Panax Ginseng Root Extract',
        searchKeywords: ['ginseng root extract', 'panax ginseng', 'جينسنغ'],
        usageScope: ['hair'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'مستخلص نباتي',
        benefits: { 'تحفيز نمو الشعر': 0.8, 'تقوية بصيلات الشعر': 0.85, 'صحة فروة الرأس': 0.82 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'phenoxyethanol',
        name: 'فينوكسي إيثانول',
        scientific_name: 'Phenoxyethanol',
        searchKeywords: ['phenoxyethanol', 'فينوكسي إيثانول'],
        usageScope: ['hair', 'skin'],
        functionalCategory: 'مادة حافظة',
        chemicalType: 'مادة حافظة',
        benefits: { 'حفظ المنتج': 0.95, 'طيف واسع ضد البكتيريا': 0.9 },
        synergy: { 'ethylhexylglycerin': 1.1 },
        negativeSynergy: {},
        warnings: [
          { level: 'info', text: 'مادة حافظة شائعة جدا تعتبر بديلا آمنا للبارابين عند استخدامها بتركيز يصل إلى 1%.' }
        ],
      },
{
        id: 'sodium-benzoate',
        name: 'بنزوات الصوديوم',
        scientific_name: 'Sodium Benzoate',
        searchKeywords: ['sodium benzoate', 'بنزوات الصوديوم'],
        usageScope: ['hair', 'skin'],
        functionalCategory: 'مادة حافظة',
        chemicalType: 'مادة حافظة',
        benefits: { 'حفظ المنتج': 0.9, 'مضاد للفطريات': 0.88 },
        synergy: { 'potassium-sorbate': 1.05 },
        negativeSynergy: {},
        warnings: [
          { level: 'caution', text: 'قد يتفاعل مع حمض الأسكوربيك (فيتامين C) في وجود أيونات المعادن والضوء لتكوين البنزين، وهو مادة مسرطنة. المصنعون المحترفون يتحكمون في التركيبة لتجنب هذا التفاعل.' }
        ],
      },
{
        id: 'potassium-sorbate',
        name: 'سوربات البوتاسيوم',
        scientific_name: 'Potassium Sorbate',
        searchKeywords: ['potassium sorbate', 'سوربات البوتاسيوم'],
        usageScope: ['hair', 'skin'],
        functionalCategory: 'مادة حافظة',
        chemicalType: 'مادة حافظة',
        benefits: { 'حفظ المنتج': 0.9, 'مضاد للفطريات والخمائر': 0.92 },
        synergy: { 'sodium-benzoate': 1.05 },
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'benzyl-alcohol',
        name: 'كحول بنزيلي',
        scientific_name: 'Benzyl Alcohol',
        searchKeywords: ['benzyl alcohol', 'كحول بنزيلي'],
        usageScope: ['hair', 'skin'],
        functionalCategory: 'مادة حافظة',
        chemicalType: 'كحول عطري',
        benefits: { 'حفظ المنتج': 0.88, 'مذيب': 0.85, 'عامل عطري': 0.75 },
        synergy: { 'dehydroacetic-acid': 1.05 },
        negativeSynergy: {},
        warnings: [
          { level: 'caution', text: 'مادة حافظة طبيعية ومكون عطري، يجب الإفصاح عنه كمسبب للحساسية في أوروبا.' }
        ],
      },
{
        id: 'ethylhexylglycerin',
        name: 'إيثيل هكسيل جليسرين',
        scientific_name: 'Ethylhexylglycerin',
        searchKeywords: ['ethylhexylglycerin', 'إيثيل هكسيل جليسرين'],
        usageScope: ['hair', 'skin'],
        functionalCategory: 'مادة حافظة',
        chemicalType: 'معزز للمواد الحافظة',
        benefits: { 'معزز للمواد الحافظة': 0.92, 'مزيل روائح خفيف': 0.8, 'مطري': 0.85 },
        synergy: { 'phenoxyethanol': 1.1 },
        negativeSynergy: {},
        warnings: [
          { level: 'info', text: 'لا يستخدم كمادة حافظة بمفرده، بل يعزز فعالية المواد الحافظة الأخرى مثل الفينوكسي إيثانول.' }
        ],
      },
{
        id: 'dmdm-hydantoin',
        name: 'DMDM هيدانتوين',
        scientific_name: 'DMDM Hydantoin',
        searchKeywords: ['dmdm hydantoin', 'dmdm'],
        usageScope: ['hair', 'skin'],
        functionalCategory: 'مادة حافظة',
        chemicalType: 'مطلق للفورمالديهايد',
        benefits: { 'حفظ المنتج': 0.8 },
        synergy: {},
        negativeSynergy: {},
        warnings: [
          { level: 'risk', text: 'مادة حافظة مثيرة للجدل تطلق ببطء مادة الفورمالديهايد، وهي مادة مسرطنة معروفة ومسببة للحساسية.' }
        ],
      },
{
        id: 'methylisothiazolinone',
        name: 'ميثيل أيزوثيازولينون (MIT)',
        scientific_name: 'Methylisothiazolinone',
        searchKeywords: ['methylisothiazolinone', 'mit', 'ميثيل أيزوثيازولينون', 'kathon', 'methyl Isothiazolinone'],
        usageScope: ['hair', 'skin'],
        functionalCategory: 'مادة حافظة',
        chemicalType: 'مادة حافظة (مثيرة للجدل)',
        benefits: { 'حفظ المنتج': 0.85 },
        synergy: {},
        negativeSynergy: {},
        warnings: [
          { level: 'risk', text: 'مسبب قوي جدا لحساسية الجلد التلامسية. تم حظره في المنتجات التي تترك على الجلد في أوروبا، ويُسمح به فقط بتركيزات منخفضة جدا في المنتجات التي تُشطف.' }
        ],
      },
{
        id: 'methylchloroisothiazolinone',
        name: 'ميثيل كلورو أيزوثيازولينون (MCI)',
        scientific_name: 'Methylchloroisothiazolinone',
        searchKeywords: ['methylchloroisothiazolinone', 'mci', 'kathon cg', 'ميثيل كلورو أيزوثيازولينون', 'methyl chloro isothiazolinone'],
        usageScope: ['hair', 'skin'],
        functionalCategory: 'مادة حافظة',
        chemicalType: 'مادة حافظة (مثيرة للجدل)',
        benefits: { 'حفظ المنتج': 0.9 },
        synergy: { 'methylisothiazolinone': 1.1 },
        negativeSynergy: {},
        warnings: [
          { level: 'risk', text: 'غالبا ما يستخدم مع MIT. وهو أيضا مسبب قوي جدا للحساسية ومقيد بشدة في الاستخدام.' }
        ],
      },
{
        id: 'hydroxypropyl-guar',
        name: 'هيدروكسي بروبيل غوار',
        scientific_name: 'Hydroxypropyl Guar',
        searchKeywords: ['hydroxypropyl guar', 'هيدروكسي بروبيل غوار'],
        usageScope: ['hair'],
        functionalCategory: 'محسن قوام',
        chemicalType: 'بوليمر طبيعي',
        benefits: { 'مكثف للقوام': 0.88, 'تكييف خفيف': 0.8 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'peg-120-methyl-glucose-dioleate',
        name: 'PEG-120 ميثيل جلوكوز ثنائي الأوليات',
        scientific_name: 'PEG-120 Methyl Glucose Dioleate',
        searchKeywords: ['peg-120 methyl glucose dioleate', 'glucamate doe-120', 'peg-120', 'peg120', 'peg 120'],
        usageScope: ['hair'],
        functionalCategory: 'محسن قوام',
        chemicalType: 'بوليمر',
        benefits: { 'مكثف فعال مشتق من النبات': 0.92, 'لا يسبب تهيج للعين': 0.9, 'لا يقلل الرغوة': 0.95 },
        synergy: {},
        negativeSynergy: {},
        warnings: [
          { level: 'info', text: 'مكثف شائع جدا وفعال للغاية في أنظمة المنظفات.' }
        ],
      },
{
        id: 'acrylates-c10-30-alkyl-acrylate-crosspolymer',
        name: 'أكريلات/C10-30 ألكيل أكريليت كروسبوليمر',
        scientific_name: 'Acrylates/C10-30 Alkyl Acrylate Crosspolymer',
        searchKeywords: ['acrylates/c10-30 alkyl acrylate crosspolymer', 'carbopol ultrez 21'],
        usageScope: ['hair', 'skin'],
        functionalCategory: 'محسن قوام',
        chemicalType: 'بوليمر صناعي',
        benefits: { 'مكثف للقوام': 0.9, 'تثبيت المستحلبات': 0.88 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'hydroxypropyl-methylcellulose',
        name: 'هيدروكسي بروبيل ميثيل سلولوز (HPMC)',
        scientific_name: 'Hydroxypropyl Methylcellulose',
        searchKeywords: ['hydroxypropyl methylcellulose', 'hpmc', 'hypromellose'],
        usageScope: ['hair'],
        functionalCategory: 'محسن قوام',
        chemicalType: 'بوليمر طبيعي',
        benefits: { 'مكثف للقوام': 0.9, 'تكوين طبقة رقيقة': 0.85, 'تعزيز الرغوة': 0.8 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'cellulose-gum',
        name: 'صمغ السليلوز',
        scientific_name: 'Cellulose Gum',
        searchKeywords: ['cellulose gum', 'cmc', 'carboxymethyl cellulose'],
        usageScope: ['hair', 'skin'],
        functionalCategory: 'محسن قوام',
        chemicalType: 'بوليمر طبيعي',
        benefits: { 'مكثف للقوام': 0.9, 'تثبيت التركيبة': 0.88 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'magnesium-aluminum-silicate',
        name: 'سيليكات المغنيسيوم والألومنيوم',
        scientific_name: 'Magnesium Aluminum Silicate',
        searchKeywords: ['magnesium aluminum silicate', 'veegum', 'فيجم'],
        usageScope: ['hair', 'skin'],
        functionalCategory: 'محسن قوام',
        chemicalType: 'طين طبيعي',
        benefits: { 'مكثف من الطين الطبيعي': 0.9, 'تثبيت التركيبة': 0.88, 'امتصاص خفيف للزيوت': 0.8 },
        synergy: { 'xanthan-gum': 1.05 },
        negativeSynergy: {},
        warnings: [
          { level: 'info', text: 'طين منقى يستخدم لتكثيف القوام وتثبيت المستحلبات.' }
        ],
      },
{
        id: 'pvp',
        name: 'PVP (بولي فينيل بيروليدون)',
        scientific_name: 'PVP',
        searchKeywords: ['pvp', 'polyvinylpyrrolidone', 'بولي فينيل بيروليدون'],
        usageScope: ['hair'],
        functionalCategory: 'مثبت للشعر',
        chemicalType: 'بوليمر صناعي',
        benefits: { 'تثبيت تصفيفة الشعر': 0.95, 'تكوين طبقة رقيقة': 0.9, 'مكثف': 0.8 },
        synergy: {},
        negativeSynergy: {},
        warnings: [
          { level: 'info', text: 'مكون أساسي في جل ومثبتات الشعر، يستخدم أحيانا في الشامبو لإضافة قوام خفيف.' }
        ],
      },
{
        id: 'sclerotium-gum',
        name: 'صمغ Sclerotium',
        scientific_name: 'Sclerotium Gum',
        searchKeywords: ['sclerotium gum', 'صمغ Sclerotium'],
        usageScope: ['skin', 'hair'],
        functionalCategory: 'محسن قوام',
        chemicalType: 'بوليمر طبيعي',
        benefits: { 'مكثف طبيعي': 0.9, 'يثبت التركيبة': 0.88, 'يعطي ملمسا ناعما': 0.85 },
        synergy: { 'xanthan-gum': 1.08 },
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'peg-40-hydrogenated-castor-oil',
        name: 'زيت الخروع المهدرج PEG-40',
        scientific_name: 'PEG-40 Hydrogenated Castor Oil',
        searchKeywords: ['peg-40 hydrogenated castor oil', 'peg 40', 'زيت الخروع المهدرج'],
        usageScope: ['hair', 'skin'],
        functionalCategory: 'مذوب',
        chemicalType: 'مستحلب',
        benefits: { 'مذوب للعطور والزيوت في الماء': 0.95, 'منظف لطيف': 0.8, 'مطري': 0.85 },
        synergy: {},
        negativeSynergy: {},
        warnings: [
          { level: 'info', text: 'يستخدم لإذابة المكونات الزيتية (مثل العطور والزيوت العطرية) في التركيبات المائية مثل الشامبو.' }
        ],
      },
{
        id: 'peg-60-hydrogenated-castor-oil',
        name: 'زيت الخروع المهدرج PEG-60',
        scientific_name: 'PEG-60 Hydrogenated Castor Oil',
        searchKeywords: ['peg-60 hydrogenated castor oil', 'peg 60'],
        usageScope: ['hair', 'skin'],
        functionalCategory: 'مذوب',
        chemicalType: 'مستحلب',
        benefits: { 'مذوب للعطور والزيوت': 0.9, 'منظف لطيف': 0.8 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'phytic-acid',
        name: 'حمض الفيتيك',
        scientific_name: 'Phytic Acid',
        searchKeywords: ['phytic acid'],
        usageScope: ['hair', 'skin'],
        functionalCategory: 'محسن قوام',
        chemicalType: 'عامل خالب',
        benefits: { 'عامل خالب طبيعي': 0.9, 'مضاد أكسدة': 0.85 },
        synergy: {},
        negativeSynergy: {},
        warnings: [
          { level: 'info', text: 'بديل طبيعي لـ EDTA، يساعد على تحسين استقرار المنتج.' }
        ],
      },
{
        id: 'tetrasodium-glutamate-diacetate',
        name: 'رباعي صوديوم جلوتامات ثنائي الأسيتات',
        scientific_name: 'Tetrasodium Glutamate Diacetate',
        searchKeywords: ['tetrasodium glutamate diacetate', 'glda'],
        usageScope: ['hair', 'skin'],
        functionalCategory: 'محسن قوام',
        chemicalType: 'عامل خالب',
        benefits: { 'عامل خالب صديق للبيئة': 0.95, 'يعزز فعالية المواد الحافظة': 0.9, 'قابل للتحلل': 0.92 },
        synergy: {},
        negativeSynergy: {},
        warnings: [
          { level: 'info', text: 'بديل ممتاز وصديق للبيئة لـ EDTA.' }
        ],
      },
{
        id: 'potassium-hydroxide',
        name: 'هيدروكسيد البوتاسيوم',
        scientific_name: 'Potassium Hydroxide',
        searchKeywords: ['potassium hydroxide'],
        usageScope: ['hair', 'skin'],
        functionalCategory: 'محسن قوام',
        chemicalType: 'معدل الحموضة',
        benefits: { 'ضبط حموضة المنتج (pH)': 0.98 },
        synergy: {},
        negativeSynergy: {},
        warnings: [
          { level: 'info', text: 'يستخدم بكميات صغيرة جدا لرفع درجة حموضة المنتج.' }
        ],
      },
{
        id: 'lactic-acid',
        name: 'حمض اللاكتيك',
        scientific_name: 'Lactic Acid',
        searchKeywords: ['lactic acid'],
        usageScope: ['hair', 'skin'],
        functionalCategory: 'محسن قوام',
        chemicalType: 'معدل الحموضة',
        benefits: { 'ضبط حموضة المنتج (pH)': 0.9, 'ترطيب': 0.85, 'تقشير لطيف': 0.8 },
        synergy: {},
        negativeSynergy: {},
        warnings: [
          { level: 'info', text: 'يستخدم بكميات صغيرة لخفض درجة حموضة الشامبو لتكون متوافقة مع الشعر (pH 4.5-5.5).' }
        ],
      },
{
        id: 'sodium-citrate',
        name: 'سترات الصوديوم',
        scientific_name: 'Sodium Citrate',
        searchKeywords: ['sodium citrate', 'سترات الصوديوم'],
        usageScope: ['hair', 'skin'],
        functionalCategory: 'محسن قوام',
        chemicalType: 'عامل منظم للحموضة',
        benefits: { 'تنظيم درجة الحموضة': 0.9, 'عامل خالب': 0.8 },
        synergy: {},
        negativeSynergy: {},
        warnings: [
          { level: 'info', text: 'يساعد على الحفاظ على درجة حموضة المنتج ثابتة.' }
        ],
      },
{
        id: 'mica',
        name: 'ميكا',
        scientific_name: 'Mica',
        searchKeywords: ['mica', 'ci 77019', 'ميكا'],
        usageScope: ['hair', 'skin'],
        functionalCategory: 'محسن جمالي',
        chemicalType: 'ملون معدني',
        benefits: { 'إعطاء لمعان وتوهج': 0.95 },
        synergy: {},
        negativeSynergy: {},
        warnings: [
          { level: 'caution', text: 'قد يكون هناك مخاوف أخلاقية بشأن عمالة الأطفال في تعدين الميكا. ابحث عن علامات تجارية تستخدم مصادر مسؤولة.' }
        ],
      },
{
        id: 'titanium-dioxide-ci-77891',
        name: 'ثاني أكسيد التيتانيوم (CI 77891)',
        scientific_name: 'Titanium Dioxide (CI 77891)',
        searchKeywords: ['titanium dioxide', 'ci 77891', 'ملون أبيض'],
        usageScope: ['hair', 'skin'],
        functionalCategory: 'محسن جمالي',
        chemicalType: 'ملون معدني',
        benefits: { 'يعطي لونا أبيضا': 0.95, 'يجعل التركيبة معتمة': 0.9 },
        synergy: {},
        negativeSynergy: {},
        warnings: [
          { level: 'info', text: 'عندما يدرج كـ CI 77891، فإنه يستخدم كملون وليس كفلتر شمسي فعال.' }
        ],
      },
{
        id: 'iron-oxides',
        name: 'أكاسيد الحديد',
        scientific_name: 'Iron Oxides (CI 77491, CI 77492, CI 77499)',
        searchKeywords: ['iron oxides', 'ci 77491', 'ci 77492', 'ci 77499'],
        usageScope: ['hair', 'skin'],
        functionalCategory: 'محسن جمالي',
        chemicalType: 'ملون معدني',
        benefits: { 'ملونات طبيعية': 0.9 },
        synergy: {},
        negativeSynergy: {},
        warnings: [
          { level: 'info', text: 'تستخدم لإعطاء ألوان (أحمر، أصفر، أسود) للشامبو، خاصة في الشامبو الملون.' }
        ],
      },
{
        id: 'tin-oxide',
        name: 'أكسيد القصدير',
        scientific_name: 'Tin Oxide',
        searchKeywords: ['tin oxide', 'أكسيد القصدير'],
        usageScope: ['hair', 'skin'],
        functionalCategory: 'محسن جمالي',
        chemicalType: 'ملون معدني',
        benefits: { 'يعزز لمعان الميكا': 0.9 },
        synergy: { 'mica': 1.05 },
        negativeSynergy: {},
        warnings: [
          { level: 'info', text: 'يستخدم لتغليف جزيئات الميكا لإعطاء تأثير لامع ومتلألئ.' }
        ],
      },
{
        id: 'disodium-phosphate',
        name: 'ثنائي فوسفات الصوديوم',
        scientific_name: 'Disodium Phosphate',
        searchKeywords: ['disodium phosphate', 'ثنائي فوسفات الصوديوم'],
        usageScope: ['hair', 'skin'],
        functionalCategory: 'محسن قوام',
        chemicalType: 'عامل منظم للحموضة',
        benefits: { 'تنظيم درجة الحموضة': 0.9 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'dipropylene-glycol',
        name: 'ثنائي بروبيلين جلايكول',
        scientific_name: 'Dipropylene Glycol',
        searchKeywords: ['dipropylene glycol', 'dpg'],
        usageScope: ['hair', 'skin'],
        functionalCategory: 'مرطب / مطري',
        chemicalType: 'مرطب',
        benefits: { 'مذيب للعطور': 0.9, 'يقلل لزوجة المنتج': 0.85, 'مرطب خفيف': 0.8 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'peg-8',
        name: 'PEG-8',
        scientific_name: 'PEG-8',
        searchKeywords: ['peg-8', 'peg 8'],
        usageScope: ['hair', 'skin'],
        functionalCategory: 'مرطب / مطري',
        chemicalType: 'مرطب',
        benefits: { 'مرطب': 0.85, 'مذيب': 0.8 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'peg-12',
        name: 'PEG-12',
        scientific_name: 'PEG-12',
        searchKeywords: ['peg-12', 'peg 12'],
        usageScope: ['hair', 'skin'],
        functionalCategory: 'مرطب / مطري',
        chemicalType: 'مرطب',
        benefits: { 'مرطب': 0.85, 'مطري': 0.8 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'pentylene-glycol',
        name: 'بنتيلين جليكول',
        scientific_name: 'Pentylene Glycol',
        searchKeywords: ['pentylene glycol', 'بنتيلين جليكول'],
        usageScope: ['hair', 'skin'],
        functionalCategory: 'مرطب / مطري',
        chemicalType: 'مرطب',
        benefits: { 'ترطيب': 0.88, 'تحسين امتصاص المكونات': 0.85, 'مضاد للميكروبات': 0.8 },
        synergy: {},
        negativeSynergy: {},
        warnings: [
          { level: 'info', text: 'يعمل أيضا كمعزز للمواد الحافظة.' }
        ],
      },
{
        id: 'caprylyl-glycol',
        name: 'كابريليل جليكول',
        scientific_name: 'Caprylyl Glycol',
        searchKeywords: ['caprylyl glycol', 'كابريليل جليكول'],
        usageScope: ['hair', 'skin'],
        functionalCategory: 'مرطب / مطري',
        chemicalType: 'مرطب',
        benefits: { 'ترطيب': 0.9, 'معزز للمواد الحافظة': 0.88, 'مضاد للميكروبات': 0.85, 'مطري': 0.82 },
        synergy: {},
        negativeSynergy: {},
        warnings: [
          { level: 'info', text: 'مرطب فعال جدا يعمل أيضا كجزء من نظام الحفظ في العديد من التركيبات الخالية من البارابين.' }
        ],
      },
{
        id: 'hexylene-glycol',
        name: 'هيكسيلين جلايكول',
        scientific_name: 'Hexylene Glycol',
        searchKeywords: ['hexylene glycol', 'هيكسيلين جلايكول'],
        usageScope: ['hair', 'skin'],
        functionalCategory: 'مرطب / مطري',
        chemicalType: 'مرطب',
        benefits: { 'مذيب': 0.9, 'يقلل اللزوجة': 0.85 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: '1,2-hexanediol',
        name: '1،2-هيكسانيديول',
        scientific_name: '1,2-Hexanediol',
        searchKeywords: ['1,2-hexanediol', 'hexanediol', 'هيكسانيديول'],
        usageScope: ['hair', 'skin'],
        functionalCategory: 'مرطب / مطري',
        chemicalType: 'مرطب',
        benefits: { 'ترطيب': 0.88, 'معزز للمواد الحافظة': 0.9, 'مذيب': 0.85 },
        synergy: { 'caprylyl-glycol': 1.05 },
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'honey',
        name: 'العسل',
        scientific_name: 'Honey / Mel',
        searchKeywords: ['honey', 'mel', 'عسل'],
        usageScope: ['hair', 'skin'],
        functionalCategory: 'مرطب / مطري',
        chemicalType: 'مرطب طبيعي',
        benefits: { 'ترطيب': 0.92, 'مضاد للميكروبات': 0.85, 'مضاد أكسدة': 0.8, 'ملطف ومرمم': 0.88 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'hydrolyzed-honey-protein',
        name: 'بروتين العسل المهدرج',
        scientific_name: 'Hydrolyzed Honey Protein',
        searchKeywords: ['hydrolyzed honey protein', 'بروتين العسل'],
        usageScope: ['hair'],
        functionalCategory: 'مرطب / مطري',
        chemicalType: 'بروتين',
        benefits: { 'ترطيب عميق': 0.9, 'تحسين مرونة الشعر': 0.85 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'saponaria-officinalis-extract',
        name: 'مستخلص عرق الحلاوة',
        scientific_name: 'Saponaria Officinalis Leaf/Root Extract',
        searchKeywords: ['saponaria officinalis', 'soapwort', 'عرق الحلاوة'],
        usageScope: ['hair'],
        functionalCategory: 'منظف',
        chemicalType: 'منظف طبيعي',
        benefits: { 'منظف طبيعي لطيف': 0.8, 'رغوة خفيفة': 0.7 },
        synergy: {},
        negativeSynergy: {},
        warnings: [
          { level: 'info', text: 'نبات غني بالصابونين، وهو منظف طبيعي يستخدم في الشامبوهات التقليدية.' }
        ],
      },
{
        id: 'sapindus-mukorossi-extract',
        name: 'مستخلص الريثا (بندق الصابون)',
        scientific_name: 'Sapindus Mukorossi Fruit Extract',
        searchKeywords: ['sapindus mukorossi', 'reetha', 'soapnut', 'ريثا', 'بندق الصابون'],
        usageScope: ['hair'],
        functionalCategory: 'منظف',
        chemicalType: 'منظف طبيعي',
        benefits: { 'منظف طبيعي لطيف جدا': 0.85, 'مضاد للميكروبات': 0.75 },
        synergy: {},
        negativeSynergy: {},
        warnings: [
          { level: 'info', text: 'فاكهة غنية بالصابونين، تستخدم كبديل طبيعي للشامبو في طب الأيورفيدا.' }
        ],
      },
{
        id: 'acacia-concinna-extract',
        name: 'مستخلص الشيكاكاي',
        scientific_name: 'Acacia Concinna Fruit Extract',
        searchKeywords: ['acacia concinna', 'shikakai', 'شيكاكاي'],
        usageScope: ['hair'],
        functionalCategory: 'منظف',
        chemicalType: 'منظف طبيعي',
        benefits: { 'تنظيف لطيف': 0.8, 'تكييف الشعر': 0.85, 'توازن حموضة الفروة': 0.82 },
        synergy: {},
        negativeSynergy: {},
        warnings: [
          { level: 'info', text: 'عشب أيورفيدي آخر يستخدم لتنظيف الشعر بلطف دون تجريده من زيوته.' }
        ],
      },
{
        id: 'sodium-cocoyl-glutamate',
        name: 'صوديوم كوكويل جلوتامات',
        scientific_name: 'Sodium Cocoyl Glutamate',
        searchKeywords: ['sodium cocoyl glutamate', 'صوديوم كوكويل جلوتامات'],
        usageScope: ['hair', 'skin'],
        functionalCategory: 'منظف',
        chemicalType: 'منظف لطيف جدا',
        benefits: { 'تنظيف لطيف مشتق من الأحماض الأمينية': 0.92, 'يحافظ على رطوبة البشرة': 0.88, 'رغوة كريمية': 0.85 },
        synergy: {},
        negativeSynergy: {},
        warnings: [
          { level: 'info', text: 'منظف لطيف جدا، مناسب للتركيبات الطبيعية والحساسة.' }
        ],
      },
{
        id: 'disodium-cocoyl-glutamate',
        name: 'ثنائي الصوديوم كوكويل جلوتامات',
        scientific_name: 'Disodium Cocoyl Glutamate',
        searchKeywords: ['disodium cocoyl glutamate', 'ثنائي الصوديوم كوكويل جلوتامات'],
        usageScope: ['hair', 'skin'],
        functionalCategory: 'منظف',
        chemicalType: 'منظف لطيف جدا',
        benefits: { 'تنظيف فائق اللطف': 0.95, 'مناسب للبشرة الحساسة': 0.9, 'رغوة ناعمة': 0.8 },
        synergy: { 'cocamidopropyl-betaine': 1.05 },
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'sodium-cocoyl-glycinate',
        name: 'صوديوم كوكويل جليسينات',
        scientific_name: 'Sodium Cocoyl Glycinate',
        searchKeywords: ['sodium cocoyl glycinate', 'كوكويل جليسينات'],
        usageScope: ['skin', 'hair'],
        functionalCategory: 'منظف',
        chemicalType: 'منظف لطيف جدا',
        benefits: { 'تنظيف لطيف جدا': 0.95, 'رغوة كريمية ناعمة': 0.9, 'يحافظ على رطوبة البشرة': 0.88 },
        synergy: {},
        negativeSynergy: {},
        warnings: [
          { level: 'info', text: 'مشتق من حمض الجلايسين الأميني، يعطي إحساسا فاخرا للرغوة ولا يجرد البشرة.' }
        ],
      },
{
        id: 'potassium-cocoyl-glycinate',
        name: 'بوتاسيوم كوكويل جليسينات',
        scientific_name: 'Potassium Cocoyl Glycinate',
        searchKeywords: ['potassium cocoyl glycinate', 'بوتاسيوم كوكويل جليسينات'],
        usageScope: ['skin', 'hair'],
        functionalCategory: 'منظف',
        chemicalType: 'منظف لطيف جدا',
        benefits: { 'تنظيف لطيف': 0.92, 'رغوة غنية': 0.88, 'يحافظ على توازن الحموضة': 0.85 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'sodium-lauroyl-oat-amino-acids',
        name: 'صوديوم لورويل أحماض الشوفان الأمينية',
        scientific_name: 'Sodium Lauroyl Oat Amino Acids',
        searchKeywords: ['sodium lauroyl oat amino acids', 'oat amino acids', 'أحماض الشوفان الأمينية'],
        usageScope: ['hair', 'skin'],
        functionalCategory: 'منظف',
        chemicalType: 'منظف لطيف جدا',
        benefits: { 'تنظيف لطيف جدا': 0.95, 'ملطف ومرمم': 0.88, 'رغوة كريمية': 0.8 },
        synergy: {},
        negativeSynergy: {},
        warnings: [
          { level: 'info', text: 'منظف فائق اللطف مشتق من الشوفان، مثالي لفروة الرأس الحساسة والمتهيجة.' }
        ],
      },
{
        id: 'sodium-cocoyl-apple-amino-acids',
        name: 'صوديوم كوكويل أحماض التفاح الأمينية',
        scientific_name: 'Sodium Cocoyl Apple Amino Acids',
        searchKeywords: ['sodium cocoyl apple amino acids', 'apple amino acids', 'أحماض التفاح الأمينية'],
        usageScope: ['hair', 'skin'],
        functionalCategory: 'منظف',
        chemicalType: 'منظف لطيف جدا',
        benefits: { 'تنظيف لطيف جدا': 0.92, 'رغوة غنية وناعمة': 0.9 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'sodium-lauryl-glucose-carboxylate',
        name: 'صوديوم لوريل جلوكوز كاربوكسيلات',
        scientific_name: 'Sodium Lauryl Glucose Carboxylate',
        searchKeywords: ['sodium lauryl glucose carboxylate', 'lauryl glucoside carboxylate'],
        usageScope: ['hair'],
        functionalCategory: 'منظف',
        chemicalType: 'منظف لطيف',
        benefits: { 'تنظيف لطيف': 0.9, 'رغوة جيدة': 0.85, 'قابل للتحلل': 0.9 },
        synergy: { 'lauryl-glucoside': 1.05 },
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'sodium-myreth-sulfate',
        name: 'صوديوم ميريث سلفات',
        scientific_name: 'Sodium Myreth Sulfate',
        searchKeywords: ['sodium myreth sulfate', 'صوديوم ميريث سلفات'],
        usageScope: ['hair'],
        functionalCategory: 'منظف',
        chemicalType: 'منظف قوي',
        benefits: { 'تنظيف فعال': 0.88, 'رغوة جيدة': 0.85 },
        synergy: {},
        negativeSynergy: {},
        warnings: [
          { level: 'caution', text: 'بديل لـ SLES، ولكنه لا يزال من عائلة السلفات القوية وقد يكون مجففا.' }
        ],
      },
{
        id: 'tea-lauryl-sulfate',
        name: 'TEA-لوريل سلفات',
        scientific_name: 'TEA-Lauryl Sulfate',
        searchKeywords: ['tea-lauryl sulfate', 'triethanolamine lauryl sulfate'],
        usageScope: ['hair'],
        functionalCategory: 'منظف',
        chemicalType: 'منظف قوي',
        benefits: { 'تنظيف فعال': 0.85, 'رغوة جيدة': 0.8 },
        synergy: {},
        negativeSynergy: {},
        warnings: [
          { level: 'caution', text: 'لا يزال من عائلة السلفات، وقد يكون مجففا.' }
        ],
      },
{
        id: 'sodium-coco-sulfate',
        name: 'صوديوم كوكو سلفات (SCS)',
        scientific_name: 'Sodium Coco-Sulfate',
        searchKeywords: ['sodium coco-sulfate', 'scs', 'صوديوم كوكو سلفات'],
        usageScope: ['hair'],
        functionalCategory: 'منظف',
        chemicalType: 'منظف قوي',
        benefits: { 'تنظيف قوي': 0.9, 'رغوة غنية': 0.88, 'مشتق من جوز الهند': 0.8 },
        synergy: {},
        negativeSynergy: {},
        warnings: [
          { level: 'caution', text: 'غالبا ما يتم تسويقه كبديل "طبيعي" لـ SLS، ولكنه كيميائيا مزيج من السلفات (بما في ذلك SLS). لا يزال منظفا قويا وقد يكون مجففا.' }
        ],
      },
{
        id: 'polyquaternium-6',
        name: 'بولي كواتيرنيوم-6',
        scientific_name: 'Polyquaternium-6',
        searchKeywords: ['polyquaternium-6', 'pq-6', 'بولي كواتيرنيوم 6'],
        usageScope: ['hair'],
        functionalCategory: 'مكيف للشعر',
        chemicalType: 'بوليمر',
        benefits: { 'تكييف ممتاز': 0.88, 'يعطي ملمسا ناعما': 0.9, 'فك تشابك الشعر': 0.85 },
        synergy: {},
        negativeSynergy: {},
        warnings: [
          { level: 'caution', text: 'قد يتراكم على الشعر الناعم.' }
        ],
      },
{
        id: 'polyquaternium-37',
        name: 'بولي كواتيرنيوم-37',
        scientific_name: 'Polyquaternium-37',
        searchKeywords: ['polyquaternium-37', 'pq-37'],
        usageScope: ['hair'],
        functionalCategory: 'مكيف للشعر',
        chemicalType: 'بوليمر',
        benefits: { 'تكييف ممتاز': 0.9, 'مكثف فعال': 0.88, 'يقلل الهيشان': 0.85 },
        synergy: {},
        negativeSynergy: {},
        warnings: [
          { level: 'caution', text: 'مكيف ومكثف قوي جدا، قد يتراكم على الشعر.' }
        ],
      },
{
        id: 'polyquaternium-44',
        name: 'بولي كواتيرنيوم-44',
        scientific_name: 'Polyquaternium-44',
        searchKeywords: ['polyquaternium-44', 'pq-44'],
        usageScope: ['hair'],
        functionalCategory: 'مكيف للشعر',
        chemicalType: 'بوليمر',
        benefits: { 'إضافة حجم للشعر': 0.9, 'تكييف خفيف': 0.85, 'يسهل التصفيف': 0.8 },
        synergy: {},
        negativeSynergy: {},
        warnings: [
          { level: 'info', text: 'ممتاز للشعر الناعم لأنه يوفر تكييفا مع حجم إضافي دون إثقال الشعر.' }
        ],
      },
{
        id: 'polyquaternium-55',
        name: 'بولي كواتيرنيوم-55',
        scientific_name: 'Polyquaternium-55',
        searchKeywords: ['polyquaternium-55', 'pq-55'],
        usageScope: ['hair'],
        functionalCategory: 'مكيف للشعر',
        chemicalType: 'بوليمر',
        benefits: { 'حماية اللون': 0.9, 'تثبيت التموجات': 0.88, 'حماية من الحرارة': 0.85 },
        synergy: {},
        negativeSynergy: {},
        warnings: [
          { level: 'info', text: 'بوليمر متعدد الوظائف، مثالي للشعر المصبوغ والمجعد.' }
        ],
      },
{
        id: 'polyquaternium-67',
        name: 'بولي كواتيرنيوم-67',
        scientific_name: 'Polyquaternium-67',
        searchKeywords: ['polyquaternium-67', 'pq-67'],
        usageScope: ['hair'],
        functionalCategory: 'مكيف للشعر',
        chemicalType: 'بوليمر طبيعي',
        benefits: { 'تكييف لطيف': 0.85, 'مشتق من السليلوز': 0.9, 'فك تشابك الشعر': 0.8 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'quaternium-80',
        name: 'كواتيرنيوم-80',
        scientific_name: 'Quaternium-80',
        searchKeywords: ['quaternium-80'],
        usageScope: ['hair'],
        functionalCategory: 'مكيف للشعر',
        chemicalType: 'سيليكون رباعي',
        benefits: { 'تكييف ممتاز': 0.9, 'تنعيم الشعر': 0.92, 'قابل للذوبان في الماء': 0.9 },
        synergy: {},
        negativeSynergy: {},
        warnings: [
          { level: 'info', text: 'سيليكون معدل (quat) قابل للذوبان في الماء، يوفر تكييفا ممتازا دون تراكم.' }
        ],
      },
{
        id: 'silicone-quaternium-16',
        name: 'سيليكون كواتيرنيوم-16',
        scientific_name: 'Silicone Quaternium-16',
        searchKeywords: ['silicone quaternium-16'],
        usageScope: ['hair'],
        functionalCategory: 'مكيف للشعر',
        chemicalType: 'سيليكون أميني',
        benefits: { 'حماية اللون': 0.92, 'حماية من الحرارة': 0.9, 'تكييف خفيف': 0.85 },
        synergy: {},
        negativeSynergy: {},
        warnings: [
          { level: 'info', text: 'سيليكون أميني لا يتراكم بسهولة ويحمي الشعر من التلف.' }
        ],
      },
{
        id: 'bis-aminopropyl-dimethicone',
        name: 'بيس-أمينوبروبيل دايميثيكون',
        scientific_name: 'Bis-Aminopropyl Dimethicone',
        searchKeywords: ['bis-aminopropyl dimethicone'],
        usageScope: ['hair'],
        functionalCategory: 'مكيف للشعر',
        chemicalType: 'سيليكون',
        benefits: { 'إصلاح الشعر التالف': 0.95, 'حماية اللون': 0.9, 'تكييف طويل الأمد': 0.92 },
        synergy: {},
        negativeSynergy: {},
        warnings: [
          { level: 'info', text: 'سيليكون أميني متقدم يلتصق بقوة بالمناطق التالفة من الشعر لإصلاحها، ويوفر تكييفا دائما حتى بعد عدة غسلات.' }
        ],
      },
{
        id: 'brassicamidopropyl-dimethylamine',
        name: 'براسيكاميدوبروبيل ثنائي ميثيل أمين',
        scientific_name: 'Brassicamidopropyl Dimethylamine',
        searchKeywords: ['brassicamidopropyl dimethylamine'],
        usageScope: ['hair'],
        functionalCategory: 'مكيف للشعر',
        chemicalType: 'أمين دهني',
        benefits: { 'بديل طبيعي للكواتس': 0.92, 'فك تشابك الشعر': 0.9, 'قابل للتحلل': 0.95 },
        synergy: {},
        negativeSynergy: {},
        warnings: [
          { level: 'info', text: 'مشتق من زيت بذور اللفت (Brassica)، وهو بديل ممتاز وصديق للبيئة للمكيفات التقليدية.' }
        ],
      },
{
        id: 'isostearamidopropyl-morpholine-lactate',
        name: 'أيزوستيراميدوبروبيل مورفولين لاكتات',
        scientific_name: 'Isostearamidopropyl Morpholine Lactate',
        searchKeywords: ['isostearamidopropyl morpholine lactate'],
        usageScope: ['hair'],
        functionalCategory: 'مكيف للشعر',
        chemicalType: 'مركب أمونيوم رباعي',
        benefits: { 'مضاد للكهرباء الساكنة': 0.9, 'تكييف': 0.85 },
        synergy: {},
        negativeSynergy: {},
        warnings: [
          { level: 'info', text: 'يستخدم بشكل خاص لتقليل الهيشان والكهرباء الساكنة.' }
        ],
      },
{
        id: 'peg-ppg-18-18-dimethicone',
        name: 'PEG/PPG-18/18 دايميثيكون',
        scientific_name: 'PEG/PPG-18/18 Dimethicone',
        searchKeywords: ['peg/ppg-18/18 dimethicone'],
        usageScope: ['hair'],
        functionalCategory: 'مكيف للشعر',
        chemicalType: 'سيليكون قابل للذوبان في الماء',
        benefits: { 'تكييف خفيف': 0.85, 'ترطيب': 0.8, 'مستحلب': 0.82 },
        synergy: {},
        negativeSynergy: {},
        warnings: [
          { level: 'info', text: 'سيليكون قابل للذوبان في الماء يعمل كعامل تكييف ومستحلب.' }
        ],
      },
{
        id: 'polyimide-1',
        name: 'بولي إيميد-1',
        scientific_name: 'Polyimide-1',
        searchKeywords: ['polyimide-1'],
        usageScope: ['hair'],
        functionalCategory: 'مثبت للشعر',
        chemicalType: 'بوليمر',
        benefits: { 'مضاد للرطوبة': 0.9, 'تثبيت التصفيف': 0.85, 'تقليل الهيشان': 0.88 },
        synergy: {},
        negativeSynergy: {},
        warnings: [
          { level: 'info', text: 'بوليمر يستخدم في منتجات التصفيف لتوفير ثبات ومقاومة للرطوبة.' }
        ],
      },
{
        id: 'hydrolyzed-baobab-protein',
        name: 'بروتين الباوباب المهدرج',
        scientific_name: 'Hydrolyzed Adansonia Digitata Seed Extract',
        searchKeywords: ['hydrolyzed baobab protein', 'baobab protein', 'بروتين الباوباب'],
        usageScope: ['hair'],
        functionalCategory: 'تقوية الشعر',
        chemicalType: 'بروتين',
        benefits: { 'مكافحة الشيخوخة للشعر': 0.9, 'حماية من التلف': 0.92, 'ترطيب': 0.88 },
        synergy: {},
        negativeSynergy: {},
        warnings: [
          { level: 'info', text: 'بروتين قوي يحمي الشعر من أضرار الأشعة فوق البنفسجية والحرارة والتلوث.' }
        ],
      },
{
        id: 'hydrolyzed-quinoa-protein',
        name: 'بروتين الكينوا المهدرج',
        scientific_name: 'Hydrolyzed Quinoa',
        searchKeywords: ['hydrolyzed quinoa', 'quinoa protein', 'بروتين الكينوا'],
        usageScope: ['hair'],
        functionalCategory: 'تقوية الشعر',
        chemicalType: 'بروتين',
        benefits: { 'حماية اللون': 0.95, 'إصلاح الشعر': 0.9, 'تقوية الشعر': 0.88 },
        synergy: {},
        negativeSynergy: {},
        warnings: [
          { level: 'info', text: 'يحتوي على جميع الأحماض الأمينية الأساسية الثمانية، وهو ممتاز للشعر المصبوغ لتعزيز اللون وإصلاح التلف.' }
        ],
      },
{
        id: 'hydrolyzed-lupine-protein',
        name: 'بروتين الترمس المهدرج',
        scientific_name: 'Hydrolyzed Lupine Protein',
        searchKeywords: ['hydrolyzed lupine protein', 'lupine protein', 'بروتين الترمس'],
        usageScope: ['hair'],
        functionalCategory: 'تقوية الشعر',
        chemicalType: 'بروتين',
        benefits: { 'تقليل تساقط الشعر': 0.85, 'تحفيز نمو الشعر': 0.8, 'تقوية الشعر': 0.82 },
        synergy: {},
        negativeSynergy: {},
        warnings: [
          { level: 'risk', text: 'يجب تجنبه من قبل الأشخاص الذين يعانون من حساسية الترمس أو البقوليات.' }
        ],
      },
{
        id: 'laurdimonium-hydroxypropyl-hydrolyzed-wheat-protein',
        name: 'بروتين القمح المهدرج لورديمونيوم هيدروكسي بروبيل',
        scientific_name: 'Laurdimonium Hydroxypropyl Hydrolyzed Wheat Protein',
        searchKeywords: ['laurdimonium hydroxypropyl hydrolyzed wheat protein'],
        usageScope: ['hair'],
        functionalCategory: 'تقوية الشعر',
        chemicalType: 'بروتين معدل',
        benefits: { 'تكييف وإصلاح': 0.9, 'تقليل التقصف': 0.85, 'حماية الشعر': 0.88 },
        synergy: {},
        negativeSynergy: {},
        warnings: [
          { level: 'info', text: 'شكل معدل من بروتين القمح يلتصق بالشعر بشكل أفضل لتوفير تكييف وإصلاح مستهدف.' }
        ],
      },
{
        id: 'cocodimonium-hydroxypropyl-hydrolyzed-keratin',
        name: 'كيراتين مهدرج كوكوديمونيوم هيدروكسي بروبيل',
        scientific_name: 'Cocodimonium Hydroxypropyl Hydrolyzed Keratin',
        searchKeywords: ['cocodimonium hydroxypropyl hydrolyzed keratin'],
        usageScope: ['hair'],
        functionalCategory: 'تقوية الشعر',
        chemicalType: 'بروتين معدل',
        benefits: { 'إصلاح مستهدف للشعر التالف': 0.95, 'تقوية الشعر': 0.92, 'تنعيم الشعر': 0.9 },
        synergy: {},
        negativeSynergy: {},
        warnings: [
          { level: 'info', text: 'شكل معدل من الكيراتين يلتصق بالمناطق التالفة من الشعر لإصلاحها بفعالية.' }
        ],
      },
{
        id: 'potassium-cocoyl-hydrolyzed-collagen',
        name: 'كولاجين مهدرج بوتاسيوم كوكويل',
        scientific_name: 'Potassium Cocoyl Hydrolyzed Collagen',
        searchKeywords: ['potassium cocoyl hydrolyzed collagen'],
        usageScope: ['hair'],
        functionalCategory: 'منظف',
        chemicalType: 'منظف لطيف',
        benefits: { 'تنظيف لطيف': 0.85, 'تكييف خفيف': 0.8 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'apigenin',
        name: 'أبيجينين',
        scientific_name: 'Apigenin',
        searchKeywords: ['apigenin', 'أبيجينين'],
        usageScope: ['hair'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'مركب نباتي',
        benefits: { 'تحسين الدورة الدموية في الفروة': 0.85, 'مضاد للالتهابات': 0.8, 'مضاد أكسدة': 0.82 },
        synergy: { 'oleanolic-acid': 1.1 },
        negativeSynergy: {},
        warnings: [
          { level: 'info', text: 'فلافونويد موجود في البقدونس والكرفس، يستخدم في علاجات تساقط الشعر لتحسين صحة البصيلات.' }
        ],
      },
{
        id: 'oleanolic-acid',
        name: 'حمض الأوليانوليك',
        scientific_name: 'Oleanolic Acid',
        searchKeywords: ['oleanolic acid', 'حمض الأوليانوليك'],
        usageScope: ['hair'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'مركب نباتي',
        benefits: { 'مثبط لإنزيم 5-alpha-reductase': 0.88, 'تقليل تساقط الشعر': 0.85 },
        synergy: { 'apigenin': 1.1 },
        negativeSynergy: {},
        warnings: [
          { level: 'info', text: 'مستخلص من أوراق الزيتون، يستخدم لمكافحة تساقط الشعر الوراثي.' }
        ],
      },
{
        id: 'ziziphus-joazeiro-bark-extract',
        name: 'مستخلص لحاء السدر',
        scientific_name: 'Ziziphus Joazeiro Bark Extract',
        searchKeywords: ['ziziphus joazeiro', 'jua extract', 'مستخلص السدر'],
        usageScope: ['hair'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'مستخلص نباتي',
        benefits: { 'مضاد للقشرة طبيعي': 0.9, 'ملطف ومرمم': 0.85, 'تنظيف لطيف': 0.8 },
        synergy: {},
        negativeSynergy: {},
        warnings: [
          { level: 'info', text: 'بديل نباتي فعال لعوامل القشرة الكيميائية مثل بيريثيون الزنك.' }
        ],
      },
{
        id: 'epilobium-angustifolium-extract',
        name: 'مستخلص زهرة الصفصاف',
        scientific_name: 'Epilobium Angustifolium Flower/Leaf/Stem Extract',
        searchKeywords: ['epilobium angustifolium', 'willowherb', 'زهرة الصفصاف'],
        usageScope: ['hair', 'skin'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'مستخلص نباتي',
        benefits: { 'ملطف ومرمم': 0.9, 'مضاد للالتهابات': 0.88, 'تنظيم الإفرازات الدهنية': 0.85 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'saccharum-officinarum-extract',
        name: 'مستخلص قصب السكر',
        scientific_name: 'Saccharum Officinarum (Sugarcane) Extract',
        searchKeywords: ['sugarcane extract', 'saccharum officinarum', 'مستخلص قصب السكر'],
        usageScope: ['hair'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'مستخلص نباتي',
        benefits: { 'مصدر طبيعي لحمض الجليكوليك': 0.85, 'تقشير لطيف للفروة': 0.8, 'ترطيب': 0.75 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'citrus-limon-fruit-extract',
        name: 'مستخلص فاكهة الليمون',
        scientific_name: 'Citrus Limon (Lemon) Fruit Extract',
        searchKeywords: ['lemon fruit extract', 'citrus limon'],
        usageScope: ['hair'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'مستخلص نباتي',
        benefits: { 'إزالة التراكمات': 0.8, 'إضافة لمعان': 0.85, 'مصدر لفيتامين C': 0.7 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'pyrus-malus-fruit-extract',
        name: 'مستخلص فاكهة التفاح',
        scientific_name: 'Pyrus Malus (Apple) Fruit Extract',
        searchKeywords: ['apple fruit extract', 'pyrus malus', 'مستخلص التفاح'],
        usageScope: ['hair'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'مستخلص نباتي',
        benefits: { 'مضاد أكسدة': 0.8, 'تقشير لطيف للفروة': 0.75, 'صحة فروة الرأس': 0.8 },
        synergy: {},
        negativeSynergy: {},
        warnings: [
          { level: 'info', text: 'مصدر طبيعي لحمض الماليك.' }
        ],
      },
{
        id: 'saccharomyces-cerevisiae-extract',
        name: 'مستخلص خميرة البيرة',
        scientific_name: 'Saccharomyces Cerevisiae Extract',
        searchKeywords: ['saccharomyces cerevisiae', 'yeast extract', 'خميرة البيرة'],
        usageScope: ['hair'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'مكون مخمر',
        benefits: { 'غني بالفيتامينات B': 0.88, 'تقوية الشعر': 0.85, 'مضاد أكسدة': 0.8 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'burdock-root-extract',
        name: 'مستخلص جذور الأرقطيون',
        scientific_name: 'Arctium Lappa Root Extract',
        searchKeywords: ['burdock root extract', 'arctium lappa', 'الأرقطيون'],
        usageScope: ['hair'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'مستخلص نباتي',
        benefits: { 'صحة فروة الرأس': 0.85, 'تقليل تساقط الشعر': 0.8, 'تنظيم الإفرازات الدهنية': 0.75 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'sage-leaf-extract',
        name: 'مستخلص أوراق المريمية',
        scientific_name: 'Salvia Officinalis (Sage) Leaf Extract',
        searchKeywords: ['sage leaf extract', 'salvia officinalis', 'مريمية'],
        usageScope: ['hair'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'مستخلص نباتي',
        benefits: { 'مضاد للميكروبات': 0.8, 'مضاد أكسدة': 0.85, 'تنظيم الإفرازات الدهنية': 0.75 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'calendula-officinalis-flower-extract',
        name: 'مستخلص زهرة الآذريون',
        scientific_name: 'Calendula Officinalis Flower Extract',
        searchKeywords: ['calendula extract', 'calendula officinalis'],
        usageScope: ['hair', 'skin'],
        functionalCategory: 'ملطف ومرمم',
        chemicalType: 'مستخلص نباتي',
        benefits: { 'ملطف ومرمم': 0.92, 'تهدئة فروة الرأس': 0.9, 'مضاد للالتهابات': 0.88 },
        synergy: {},
        negativeSynergy: {},
        warnings: [
          { level: 'risk', text: 'يجب تجنبه من قبل الأشخاص الذين يعانون من حساسية نباتات عائلة الأقحوان.' }
        ],
      },
{ id: 'sodium-sulfate', name: 'كبريتات الصوديوم', scientific_name: 'Sodium Sulfate', searchKeywords: ['sodium sulfate'], usageScope: ['hair'], functionalCategory: 'محسن قوام', chemicalType: 'ملح', benefits: {'مكثف للقوام': 0.85}, synergy: {}, negativeSynergy: {}, warnings: [{level: 'caution', text: 'ليس منظفا، بل ملح يستخدم لزيادة لزوجة الشامبو. قد يكون مجففا.'}]},
{ id: 'ppg-9', name: 'PPG-9', scientific_name: 'PPG-9', searchKeywords: ['ppg-9', 'ppg 9'], usageScope: ['hair'], functionalCategory: 'مكيف للشعر', chemicalType: 'بوليمر', benefits: {'يعطي ملمسا ناعما': 0.8, 'مذيب': 0.85}, synergy: {}, negativeSynergy: {}, warnings: []},
{ id: 'laureth-23', name: 'لوريث-23', scientific_name: 'Laureth-23', searchKeywords: ['laureth-23', 'لوريث 23'], usageScope: ['hair', 'skin'], functionalCategory: 'مستحلب', chemicalType: 'مستحلب', benefits: {'مستحلب': 0.85, 'عامل منظف': 0.8}, synergy: {}, negativeSynergy: {}, warnings: []},
{ id: 'laureth-4', name: 'لوريث-4', scientific_name: 'Laureth-4', searchKeywords: ['laureth-4', 'لوريث 4'], usageScope: ['hair', 'skin'], functionalCategory: 'مستحلب', chemicalType: 'مستحلب', benefits: {'مستحلب (يخلط الزيت بالماء)': 0.9, 'عامل رغوي': 0.75}, synergy: {}, negativeSynergy: {}, warnings: [{level: 'caution', text: 'قد يكون كوميدوجينيك ويسد المسام إذا استخدم في منتجات تترك على البشرة.'}]},
{ id: 'peg-150-pentaerythrityl-tetrastearate', name: 'PEG-150 بنتاريثريتيل تتراستيرات', scientific_name: 'PEG-150 Pentaerythrityl Tetrastearate', searchKeywords: ['peg-150 pentaerythrityl tetrastearate'], usageScope: ['hair'], functionalCategory: 'محسن قوام', chemicalType: 'بوليمر', benefits: {'مكثف فعال جدا': 0.9, 'يعطي ملمسا فاخرا': 0.85}, synergy: {}, negativeSynergy: {}, warnings: []},
{ id: 'ppg-2-hydroxyethyl-cocamide', name: 'PPG-2 هيدروكسي إيثيل كوكاميد', scientific_name: 'PPG-2 Hydroxyethyl Cocamide', searchKeywords: ['ppg-2 hydroxyethyl cocamide'], usageScope: ['hair'], functionalCategory: 'محسن قوام', chemicalType: 'أمين دهني', benefits: {'مكثف للقوام': 0.88, 'معزز للرغوة': 0.85}, synergy: {}, negativeSynergy: {}, warnings: []},
{ id: 'maleic-acid', name: 'حمض الماليك (للروابط)', scientific_name: 'Maleic Acid', searchKeywords: ['maleic acid', 'bond repair'], usageScope: ['hair'], functionalCategory: 'تقوية الشعر', chemicalType: 'حمض عضوي', benefits: {'إصلاح روابط الشعر': 0.92, 'تقوية الشعر التالف': 0.9, 'حماية من التلف الكيميائي': 0.88}, synergy: {}, negativeSynergy: {}, warnings: [{level: 'info', text: 'يستخدم في تقنيات إصلاح روابط الشعر (مثل Olaplex) للمساعدة في إعادة توصيل الروابط ثنائية الكبريتيد المكسورة داخل الشعر.'}]},
{ id: 'bis-cetearyl-amodimethicone', name: 'بيس-سيتريل أمodimethicone', scientific_name: 'Bis-Cetearyl Amodimethicone', searchKeywords: ['bis-cetearyl amodimethicone'], usageScope: ['hair'], functionalCategory: 'مكيف للشعر', chemicalType: 'سيليكون', benefits: {'تكييف عميق': 0.9, 'تنعيم الشعر': 0.88, 'حماية اللون': 0.85}, synergy: {}, negativeSynergy: {}, warnings: []},
{ id: 'c12-13-pareth-23', name: 'C12-13 باريث-23', scientific_name: 'C12-13 Pareth-23', searchKeywords: ['c12-13 pareth-23'], usageScope: ['hair'], functionalCategory: 'مستحلب', chemicalType: 'مستحلب', benefits: {'مستحلب': 0.8, 'منظف ثانوي': 0.75}, synergy: {}, negativeSynergy: {}, warnings: []},
{ id: 'divinyldimethicone-dimethicone-copolymer', name: 'دايفينيل دايميثيكون/دايميثيكون كوبوليمر', scientific_name: 'Divinyldimethicone/Dimethicone Copolymer', searchKeywords: ['divinyldimethicone/dimethicone copolymer'], usageScope: ['hair'], functionalCategory: 'مكيف للشعر', chemicalType: 'سيليكون', benefits: {'تكوين طبقة حماية قوية': 0.9, 'لمعان فائق': 0.88, 'مقاومة للماء': 0.85}, synergy: {}, negativeSynergy: {}, warnings: [{level: 'caution', text: 'سيليكون ثقيل جدا، يوفر حماية ولمعانا استثنائيين ولكنه يتراكم بكثرة.'}]},
{
        id: 'linoleic-acid',
        name: 'حمض اللينوليك (أوميغا 6)',
        scientific_name: 'Linoleic Acid',
        searchKeywords: ['linoleic acid', 'omega-6', 'vitamin f', 'حمض اللينوليك', 'أوميغا 6'],
        usageScope: ['skin'],
        functionalCategory: 'دعم حاجز البشرة',
        chemicalType: 'حمض دهني',
        benefits: { 'دعم حاجز البشرة': 0.95, 'مضاد للالتهابات': 0.9, 'ترطيب': 0.85, 'مكافحة حب الشباب': 0.8 },
        synergy: { 'ceramides': 1.1, 'cholesterol': 1.1 },
        negativeSynergy: {},
        warnings: [
          { level: 'info', text: 'حمض دهني أساسي. البشرة المعرضة لحب الشباب غالبا ما يكون لديها نقص فيه، مما يجعله مكونا ممتازا لإعادة توازنها.' }
        ],
      },
{
        id: 'alpha-linolenic-acid',
        name: 'حمض ألفا لينولينيك (أوميغا 3)',
        scientific_name: 'Alpha-Linolenic Acid',
        searchKeywords: ['alpha-linolenic acid', 'ala', 'omega-3', 'حمض ألفا لينولينيك', 'أوميغا 3'],
        usageScope: ['skin'],
        functionalCategory: 'ملطف ومرمم',
        chemicalType: 'حمض دهني',
        benefits: { 'مضاد للالتهابات': 0.92, 'ملطف ومرمم': 0.9, 'تقليل الاحمرار': 0.88 },
        synergy: { 'linoleic-acid': 1.05 },
        negativeSynergy: {},
        warnings: [
          { level: 'info', text: 'حمض دهني أساسي معروف بخصائصه القوية المضادة للالتهابات، ممتاز للبشرة الحساسة.' }
        ],
      },
{
        id: 'oleic-acid',
        name: 'حمض الأوليك (أوميغا 9)',
        scientific_name: 'Oleic Acid',
        searchKeywords: ['oleic acid', 'omega-9', 'حمض الأوليك', 'أوميغا 9'],
        usageScope: ['skin'],
        functionalCategory: 'مرطب / مطري',
        chemicalType: 'حمض دهني',
        benefits: { 'ترطيب': 0.9, 'تنعيم البشرة': 0.85, 'تحسين امتصاص المكونات': 0.8 },
        synergy: {},
        negativeSynergy: {},
        warnings: [
          { level: 'caution', text: 'قد يكون كوميدوجينيك (ساد للمسام) ومزعجا للبشرة المعرضة لحب الشباب أو التي تعاني من ضعف الحاجز. يوجد بكثرة في زيت الزيتون والأفوكادو.' }
        ],
      },
{
        id: 'palmitic-acid',
        name: 'حمض البالمتيك',
        scientific_name: 'Palmitic Acid',
        searchKeywords: ['palmitic acid', 'hexadecanoic acid', 'حمض البالمتيك'],
        usageScope: ['skin'],
        functionalCategory: 'مرطب / مطري',
        chemicalType: 'حمض دهني',
        benefits: { 'مطري ومنعم للبشرة': 0.85, 'يشكل طبقة حماية': 0.8, 'مستحلب مساعد': 0.75 },
        synergy: {},
        negativeSynergy: {},
        warnings: [
          { level: 'caution', text: 'أحد أكثر الأحماض الدهنية المشبعة شيوعا. قد يكون كوميدوجينيك بتركيزات عالية.' }
        ],
      },
{
        id: 'stearic-acid',
        name: 'حمض الستياريك',
        scientific_name: 'Stearic Acid',
        searchKeywords: ['stearic acid', 'octadecanoic acid', 'حمض الستياريك'],
        usageScope: ['skin'],
        functionalCategory: 'محسن قوام',
        chemicalType: 'حمض دهني',
        benefits: { 'عامل سماكة ومستحلب': 0.9, 'تنظيف البشرة': 0.8, 'تنعيم البشرة': 0.75 },
        synergy: {},
        negativeSynergy: {},
        warnings: [
          { level: 'caution', text: 'يستخدم على نطاق واسع في الكريمات لإعطاء قوام غني وشمعي. قد يكون كوميدوجينيك بشكل طفيف.' }
        ],
      },
{
        id: 'myristic-acid',
        name: 'حمض الميريستيك',
        scientific_name: 'Myristic Acid',
        searchKeywords: ['myristic acid', 'tetradecanoic acid', 'حمض الميريستيك'],
        usageScope: ['skin'],
        functionalCategory: 'منظف',
        chemicalType: 'حمض دهني',
        benefits: { 'عامل منظف ورغوي': 0.88, 'مادة تشحيم': 0.8, 'مستحلب': 0.75 },
        synergy: {},
        negativeSynergy: {},
        warnings: [
          { level: 'risk', text: 'يعتبر من الأحماض الدهنية العالية السد للمسام (كوميدوجينيك). شائع في غسولات الوجه الكريمية.' }
        ],
      },
{
        id: 'lauric-acid',
        name: 'حمض اللوريك',
        scientific_name: 'Lauric Acid',
        searchKeywords: ['lauric acid', 'dodecanoic acid', 'حمض اللوريك'],
        usageScope: ['skin'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'حمض دهني',
        benefits: { 'مضاد للميكروبات': 0.9, 'مضاد للالتهابات': 0.8, 'مكافحة حب الشباب': 0.85 },
        synergy: {},
        negativeSynergy: {},
        warnings: [
          { level: 'info', text: 'المكون الرئيسي في زيت جوز الهند، له خصائص قوية مضادة لبكتيريا حب الشباب.' },
          { level: 'caution', text: 'قد يكون مجففا عند استخدامه في المنظفات.' }
        ],
      },
{
        id: 'capric-acid',
        name: 'حمض الكابريك',
        scientific_name: 'Capric Acid',
        searchKeywords: ['capric acid', 'decanoic acid', 'حمض الكابريك'],
        usageScope: ['skin'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'حمض دهني',
        benefits: { 'مضاد للميكروبات': 0.85, 'مضاد للفطريات': 0.8, 'مضاد للالتهابات': 0.75 },
        synergy: { 'caprylic-acid': 1.05 },
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'caprylic-acid',
        name: 'حمض الكابريليك',
        scientific_name: 'Caprylic Acid',
        searchKeywords: ['caprylic acid', 'octanoic acid', 'حمض الكابريليك'],
        usageScope: ['skin'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'حمض دهني',
        benefits: { 'مضاد للفطريات': 0.88, 'مضاد للميكروبات': 0.85, 'مضاد للالتهابات': 0.8 },
        synergy: { 'capric-acid': 1.05 },
        negativeSynergy: {},
        warnings: [
          { level: 'info', text: 'فعال ضد الفطريات المسببة لحب الشباب الفطري (Malassezia).' }
        ],
      },
{
        id: 'behenic-acid',
        name: 'حمض البيهينيك',
        scientific_name: 'Behenic Acid',
        searchKeywords: ['behenic acid', 'docosanoic acid', 'حمض البيهينيك'],
        usageScope: ['hair', 'skin'],
        functionalCategory: 'محسن قوام',
        chemicalType: 'حمض دهني',
        benefits: { 'مكثف للقوام': 0.88, 'مطري ومنعم': 0.85, 'تكييف الشعر': 0.8 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'isostearic-acid',
        name: 'حمض الأيزوستيريك',
        scientific_name: 'Isostearic Acid',
        searchKeywords: ['isostearic acid', 'حمض الأيزوستيريك'],
        usageScope: ['skin'],
        functionalCategory: 'محسن قوام',
        chemicalType: 'حمض دهني',
        benefits: { 'مطري ومثبت': 0.85, 'مشتت للأصباغ': 0.8, 'عامل منظف': 0.75 },
        synergy: {},
        negativeSynergy: {},
        warnings: [
          { level: 'info', text: 'يساعد على توزيع الأصباغ وفلاتر الشمس المعدنية بشكل متساوٍ في التركيبة.' }
        ],
      },
{
        id: 'palmitoleic-acid',
        name: 'حمض البالميتوليك (أوميغا 7)',
        scientific_name: 'Palmitoleic Acid',
        searchKeywords: ['palmitoleic acid', 'omega-7', 'حمض البالميتوليك', 'أوميغا 7'],
        usageScope: ['skin'],
        functionalCategory: 'دعم حاجز البشرة',
        chemicalType: 'حمض دهني',
        benefits: { 'مضاد للميكروبات': 0.85, 'مضاد للالتهابات': 0.8, 'مكافحة الشيخوخة': 0.82 },
        synergy: {},
        negativeSynergy: {},
        warnings: [
          { level: 'info', text: 'يوجد بشكل طبيعي في دهون البشرة الشابة ويقل مع تقدم العمر. زيت المكاديميا ونبق البحر غنيان به.' }
        ],
      },
{
        id: 'arachidic-acid',
        name: 'حمض الأراكيديك',
        scientific_name: 'Arachidic Acid',
        searchKeywords: ['arachidic acid', 'eicosanoic acid', 'حمض الأراكيديك'],
        usageScope: ['skin'],
        functionalCategory: 'محسن قوام',
        chemicalType: 'حمض دهني',
        benefits: { 'مطري ومنعم': 0.8, 'مستحلب مساعد': 0.75 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'caproic-acid',
        name: 'حمض الكابرويك',
        scientific_name: 'Caproic Acid',
        searchKeywords: ['caproic acid', 'hexanoic acid', 'حمض الكابرويك'],
        usageScope: ['skin'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'حمض دهني',
        benefits: { 'مضاد للميكروبات': 0.75, 'مطري': 0.7 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'pelargonic-acid',
        name: 'حمض البيلارجونيك',
        scientific_name: 'Pelargonic Acid',
        searchKeywords: ['pelargonic acid', 'nonanoic acid', 'حمض البيلارجونيك'],
        usageScope: ['skin'],
        functionalCategory: 'مرطب / مطري',
        chemicalType: 'حمض دهني',
        benefits: { 'مطري': 0.7, 'مكون عطري': 0.65 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'arginine',
        name: 'أرجينين',
        scientific_name: 'Arginine',
        searchKeywords: ['arginine', 'أرجينين'],
        usageScope: ['skin', 'hair'],
        functionalCategory: 'دعم حاجز البشرة',
        chemicalType: 'حمض أميني',
        benefits: { 'ترطيب': 0.9, 'مضاد أكسدة': 0.8, 'إصلاح البشرة': 0.85, 'تقوية الشعر': 0.75 },
        synergy: { 'lactic-acid': 1.05 },
        negativeSynergy: {},
        warnings: [
          { level: 'info', text: 'حمض أميني أساسي يساعد في الحفاظ على ترطيب البشرة وإصلاحها.' }
        ],
      },
{
        id: 'glycine',
        name: 'جلايسين',
        scientific_name: 'Glycine',
        searchKeywords: ['glycine', 'جلايسين'],
        usageScope: ['skin', 'hair'],
        functionalCategory: 'دعم حاجز البشرة',
        chemicalType: 'حمض أميني',
        benefits: { 'إصلاح الشعر التالف': 0.85, 'ترطيب البشرة': 0.8, 'تحفيز إنتاج الكولاجين': 0.82 },
        synergy: {},
        negativeSynergy: {},
        warnings: [
          { level: 'info', text: 'أصغر حمض أميني، وهو مكون رئيسي للكولاجين.' }
        ],
      },
{
        id: 'proline',
        name: 'برولين',
        scientific_name: 'Proline',
        searchKeywords: ['proline', 'برولين'],
        usageScope: ['skin'],
        functionalCategory: 'دعم حاجز البشرة',
        chemicalType: 'حمض أميني',
        benefits: { 'دعم إنتاج الكولاجين': 0.9, 'ترطيب البشرة': 0.85, 'إصلاح حاجز البشرة': 0.8 },
        synergy: { 'glycine': 1.05, 'vitamin-c-serum': 1.08 },
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'serine',
        name: 'سيرين',
        scientific_name: 'Serine',
        searchKeywords: ['serine', 'سيرين'],
        usageScope: ['skin', 'hair'],
        functionalCategory: 'دعم حاجز البشرة',
        chemicalType: 'حمض أميني',
        benefits: { 'ترطيب': 0.88, 'مضاد للكهرباء الساكنة': 0.8, 'يقوي حاجز البشرة': 0.82 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'threonine',
        name: 'ثريونين',
        scientific_name: 'Threonine',
        searchKeywords: ['threonine', 'ثريونين'],
        usageScope: ['skin', 'hair'],
        functionalCategory: 'دعم حاجز البشرة',
        chemicalType: 'حمض أميني',
        benefits: { 'تكييف الشعر والبشرة': 0.85, 'يساعد في إنتاج الكولاجين': 0.8 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'alanine',
        name: 'ألانين',
        scientific_name: 'Alanine',
        searchKeywords: ['alanine', 'ألانين'],
        usageScope: ['skin', 'hair'],
        functionalCategory: 'دعم حاجز البشرة',
        chemicalType: 'حمض أميني',
        benefits: { 'ترطيب': 0.8, 'تكييف البشرة والشعر': 0.85 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'valine',
        name: 'فالين',
        scientific_name: 'Valine',
        searchKeywords: ['valine', 'فالين'],
        usageScope: ['skin'],
        functionalCategory: 'دعم حاجز البشرة',
        chemicalType: 'حمض أميني',
        benefits: { 'إصلاح البشرة': 0.8, 'ترطيب': 0.85 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'isoleucine',
        name: 'آيزوليوسين',
        scientific_name: 'Isoleucine',
        searchKeywords: ['isoleucine', 'آيزوليوسين'],
        usageScope: ['skin'],
        functionalCategory: 'دعم حاجز البشرة',
        chemicalType: 'حمض أميني',
        benefits: { 'إصلاح حاجز البشرة': 0.88, 'ترطيب': 0.8 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'leucine',
        name: 'ليوسين',
        scientific_name: 'Leucine',
        searchKeywords: ['leucine', 'ليوسين'],
        usageScope: ['skin'],
        functionalCategory: 'دعم حاجز البشرة',
        chemicalType: 'حمض أميني',
        benefits: { 'إصلاح البشرة': 0.85, 'تقليل الخطوط الدقيقة': 0.8 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'phenylalanine',
        name: 'فينيل ألانين',
        scientific_name: 'Phenylalanine',
        searchKeywords: ['phenylalanine', 'فينيل ألانين'],
        usageScope: ['skin'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'حمض أميني',
        benefits: { 'قد يساعد في توحيد لون البشرة': 0.8 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'histidine',
        name: 'هيستيدين',
        scientific_name: 'Histidine',
        searchKeywords: ['histidine', 'هستيدين'],
        usageScope: ['skin'],
        functionalCategory: 'مضاد أكسدة',
        chemicalType: 'حمض أميني',
        benefits: { 'مضاد أكسدة': 0.85, 'ملطف ومرمم': 0.8 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'lysine',
        name: 'لايسين',
        scientific_name: 'Lysine',
        searchKeywords: ['lysine', 'لايسين'],
        usageScope: ['skin'],
        functionalCategory: 'دعم حاجز البشرة',
        chemicalType: 'حمض أميني',
        benefits: { 'دعم إنتاج الكولاجين': 0.85, 'تقوية البشرة': 0.8 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'methionine',
        name: 'ميثيونين',
        scientific_name: 'Methionine',
        searchKeywords: ['methionine', 'ميثيونين'],
        usageScope: ['skin', 'hair'],
        functionalCategory: 'مضاد أكسدة',
        chemicalType: 'حمض أميني',
        benefits: { 'مضاد أكسدة قوي': 0.88, 'يحمي البشرة والشعر من التلف': 0.85 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'cysteine',
        name: 'سيستين',
        scientific_name: 'Cysteine',
        searchKeywords: ['cysteine', 'سيستين'],
        usageScope: ['hair'],
        functionalCategory: 'تقوية الشعر',
        chemicalType: 'حمض أميني',
        benefits: { 'تقوية روابط الشعر': 0.9, 'مضاد أكسدة': 0.8, 'تحسين مرونة الشعر': 0.85 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'tyrosine',
        name: 'تيروسين',
        scientific_name: 'Tyrosine',
        searchKeywords: ['tyrosine', 'تيروسين'],
        usageScope: ['skin'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'حمض أميني',
        benefits: { 'يساعد في إنتاج الميلانين': 0.75, 'ترطيب البشرة': 0.7 },
        synergy: {},
        negativeSynergy: {},
        warnings: [
          { level: 'info', text: 'يستخدم أحيانا في منتجات تسمير البشرة الذاتية.' }
        ],
      },
{
        id: 'glutamic-acid',
        name: 'حمض الجلوتاميك',
        scientific_name: 'Glutamic Acid',
        searchKeywords: ['glutamic acid', 'حمض الجلوتاميك'],
        usageScope: ['skin', 'hair'],
        functionalCategory: 'مرطب / مطري',
        chemicalType: 'حمض أميني',
        benefits: { 'ترطيب': 0.85, 'مضاد للكهرباء الساكنة': 0.8, 'يحافظ على توازن الحموضة': 0.75 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'aspartic-acid',
        name: 'حمض الأسبارتيك',
        scientific_name: 'Aspartic Acid',
        searchKeywords: ['aspartic acid', 'حمض الأسبارتيك'],
        usageScope: ['skin', 'hair'],
        functionalCategory: 'مرطب / مطري',
        chemicalType: 'حمض أميني',
        benefits: { 'ترطيب': 0.8, 'تجديد البشرة': 0.75 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'pca',
        name: 'PCA (حمض البيروليدون الكربوكسيلي)',
        scientific_name: 'PCA (Pyrrolidone Carboxylic Acid)',
        searchKeywords: ['pca', 'pyrrolidone carboxylic acid'],
        usageScope: ['skin'],
        functionalCategory: 'مرطب / مطري',
        chemicalType: 'مرطب',
        benefits: { 'مرطب طبيعي قوي': 0.9, 'يجذب الرطوبة للبشرة': 0.88, 'جزء من عامل الترطيب الطبيعي': 0.95 },
        synergy: { 'sodium-pca': 1.05 },
        negativeSynergy: {},
        warnings: [
          { level: 'info', text: 'غالبا ما يوجد في شكل ملح مثل Sodium PCA لزيادة استقراره.' }
        ],
      },
{
        id: 'capryloyl-salicylic-acid',
        name: 'حمض كابريلويل الساليسيليك (LHA)',
        scientific_name: 'Capryloyl Salicylic Acid',
        searchKeywords: ['capryloyl salicylic acid', 'lha'],
        usageScope: ['skin'],
        functionalCategory: 'مقشر',
        chemicalType: 'حمض بيتا هيدروكسي (BHA)',
        benefits: { 'تقشير دقيق ولطيف': 0.88, 'مكافحة حب الشباب': 0.8, 'مناسب للحساسة': 0.85 },
        synergy: { 'niacinamide': 1.05 },
        negativeSynergy: {},
        warnings: [
          { level: 'info', text: 'شكل معدل من حمض الساليسيليك، جزيئاته أكبر وأكثر دهنية، مما يجعله يخترق الجلد ببطء وبلطف أكبر.' },
          { level: 'risk', text: 'يجب تجنبه من قبل الأشخاص الذين يعانون من حساسية الأسبرين.' }
        ],
      },
{
        id: 'etidronic-acid',
        name: 'حمض الإيتيدرونيك',
        scientific_name: 'Etidronic Acid',
        searchKeywords: ['etidronic acid', 'حمض الإيتيدرونيك'],
        usageScope: ['hair', 'skin'],
        functionalCategory: 'محسن قوام',
        chemicalType: 'عامل خالب',
        benefits: { 'تحسين استقرار المنتج': 0.9, 'يمنع تغير اللون والرائحة': 0.85 },
        synergy: {},
        negativeSynergy: {},
        warnings: [
          { level: 'info', text: 'عامل خالب يربط أيونات المعادن في التركيبة للحفاظ على استقرارها.' }
        ],
      },
{
        id: 'sebacic-acid',
        name: 'حمض السيباسيك',
        scientific_name: 'Sebacic Acid',
        searchKeywords: ['sebacic acid', 'حمض السيباسيك'],
        usageScope: ['skin'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'حمض ثنائي الكربوكسيل',
        benefits: { 'تنظيم الإفرازات الدهنية': 0.85, 'ترطيب': 0.8, 'تحسين ملمس البشرة': 0.75 },
        synergy: { '10-hydroxydecanoic-acid': 1.05 },
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'levulinic-acid',
        name: 'حمض الليفولينيك',
        scientific_name: 'Levulinic Acid',
        searchKeywords: ['levulinic acid', 'حمض الليفولينيك'],
        usageScope: ['skin'],
        functionalCategory: 'مادة حافظة',
        chemicalType: 'حمض عضوي',
        benefits: { 'حفظ المنتج': 0.85, 'تنعيم البشرة': 0.8, 'توازن الحموضة': 0.75 },
        synergy: {},
        negativeSynergy: {},
        warnings: [
          { level: 'info', text: 'مادة حافظة طبيعية مشتقة من السكر.' }
        ],
      },
{
        id: 'anisic-acid',
        name: 'حمض الأنيسيك',
        scientific_name: 'p-Anisic Acid',
        searchKeywords: ['p-anisic acid', 'anisic acid', 'حمض الأنيسيك'],
        usageScope: ['skin'],
        functionalCategory: 'مادة حافظة',
        chemicalType: 'حمض عضوي',
        benefits: { 'حفظ المنتج': 0.8, 'مضاد للفطريات': 0.85, 'عامل عطري خفيف': 0.7 },
        synergy: {},
        negativeSynergy: {},
        warnings: [
          { level: 'info', text: 'مادة حافظة طبيعية مشتقة من اليانسون.' }
        ],
      },
{
        id: 'salicylhydroxamic-acid',
        name: 'حمض الساليسيل هيدروكساميك',
        scientific_name: 'Salicylhydroxamic Acid',
        searchKeywords: ['salicylhydroxamic acid', 'sha', 'حمض الساليسيل هيدروكساميك'],
        usageScope: ['skin'],
        functionalCategory: 'مادة حافظة',
        chemicalType: 'حمض عضوي',
        benefits: { 'حفظ المنتج': 0.85, 'بديل للفينوكسي إيثانول': 0.9, 'مضاد للفطريات': 0.8 },
        synergy: { 'caprylyl-glycol': 1.05 },
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'hydroxyethylpiperazine-ethane-sulfonic-acid',
        name: 'حمض هيدروكسي إيثيل بيبيرازين إيثان السلفونيك (HEPES)',
        scientific_name: 'Hydroxyethylpiperazine Ethane Sulfonic Acid',
        searchKeywords: ['hydroxyethylpiperazine ethane sulfonic acid', 'hepes', 'هيبس'],
        usageScope: ['skin'],
        functionalCategory: 'مقشر',
        chemicalType: 'عامل منظم للحموضة',
        benefits: { 'توازن الحموضة': 0.9, 'يعزز التقشير الأنزيمي': 0.85, 'يحسن امتصاص المكونات': 0.8 },
        synergy: {},
        negativeSynergy: {},
        warnings: [
          { level: 'info', text: 'يعمل على استقرار درجة حموضة التركيبة ويعزز فعالية الإنزيمات الطبيعية المقشرة في الجلد.' }
        ],
      },
{
        id: 'glycyrrhetinic-acid',
        name: 'حمض الجليسيرتينيك',
        scientific_name: 'Glycyrrhetinic Acid',
        searchKeywords: ['glycyrrhetinic acid', 'enoxolone', 'حمض الجليسيرتينيك'],
        usageScope: ['skin'],
        functionalCategory: 'ملطف ومرمم',
        chemicalType: 'مركب نشط نباتي',
        benefits: { 'مضاد للالتهابات': 0.92, 'ملطف ومرمم (شبيه بالكورتيزون)': 0.9, 'تفتيح وتوحيد اللون': 0.85 },
        synergy: { 'licorice-root-extract': 1.1 },
        negativeSynergy: {},
        warnings: [
          { level: 'info', text: 'المكون النشط المضاد للالتهابات في جذر عرق السوس.' }
        ],
      },
{
        id: 'asiatic-acid',
        name: 'حمض الآسياتيك',
        scientific_name: 'Asiatic Acid',
        searchKeywords: ['asiatic acid', 'حمض الآسياتيك'],
        usageScope: ['skin'],
        functionalCategory: 'ملطف ومرمم',
        chemicalType: 'مركب نشط نباتي',
        benefits: { 'تحفيز الكولاجين': 0.9, 'تسريع التئام الجروح': 0.88, 'مضاد أكسدة': 0.8, 'إصلاح البشرة': 0.85 },
        synergy: { 'centella-asiatica': 1.1, 'madecassic-acid': 1.05 },
        negativeSynergy: {},
        warnings: [
          { level: 'info', text: 'أحد المركبات النشطة الرئيسية في السنتيلا (Cica).' }
        ],
      },
{
        id: 'madecassic-acid',
        name: 'حمض الماديكاسيك',
        scientific_name: 'Madecassic Acid',
        searchKeywords: ['madecassic acid', 'حمض الماديكاسيك'],
        usageScope: ['skin'],
        functionalCategory: 'ملطف ومرمم',
        chemicalType: 'مركب نشط نباتي',
        benefits: { 'مضاد للالتهابات': 0.9, 'دعم حاجز البشرة': 0.88, 'تقليل الاحمرار': 0.85, 'علاج الندوب': 0.8 },
        synergy: { 'centella-asiatica': 1.1, 'asiatic-acid': 1.05 },
        negativeSynergy: {},
        warnings: [
          { level: 'info', text: 'أحد المركبات النشطة الرئيسية في السنتيلا (Cica).' }
        ],
      },
{
        id: 'ursolic-acid',
        name: 'حمض اليورسوليك',
        scientific_name: 'Ursolic Acid',
        searchKeywords: ['ursolic acid', 'حمض اليورسوليك'],
        usageScope: ['skin'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'مركب نشط نباتي',
        benefits: { 'مضاد للالتهابات': 0.85, 'مكافحة الشيخوخة': 0.8, 'دعم حاجز البشرة': 0.78 },
        synergy: { 'rosemary-essential': 1.05 },
        negativeSynergy: {},
        warnings: [
          { level: 'info', text: 'يوجد في العديد من النباتات مثل إكليل الجبل وقشور التفاح.' }
        ],
      },
{
        id: 'boswellic-acid',
        name: 'حمض البوزويليك',
        scientific_name: 'Boswellic Acid',
        searchKeywords: ['boswellic acid', 'frankincense extract', 'حمض البوزويليك', 'مستخلص اللبان'],
        usageScope: ['skin'],
        functionalCategory: 'ملطف ومرمم',
        chemicalType: 'مركب نشط نباتي',
        benefits: { 'مضاد للالتهابات': 0.9, 'ملطف ومرمم': 0.88, 'تقليل الاحمرار': 0.85, 'مكافحة الشيخوخة': 0.8 },
        synergy: { 'frankincense-essential': 1.1 },
        negativeSynergy: {},
        warnings: [
          { level: 'info', text: 'المكون النشط الرئيسي في اللبان (Frankincense)، معروف بخصائصه المهدئة القوية.' }
        ],
      },
{ id: 'shikimic-acid', name: 'حمض الشيكيميك', scientific_name: 'Shikimic Acid', searchKeywords: ['shikimic acid'], usageScope: ['skin'], functionalCategory: 'مقشر', chemicalType: 'حمض عضوي', benefits: { 'تقشير': 0.8, 'مضاد للميكروبات': 0.75 }, synergy: {}, negativeSynergy: {}, warnings: [{ level: 'info', text: 'مقشر لطيف مشتق من اليانسون النجمي.' }] },
{ id: 'suberic-acid', name: 'حمض السوبريك', scientific_name: 'Suberic Acid', searchKeywords: ['suberic acid'], usageScope: ['skin'], functionalCategory: 'مكون فعال', chemicalType: 'حمض ثنائي الكربوكسيل', benefits: { 'مكافحة حب الشباب': 0.8, 'تنظيم الإفرازات الدهنية': 0.75 }, synergy: {}, negativeSynergy: {}, warnings: [] },
{ id: 'eicosapentaenoic-acid', name: 'حمض الإيكوسابنتاينويك (EPA)', scientific_name: 'Eicosapentaenoic Acid', searchKeywords: ['eicosapentaenoic acid', 'epa'], usageScope: ['skin'], functionalCategory: 'ملطف ومرمم', chemicalType: 'حمض دهني', benefits: { 'مضاد للالتهابات': 0.95, 'دعم حاجز البشرة': 0.9 }, synergy: {}, negativeSynergy: {}, warnings: [{ level: 'info', text: 'حمض أوميغا 3 قوي جدا، يوجد في زيت السمك.' }] },
{ id: 'docosahexaenoic-acid', name: 'حمض الدوكوساهيكسانويك (DHA)', scientific_name: 'Docosahexaenoic Acid', searchKeywords: ['docosahexaenoic acid', 'dha'], usageScope: ['skin'], functionalCategory: 'ملطف ومرمم', chemicalType: 'حمض دهني', benefits: { 'مضاد للالتهابات': 0.92, 'صحة أغشية الخلايا': 0.9 }, synergy: {}, negativeSynergy: {}, warnings: [{ level: 'info', text: 'حمض أوميغا 3 أساسي لصحة الجلد والدماغ.' }] },
{ id: 'carnitine-hcl', name: 'كارنيتين هيدروكلوريد', scientific_name: 'Carnitine HCl', searchKeywords: ['carnitine hcl'], usageScope: ['skin'], functionalCategory: 'مكون فعال', chemicalType: 'حمض أميني', benefits: { 'تنظيم الإفرازات الدهنية': 0.8, 'تقشير لطيف': 0.75 }, synergy: {}, negativeSynergy: {}, warnings: [] },
{ id: 'hydroxycaproic-acid', name: 'حمض هيدروكسي كابرويك', scientific_name: 'Hydroxycaproic Acid', searchKeywords: ['hydroxycaproic acid'], usageScope: ['skin'], functionalCategory: 'مقشر', chemicalType: 'حمض عضوي', benefits: { 'تقشير': 0.75, 'تجديد الخلايا': 0.7 }, synergy: {}, negativeSynergy: {}, warnings: [] },
{ id: 'hydroxycinnamic-acid', name: 'حمض هيدروكسي سيناميك', scientific_name: 'Hydroxycinnamic Acid', searchKeywords: ['hydroxycinnamic acid'], usageScope: ['skin'], functionalCategory: 'مضاد أكسدة', chemicalType: 'حمض فينولي', benefits: { 'مضاد أكسدة': 0.88, 'حماية من أضرار الشمس': 0.8 }, synergy: {}, negativeSynergy: {}, warnings: [] },
{ id: 'vanillic-acid', name: 'حمض الفانيليك', scientific_name: 'Vanillic Acid', searchKeywords: ['vanillic acid'], usageScope: ['skin'], functionalCategory: 'مضاد أكسدة', chemicalType: 'حمض فينولي', benefits: { 'مضاد أكسدة': 0.8, 'مضاد للميكروبات': 0.7 }, synergy: {}, negativeSynergy: {}, warnings: [] },
{ id: 'salicylic-acid-betaine', name: 'ساليسيلات البيتين', scientific_name: 'Betaine Salicylate', searchKeywords: ['betaine salicylate'], usageScope: ['skin'], functionalCategory: 'مقشر', chemicalType: 'حمض بيتا هيدروكسي (BHA)', benefits: { 'تقشير لطيف': 0.9, 'مكافحة حب الشباب': 0.85 }, synergy: {}, negativeSynergy: {}, warnings: [{ level: 'info', text: 'مركب من حمض الساليسيليك والبيتين المرطب، مما يجعله ألطف على البشرة.' }, { level: 'risk', text: 'يجب تجنبه من قبل الأشخاص الذين يعانون من حساسية الأسبرين.' }] },
{ id: 'thioctic-acid', name: 'حمض الثيوكتيك', scientific_name: 'Thioctic Acid', searchKeywords: ['thioctic acid', 'alpha lipoic acid'], usageScope: ['skin'], functionalCategory: 'مضاد أكسدة', chemicalType: 'حمض دهني', benefits: { 'مضاد أكسدة قوي': 0.92, 'مكافحة الشيخوخة': 0.85 }, synergy: {}, negativeSynergy: {}, warnings: [{ level: 'risk', text: 'قوي جدا ويزيد من حساسية البشرة للشمس. يجب استخدامه بحذر وبضع مرات في الأسبوع فقط.' }] },
{
        id: 'sh-polypeptide-50',
        name: 'sh-Polypeptide-50 (FGF)',
        scientific_name: 'sh-Polypeptide-50',
        searchKeywords: ['sh-polypeptide-50', 'fgf', 'fibroblast growth factor'],
        usageScope: ['skin'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'عامل نمو (ببتيد)',
        benefits: { 'تحفيز الكولاجين': 0.95, 'إصلاح البشرة': 0.92, 'مكافحة الشيخوخة': 0.9 },
        synergy: { 'hyaluronic-acid-serum': 1.1 },
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'عامل نمو مُصنّع بيولوجيا يحاكي عامل نمو الخلايا الليفية في الجسم، وهو ضروري لتجديد الجلد وإنتاج الكولاجين.' }],
      },
{
        id: 'sh-oligopeptide-1',
        name: 'sh-Oligopeptide-1 (EGF)',
        scientific_name: 'sh-Oligopeptide-1',
        searchKeywords: ['sh-oligopeptide-1', 'egf', 'epidermal growth factor', 'عامل نمو البشرة'],
        usageScope: ['skin'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'عامل نمو (ببتيد)',
        benefits: { 'تسريع التئام الجروح': 0.95, 'مكافحة الشيخوخة': 0.9, 'تقليل الندوب': 0.88 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'عامل نمو البشرة، وهو ببتيد قوي يساعد على تسريع إصلاح وتجديد خلايا الجلد.' }],
      },
{
        id: 'palmitoyl-dipeptide-5-diaminobutyroyl-hydroxythreonine',
        name: 'بالميتويل ثنائي الببتيد-5',
        scientific_name: 'Palmitoyl Dipeptide-5 Diaminobutyroyl Hydroxythreonine',
        searchKeywords: ['palmitoyl dipeptide-5', 'syn-tacks'],
        usageScope: ['skin'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'ببتيد',
        benefits: { 'تحسين تماسك البشرة': 0.9, 'مكافحة الشيخوخة': 0.88, 'دعم بنية الجلد': 0.85 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'يعرف تجاريا باسم SYN®-TACKS. يستهدف تقاطع البشرة مع الأدمة لتحسين التواصل الخلوي وزيادة صلابة الجلد.' }],
      },
{
        id: 'palmitoyl-tripeptide-38',
        name: 'بالميتويل ثلاثي الببتيد-38',
        scientific_name: 'Palmitoyl Tripeptide-38',
        searchKeywords: ['palmitoyl tripeptide-38', 'matrixyl synthe-6', 'ماتريكسيل سينث 6'],
        usageScope: ['skin'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'ببتيد',
        benefits: { 'تقليل التجاعيد': 0.92, 'تحفيز الكولاجين': 0.9, 'إعادة بناء الجلد': 0.88 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'يعرف تجاريا باسم Matrixyl® synthe\'6®. يستهدف 6 مكونات رئيسية في بنية الجلد لملء التجاعيد من الداخل.' }],
      },
{
        id: 'tetrapeptide-30',
        name: 'رباعي الببتيد-30',
        scientific_name: 'Tetrapeptide-30',
        searchKeywords: ['tetrapeptide-30', 'teprelone'],
        usageScope: ['skin'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'ببتيد',
        benefits: { 'تفتيح وتوحيد اللون': 0.9, 'تقليل البقع الداكنة': 0.88, 'مضاد للالتهابات': 0.8 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'يعمل على تفتيح البشرة عن طريق تقليل نشاط الخلايا الصباغية.' }],
      },
{
        id: 'acetyl-arginyltryptophyl-diphenylglycine',
        name: 'أسيتيل أرجينيل تريبتوفيل ثنائي فينيل جلايسين',
        scientific_name: 'Acetyl Arginyltryptophyl Diphenylglycine',
        searchKeywords: ['acetyl arginyltryptophyl diphenylglycine', 'relistase'],
        usageScope: ['skin'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'ببتيد',
        benefits: { 'تحسين مرونة الجلد': 0.9, 'مقاومة ترهل الجلد': 0.88, 'زيادة صلابة البشرة': 0.85 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'يعرف تجاريا باسم Relistase™. يعمل على تثبيط الإيلاستاز، وهو الإنزيم الذي يكسر الإيلاستين في الجلد.' }],
      },
{
        id: 'nonapeptide-1',
        name: 'نوناببتيد-1',
        scientific_name: 'Nonapeptide-1',
        searchKeywords: ['nonapeptide-1', 'melanostatine'],
        usageScope: ['skin'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'ببتيد',
        benefits: { 'تفتيح وتوحيد اللون': 0.88, 'منع تكون التصبغات': 0.9 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'يعمل على منع إنتاج الميلانين عن طريق منع هرمون تحفيز الخلايا الصباغية (MSH).' }],
      },
{
        id: 'hexapeptide-11',
        name: 'سداسي الببتيد-11',
        scientific_name: 'Hexapeptide-11',
        searchKeywords: ['hexapeptide-11'],
        usageScope: ['skin', 'hair'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'ببتيد',
        benefits: { 'تحسين مرونة الجلد': 0.85, 'دعم صحة فروة الرأس': 0.8, 'مكافحة الشيخوخة': 0.82 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'ببتيد مُصنّع بيولوجيا من الخميرة، يساعد على زيادة إنتاج الكولاجين وتحسين صحة الجلد بشكل عام.' }],
      },
{
        id: 'quercetin',
        name: 'كيرسيتين',
        scientific_name: 'Quercetin',
        searchKeywords: ['quercetin', 'كيرسيتين'],
        usageScope: ['skin'],
        functionalCategory: 'مضاد أكسدة',
        chemicalType: 'مركب نباتي (فلافونويد)',
        benefits: { 'مضاد أكسدة': 0.9, 'مضاد للالتهابات': 0.88, 'ملطف ومرمم': 0.85 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'فلافونويد قوي موجود في العديد من النباتات مثل البصل والتفاح، معروف بخصائصه المضادة للأكسدة والالتهابات.' }],
      },
{
        id: 'silymarin',
        name: 'سيليمارين',
        scientific_name: 'Silymarin',
        searchKeywords: ['silymarin', 'milk thistle extract', 'silybum marianum', 'سيليمارين', 'مستخلص حليب الشوك'],
        usageScope: ['skin'],
        functionalCategory: 'مضاد أكسدة',
        chemicalType: 'مركب نباتي',
        benefits: { 'مضاد أكسدة': 0.92, 'حماية من أضرار الشمس': 0.88, 'تنظيم الإفرازات الدهنية': 0.8, 'مضاد للالتهابات': 0.85 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'المكون النشط في حليب الشوك، فعال جدا في حماية البشرة الدهنية من أكسدة الدهون التي تؤدي إلى حب الشباب.' }],
      },
{
        id: 'pycnogenol',
        name: 'بيكنوجينول',
        scientific_name: 'Pinus Pinaster Bark Extract',
        searchKeywords: ['pycnogenol', 'pinus pinaster', 'pine bark extract', 'بيكنوجينول', 'مستخلص لحاء الصنوبر'],
        usageScope: ['skin'],
        functionalCategory: 'مضاد أكسدة',
        chemicalType: 'مستخلص نباتي',
        benefits: { 'مضاد أكسدة فائق القوة': 0.98, 'تحفيز الكولاجين': 0.88, 'تحسين مرونة الجلد': 0.9, 'مضاد للالتهابات': 0.9 },
        synergy: { 'vitamin-c-serum': 1.1 },
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'مستخلص من لحاء الصنوبر البحري الفرنسي، وهو مضاد أكسدة قوي جدا يعيد تدوير فيتامين C ويحمي الكولاجين.' }],
      },
{
        id: 'glabridin',
        name: 'جلابريدين',
        scientific_name: 'Glabridin',
        searchKeywords: ['glabridin', 'جلابريدين'],
        usageScope: ['skin'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'مركب نباتي',
        benefits: { 'تفتيح وتوحيد اللون': 0.9, 'مثبط قوي للتيروزيناز': 0.92, 'مضاد أكسدة': 0.8 },
        synergy: { 'licorice-root-extract': 1.1 },
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'المكون النشط الرئيسي المسؤول عن تفتيح البشرة في مستخلص جذور عرق السوس.' }],
      },
{
        id: 'curcuminoids',
        name: 'كركمينويدات',
        scientific_name: 'Curcuma Longa (Turmeric) Root Extract',
        searchKeywords: ['curcuminoids', 'curcumin', 'turmeric extract', 'كركمين', 'مستخلص الكركم'],
        usageScope: ['skin'],
        functionalCategory: 'مضاد أكسدة',
        chemicalType: 'مركب نباتي',
        benefits: { 'مضاد للالتهابات': 0.95, 'مضاد أكسدة': 0.9, 'تفتيح وتوحيد اللون': 0.8, 'مكافحة حب الشباب': 0.82 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'caution', text: 'قد يصبغ البشرة باللون الأصفر مؤقتا.' }],
      },
{
        id: 'emodin',
        name: 'إيمودين',
        scientific_name: 'Emodin',
        searchKeywords: ['emodin', 'aloe emodin', 'إيمودين'],
        usageScope: ['skin'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'مركب نباتي',
        benefits: { 'مضاد للفيروسات': 0.8, 'مضاد للالتهابات': 0.75 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'مركب نشط يوجد في الألوفيرا والراوند، له خصائص علاجية.' }],
      },
{
        id: 'sodium-polyglutamate',
        name: 'بولي جلوتامات الصوديوم (PGA)',
        scientific_name: 'Sodium Polyglutamate',
        searchKeywords: ['sodium polyglutamate', 'polyglutamic acid', 'pga', 'حمض البوليجلوتاميك'],
        usageScope: ['skin'],
        functionalCategory: 'مرطب / مطري',
        chemicalType: 'مرطب',
        benefits: { 'ترطيب أقوى من الهيالورونيك': 0.98, 'تكوين طبقة مرطبة': 0.92, 'تحسين مرونة الجلد': 0.88 },
        synergy: { 'hyaluronic-acid-serum': 1.1 },
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'بوليمر من حمض الجلوتاميك الأميني، يمكنه الاحتفاظ بالماء بما يصل إلى 5000 مرة من وزنه، مكونا طبقة مرطبة ناعمة على الجلد.' }],
      },
{
        id: 'polyhydroxy-acids',
        name: 'أحماض البولي هيدروكسي (PHA)',
        scientific_name: 'Gluconolactone, Lactobionic Acid, Galactose',
        searchKeywords: ['polyhydroxy acids', 'pha', 'gluconolactone', 'lactobionic acid'],
        usageScope: ['skin'],
        functionalCategory: 'مقشر',
        chemicalType: 'حمض بولي هيدروكسي (PHA)',
        benefits: { 'تقشير لطيف جدا': 0.92, 'ترطيب': 0.9, 'مضاد أكسدة': 0.85, 'لا يسبب حساسية للشمس': 0.9 },
        synergy: { 'hyaluronic-acid-serum': 1.1 },
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'الجيل الجديد من الأحماض المقشرة. جزيئاتها كبيرة جدا، تقشر بلطف شديد وتعمل كمرطب. مناسبة للبشرة الحساسة جدا.' }],
      },
{
        id: 'aquaxyl',
        name: 'أكواكسيل',
        scientific_name: 'Xylitylglucoside, Anhydroxylitol, Xylitol',
        searchKeywords: ['aquaxyl', 'xylitylglucoside', 'anhydroxylitol', 'xylitol', 'أكواكسيل'],
        usageScope: ['skin'],
        functionalCategory: 'مرطب / مطري',
        chemicalType: 'مركب ترطيب',
        benefits: { 'ترطيب ثلاثي الأبعاد': 0.95, 'دعم حاجز البشرة': 0.92, 'زيادة مخزون الماء في الجلد': 0.9 },
        synergy: { 'glycerin': 1.05 },
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'مركب ترطيب حاصل على براءة اختراع، يعمل على تحسين تدفق المياه في طبقات الجلد وتقوية حاجز البشرة.' }],
      },
{
        id: 'ciclopirox-olamine',
        name: 'سيكلوبيروكس أولامين',
        scientific_name: 'Ciclopirox Olamine',
        searchKeywords: ['ciclopirox olamine', 'ciclopirox', 'سيكلوبيروكس'],
        usageScope: ['hair'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'عامل مضاد للفطريات',
        benefits: { 'مضاد للقشرة قوي': 0.92, 'مضاد للفطريات واسع الطيف': 0.9, 'مضاد للالتهابات': 0.8 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'عامل طبي فعال جدا لعلاج القشرة والتهاب الجلد الدهني.' }],
      },
{
        id: 'aminexil',
        name: 'أمينيكسيل',
        scientific_name: 'Diaminopyrimidine Oxide',
        searchKeywords: ['aminexil', 'diaminopyrimidine oxide', 'أمينيكسيل'],
        usageScope: ['hair'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'مركب كيميائي',
        benefits: { 'تقليل تساقط الشعر': 0.88, 'منع تصلب الكولاجين حول البصيلات': 0.9 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'مكون حاصل على براءة اختراع (مشابه للمينوكسيديل) يهدف إلى منع تصلب غلاف الكولاجين حول بصيلات الشعر، مما يسمح للشعر بالنمو بشكل أقوى.' }],
      },
{
        id: 'stemoxydine',
        name: 'ستيموكسيدين',
        scientific_name: 'Stemoxydine',
        searchKeywords: ['stemoxydine', 'ستيموكسيدين'],
        usageScope: ['hair'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'مركب كيميائي',
        benefits: { 'زيادة كثافة الشعر': 0.85, 'تحفيز البصيلات الخاملة': 0.88 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'مكون حاصل على براءة اختراع يعمل على خلق بيئة منخفضة الأكسجين حول الخلايا الجذعية في بصيلات الشعر، مما يساعد على "إيقاظ" البصيلات الخاملة.' }],
      },
{
        id: 'redensyl',
        name: 'ريدينسيل',
        scientific_name: 'Glycerin, Aqua, Sodium Metabisulfite, Larix Europaea Wood Extract, Glycine, Zinc Chloride, Camellia Sinensis Leaf Extract',
        searchKeywords: ['redensyl', 'ريدينسيل'],
        usageScope: ['hair'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'مركب مكونات فعالة',
        benefits: { 'تحفيز نمو الشعر': 0.9, 'تقليل تساقط الشعر': 0.88 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'مركب حاصل على براءة اختراع يستهدف الخلايا الجذعية في بصيلات الشعر لتحفيز مرحلة نمو الشعر (Anagen).' }],
      },
{
        id: 'capixyl',
        name: 'كابيكسيل',
        scientific_name: 'Butylene Glycol, Aqua, Dextran, Acetyl Tetrapeptide-3, Trifolium Pratense (Clover) Flower Extract',
        searchKeywords: ['capixyl', 'acetyl tetrapeptide-3', 'red clover extract', 'كابيكسيل'],
        usageScope: ['hair'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'مركب مكونات فعالة',
        benefits: { 'تقليل تساقط الشعر': 0.9, 'تحفيز نمو الشعر': 0.88, 'مضاد للالتهابات': 0.85 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'مركب يجمع بين ببتيد ومستخلص البرسيم الأحمر لمنع تساقط الشعر وتحفيز نموه.' }],
      },
{
        id: 'procapil',
        name: 'بروكابيل',
        scientific_name: 'Butylene Glycol, Aqua, PPG-26-Buteth-26, PEG-40 Hydrogenated Castor Oil, Apigenin, Oleanolic Acid, Biotinoyl Tripeptide-1',
        searchKeywords: ['procapil', 'بروكابيل'],
        usageScope: ['hair'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'مركب مكونات فعالة',
        benefits: { 'تقليل تساقط الشعر': 0.92, 'تقوية بصيلات الشعر': 0.9, 'تحسين الدورة الدموية': 0.88 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'مركب ثلاثي المفعول يحارب شيخوخة البصيلات، ويمنع تساقط الشعر، ويقوي الشعر من الجذور.' }],
      },
{
        id: 'tripeptide-10-citrulline',
        name: 'ثلاثي الببتيد-10 سيترولين',
        scientific_name: 'Tripeptide-10 Citrulline',
        searchKeywords: ['tripeptide-10 citrulline', 'decorinyl'],
        usageScope: ['skin'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'ببتيد',
        benefits: { 'تنظيم ألياف الكولاجين': 0.9, 'تحسين جودة الكولاجين': 0.88, 'زيادة ليونة الجلد': 0.85 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'يعرف تجاريا باسم Decorinyl®. يساعد على تنظيم ألياف الكولاجين للحصول على بشرة أكثر نعومة وتجانسا.' }],
      },
{
        id: 'hexanoyl-dipeptide-3-norleucine-acetate',
        name: 'هيكسانويل ثنائي الببتيد-3 نورليوسين أسيتات',
        scientific_name: 'Hexanoyl Dipeptide-3 Norleucine Acetate',
        searchKeywords: ['hexanoyl dipeptide-3', 'perfectionpeptide p3'],
        usageScope: ['skin'],
        functionalCategory: 'مقشر',
        chemicalType: 'ببتيد',
        benefits: { 'تسريع تجدد الخلايا': 0.9, 'تقشير لطيف': 0.88, 'تحسين ملمس البشرة': 0.85 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'يعرف تجاريا باسم PerfectionPeptide P3™. ببتيد مقشر يعيد تنشيط عملية التقشير الطبيعية للبشرة.' }],
      },
{
        id: 'myristoyl-pentapeptide-17',
        name: 'ميريستويل خماسي الببتيد-17',
        scientific_name: 'Myristoyl Pentapeptide-17',
        searchKeywords: ['myristoyl pentapeptide-17', 'eyelash peptide'],
        usageScope: ['hair'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'ببتيد',
        benefits: { 'تحفيز نمو الرموش': 0.9, 'زيادة كثافة الرموش': 0.88 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'ببتيد شائع في سيرومات الرموش، يعزز إنتاج الكيراتين في بصيلات الشعر.' }],
      },
{
        id: 'acetyl-tetrapeptide-2',
        name: 'أسيتيل رباعي الببتيد-2',
        scientific_name: 'Acetyl Tetrapeptide-2',
        searchKeywords: ['acetyl tetrapeptide-2', 'upletivity', 'أبليفتيفي', 'Acetyl Tetrapeptide 2' ],
        usageScope: ['skin'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'ببتيد',
        benefits: { 'مقاومة ترهل الجلد': 0.88, 'تحسين صلابة البشرة': 0.85 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'يعرف تجاريا باسم Uplevity™. يحفز إنتاج الكولاجين والإيلاستين لمكافحة الترهل الناتج عن الجاذبية.' }],
      },
{
        id: 'palmitoyl-hexapeptide-12',
        name: 'بالميتويل سداسي الببتيد-12',
        scientific_name: 'Palmitoyl Hexapeptide-12',
        searchKeywords: ['palmitoyl hexapeptide-12', 'ceramide-like peptide'],
        usageScope: ['skin'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'ببتيد',
        benefits: { 'إصلاح البشرة': 0.88, 'دعم حاجز البشرة': 0.85, 'تحسين مرونة الجلد': 0.8 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'tripeptide-1',
        name: 'ثلاثي الببتيد-1',
        scientific_name: 'Tripeptide-1',
        searchKeywords: ['tripeptide-1', 'ghk peptide'],
        usageScope: ['skin'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'ببتيد',
        benefits: { 'تحفيز الكولاجين': 0.85, 'إصلاح البشرة': 0.88 },
        synergy: { 'copper-peptides': 1.1 },
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'جزء من ببتيد النحاس الشهير (GHK-Cu)، وهو فعال في إصلاح الجلد وتحفيز الكولاجين.' }],
      },
{
        id: 'genistein',
        name: 'جينيستين',
        scientific_name: 'Genistein',
        searchKeywords: ['genistein', 'soy isoflavones', 'جينيستين'],
        usageScope: ['skin'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'مركب نباتي (ايسوفلافون)',
        benefits: { 'تحفيز الكولاجين': 0.88, 'مكافحة الشيخوخة': 0.85, 'مضاد أكسدة': 0.82 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'ايسوفلافون من الصويا، له تأثير شبيه بالإستروجين يساعد على مكافحة شيخوخة الجلد الناتجة عن انقطاع الطمث.' }],
      },
{
        id: 'viniferine',
        name: 'فينيفرين',
        scientific_name: 'Vitis Vinifera (Grape) Vine Extract',
        searchKeywords: ['viniferine', 'grape vine extract', 'فينيفرين'],
        usageScope: ['skin'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'مركب نباتي',
        benefits: { 'تفتيح وتوحيد اللون': 0.92, 'علاج البقع الداكنة': 0.9, 'مضاد أكسدة': 0.85 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'مستخلص من نسغ شجرة العنب، يقال إنه أكثر فعالية من فيتامين C في تفتيح البقع الداكنة.' }],
      },
{
        id: 'hesperidin',
        name: 'هيسبيريدين',
        scientific_name: 'Hesperidin Methyl Chalcone',
        searchKeywords: ['hesperidin', 'hesperidin methyl chalcone', 'هيسبيريدين'],
        usageScope: ['skin'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'مركب نباتي (فلافونويد)',
        benefits: { 'تقوية الأوعية الدموية': 0.88, 'تقليل الهالات السوداء': 0.85, 'مضاد للالتهابات': 0.8 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'يوجد في قشور الحمضيات، وهو ممتاز لتقوية الشعيرات الدموية وتقليل الانتفاخ والهالات السوداء حول العين.' }],
      },
{
        id: 'baicalin',
        name: 'بايكالين',
        scientific_name: 'Scutellaria Baicalensis Root Extract',
        searchKeywords: ['baicalin', 'scutellaria baicalensis', 'chinese skullcap', 'بايكالين'],
        usageScope: ['skin'],
        functionalCategory: 'مضاد أكسدة',
        chemicalType: 'مركب نباتي (فلافونويد)',
        benefits: { 'مضاد أكسدة': 0.9, 'حماية من أضرار الشمس': 0.85, 'مضاد للميكروبات': 0.8 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'mangosteen-extract',
        name: 'مستخلص المانغوستين',
        scientific_name: 'Garcinia Mangostana Peel Extract',
        searchKeywords: ['mangosteen extract', 'garcinia mangostana', 'مانغوستين'],
        usageScope: ['skin'],
        functionalCategory: 'مضاد أكسدة',
        chemicalType: 'مستخلص نباتي',
        benefits: { 'مضاد أكسدة': 0.92, 'مضاد للميكروبات': 0.88, 'مضاد للالتهابات': 0.85 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'غني بالزانثونات، وهي مضادات أكسدة قوية جدا.' }],
      },
{
        id: 'edelweiss-extract',
        name: 'مستخلص زهرة إديلويس',
        scientific_name: 'Leontopodium Alpinum Extract',
        searchKeywords: ['edelweiss extract', 'leontopodium alpinum', 'إديلويس'],
        usageScope: ['skin'],
        functionalCategory: 'مضاد أكسدة',
        chemicalType: 'مستخلص نباتي',
        benefits: { 'مضاد أكسدة': 0.9, 'حماية من أضرار البيئة': 0.88, 'ملطف ومرمم': 0.8 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'ceramide-np',
        name: 'سيراميد NP (سيراميد 3)',
        scientific_name: 'Ceramide NP',
        searchKeywords: ['ceramide np', 'ceramide 3', 'سيراميد NP'],
        usageScope: ['skin'],
        functionalCategory: 'دعم حاجز البشرة',
        chemicalType: 'دهون',
        benefits: { 'دعم حاجز البشرة': 0.98, 'ترطيب': 0.9, 'حماية البشرة': 0.95 },
        synergy: { 'cholesterol': 1.1, 'fatty-acids': 1.1 },
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'أحد أكثر أنواع السيراميد شيوعا وأهمية في الحفاظ على صحة حاجز البشرة.' }],
      },
{
        id: 'ceramide-ap',
        name: 'سيراميد AP (سيراميد 6-II)',
        scientific_name: 'Ceramide AP',
        searchKeywords: ['ceramide ap', 'ceramide 6-ii', 'سيراميد AP'],
        usageScope: ['skin'],
        functionalCategory: 'دعم حاجز البشرة',
        chemicalType: 'دهون',
        benefits: { 'ترطيب': 0.9, 'تحسين ملمس البشرة': 0.85 },
        synergy: { 'ceramide-np': 1.05 },
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'ceramide-eop',
        name: 'سيراميد EOP (سيراميد 1)',
        scientific_name: 'Ceramide EOP',
        searchKeywords: ['ceramide eop', 'ceramide 1', 'سيراميد EOP'],
        usageScope: ['skin'],
        functionalCategory: 'دعم حاجز البشرة',
        chemicalType: 'دهون',
        benefits: { 'ربط خلايا حاجز البشرة': 0.95, 'دعم حاجز البشرة': 0.92 },
        synergy: { 'ceramide-np': 1.05 },
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'يعمل كـ "مادة لاصقة" تربط خلايا حاجز البشرة معا.' }],
      },
{
        id: 'glycosphingolipids',
        name: 'جلايكوسفينجوليبيد',
        scientific_name: 'Glycosphingolipids',
        searchKeywords: ['glycosphingolipids', 'جلايكوسفينجوليبيد'],
        usageScope: ['skin'],
        functionalCategory: 'دعم حاجز البشرة',
        chemicalType: 'دهون',
        benefits: { 'دعم حاجز البشرة': 0.88, 'ترطيب': 0.85 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'لبنة بناء للسيراميدات في الجلد.' }],
      },
{
        id: 'sodium-hyaluronate-crosspolymer',
        name: 'صوديوم هيالورونات كروسبوليمر',
        scientific_name: 'Sodium Hyaluronate Crosspolymer',
        searchKeywords: ['sodium hyaluronate crosspolymer', 'هيالورونات كروسبوليمر'],
        usageScope: ['skin'],
        functionalCategory: 'مرطب / مطري',
        chemicalType: 'مرطب',
        benefits: { 'ترطيب مستمر وطويل الأمد': 0.95, 'ملء الخطوط الدقيقة': 0.9 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'شبكة ثلاثية الأبعاد من حمض الهيالورونيك تشكل طبقة مرطبة على سطح الجلد وتطلق الرطوبة ببطء.' }],
      },
{
        id: 'hydrolyzed-hyaluronic-acid',
        name: 'حمض الهيالورونيك المهدرج',
        scientific_name: 'Hydrolyzed Hyaluronic Acid',
        searchKeywords: ['hydrolyzed hyaluronic acid', 'low molecular weight ha', 'هيالورونيك مهدرج'],
        usageScope: ['skin'],
        functionalCategory: 'مرطب / مطري',
        chemicalType: 'مرطب',
        benefits: { 'اختراق عميق للبشرة': 0.9, 'ترطيب من الداخل': 0.88, 'إصلاح البشرة': 0.85 },
        synergy: { 'sodium-hyaluronate': 1.05 },
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'جزيئات صغيرة من حمض الهيالورونيك يمكنها اختراق طبقات أعمق من الجلد لتوفير الترطيب من الداخل.' }],
      },
{
        id: 'sodium-acetylated-hyaluronate',
        name: 'هيالورونات الصوديوم الأسيتيلية',
        scientific_name: 'Sodium Acetylated Hyaluronate',
        searchKeywords: ['sodium acetylated hyaluronate', 'super hyaluronic acid'],
        usageScope: ['skin'],
        functionalCategory: 'مرطب / مطري',
        chemicalType: 'مرطب',
        benefits: { 'قدرة فائقة على حبس الماء': 0.95, 'ترطيب أفضل من الهيالورونيك العادي': 0.92, 'إصلاح حاجز البشرة': 0.88 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'يعرف باسم "Super Hyaluronic Acid". شكل معدل يلتصق بالجلد بقوة ويوفر ترطيبا مضاعفا.' }],
      },
{
        id: 'hydroxypropyltrimonium-hyaluronate',
        name: 'هيالورونات هيدروكسي بروبيل تريمونيوم',
        scientific_name: 'Hydroxypropyltrimonium Hyaluronate',
        searchKeywords: ['hydroxypropyltrimonium hyaluronate', 'cationic ha'],
        usageScope: ['skin', 'hair'],
        functionalCategory: 'مرطب / مطري',
        chemicalType: 'مرطب',
        benefits: { 'يلتصق بالجلد والشعر': 0.9, 'ترطيب دائم': 0.88 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'حمض هيالورونيك موجب الشحنة، مما يجعله يلتصق بالجلد والشعر (سالب الشحنة) ولا يُشطف بسهولة، موفرا ترطيبا طويل الأمد.' }],
      },
{
        id: 'biotinoyl-tripeptide-1',
        name: 'بيوتينويل ثلاثي الببتيد-1',
        scientific_name: 'Biotinoyl Tripeptide-1',
        searchKeywords: ['biotinoyl tripeptide-1', 'procapil peptide'],
        usageScope: ['hair'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'ببتيد',
        benefits: { 'تقوية بصيلات الشعر': 0.88, 'تحفيز نمو الشعر': 0.85 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'ببتيد معدل من البيوتين، وهو المكون الرئيسي في مركب Procapil®.' }],
      },
{
        id: 'thermal-spring-water',
        name: 'مياه الينابيع الحرارية',
        scientific_name: 'Aqua / Thermal Spring Water',
        searchKeywords: ['thermal spring water', 'avene thermal water', 'la roche-posay thermal water', 'vichy volcanic water', 'مياه الينابيع الحرارية', 'المياه الحرارية'],
        usageScope: ['skin'],
        functionalCategory: 'ملطف ومرمم',
        chemicalType: 'ماء غني بالمعادن',
        benefits: { 'ملطف ومرمم': 0.95, 'مضاد للالتهابات': 0.9, 'مضاد أكسدة': 0.85, 'دعم حاجز البشرة': 0.9 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'مكون أساسي في علامات تجارية مثل La Roche-Posay وAvène وVichy. غنية بالمعادن النادرة مثل السيلينيوم والسترونتيوم التي تهدئ البشرة.' }],
      },
{
        id: 'acetyl-dipeptide-1-cetyl-ester',
        name: 'أسيتيل ثنائي الببتيد-1 سيتيل استر (Neurosensine)',
        scientific_name: 'Acetyl Dipeptide-1 Cetyl Ester',
        searchKeywords: ['acetyl dipeptide-1 cetyl ester', 'neurosensine', 'calmosensine', 'نيوروسينسين'],
        usageScope: ['skin'],
        functionalCategory: 'ملطف ومرمم',
        chemicalType: 'ببتيد',
        benefits: { 'تقليل حساسية البشرة': 0.95, 'ملطف ومرمم': 0.92, 'مضاد للتهيج': 0.9 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'يعرف تجاريا باسم Neurosensine™. ببتيد مهدئ قوي يقلل من إحساس البشرة بالوخز والحرارة عن طريق تنظيم استجابة الأعصاب للمهيجات.' }],
      },
{
        id: 'vitreoscilla-ferment',
        name: 'خميرة الفيتروسيلا (Life Plankton™)',
        scientific_name: 'Vitreoscilla Ferment',
        searchKeywords: ['vitreoscilla ferment', 'life plankton', 'بلانكتون حيوي'],
        usageScope: ['skin'],
        functionalCategory: 'دعم حاجز البشرة',
        chemicalType: 'مكون مخمر (بروبيوتيك)',
        benefits: { 'دعم ميكروبيوم البشرة': 0.9, 'ملطف ومرمم': 0.88, 'دعم حاجز البشرة': 0.92 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'يعرف تجاريا باسم Life Plankton™ (Biotherm). وهو مكون بروبيوتيك يساعد على تهدئة البشرة وتقوية دفاعاتها الطبيعية.' }],
      },
{
        id: 'i-rhamnose',
        name: 'رامنوز',
        scientific_name: 'Rhamnose',
        searchKeywords: ['rhamnose', 'رامنوز'],
        usageScope: ['skin'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'سكر نباتي',
        benefits: { 'مكافحة الشيخوخة': 0.88, 'تحسين صلابة البشرة': 0.85, 'ملطف ومرمم': 0.8 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'سكر نباتي حاصل على براءة اختراع (Vichy)، يساعد على تواصل الخلايا ويعزز صحة الجلد ومكافحة الشيخوخة.' }],
      },
{
        id: 'lha',
        name: 'حمض ليبو هيدروكسي (LHA)',
        scientific_name: 'Capryloyl Salicylic Acid',
        searchKeywords: ['lipo hydroxy acid', 'lha', 'capryloyl salicylic acid'],
        usageScope: ['skin'],
        functionalCategory: 'مقشر',
        chemicalType: 'حمض بيتا هيدروكسي (BHA)',
        benefits: { 'تقشير دقيق ولطيف': 0.88, 'مكافحة حب الشباب': 0.8, 'مناسب للحساسة': 0.9 },
        synergy: { 'niacinamide': 1.05 },
        negativeSynergy: {},
        warnings: [
          { level: 'info', text: 'شكل معدل من حمض الساليسيليك حاصل على براءة اختراع (L\'Oréal). جزيئاته أكبر وأكثر دهنية، مما يجعله يخترق الجلد ببطء وبلطف أكبر.' },
          { level: 'risk', text: 'يجب تجنبه من قبل الأشخاص الذين يعانون من حساسية الأسبرين.' }
        ],
      },
{
        id: 'terephthalylidene-dicamphor-sulfonic-acid',
        name: 'حمض تريفثاليليدين ثنائي الكافور السلفونيك (Mexoryl SX)',
        scientific_name: 'Terephthalylidene Dicamphor Sulfonic Acid',
        searchKeywords: ['terephthalylidene dicamphor sulfonic acid', 'mexoryl sx', 'ecamsule', 'ميكسوريل SX'],
        usageScope: ['skin'],
        functionalCategory: 'حماية من الشمس',
        chemicalType: 'فلتر كيميائي حديث',
        benefits: { 'حماية فائقة من UVA': 0.98, 'مستقر ضوئيا': 0.95 },
        synergy: { 'avobenzone': 1.1 },
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'يعرف تجاريا باسم Mexoryl® SX. فلتر UVA قوي جدا ومستقر، وهو حصري لعلامات تجارية مثل La Roche-Posay.' }],
      },
{
        id: 'drometrizole-trisiloxane',
        name: 'دروميتريزول تريسيلوكسان (Mexoryl XL)',
        scientific_name: 'Drometrizole Trisiloxane',
        searchKeywords: ['drometrizole trisiloxane', 'mexoryl xl', 'ميكسوريل XL'],
        usageScope: ['skin'],
        functionalCategory: 'حماية من الشمس',
        chemicalType: 'فلتر كيميائي حديث',
        benefits: { 'حماية واسعة الطيف (UVA/UVB)': 0.95, 'مستقر ضوئيا': 0.92, 'يعزز فعالية الفلاتر الأخرى': 0.9 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'يعرف تجاريا باسم Mexoryl® XL. فلتر واسع الطيف قابل للذوبان في الزيت، يعمل بتآزر مع Mexoryl SX.' }],
      },
{
        id: 'bis-ethylhexyloxyphenol-methoxyphenyl-triazine',
        name: 'بس-إيثيل هكسيلوكسي فينول ميثوكسي فينيل تريازين (Tinosorb S)',
        scientific_name: 'Bis-Ethylhexyloxyphenol Methoxyphenyl Triazine',
        searchKeywords: ['bis-ethylhexyloxyphenol methoxyphenyl triazine', 'bemotrizinol', 'tinosorb s', 'تينوسورب S'],
        usageScope: ['skin'],
        functionalCategory: 'حماية من الشمس',
        chemicalType: 'فلتر كيميائي حديث',
        benefits: { 'حماية واسعة الطيف (UVA/UVB)': 0.99, 'استقرار ضوئي فائق': 0.98, 'يعزز استقرار الفلاتر الأخرى': 0.95 },
        synergy: { 'avobenzone': 1.1 },
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'يعرف تجاريا باسم Tinosorb® S. يعتبر من أفضل فلاتر الشمس في العالم لاستقراره وتغطيته الواسعة. معتمد في أوروبا وآسيا ولكن ليس بعد في الولايات المتحدة.' }],
      },
{
        id: 'methylene-bis-benzotriazolyl-tetramethylbutylphenol',
        name: 'ميثيلين بس-بنزوتريازوليل رباعي ميثيل بوتيل فينول (Tinosorb M)',
        scientific_name: 'Methylene Bis-Benzotriazolyl Tetramethylbutylphenol',
        searchKeywords: ['methylene bis-benzotriazolyl tetramethylbutylphenol', 'bisoctrizole', 'tinosorb m', 'تينوسورب M', 'ميثيلين بيس-بنزوتريازوليل تتراميثيل بوتيل فينول'],
        usageScope: ['skin'],
        functionalCategory: 'حماية من الشمس',
        chemicalType: 'فلتر هجين حديث',
        benefits: { 'حماية واسعة الطيف (UVA/UVB)': 0.98, 'يعمل كفلتر فيزيائي وكيميائي': 0.9 },
        synergy: {},
        negativeSynergy: {},
        warnings: [
          { level: 'info', text: 'يعرف تجاريا باسم Tinosorb® M. فلتر فريد يعمل عن طريق امتصاص وعكس وتشتيت الأشعة فوق البنفسجية.' },
          { level: 'caution', text: 'قد يترك طبقة بيضاء طفيفة لأنه جسيمات دقيقة (nano).' }
        ],
      },
{
        id: 'diethylhexyl-butamido-triazone',
        name: 'ثنائي إيثيل هكسيل بوتاميدو تريازون (Uvasorb HEB)',
        scientific_name: 'Diethylhexyl Butamido Triazone',
        searchKeywords: ['diethylhexyl butamido triazone', 'uvasorb heb', 'iscotrizinol'],
        usageScope: ['skin'],
        functionalCategory: 'حماية من الشمس',
        chemicalType: 'فلتر كيميائي حديث',
        benefits: { 'حماية فائقة من UVB': 0.98, 'مستقر ضوئيا جدا': 0.95, 'مقاوم للماء بشكل ممتاز': 0.9 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'يعرف تجاريا باسم Uvasorb® HEB. أحد أكثر فلاتر UVB استقرارا وفعالية.' }],
      },
{
        id: 'ethylhexyl-triazone',
        name: 'إيثيل هكسيل تريازون (Uvinul T 150)',
        scientific_name: 'Ethylhexyl Triazone',
        searchKeywords: ['ethylhexyl triazone', 'uvinul t 150', 'octyl triazone', 'يوفينول T 150'],
        usageScope: ['skin'],
        functionalCategory: 'حماية من الشمس',
        chemicalType: 'فلتر كيميائي حديث',
        benefits: { 'أعلى امتصاص لـ UVB': 0.99, 'مستقر ضوئيا جدا': 0.95, 'مقاوم للماء': 0.9 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'يعرف تجاريا باسم Uvinul® T 150. لديه أعلى قدرة امتصاص لأشعة UVB بين جميع فلاتر الشمس المتاحة.' }],
      },
{
        id: 'diethylamino-hydroxybenzoyl-hexyl-benzoate',
        name: 'ثنائي إيثيل أمينو هيدروكسي بنزويل هيكسيل بنزوات (Uvinul A Plus)',
        scientific_name: 'Diethylamino Hydroxybenzoyl Hexyl Benzoate',
        searchKeywords: ['diethylamino hydroxybenzoyl hexyl benzoate', 'uvinul a plus', 'يوفينول A بلس'],
        usageScope: ['skin'],
        functionalCategory: 'حماية من الشمس',
        chemicalType: 'فلتر كيميائي حديث',
        benefits: { 'حماية فائقة من UVA': 0.98, 'مستقر ضوئيا جدا': 0.95, 'مضاد أكسدة': 0.8 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'يعرف تجاريا باسم Uvinul® A Plus. يوفر حماية ممتازة من أشعة UVA ويساعد على منع تلف الكولاجين.' }],
      },
{
        id: 'tris-biphenyl-triazine-nano',
        name: 'تريس-ثنائي فينيل تريازين (Tinosorb A2B)',
        scientific_name: 'Tris-Biphenyl Triazine',
        searchKeywords: ['tris-biphenyl triazine', 'tinosorb a2b', 'تينوسورب A2B'],
        usageScope: ['skin'],
        functionalCategory: 'حماية من الشمس',
        chemicalType: 'فلتر هجين حديث',
        benefits: { 'حماية فائقة من UVA2 و UVB': 0.98, 'يعزز فعالية الفلاتر الأخرى': 0.9 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'يعرف تجاريا باسم Tinosorb® A2B. فعال بشكل خاص في سد الفجوة بين حماية UVB و UVA.' }],
      },
{
        id: 'o-cymen-5-ol',
        name: 'o-سيمين-5-أول (Biosol)',
        scientific_name: 'o-Cymen-5-ol',
        searchKeywords: ['o-cymen-5-ol', 'biosol', 'isopropyl methylphenol', 'بيوسول'],
        usageScope: ['skin'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'عامل مضاد للميكروبات',
        benefits: { 'مضاد للميكروبات': 0.9, 'مضاد أكسدة': 0.8, 'مادة حافظة مساعدة': 0.85 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'مكون مضاد للبكتيريا والفطريات مشتق من الزعتر، يستخدم في منتجات حب الشباب والمنتجات المهدئة.' }],
      },
{
        id: 'symcalmin',
        name: 'سيمكالمين',
        scientific_name: 'Hydroxyphenyl Propamidobenzoic Acid',
        searchKeywords: ['symcalmin', 'hydroxyphenyl propamidobenzoic acid', 'سيمكالمين'],
        usageScope: ['skin'],
        functionalCategory: 'ملطف ومرمم',
        chemicalType: 'مركب صناعي',
        benefits: { 'مضاد للحكة والتهيج': 0.95, 'تقليل الاحمرار': 0.9, 'ملطف ومرمم': 0.92 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'مركب صناعي يحاكي المكونات المهدئة في الشوفان (الأفينانثراميدات)، وهو فعال جدا في تخفيف الحكة والاحمرار.' }],
      },
{
        id: 'tazman-pepper-extract',
        name: 'مستخلص فلفل تسمانيا',
        scientific_name: 'Tasmannia Lanceolata Fruit/Leaf Extract',
        searchKeywords: ['tazman pepper', 'tasmannia lanceolata', 'فلفل تسمانيا'],
        usageScope: ['skin'],
        functionalCategory: 'ملطف ومرمم',
        chemicalType: 'مستخلص نباتي',
        benefits: { 'مضاد للالتهابات': 0.9, 'تقليل فوري للحكة والاحمرار': 0.92, 'مضاد أكسدة': 0.8 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'مستخلص نباتي فعال جدا في تهدئة البشرة الحساسة والمتهيجة بشكل فوري.' }],
      },
{
        id: 'sphingosine',
        name: 'سفينغوزين',
        scientific_name: 'Sphingosine',
        searchKeywords: ['sphingosine', 'سفينغوزين'],
        usageScope: ['skin'],
        functionalCategory: 'دعم حاجز البشرة',
        chemicalType: 'دهون',
        benefits: { 'لبنة بناء للسيراميدات': 0.9, 'مضاد للميكروبات': 0.85, 'دعم حاجز البشرة': 0.88 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'n-acetyl-ethanolamine',
        name: 'N-أسيتيل إيثانولامين (PEA)',
        scientific_name: 'Palmitamide MEA',
        searchKeywords: ['palmitamide mea', 'pea', 'n-acetyl-ethanolamine'],
        usageScope: ['skin'],
        functionalCategory: 'ملطف ومرمم',
        chemicalType: 'دهون',
        benefits: { 'مضاد للالتهابات': 0.9, 'تقليل الحكة': 0.88, 'دعم حاجز البشرة': 0.85 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'حمض دهني طبيعي في الجسم يساعد على تهدئة الالتهابات واستجابات الحساسية في الجلد.' }],
      },
{
        id: 'phloretin',
        name: 'فلوريتين',
        scientific_name: 'Phloretin',
        searchKeywords: ['phloretin', 'فلوريتين'],
        usageScope: ['skin'],
        functionalCategory: 'مضاد أكسدة',
        chemicalType: 'مركب نباتي',
        benefits: { 'مضاد أكسدة واسع الطيف': 0.9, 'تفتيح وتوحيد اللون': 0.85, 'تحسين امتصاص المكونات': 0.8 },
        synergy: { 'vitamin-c-serum': 1.1, 'ferulic-acid': 1.1 },
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'مضاد أكسدة قوي مستخلص من التفاح، مشهور في سيرومات Skinceuticals.' }],
      },
{
        id: 'thiamidol',
        name: 'ثياميدول',
        scientific_name: 'Isobutylamido Thiazolyl Resorcinol',
        searchKeywords: ['thiamidol', 'isobutylamido thiazolyl resorcinol', 'eucerin anti-pigment', 'ثياميدول'],
        usageScope: ['skin'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'مثبط للتصبغ',
        benefits: { 'مثبط قوي جدا للتيروزيناز': 0.98, 'علاج فعال للكلف والبقع الداكنة': 0.95 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'مكون حاصل على براءة اختراع من Eucerin، وهو أحد أقوى مثبطات إنتاج الميلانين المتاحة.' }],
      },
{
        id: 'hexylresorcinol',
        name: 'هيكسيل ريزورسينول',
        scientific_name: 'Hexylresorcinol',
        searchKeywords: ['hexylresorcinol', 'هيكسيل ريزورسينول'],
        usageScope: ['skin'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'مثبط للتصبغ',
        benefits: { 'تفتيح وتوحيد اللون': 0.9, 'مضاد أكسدة': 0.8, 'مضاد للميكروبات': 0.75 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'carnosine',
        name: 'كارنوزين',
        scientific_name: 'Carnosine',
        searchKeywords: ['carnosine', 'كارنوزين'],
        usageScope: ['skin'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'ثنائي الببتيد',
        benefits: { 'مضاد لعملية الغلايكيشن': 0.95, 'مضاد أكسدة': 0.88, 'مكافحة الشيخوخة': 0.9 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'ببتيد طبيعي في الجسم يحارب عملية الغلايكيشن (ارتباط السكر بالكولاجين)، والتي تسبب تصلب الجلد وظهور التجاعيد.' }],
      },
{
        id: 'dimethylmethoxy-chromanol',
        name: 'ثنائي ميثيل ميثوكسي كرومانول',
        scientific_name: 'Dimethylmethoxy Chromanol',
        searchKeywords: ['dimethylmethoxy chromanol', 'lipochroman'],
        usageScope: ['skin'],
        functionalCategory: 'مضاد أكسدة',
        chemicalType: 'مركب صناعي',
        benefits: { 'مضاد أكسدة فائق القوة': 0.95, 'حماية من التلوث': 0.9, 'إزالة السموم': 0.88 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'يعرف تجاريا باسم Lipochroman®. مضاد أكسدة قوي جدا يحمي من أنواع متعددة من الجذور الحرة الناتجة عن التلوث والأشعة فوق البنفسجية.' }],
      },
{
        id: 'thioredoxin',
        name: 'ثيوريدوكسين',
        scientific_name: 'Thioredoxin',
        searchKeywords: ['thioredoxin', 'ثيوريدوكسين'],
        usageScope: ['skin'],
        functionalCategory: 'مضاد أكسدة',
        chemicalType: 'بروتين',
        benefits: { 'مضاد أكسدة إنزيمي': 0.9, 'إصلاح الحمض النووي': 0.85, 'حماية الخلايا': 0.88 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'بروتين طبيعي في الجسم يعمل كمضاد أكسدة قوي ويساعد على إصلاح تلف الخلايا.' }],
      },
{
        id: 'vitis-vinifera-seed-extract',
        name: 'مستخلص بذور العنب',
        scientific_name: 'Vitis Vinifera (Grape) Seed Extract',
        searchKeywords: ['grape seed extract', 'vitis vinifera', 'oligomeric proanthocyanidins', 'opc'],
        usageScope: ['skin'],
        functionalCategory: 'مضاد أكسدة',
        chemicalType: 'مستخلص نباتي',
        benefits: { 'مضاد أكسدة': 0.92, 'حماية الكولاجين': 0.88, 'مضاد للالتهابات': 0.8 },
        synergy: { 'vitamin-c-serum': 1.05 },
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'غني بالبروأنثوسيانيدينات (OPCs)، وهي مضادات أكسدة أقوى من فيتامين C و E.' }],
      },
{
        id: 'palmitoyl-grape-seed-extract',
        name: 'مستخلص بذور العنب بالميتويل',
        scientific_name: 'Palmitoyl Grape Seed Extract',
        searchKeywords: ['palmitoyl grape seed extract', 'caudalie patent'],
        usageScope: ['skin'],
        functionalCategory: 'مضاد أكسدة',
        chemicalType: 'مركب حاصل على براءة اختراع',
        benefits: { 'مضاد أكسدة': 0.95, 'مكافحة الشيخوخة': 0.9 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'مركب حاصل على براءة اختراع من Caudalie، يجمع بين بوليفينولات بذور العنب وحمض دهني لتحسين الامتصاص والفعالية.' }],
      },
{
        id: '4-t-butylcyclohexanol',
        name: '4-تي-بوتيل سيكلوهكسانول',
        scientific_name: '4-t-Butylcyclohexanol',
        searchKeywords: ['4-t-butylcyclohexanol', 'symsitive', 'سيمسيتيف'],
        usageScope: ['skin'],
        functionalCategory: 'ملطف ومرمم',
        chemicalType: 'مركب صناعي',
        benefits: { 'تقليل فوري للحساسية': 0.98, 'مضاد للتهيج': 0.95, 'يهدئ الإحساس بالوخز والحرقة': 0.92 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'يعرف تجاريا باسم SymSitive®. مكون فعال جدا للبشرة شديدة الحساسية، يعمل على تنظيم مستقبلات الحساسية في الجلد لتهدئة فورية.' }],
      },
{
        id: 'mirabilis-jalapa-extract',
        name: 'مستخلص زهرة الساعة الرابعة',
        scientific_name: 'Mirabilis Jalapa Extract',
        searchKeywords: ['mirabilis jalapa extract', 'four o clock flower'],
        usageScope: ['skin'],
        functionalCategory: 'ملطف ومرمم',
        chemicalType: 'مستخلص نباتي',
        benefits: { 'تقليل حساسية البشرة': 0.9, 'دعم حاجز البشرة': 0.85, 'تقليل الاحمرار': 0.88 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'يساعد على تهدئة البشرة الحساسة وتقوية حاجزها لتقليل تفاعلها مع المهيجات.' }],
      },
{
        id: 'ophiopogon-japonicus-root-extract',
        name: 'مستخلص جذور الأفعى الملتحية',
        scientific_name: 'Ophiopogon Japonicus Root Extract',
        searchKeywords: ['ophiopogon japonicus', 'mondo grass', 'عشبة الأفعى الملتحية'],
        usageScope: ['skin'],
        functionalCategory: 'ملطف ومرمم',
        chemicalType: 'مستخلص نباتي',
        benefits: { 'دعم حاجز البشرة': 0.92, 'ترطيب': 0.88, 'دعم ميكروبيوم البشرة': 0.85 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'stearyl-glycyrrhetinate',
        name: 'ستيريل جليسيرتينات',
        scientific_name: 'Stearyl Glycyrrhetinate',
        searchKeywords: ['stearyl glycyrrhetinate', 'licorice derivative'],
        usageScope: ['skin'],
        functionalCategory: 'ملطف ومرمم',
        chemicalType: 'مركب نشط نباتي',
        benefits: { 'مضاد للالتهابات': 0.9, 'ملطف ومرمم': 0.88 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'مشتق زيتي من حمض الجليسيرتينيك (الموجود في عرق السوس)، مما يحسن امتصاصه وفعاليته كمهدئ.' }],
      },
{
        id: 'ruscus-aculeatus-root-extract',
        name: 'مستخلص جذور السفندر',
        scientific_name: 'Ruscus Aculeatus Root Extract',
        searchKeywords: ['ruscus aculeatus', 'butcher\'s broom', 'سفندر', 'مكنسة الجزار'],
        usageScope: ['skin'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'مستخلص نباتي',
        benefits: { 'تقوية الأوعية الدموية': 0.88, 'تقليل الاحمرار': 0.85, 'تقليل الانتفاخ': 0.8 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'يستخدم في منتجات البشرة الحساسة المعرضة للوردية والاحمرار.' }],
      },
{
        id: 'mannitol',
        name: 'مانيتول',
        scientific_name: 'Mannitol',
        searchKeywords: ['mannitol', 'مانيتول'],
        usageScope: ['skin'],
        functionalCategory: 'مرطب / مطري',
        chemicalType: 'مرطب',
        benefits: { 'ترطيب': 0.8, 'مضاد أكسدة': 0.85 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'سكر كحولي يستخدم كمرطب ومضاد أكسدة، خاصة في منتجات Bioderma الحاصلة على براءة اختراع.' }],
      },
{
        id: 'fructooligosaccharides',
        name: 'فركت أوليجو سكاريدات',
        scientific_name: 'Fructooligosaccharides',
        searchKeywords: ['fructooligosaccharides', 'fos', 'بريبيوتيك'],
        usageScope: ['skin'],
        functionalCategory: 'دعم حاجز البشرة',
        chemicalType: 'بريبيوتيك',
        benefits: { 'دعم ميكروبيوم البشرة': 0.9, 'ترطيب': 0.8 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'بريبيوتيك يغذي البكتيريا المفيدة على سطح الجلد.' }],
      },
{
        id: 'nctf-complex',
        name: 'مركب NCTF',
        scientific_name: 'NCTF (New Cellular Treatment Factor)',
        searchKeywords: ['nctf', 'filorga complex', 'new cellular treatment factor'],
        usageScope: ['skin'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'مركب مكونات فعالة',
        benefits: { 'تجديد شامل للبشرة': 0.95, 'مكافحة الشيخوخة': 0.92, 'تحسين جودة البشرة': 0.9 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'مركب حاصل على براءة اختراع من Filorga يحتوي على أكثر من 50 مكونا (فيتامينات، أحماض أمينية، معادن، إنزيمات مساعدة) يحاكي تركيبات حقن الميزوثيرابي.' }],
      },
{
        id: 'pro-xylane',
        name: 'برو-كسيلان',
        scientific_name: 'Hydroxypropyl Tetrahydropyrantriol',
        searchKeywords: ['pro-xylane', 'hydroxypropyl tetrahydropyrantriol', 'برو-كسيلان'],
        usageScope: ['skin'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'مركب سكري',
        benefits: { 'زيادة كثافة الجلد': 0.92, 'تحفيز إنتاج GAGs': 0.95, 'مكافحة الشيخوخة': 0.9, 'تحسين صلابة البشرة': 0.88 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'جزيء حاصل على براءة اختراع من L\'Oréal، مشتق من خشب الزان. يعمل على تحفيز إنتاج الجليكوز أمينوجليكان (GAGs) مثل حمض الهيالورونيك، مما يعزز كثافة وامتلاء الجلد.' }],
      },
{
        id: 'ambophenol',
        name: 'أمبوفينول',
        scientific_name: 'Tambourissa Trichophylla Leaf Extract',
        searchKeywords: ['ambophenol', 'tambourissa trichophylla', 'أمبوفينول'],
        usageScope: ['skin'],
        functionalCategory: 'ملطف ومرمم',
        chemicalType: 'مستخلص نباتي',
        benefits: { 'تقليل الاحمرار': 0.9, 'تقوية الأوعية الدموية': 0.85 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'مستخلص نباتي حاصل على براءة اختراع (La Roche-Posay) فعال في تقليل الاحمرار المصاحب للوردية.' }],
      },
{
        id: 'myrtus-communis-leaf-extract',
        name: 'مستخلص أوراق الآس',
        scientific_name: 'Myrtus Communis Leaf Extract',
        searchKeywords: ['myrtus communis', 'myrtle extract', 'آس', 'مستخلص الآس'],
        usageScope: ['skin'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'مستخلص نباتي',
        benefits: { 'مضاد للميكروبات': 0.8, 'تنظيم الإفرازات الدهنية': 0.75, 'مضاد للالتهابات': 0.82 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'يستخدم في منتجات البشرة الدهنية للمساعدة في تنظيم إفراز الدهون ومكافحة البكتيريا.' }],
      },
{
        id: 'peumus-boldus-leaf-extract',
        name: 'مستخلص أوراق البولدو',
        scientific_name: 'Peumus Boldus Leaf Extract',
        searchKeywords: ['peumus boldus', 'boldo extract', 'بولدو'],
        usageScope: ['skin'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'مستخلص نباتي',
        benefits: { 'دعم ميكروبيوم البشرة': 0.85, 'مضاد للالتهابات': 0.8 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'يساعد على تحفيز دفاعات البشرة الطبيعية (β-defensins) لتحقيق توازن في ميكروبيوم الجلد.' }],
      },
{
        id: 'glyceryl-stearate',
        name: 'جليسريل ستيرات',
        scientific_name: 'Glyceryl Stearate',
        searchKeywords: ['glyceryl stearate', 'gMS', 'جليسريل ستيرات', 'جليسريل ستيرات'],
        usageScope: ['skin', 'hair'],
        functionalCategory: 'محسن قوام',
        chemicalType: 'مستحلب',
        benefits: { 'مستحلب (يربط الزيت بالماء)': 0.9, 'مطري ومنعم للبشرة': 0.85 },
        synergy: { 'peg-100-stearate': 1.05 },
        negativeSynergy: {},
        warnings: [{ level: 'caution', text: 'قد يكون كوميدوجينيك بشكل طفيف لبعض أنواع البشرة.' }],
      },
{
        id: 'peg-100-stearate',
        name: 'ستيرات PEG-100',
        scientific_name: 'PEG-100 Stearate',
        searchKeywords: ['peg-100 stearate', 'ستيرات بي اي جي'],
        usageScope: ['skin', 'hair'],
        functionalCategory: 'محسن قوام',
        chemicalType: 'مستحلب',
        benefits: { 'مستحلب قوي': 0.9, 'عامل منظف': 0.8 },
        synergy: { 'glyceryl-stearate': 1.05 },
        negativeSynergy: {},
        warnings: [{ level: 'caution', text: 'مثل مركبات PEG الأخرى، قد يكون هناك مخاوف نظرية بشأن التلوث بالإيثيلين أوكسيد أثناء التصنيع، ولكنها منظمة بشدة.' }],
      },
{
        id: 'cetearyl-glucoside',
        name: 'سيتريل غلوكوزيد',
        scientific_name: 'Cetearyl Glucoside',
        searchKeywords: ['cetearyl glucoside', 'سيتريل غلوكوزيد'],
        usageScope: ['skin'],
        functionalCategory: 'محسن قوام',
        chemicalType: 'مستحلب',
        benefits: { 'مستحلب لطيف مشتق من النبات': 0.92, 'يعطي ملمسا خفيفا': 0.88, 'يدعم ترطيب البشرة': 0.8 },
        synergy: { 'cetearyl-alcohol': 1.05 },
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'مستحلب طبيعي معتمد من Ecocert، لطيف جدا ومناسب للبشرة الحساسة.' }],
      },
{
        id: 'sorbitan-oleate',
        name: 'أوليات السوربيتان',
        scientific_name: 'Sorbitan Oleate',
        searchKeywords: ['sorbitan oleate', 'أوليات السوربيتان'],
        usageScope: ['skin'],
        functionalCategory: 'محسن قوام',
        chemicalType: 'مستحلب',
        benefits: { 'مستحلب للماء في الزيت': 0.9, 'مشتق من النبات': 0.85 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'polysorbate-20',
        name: 'بوليسوربات 20',
        scientific_name: 'Polysorbate 20',
        searchKeywords: ['polysorbate 20', 'tween 20', 'بوليسوربات 20'],
        usageScope: ['skin', 'hair'],
        functionalCategory: 'مذوب',
        chemicalType: 'مستحلب',
        benefits: { 'مذوب للزيوت العطرية في الماء': 0.95, 'منظف لطيف': 0.8, 'مستحلب': 0.85 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'polysorbate-60',
        name: 'بوليسوربات 60',
        scientific_name: 'Polysorbate 60',
        searchKeywords: ['polysorbate 60', 'tween 60'],
        usageScope: ['skin'],
        functionalCategory: 'محسن قوام',
        chemicalType: 'مستحلب',
        benefits: { 'يستحلب الزيوت في الماء': 0.9, 'يثبت التركيبات': 0.85 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'polysorbate-80',
        name: 'بوليسوربات 80',
        scientific_name: 'Polysorbate 80',
        searchKeywords: ['polysorbate 80', 'tween 80'],
        usageScope: ['skin'],
        functionalCategory: 'محسن قوام',
        chemicalType: 'مستحلب',
        benefits: { 'مستحلب فعال للزيوت الثقيلة': 0.9, 'يثبت التركيبة': 0.85 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'caution', text: 'قد يسبب مشاكل للبشرة المعرضة لحب الشباب الفطري.' }],
      },
{
        id: 'ceteareth-20',
        name: 'سيتاريث-20',
        scientific_name: 'Ceteareth-20',
        searchKeywords: ['ceteareth-20', 'ceteareth 20', 'سيتاريث-20', 'سيتيريث-20'],
        usageScope: ['skin', 'hair'],
        functionalCategory: 'محسن قوام',
        chemicalType: 'مستحلب',
        benefits: { 'مستحلب قوي': 0.9, 'يثبت التركيبات الصعبة': 0.88 },
        synergy: {},
        negativeSynergy: {},
        warnings: [
          { level: 'caution', text: 'قد يزيد من نفاذية الجلد للمكونات الأخرى.' },
          { level: 'risk', text: 'لا ينصح باستخدامه على البشرة المتضررة أو المجروحة بسبب مخاوف قديمة بشأن التلوث.' }
        ],
      },
{
        id: 'steareth-2',
        name: 'ستيريث-2',
        scientific_name: 'Steareth-2',
        searchKeywords: ['steareth-2', 'ستيريث 2', 'ستيريث-2', 'ستيريث 2', 'steareth 2', 'steareth-2'],
        usageScope: ['skin'],
        functionalCategory: 'محسن قوام',
        chemicalType: 'مستحلب',
        benefits: { 'مستحلب فعال للماء في الزيت': 0.9, 'يستخدم غالبا في مزيلات العرق': 0.85 },
        synergy: { 'steareth-21': 1.05 },
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'steareth-20',
        name: 'ستيريث-20',
        scientific_name: 'Steareth-20',
        searchKeywords: ['steareth-20', 'steareth 20', 'ستيريث-20', 'ستيريث 20', 'ستياريث-20', 'ستياريث 20', ],
        usageScope: ['skin'],
        functionalCategory: 'محسن قوام',
        chemicalType: 'مستحلب',
        benefits: { 'مستحلب فعال': 0.9, 'عامل منظف': 0.75, 'مكثف للقوام': 0.8 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'cetearyl-olivate',
        name: 'سيتريل أوليفات',
        scientific_name: 'Cetearyl Olivate',
        searchKeywords: ['cetearyl olivate', 'olivem 1000'],
        usageScope: ['skin'],
        functionalCategory: 'محسن قوام',
        chemicalType: 'مستحلب',
        benefits: { 'مستحلب طبيعي من زيت الزيتون': 0.92, 'يعطي ملمسا ناعما': 0.88, 'يدعم حاجز البشرة': 0.85 },
        synergy: { 'sorbitan-olivate': 1.1 },
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'يستخدم غالبا مع "Sorbitan Olivate" لتكوين نظام استحلاب كامل (Olivem 1000).' }],
      },
{
        id: 'sorbitan-olivate',
        name: 'سوربيتان أوليفات',
        scientific_name: 'Sorbitan Olivate',
        searchKeywords: ['sorbitan olivate', 'olivem 1000'],
        usageScope: ['skin'],
        functionalCategory: 'محسن قوام',
        chemicalType: 'مستحلب',
        benefits: { 'مستحلب طبيعي من زيت الزيتون': 0.9, 'ترطيب خفيف': 0.8 },
        synergy: { 'cetearyl-olivate': 1.1 },
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'يستخدم غالبا مع "Cetearyl Olivate".' }],
      },
{
        id: 'lecithin',
        name: 'ليسيثين',
        scientific_name: 'Lecithin',
        searchKeywords: ['lecithin', 'ليسيثين'],
        usageScope: ['skin', 'hair'],
        functionalCategory: 'محسن قوام',
        chemicalType: 'مستحلب',
        benefits: { 'مستحلب طبيعي': 0.88, 'مطري ومرطب': 0.9, 'إصلاح حاجز البشرة': 0.85 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'caution', text: 'قد يكون مصدره الصويا أو البيض أو دوار الشمس، مما قد يثير قلق أصحاب الحساسية.' }],
      },
{
        id: 'hydrogenated-lecithin',
        name: 'ليسيثين مهدرج',
        scientific_name: 'Hydrogenated Lecithin',
        searchKeywords: ['hydrogenated lecithin', 'ليسيثين مهدرج'],
        usageScope: ['skin'],
        functionalCategory: 'محسن قوام',
        chemicalType: 'مستحلب',
        benefits: { 'مستحلب لطيف على البشرة': 0.9, 'إصلاح حاجز البشرة': 0.92, 'تحسين امتصاص المكونات': 0.85 },
        synergy: { 'ceramides': 1.08 },
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'شكل أكثر استقرارا من الليسيثين العادي.' }],
      },
{
        id: 'potassium-cetyl-phosphate',
        name: 'فوسفات سيتيل البوتاسيوم',
        scientific_name: 'Potassium Cetyl Phosphate',
        searchKeywords: ['potassium cetyl phosphate', 'فوسفات سيتيل البوتاسيوم'],
        usageScope: ['skin'],
        functionalCategory: 'محسن قوام',
        chemicalType: 'مستحلب',
        benefits: { 'مستحلب قوي ومستقر': 0.95, 'يعطي ملمسا مخمليا': 0.88, 'يعزز فعالية واقيات الشمس': 0.82 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'alcohol-denat',
        name: 'كحول مشوه (محوّل)',
        scientific_name: 'Alcohol Denat.',
        searchKeywords: ['alcohol denat', 'sd alcohol', 'denatured alcohol', 'كحول دينات', 'دينات كحول'],
        usageScope: ['skin', 'hair'],
        functionalCategory: 'مذيب',
        chemicalType: 'كحول بسيط',
        benefits: { 'مذيب فعال': 0.9, 'تسريع جفاف المنتج': 0.88, 'مادة قابضة': 0.8 },
        synergy: {},
        negativeSynergy: {},
        warnings: [
          { level: 'caution', text: 'يمكن أن يسبب جفافا شديدا ويدمر حاجز البشرة على المدى الطويل، مما يؤدي إلى زيادة التهيج والتحسس.' },
          { level: 'caution', text: 'على الرغم من سمعته السيئة، إلا أنه يستخدم في بعض التركيبات لتحسين امتصاص المكونات النشطة وتقليل الملمس الدهني.' }
        ],
      },
{
        id: 'isopropyl-alcohol',
        name: 'كحول الأيزوبروبيل',
        scientific_name: 'Isopropyl Alcohol',
        searchKeywords: ['isopropyl alcohol', 'isopropanol', 'rubbing alcohol', 'كحول الأيزوبروبيل'],
        usageScope: ['skin'],
        functionalCategory: 'مذيب',
        chemicalType: 'كحول بسيط',
        benefits: { 'مذيب قوي': 0.9, 'مطهر': 0.85 },
        synergy: {},
        negativeSynergy: {},
        warnings: [
          { level: 'risk', text: 'مجفف ومهيج جدا للبشرة. يجب تجنبه في مستحضرات التجميل التي تترك على الجلد.' }
        ],
      },
{
        id: 'methylpropanediol',
        name: 'ميثيل بروبانيديول',
        scientific_name: 'Methylpropanediol',
        searchKeywords: ['methylpropanediol', 'ميثيل بروبانيديول'],
        usageScope: ['skin'],
        functionalCategory: 'مذيب',
        chemicalType: 'جلايكول',
        benefits: { 'مذيب فعال': 0.9, 'يعزز امتصاص المكونات': 0.92, 'مرطب خفيف': 0.8 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'ethoxydiglycol',
        name: 'إيثوكسي ديجليكول',
        scientific_name: 'Ethoxydiglycol',
        searchKeywords: ['ethoxydiglycol', 'transcutol'],
        usageScope: ['skin'],
        functionalCategory: 'مذيب',
        chemicalType: 'جلايكول',
        benefits: { 'مذيب ممتاز': 0.95, 'معزز امتصاص قوي': 0.92, 'يعطي ملمسا خفيفا': 0.85 },
        synergy: {},
        negativeSynergy: {},
        warnings: [
          { level: 'info', text: 'فعال جدا في إذابة المكونات الصعبة مثل فيتامين C ومشتقاته، وتحسين توصيلها إلى الجلد.' }
        ],
      },
{
        id: 'isododecane',
        name: 'أيزودوديكان',
        scientific_name: 'Isododecane',
        searchKeywords: ['isododecane', 'أيزودوديكان'],
        usageScope: ['skin', 'hair'],
        functionalCategory: 'مذيب',
        chemicalType: 'مذيب متطاير',
        benefits: { 'يعطي لمسة نهائية جافة (Matte)': 0.95, 'يمنع تلطخ المكياج': 0.9, 'مذيب متطاير': 0.92 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'يتبخر بسرعة بعد التطبيق تاركا وراءه الأصباغ والمكونات الأخرى، مما يجعله مثاليا لأحمر الشفاه السائل وكريمات الأساس طويلة الأمد.' }],
      },
{
        id: 'isohexadecane',
        name: 'أيزوهيكساديكان',
        scientific_name: 'Isohexadecane',
        searchKeywords: ['isohexadecane', 'أيزوهيكساديكان'],
        usageScope: ['skin'],
        functionalCategory: 'مذيب',
        chemicalType: 'مطري',
        benefits: { 'ملمس خفيف وغير دهني': 0.9, 'تحسين انزلاق المنتج': 0.88, 'إزالة المكياج بفعالية': 0.85 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'ammonium-polyacryloyldimethyl-taurate',
        name: 'أمونيوم بولي أكريلويل دايميثيل توريت',
        scientific_name: 'Ammonium Polyacryloyldimethyl Taurate',
        searchKeywords: ['ammonium polyacryloyldimethyl taurate', 'aristoflex avc'],
        usageScope: ['skin'],
        functionalCategory: 'محسن قوام',
        chemicalType: 'بوليمر صناعي',
        benefits: { 'مكثف بوليمري': 0.92, 'يعطي ملمسا خفيفا وغير لزج': 0.95, 'تكوين جل أنيق': 0.9 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'يعطي ملمسا مخمليا منعشا للتركيبات.' }],
      },
{
        id: 'hydroxyethyl-acrylate-sodium-acryloyldimethyl-taurate-copolymer',
        name: 'هيدروكسي إيثيل أكريلات/صوديوم أكريلويل دايميثيل توريت كوبوليمر',
        scientific_name: 'Hydroxyethyl Acrylate/Sodium Acryloyldimethyl Taurate Copolymer',
        searchKeywords: ['hydroxyethyl acrylate/sodium acryloyldimethyl taurate copolymer', 'sepinov emt 10'],
        usageScope: ['skin'],
        functionalCategory: 'محسن قوام',
        chemicalType: 'بوليمر صناعي',
        benefits: { 'مكثف بوليمري فعال جدا': 0.95, 'يعطي ملمسا خفيفا وأنيقا': 0.9, 'يثبت الزيوت': 0.92 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'ممتاز لتركيبات الجل-كريم.' }],
      },
{
        id: 'sodium-polyacrylate',
        name: 'بولي أكريلات الصوديوم',
        scientific_name: 'Sodium Polyacrylate',
        searchKeywords: ['sodium polyacrylate', 'بولي أكريلات الصوديوم'],
        usageScope: ['skin'],
        functionalCategory: 'محسن قوام',
        chemicalType: 'بوليمر صناعي',
        benefits: { 'مكثف للقوام': 0.9, 'تثبيت التركيبة': 0.85, 'ملمس ناعم': 0.88 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'acacia-senegal-gum',
        name: 'صمغ السنغال (الصمغ العربي)',
        scientific_name: 'Acacia Senegal Gum',
        searchKeywords: ['acacia senegal gum', 'gum arabic', 'صمغ عربي'],
        usageScope: ['skin'],
        functionalCategory: 'محسن قوام',
        chemicalType: 'بوليمر طبيعي',
        benefits: { 'مكثف طبيعي': 0.88, 'يعطي تأثيرا شادا خفيفا': 0.8, 'تثبيت التركيبة': 0.85 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'carrageenan',
        name: 'كاراجينان',
        scientific_name: 'Carrageenan',
        searchKeywords: ['carrageenan', 'irish moss', 'كاراجينان'],
        usageScope: ['skin'],
        functionalCategory: 'محسن قوام',
        chemicalType: 'بوليمر طبيعي',
        benefits: { 'مكثف من الطحالب البحرية': 0.85, 'تكوين جل': 0.8 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'caution', text: 'هناك جدل حوله، حيث تشير بعض الدراسات إلى أنه قد يكون مهيجا ومسببا للالتهابات، على الرغم من أن الهيئات التنظيمية تعتبره آمنا للاستخدام الموضعي.' }],
      },
{
        id: 'chlorphenesin',
        name: 'كلورفينيسين',
        scientific_name: 'Chlorphenesin',
        searchKeywords: ['chlorphenesin', 'كلورفينيسين'],
        usageScope: ['skin'],
        functionalCategory: 'مادة حافظة',
        chemicalType: 'مادة حافظة',
        benefits: { 'حفظ المنتج ضد البكتيريا والفطريات': 0.88 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'مقيد الاستخدام بتركيزات منخفضة (عادة حوالي 0.3%) في أوروبا. قد يسبب حساسية تلامسية للبعض.' }],
      },
{
        id: 'chlorhexidine-digluconate',
        name: 'كلورهيكسيدين ديجلوكونات',
        scientific_name: 'Chlorhexidine Digluconate',
        searchKeywords: ['chlorhexidine digluconate', 'chlorhexidine', 'كلورهيكسيدين'],
        usageScope: ['skin'],
        functionalCategory: 'مادة حافظة',
        chemicalType: 'مطهر',
        benefits: { 'مادة حافظة ومطهرة قوية': 0.92, 'فعال ضد مجموعة واسعة من البكتيريا': 0.9 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'caution', text: 'مقيد بتركيز أقصى 0.3% في أوروبا. قد يسبب حساسية نادرة ولكنها شديدة.' }],
      },
{
        id: 'polyaminopropyl-biguanide',
        name: 'بولي أمينوبروبيل بيجوانيد (PHMB)',
        scientific_name: 'Polyaminopropyl Biguanide',
        searchKeywords: ['polyaminopropyl biguanide', 'phmb'],
        usageScope: ['skin'],
        functionalCategory: 'مادة حافظة',
        chemicalType: 'مادة حافظة',
        benefits: { 'مادة حافظة لطيفة لا تسبب تهيجا': 0.88 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'risk', text: 'مقيد بتركيز أقصى 0.1% في أوروبا ومصنف كمادة مسرطنة مشتبه بها من الفئة 2.' }],
      },
{
        id: 'capryloyl-glycine',
        name: 'كابريلويل جلايسين',
        scientific_name: 'Capryloyl Glycine',
        searchKeywords: ['capryloyl glycine', 'lipacide c8g'],
        usageScope: ['skin'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'حمض أميني دهني',
        benefits: { 'تنظيم الإفرازات الدهنية': 0.88, 'تطهير البشرة': 0.85, 'إعادة توازن حموضة البشرة': 0.8 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'يساعد على الحفاظ على صحة ميكروبيوم البشرة.' }],
      },
{
        id: 'undecylenoyl-phenylalanine',
        name: 'أنديسيلينويل فينيل ألانين',
        scientific_name: 'Undecylenoyl Phenylalanine',
        searchKeywords: ['undecylenoyl phenylalanine', 'sepiwhite'],
        usageScope: ['skin'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'مثبط للتصبغ',
        benefits: { 'تفتيح وتوحيد اللون': 0.9, 'يعمل على مسار مختلف': 0.88, 'علاج البقع الداكنة': 0.85 },
        synergy: { 'niacinamide': 1.05 },
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'يعرف تجاريا باسم Sepiwhite™ MSH. يعمل عن طريق منع هرمون تحفيز الخلايا الصباغية (MSH).' }],
      },
{
        id: 'panax-ginseng-root-extract',
        name: 'مستخلص جذور الجينسنغ',
        scientific_name: 'Panax Ginseng Root Extract',
        searchKeywords: ['panax ginseng', 'korean red ginseng', 'ginseng extract', 'جينسنغ', 'مستخلص الجينسنغ'],
        usageScope: ['skin', 'hair'],
        functionalCategory: 'مضاد أكسدة',
        chemicalType: 'مستخلص نباتي (أدابتوجين)',
        benefits: { 'مضاد أكسدة': 0.92, 'مكافحة الشيخوخة': 0.88, 'تنشيط البشرة': 0.9, 'تحسين الدورة الدموية': 0.85 },
        synergy: { 'vitamin-c-serum': 1.08 },
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'نبات أدابتوجيني يساعد البشرة على مقاومة الإجهاد البيئي ويعزز حيويتها.' }],
      },
{
        id: 'ganoderma-lucidum-extract',
        name: 'مستخلص فطر الريشي',
        scientific_name: 'Ganoderma Lucidum (Reishi Mushroom) Extract',
        searchKeywords: ['ganoderma lucidum', 'reishi mushroom', 'lingzhi', 'فطر الريشي', 'ريشي'],
        usageScope: ['skin'],
        functionalCategory: 'ملطف ومرمم',
        chemicalType: 'مستخلص نباتي (أدابتوجين)',
        benefits: { 'مضاد للالتهابات': 0.9, 'مضاد أكسدة': 0.88, 'ترطيب': 0.85, 'دعم حاجز البشرة': 0.82 },
        synergy: { 'beta-glucan': 1.05 },
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'فطر أدابتوجيني قوي، غني بالبيتا جلوكان والبوليسكاريدات المهدئة والمرطبة.' }],
      },
{
        id: 'rhodiola-rosea-root-extract',
        name: 'مستخلص جذور الروديولا الوردية',
        scientific_name: 'Rhodiola Rosea Root Extract',
        searchKeywords: ['rhodiola rosea', 'golden root', 'arctic root', 'روديولا'],
        usageScope: ['skin'],
        functionalCategory: 'مضاد أكسدة',
        chemicalType: 'مستخلص نباتي (أدابتوجين)',
        benefits: { 'حماية من الإجهاد التأكسدي': 0.9, 'مضاد أكسدة': 0.92, 'مكافحة الشيخوخة': 0.85 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'نبات أدابتوجيني يساعد على حماية البشرة من الإجهاد الناتج عن العوامل البيئية مثل الأشعة فوق البنفسجية والتلوث.' }],
      },
{
        id: 'withania-somnifera-root-extract',
        name: 'مستخلص جذور الأشواغاندا',
        scientific_name: 'Withania Somnifera (Ashwagandha) Root Extract',
        searchKeywords: ['withania somnifera', 'ashwagandha', 'indian ginseng', 'أشواغاندا'],
        usageScope: ['skin'],
        functionalCategory: 'مضاد أكسدة',
        chemicalType: 'مستخلص نباتي (أدابتوجين)',
        benefits: { 'مضاد أكسدة': 0.88, 'مضاد للالتهابات': 0.85, 'حماية من الضوء الأزرق': 0.8 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'inonotus-obliquus-extract',
        name: 'مستخلص فطر الشاجا',
        scientific_name: 'Inonotus Obliquus (Chaga Mushroom) Extract',
        searchKeywords: ['inonotus obliquus', 'chaga mushroom', 'فطر الشاجا', 'شاجا'],
        usageScope: ['skin'],
        functionalCategory: 'مضاد أكسدة',
        chemicalType: 'مستخلص نباتي (أدابتوجين)',
        benefits: { 'مضاد أكسدة فائق القوة': 0.95, 'حماية من أضرار البيئة': 0.9, 'ملطف ومرمم': 0.85 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'يحتوي على مستويات عالية جدا من مضادات الأكسدة لحماية البشرة من الجذور الحرة.' }],
      },
{
        id: 'schisandra-chinensis-fruit-extract',
        name: 'مستخلص فاكهة الشيساندرا',
        scientific_name: 'Schisandra Chinensis Fruit Extract',
        searchKeywords: ['schisandra chinensis', 'magnolia berry', 'شيساندرا'],
        usageScope: ['skin'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'مستخلص نباتي (أدابتوجين)',
        benefits: { 'تحسين صلابة البشرة': 0.88, 'مضاد أكسدة': 0.85, 'توحيد لون البشرة': 0.8 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'astragalus-membranaceus-root-extract',
        name: 'مستخلص جذور القتاد',
        scientific_name: 'Astragalus Membranaceus Root Extract',
        searchKeywords: ['astragalus membranaceus', 'astragalus', 'huang qi', 'قتاد'],
        usageScope: ['skin'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'مستخلص نباتي (أدابتوجين)',
        benefits: { 'مضاد للالتهابات': 0.85, 'مضاد أكسدة': 0.8, 'تحسين الدورة الدموية': 0.82 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'camellia-sinensis-leaf-extract',
        name: 'مستخلص أوراق الشاي (الأخضر، الأبيض، الأسود)',
        scientific_name: 'Camellia Sinensis Leaf Extract',
        searchKeywords: ['camellia sinensis', 'green tea', 'white tea', 'black tea', 'egcg', 'شاي أخضر'],
        usageScope: ['skin', 'hair'],
        functionalCategory: 'مضاد أكسدة',
        chemicalType: 'مستخلص نباتي',
        benefits: { 'مضاد أكسدة': 0.95, 'مضاد للالتهابات': 0.9, 'حماية من أضرار الشمس': 0.88, 'تنظيم الإفرازات الدهنية': 0.8 },
        synergy: { 'caffeine': 1.05 },
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'غني بالبوليفينولات، خاصة EGCG، التي تحمي البشرة من الإجهاد التأكسدي.' }],
      },
{
        id: 'vitis-vinifera-fruit-extract',
        name: 'مستخلص فاكهة العنب',
        scientific_name: 'Vitis Vinifera (Grape) Fruit Extract',
        searchKeywords: ['grape fruit extract', 'vitis vinifera', 'resveratrol', 'عنب'],
        usageScope: ['skin'],
        functionalCategory: 'مضاد أكسدة',
        chemicalType: 'مستخلص نباتي',
        benefits: { 'مضاد أكسدة': 0.9, 'مكافحة الشيخوخة': 0.88, 'مصدر للريسفيراترول': 0.85 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'punica-granatum-extract',
        name: 'مستخلص الرمان',
        scientific_name: 'Punica Granatum Extract',
        searchKeywords: ['punica granatum', 'pomegranate extract', 'رمان', 'مستخلص الرمان'],
        usageScope: ['skin'],
        functionalCategory: 'مضاد أكسدة',
        chemicalType: 'مستخلص نباتي',
        benefits: { 'مضاد أكسدة': 0.92, 'حماية من أضرار الشمس': 0.85, 'تفتيح وتوحيد اللون': 0.8 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'coffea-arabica-seed-extract',
        name: 'مستخلص بذور البن',
        scientific_name: 'Coffea Arabica (Coffee) Seed Extract',
        searchKeywords: ['coffea arabica', 'coffee extract', 'مستخلص البن', 'قهوة'],
        usageScope: ['skin'],
        functionalCategory: 'مضاد أكسدة',
        chemicalType: 'مستخلص نباتي',
        benefits: { 'مضاد أكسدة': 0.9, 'تنشيط البشرة': 0.85, 'تقليل الانتفاخ': 0.8 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'theobroma-cacao-extract',
        name: 'مستخلص الكاكاو',
        scientific_name: 'Theobroma Cacao (Cocoa) Extract',
        searchKeywords: ['theobroma cacao', 'cocoa extract', 'كاكاو', 'مستخلص الكاكاو'],
        usageScope: ['skin'],
        functionalCategory: 'مضاد أكسدة',
        chemicalType: 'مستخلص نباتي',
        benefits: { 'مضاد أكسدة': 0.88, 'تحسين الدورة الدموية': 0.8, 'ترطيب': 0.82 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'euterpe-oleracea-fruit-extract',
        name: 'مستخلص فاكهة الآساي',
        scientific_name: 'Euterpe Oleracea (Acai) Fruit Extract',
        searchKeywords: ['euterpe oleracea', 'acai extract', 'آساي'],
        usageScope: ['skin'],
        functionalCategory: 'مضاد أكسدة',
        chemicalType: 'مستخلص نباتي',
        benefits: { 'مضاد أكسدة فائق القوة': 0.95, 'مكافحة الشيخوخة': 0.88, 'مضاد للالتهابات': 0.85 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'lycium-barbarum-fruit-extract',
        name: 'مستخلص فاكهة الغوجي',
        scientific_name: 'Lycium Barbarum (Goji) Fruit Extract',
        searchKeywords: ['lycium barbarum', 'goji berry extract', 'wolfberry', 'غوجي'],
        usageScope: ['skin'],
        functionalCategory: 'مضاد أكسدة',
        chemicalType: 'مستخلص نباتي',
        benefits: { 'مضاد أكسدة': 0.9, 'غني بالفيتامينات': 0.85, 'تفتيح وتوحيد اللون': 0.8 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'malpighia-emarginata-fruit-extract',
        name: 'مستخلص كرز الأسيرولا',
        scientific_name: 'Malpighia Emarginata (Acerola) Fruit Extract',
        searchKeywords: ['malpighia emarginata', 'acerola cherry', 'كرز الأسيرولا'],
        usageScope: ['skin'],
        functionalCategory: 'مضاد أكسدة',
        chemicalType: 'مستخلص نباتي',
        benefits: { 'مصدر غني جدا بفيتامين C': 0.98, 'مضاد أكسدة': 0.95, 'تفتيح وتوحيد اللون': 0.9 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'أحد أغنى المصادر الطبيعية لفيتامين C في العالم.' }],
      },
{
        id: 'morus-alba-root-extract',
        name: 'مستخلص جذور التوت الأبيض',
        scientific_name: 'Morus Alba (White Mulberry) Root Extract',
        searchKeywords: ['morus alba', 'white mulberry', 'توت أبيض'],
        usageScope: ['skin'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'مستخلص نباتي',
        benefits: { 'تفتيح وتوحيد اللون': 0.9, 'علاج البقع الداكنة': 0.88, 'مضاد أكسدة': 0.8 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'chrysanthemum-indicum-flower-extract',
        name: 'مستخلص زهرة الأقحوان',
        scientific_name: 'Chrysanthemum Indicum Flower Extract',
        searchKeywords: ['chrysanthemum indicum', 'chrysanthemum', 'أقحوان'],
        usageScope: ['skin'],
        functionalCategory: 'ملطف ومرمم',
        chemicalType: 'مستخلص نباتي',
        benefits: { 'مضاد للالتهابات': 0.85, 'ملطف ومرمم': 0.88 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'risk', text: 'قد يسبب حساسية للأشخاص الذين يعانون من حساسية نباتات عائلة الأقحوان.' }],
      },
{
        id: 'spirulina-platensis-extract',
        name: 'مستخلص سبيرولينا',
        scientific_name: 'Spirulina Platensis Extract',
        searchKeywords: ['spirulina platensis', 'spirulina', 'سبيرولينا'],
        usageScope: ['skin'],
        functionalCategory: 'مضاد أكسدة',
        chemicalType: 'مستخلص طحالب',
        benefits: { 'غني بالبروتين والفيتامينات': 0.9, 'مضاد أكسدة': 0.88, 'إزالة السموم': 0.85 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'laminaria-digitata-extract',
        name: 'مستخلص لاميناريا ديجيتاتا',
        scientific_name: 'Laminaria Digitata Extract',
        searchKeywords: ['laminaria digitata', 'kelp', 'لاميناريا', 'عشب البحر'],
        usageScope: ['skin'],
        functionalCategory: 'مرطب / مطري',
        chemicalType: 'مستخلص طحالب',
        benefits: { 'ترطيب': 0.9, 'غني بالمعادن': 0.88, 'مضاد للالتهابات': 0.8 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'fucus-vesiculosus-extract',
        name: 'مستخلص الفوقس الحويصلي',
        scientific_name: 'Fucus Vesiculosus Extract',
        searchKeywords: ['fucus vesiculosus', 'bladderwrack', 'فوقس'],
        usageScope: ['skin'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'مستخلص طحالب',
        benefits: { 'تحسين مرونة الجلد': 0.85, 'مضاد أكسدة': 0.8, 'ترطيب': 0.82 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'chondrus-crispus-extract',
        name: 'مستخلص الطحلب الأيرلندي',
        scientific_name: 'Chondrus Crispus (Carrageenan) Extract',
        searchKeywords: ['chondrus crispus', 'irish moss', 'carrageenan', 'طحلب أيرلندي'],
        usageScope: ['skin'],
        functionalCategory: 'محسن قوام',
        chemicalType: 'مستخلص طحالب',
        benefits: { 'تكوين طبقة مرطبة': 0.9, 'ترطيب': 0.85, 'ملطف ومرمم': 0.8 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'مصدر طبيعي للكاراجينان، يشكل طبقة رقيقة ومرطبة على الجلد.' }],
      },
{
        id: 'astrocaryum-vulgare-pulp-extract',
        name: 'مستخلص لب توكوما',
        scientific_name: 'Astrocaryum Vulgare Pulp Extract',
        searchKeywords: ['astrocaryum vulgare', 'tucuma', 'توكوما'],
        usageScope: ['skin'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'مستخلص نباتي',
        benefits: { 'بديل حيوي للريتينول': 0.88, 'تحفيز إنتاج حمض الهيالورونيك': 0.9 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{ id: 'lentinula-edodes-extract', name: 'مستخلص فطر شيتاكي', scientific_name: 'Lentinula Edodes Extract', searchKeywords: ['shiitake mushroom', 'lentinula edodes'], usageScope: ['skin'], functionalCategory: 'مكون فعال', chemicalType: 'مستخلص نباتي', benefits: {'تفتيح وتوحيد اللون': 0.85, 'مضاد أكسدة': 0.8}, synergy: {}, negativeSynergy: {}, warnings: []},
{ id: 'cordyceps-sinensis-extract', name: 'مستخلص فطر كورديسيبس', scientific_name: 'Cordyceps Sinensis Extract', searchKeywords: ['cordyceps'], usageScope: ['skin'], functionalCategory: 'مضاد أكسدة', chemicalType: 'مستخلص نباتي', benefits: {'مضاد للالتهابات': 0.88, 'مضاد أكسدة': 0.85}, synergy: {}, negativeSynergy: {}, warnings: []},
{ id: 'algae-extract', name: 'مستخلص الطحالب', scientific_name: 'Algae Extract', searchKeywords: ['algae extract', 'seaweed'], usageScope: ['skin'], functionalCategory: 'مرطب / مطري', chemicalType: 'مستخلص طحالب', benefits: {'ترطيب': 0.9, 'مضاد أكسدة': 0.85}, synergy: {}, negativeSynergy: {}, warnings: []},
{ id: 'paeonia-lactiflora-root-extract', name: 'مستخلص جذور الفاوانيا', scientific_name: 'Paeonia Lactiflora Root Extract', searchKeywords: ['peony extract'], usageScope: ['skin'], functionalCategory: 'مكون فعال', chemicalType: 'مستخلص نباتي', benefits: {'تفتيح وتوحيد اللون': 0.88, 'مضاد أكسدة': 0.85}, synergy: {}, negativeSynergy: {}, warnings: []},
{ id: 'ananas-sativus-fruit-extract', name: 'مستخلص فاكهة الأناناس', scientific_name: 'Ananas Sativus (Pineapple) Fruit Extract', searchKeywords: ['pineapple extract'], usageScope: ['skin'], functionalCategory: 'مقشر', chemicalType: 'مستخلص نباتي', benefits: {'تقشير إنزيمي': 0.85, 'مضاد للالتهابات': 0.8}, synergy: {}, negativeSynergy: {}, warnings: [{level: 'info', text: 'يحتوي على إنزيم البروميلين الذي يقشر البشرة بلطف.'}]},
{ id: 'carica-papaya-fruit-extract', name: 'مستخلص فاكهة البابايا', scientific_name: 'Carica Papaya (Papaya) Fruit Extract', searchKeywords: ['papaya extract'], usageScope: ['skin'], functionalCategory: 'مقشر', chemicalType: 'مستخلص نباتي', benefits: {'تقشير إنزيمي': 0.88, 'تفتيح وتوحيد اللون': 0.82}, synergy: {}, negativeSynergy: {}, warnings: [{level: 'info', text: 'يحتوي على إنزيم الباباين الذي يقشر خلايا الجلد الميتة.'}]},
{ id: 'ficus-carica-fruit-extract', name: 'مستخلص فاكهة التين', scientific_name: 'Ficus Carica (Fig) Fruit Extract', searchKeywords: ['fig extract'], usageScope: ['skin'], functionalCategory: 'مضاد أكسدة', chemicalType: 'مستخلص نباتي', benefits: {'مضاد أكسدة': 0.85, 'ترطيب': 0.8}, synergy: {}, negativeSynergy: {}, warnings: []},
{ id: 'alchemilla-vulgaris-extract', name: 'مستخلص عباءة السيدة', scientific_name: 'Alchemilla Vulgaris Extract', searchKeywords: ['lady\'s mantle'], usageScope: ['skin'], functionalCategory: 'مكون فعال', chemicalType: 'مستخلص نباتي', benefits: {'مضاد أكسدة': 0.8, 'قابض للمسام': 0.85}, synergy: {}, negativeSynergy: {}, warnings: []},
{ id: 'malva-sylvestris-extract', name: 'مستخلص الخبيزة', scientific_name: 'Malva Sylvestris (Mallow) Extract', searchKeywords: ['mallow extract'], usageScope: ['skin'], functionalCategory: 'ملطف ومرمم', chemicalType: 'مستخلص نباتي', benefits: {'ملطف ومرمم': 0.9, 'مضاد للالتهابات': 0.85}, synergy: {}, negativeSynergy: {}, warnings: []},
{ id: 'melissa-officinalis-leaf-extract', name: 'مستخلص أوراق المليسة', scientific_name: 'Melissa Officinalis Leaf Extract', searchKeywords: ['lemon balm'], usageScope: ['skin'], functionalCategory: 'ملطف ومرمم', chemicalType: 'مستخلص نباتي', benefits: {'مهدئ': 0.9, 'مضاد أكسدة': 0.85}, synergy: {}, negativeSynergy: {}, warnings: []},
{
        id: 'hericium-erinaceus-extract',
        name: 'مستخلص فطر عرف الأسد',
        scientific_name: 'Hericium Erinaceus (Mushroom) Extract',
        searchKeywords: ['hericium erinaceus', 'lion\'s mane mushroom', 'yamabushitake', 'عرف الأسد', 'فطر عرف الأسد'],
        usageScope: ['skin'],
        functionalCategory: 'ملطف ومرمم',
        chemicalType: 'مستخلص نباتي (فطر)',
        benefits: { 'ملطف ومرمم': 0.9, 'مضاد أكسدة': 0.88, 'تحفيز الكولاجين': 0.85, 'تسريع التئام الجروح': 0.82 },
        synergy: { 'centella-asiatica': 1.05 },
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'يساعد على تعزيز إصلاح الجلد وتجديد الخلايا.' }],
      },
{
        id: 'grifola-frondosa-extract',
        name: 'مستخلص فطر المايتاكي',
        scientific_name: 'Grifola Frondosa Extract',
        searchKeywords: ['grifola frondosa', 'maitake mushroom', 'maitake', 'مايتاكي'],
        usageScope: ['skin'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'مستخلص نباتي (فطر)',
        benefits: { 'تحسين مرونة الجلد': 0.88, 'مضاد أكسدة': 0.85, 'ترطيب': 0.8 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'trametes-versicolor-extract',
        name: 'مستخلص فطر ذيل الديك الرومي',
        scientific_name: 'Trametes Versicolor Extract',
        searchKeywords: ['trametes versicolor', 'coriolus versicolor', 'turkey tail mushroom', 'ذيل الديك الرومي'],
        usageScope: ['skin'],
        functionalCategory: 'مضاد أكسدة',
        chemicalType: 'مستخلص نباتي (فطر)',
        benefits: { 'مضاد أكسدة': 0.92, 'دعم ميكروبيوم البشرة': 0.85, 'تفتيح وتوحيد اللون': 0.8 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'غني جدا بمضادات الأكسدة والبريبيوتيك التي تدعم صحة الجلد.' }],
      },
{
        id: 'poria-cocos-extract',
        name: 'مستخلص فطر بوريا كوكوس',
        scientific_name: 'Poria Cocos Extract',
        searchKeywords: ['poria cocos', 'fu ling', 'hoelen', 'بوريا كوكوس'],
        usageScope: ['skin'],
        functionalCategory: 'ملطف ومرمم',
        chemicalType: 'مستخلص نباتي (فطر)',
        benefits: { 'مضاد للالتهابات': 0.9, 'ملطف ومرمم': 0.88, 'مضاد أكسدة': 0.8 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'يستخدم في الطب الصيني التقليدي لتهدئة البشرة وتقليل الانتفاخ.' }],
      },
{
        id: 'phellinus-linteus-extract',
        name: 'مستخلص فطر ميشيما',
        scientific_name: 'Phellinus Linteus Extract',
        searchKeywords: ['phellinus linteus', 'meshima mushroom', 'sang-hwang', 'ميشيما'],
        usageScope: ['skin'],
        functionalCategory: 'مضاد أكسدة',
        chemicalType: 'مستخلص نباتي (فطر)',
        benefits: { 'مضاد أكسدة': 0.9, 'مضاد للالتهابات': 0.88, 'تفتيح وتوحيد اللون': 0.82 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'fomitopsis-officinalis-extract',
        name: 'مستخلص فطر أغاريكون',
        scientific_name: 'Fomitopsis Officinalis (Mushroom) Extract',
        searchKeywords: ['fomitopsis officinalis', 'agarikon mushroom', 'أغاريكون'],
        usageScope: ['skin'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'مستخلص نباتي (فطر)',
        benefits: { 'قابض للمسام': 0.9, 'تنظيم الإفرازات الدهنية': 0.85, 'شد البشرة': 0.8 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'يعمل على تقليل حجم المسام والتحكم في لمعان البشرة الدهنية.' }],
      },
{
        id: 'pleurotus-ostreatus-extract',
        name: 'مستخلص فطر المحار',
        scientific_name: 'Pleurotus Ostreatus (Oyster Mushroom) Extract',
        searchKeywords: ['pleurotus ostreatus', 'oyster mushroom', 'فطر المحار'],
        usageScope: ['skin'],
        functionalCategory: 'مضاد أكسدة',
        chemicalType: 'مستخلص نباتي (فطر)',
        benefits: { 'مضاد أكسدة': 0.88, 'غني بالفيتامينات والمعادن': 0.8 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'flammulina-velutipes-extract',
        name: 'مستخلص فطر إينوكي',
        scientific_name: 'Flammulina Velutipes Extract',
        searchKeywords: ['flammulina velutipes', 'enoki mushroom', 'enokitake', 'إينوكي'],
        usageScope: ['skin'],
        functionalCategory: 'مضاد أكسدة',
        chemicalType: 'مستخلص نباتي (فطر)',
        benefits: { 'مضاد أكسدة': 0.85, 'دعم مناعة الجلد': 0.8 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'auricularia-auricula-judae-extract',
        name: 'مستخلص فطر أذن يهوذا',
        scientific_name: 'Auricularia Auricula-Judae Extract',
        searchKeywords: ['auricularia auricula-judae', 'wood ear mushroom', 'jelly ear', 'أذن يهوذا'],
        usageScope: ['skin'],
        functionalCategory: 'مرطب / مطري',
        chemicalType: 'مستخلص نباتي (فطر)',
        benefits: { 'ترطيب': 0.9, 'مضاد أكسدة': 0.8 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'يشبه فطر الثلج في قدرته على الترطيب.' }],
      },
{
        id: 'albatrellus-confluens-extract',
        name: 'مستخلص فطر ألباتريلوس',
        scientific_name: 'Albatrellus Confluens (Mushroom) Extract',
        searchKeywords: ['albatrellus confluens', 'albatrellus ovinus'],
        usageScope: ['skin'],
        functionalCategory: 'ملطف ومرمم',
        chemicalType: 'مستخلص نباتي (فطر)',
        benefits: { 'تقليل حساسية البشرة': 0.9, 'مضاد للتهيج': 0.92, 'تقليل الاحمرار': 0.85 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'يستخدم في منتجات البشرة الحساسة لتهدئة الإحساس بالوخز والحرقة.' }],
      },
{
        id: 'sparassis-crispa-extract',
        name: 'مستخلص فطر سباراسيس كريسبا',
        scientific_name: 'Sparassis Crispa Extract',
        searchKeywords: ['sparassis crispa', 'cauliflower mushroom', 'فطر القرنبيط'],
        usageScope: ['skin'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'مستخلص نباتي (فطر)',
        benefits: { 'حماية من أضرار الشمس': 0.85, 'تحفيز الكولاجين': 0.8, 'مضاد أكسدة': 0.82 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'agaricus-bisporus-extract',
        name: 'مستخلص فطر الأغاريكوس (الفطر الشائع)',
        scientific_name: 'Agaricus Bisporus (Mushroom) Extract',
        searchKeywords: ['agaricus bisporus', 'common mushroom', 'button mushroom', 'portobello'],
        usageScope: ['skin'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'مستخلص نباتي (فطر)',
        benefits: { 'تفتيح وتوحيد اللون': 0.8, 'مضاد أكسدة': 0.75 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'coprinus-comatus-extract',
        name: 'مستخلص فطر كوباينس',
        scientific_name: 'Coprinus Comatus (Mushroom) Extract',
        searchKeywords: ['coprinus comatus', 'shaggy mane', 'lawyer\'s wig'],
        usageScope: ['skin'],
        functionalCategory: 'مضاد أكسدة',
        chemicalType: 'مستخلص نباتي (فطر)',
        benefits: { 'مضاد أكسدة': 0.85, 'مكافحة الشيخوخة': 0.8 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'pleurotus-eryngii-extract',
        name: 'مستخلص فطر المحار الملكي',
        scientific_name: 'Pleurotus Eryngii Extract',
        searchKeywords: ['pleurotus eryngii', 'king trumpet mushroom', 'king oyster'],
        usageScope: ['skin'],
        functionalCategory: 'مضاد أكسدة',
        chemicalType: 'مستخلص نباتي (فطر)',
        benefits: { 'مضاد أكسدة': 0.88, 'غني بالإرغوثيونين': 0.9 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'fomes-fomentarius-extract',
        name: 'مستخلص فطر فوميس',
        scientific_name: 'Fomes Fomentarius Extract',
        searchKeywords: ['fomes fomentarius', 'tinder fungus', 'hoof fungus'],
        usageScope: ['skin'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'مستخلص نباتي (فطر)',
        benefits: { 'تنظيم الإفرازات الدهنية': 0.85, 'مضاد للميكروبات': 0.8 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'ganoderma-applanatum-extract',
        name: 'مستخلص فطر جانوديرما أبلاناتوم',
        scientific_name: 'Ganoderma Applanatum (Artist\'s Conk) Extract',
        searchKeywords: ['ganoderma applanatum', 'artist\'s conk'],
        usageScope: ['skin'],
        functionalCategory: 'مضاد أكسدة',
        chemicalType: 'مستخلص نباتي (فطر)',
        benefits: { 'مضاد أكسدة': 0.85, 'تفتيح وتوحيد اللون': 0.75 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'schizophyllum-commune-extract',
        name: 'مستخلص فطر شيزوفيلوم',
        scientific_name: 'Schizophyllum Commune Extract',
        searchKeywords: ['schizophyllum commune', 'split-gill mushroom'],
        usageScope: ['skin'],
        functionalCategory: 'مرطب / مطري',
        chemicalType: 'مستخلص نباتي (فطر)',
        benefits: { 'ترطيب': 0.88, 'مضاد أكسدة': 0.8, 'دعم حاجز البشرة': 0.82 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'غني بالبيتا جلوكان.' }],
      },
{
        id: 'tuber-melanosporum-extract',
        name: 'مستخلص الكمأة السوداء',
        scientific_name: 'Tuber Melanosporum (Black Truffle) Extract',
        searchKeywords: ['tuber melanosporum', 'black truffle', 'كمأة سوداء'],
        usageScope: ['skin'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'مستخلص نباتي (فطر)',
        benefits: { 'مكافحة الشيخوخة': 0.9, 'غني بالمعادن والأحماض الأمينية': 0.88, 'تجديد الجلد': 0.85 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'مكون فاخر يستخدم في منتجات مكافحة الشيخوخة الراقية.' }],
      },
{
        id: 'tuber-magnatum-extract',
        name: 'مستخلص الكمأة البيضاء',
        scientific_name: 'Tuber Magnatum Extract',
        searchKeywords: ['tuber magnatum', 'white truffle', 'كمأة بيضاء'],
        usageScope: ['skin'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'مستخلص نباتي (فطر)',
        benefits: { 'تفتيح وتوحيد اللون': 0.9, 'مضاد أكسدة': 0.88, 'تجديد الجلد': 0.85 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'tricholoma-matsutake-extract',
        name: 'مستخلص فطر ماتسوتاكي',
        scientific_name: 'Tricholoma Matsutake Extract',
        searchKeywords: ['tricholoma matsutake', 'matsutake mushroom', 'ماتسوتاكي'],
        usageScope: ['skin'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'مستخلص نباتي (فطر)',
        benefits: { 'تفتيح وتوحيد اللون': 0.92, 'مثبط للتيروزيناز': 0.9, 'مضاد أكسدة': 0.85 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'مشهور في مستحضرات التجميل الآسيوية لقدرته على تفتيح البشرة ومنع التصبغات.' }],
      },
{
        id: 'laetiporus-sulphureus-extract',
        name: 'مستخلص فطر دجاجة الغابة',
        scientific_name: 'Laetiporus Sulphureus (Chicken of the Woods) Extract',
        searchKeywords: ['laetiporus sulphureus', 'chicken of the woods'],
        usageScope: ['skin'],
        functionalCategory: 'مضاد أكسدة',
        chemicalType: 'مستخلص نباتي (فطر)',
        benefits: { 'مضاد للميكروبات': 0.8, 'مضاد أكسدة': 0.75 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'morchella-esculenta-extract',
        name: 'مستخلص فطر الموريل',
        scientific_name: 'Morchella Esculenta (Morel) Extract',
        searchKeywords: ['morchella esculenta', 'morel mushroom', 'موريل'],
        usageScope: ['skin'],
        functionalCategory: 'مضاد أكسدة',
        chemicalType: 'مستخلص نباتي (فطر)',
        benefits: { 'مضاد أكسدة': 0.85, 'غني بالمعادن': 0.8 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'hypsizygus-marmoreus-extract',
        name: 'مستخلص فطر الزان',
        scientific_name: 'Hypsizygus Marmoreus Extract',
        searchKeywords: ['hypsizygus marmoreus', 'beech mushroom', 'shimeji', 'فطر الزان'],
        usageScope: ['skin'],
        functionalCategory: 'مضاد أكسدة',
        chemicalType: 'مستخلص نباتي (فطر)',
        benefits: { 'مضاد أكسدة': 0.8, 'مضاد للالتهابات': 0.75 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'boletus-edulis-extract',
        name: 'مستخلص فطر البوليط',
        scientific_name: 'Boletus Edulis (Porcini) Extract',
        searchKeywords: ['boletus edulis', 'porcini mushroom', 'بورشيني'],
        usageScope: ['skin'],
        functionalCategory: 'مضاد أكسدة',
        chemicalType: 'مستخلص نباتي (فطر)',
        benefits: { 'مضاد أكسدة': 0.85, 'غني بالإرغوثيونين': 0.88 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'cantharellus-cibarius-extract',
        name: 'مستخلص فطر الشانتريل',
        scientific_name: 'Cantharellus Cibarius (Chanterelle) Extract',
        searchKeywords: ['cantharellus cibarius', 'chanterelle mushroom', 'شانتريل'],
        usageScope: ['skin'],
        functionalCategory: 'مضاد أكسدة',
        chemicalType: 'مستخلص نباتي (فطر)',
        benefits: { 'غني بالفيتامينات': 0.8, 'مضاد للميكروبات': 0.75 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'armillaria-mellea-extract',
        name: 'مستخلص فطر العسل',
        scientific_name: 'Armillaria Mellea (Honey Mushroom) Extract',
        searchKeywords: ['armillaria mellea', 'honey mushroom', 'فطر العسل'],
        usageScope: ['skin'],
        functionalCategory: 'مضاد أكسدة',
        chemicalType: 'مستخلص نباتي (فطر)',
        benefits: { 'مضاد أكسدة': 0.8, 'مضاد للميكروبات': 0.75 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'volvariella-volvacea-extract',
        name: 'مستخلص فطر قش الأرز',
        scientific_name: 'Volvariella Volvacea Extract',
        searchKeywords: ['volvariella volvacea', 'paddy straw mushroom', 'فطر قش الأرز'],
        usageScope: ['skin'],
        functionalCategory: 'مضاد أكسدة',
        chemicalType: 'مستخلص نباتي (فطر)',
        benefits: { 'مضاد أكسدة': 0.8, 'ترطيب': 0.75 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'auricularia-polytricha-extract',
        name: 'مستخلص فطر الأذن السحابي',
        scientific_name: 'Auricularia Polytricha Extract',
        searchKeywords: ['auricularia polytricha', 'cloud ear mushroom', 'فطر الأذن السحابي'],
        usageScope: ['skin'],
        functionalCategory: 'مرطب / مطري',
        chemicalType: 'مستخلص نباتي (فطر)',
        benefits: { 'ترطيب': 0.88, 'مضاد أكسدة': 0.8 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'grifola-umbellata-extract',
        name: 'مستخلص فطر غريفولا أومبيلاتا',
        scientific_name: 'Grifola Umbellata (Zhu Ling) Extract',
        searchKeywords: ['grifola umbellata', 'zhu ling'],
        usageScope: ['skin'],
        functionalCategory: 'مضاد أكسدة',
        chemicalType: 'مستخلص نباتي (فطر)',
        benefits: { 'مضاد أكسدة': 0.8, 'مضاد للالتهابات': 0.75 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'wolfiporia-extensa-extract',
        name: 'مستخلص وولفيبوريا إكستنسا',
        scientific_name: 'Wolfiporia Extensa Extract',
        searchKeywords: ['wolfiporia extensa', 'poria cocos sclerotium'],
        usageScope: ['skin'],
        functionalCategory: 'ملطف ومرمم',
        chemicalType: 'مستخلص نباتي (فطر)',
        benefits: { 'مضاد للالتهابات': 0.88, 'مضاد أكسدة': 0.85 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'جزء آخر من فطر Poria Cocos.' }],
      },
{
        id: 'ustulina-deusta-extract',
        name: 'مستخلص فطر أوستولينا',
        scientific_name: 'Ustulina Deusta Extract',
        searchKeywords: ['ustulina deusta', 'carbon cushion'],
        usageScope: ['skin'],
        functionalCategory: 'مضاد أكسدة',
        chemicalType: 'مستخلص نباتي (فطر)',
        benefits: { 'مضاد أكسدة': 0.8 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'amanita-muscaria-extract',
        name: 'مستخلص فطر أمانيتا موسكاريا',
        scientific_name: 'Amanita Muscaria Extract',
        searchKeywords: ['amanita muscaria', 'fly agaric'],
        usageScope: ['skin'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'مستخلص نباتي (فطر)',
        benefits: { 'مضاد للالتهابات': 0.75 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'risk', text: 'فطر سام عند تناوله، ولكن مستخلصاته المعالجة تستخدم بتركيزات منخفضة جدا لخصائصها المضادة للالتهابات.' }],
      },
{ id: 'lactarius-deliciosus-extract', name: 'مستخلص فطر لاكتاريوس', scientific_name: 'Lactarius Deliciosus Extract', searchKeywords: ['saffron milk cap'], usageScope: ['skin'], functionalCategory: 'مضاد أكسدة', chemicalType: 'مستخلص نباتي (فطر)', benefits: {'مضاد أكسدة': 0.8}, synergy: {}, negativeSynergy: {}, warnings: [] },
{ id: 'russula-virescens-extract', name: 'مستخلص فطر روسولا', scientific_name: 'Russula Virescens Extract', searchKeywords: ['russula virescens'], usageScope: ['skin'], functionalCategory: 'مضاد أكسدة', chemicalType: 'مستخلص نباتي (فطر)', benefits: {'مضاد أكسدة': 0.75}, synergy: {}, negativeSynergy: {}, warnings: [] },
{ id: 'suillus-bovinus-extract', name: 'مستخلص فطر سويليوس', scientific_name: 'Suillus Bovinus Extract', searchKeywords: ['suillus bovinus'], usageScope: ['skin'], functionalCategory: 'مضاد أكسدة', chemicalType: 'مستخلص نباتي (فطر)', benefits: {'مضاد أكسدة': 0.75}, synergy: {}, negativeSynergy: {}, warnings: [] },
{ id: 'paxillus-involutus-extract', name: 'مستخلص فطر باكسيلوس', scientific_name: 'Paxillus Involutus Extract', searchKeywords: ['paxillus involutus'], usageScope: ['skin'], functionalCategory: 'مضاد أكسدة', chemicalType: 'مستخلص نباتي (فطر)', benefits: {'مضاد أكسدة': 0.7}, synergy: {}, negativeSynergy: {}, warnings: [] },
{ id: 'lycoperdon-perlatum-extract', name: 'مستخلص فطر ليكوبيردون', scientific_name: 'Lycoperdon Perlatum Extract', searchKeywords: ['common puffball'], usageScope: ['skin'], functionalCategory: 'ملطف ومرمم', chemicalType: 'مستخلص نباتي (فطر)', benefits: {'ملطف ومرمم': 0.8}, synergy: {}, negativeSynergy: {}, warnings: [] },
{ id: 'ganoderma-sinense-extract', name: 'مستخلص فطر جانوديرما سينينس', scientific_name: 'Ganoderma Sinense Extract', searchKeywords: ['ganoderma sinense'], usageScope: ['skin'], functionalCategory: 'مضاد أكسدة', chemicalType: 'مستخلص نباتي (فطر)', benefits: {'مضاد أكسدة': 0.85}, synergy: {}, negativeSynergy: {}, warnings: [] },
{ id: 'ophiocordyceps-sinensis-extract', name: 'مستخلص أوفيوكورديسيبس', scientific_name: 'Ophiocordyceps Sinensis Extract', searchKeywords: ['ophiocordyceps sinensis'], usageScope: ['skin'], functionalCategory: 'مضاد أكسدة', chemicalType: 'مستخلص نباتي (فطر)', benefits: {'مضاد أكسدة': 0.9, 'مكافحة الشيخوخة': 0.85}, synergy: {}, negativeSynergy: {}, warnings: [] },
{ id: 'pholiota-nameko-extract', name: 'مستخلص فطر ناميكو', scientific_name: 'Pholiota Nameko Extract', searchKeywords: ['pholiota nameko', 'nameko mushroom'], usageScope: ['skin'], functionalCategory: 'مرطب / مطري', chemicalType: 'مستخلص نباتي (فطر)', benefits: {'ترطيب': 0.85}, synergy: {}, negativeSynergy: {}, warnings: [] },
{ id: 'agrocybe-aegerita-extract', name: 'مستخلص فطر أغروسايب', scientific_name: 'Agrocybe Aegerita Extract', searchKeywords: ['agrocybe aegerita', 'pioppino'], usageScope: ['skin'], functionalCategory: 'مضاد أكسدة', chemicalType: 'مستخلص نباتي (فطر)', benefits: {'مضاد أكسدة': 0.8}, synergy: {}, negativeSynergy: {}, warnings: [] },
{ id: 'coprinellus-micaceus-extract', name: 'مستخلص فطر كوبرينيلوس', scientific_name: 'Coprinellus Micaceus Extract', searchKeywords: ['coprinellus micaceus'], usageScope: ['skin'], functionalCategory: 'مضاد أكسدة', chemicalType: 'مستخلص نباتي (فطر)', benefits: {'مضاد أكسدة': 0.75}, synergy: {}, negativeSynergy: {}, warnings: [] },
{ id: 'hydnum-repandum-extract', name: 'مستخلص فطر هيدنوم', scientific_name: 'Hydnum Repandum Extract', searchKeywords: ['hydnum repandum', 'hedgehog mushroom'], usageScope: ['skin'], functionalCategory: 'مضاد أكسدة', chemicalType: 'مستخلص نباتي (فطر)', benefits: {'مضاد أكسدة': 0.8}, synergy: {}, negativeSynergy: {}, warnings: [] },
{ id: 'lepista-nuda-extract', name: 'مستخلص فطر ليبيستا', scientific_name: 'Lepista Nuda Extract', searchKeywords: ['lepista nuda', 'wood blewit'], usageScope: ['skin'], functionalCategory: 'مضاد أكسدة', chemicalType: 'مستخلص نباتي (فطر)', benefits: {'مضاد أكسدة': 0.8}, synergy: {}, negativeSynergy: {}, warnings: [] },
{ id: 'marasmius-oreades-extract', name: 'مستخلص فطر ماراسميوس', scientific_name: 'Marasmius Oreades Extract', searchKeywords: ['marasmius oreades', 'fairy ring mushroom'], usageScope: ['skin'], functionalCategory: 'مضاد أكسدة', chemicalType: 'مستخلص نباتي (فطر)', benefits: {'مضاد أكسدة': 0.75}, synergy: {}, negativeSynergy: {}, warnings: [] },
{ id: 'stereum-hirsutum-extract', name: 'مستخلص فطر ستيريوم', scientific_name: 'Stereum Hirsutum Extract', searchKeywords: ['stereum hirsutum', 'hairy curtain crust'], usageScope: ['skin'], functionalCategory: 'مضاد أكسدة', chemicalType: 'مستخلص نباتي (فطر)', benefits: {'مضاد أكسدة': 0.8}, synergy: {}, negativeSynergy: {}, warnings: [] },
{ id: 'piptoporus-betulinus-extract', name: 'مستخلص فطر بيتوليناس', scientific_name: 'Piptoporus Betulinus Extract', searchKeywords: ['piptoporus betulinus', 'birch polypore'], usageScope: ['skin'], functionalCategory: 'ملطف ومرمم', chemicalType: 'مستخلص نباتي (فطر)', benefits: {'مضاد للالتهابات': 0.85, 'مضاد للميكروبات': 0.8}, synergy: {}, negativeSynergy: {}, warnings: [] },
{
        id: 'tea-dodecylbenzenesulfonate',
        name: 'TEA-دوديسيل بنزين سلفونات',
        scientific_name: 'TEA-Dodecylbenzenesulfonate',
        searchKeywords: ['tea-dodecylbenzenesulfonate'],
        usageScope: ['hair'],
        functionalCategory: 'منظف',
        chemicalType: 'منظف قوي',
        benefits: { 'تنظيف فعال': 0.88, 'رغوة جيدة': 0.8 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'caution', text: 'منظف أنيوني قوي، قد يكون مجففا.' }],
      },
{
        id: 'trideceth-10',
        name: 'ترايديسيث-10',
        scientific_name: 'Trideceth-10',
        searchKeywords: ['trideceth-10'],
        usageScope: ['hair'],
        functionalCategory: 'مستحلب',
        chemicalType: 'مستحلب',
        benefits: { 'يساعد على توزيع السيليكون': 0.85, 'مستحلب': 0.8 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'sodium-salicylate',
        name: 'ساليسيلات الصوديوم',
        scientific_name: 'Sodium Salicylate',
        searchKeywords: ['sodium salicylate', 'ساليسيلات الصوديوم'],
        usageScope: ['hair'],
        functionalCategory: 'مادة حافظة',
        chemicalType: 'مادة حافظة',
        benefits: { 'مادة حافظة': 0.8, 'مضاد للالتهابات خفيف': 0.7 },
        synergy: {},
        negativeSynergy: {},
        warnings: [
          { level: 'info', text: 'ملح حمض الساليسيليك، لا يمتلك نفس قدرات التقشير ولكنه يعمل كمادة حافظة.' },
          { level: 'risk', text: 'يجب تجنبه من قبل الأشخاص الذين يعانون من حساسية الأسبرين.' }
        ],
      },
{
        id: 'sodium-acetate',
        name: 'أسيتات الصوديوم',
        scientific_name: 'Sodium Acetate',
        searchKeywords: ['sodium acetate', 'أسيتات الصوديوم'],
        usageScope: ['hair'],
        functionalCategory: 'محسن قوام',
        chemicalType: 'معدل الحموضة',
        benefits: { 'تنظيم درجة الحموضة': 0.8 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'isopropyl-palmitate',
        name: 'بالميتات الآيزوبروبيل',
        scientific_name: 'Isopropyl Palmitate',
        searchKeywords: ['isopropyl palmitate', 'ipp', 'بالميتات الآيزوبروبيل'],
        usageScope: ['skin', 'hair'],
        functionalCategory: 'مرطب / مطري',
        chemicalType: 'مطري',
        benefits: { 'مطري ومنعم': 0.85, 'تحسين انتشار المنتج': 0.8 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'risk', text: 'يعتبر من المواد شديدة السد للمسام (كوميدوجينيك). يجب تجنبه على البشرة المعرضة لحب الشباب.' }],
      },
{
        id: 'isopropyl-myristate',
        name: 'ميريستات الآيزوبروبيل',
        scientific_name: 'Isopropyl Myristate',
        searchKeywords: ['isopropyl myristate', 'ipm', 'ميريستات الآيزوبروبيل'],
        usageScope: ['skin', 'hair'],
        functionalCategory: 'مرطب / مطري',
        chemicalType: 'مطري',
        benefits: { 'مطري ومنعم': 0.8, 'تحسين امتصاص المكونات': 0.85, 'تقليل الملمس الدهني': 0.75 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'risk', text: 'يعتبر من المواد شديدة السد للمسام (كوميدوجينيك).' }],
      },
{
        id: 'polyquaternium-4',
        name: 'بولي كواتيرنيوم-4',
        scientific_name: 'Polyquaternium-4',
        searchKeywords: ['polyquaternium-4', 'pq-4'],
        usageScope: ['hair'],
        functionalCategory: 'مثبت للشعر',
        chemicalType: 'بوليمر',
        benefits: { 'تثبيت التصفيف': 0.88, 'تكييف خفيف': 0.8 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'يستخدم في منتجات التصفيف مثل الجل والموس لإعطاء ثبات مرن.' }],
      },
{
        id: 'polyquaternium-16',
        name: 'بولي كواتيرنيوم-16',
        scientific_name: 'Polyquaternium-16',
        searchKeywords: ['polyquaternium-16', 'pq-16'],
        usageScope: ['hair'],
        functionalCategory: 'مكيف للشعر',
        chemicalType: 'بوليمر',
        benefits: { 'إضافة حجم للشعر': 0.85, 'تكييف خفيف': 0.8, 'مضاد للكهرباء الساكنة': 0.82 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'polyquaternium-22',
        name: 'بولي كواتيرنيوم-22',
        scientific_name: 'Polyquaternium-22',
        searchKeywords: ['polyquaternium-22', 'pq-22'],
        usageScope: ['hair'],
        functionalCategory: 'مكيف للشعر',
        chemicalType: 'بوليمر',
        benefits: { 'تكييف ممتاز (رطب وجاف)': 0.9, 'فك تشابك الشعر': 0.88 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'polyquaternium-28',
        name: 'بولي كواتيرنيوم-28',
        scientific_name: 'Polyquaternium-28',
        searchKeywords: ['polyquaternium-28', 'pq-28'],
        usageScope: ['hair'],
        functionalCategory: 'مكيف للشعر',
        chemicalType: 'بوليمر',
        benefits: { 'تكييف': 0.85, 'تثبيت التموجات': 0.8 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'polyquaternium-47',
        name: 'بولي كواتيرنيوم-47',
        scientific_name: 'Polyquaternium-47',
        searchKeywords: ['polyquaternium-47', 'pq-47'],
        usageScope: ['hair'],
        functionalCategory: 'مكيف للشعر',
        chemicalType: 'بوليمر',
        benefits: { 'تكييف ممتاز': 0.9, 'فك تشابك الشعر': 0.92, 'يترك الشعر ناعما': 0.88 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'polyquaternium-74',
        name: 'بولي كواتيرنيوم-74',
        scientific_name: 'Polyquaternium-74',
        searchKeywords: ['polyquaternium-74', 'pq-74'],
        usageScope: ['hair'],
        functionalCategory: 'مكيف للشعر',
        chemicalType: 'بوليمر',
        benefits: { 'تكييف': 0.8, 'إضافة لمعان': 0.85 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'vp-va-copolymer',
        name: 'VP/VA كوبوليمر',
        scientific_name: 'VP/VA Copolymer',
        searchKeywords: ['vp/va copolymer', 'polyvinylpyrrolidone/vinyl acetate copolymer'],
        usageScope: ['hair'],
        functionalCategory: 'مثبت للشعر',
        chemicalType: 'بوليمر صناعي',
        benefits: { 'تثبيت تصفيفة الشعر': 0.92, 'تكوين طبقة رقيقة': 0.9, 'مقاومة للرطوبة': 0.85 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'بوليمر أساسي في مثبتات الشعر (السبراي) والجل.' }],
      },
{
        id: 'acrylates-beheneth-25-methacrylate-copolymer',
        name: 'أكريلات/بيهينيث-25 ميثاكريليت كوبوليمر',
        scientific_name: 'Acrylates/Beheneth-25 Methacrylate Copolymer',
        searchKeywords: ['acrylates/beheneth-25 methacrylate copolymer'],
        usageScope: ['hair'],
        functionalCategory: 'محسن قوام',
        chemicalType: 'بوليمر صناعي',
        benefits: { 'مكثف للقوام': 0.9, 'مثبت للتصفيف': 0.8 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'nigella-sativa-seed-extract',
        name: 'مستخلص بذور حبة البركة',
        scientific_name: 'Nigella Sativa Seed Extract',
        searchKeywords: ['nigella sativa extract', 'black seed extract', 'مستخلص حبة البركة'],
        usageScope: ['hair'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'مستخلص نباتي',
        benefits: { 'مضاد للالتهابات': 0.9, 'مضاد أكسدة': 0.88, 'صحة فروة الرأس': 0.85 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'مكون شائع جدا في المنتجات الموجهة للسوق الإقليمية لفوائده التقليدية.' }],
      },
{
        id: 'argania-spinosa-kernel-extract',
        name: 'مستخلص نواة الأرغان',
        scientific_name: 'Argania Spinosa Kernel Extract',
        searchKeywords: ['argania spinosa extract', 'argan extract', 'مستخلص الأرغان'],
        usageScope: ['hair'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'مستخلص نباتي',
        benefits: { 'مضاد أكسدة': 0.85, 'مكافحة الشيخوخة': 0.8 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'يختلف عن زيت الأرغان؛ هذا هو المستخلص المائي الذي يحتوي على البروتينات والبوليفينولات.' }],
      },
{
        id: 'prunus-amygdalus-dulcis-seed-extract',
        name: 'مستخلص بذور اللوز الحلو',
        scientific_name: 'Prunus Amygdalus Dulcis (Sweet Almond) Seed Extract',
        searchKeywords: ['sweet almond extract', 'مستخلص اللوز الحلو'],
        usageScope: ['hair'],
        functionalCategory: 'مرطب / مطري',
        chemicalType: 'مستخلص نباتي',
        benefits: { 'ترطيب': 0.8, 'تنعيم الشعر': 0.85 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'butyrospermum-parkii-butter-extract',
        name: 'مستخلص زبدة الشيا',
        scientific_name: 'Butyrospermum Parkii (Shea) Butter Extract',
        searchKeywords: ['shea butter extract', 'مستخلص زبدة الشيا'],
        usageScope: ['hair'],
        functionalCategory: 'مرطب / مطري',
        chemicalType: 'مستخلص نباتي',
        benefits: { 'ترطيب': 0.85, 'تغذية': 0.8 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'anthemis-nobilis-flower-extract',
        name: 'مستخلص زهرة البابونج الروماني',
        scientific_name: 'Anthemis Nobilis Flower Extract',
        searchKeywords: ['anthemis nobilis', 'roman chamomile extract', 'مستخلص البابونج الروماني'],
        usageScope: ['hair'],
        functionalCategory: 'ملطف ومرمم',
        chemicalType: 'مستخلص نباتي',
        benefits: { 'ملطف ومرمم': 0.9, 'تهدئة فروة الرأس': 0.88, 'إضافة لمعان للشعر الفاتح': 0.8 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'urtica-dioica-leaf-extract',
        name: 'مستخلص أوراق نبات القراص',
        scientific_name: 'Urtica Dioica (Nettle) Leaf Extract',
        searchKeywords: ['nettle leaf extract', 'urtica dioica extract', 'مستخلص القراص'],
        usageScope: ['hair'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'مستخلص نباتي',
        benefits: { 'تقليل تساقط الشعر': 0.8, 'مضاد للالتهابات': 0.85, 'علاج القشرة': 0.78 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'paullinia-cupana-seed-extract',
        name: 'مستخلص بذور الغوارانا',
        scientific_name: 'Paullinia Cupana Seed Extract',
        searchKeywords: ['paullinia cupana', 'guarana extract', 'غوارانا'],
        usageScope: ['hair'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'مستخلص نباتي',
        benefits: { 'غني بالكافيين': 0.9, 'تنشيط فروة الرأس': 0.88, 'تحفيز الدورة الدموية': 0.85 },
        synergy: { 'caffeine': 1.05 },
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'lawsonia-inermis-extract',
        name: 'مستخلص الحناء',
        scientific_name: 'Lawsonia Inermis (Henna) Extract',
        searchKeywords: ['lawsonia inermis', 'henna extract', 'مستخلص الحناء'],
        usageScope: ['hair'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'مستخلص نباتي',
        benefits: { 'تقوية الشعر': 0.88, 'إضافة لون نحاسي': 0.9, 'تغطية الشعر الأبيض': 0.85 },
        synergy: {},
        negativeSynergy: {},
        warnings: [
          { level: 'caution', text: 'يعطي لونا للشعر، قد لا يكون مرغوبا للجميع. قد يتفاعل بشكل سيء مع الصبغات الكيميائية.' }
        ],
      },
{
        id: 'eclipta-prostrata-extract',
        name: 'مستخلص البرينجراج',
        scientific_name: 'Eclipta Prostrata Extract',
        searchKeywords: ['eclipta prostrata', 'bhringraj', 'false daisy', 'برينجراج'],
        usageScope: ['hair'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'مستخلص نباتي',
        benefits: { 'تقليل تساقط الشعر': 0.88, 'تحفيز نمو الشعر': 0.85, 'تأخير الشيب': 0.8 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'عشب أيورفيدي مشهور لصحة الشعر.' }],
      },
{
        id: 'centaurea-cyanus-flower-extract',
        name: 'مستخلص زهرة القنطريون العنبري',
        scientific_name: 'Centaurea Cyanus Flower Extract',
        searchKeywords: ['centaurea cyanus', 'cornflower extract', 'قنطريون عنبري'],
        usageScope: ['hair'],
        functionalCategory: 'ملطف ومرمم',
        chemicalType: 'مستخلص نباتي',
        benefits: { 'تهدئة فروة الرأس': 0.85, 'تقليل الاصفرار في الشعر الفاتح': 0.8 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'iodopropynyl-butylcarbamate',
        name: 'يودوبروبينيل بوتيل كاربامات (IPBC)',
        scientific_name: 'Iodopropynyl Butylcarbamate',
        searchKeywords: ['iodopropynyl butylcarbamate', 'ipbc'],
        usageScope: ['hair', 'skin'],
        functionalCategory: 'مادة حافظة',
        chemicalType: 'مادة حافظة (مثيرة للجدل)',
        benefits: { 'حفظ المنتج ضد الفطريات': 0.9 },
        synergy: {},
        negativeSynergy: {},
        warnings: [
          { level: 'risk', text: 'مادة حافظة فعالة ولكنها قد تسبب حساسية وتهيجا شديدا. مقيدة بشدة في العديد من البلدان (مثل أوروبا واليابان).' },
          { level: 'risk', text: 'سام عند الاستنشاق، لا يستخدم في منتجات الرش (سبراي).' }
        ],
      },
{
        id: 'imidazolidinyl-urea',
        name: 'إيميدازوليدينيل يوريا',
        scientific_name: 'Imidazolidinyl Urea',
        searchKeywords: ['imidazolidinyl urea', 'germal 115'],
        usageScope: ['hair', 'skin'],
        functionalCategory: 'مادة حافظة',
        chemicalType: 'مطلق للفورمالديهايد',
        benefits: { 'حفظ المنتج من البكتيريا': 0.85 },
        synergy: {},
        negativeSynergy: {},
        warnings: [
          { level: 'risk', text: 'يطلق ببطء مادة الفورمالديهايد، وهي مادة مسرطنة معروفة ومسببة للحساسية.' }
        ],
      },
{
        id: 'diazolidinyl-urea',
        name: 'ديازوليدينيل يوريا',
        scientific_name: 'Diazolidinyl Urea',
        searchKeywords: ['diazolidinyl urea', 'germal II'],
        usageScope: ['hair', 'skin'],
        functionalCategory: 'مادة حافظة',
        chemicalType: 'مطلق للفورمالديهايد',
        benefits: { 'حفظ المنتج من البكتيريا': 0.88 },
        synergy: {},
        negativeSynergy: {},
        warnings: [
          { level: 'risk', text: 'يطلق ببطء مادة الفورمالديهايد، وهي مادة مسرطنة معروفة ومسببة للحساسية.' }
        ],
      },
{
        id: 'quaternium-15',
        name: 'كواتيرنيوم-15',
        scientific_name: 'Quaternium-15',
        searchKeywords: ['quaternium-15'],
        usageScope: ['hair', 'skin'],
        functionalCategory: 'مادة حافظة',
        chemicalType: 'مطلق للفورمالديهايد',
        benefits: { 'حفظ المنتج': 0.8 },
        synergy: {},
        negativeSynergy: {},
        warnings: [
          { level: 'risk', text: 'أقوى مطلق للفورمالديهايد ومسبب شائع جدا لالتهاب الجلد التماسي التحسسي.' }
        ],
      },
{
        id: 'ppg-5-ceteth-20',
        name: 'PPG-5 سيتيث-20',
        scientific_name: 'PPG-5-Ceteth-20',
        searchKeywords: ['ppg-5-ceteth-20'],
        usageScope: ['hair'],
        functionalCategory: 'مستحلب',
        chemicalType: 'مستحلب',
        benefits: { 'مذوب للعطور': 0.85, 'مستحلب': 0.8 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'peg-55-propylene-glycol-oleate',
        name: 'PEG-55 بروبيلين جلايكول أوليات',
        scientific_name: 'PEG-55 Propylene Glycol Oleate',
        searchKeywords: ['peg-55 propylene glycol oleate'],
        usageScope: ['hair'],
        functionalCategory: 'محسن قوام',
        chemicalType: 'بوليمر',
        benefits: { 'مكثف للقوام': 0.8 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'trideceth-12',
        name: 'ترايديسيث-12',
        scientific_name: 'Trideceth-12',
        searchKeywords: ['trideceth-12'],
        usageScope: ['hair'],
        functionalCategory: 'مستحلب',
        chemicalType: 'مستحلب',
        benefits: { 'يساعد على توزيع السيليكون': 0.9, 'يمنع تراكم السيليكون': 0.85 },
        synergy: { 'amodimethicone': 1.1 },
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'غالبا ما يستخدم مع Amodimethicone لجعله أسهل في الشطف ومنع تراكمه.' }],
      },
{
        id: 'c11-15-pareth-7',
        name: 'C11-15 باريث-7',
        scientific_name: 'C11-15 Pareth-7',
        searchKeywords: ['c11-15 pareth-7'],
        usageScope: ['hair'],
        functionalCategory: 'مستحلب',
        chemicalType: 'مستحلب',
        benefits: { 'مستحلب': 0.8, 'منظف ثانوي': 0.75 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'zinc-oxide',
        name: 'أكسيد الزنك',
        scientific_name: 'Zinc Oxide',
        searchKeywords: ['zinc oxide', 'z-cote', 'zinclear', 'أكسيد الزنك', 'زنك اوكسيد'],
        usageScope: ['skin'],
        functionalCategory: 'حماية من الشمس',
        chemicalType: 'فلتر معدني',
        benefits: { 'حماية واسعة الطيف (UVA/UVB)': 0.98, 'ملطف ومرمم': 0.85, 'مضاد للالتهابات': 0.8, 'مناسب للبشرة الحساسة': 0.95 },
        synergy: { 'titanium-dioxide': 1.1 },
        negativeSynergy: {},
        warnings: [{ level: 'caution', text: 'قد يترك أثرا أبيض على البشرة (white cast)، خاصة في التركيبات غير النانوية وعلى البشرة الداكنة.' }],
      },
{
        id: 'titanium-dioxide',
        name: 'ثاني أكسيد التيتانيوم',
        scientific_name: 'Titanium Dioxide',
        searchKeywords: ['titanium dioxide', 'tio2', 'ci 77891', 'ثاني أكسيد التيتانيوم', 'تيتانيوم ديوكسيد'],
        usageScope: ['skin'],
        functionalCategory: 'حماية من الشمس',
        chemicalType: 'فلتر معدني',
        benefits: { 'حماية قوية من UVB': 0.95, 'حماية من UVA2': 0.8, 'مناسب للبشرة الحساسة': 0.9 },
        synergy: { 'zinc-oxide': 1.1 },
        negativeSynergy: {},
        warnings: [
          { level: 'caution', text: 'حمايته من أشعة UVA الطويلة (UVA1) أضعف من أكسيد الزنك، لذا غالبا ما يتم دمجهما.' },
          { level: 'caution', text: 'قد يترك أثرا أبيض واضحا على البشرة.' },
          { level: 'risk', text: 'شكله كمسحوق (powder) مصنف كمسرطن محتمل (الفئة 2B) عند الاستنشاق، ولكنه يعتبر آمنا في الكريمات والمستحضرات.' }
        ],
      },
{
        id: 'avobenzone',
        name: 'أفوبينزون',
        scientific_name: 'Butyl Methoxydibenzoylmethane',
        searchKeywords: ['avobenzone', 'butyl methoxydibenzoylmethane', 'parsol 1789', 'أفوبينزون', 'Butyl Methoxydibenzoyl-methane'],
        usageScope: ['skin'],
        functionalCategory: 'حماية من الشمس',
        chemicalType: 'فلتر كيميائي (UVA)',
        benefits: { 'حماية ممتازة من UVA': 0.95 },
        synergy: { 'octocrylene': 1.1, 'tinosorb-s': 1.1, 'mexoryl-xl': 1.1 },
        negativeSynergy: {
          'octinoxate': { reason: 'الأوكتينوكسات يزعزع استقرار الأفوبينزون بسرعة تحت الشمس، مما يقلل بشكل كبير من فعالية الحماية ضد أشعة UVA.' }
        },
        warnings: [
          { level: 'risk', text: 'غير مستقر ضوئيا بشكل كبير لوحده، ويفقد فعاليته بسرعة تحت الشمس. يجب تثبيته بمكونات أخرى (مثل Octocrylene, Tinosorb, Mexoryl) ليعمل بفعالية.' },
          { level: 'caution', text: 'قد يسبب تهيجا للعينين أو حساسية جلدية لدى البعض.' }
        ],
      },
{
        id: 'octinoxate',
        name: 'أوكتينوكسات',
        scientific_name: 'Ethylhexyl Methoxycinnamate',
        searchKeywords: ['octinoxate', 'ethylhexyl methoxycinnamate', 'ehcm', 'octyl methoxycinnamate', 'أوكتينوكسات'],
        usageScope: ['skin'],
        functionalCategory: 'حماية من الشمس',
        chemicalType: 'فلتر كيميائي (UVB)',
        benefits: { 'حماية قوية من UVB': 0.9 },
        synergy: {},
        negativeSynergy: {},
        warnings: [
          { level: 'risk', text: 'ضار جدا بالشعاب المرجانية وممنوع في العديد من المناطق الساحلية (مثل هاواي). يُمتص عبر الجلد إلى مجرى الدم.' },
          { level: 'caution', text: 'قد يكون له تأثير هرموني طفيف (شبيه بالإستروجين). غير مستقر ضوئيا ويمكن أن يزعزع استقرار فلاتر أخرى مثل الأفوبينزون.' }
        ],
      },
{
        id: 'oxybenzone',
        name: 'أوكسي بنزون',
        scientific_name: 'Oxybenzone / Benzophenone-3',
        searchKeywords: ['oxybenzone', 'benzophenone-3', 'bp-3', 'أوكسي بنزون'],
        usageScope: ['skin'],
        functionalCategory: 'حماية من الشمس',
        chemicalType: 'فلتر كيميائي (مثيرة للجدل)',
        benefits: { 'حماية من UVB و UVA2': 0.85 },
        synergy: {},
        negativeSynergy: {},
        warnings: [
          { level: 'risk', text: 'ضار جدا بالشعاب المرجانية. يُمتص عبر الجلد بشكل كبير وقد تم ربطه باضطرابات هرمونية محتملة.' },
          { level: 'risk', text: 'مسبب شائع للحساسية الجلدية التلامسية. العديد من أطباء الجلد يوصون بتجنبه.' }
        ],
      },
{
        id: 'homosalate',
        name: 'هوموسالات',
        scientific_name: 'Homosalate',
        searchKeywords: ['homosalate', 'هوموسالات'],
        usageScope: ['skin'],
        functionalCategory: 'حماية من الشمس',
        chemicalType: 'فلتر كيميائي (UVB)',
        benefits: { 'حماية من UVB': 0.8, 'مذيب للفلاتر الأخرى': 0.85 },
        synergy: {},
        negativeSynergy: {},
        warnings: [
          { level: 'caution', text: 'فلتر UVB ضعيف، يستخدم بشكل أساسي للمساعدة في إذابة الفلاتر الأخرى الأصعب في التركيبة.' },
          { level: 'caution', text: 'قد يكون له تأثير هرموني طفيف ويُمتص عبر الجلد.' }
        ],
      },
{
        id: 'octisalate',
        name: 'أوكتيسالات',
        scientific_name: 'Ethylhexyl Salicylate',
        searchKeywords: ['octisalate', 'ethylhexyl salicylate', 'octyl salicylate', 'أوكتيسالات'],
        usageScope: ['skin'],
        functionalCategory: 'حماية من الشمس',
        chemicalType: 'فلتر كيميائي (UVB)',
        benefits: { 'حماية من UVB': 0.85, 'مطري': 0.7 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'فلتر UVB مستقر ولكنه ضعيف نسبيا، يستخدم دائما مع فلاتر أخرى لتعزيز الحماية.' }],
      },
{
        id: 'octocrylene',
        name: 'أوكتوكريلين',
        scientific_name: 'Octocrylene',
        searchKeywords: ['octocrylene', 'أوكتوكريلين'],
        usageScope: ['skin'],
        functionalCategory: 'حماية من الشمس',
        chemicalType: 'فلتر كيميائي (مثبت)',
        benefits: { 'مثبت للأفوبينزون': 0.92, 'حماية من UVB و UVA2': 0.8, 'مقاوم للماء': 0.85 },
        synergy: { 'avobenzone': 1.1 },
        negativeSynergy: {},
        warnings: [
          { level: 'caution', text: 'قد يسبب حساسية لدى البعض، خاصة الأطفال. يمكن أن يتحلل بمرور الوقت إلى البنزوفينون، وهي مادة مسرطنة محتملة.' },
          { level: 'risk', text: 'تم تقييد استخدامه في بعض المناطق بسبب مخاوف التحلل.' }
        ],
      },
{
        id: 'ensulizole',
        name: 'إنسوليزول',
        scientific_name: 'Phenylbenzimidazole Sulfonic Acid',
        searchKeywords: ['ensulizole', 'phenylbenzimidazole sulfonic acid', 'إنسوليزول'],
        usageScope: ['skin'],
        functionalCategory: 'حماية من الشمس',
        chemicalType: 'فلتر كيميائي (UVB)',
        benefits: { 'حماية قوية من UVB': 0.9, 'ملمس خفيف وغير دهني': 0.95 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'فلتر شمسي قابل للذوبان في الماء، مما يعطي إحساسا خفيفا جدا للمنتج. لا يوفر حماية من UVA، يجب دمجه مع فلاتر UVA.' }],
      },
{
        id: 'paba',
        name: 'PABA (حمض بارا أمينوبنزويك)',
        scientific_name: 'Para-Aminobenzoic Acid',
        searchKeywords: ['paba', 'para-aminobenzoic acid', 'padimate-o', 'حمض بارا أمينوبنزويك', 'حمض أمينوبنزويك', 'aminobenzoic acid'],
        usageScope: ['skin'],
        functionalCategory: 'حماية من الشمس',
        chemicalType: 'فلتر كيميائي (مهجور)',
        benefits: { 'حماية جيدة من UVB': 0.8 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'risk', text: 'فلتر قديم جدا لم يعد مستخدما تقريبا بسبب ارتفاع معدلات الحساسية الجلدية وتسببه في تلطيخ الملابس باللون الأصفر.' }],
      },
{
        id: 'cinoxate',
        name: 'سينوكسات',
        scientific_name: 'Cinoxate',
        searchKeywords: ['cinoxate', 'سينوكسات'],
        usageScope: ['skin'],
        functionalCategory: 'حماية من الشمس',
        chemicalType: 'فلتر كيميائي (مهجور)',
        benefits: { 'حماية من UVB': 0.75 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'risk', text: 'فلتر UVB ضعيف وغير مستقر، لم يعد مستخدما في واقيات الشمس الحديثة.' }],
      },
{
        id: 'tinosorb-s',
        name: 'تينوسورب إس (Bemotrizinol)',
        scientific_name: 'Bis-Ethylhexyloxyphenol Methoxyphenyl Triazine',
        searchKeywords: ['tinosorb s', 'bemotrizinol', 'bis-ethylhexyloxyphenol methoxyphenyl triazine', 'تينوسورب إس'],
        usageScope: ['skin'],
        functionalCategory: 'حماية من الشمس',
        chemicalType: 'فلتر كيميائي حديث',
        benefits: { 'حماية واسعة الطيف (UVA/UVB)': 0.99, 'استقرار ضوئي فائق': 0.98, 'يعزز استقرار الفلاتر الأخرى': 0.95 },
        synergy: { 'avobenzone': 1.1 },
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'يعتبر من أفضل فلاتر الشمس في العالم لاستقراره وتغطيته الواسعة. معتمد في أوروبا وآسيا ولكن ليس بعد في الولايات المتحدة.' }],
      },
{
        id: 'tinosorb-m',
        name: 'تينوسورب إم (Bisoctrizole)',
        scientific_name: 'Methylene Bis-Benzotriazolyl Tetramethylbutylphenol',
        searchKeywords: ['tinosorb m', 'bisoctrizole', 'تينوسورب إم', 'Methylene Bis-Benzotriazolyl Tetramethylbutyl-phenol'],
        usageScope: ['skin'],
        functionalCategory: 'حماية من الشمس',
        chemicalType: 'فلتر هجين حديث',
        benefits: { 'حماية واسعة الطيف (UVA/UVB)': 0.98, 'يعمل كفلتر فيزيائي وكيميائي': 0.9 },
        synergy: {},
        negativeSynergy: {},
        warnings: [
          { level: 'info', text: 'فلتر فريد يعمل عن طريق امتصاص وعكس وتشتيت الأشعة فوق البنفسجية.' },
          { level: 'caution', text: 'قد يترك طبقة بيضاء طفيفة لأنه جسيمات دقيقة (nano).' }
        ],
      },
{
        id: 'uvinul-a-plus',
        name: 'يوفينول أ بلس',
        scientific_name: 'Diethylamino Hydroxybenzoyl Hexyl Benzoate',
        searchKeywords: ['uvinul a plus', 'diethylamino hydroxybenzoyl hexyl benzoate', 'يوفينول أ بلس'],
        usageScope: ['skin'],
        functionalCategory: 'حماية من الشمس',
        chemicalType: 'فلتر كيميائي حديث',
        benefits: { 'حماية فائقة من UVA': 0.98, 'استقرار ضوئي ممتاز': 0.95 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'يوفر حماية ممتازة من أشعة UVA ويساعد على منع تلف الكولاجين. معتمد في أوروبا وآسيا ولكن ليس بعد في الولايات المتحدة.' }],
      },
{
        id: 'uvinul-t-150',
        name: 'يوفينول تي 150',
        scientific_name: 'Ethylhexyl Triazone',
        searchKeywords: ['uvinul t 150', 'ethylhexyl triazone', 'octyl triazone', 'يوفينول تي 150'],
        usageScope: ['skin'],
        functionalCategory: 'حماية من الشمس',
        chemicalType: 'فلتر كيميائي حديث',
        benefits: { 'أعلى امتصاص لـ UVB': 0.99, 'استقرار ضوئي فائق': 0.95, 'مقاوم للماء': 0.9 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'لديه أعلى قدرة امتصاص لأشعة UVB بين جميع فلاتر الشمس المتاحة. معتمد في أوروبا وآسيا ولكن ليس بعد في الولايات المتحدة.' }],
      },
{
        id: 'mexoryl-sx',
        name: 'ميكسوريل SX',
        scientific_name: 'Terephthalylidene Dicamphor Sulfonic Acid',
        searchKeywords: ['mexoryl sx', 'ecamsule', 'ميكسوريل SX'],
        usageScope: ['skin'],
        functionalCategory: 'حماية من الشمس',
        chemicalType: 'فلتر كيميائي حديث',
        benefits: { 'حماية فائقة من UVA': 0.98, 'مستقر ضوئيا': 0.95 },
        synergy: { 'avobenzone': 1.1 },
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'فلتر UVA قوي جدا ومستقر، حاصل على براءة اختراع من L\'Oréal. قابل للذوبان في الماء.' }],
      },
{
        id: 'mexoryl-xl',
        name: 'ميكسوريل XL',
        scientific_name: 'Drometrizole Trisiloxane',
        searchKeywords: ['mexoryl xl', 'drometrizole trisiloxane', 'ميكسوريل XL'],
        usageScope: ['skin'],
        functionalCategory: 'حماية من الشمس',
        chemicalType: 'فلتر كيميائي حديث',
        benefits: { 'حماية واسعة الطيف (UVA/UVB)': 0.95, 'مستقر ضوئيا': 0.92, 'يعزز فعالية الفلاتر الأخرى': 0.9 },
        synergy: { 'mexoryl-sx': 1.1 },
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'فلتر واسع الطيف قابل للذوبان في الزيت، يعمل بتآزر مع Mexoryl SX. حاصل على براءة اختراع من L\'Oréal.' }],
      },
{
        id: 'iscotrizinol',
        name: 'إيسكوتريزينول (Uvasorb HEB)',
        scientific_name: 'Diethylhexyl Butamido Triazone',
        searchKeywords: ['iscotrizinol', 'diethylhexyl butamido triazone', 'uvasorb heb'],
        usageScope: ['skin'],
        functionalCategory: 'حماية من الشمس',
        chemicalType: 'فلتر كيميائي حديث',
        benefits: { 'حماية فائقة من UVB': 0.98, 'مستقر ضوئيا جدا': 0.95, 'مقاوم للماء بشكل ممتاز': 0.9 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'يعرف تجاريا باسم Uvasorb® HEB. أحد أكثر فلاتر UVB استقرارا وفعالية.' }],
      },
{
        id: 'tinosorb-a2b',
        name: 'تينوسورب A2B',
        scientific_name: 'Tris-Biphenyl Triazine (nano)',
        searchKeywords: ['tinosorb a2b', 'tris-biphenyl triazine', 'تينوسورب A2B'],
        usageScope: ['skin'],
        functionalCategory: 'حماية من الشمس',
        chemicalType: 'فلتر هجين حديث',
        benefits: { 'حماية فائقة من UVA2 و UVB': 0.98, 'يعزز فعالية الفلاتر الأخرى': 0.9 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'فعال بشكل خاص في سد الفجوة بين حماية UVB و UVA، مما يوفر حماية أكثر توازنا.' }],
      },
{
        id: 'polysilicone-15',
        name: 'بولي سيليكون-15 (Parsol SLX)',
        scientific_name: 'Polysilicone-15',
        searchKeywords: ['polysilicone-15', 'parsol slx', 'بولي سيليكون 15'],
        usageScope: ['skin'],
        functionalCategory: 'حماية من الشمس',
        chemicalType: 'فلتر كيميائي حديث',
        benefits: { 'حماية قوية من UVB': 0.92, 'يعطي ملمسا أنيقا': 0.95, 'مثبت ضوئي': 0.85 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'فلتر UVB قائم على السيليكون، مما يمنح المنتج ملمسا حريريا وغير دهني.' }],
      },
{
        id: 'neo-heliopan-ap',
        name: 'نيو هيليوبان AP',
        scientific_name: 'Disodium Phenyl Dibenzimidazole Tetrasulfonate',
        searchKeywords: ['neo heliopan ap', 'disodium phenyl dibenzimidazole tetrasulfonate'],
        usageScope: ['skin'],
        functionalCategory: 'حماية من الشمس',
        chemicalType: 'فلتر كيميائي حديث',
        benefits: { 'حماية قوية جدا من UVA2': 0.95, 'قابل للذوبان في الماء': 0.9, 'مستقر ضوئيا': 0.92 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'mexoryl-400',
        name: 'ميكسوريل 400',
        scientific_name: 'Methoxypropylamino Cyclohexenylidene Ethoxyethylcyanoacetate',
        searchKeywords: ['mexoryl 400', 'uv mune 400', 'ميكسوريل 400'],
        usageScope: ['skin'],
        functionalCategory: 'حماية من الشمس',
        chemicalType: 'فلتر كيميائي حديث',
        benefits: { 'حماية من أشعة UVA الطويلة جدا': 0.99, 'يمنع التصبغات العميقة': 0.95 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'أحدث فلتر حاصل على براءة اختراع من L\'Oréal، وهو الفلتر الوحيد القادر على الحماية بفعالية من نهاية طيف أشعة UVA الطويلة (380-400 نانومتر).' }],
      },
{
        id: 'ectoin',
        name: 'إكتوين',
        scientific_name: 'Ectoin',
        searchKeywords: ['ectoin', 'إكتوين'],
        usageScope: ['skin'],
        functionalCategory: 'مضاد أكسدة',
        chemicalType: 'حمض أميني',
        benefits: { 'حماية الخلايا من الإجهاد': 0.95, 'ترطيب طويل الأمد': 0.9, 'حماية من الضوء الأزرق': 0.85, 'دعم حاجز البشرة': 0.9 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'حمض أميني طبيعي يحمي الخلايا من الإجهاد البيئي مثل التلوث والأشعة فوق البنفسجية والضوء المرئي.' }],
      },
{
        id: 'scutellaria-baicalensis-root-extract',
        name: 'مستخلص جذور سكوتيلاريا بايكالينسيس',
        scientific_name: 'Scutellaria Baicalensis Root Extract',
        searchKeywords: ['scutellaria baicalensis', 'baicalin', 'chinese skullcap'],
        usageScope: ['skin'],
        functionalCategory: 'مضاد أكسدة',
        chemicalType: 'مستخلص نباتي',
        benefits: { 'مضاد أكسدة واسع الطيف': 0.9, 'حماية من أضرار الشمس': 0.88, 'حماية من الضوء الأزرق': 0.82 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'pentaerythrityl-tetra-di-t-butyl-hydroxyhydrocinnamate',
        name: 'بنتاريثريتيل تترا-دي-تي-بوتيل هيدروكسي هيدروسينامات',
        scientific_name: 'Pentaerythrityl Tetra-di-t-butyl Hydroxyhydrocinnamate',
        searchKeywords: ['pentaerythrityl tetra-di-t-butyl hydroxyhydrocinnamate', 'tinogard tt'],
        usageScope: ['skin'],
        functionalCategory: 'مضاد أكسدة',
        chemicalType: 'مثبت',
        benefits: { 'حماية المنتج من الأكسدة': 0.95, 'يحافظ على فعالية الفلاتر': 0.9 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'لا يقدم فائدة مباشرة للبشرة، بل هو مضاد أكسدة قوي يحافظ على استقرار وجودة واقي الشمس نفسه.' }],
      },
{
        id: 'thermus-thermophillus-ferment',
        name: 'خميرة ثيرموس ثيرموفيلوس',
        scientific_name: 'Thermus Thermophillus Ferment',
        searchKeywords: ['thermus thermophillus ferment', 'vento-ceane'],
        usageScope: ['skin'],
        functionalCategory: 'مضاد أكسدة',
        chemicalType: 'مكون مخمر',
        benefits: { 'حماية من الأشعة تحت الحمراء (IR)': 0.88, 'مضاد أكسدة': 0.85, 'دعم حاجز البشرة': 0.8 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'مكون يتم تنشيطه بالحرارة والأشعة فوق البنفسجية، يوفر حماية مضادة للأكسدة ضد أضرار الأشعة تحت الحمراء.' }],
      },
{
        id: 'artemia-extract',
        name: 'مستخلص الأرتيميا',
        scientific_name: 'Artemia Extract',
        searchKeywords: ['artemia extract', 'gp4g'],
        usageScope: ['skin'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'مستخلص بحري',
        benefits: { 'تنشيط طاقة الخلايا': 0.9, 'حماية من أضرار الشمس': 0.85, 'إصلاح الحمض النووي': 0.82 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'مستخلص من العوالق البحرية، يساعد على حماية خلايا الجلد من الإجهاد الناتج عن الأشعة فوق البنفسجية.' }],
      },
{
        id: 'polygonum-aviculare-extract',
        name: 'مستخلص عصا الراعي',
        scientific_name: 'Polygonum Aviculare Extract',
        searchKeywords: ['polygonum aviculare', 'knotgrass', 'elix-ir'],
        usageScope: ['skin'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'مستخلص نباتي',
        benefits: { 'حماية من الأشعة تحت الحمراء (IR)': 0.9, 'مكافحة الشيخوخة الضوئية': 0.85 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'يستهدف أضرار الأشعة تحت الحمراء لمنع فقدان مرونة الجلد.' }],
      },
{ id: 'bisabolol', name: 'بيسابولول', scientific_name: 'Bisabolol', searchKeywords: ['bisabolol'], usageScope: ['skin'], functionalCategory: 'ملطف ومرمم', chemicalType: 'مركب نشط نباتي', benefits: {'مضاد للالتهابات':0.92, 'ملطف ومرمم':0.9}, synergy: {}, negativeSynergy: {}, warnings: [{level:'info',text:'المكون النشط في البابونج، يضاف لواقيات الشمس لتهدئة البشرة.'}] },
{ id: 'hydrolyzed-wheat-protein-pg-propyl-silanetriol', name: 'بروتين القمح المهدرج PG-بروبيل سيلانتريول', scientific_name: 'Hydrolyzed Wheat Protein/PVP Crosspolymer', searchKeywords: ['cromoist'], usageScope: ['skin'], functionalCategory: 'مكون طبقة رقيقة', chemicalType: 'بروتين معدل', benefits: {'تكوين طبقة مقاومة للماء':0.88}, synergy: {}, negativeSynergy: {}, warnings: [{level:'info',text:'يشكل طبقة رقيقة على الجلد للمساعدة في مقاومة الماء.'}] },
{
        id: 'isostearyl-isostearate',
        name: 'أيزوستيرات الأيزوستيريل',
        scientific_name: 'Isostearyl Isostearate',
        searchKeywords: ['isostearyl isostearate'],
        usageScope: ['skin'],
        functionalCategory: 'مرطب / مطري',
        chemicalType: 'مطري',
        benefits: { 'مطري غني': 0.9, 'تكوين طبقة واقية': 0.85, 'يعطي لمعانا': 0.8 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'pentaerythrityl-tetraisostearate',
        name: 'بنتاريثريتيل تترا أيزوستيرات',
        scientific_name: 'Pentaerythrityl Tetraisostearate',
        searchKeywords: ['pentaerythrityl tetraisostearate'],
        usageScope: ['skin'],
        functionalCategory: 'مرطب / مطري',
        chemicalType: 'مطري',
        benefits: { 'مطري غني وفعال': 0.92, 'يعطي ملمسا فاخرا': 0.9, 'مقاوم للماء': 0.85 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'triisostearin',
        name: 'ثلاثي الأيزوستيرين',
        scientific_name: 'Triisostearin',
        searchKeywords: ['triisostearin'],
        usageScope: ['skin'],
        functionalCategory: 'مرطب / مطري',
        chemicalType: 'مطري',
        benefits: { 'مطري': 0.85, 'مذيب': 0.8 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'diisopropyl-sebacate',
        name: 'ثنائي أيزوبروبيل سيباكات',
        scientific_name: 'Diisopropyl Sebacate',
        searchKeywords: ['diisopropyl sebacate'],
        usageScope: ['skin'],
        functionalCategory: 'مرطب / مطري',
        chemicalType: 'مطري',
        benefits: { 'مطري خفيف جدا': 0.9, 'مذيب لفلاتر الشمس': 0.88, 'يقلل الملمس الدهني': 0.92 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'ممتاز في واقيات الشمس لتحسين الملمس النهائي.' }],
      },
{
        id: 'diisopropyl-adipate',
        name: 'ثنائي أيزوبروبيل أديبات',
        scientific_name: 'Diisopropyl Adipate',
        searchKeywords: ['diisopropyl adipate'],
        usageScope: ['skin'],
        functionalCategory: 'مرطب / مطري',
        chemicalType: 'مطري',
        benefits: { 'مطري خفيف': 0.88, 'مذيب': 0.9, 'تحسين انزلاق المنتج': 0.85 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'dibutyl-adipate',
        name: 'ثنائي بوتيل أديبات',
        scientific_name: 'Dibutyl Adipate',
        searchKeywords: ['dibutyl adipate'],
        usageScope: ['skin'],
        functionalCategory: 'مرطب / مطري',
        chemicalType: 'مطري',
        benefits: { 'مطري خفيف': 0.9, 'مذيب لفلاتر الشمس': 0.92, 'يعطي لمعانا': 0.8 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'coco-caprylate',
        name: 'كوكو-كابريلات',
        scientific_name: 'Coco-Caprylate',
        searchKeywords: ['coco-caprylate', 'coconut alkanes', 'كوكو كابريلات', 'coco caprylate'],
        usageScope: ['skin', 'hair'],
        functionalCategory: 'مرطب / مطري',
        chemicalType: 'مطري',
        benefits: { 'بديل طبيعي للسيليكون': 0.95, 'مطري خفيف جدا': 0.92, 'امتصاص سريع': 0.9, 'تحسين انتشار المنتج': 0.88 },
        synergy: {},
        negativeSynergy: {},
        warnings: [
          { level: 'info', text: 'مطري طبيعي وخفيف مشتق من جوز الهند. يعتبر بديلاً ممتازاً للسيليكونات المتطايرة لإعطاء ملمس ناعم وحريري دون إحساس دهني.' }
        ],
      },
{
        id: 'coco-caprylate-caprate',
        name: 'كوكو-كابريلات/كبرات',
        scientific_name: 'Coco-Caprylate/Caprate',
        searchKeywords: ['coco-caprylate/caprate', 'Coco Caprylate/Caprate', 'Coco Caprylate Caprate'],
        usageScope: ['skin'],
        functionalCategory: 'مرطب / مطري',
        chemicalType: 'مطري',
        benefits: { 'مطري خفيف مشتق من النبات': 0.92, 'بديل طبيعي للسيليكون': 0.9, 'امتصاص سريع': 0.88 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'cetyl-ricinoleate',
        name: 'ريسينولات السيتيل',
        scientific_name: 'Cetyl Ricinoleate',
        searchKeywords: ['cetyl ricinoleate'],
        usageScope: ['skin'],
        functionalCategory: 'مرطب / مطري',
        chemicalType: 'مطري',
        benefits: { 'مطري غني': 0.85, 'يعطي ملمسا كريميا': 0.8 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'cetyl-acetate',
        name: 'أسيتات السيتيل',
        scientific_name: 'Cetyl Acetate',
        searchKeywords: ['cetyl acetate'],
        usageScope: ['skin'],
        functionalCategory: 'مرطب / مطري',
        chemicalType: 'مطري',
        benefits: { 'مطري خفيف': 0.8, 'مقاوم للماء': 0.75 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'acetylated-lanolin-alcohol',
        name: 'كحول اللانولين المؤستل',
        scientific_name: 'Acetylated Lanolin Alcohol',
        searchKeywords: ['acetylated lanolin alcohol'],
        usageScope: ['skin'],
        functionalCategory: 'مرطب / مطري',
        chemicalType: 'مطري',
        benefits: { 'مطري فعال': 0.8, 'مستحلب': 0.75 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'caution', text: 'قد يكون كوميدوجينيك ومسببا للحساسية لدى الأشخاص الذين يعانون من حساسية الصوف.' }],
      },
{
        id: 'dipotassium-phosphate',
        name: 'ثنائي فوسفات البوتاسيوم',
        scientific_name: 'Dipotassium Phosphate',
        searchKeywords: ['dipotassium phosphate'],
        usageScope: ['skin'],
        functionalCategory: 'محسن قوام',
        chemicalType: 'معدل الحموضة',
        benefits: { 'تنظيم درجة الحموضة': 0.9 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'potassium-phosphate',
        name: 'فوسفات البوتاسيوم',
        scientific_name: 'Potassium Phosphate',
        searchKeywords: ['potassium phosphate'],
        usageScope: ['skin'],
        functionalCategory: 'محسن قوام',
        chemicalType: 'معدل الحموضة',
        benefits: { 'تنظيم درجة الحموضة': 0.9 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'poloxamer-407',
        name: 'بولوكسامير 407',
        scientific_name: 'Poloxamer 407',
        searchKeywords: ['poloxamer 407'],
        usageScope: ['skin'],
        functionalCategory: 'منظف',
        chemicalType: 'منظف لطيف',
        benefits: { 'منظف لطيف': 0.85, 'مستحلب': 0.8, 'مكثف حراري': 0.82 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'يستخدم في بعض غسولات الفم والمنظفات اللطيفة.' }],
      },
{
        id: 'poloxamer-184',
        name: 'بولوكسامير 184',
        scientific_name: 'Poloxamer 184',
        searchKeywords: ['poloxamer 184'],
        usageScope: ['skin'],
        functionalCategory: 'منظف',
        chemicalType: 'منظف لطيف',
        benefits: { 'منظف لطيف جدا في ماء الميسيلار': 0.95, 'يزيل المكياج بفعالية': 0.9, 'لا يسبب تهيج للعين': 0.88 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'المكون الرئيسي المسؤول عن تكوين الميسيلات في العديد من منتجات ماء الميسيلار.' }],
      },
{
        id: 'peg-6-caprylic-capric-glycerides',
        name: 'PEG-6 كابريليك/كابريك جليسريدات',
        scientific_name: 'PEG-6 Caprylic/Capric Glycerides',
        searchKeywords: ['peg-6 caprylic/capric glycerides', 'softigen 767'],
        usageScope: ['skin'],
        functionalCategory: 'منظف',
        chemicalType: 'منظف لطيف',
        benefits: { 'منظف لطيف في ماء الميسيلار': 0.92, 'مطري مذيب للدهون': 0.9, 'إعادة ترطيب البشرة': 0.85 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'مكون شائع آخر في ماء الميسيلار، يعمل كمنظف لطيف ومطري في نفس الوقت.' }],
      },
{
        id: '2-oleamido-1-3-octadecanediol',
        name: '2-أولياميدو-1،3-أوكتاديكانيديول',
        scientific_name: '2-Oleamido-1,3-Octadecanediol',
        searchKeywords: ['2-oleamido-1,3-octadecanediol', 'ceramide-like molecule'],
        usageScope: ['skin', 'hair'],
        functionalCategory: 'دعم حاجز البشرة',
        chemicalType: 'سيراميد صناعي',
        benefits: { 'تقليد السيراميد الطبيعي': 0.9, 'دعم حاجز البشرة': 0.92, 'تقوية الشعر': 0.85 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'جزيء حاصل على براءة اختراع من L\'Oréal، وهو سيراميد صناعي يقوي حاجز البشرة ويصلح الشعر.' }],
      },
{
        id: 'nylon-12',
        name: 'نايلون-12',
        scientific_name: 'Nylon-12',
        searchKeywords: ['nylon-12', 'نايلون 12'],
        usageScope: ['skin'],
        functionalCategory: 'محسن قوام',
        chemicalType: 'مسحوق',
        benefits: { 'يعطي ملمسا ناعما وحريريا': 0.9, 'يمتص الزيوت': 0.85, 'تأثير مطفي (Matte)': 0.88 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'hdi-trimethylol-hexyllactone-crosspolymer',
        name: 'HDI/تريميثيلول هيكسيلاكتون كروسبوليمر',
        scientific_name: 'HDI/Trimethylol Hexyllactone Crosspolymer',
        searchKeywords: ['hdi/trimethylol hexyllactone crosspolymer', 'soft-focus powder'],
        usageScope: ['skin'],
        functionalCategory: 'محسن قوام',
        chemicalType: 'مسحوق',
        benefits: { 'تأثير بصري لتمويه التجاعيد': 0.95, 'يعطي ملمسا ناعما جدا': 0.9, 'يمتص الزيوت': 0.8 },
        synergy: { 'silica': 1.05 },
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'مسحوق بوليمري يخلق تأثير "التركيز الناعم" (soft-focus) الذي يقلل بصريا من ظهور الخطوط الدقيقة والعيوب.' }],
      },
{
        id: 'lithium-magnesium-sodium-silicate',
        name: 'سيليكات ليثيوم مغنيسيوم صوديوم',
        scientific_name: 'Lithium Magnesium Sodium Silicate',
        searchKeywords: ['lithium magnesium sodium silicate', 'laponite'],
        usageScope: ['skin'],
        functionalCategory: 'محسن قوام',
        chemicalType: 'طين صناعي',
        benefits: { 'مكثف من الطين الاصطناعي': 0.9, 'يمنع انفصال المنتج': 0.88, 'يعطي قوام جل ناعم': 0.85 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'يعرف تجاريا باسم Laponite®.' }],
      },
{
        id: 'ammonium-acryloyldimethyltaurate-beheneth-25-methacrylate-crosspolymer',
        name: 'أمونيوم أكريلويل دايميثيل تورات/بيهينيث-25 ميثاكريليت كروسبوليمر',
        scientific_name: 'Ammonium Acryloyldimethyltaurate/Beheneth-25 Methacrylate Crosspolymer',
        searchKeywords: ['ammonium acryloyldimethyltaurate/beheneth-25 methacrylate crosspolymer', 'aristoflex hmb'],
        usageScope: ['skin'],
        functionalCategory: 'محسن قوام',
        chemicalType: 'بوليمر صناعي',
        benefits: { 'مكثف ومثبت': 0.9, 'مقاوم للكهارل': 0.88 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'steareth-100-hdi-peg-136-copolymer',
        name: 'ستيريث-100/HDI/PEG-136 كوبوليمر',
        scientific_name: 'Steareth-100/PEG-136/HDI Copolymer',
        searchKeywords: ['steareth-100/peg-136/hdi copolymer', 'acudyne'],
        usageScope: ['hair'],
        functionalCategory: 'مثبت للشعر',
        chemicalType: 'بوليمر صناعي',
        benefits: { 'تثبيت مرن للشعر': 0.9, 'مقاومة للرطوبة': 0.88 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'بوليمر يستخدم في منتجات التصفيف لتوفير ثبات قوي ومرن.' }],
      },
{ id: 'c12-13-alkyl-lactate', name: 'C12-13 ألكيل لاكتات', scientific_name: 'C12-13 Alkyl Lactate', searchKeywords: ['c12-13 alkyl lactate'], usageScope: ['skin'], functionalCategory: 'مرطب / مطري', chemicalType: 'مطري', benefits: {'مطري خفيف':0.88, 'مقشر لطيف':0.7}, synergy: {}, negativeSynergy: {}, warnings: []},
{ id: 'propylene-glycol-dicaprylate-dicaprate', name: 'بروبيلين جلايكول ثنائي الكابريلات/الكبرات', scientific_name: 'Propylene Glycol Dicaprylate/Dicaprate', searchKeywords: ['propylene glycol dicaprylate/dicaprate'], usageScope: ['skin'], functionalCategory: 'مرطب / مطري', chemicalType: 'مطري', benefits: {'مطري خفيف':0.9, 'تحسين انتشار المنتج':0.85}, synergy: {}, negativeSynergy: {}, warnings: []},
{ id: 'cetearyl-isononanoate', name: 'سيتريل أيزونونانوات', scientific_name: 'Cetearyl Isononanoate', searchKeywords: ['cetearyl isononanoate'], usageScope: ['skin'], functionalCategory: 'مرطب / مطري', chemicalType: 'مطري', benefits: {'مطري مقاوم للماء':0.88, 'ملمس ناعم':0.85}, synergy: {}, negativeSynergy: {}, warnings: []},
{ id: 'peg-30-dipolyhydroxystearate', name: 'PEG-30 ثنائي بولي هيدروكسي ستيرات', scientific_name: 'PEG-30 Dipolyhydroxystearate', searchKeywords: ['peg-30 dipolyhydroxystearate'], usageScope: ['skin'], functionalCategory: 'محسن قوام', chemicalType: 'مستحلب', benefits: {'مستحلب قوي للماء في الزيت':0.95, 'مثالي للكريمات الغنية':0.9}, synergy: {}, negativeSynergy: {}, warnings: []},
{ id: 'polyglyceryl-4-isostearate', name: 'بولي جليسريل-4 أيزوستيرات', scientific_name: 'Polyglyceryl-4 Isostearate', searchKeywords: ['polyglyceryl-4 isostearate'], usageScope: ['skin'], functionalCategory: 'محسن قوام', chemicalType: 'مستحلب', benefits: {'مستحلب قوي للماء في الزيت':0.92, 'يتحمل كمية كبيرة من الماء':0.9}, synergy: {}, negativeSynergy: {}, warnings: []},
{ id: 'cetyl-peg-ppg-10-1-dimethicone', name: 'سيتيل PEG/PPG-10/1 دايميثيكون', scientific_name: 'Cetyl PEG/PPG-10/1 Dimethicone', searchKeywords: ['cetyl peg/ppg-10/1 dimethicone'], usageScope: ['skin'], functionalCategory: 'محسن قوام', chemicalType: 'مستحلب سيليكوني', benefits: {'مستحلب قوي للماء في السيليكون':0.95, 'يعطي ملمسا ناعما':0.9}, synergy: {}, negativeSynergy: {}, warnings: [{level: 'info', text: 'شائع جدا في كريمات الأساس وواقيات الشمس طويلة الأمد.'}]},
{ id: 'hectorite', name: 'هيكتوريت', scientific_name: 'Hectorite', searchKeywords: ['hectorite', 'هيكتوريت'], usageScope: ['skin'], functionalCategory: 'محسن قوام', chemicalType: 'طين طبيعي', benefits: {'مكثف طبيعي من الطين':0.9, 'يثبت المستحلبات':0.88, 'يعطي قواما كريميا':0.85}, synergy: {}, negativeSynergy: {}, warnings: []},
{ id: 'stearalkonium-hectorite', name: 'ستيرالكونيوم هيكتوريت', scientific_name: 'Stearalkonium Hectorite', searchKeywords: ['stearalkonium hectorite'], usageScope: ['skin'], functionalCategory: 'محسن قوام',
        chemicalType: 'مثبت', benefits: {'يثبت المستحلبات':0.9, 'يعلق الأصباغ':0.88}, synergy: {}, negativeSynergy: {}, warnings: []},
{ id: 'propylene-carbonate', name: 'كربونات البروبيلين', scientific_name: 'Propylene Carbonate', searchKeywords: ['propylene carbonate'], usageScope: ['skin'], functionalCategory: 'مذيب', chemicalType: 'مذيب', benefits: {'مذيب':0.9, 'ينشط مكثفات الطين':0.85}, synergy: {'disteardimonium-hectorite': 1.05}, negativeSynergy: {}, warnings: []},
{ id: 'trimethylsiloxysilicate', name: 'تريميثيل سيلوكسي سيليكات', scientific_name: 'Trimethylsiloxysilicate', searchKeywords: ['trimethylsiloxysilicate'], usageScope: ['skin'], functionalCategory: 'محسن قوام', chemicalType: 'سيليكون', benefits: {'تكوين طبقة مقاومة للماء والنقل':0.98, 'تثبيت المكياج':0.95}, synergy: {}, negativeSynergy: {}, warnings: [{level: 'info', text: 'راتنج سيليكوني يشكل طبقة قوية ومقاومة للماء، وهو المكون الرئيسي في معظم منتجات المكياج طويلة الأمد.'}]},
{
        id: 'behenamidopropyl-dimethylamine',
        name: 'بيهيناميدوبروبيل ثنائي ميثيل أمين',
        scientific_name: 'Behenamidopropyl Dimethylamine',
        searchKeywords: ['behenamidopropyl dimethylamine'],
        usageScope: ['hair'],
        functionalCategory: 'مكيف للشعر',
        chemicalType: 'أمين دهني',
        benefits: { 'فك تشابك الشعر': 0.9, 'تكييف': 0.88, 'بديل للكواتس': 0.85, 'قابل للتحلل': 0.9 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'يعمل كمكيف فعال في البيئة الحمضية للبلسم. يعتبر بديلا صديقا للبيئة للمكيفات الرباعية التقليدية.' }],
      },
{
        id: 'cetyl-esters',
        name: 'استرات السيتيل',
        scientific_name: 'Cetyl Esters',
        searchKeywords: ['cetyl esters', 'synthetic spermaceti', 'استرات السيتيل'],
        usageScope: ['hair', 'skin'],
        functionalCategory: 'محسن قوام',
        chemicalType: 'شمع صناعي',
        benefits: { 'مكثف للقوام': 0.9, 'مطري': 0.88, 'يعطي ملمسا شمعيا فاخرا': 0.85 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'شمع صناعي يحاكي شمع العنبرية (spermaceti)، يستخدم لإعطاء قوام غني وكريمي للبلسمات والكريمات.' }],
      },
{
        id: 'lanolin-alcohol',
        name: 'كحول اللانولين',
        scientific_name: 'Lanolin Alcohol',
        searchKeywords: ['lanolin alcohol', 'كحول اللانولين'],
        usageScope: ['hair', 'skin'],
        functionalCategory: 'مرطب / مطري',
        chemicalType: 'مطري',
        benefits: { 'مطري فعال جدا': 0.9, 'مستحلب': 0.8, 'يدعم حاجز البشرة': 0.85 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'caution', text: 'فعال جدا ولكن قد يسبب حساسية لدى الأشخاص الذين يعانون من حساسية الصوف.' }],
      },
{
        id: 'ppg-3-benzyl-ether-myristate',
        name: 'PPG-3 بنزيل إيثر ميريستات',
        scientific_name: 'PPG-3 Benzyl Ether Myristate',
        searchKeywords: ['ppg-3 benzyl ether myristate', 'crodamol bmb'],
        usageScope: ['hair'],
        functionalCategory: 'مكيف للشعر',
        chemicalType: 'مطري',
        benefits: { 'إضافة لمعان فائق': 0.95, 'بديل للسيليكون': 0.9, 'تنعيم الشعر': 0.88 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'مطري مصمم خصيصا لإعطاء لمعان شديد للشعر، ويعتبر بديلا ممتازا لبعض السيليكونات.' }],
      },
{
        id: 'isoamyl-laurate',
        name: 'أيزو أميل لوريت',
        scientific_name: 'Isoamyl Laurate',
        searchKeywords: ['isoamyl laurate'],
        usageScope: ['hair', 'skin'],
        functionalCategory: 'مرطب / مطري',
        chemicalType: 'مطري',
        benefits: { 'مطري خفيف جدا': 0.9, 'بديل طبيعي للسيليكون': 0.92, 'ملمس بودري': 0.88 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'مطري طبيعي معتمد من Ecocert، يعطي ملمسا خفيفا وغير دهني مشابها للسيكلوميثيكون.' }],
      },
{
        id: 'hydrogenated-ethylhexyl-olivate',
        name: 'أوليفات الإيثيل هكسيل المهدرج',
        scientific_name: 'Hydrogenated Ethylhexyl Olivate',
        searchKeywords: ['hydrogenated ethylhexyl olivate', 'olive oil unsaponifiables'],
        usageScope: ['hair', 'skin'],
        functionalCategory: 'مرطب / مطري',
        chemicalType: 'مطري',
        benefits: { 'بديل طبيعي للسيليكون': 0.9, 'تنعيم الشعر والبشرة': 0.88, 'انزلاق ممتاز': 0.85 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'مشتق من زيت الزيتون، يوفر ملمسا أنيقا وانزلاقا مشابها للسيليكون.' }],
      },
{
        id: 'cystine-bis-pg-propyl-silanetriol',
        name: 'سيستين بيس-PG-بروبيل سيلانتريول',
        scientific_name: 'Cystine Bis-PG-Propyl Silanetriol',
        searchKeywords: ['cystine bis-pg-propyl silanetriol', 'olaplex'],
        usageScope: ['hair'],
        functionalCategory: 'تقوية الشعر',
        chemicalType: 'مركب بناء الروابط',
        benefits: { 'إصلاح روابط الشعر': 0.95, 'حماية من التلف الكيميائي': 0.92, 'تقوية الشعر التالف': 0.9 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'مركب هجين من السيليكون والحمض الأميني (السيستين) حاصل على براءة اختراع، يعمل على إعادة توصيل الروابط ثنائية الكبريتيد المكسورة داخل الشعر.' }],
      },
{
        id: 'acrylates-octylacrylamide-copolymer',
        name: 'أكريلات/أوكتيل أكريلاميد كوبوليمر',
        scientific_name: 'Acrylates/Octylacrylamide Copolymer',
        searchKeywords: ['acrylates/octylacrylamide copolymer'],
        usageScope: ['hair'],
        functionalCategory: 'مثبت للشعر',
        chemicalType: 'بوليمر صناعي',
        benefits: { 'تثبيت قوي للشعر': 0.9, 'مقاومة للرطوبة': 0.88, 'تكوين طبقة رقيقة': 0.85 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'بوليمر أساسي في مثبتات الشعر (السبراي).' }],
      },
{
        id: 'amp-acrylates-copolymer',
        name: 'AMP-أكريلات كوبوليمر',
        scientific_name: 'AMP-Acrylates Copolymer',
        searchKeywords: ['amp-acrylates copolymer'],
        usageScope: ['hair'],
        functionalCategory: 'مثبت للشعر',
        chemicalType: 'بوليمر صناعي',
        benefits: { 'تثبيت مرن للشعر': 0.9, 'سهل الإزالة بالشامبو': 0.85 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'polyurethane-14',
        name: 'بولي يوريثان-14',
        scientific_name: 'Polyurethane-14',
        searchKeywords: ['polyurethane-14', 'amp-acrylates copolymer'],
        usageScope: ['hair'],
        functionalCategory: 'مثبت للشعر',
        chemicalType: 'بوليمر صناعي',
        benefits: { 'تثبيت متعدد الوظائف': 0.88, 'ذاكرة للتصفيف': 0.9 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'بوليمر حديث يساعد الشعر على "تذكر" شكله، مما يوفر ثباتا طويل الأمد.' }],
      },
{
        id: 'octylacrylamide-acrylates-butylaminoethyl-methacrylate-copolymer',
        name: 'أوكتيل أكريلاميد/أكريلات/بيوتيل أمينو إيثيل ميثاكريليت كوبوليمر',
        scientific_name: 'Octylacrylamide/Acrylates/Butylaminoethyl Methacrylate Copolymer',
        searchKeywords: ['octylacrylamide/acrylates/butylaminoethyl methacrylate copolymer', 'amphomer'],
        usageScope: ['hair'],
        functionalCategory: 'مثبت للشعر',
        chemicalType: 'بوليمر صناعي',
        benefits: { 'تثبيت قوي جدا': 0.95, 'مقاومة عالية للرطوبة': 0.92 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'بوليمر شائع جدا في مثبتات الشعر الاحترافية لقوته ومقاومته للرطوبة.' }],
      },
{
        id: 'hydrolyzed-pea-protein',
        name: 'بروتين البازلاء المهدرج',
        scientific_name: 'Hydrolyzed Pea Protein',
        searchKeywords: ['hydrolyzed pea protein', 'pea protein', 'بروتين البازلاء'],
        usageScope: ['hair'],
        functionalCategory: 'تقوية الشعر',
        chemicalType: 'بروتين',
        benefits: { 'إضافة حجم للشعر': 0.85, 'ترطيب': 0.8 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'talc',
        name: 'التلك',
        scientific_name: 'Talc',
        searchKeywords: ['talc', 'talcum powder', 'تلك', 'بودرة التلك'],
        usageScope: ['skin'],
        functionalCategory: 'محسن قوام',
        chemicalType: 'ممتص للدهون',
        benefits: { 'امتصاص الرطوبة والزيوت': 0.9, 'تحسين ملمس المنتج': 0.92, 'مادة حشو': 0.85 },
        synergy: {},
        negativeSynergy: {},
        warnings: [
          { level: 'risk', text: 'قد يكون ملوثا بالأسبستوس، وهي مادة مسرطنة معروفة، إذا لم يتم تنقيته بشكل صحيح. يجب التأكد من أنه من مصدر موثوق وخالٍ من الأسبستوس.' },
          { level: 'risk', text: 'استنشاق مسحوق التلك خطير على الرئتين. تم ربطه بزيادة خطر الإصابة بسرطان المبيض عند استخدامه بشكل متكرر في منطقة الأعضاء التناسلية.' }
        ],
      },
{
        id: 'ultramarines-ci-77007',
        name: 'ألترامارين (CI 77007)',
        scientific_name: 'Ultramarines (CI 77007)',
        searchKeywords: ['ultramarines', 'ci 77007', 'pigment blue 29', 'ألترامارين'],
        usageScope: ['skin'],
        functionalCategory: 'محسن جمالي',
        chemicalType: 'ملون معدني',
        benefits: { 'يعطي لونا أزرق أو بنفسجي': 0.95 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'risk', text: 'آمن بشكل عام، ولكنه غير مسموح به في الولايات المتحدة للاستخدام في منتجات الشفاه.' }],
      },
{
        id: 'chromium-oxide-greens-ci-77288',
        name: 'أخضر أكسيد الكروم (CI 77288)',
        scientific_name: 'Chromium Oxide Greens (CI 77288)',
        searchKeywords: ['chromium oxide greens', 'ci 77288', 'pigment green 17'],
        usageScope: ['skin'],
        functionalCategory: 'محسن جمالي',
        chemicalType: 'ملون معدني',
        benefits: { 'يعطي لونا أخضر': 0.95 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'risk', text: 'آمن بشكل عام، ولكنه غير مسموح به في الولايات المتحدة للاستخدام في منتجات الشفاه.' }],
      },
{
        id: 'manganese-violet-ci-77742',
        name: 'بنفسجي المنغنيز (CI 77742)',
        scientific_name: 'Manganese Violet (CI 77742)',
        searchKeywords: ['manganese violet', 'ci 77742', 'pigment violet 16'],
        usageScope: ['skin'],
        functionalCategory: 'محسن جمالي',
        chemicalType: 'ملون معدني',
        benefits: { 'يعطي لونا بنفسجيا': 0.95 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'risk', text: 'آمن بشكل عام، ولكنه غير مسموح به في الولايات المتحدة للاستخدام في منتجات الشفاه.' }],
      },
{
        id: 'bismuth-oxychloride-ci-77163',
        name: 'أوكسي كلوريد البزموت (CI 77163)',
        scientific_name: 'Bismuth Oxychloride (CI 77163)',
        searchKeywords: ['bismuth oxychloride', 'ci 77163', 'synthetic pearl'],
        usageScope: ['skin'],
        functionalCategory: 'محسن جمالي',
        chemicalType: 'ملون معدني',
        benefits: { 'يعطي لمعانا لؤلؤيا': 0.92, 'تحسين الالتصاق': 0.8 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'caution', text: 'قد يسبب تهيجا أو حكة لدى بعض الأشخاص، خاصة في المكياج المعدني.' }],
      },
{
        id: 'zinc-stearate',
        name: 'ستيرات الزنك',
        scientific_name: 'Zinc Stearate',
        searchKeywords: ['zinc stearate', 'ستيرات الزنك'],
        usageScope: ['skin'],
        functionalCategory: 'محسن قوام',
        chemicalType: 'مادة رابطة',
        benefits: { 'يحسن الالتصاق': 0.9, 'عامل مضاد للتكتل': 0.88, 'يعطي ملمسا ناعما': 0.85 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'magnesium-myristate',
        name: 'ميريستات المغنيسيوم',
        scientific_name: 'Magnesium Myristate',
        searchKeywords: ['magnesium myristate', 'ميريستات المغنيسيوم'],
        usageScope: ['skin'],
        functionalCategory: 'محسن قوام',
        chemicalType: 'مادة رابطة',
        benefits: { 'يعطي ملمسا كريميا': 0.9, 'يحسن الالتصاق': 0.85 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'synthetic-fluorphlogopite',
        name: 'فلورفلوغوبيت صناعي',
        scientific_name: 'Synthetic Fluorphlogopite',
        searchKeywords: ['synthetic fluorphlogopite', 'synthetic mica', 'ميكا صناعية'],
        usageScope: ['skin'],
        functionalCategory: 'محسن جمالي',
        chemicalType: 'ملون معدني',
        benefits: { 'بديل أخلاقي للميكا': 0.95, 'يعطي لمعانا أكثر نقاء': 0.92, 'ملمس ناعم': 0.9 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'بديل للميكا الطبيعية، يتميز بنقائه ولمعانه الفائق، ويتجنب المخاوف الأخلاقية المتعلقة بتعدين الميكا.' }],
      },
{
        id: 'calcium-sodium-borosilicate',
        name: 'بوروسيليكات الكالسيوم والصوديوم',
        scientific_name: 'Calcium Sodium Borosilicate',
        searchKeywords: ['calcium sodium borosilicate', 'borosilicate glasses'],
        usageScope: ['skin'],
        functionalCategory: 'محسن جمالي',
        chemicalType: 'ملون معدني',
        benefits: { 'يعطي تأثير جليتر فائق اللمعان': 0.98, 'انعكاس عالي للضوء': 0.95 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'رقائق زجاجية صغيرة جدا مغلفة بأكاسيد المعادن، تستخدم لخلق تأثير الجليتر والبريق في ظلال العيون والهايلايتر.' }],
      },
{
        id: 'red-7-lake-ci-15850',
        name: 'Red 7 Lake (CI 15850)',
        scientific_name: 'CI 15850',
        searchKeywords: ['ci 15850', 'red 7 lake', 'd&c red no. 7'],
        usageScope: ['skin'],
        functionalCategory: 'محسن جمالي',
        chemicalType: 'ملون عضوي',
        benefits: { 'يعطي لونا أحمر زاهي': 0.95 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'red-6-lake-ci-15850',
        name: 'Red 6 Lake (CI 15850)',
        scientific_name: 'CI 15850',
        searchKeywords: ['ci 15850', 'red 6 lake', 'd&c red no. 6'],
        usageScope: ['skin'],
        functionalCategory: 'محسن جمالي',
        chemicalType: 'ملون عضوي',
        benefits: { 'يعطي لونا أحمر مائل للبرتقالي': 0.9 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'yellow-5-lake-ci-19140',
        name: 'Yellow 5 Lake (CI 19140)',
        scientific_name: 'CI 19140',
        searchKeywords: ['ci 19140', 'yellow 5 lake', 'tartrazine lake'],
        usageScope: ['skin'],
        functionalCategory: 'محسن جمالي',
        chemicalType: 'ملون عضوي',
        benefits: { 'يعطي لونا أصفر': 0.95 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'caution', text: 'قد يسبب حساسية نادرة لدى بعض الأشخاص، خاصة الذين يعانون من حساسية الأسبرين.' }],
      },
{
        id: 'blue-1-lake-ci-42090',
        name: 'Blue 1 Lake (CI 42090)',
        scientific_name: 'CI 42090',
        searchKeywords: ['ci 42090', 'blue 1 lake', 'brilliant blue fcf'],
        usageScope: ['skin'],
        functionalCategory: 'محسن جمالي',
        chemicalType: 'ملون عضوي',
        benefits: { 'يعطي لونا أزرق': 0.95 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'red-40-lake-ci-16035',
        name: 'Red 40 Lake (CI 16035)',
        scientific_name: 'CI 16035',
        searchKeywords: ['ci 16035', 'red 40 lake', 'allura red ac'],
        usageScope: ['skin'],
        functionalCategory: 'محسن جمالي',
        chemicalType: 'ملون عضوي',
        benefits: { 'يعطي لونا أحمر': 0.9 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'hydroxytyrosol',
        name: 'هيدروكسي إيروسول',
        scientific_name: 'Hydroxytyrosol',
        searchKeywords: ['hydroxytyrosol', 'olive fruit extract'],
        usageScope: ['skin'],
        functionalCategory: 'مضاد أكسدة',
        chemicalType: 'مركب نباتي',
        benefits: { 'مضاد أكسدة فائق القوة': 0.95, 'مضاد للالتهابات': 0.9, 'حماية من أضرار البيئة': 0.88 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'أحد أقوى مضادات الأكسدة الموجودة في زيت الزيتون وأوراقه.' }],
      },
{
        id: 'caesalpinia-spinosa-gum',
        name: 'صمغ تارا',
        scientific_name: 'Caesalpinia Spinosa Gum',
        searchKeywords: ['caesalpinia spinosa gum', 'tara gum', 'صمغ تارا'],
        usageScope: ['skin'],
        functionalCategory: 'محسن قوام',
        chemicalType: 'بوليمر طبيعي',
        benefits: { 'مكثف طبيعي': 0.88, 'يعطي ملمسا ناعما': 0.85 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'moringa-pterygosperma-seed-extract',
        name: 'مستخلص بذور المورينجا',
        scientific_name: 'Moringa Pterygosperma Seed Extract',
        searchKeywords: ['moringa seed extract', 'moringa pterygosperma'],
        usageScope: ['skin'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'مستخلص نباتي',
        benefits: { 'مضاد للتلوث': 0.9, 'مضاد أكسدة': 0.85, 'تنقية البشرة': 0.82 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'يساعد على منع جزيئات التلوث من الالتصاق بسطح الجلد.' }],
      },
{
        id: 'buddleja-davidii-extract',
        name: 'مستخلص بودليجا (شجيرة الفراشة)',
        scientific_name: 'Buddleja Davidii Extract',
        searchKeywords: ['buddleja davidii', 'butterfly bush extract', 'شجيرة الفراشة'],
        usageScope: ['skin'],
        functionalCategory: 'مضاد أكسدة',
        chemicalType: 'مستخلص نباتي',
        benefits: { 'حماية من الضوء الأزرق': 0.88, 'حماية من الأشعة تحت الحمراء': 0.85, 'مضاد أكسدة': 0.9 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'sodium-phytate',
        name: 'فيتات الصوديوم',
        scientific_name: 'Sodium Phytate',
        searchKeywords: ['sodium phytate', 'phytic acid sodium salt'],
        usageScope: ['skin'],
        functionalCategory: 'محسن قوام',
        chemicalType: 'عامل خالب',
        benefits: { 'عامل خالب طبيعي': 0.92, 'يحسن استقرار المنتج': 0.9, 'مضاد أكسدة': 0.8 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'بديل طبيعي ممتاز لـ EDTA، مشتق من نخالة الأرز.' }],
      },
{
        id: 'sh-polypeptide-121',
        name: 'sh-Polypeptide-121 (كولاجين بشري نباتي)',
        scientific_name: 'sh-Polypeptide-121',
        searchKeywords: ['sh-polypeptide-121', 'vegan collagen', 'human collagen fragment'],
        usageScope: ['skin'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'ببتيد (كولاجين حيوي)',
        benefits: { 'تحفيز إنتاج الكولاجين': 0.95, 'تحسين مرونة الجلد': 0.9, 'ترطيب': 0.88 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'قطعة من الكولاجين البشري من النوع 21، مُصنّعة بيولوجيا في النباتات. مصممة لتكون متطابقة بيولوجيا مع كولاجين البشرة لفعالية عالية.' }],
      },
{
        id: 'palmitoyl-tripeptide-8',
        name: 'بالميتويل ثلاثي الببتيد-8',
        scientific_name: 'Palmitoyl Tripeptide-8',
        searchKeywords: ['palmitoyl tripeptide-8', 'neuropeptide', 'neutrazen'],
        usageScope: ['skin'],
        functionalCategory: 'ملطف ومرمم',
        chemicalType: 'ببتيد عصبي',
        benefits: { 'تقليل الاحمرار والتهيج': 0.92, 'ملطف ومرمم': 0.9, 'مضاد للالتهابات العصبية': 0.95 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'يعرف تجاريا باسم Neutrazen™. ببتيد مصمم خصيصا لتهدئة ومنع الالتهابات العصبية في البشرة، مما يجعله مثاليا للبشرة الحساسة والمتهيجة.' }],
      },
{
        id: 'acetyl-octapeptide-3',
        name: 'أسيتيل أوكتاببتيد-3',
        scientific_name: 'Acetyl Octapeptide-3',
        searchKeywords: ['acetyl octapeptide-3', 'snap-8', 'أسيتيل أوكتاببتيد-3'],
        usageScope: ['skin'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'ببتيد',
        benefits: { 'تقليل التجاعيد التعبيرية': 0.9, 'تأثير شبيه بالبوتوكس': 0.88 },
        synergy: { 'argireline': 1.05 },
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'يعرف تجاريا باسم SNAP-8™. نسخة مطولة وأكثر قوة من الأرجيرلاين (Acetyl Hexapeptide-8)، يعمل على إرخاء العضلات السطحية للوجه.' }],
      },
{
        id: 'pentapeptide-18',
        name: 'خماسي الببتيد-18',
        scientific_name: 'Pentapeptide-18',
        searchKeywords: ['pentapeptide-18', 'leuphasyl'],
        usageScope: ['skin'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'ببتيد',
        benefits: { 'تقليل التجاعيد التعبيرية': 0.88, 'يعمل بتآزر مع الأرجيرلاين': 0.9 },
        synergy: { 'argireline': 1.1 },
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'يعرف تجاريا باسم Leuphasyl®. يعمل بطريقة مختلفة عن الأرجيرلاين لتقليل تقلصات العضلات، وعندما يتم دمجهما، يكون لهما تأثير تآزري قوي.' }],
      },
{
        id: 'diaminopropionoyl-tripeptide-33',
        name: 'ديامينوبروبيونيل ثلاثي الببتيد-33',
        scientific_name: 'Diaminopropionoyl Tripeptide-33',
        searchKeywords: ['diaminopropionoyl tripeptide-33', 'preventhelia'],
        usageScope: ['skin'],
        functionalCategory: 'مضاد أكسدة',
        chemicalType: 'ببتيد',
        benefits: { 'حماية من أضرار الشمس': 0.9, 'حماية الحمض النووي للخلايا': 0.92, 'مضاد للتلوث': 0.85 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'يعرف تجاريا باسم Preventhelia®. ببتيد يحمي خلايا الجلد من التلف الناتج عن الأشعة فوق البنفسجية والتلوث.' }],
      },
{
        id: 'acetyl-hexapeptide-1',
        name: 'أسيتيل سداسي الببتيد-1',
        scientific_name: 'Acetyl Hexapeptide-1',
        searchKeywords: ['acetyl hexapeptide-1', 'melitane'],
        usageScope: ['skin'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'ببتيد',
        benefits: { 'تحفيز تسمير البشرة': 0.88, 'حماية من أضرار الشمس': 0.85, 'مضاد للالتهابات': 0.8 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'يعرف تجاريا باسم Melitane™. يحاكي هرمون تحفيز الخلايا الصباغية لتعزيز إنتاج الميلانين، مما يوفر حماية طبيعية للبشرة ويعزز السمرة.' }],
      },
{
        id: 'tetrapeptide-21',
        name: 'رباعي الببتيد-21',
        scientific_name: 'Tetrapeptide-21',
        searchKeywords: ['tetrapeptide-21'],
        usageScope: ['skin'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'ببتيد',
        benefits: { 'تحفيز الكولاجين': 0.88, 'تحسين ملمس البشرة': 0.85, 'تقليل التجاعيد': 0.8 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'nicotiana-benthamiana-hexapeptide-40-sh-polypeptide-76',
        name: 'ببتيد عامل نمو البشرة النباتي',
        scientific_name: 'Nicotiana Benthamiana Hexapeptide-40 sh-Polypeptide-76',
        searchKeywords: ['barley egf', 'bioeffect', 'nicotiana benthamiana'],
        usageScope: ['skin'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'عامل نمو (ببتيد)',
        benefits: { 'عامل نمو البشرة (EGF)': 0.95, 'مكافحة الشيخوخة': 0.92, 'زيادة كثافة الجلد': 0.9 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'شكل من أشكال عامل نمو البشرة (EGF) يتم إنتاجه في نبات الشعير، وهو المكون النجمي في منتجات Bioeffect.' }],
      },
{
        id: 'idebenone',
        name: 'أيديبينون',
        scientific_name: 'Hydroxydecyl Ubiquinone',
        searchKeywords: ['idebenone', 'hydroxydecyl ubiquinone', 'أيديبينون'],
        usageScope: ['skin'],
        functionalCategory: 'مضاد أكسدة',
        chemicalType: 'مركب صناعي',
        benefits: { 'مضاد أكسدة فائق القوة': 0.98, 'حماية من أضرار البيئة': 0.9, 'مكافحة الشيخوخة': 0.88 },
        synergy: {},
        negativeSynergy: {},
        warnings: [
          { level: 'info', text: 'نظير صناعي لإنزيم Q10، ولكنه أصغر حجما وأكثر قوة في الحماية من الإجهاد التأكسدي.' },
          { level: 'caution', text: 'قد يسبب حساسية تلامسية لدى بعض الأفراد. قد يصبغ المنتجات باللون الأصفر أو البرتقالي.' }
        ],
      },
{
        id: 'fullerene',
        name: 'الفوليرين',
        scientific_name: 'Fullerenes',
        searchKeywords: ['fullerenes', 'fullerene', 'فوليرين'],
        usageScope: ['skin'],
        functionalCategory: 'مضاد أكسدة',
        chemicalType: 'جزيء كربون',
        benefits: { 'مضاد أكسدة طويل المفعول': 0.95, 'تفتيح وتوحيد اللون': 0.8, 'مكافحة حب الشباب': 0.82 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'جزيء كربون كروي يعمل كـ "إسفنجة للجذور الحرة"، وهو مضاد أكسدة قوي جدا وطويل الأمد. شائع في مستحضرات التجميل اليابانية.' }],
      },
{
        id: 'spin-trap',
        name: 'مصيدة الدوران (Phenyl t-Butylnitrone)',
        scientific_name: 'Phenyl t-Butylnitrone',
        searchKeywords: ['phenyl t-butylnitrone', 'pbn', 'spin trap', 'مصيدة الدوران'],
        usageScope: ['skin'],
        functionalCategory: 'مضاد أكسدة',
        chemicalType: 'مركب صناعي ذكي',
        benefits: { 'مضاد أكسدة ذكي': 0.92, 'يحول الجذور الحرة إلى مواد مفيدة': 0.9, 'حماية الخلايا': 0.88 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'مضاد أكسدة "ذكي" لا يدمر الجذور الحرة فحسب، بل يحبسها ويحولها إلى مواد مفيدة مثل الأكسجين.' }],
      },
{
        id: 'pyruvic-acid',
        name: 'حمض البيروفيك',
        scientific_name: 'Pyruvic Acid',
        searchKeywords: ['pyruvic acid', 'pyruvic', 'حمض البيروفيك'],
        usageScope: ['skin'],
        functionalCategory: 'مقشر',
        chemicalType: 'حمض ألفا هيدروكسي (AHA)',
        benefits: { 'مكافحة حب الشباب': 0.88, 'مكافحة الشيخوخة': 0.85, 'تقشير متوسط العمق': 0.82, 'تفتيح وتوحيد اللون': 0.8 },
        synergy: {},
        negativeSynergy: { 'retinoid-serum': { reason: 'خطر تهيج شديد وحروق كيميائية.' } },
        warnings: [{ level: 'risk', text: 'حمض قوي جدا، يتحول جزئيا إلى حمض اللاكتيك في الجلد. يجب استخدامه فقط تحت إشراف متخصص في عيادات التجميل.' }],
      },
{
        id: 'diglucosyl-gallic-acid',
        name: 'حمض الغاليك ثنائي الجلوكوزيل',
        scientific_name: 'Diglucosyl Gallic Acid',
        searchKeywords: ['diglucosyl gallic acid', 'tri-bright', 'brightenyll'],
        usageScope: ['skin'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'مركب نشط حيويا',
        benefits: { 'تفتيح وتوحيد اللون': 0.95, 'تقليل الاحمرار': 0.88, 'مضاد أكسدة': 0.9 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'يعرف تجاريا باسم Brightenyl®. جيل جديد من مكونات التفتيح يتم تنشيطه بواسطة ميكروبيوم الجلد ليعمل على 7 مستويات مختلفة من عملية التصبغ.' }],
      },
{
        id: 'dimethyl-isosorbide',
        name: 'ثنائي ميثيل إيزوسوربيد (DMI)',
        scientific_name: 'Dimethyl Isosorbide',
        searchKeywords: ['dimethyl isosorbide', 'dmi', 'ثنائي ميثيل إيزوسوربيد'],
        usageScope: ['skin'],
        functionalCategory: 'مذيب',
        chemicalType: 'معزز امتصاص',
        benefits: { 'تعزيز امتصاص المكونات النشطة': 0.98, 'مذيب فعال': 0.9, 'يقلل من تهيج بعض المكونات': 0.85 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'مذيب ممتاز يساعد على توصيل المكونات النشطة (مثل الريتينويدات وفيتامين C) إلى طبقات أعمق من الجلد، مما يزيد من فعاليتها ويقلل من تهيجها السطحي.' }],
      },
{
        id: 'pseudoalteromonas-ferment-extract',
        name: 'مستخلص تخمير سودوألتروموناس',
        scientific_name: 'Pseudoalteromonas Ferment Extract',
        searchKeywords: ['pseudoalteromonas ferment extract', 'antarcticine', 'أنتاركتيسين'],
        usageScope: ['skin'],
        functionalCategory: 'مرطب / مطري',
        chemicalType: 'مكون مخمر',
        benefits: { 'حماية من الجفاف الشديد': 0.9, 'تحفيز الكولاجين': 0.85, 'ترطيب': 0.88 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'يعرف تجاريا باسم Antarcticine®. بروتين سكري من بكتيريا تم اكتشافها في الأنهار الجليدية في القارة القطبية الجنوبية، يحمي الجلد من الجفاف والبرد الشديد.' }],
      },
{
        id: 'betula-alba-juice',
        name: 'عصارة شجرة البتولا',
        scientific_name: 'Betula Alba (Birch) Juice',
        searchKeywords: ['betula alba juice', 'birch sap', 'عصارة البتولا'],
        usageScope: ['skin'],
        functionalCategory: 'مرطب / مطري',
        chemicalType: 'مرطب طبيعي',
        benefits: { 'ترطيب غني بالمعادن': 0.9, 'مضاد للالتهابات': 0.8, 'مضاد أكسدة': 0.82 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'غنية بالمعادن والأحماض الأمينية والسكريات، وهي مرطب طبيعي ممتاز.' }],
      },
{
        id: 'sugarcane-derived-squalane',
        name: 'سكوالان مشتق من قصب السكر',
        scientific_name: 'Squalane',
        searchKeywords: ['sugarcane squalane', 'neosannce squalane'],
        usageScope: ['skin', 'hair'],
        functionalCategory: 'مرطب / مطري',
        chemicalType: 'مرطب',
        benefits: { 'ترطيب خفيف وغير دهني': 0.95, 'مصدر مستدام': 0.9 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'شكل من أشكال السكوالان يتم إنتاجه بشكل مستدام من تخمير قصب السكر.' }],
      },
{
        id: 'hydrolyzed-algin',
        name: 'ألجين مهدرج',
        scientific_name: 'Hydrolyzed Algin',
        searchKeywords: ['hydrolyzed algin', 'algin'],
        usageScope: ['skin'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'مستخلص طحالب',
        benefits: { 'حماية من التلوث': 0.88, 'ترطيب': 0.8 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'مشتق من الطحالب البحرية، يشكل طبقة واقية على الجلد لمنع التصاق جزيئات التلوث.' }],
      },
{
        id: 'saccharide-hydrolysate',
        name: 'سكاريد مهدرج',
        scientific_name: 'Saccharide Hydrolysate',
        searchKeywords: ['saccharide hydrolysate'],
        usageScope: ['skin'],
        functionalCategory: 'مرطب / مطري',
        chemicalType: 'مرطب',
        benefits: { 'ترطيب': 0.85, 'ملطف ومرمم': 0.8 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{ id: 'erythrulose', name: 'إريثرولوز', scientific_name: 'Erythrulose', searchKeywords: ['erythrulose'], usageScope: ['skin'], functionalCategory: 'مكون فعال', chemicalType: 'عامل تسمير ذاتي', benefits: {'تسمير ذاتي طبيعي المظهر':0.9, 'يدوم أطول من DHA':0.85}, synergy: {'dihydroxyacetone': 1.1}, negativeSynergy: {}, warnings: [{level: 'info', text: 'سكر كيتوني يتفاعل مع الأحماض الأمينية في الجلد لإنتاج سمرة. يعطي لونا أبطأ وأكثر طبيعية من DHA.'}] },
{ id: 'dihydroxyacetone', name: 'ثنائي هيدروكسي أسيتون (DHA)', scientific_name: 'Dihydroxyacetone', searchKeywords: ['dihydroxyacetone', 'dha', 'self-tanner'], usageScope: ['skin'], functionalCategory: 'مكون فعال', chemicalType: 'عامل تسمير ذاتي', benefits: {'تسمير ذاتي سريع':0.92}, synergy: {'erythrulose': 1.1}, negativeSynergy: {}, warnings: [{level: 'caution', text: 'المكون الأكثر شيوعا في منتجات التسمير الذاتي. قد يعطي لونا برتقاليا ورائحة مميزة.'}] },
{ id: 'hydrolyzed-candida-saitoana-extract', name: 'مستخلص كانديدا سايتوانا المهدرج', scientific_name: 'Hydrolyzed Candida Saitoana Extract', searchKeywords: ['candida saitoana', 'cell-detox'], usageScope: ['skin'], functionalCategory: 'مكون فعال', chemicalType: 'مكون مخمر', benefits: {'تعزيز الالتهام الذاتي للخلايا':0.9, 'إزالة السموم':0.88}, synergy: {}, negativeSynergy: {}, warnings: [{level: 'info', text: 'يحفز عملية "إعادة التدوير" الطبيعية للخلايا (الالتهام الذاتي) للتخلص من الفضلات الخلوية وتحسين إشراقة البشرة.'}] },
{ id: 'acetyl-glucosamine-6-phosphate-disodium-salt', name: 'ملح فوسفات أسيتيل الجلوكوزامين', scientific_name: 'Disodium Acetyl Glucosamine Phosphate', searchKeywords: ['disodium acetyl glucosamine phosphate', 'novhyal'], usageScope: ['skin'], functionalCategory: 'مكون فعال', chemicalType: 'مركب سكري', benefits: {'تحفيز إنتاج حمض الهيالورونيك':0.92}, synergy: {}, negativeSynergy: {}, warnings: [] },
{ id: 'crocus-chrysanthus-bulb-extract', name: 'مستخلص بصلة الزعفران', scientific_name: 'Crocus Chrysanthus Bulb Extract', searchKeywords: ['crocus chrysanthus', 'dermcom'], usageScope: ['skin'], functionalCategory: 'مكون فعال', chemicalType: 'مستخلص نباتي', benefits: {'تحسين تواصل الخلايا':0.9, 'تحفيز الكولاجين والإيلاستين':0.88}, synergy: {}, negativeSynergy: {}, warnings: [] },
{
        id: 'maris-aqua',
        name: 'ماء البحر',
        scientific_name: 'Maris Aqua / Sea Water',
        searchKeywords: ['maris aqua', 'sea water', 'deep sea water', 'ماء البحر'],
        usageScope: ['skin'],
        functionalCategory: 'ملطف ومرمم',
        chemicalType: 'ماء غني بالمعادن',
        benefits: { 'غني بالمعادن النادرة': 0.9, 'إعادة التوازن للبشرة': 0.88, 'مضاد للالتهابات': 0.8 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'يحتوي على تركيز عالٍ من المعادن مثل المغنيسيوم والبوتاسيوم والكالسيوم، مما يساعد على تهدئة البشرة وتقوية حاجزها.' }],
      },
{
        id: 'methylsilanol-mannuronate',
        name: 'ميثيل سيلانول مانورونات',
        scientific_name: 'Methylsilanol Mannuronate',
        searchKeywords: ['methylsilanol mannuronate', 'silicium derivative'],
        usageScope: ['skin'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'سيليكون عضوي',
        benefits: { 'مكافحة الشيخوخة': 0.88, 'تحسين صلابة البشرة': 0.85, 'مضاد أكسدة': 0.8 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'مشتق من السيليكون العضوي والطحالب البحرية، يساعد على إعادة هيكلة الجلد وحمايته من الجذور الحرة.' }],
      },
{
        id: 'hydroxypropyl-cyclodextrin',
        name: 'هيدروكسي بروبيل سيكلودكسترين',
        scientific_name: 'Hydroxypropyl Cyclodextrin',
        searchKeywords: ['hydroxypropyl cyclodextrin'],
        usageScope: ['skin'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'مركب توصيل',
        benefits: { 'توصيل المكونات النشطة': 0.9, 'تحسين استقرار التركيبة': 0.85, 'شد البشرة': 0.8 },
        synergy: { 'retinol': 1.1 },
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'جزيء سكري يشكل بنية قفصية لتغليف المكونات النشطة (مثل الريتينول)، مما يحسن من استقرارها وتوصيلها إلى الجلد.' }],
      },
{
        id: 'swertia-chirata-extract',
        name: 'مستخلص عشبة السويرتيا',
        scientific_name: 'Swertia Chirata Extract',
        searchKeywords: ['swertia chirata', 'gentian', 'swertiamarin'],
        usageScope: ['skin'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'مستخلص نباتي',
        benefits: { 'تجديد البشرة': 0.9, 'تقليل التجاعيد العمودية': 0.88, 'تحفيز الخلايا الجذعية': 0.85 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'يستخدم في السيرومات المتقدمة لمكافحة الشيخوخة، حيث يحفز تجدد طبقة البشرة ويقلل من ظهور التجاعيد العميقة.' }],
      },
{
        id: 'tephrosia-purpurea-seed-extract',
        name: 'مستخلص بذور تفروسيا',
        scientific_name: 'Tephrosia Purpurea Seed Extract',
        searchKeywords: ['tephrosia purpurea', 'neurophroline'],
        usageScope: ['skin'],
        functionalCategory: 'ملطف ومرمم',
        chemicalType: 'مستخلص نباتي',
        benefits: { 'مقاومة الإجهاد': 0.92, 'تقليل الاحمرار': 0.9, 'تحسين إشراقة البشرة': 0.85 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'يعرف تجاريا باسم Neurophroline™. مستخلص نباتي يساعد على كسر حلقة الإجهاد في الجلد عن طريق تقليل إنتاج الكورتيزول وتعزيز إطلاق الإندورفين المهدئ.' }],
      },
{
        id: 'palmitoyl-tripeptide-1',
        name: 'بالميتويل ثلاثي الببتيد-1',
        scientific_name: 'Palmitoyl Tripeptide-1',
        searchKeywords: ['palmitoyl tripeptide-1', 'pal-ghk', 'matrixyl 3000 component'],
        usageScope: ['skin'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'ببتيد',
        benefits: { 'تحفيز إنتاج الكولاجين': 0.9, 'تحسين ملمس البشرة': 0.85, 'تقليل التجاعيد': 0.82 },
        synergy: { 'palmitoyl-tetrapeptide-7': 1.1 },
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'أحد مكوني مركب "Matrixyl 3000" الشهير.' }],
      },
{
        id: 'palmitoyl-tetrapeptide-7',
        name: 'بالميتويل رباعي الببتيد-7',
        scientific_name: 'Palmitoyl Tetrapeptide-7',
        searchKeywords: ['palmitoyl tetrapeptide-7', 'pal-gqpr', 'matrixyl 3000 component'],
        usageScope: ['skin'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'ببتيد',
        benefits: { 'مضاد للالتهابات': 0.9, 'تقليل تلف الكولاجين': 0.88, 'مكافحة الشيخوخة': 0.85 },
        synergy: { 'palmitoyl-tripeptide-1': 1.1 },
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'المكون الثاني في "Matrixyl 3000"، يعمل على تقليل الالتهابات التي تساهم في شيخوخة الجلد.' }],
      },
{
        id: 'acetyl-tetrapeptide-9',
        name: 'أسيتيل رباعي الببتيد-9',
        scientific_name: 'Acetyl Tetrapeptide-9',
        searchKeywords: ['acetyl tetrapeptide-9', 'dermican'],
        usageScope: ['skin'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'ببتيد',
        benefits: { 'تحسين كثافة الجلد': 0.9, 'تنظيم ألياف الكولاجين': 0.88, 'زيادة صلابة البشرة': 0.85 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'يعرف تجاريا باسم Dermican™. يحفز إنتاج بروتين "لوميكان" الذي يساعد على تنظيم ألياف الكولاجين بشكل صحيح.' }],
      },
{
        id: 'acetyl-tetrapeptide-11',
        name: 'أسيتيل رباعي الببتيد-11',
        scientific_name: 'Acetyl Tetrapeptide-11',
        searchKeywords: ['acetyl tetrapeptide-11', 'syniorage'],
        usageScope: ['skin'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'ببتيد',
        benefits: { 'مكافحة الشيخوخة للبشرة الناضجة': 0.9, 'تحسين تماسك البشرة': 0.88 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'يعرف تجاريا باسم Syniorage™. يستهدف بروتينات محددة لتحسين تماسك وتواصل خلايا البشرة المتقدمة في السن.' }],
      },
{
        id: 'bacopa-monnieri-extract',
        name: 'مستخلص الباكوبا',
        scientific_name: 'Bacopa Monnieri Extract',
        searchKeywords: ['bacopa monnieri', 'brahmi', 'براهمي'],
        usageScope: ['skin'],
        functionalCategory: 'مضاد أكسدة',
        chemicalType: 'مستخلص نباتي (أيورفيدا)',
        benefits: { 'مضاد أكسدة': 0.88, 'حماية الخلايا العصبية': 0.85, 'مضاد للالتهابات': 0.8 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'عشب أيورفيدي يستخدم لخصائصه الوقائية العصبية ومضادة للأكسدة.' }],
      },
{
        id: 'terminalia-chebula-fruit-extract',
        name: 'مستخلص فاكهة الهليلج',
        scientific_name: 'Terminalia Chebula Fruit Extract',
        searchKeywords: ['terminalia chebula', 'haritaki', 'هليلج'],
        usageScope: ['skin'],
        functionalCategory: 'مضاد أكسدة',
        chemicalType: 'مستخلص نباتي (أيورفيدا)',
        benefits: { 'مضاد أكسدة واسع الطيف': 0.92, 'مضاد للغلايكيشن': 0.9, 'تفتيح وتوحيد اللون': 0.85 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'يعتبر من أقوى مضادات الأكسدة في طب الأيورفيدا.' }],
      },
{
        id: 'phyllanthus-emblica-fruit-extract',
        name: 'مستخلص فاكهة الأملا',
        scientific_name: 'Phyllanthus Emblica (Amla) Fruit Extract',
        searchKeywords: ['phyllanthus emblica', 'amla', 'indian gooseberry', 'أملا', 'أملج'],
        usageScope: ['skin', 'hair'],
        functionalCategory: 'مضاد أكسدة',
        chemicalType: 'مستخلص نباتي (أيورفيدا)',
        benefits: { 'مصدر غني جدا بفيتامين C': 0.95, 'مضاد أكسدة': 0.92, 'تفتيح وتوحيد اللون': 0.88, 'تقوية الشعر': 0.9 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'sophora-flavescens-root-extract',
        name: 'مستخلص جذور الصفيراء',
        scientific_name: 'Sophora Flavescens Root Extract',
        searchKeywords: ['sophora flavescens', 'ku shen'],
        usageScope: ['skin'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'مستخلص نباتي (TCM)',
        benefits: { 'تفتيح وتوحيد اللون': 0.9, 'مضاد للميكروبات': 0.85, 'تنظيم الإفرازات الدهنية': 0.8 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'يستخدم في الطب الصيني التقليدي (TCM) لعلاج مشاكل الجلد الالتهابية وتثبيط إنتاج الميلانين.' }],
      },
{
        id: 'angelica-gigas-root-extract',
        name: 'مستخلص جذور حشيشة الملاك الكورية',
        scientific_name: 'Angelica Gigas Root Extract',
        searchKeywords: ['angelica gigas', 'korean angelica'],
        usageScope: ['skin'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'مستخلص نباتي (TCM)',
        benefits: { 'تفتيح وتوحيد اللون': 0.85, 'مضاد للالتهابات': 0.8 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'cnidium-officinale-root-extract',
        name: 'مستخلص جذور السنيديوم',
        scientific_name: 'Cnidium Officinale Root Extract',
        searchKeywords: ['cnidium officinale'],
        usageScope: ['skin'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'مستخلص نباتي (TCM)',
        benefits: { 'تحسين الدورة الدموية': 0.85, 'مضاد للالتهابات': 0.8 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'bixa-orellana-seed-extract',
        name: 'مستخلص بذور الأناتو',
        scientific_name: 'Bixa Orellana Seed Extract',
        searchKeywords: ['bixa orellana', 'annatto', 'roucou', 'أناتو'],
        usageScope: ['skin'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'مستخلص نباتي (أمازوني)',
        benefits: { 'تنظيم الإفرازات الدهنية': 0.88, 'تقليل حجم المسام': 0.85, 'مضاد أكسدة': 0.8 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'يساعد على التحكم في لمعان البشرة وتقليل العيوب الناتجة عن زيادة إفراز الدهون.' }],
      },
{
        id: 'anogeissus-leiocarpa-bark-extract',
        name: 'مستخلص لحاء أنوجيسوس',
        scientific_name: 'Anogeissus Leiocarpa Bark Extract',
        searchKeywords: ['anogeissus leiocarpa', 'african birch'],
        usageScope: ['skin'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'مستخلص نباتي',
        benefits: { 'تحفيز إنتاج الفيبريلين': 0.9, 'مكافحة الشيخوخة': 0.88, 'مضاد أكسدة': 0.85 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'يساعد على حماية وتحفيز بروتين الفيبريلين الضروري لمرونة الجلد.' }],
      },
{
        id: 'xylitylglucoside',
        name: 'إكسيليتيل غلوكوزيد',
        scientific_name: 'Xylitylglucoside',
        searchKeywords: ['xylitylglucoside', 'aquaxyl component'],
        usageScope: ['skin'],
        functionalCategory: 'مرطب / مطري',
        chemicalType: 'مرطب',
        benefits: { 'يعزز ترطيب البشرة': 0.9, 'يقوي حاجز البشرة': 0.88 },
        synergy: { 'anhydroxylitol': 1.1, 'xylitol': 1.1 },
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'أحد مكونات مركب الترطيب "Aquaxyl".' }],
      },
{
        id: 'anhydroxylitol',
        name: 'أنهيدروكسيليتول',
        scientific_name: 'Anhydroxylitol',
        searchKeywords: ['anhydroxylitol', 'aquaxyl component'],
        usageScope: ['skin'],
        functionalCategory: 'مرطب / مطري',
        chemicalType: 'مرطب',
        benefits: { 'إعادة هيكلة البشرة للترطيب': 0.9, 'يقوي حاجز البشرة': 0.88 },
        synergy: { 'xylitylglucoside': 1.1, 'xylitol': 1.1 },
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'أحد مكونات مركب الترطيب "Aquaxyl".' }],
      },
{
        id: 'xylitol',
        name: 'إكسيليتول',
        scientific_name: 'Xylitol',
        searchKeywords: ['xylitol', 'aquaxyl component'],
        usageScope: ['skin'],
        functionalCategory: 'مرطب / مطري',
        chemicalType: 'مرطب',
        benefits: { 'مرطب': 0.88, 'يدعم ميكروبيوم البشرة': 0.8 },
        synergy: { 'xylitylglucoside': 1.1, 'anhydroxylitol': 1.1 },
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'أحد مكونات مركب الترطيب "Aquaxyl".' }],
      },
{
        id: 'lactococcus-ferment-lysate',
        name: 'خميرة اللاكتوكوكس',
        scientific_name: 'Lactococcus Ferment Lysate',
        searchKeywords: ['lactococcus ferment lysate', 'probiotic'],
        usageScope: ['skin'],
        functionalCategory: 'دعم حاجز البشرة',
        chemicalType: 'مكون مخمر (بروبيوتيك)',
        benefits: { 'تسريع تجدد البشرة': 0.9, 'دعم حاجز البشرة': 0.92, 'تحسين جودة البشرة': 0.88 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'مكون بروبيوتيك يعزز عملية التقشير الطبيعية للبشرة ويقوي حاجزها.' }],
      },
{
        id: 'saccharomyces-lysate-extract',
        name: 'مستخلص خميرة السكاروميسيس',
        scientific_name: 'Saccharomyces Lysate Extract',
        searchKeywords: ['saccharomyces lysate extract', 'yeast lysate'],
        usageScope: ['skin'],
        functionalCategory: 'ملطف ومرمم',
        chemicalType: 'مكون مخمر',
        benefits: { 'ملطف ومرمم': 0.85, 'تقليل التهيج': 0.8, 'ترطيب': 0.82 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'bacillus-ferment',
        name: 'خميرة الباسيلس',
        scientific_name: 'Bacillus Ferment',
        searchKeywords: ['bacillus ferment'],
        usageScope: ['skin'],
        functionalCategory: 'مقشر',
        chemicalType: 'مكون مخمر (إنزيمي)',
        benefits: { 'تقشير إنزيمي لطيف': 0.88, 'تحسين إشراقة البشرة': 0.85 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'يوفر تقشيرا لطيفا عن طريق الإنزيمات بدلا من الأحماض.' }],
      },
{
        id: 'imperata-cylindrica-root-extract',
        name: 'مستخلص جذور الإمبراتا سيلندريكا',
        scientific_name: 'Imperata Cylindrica Root Extract',
        searchKeywords: ['imperata cylindrica', 'cogon grass', 'moist 24'],
        usageScope: ['skin'],
        functionalCategory: 'مرطب / مطري',
        chemicalType: 'مستخلص نباتي',
        benefits: { 'ترطيب طويل الأمد (24 ساعة)': 0.9, 'غني بالبوتاسيوم': 0.85 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'نبات صحراوي يحتفظ بالماء بكفاءة عالية، يوفر ترطيبا مكثفا وطويل الأمد للبشرة.' }],
      },
{
        id: 'acetyl-tetrapeptide-15',
        name: 'أسيتيل رباعي الببتيد-15',
        scientific_name: 'Acetyl Tetrapeptide-15',
        searchKeywords: ['acetyl tetrapeptide-15', 'skin-asensyl'],
        usageScope: ['skin'],
        functionalCategory: 'ملطف ومرمم',
        chemicalType: 'ببتيد',
        benefits: { 'زيادة عتبة تحمل البشرة': 0.9, 'تقليل حساسية البشرة': 0.92, 'مضاد للتهيج': 0.88 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'ببتيد مصمم لزيادة عتبة تحمل البشرة الحساسة، مما يجعلها أقل تفاعلا مع المهيجات.' }],
      },
{
        id: 'acetyl-tetrapeptide-17-colloidal-platinum',
        name: 'أسيتيل رباعي الببتيد-17 (مع البلاتين)',
        scientific_name: 'Acetyl Tetrapeptide-17, Colloidal Platinum',
        searchKeywords: ['acetyl tetrapeptide-17', 'platinum peptide'],
        usageScope: ['skin'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'ببتيد',
        benefits: { 'تحسين مرونة الجلد': 0.88, 'تعزيز توصيل الببتيد': 0.9 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'ببتيد مرتبط بجزيئات البلاتين الغروية، والتي تعمل كنظام توصيل لتحسين استقرار وفعالية الببتيد.' }],
      },
{
        id: 'copper-lysinate-prolinate',
        name: 'ليسينات/برولينات النحاس',
        scientific_name: 'Methylglucoside Phosphate, Copper Lysinate/Prolinate',
        searchKeywords: ['copper lysinate/prolinate', 'neodermyl'],
        usageScope: ['skin'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'حمض أميني معدني',
        benefits: { 'تحفيز الكولاجين والإيلاستين': 0.9, '"إبرة كولاجين" بدون إبر': 0.92, 'تقليل التجاعيد': 0.88 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'يعرف تجاريا باسم Neodermyl®. مركب يوفر طاقة للخلايا الليفية لإنتاج الكولاجين والإيلاستين بكفاءة.' }],
      },
{
        id: 'hydrolyzed-yeast-protein',
        name: 'بروتين الخميرة المهدرج',
        scientific_name: 'Hydrolyzed Yeast Protein',
        searchKeywords: ['hydrolyzed yeast protein', 'yeast protein'],
        usageScope: ['skin'],
        functionalCategory: 'مرطب / مطري',
        chemicalType: 'بروتين',
        benefits: { 'ترطيب': 0.85, 'تحسين ملمس البشرة': 0.8 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'hydrolyzed-sesame-protein-pg-propyl-methylsilanediol',
        name: 'بروتين السمسم المهدرج',
        scientific_name: 'Hydrolyzed Sesame Protein PG-Propyl Methylsilanediol',
        searchKeywords: ['hydrolyzed sesame protein'],
        usageScope: ['skin'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'بروتين معدل',
        benefits: { 'شد فوري للبشرة': 0.9, 'تقليل التجاعيد (مؤقت)': 0.85 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'مركب يشكل طبقة رقيقة على الجلد، مما يعطي تأثير شد فوري ومؤقت.' }],
      },
{
        id: 'albizia-julibrissin-bark-extract',
        name: 'مستخلص لحاء شجرة الحرير',
        scientific_name: 'Albizia Julibrissin Bark Extract',
        searchKeywords: ['albizia julibrissin', 'persian silk tree', 'prodizia'],
        usageScope: ['skin'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'مستخلص نباتي',
        benefits: { 'مضاد للغلايكيشن': 0.9, 'تقليل علامات التعب': 0.88, 'تقليل الهالات السوداء': 0.85 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'يحمي البشرة من التلف الناتج عن عملية الغلايكيشن (ارتباط السكر بالكولاجين) ويساعد على تقليل مظهر العيون المتعبة.' }],
      },
{
        id: 'evodia-rutaecarpa-fruit-extract',
        name: 'مستخلص فاكهة إيفوديا',
        scientific_name: 'Evodia Rutaecarpa Fruit Extract',
        searchKeywords: ['evodia rutaecarpa', 'wu-zhu-yu'],
        usageScope: ['skin'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'مستخلص نباتي',
        benefits: { 'تحسين إشراقة البشرة': 0.88, 'مضاد للالتهابات': 0.85, 'تحسين الدورة الدموية الدقيقة': 0.8 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'lespedeza-capitata-leaf-stem-extract',
        name: 'مستخلص عشبة الليسبيديزا',
        scientific_name: 'Lespedeza Capitata Leaf/Stem Extract',
        searchKeywords: ['lespedeza capitata'],
        usageScope: ['skin'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'مستخلص نباتي',
        benefits: { 'مزامنة إيقاع الساعة البيولوجية للجلد': 0.9, 'حماية من الضوء الأزرق': 0.85, 'مضاد أكسدة': 0.8 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'يساعد على إعادة تنظيم إيقاع الساعة البيولوجية للبشرة الذي يختل بسبب التعرض للضوء الأزرق والإجهاد.' }],
      },
{
        id: 'plantago-lanceolata-leaf-extract',
        name: 'مستخلص أوراق لسان الحمل',
        scientific_name: 'Plantago Lanceolata Leaf Extract',
        searchKeywords: ['plantago lanceolata', 'ribwort plantain'],
        usageScope: ['skin'],
        functionalCategory: 'ملطف ومرمم',
        chemicalType: 'مستخلص نباتي',
        benefits: { 'ملطف ومرمم': 0.88, 'مضاد للالتهابات': 0.85, 'مكافحة الشيخوخة': 0.8 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'lepidium-sativum-sprout-extract',
        name: 'مستخلص براعم حب الرشاد',
        scientific_name: 'Lepidium Sativum Sprout Extract',
        searchKeywords: ['lepidium sativum', 'garden cress sprout', 'sulforawhite'],
        usageScope: ['skin'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'مستخلص نباتي',
        benefits: { 'تفتيح وتوحيد اللون': 0.88, 'مضاد أكسدة': 0.9, 'إزالة السموم': 0.85 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'غني بالسلفورافان، وهو مضاد أكسدة قوي يساعد على تفتيح البقع الداكنة وحماية البشرة.' }],
      },
{
        id: 'kigelia-africana-fruit-extract',
        name: 'مستخلص فاكهة الكجلة',
        scientific_name: 'Kigelia Africana Fruit Extract',
        searchKeywords: ['kigelia africana', 'sausage tree'],
        usageScope: ['skin'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'مستخلص نباتي',
        benefits: { 'شد البشرة': 0.85, 'تحسين مرونة الجلد': 0.8, 'مضاد للالتهابات': 0.75 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'quillaja-saponaria-bark-extract',
        name: 'مستخلص لحاء الصابونية',
        scientific_name: 'Quillaja Saponaria Bark Extract',
        searchKeywords: ['quillaja saponaria', 'soapbark'],
        usageScope: ['hair'],
        functionalCategory: 'منظف',
        chemicalType: 'منظف طبيعي',
        benefits: { 'منظف طبيعي لطيف': 0.85, 'رغوة خفيفة': 0.75 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'مصدر طبيعي للصابونين، يستخدم كمنظف لطيف جدا في الشامبوهات الطبيعية.' }],
      },
{
        id: 'marrubium-vulgare-extract',
        name: 'مستخلص حشيشة الكلب',
        scientific_name: 'Marrubium Vulgare (White Horehound) Extract',
        searchKeywords: ['marrubium vulgare', 'white horehound'],
        usageScope: ['skin'],
        functionalCategory: 'مضاد أكسدة',
        chemicalType: 'مستخلص نباتي',
        benefits: { 'مضاد للتلوث': 0.9, 'مضاد أكسدة': 0.85, 'إزالة السموم': 0.8 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'يحمي البشرة من أضرار التلوث ويقوي حاجزها.' }],
      },
{
        id: 'laminaria-ochroleuca-extract',
        name: 'مستخلص لاميناريا أوكروليuca',
        scientific_name: 'Laminaria Ochroleuca Extract',
        searchKeywords: ['laminaria ochroleuca', 'golden seaweed', 'antileukine 6'],
        usageScope: ['skin'],
        functionalCategory: 'ملطف ومرمم',
        chemicalType: 'مستخلص طحالب',
        benefits: { 'حماية من أضرار الشمس': 0.88, 'مضاد للالتهابات': 0.9, 'دعم حاجز البشرة': 0.85 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'يعرف تجاريا باسم Antileukine 6. طحلب يساعد على تقليل الالتهابات الناتجة عن الأشعة فوق البنفسجية.' }],
      },
{
        id: 'artichoke-leaf-extract',
        name: 'مستخلص أوراق الخرشوف',
        scientific_name: 'Cynara Scolymus (Artichoke) Leaf Extract',
        searchKeywords: ['cynara scolymus', 'artichoke leaf extract', 'خرشوف'],
        usageScope: ['skin'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'مستخلص نباتي',
        benefits: { 'تقليل حجم المسام': 0.85, 'تحسين مرونة الجلد': 0.8, 'مضاد أكسدة': 0.82 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'biosaccharide-gum-1',
        name: 'صمغ البيوسكاريد-1',
        scientific_name: 'Biosaccharide Gum-1',
        searchKeywords: ['biosaccharide gum-1', 'fucogel'],
        usageScope: ['skin'],
        functionalCategory: 'مرطب / مطري',
        chemicalType: 'متعدد السكاريد',
        benefits: { 'ترطيب طويل الأمد': 0.9, 'ملطف ومرمم': 0.92, 'تكوين طبقة واقية': 0.85 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'متعدد السكاريد مخمر، يشكل طبقة مرطبة ومهدئة على البشرة، مما يقلل من التهيج ويعطي إحساسا بالنعومة.' }],
      },
{
        id: 'biosaccharide-gum-4',
        name: 'صمغ البيوسكاريد-4',
        scientific_name: 'Biosaccharide Gum-4',
        searchKeywords: ['biosaccharide gum-4', 'pollustop'],
        usageScope: ['skin'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'متعدد السكاريد',
        benefits: { 'مضاد للتلوث': 0.95, 'تكوين درع واقي': 0.92, 'حماية من أضرار البيئة': 0.9 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'يشكل طبقة واقية غير مرئية على سطح الجلد لحمايته من جزيئات التلوث والمعادن الثقيلة والأشعة فوق البنفسجية.' }],
      },
{
        id: 'taurine',
        name: 'تورين',
        scientific_name: 'Taurine',
        searchKeywords: ['taurine', 'تورين'],
        usageScope: ['skin'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'حمض أميني',
        benefits: { 'تنشيط طاقة الخلايا': 0.85, 'مضاد أكسدة': 0.8, 'ترطيب': 0.82 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'حمض أميني يساعد على تنشيط خلايا الجلد وحمايتها من الجفاف والإجهاد.' }],
      },
{
        id: 'hydroxyethyl-urea',
        name: 'هيدروكسي إيثيل يوريا',
        scientific_name: 'Hydroxyethyl Urea',
        searchKeywords: ['hydroxyethyl urea', 'hydrovance'],
        usageScope: ['skin'],
        functionalCategory: 'مرطب / مطري',
        chemicalType: 'مرطب',
        benefits: { 'ترطيب مكثف': 0.92, 'لا يسبب لزوجة': 0.9, 'تحسين مرونة الجلد': 0.85 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'مشتق من اليوريا يوفر ترطيبا مكافئا للجلسرين ولكن بملمس أخف وأقل لزوجة.' }],
      },
{
        id: 'ornithine',
        name: 'أورنيثين',
        scientific_name: 'Ornithine',
        searchKeywords: ['ornithine', 'adipotrap'],
        usageScope: ['skin'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'حمض أميني',
        benefits: { 'تأثير ملء الدهون': 0.85, 'شد البشرة': 0.8 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'حمض أميني مغلف في أنظمة توصيل (ionosomes) للمساعدة في زيادة حجم الخلايا الدهنية موضعيا، مما يعطي تأثير ملء للتجاعيد.' }],
      },
{
        id: 'arabinogalactan',
        name: 'أرابينو جالاكتان',
        scientific_name: 'Galactoarabinan',
        searchKeywords: ['galactoarabinan', 'arabinogalactan', 'larch tree extract'],
        usageScope: ['skin'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'متعدد السكاريد',
        benefits: { 'تقشير لطيف': 0.8, 'تقليل فقدان الماء': 0.85, 'يعزز فعالية واقي الشمس': 0.82 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'متعدد السكاريد مستخلص من شجرة الأرز، يعمل كمقشر لطيف ويقلل من فقدان الماء عبر البشرة.' }],
      },
{
        id: 'ethylhexyl-palmitate',
        name: 'بالميتات الإيثيل هكسيل',
        scientific_name: 'Ethylhexyl Palmitate',
        searchKeywords: ['ethylhexyl palmitate', 'octyl palmitate'],
        usageScope: ['skin'],
        functionalCategory: 'مرطب / مطري',
        chemicalType: 'مطري',
        benefits: { 'مطري ومنعم': 0.9, 'بديل للزيوت المعدنية': 0.85, 'تحسين انتشار المنتج': 0.8 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'risk', text: 'قد يكون كوميدوجينيك (ساد للمسام) بدرجة عالية لبعض الأشخاص، خاصة المعرضين لحب الشباب.' }],
      },
{
        id: 'ethylhexyl-stearate',
        name: 'ستيرات الإيثيل هكسيل',
        scientific_name: 'Ethylhexyl Stearate',
        searchKeywords: ['ethylhexyl stearate', 'octyl stearate'],
        usageScope: ['skin'],
        functionalCategory: 'مرطب / مطري',
        chemicalType: 'مطري',
        benefits: { 'مطري': 0.88, 'يعطي ملمسا غنيا': 0.8 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'risk', text: 'قد يكون كوميدوجينيك بدرجة عالية.' }],
      },
{
        id: 'pentaerythrityl-tetraethylhexanoate',
        name: 'بنتاريثريتيل تترا إيثيل هكسانوات',
        scientific_name: 'Pentaerythrityl Tetraethylhexanoate',
        searchKeywords: ['pentaerythrityl tetraethylhexanoate'],
        usageScope: ['skin'],
        functionalCategory: 'مرطب / مطري',
        chemicalType: 'مطري',
        benefits: { 'مطري غني وغير لزج': 0.9, 'ملمس ناعم': 0.88 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'triethylhexanoin',
        name: 'ثلاثي إيثيل الهكسانوين',
        scientific_name: 'Triethylhexanoin',
        searchKeywords: ['triethylhexanoin'],
        usageScope: ['skin'],
        functionalCategory: 'مرطب / مطري',
        chemicalType: 'مطري',
        benefits: { 'مطري غني وغير دهني': 0.9, 'يعطي ملمسا مخمليا': 0.88, 'مشتق من الجلسرين': 0.85 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'cetyl-ethylhexanoate',
        name: 'سيتيل إيثيل هكسانوات',
        scientific_name: 'Cetyl Ethylhexanoate',
        searchKeywords: ['cetyl ethylhexanoate'],
        usageScope: ['skin'],
        functionalCategory: 'مرطب / مطري',
        chemicalType: 'مطري',
        benefits: { 'مطري مقاوم للماء': 0.88, 'ملمس ناعم': 0.85 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'neopentyl-glycol-dicaprate',
        name: 'نيوبنتيل جليكول ثنائي الكابرات',
        scientific_name: 'Neopentyl Glycol Dicaprate',
        searchKeywords: ['neopentyl glycol dicaprate'],
        usageScope: ['skin'],
        functionalCategory: 'مرطب / مطري',
        chemicalType: 'مطري',
        benefits: { 'مطري خفيف': 0.9, 'ملمس ناعم': 0.85 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'hydrogenated-polydecene',
        name: 'بولي ديسين مهدرج',
        scientific_name: 'Hydrogenated Polydecene',
        searchKeywords: ['hydrogenated polydecene'],
        usageScope: ['skin'],
        functionalCategory: 'مرطب / مطري',
        chemicalType: 'مطري',
        benefits: { 'مطري خفيف وغير لزج': 0.9, 'بديل للزيوت المعدنية': 0.88, 'يعطي ملمسا ناعما': 0.85 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'ppg-15-stearyl-ether',
        name: 'PPG-15 ستياريلي إيثر',
        scientific_name: 'PPG-15 Stearyl Ether',
        searchKeywords: ['ppg-15 stearyl ether'],
        usageScope: ['skin'],
        functionalCategory: 'مرطب / مطري',
        chemicalType: 'مطري',
        benefits: { 'مطري خفيف وغير كوميدوجينيك': 0.92, 'يعطي ملمسا ناعما وأنيقا': 0.9, 'مذيب لفلاتر الشمس': 0.85 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'myristyl-myristate',
        name: 'ميريستات الميريستيل',
        scientific_name: 'Myristyl Myristate',
        searchKeywords: ['myristyl myristate'],
        usageScope: ['skin'],
        functionalCategory: 'محسن قوام',
        chemicalType: 'مطري شمعي',
        benefits: { 'يعطي ملمسا غنيا وزبديا': 0.9, 'مطري ومنعم': 0.88, 'مكثف للقوام': 0.85 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'caution', text: 'قد يكون كوميدوجينيك (ساد للمسام) بدرجة عالية.' }],
      },
{
        id: 'cetyl-palmitate',
        name: 'بالميتات السيتيل',
        scientific_name: 'Cetyl Palmitate',
        searchKeywords: ['cetyl palmitate', 'synthetic spermaceti'],
        usageScope: ['skin'],
        functionalCategory: 'محسن قوام',
        chemicalType: 'مطري شمعي',
        benefits: { 'مكثف للقوام': 0.88, 'مطري ومنعم للبشرة': 0.85, 'يعطي ملمسا شمعيا': 0.8 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'shea-butter-ethyl-esters',
        name: 'استرات إيثيل زبدة الشيا',
        scientific_name: 'Shea Butter Ethyl Esters',
        searchKeywords: ['shea butter ethyl esters'],
        usageScope: ['skin'],
        functionalCategory: 'مرطب / مطري',
        chemicalType: 'مطري',
        benefits: { 'مطري خفيف مشتق من زبدة الشيا': 0.92, 'ملمس حريري وغير دهني': 0.9, 'ترطيب': 0.85 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'يقدم فوائد ترطيب الشيا ولكن بملمس أخف وأقل دهنية.' }],
      },
{
        id: 'di-c12-13-alkyl-malate',
        name: 'ثنائي C12-13 ألكيل مالات',
        scientific_name: 'Di-C12-13 Alkyl Malate',
        searchKeywords: ['di-c12-13 alkyl malate', 'cosmacol emi'],
        usageScope: ['skin'],
        functionalCategory: 'مرطب / مطري',
        chemicalType: 'مطري',
        benefits: { 'مطري مقشر خفيف': 0.85, 'يعطي ملمسا ناعما': 0.8, 'غير كوميدوجينيك': 0.82 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'hydrogenated-castor-oil',
        name: 'زيت الخروع المهدرج',
        scientific_name: 'Hydrogenated Castor Oil',
        searchKeywords: ['hydrogenated castor oil', 'castor wax'],
        usageScope: ['skin'],
        functionalCategory: 'محسن قوام',
        chemicalType: 'شمع',
        benefits: { 'مكثف شمعي للقوام': 0.9, 'مثبت للمستحلبات': 0.85 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'trehalose',
        name: 'تريهالوز',
        scientific_name: 'Trehalose',
        searchKeywords: ['trehalose', 'تريهالوز'],
        usageScope: ['skin'],
        functionalCategory: 'مرطب / مطري',
        chemicalType: 'مرطب',
        benefits: { 'ترطيب وحبس الرطوبة': 0.9, 'مضاد أكسدة': 0.8, 'حماية الخلايا من الجفاف': 0.92 },
        synergy: { 'hyaluronic-acid-serum': 1.08 },
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'سكر طبيعي يوجد في النباتات الصحراوية، يساعدها على البقاء على قيد الحياة في ظروف الجفاف الشديد.' }],
      },
{
        id: 'methyl-gluceth-10',
        name: 'ميثيل جلوكيث-10',
        scientific_name: 'Methyl Gluceth-10',
        searchKeywords: ['methyl gluceth-10'],
        usageScope: ['skin'],
        functionalCategory: 'مرطب / مطري',
        chemicalType: 'مرطب',
        benefits: { 'مرطب لطيف': 0.88, 'يعطي إحساسا ناعما': 0.85 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'sodium-lactate',
        name: 'لاكتات الصوديوم',
        scientific_name: 'Sodium Lactate',
        searchKeywords: ['sodium lactate', 'لاكتات الصوديوم'],
        usageScope: ['skin'],
        functionalCategory: 'مرطب / مطري',
        chemicalType: 'مرطب',
        benefits: { 'مرطب طبيعي قوي': 0.92, 'يعزز ترطيب البشرة': 0.9, 'جزء من عامل الترطيب الطبيعي': 0.95 },
        synergy: { 'lactic-acid': 1.05 },
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'ملح حمض اللاكتيك، يعمل بشكل أساسي كمرطب وليس مقشر.' }],
      },
{
        id: 'peg-4',
        name: 'PEG-4',
        scientific_name: 'PEG-4',
        searchKeywords: ['peg-4'],
        usageScope: ['skin'],
        functionalCategory: 'مرطب / مطري',
        chemicalType: 'مرطب',
        benefits: { 'مرطب': 0.8, 'مذيب': 0.85 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'peg-10',
        name: 'PEG-10',
        scientific_name: 'PEG-10',
        searchKeywords: ['peg-10'],
        usageScope: ['skin'],
        functionalCategory: 'مرطب / مطري',
        chemicalType: 'مرطب',
        benefits: { 'مرطب': 0.82, 'مذيب': 0.8 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'paraffinum-liquidum',
        name: 'زيت البارافين (الزيت المعدني)',
        scientific_name: 'Paraffinum Liquidum / Mineral Oil',
        searchKeywords: ['paraffinum liquidum', 'mineral oil', 'زيت البارافين', 'زيت معدني'],
        usageScope: ['skin'],
        functionalCategory: 'عازل',
        chemicalType: 'عازل',
        benefits: { 'عازل فعال جدا': 0.95, 'يمنع فقدان الماء': 0.92, 'يحمي البشرة': 0.85 },
        synergy: {},
        negativeSynergy: {},
        warnings: [
          { level: 'caution', text: 'مادة عازلة ممتازة وآمنة جدا، ولكنها قد تكون كوميدوجينيك (سادة للمسام) للبشرة المعرضة بشدة لحب الشباب.' },
          { level: 'info', text: 'الزيوت المعدنية المستخدمة في مستحضرات التجميل عالية النقاء ولا تسبب مشاكل صحية.' }
        ],
      },
{
        id: 'cera-microcristallina',
        name: 'شمع مايكروكريستالين',
        scientific_name: 'Cera Microcristallina / Microcrystalline Wax',
        searchKeywords: ['cera microcristallina', 'microcrystalline wax', 'شمع مايكروكريستالين'],
        usageScope: ['skin'],
        functionalCategory: 'عازل',
        chemicalType: 'شمع',
        benefits: { 'مكثف للقوام': 0.9, 'عازل': 0.85, 'يزيد من مرونة الشمع': 0.88 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'شمع مشتق من البترول، يستخدم غالبا في بلسم الشفاه والكريمات الكثيفة.' }],
      },
{
        id: 'polyisobutene',
        name: 'بولي أيزوبيوتين',
        scientific_name: 'Polyisobutene',
        searchKeywords: ['polyisobutene', 'بولي أيزوبيوتين'],
        usageScope: ['skin'],
        functionalCategory: 'عازل',
        chemicalType: 'بوليمر',
        benefits: { 'تكوين طبقة رقيقة': 0.88, 'عازل': 0.85, 'مقاوم للماء': 0.8 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'lanolin',
        name: 'لانولين',
        scientific_name: 'Lanolin',
        searchKeywords: ['lanolin', 'wool wax', 'لانولين', 'شمع الصوف'],
        usageScope: ['skin'],
        functionalCategory: 'عازل',
        chemicalType: 'شمع حيواني',
        benefits: { 'مطري ومرمم ممتاز': 0.95, 'عازل فعال': 0.9, 'يشبه دهون البشرة': 0.92 },
        synergy: {},
        negativeSynergy: {},
        warnings: [
          { level: 'info', text: 'مرطب طبيعي قوي جدا مشتق من صوف الأغنام.' },
          { level: 'risk', text: 'قد يسبب حساسية لدى الأشخاص الذين يعانون من حساسية الصوف.' }
        ],
      },
{
        id: 'candelilla-cera',
        name: 'شمع الكانديليلا',
        scientific_name: 'Euphorbia Cerifera (Candelilla) Wax',
        searchKeywords: ['candelilla cera', 'candelilla wax', 'euphorbia cerifera', 'شمع الكانديليلا'],
        usageScope: ['skin'],
        functionalCategory: 'عازل',
        chemicalType: 'شمع نباتي',
        benefits: { 'بديل نباتي لشمع النحل': 0.9, 'تكوين حاجز واقي': 0.88, 'مكثف ويعطي صلابة للمنتج': 0.92 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'carnauba-wax',
        name: 'شمع الكرنوبا',
        scientific_name: 'Copernicia Cerifera (Carnauba) Wax',
        searchKeywords: ['carnauba wax', 'copernicia cerifera', 'شمع الكرنوبا'],
        usageScope: ['skin'],
        functionalCategory: 'عازل',
        chemicalType: 'شمع نباتي',
        benefits: { 'أقوى أنواع الشمع النباتي صلابة': 0.95, 'مقاوم للماء': 0.88, 'يعطي لمعانا': 0.8 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'يستخدم في الماسكارا وأحمر الشفاه لإعطاء صلابة ومتانة.' }],
      },
{
        id: 'ozokerite',
        name: 'أوزوكريت',
        scientific_name: 'Ozokerite',
        searchKeywords: ['ozokerite', 'earth wax', 'أوزوكريت'],
        usageScope: ['skin'],
        functionalCategory: 'محسن قوام',
        chemicalType: 'شمع معدني',
        benefits: { 'مثبت للمستحلبات': 0.9, 'مكثف للزيوت': 0.88 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'synthetic-wax',
        name: 'شمع صناعي',
        scientific_name: 'Synthetic Wax',
        searchKeywords: ['synthetic wax', 'شمع صناعي'],
        usageScope: ['skin'],
        functionalCategory: 'محسن قوام',
        chemicalType: 'شمع',
        benefits: { 'مكثف للقوام': 0.88, 'مثبت للمستحلبات': 0.85 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'squalene',
        name: 'السكوالين',
        scientific_name: 'Squalene',
        searchKeywords: ['squalene', 'سكوالين'],
        usageScope: ['skin'],
        functionalCategory: 'مرطب / مطري',
        chemicalType: 'دهون',
        benefits: { 'مرطب طبيعي': 0.9, 'مضاد أكسدة': 0.85, 'جزء من دهون البشرة': 0.95 },
        synergy: {},
        negativeSynergy: {},
        warnings: [
          { level: 'risk', text: 'مادة غير مستقرة وتتأكسد بسرعة عند تعرضها للهواء، مما يجعلها كوميدوجينيك. السكوالان (Squalane) هو النسخة المهدرجة والمستقرة والمفضلة في مستحضرات التجميل.' }
        ],
      },
{
        id: 'ceramide-ng',
        name: 'سيراميد NG (سيراميد 2)',
        scientific_name: 'Ceramide NG',
        searchKeywords: ['ceramide ng', 'ceramide 2'],
        usageScope: ['skin'],
        functionalCategory: 'دعم حاجز البشرة',
        chemicalType: 'دهون',
        benefits: { 'دعم حاجز البشرة': 0.9, 'ترطيب': 0.88 },
        synergy: { 'ceramides': 1.05 },
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'ceramide-ns',
        name: 'سيراميد NS',
        scientific_name: 'Ceramide NS',
        searchKeywords: ['ceramide ns'],
        usageScope: ['skin'],
        functionalCategory: 'دعم حاجز البشرة',
        chemicalType: 'دهون',
        benefits: { 'دعم حاجز البشرة': 0.9, 'الحفاظ على بنية الدهون': 0.85 },
        synergy: { 'ceramides': 1.05 },
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'ppg-14-butyl-ether',
        name: 'PPG-14 بوتيل إيثر',
        scientific_name: 'PPG-14 Butyl Ether',
        searchKeywords: ['ppg-14 butyl ether'],
        usageScope: ['skin'],
        functionalCategory: 'مرطب / مطري',
        chemicalType: 'مطري',
        benefits: { 'مطري خفيف جدا': 0.88, 'يقلل الملمس الدهني': 0.9, 'مذيب': 0.8 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'شائع جدا في مزيلات العرق ومضادات التعرق لإعطاء ملمس ناعم وجاف.' }],
      },
{
        id: 'pentaerythrityl-distearate',
        name: 'ثنائي ستيرات بنتاريثريتيل',
        scientific_name: 'Pentaerythrityl Distearate',
        searchKeywords: ['pentaerythrityl distearate'],
        usageScope: ['skin'],
        functionalCategory: 'محسن قوام',
        chemicalType: 'مكثف',
        benefits: { 'مكثف للقوام': 0.88, 'مثبت للمستحلبات': 0.85 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'myristyl-lactate',
        name: 'لاكتات الميريستيل',
        scientific_name: 'Myristyl Lactate',
        searchKeywords: ['myristyl lactate'],
        usageScope: ['skin'],
        functionalCategory: 'مرطب / مطري',
        chemicalType: 'مطري',
        benefits: { 'مطري': 0.85, 'يعطي ملمسا كريميا': 0.8 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'caution', text: 'قد يكون كوميدوجينيك بدرجة متوسطة.' }],
      },
{
        id: 'oleyl-oleate',
        name: 'أوليات الأولييل',
        scientific_name: 'Oleyl Oleate',
        searchKeywords: ['oleyl oleate'],
        usageScope: ['skin'],
        functionalCategory: 'مرطب / مطري',
        chemicalType: 'مطري',
        benefits: { 'مطري غني': 0.88, 'يشبه زيت الجوجوبا': 0.8 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'octyldodecyl-myristate',
        name: 'ميريستات الأوكتيل دوديسيل',
        scientific_name: 'Octyldodecyl Myristate',
        searchKeywords: ['octyldodecyl myristate'],
        usageScope: ['skin'],
        functionalCategory: 'مرطب / مطري',
        chemicalType: 'مطري',
        benefits: { 'مطري غني': 0.85, 'تحسين انتشار المنتج': 0.8 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'isocetyl-stearate',
        name: 'ستيرات الأيزوسيتيل',
        scientific_name: 'Isocetyl Stearate',
        searchKeywords: ['isocetyl stearate'],
        usageScope: ['skin'],
        functionalCategory: 'مرطب / مطري',
        chemicalType: 'مطري',
        benefits: { 'مطري': 0.8, 'مذيب': 0.75 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'risk', text: 'يعتبر من المواد شديدة السد للمسام (كوميدوجينيك).' }],
      },
{
        id: 'decyl-oleate',
        name: 'أوليات الديسيل',
        scientific_name: 'Decyl Oleate',
        searchKeywords: ['decyl oleate'],
        usageScope: ['skin'],
        functionalCategory: 'مرطب / مطري',
        chemicalType: 'مطري',
        benefits: { 'مطري سريع الامتصاص': 0.85, 'ملمس غير دهني': 0.8 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'caution', text: 'قد يكون كوميدوجينيك.' }],
      },
{
        id: 'hexyldecyl-stearate',
        name: 'ستيرات الهيكسيل ديسيل',
        scientific_name: 'Hexyldecyl Stearate',
        searchKeywords: ['hexyldecyl stearate'],
        usageScope: ['skin'],
        functionalCategory: 'مرطب / مطري',
        chemicalType: 'مطري',
        benefits: { 'مطري': 0.8, 'عازل خفيف': 0.75 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'ethylhexyl-isononanoate',
        name: 'أيزونونانوات الإيثيل هكسيل',
        scientific_name: 'Ethylhexyl Isononanoate',
        searchKeywords: ['ethylhexyl isononanoate'],
        usageScope: ['skin'],
        functionalCategory: 'مرطب / مطري',
        chemicalType: 'مطري',
        benefits: { 'مطري خفيف': 0.88, 'ملمس حريري': 0.85 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'propylene-glycol-isostearate',
        name: 'أيزوستيرات البروبيلين جلايكول',
        scientific_name: 'Propylene Glycol Isostearate',
        searchKeywords: ['propylene glycol isostearate'],
        usageScope: ['skin'],
        functionalCategory: 'مرطب / مطري',
        chemicalType: 'مطري',
        benefits: { 'مطري': 0.8, 'مستحلب': 0.75 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'peg-75',
        name: 'PEG-75',
        scientific_name: 'PEG-75',
        searchKeywords: ['peg-75'],
        usageScope: ['skin'],
        functionalCategory: 'مرطب / مطري',
        chemicalType: 'مرطب',
        benefits: { 'مرطب': 0.85, 'مذيب': 0.8 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'peg-150',
        name: 'PEG-150',
        scientific_name: 'PEG-150',
        searchKeywords: ['peg-150'],
        usageScope: ['skin'],
        functionalCategory: 'مرطب / مطري',
        chemicalType: 'مرطب',
        benefits: { 'مرطب': 0.8, 'مكثف للقوام': 0.75 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'glycereth-26',
        name: 'جليسيريث-26',
        scientific_name: 'Glycereth-26',
        searchKeywords: ['glycereth-26'],
        usageScope: ['skin'],
        functionalCategory: 'مرطب / مطري',
        chemicalType: 'مرطب',
        benefits: { 'مرطب فعال': 0.9, 'ملمس ناعم غير لزج': 0.88 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'مشتق من الجلسرين يوفر ترطيبا ممتازا مع إحساس أقل لزوجة.' }],
      },
{
        id: 'methyl-gluceth-20',
        name: 'ميثيل جلوكيث-20',
        scientific_name: 'Methyl Gluceth-20',
        searchKeywords: ['methyl gluceth-20', 'glucam e-20'],
        usageScope: ['skin'],
        functionalCategory: 'مرطب / مطري',
        chemicalType: 'مرطب',
        benefits: { 'مرطب لطيف مشتق من الذرة': 0.9, 'يعطي إحساسا ناعما غير لزج': 0.92, 'يقلل من لزوجة التركيبة': 0.8 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'diglycerin',
        name: 'ديجليسرين',
        scientific_name: 'Diglycerin',
        searchKeywords: ['diglycerin', 'ديجليسرين'],
        usageScope: ['skin'],
        functionalCategory: 'مرطب / مطري',
        chemicalType: 'مرطب',
        benefits: { 'ترطيب أعمق وأطول من الجلسرين': 0.92, 'ملمس أقل لزوجة': 0.88, 'تقوية حاجز البشرة': 0.85 },
        synergy: { 'glycerin': 1.05 },
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'erythritol',
        name: 'إريثريتول',
        scientific_name: 'Erythritol',
        searchKeywords: ['erythritol', 'إريثريتول'],
        usageScope: ['skin'],
        functionalCategory: 'مرطب / مطري',
        chemicalType: 'مرطب',
        benefits: { 'ترطيب': 0.85, 'يعطي إحساسا بالبرودة': 0.75 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'سكر كحولي يعمل كمرطب.' }],
      },
{
        id: 'fructose',
        name: 'فركتوز',
        scientific_name: 'Fructose',
        searchKeywords: ['fructose', 'fruit sugar', 'فركتوز'],
        usageScope: ['skin'],
        functionalCategory: 'مرطب / مطري',
        chemicalType: 'مرطب',
        benefits: { 'مرطب (يجذب الماء)': 0.85 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'glucose',
        name: 'جلوكوز',
        scientific_name: 'Glucose',
        searchKeywords: ['glucose', 'dextrose', 'جلوكوز'],
        usageScope: ['skin'],
        functionalCategory: 'مرطب / مطري',
        chemicalType: 'مرطب',
        benefits: { 'مرطب (يجذب الماء)': 0.85, 'تغذية خلايا البشرة': 0.7 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'sodium-gluconate',
        name: 'جلوكونات الصوديوم',
        scientific_name: 'Sodium Gluconate',
        searchKeywords: ['sodium gluconate', 'جلوكونات الصوديوم'],
        usageScope: ['skin'],
        functionalCategory: 'محسن قوام',
        chemicalType: 'عامل خالب',
        benefits: { 'عامل خالب طبيعي': 0.9, 'مرطب': 0.75 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'بديل طبيعي لـ EDTA.' }],
      },
{
        id: 'glyceryl-stearate-citrate',
        name: 'سيترات ستيرات الجليسريل',
        scientific_name: 'Glyceryl Stearate Citrate',
        searchKeywords: ['glyceryl stearate citrate'],
        usageScope: ['skin'],
        functionalCategory: 'محسن قوام',
        chemicalType: 'مستحلب',
        benefits: { 'مستحلب طبيعي (معتمد من Ecocert)': 0.92, 'لطيف على البشرة': 0.9, 'يعطي ملمسا ناعما': 0.85 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'polyglyceryl-3-methylglucose-distearate',
        name: 'بولي جليسريل-3 ميثيل جلوكوز ثنائي الستيارات',
        scientific_name: 'Polyglyceryl-3 Methylglucose Distearate',
        searchKeywords: ['polyglyceryl-3 methylglucose distearate'],
        usageScope: ['skin'],
        functionalCategory: 'محسن قوام',
        chemicalType: 'مستحلب',
        benefits: { 'مستحلب لطيف مشتق من النبات': 0.92, 'مقاوم للماء بشكل جيد': 0.85, 'يعطي ملمسا ناعما': 0.88 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'sodium-polyacryloyldimethyl-taurate',
        name: 'صوديوم بولي أكريلويل دايميثيل توريت',
        scientific_name: 'Sodium Polyacryloyldimethyl Taurate',
        searchKeywords: ['sodium polyacryloyldimethyl taurate', 'simulgel 800'],
        usageScope: ['skin'],
        functionalCategory: 'محسن قوام',
        chemicalType: 'بوليمر صناعي',
        benefits: { 'مكثف ومثبت': 0.9, 'يعطي ملمسا مخمليا': 0.88 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'sodium-acrylate-sodium-acryloyldimethyl-taurate-copolymer',
        name: 'صوديوم أكريلات/صوديوم أكريلويل دايميثيل توريت كوبوليمر',
        scientific_name: 'Sodium Acrylate/Sodium Acryloyldimethyl Taurate Copolymer',
        searchKeywords: ['sodium acrylate/sodium acryloyldimethyl taurate copolymer', 'simulgel eg'],
        usageScope: ['skin'],
        functionalCategory: 'محسن قوام',
        chemicalType: 'بوليمر صناعي',
        benefits: { 'مكثف ومستحلب': 0.92, 'يعطي ملمسا خفيفا': 0.9 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'polyacrylate-crosspolymer-6',
        name: 'بولي أكريلات كروسبوليمر-6',
        scientific_name: 'Polyacrylate Crosspolymer-6',
        searchKeywords: ['polyacrylate crosspolymer-6', 'sepimax zen'],
        usageScope: ['skin'],
        functionalCategory: 'محسن قوام',
        chemicalType: 'بوليمر صناعي',
        benefits: { 'مكثف ومثبت فائق': 0.95, 'مقاوم للكهارل والأحماض': 0.92, 'يعطي ملمسا غنيا': 0.88 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'بوليمر حديث وقوي جدا، قادر على تكثيف التركيبات الصعبة التي تحتوي على الأحماض أو الأملاح.' }],
      },
{
        id: 'microcrystalline-cellulose',
        name: 'سليلوز بلوري دقيق',
        scientific_name: 'Microcrystalline Cellulose',
        searchKeywords: ['microcrystalline cellulose'],
        usageScope: ['skin'],
        functionalCategory: 'محسن قوام',
        chemicalType: 'مثبت',
        benefits: { 'مثبت للتركيبة': 0.9, 'يمتص الزيوت': 0.8, 'مقشر فيزيائي لطيف': 0.75 },
        synergy: { 'cellulose-gum': 1.05 },
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'magnesium-stearate',
        name: 'ستيرات المغنيسيوم',
        scientific_name: 'Magnesium Stearate',
        searchKeywords: ['magnesium stearate'],
        usageScope: ['skin'],
        functionalCategory: 'محسن قوام',
        chemicalType: 'مكثف',
        benefits: { 'مضاد للتكتل': 0.9, 'مكثف': 0.8, 'تحسين الملمس': 0.85 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'distarch-phosphate',
        name: 'فوسفات النشا',
        scientific_name: 'Distarch Phosphate',
        searchKeywords: ['distarch phosphate', 'corn starch modified'],
        usageScope: ['skin'],
        functionalCategory: 'محسن قوام',
        chemicalType: 'ممتص للدهون',
        benefits: { 'يمتص الزيوت': 0.88, 'يعطي ملمسا مطفيا': 0.85, 'مكثف طبيعي': 0.8 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'niacinamide',
        name: 'النياسيناميد (فيتامين B3)',
        scientific_name: 'Niacinamide',
        searchKeywords: ['niacinamide', 'nicotinamide', 'vitamin b3', 'نياسيناميد', 'نيكوتيناميد', 'فيتامين ب3'],
        usageScope: ['skin', 'hair'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'فيتامين',
        benefits: { 'دعم حاجز البشرة': 0.95, 'تنظيم الإفرازات الدهنية': 0.9, 'ملطف ومرمم': 0.88, 'تفتيح وتوحيد اللون': 0.8 },
        synergy: { 'hyaluronic-acid-serum': 1.08, 'zinc-pca': 1.1, 'ceramides': 1.1 },
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'قد يسبب احمرارا أو حكة خفيفة في بداية الاستخدام، خاصة بتركيزات عالية (أعلى من 5%).' }],
      },
{
        id: 'retinoid',
        name: 'الريتينويدات (ريتينول، ريتينال)',
        scientific_name: 'Retinol, Retinal, etc.',
        searchKeywords: ['retinoid', 'retinol', 'retinal', 'retinaldehyde', 'ريتينويد', 'ريتينول', 'ريتينال'],
        usageScope: ['skin'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'ريتينويد',
        benefits: { 'مكافحة الشيخوخة': 0.95, 'مكافحة حب الشباب': 0.88, 'تفتيح وتوحيد اللون': 0.85, 'تحفيز الكولاجين': 0.92 },
        synergy: { 'niacinamide': 1.05, 'hyaluronic-acid-serum': 1.08 },
        negativeSynergy: { 'glycolic-acid': { reason: 'قد يزيد من التهيج والجفاف عند استخدامهما في نفس الروتين.' }, 'vitamin-c-serum': { reason: 'يعملان في درجات حموضة مختلفة وقد يقللان من فعالية بعضهما البعض.' } },
        warnings: [
          { level: 'info', text: 'يجب استخدامه ليلا فقط لضمان فعاليته وتجنب تحلله بالضوء.' },
          { level: 'risk', text: 'يزيد من حساسية البشرة للشمس بشكل كبير؛ استخدام واقي شمسي عالي الحماية (SPF 30+) يوميا ضروري.' },
          { level: 'risk', text: 'غير آمن للاستخدام أثناء الحمل أو الرضاعة.' }
        ],
      },
{
        id: 'vitamin-c',
        name: 'فيتامين C (حمض الأسكوربيك النقي)',
        scientific_name: 'Ascorbic Acid',
        searchKeywords: ['vitamin c', 'ascorbic acid', 'l-ascorbic acid', 'فيتامين سي', 'حمض الأسكوربيك'],
        usageScope: ['skin'],
        functionalCategory: 'مضاد أكسدة',
        chemicalType: 'فيتامين',
        benefits: { 'مضاد أكسدة': 0.98, 'تفتيح وتوحيد اللون': 0.9, 'تحفيز الكولاجين': 0.88, 'حماية من أضرار الشمس': 0.85 },
        synergy: { 'ferulic-acid': 1.15, 'tocopherol': 1.1 },
        negativeSynergy: { 'copper-peptides': { reason: 'قد يتأكسد كل منهما الآخر ويقلل من فعاليتهما.' } },
        warnings: [
          { level: 'caution', text: 'غير مستقر للغاية ويتأكسد بسرعة عند تعرضه للهواء والضوء (يتغير لونه إلى الأصفر أو البني).' },
          { level: 'caution', text: 'قد يسبب وخزا أو تهيجا، خاصة بتركيزات عالية أو على البشرة الحساسة.' }
        ],
      },
{
        id: 'glycolic-acid',
        name: 'حمض الجليكوليك',
        scientific_name: 'Glycolic Acid',
        searchKeywords: ['glycolic acid', 'aha', 'حمض الجليكوليك', 'جليكوليك أسيد'],
        usageScope: ['skin'],
        functionalCategory: 'مقشر',
        chemicalType: 'حمض ألفا هيدروكسي (AHA)',
        benefits: { 'تقشير': 0.92, 'تفتيح وتوحيد اللون': 0.88, 'مكافحة الشيخوخة': 0.85 },
        synergy: { 'hyaluronic-acid-serum': 1.1 },
        negativeSynergy: { 'retinoid-serum': { reason: 'قد يزيد من التهيج والجفاف عند استخدامهما في نفس الروتين.' } },
        warnings: [
          { level: 'risk', text: 'يزيد من حساسية البشرة للشمس؛ يجب استخدام واقي الشمس بانتظام.' },
          { level: 'caution', text: 'قد يسبب وخزا أو احمرارا عند الاستخدام، خاصة للمبتدئين.' }
        ],
      },
{
        id: 'azelaic-acid',
        name: 'حمض الأزيليك',
        scientific_name: 'Azelaic Acid',
        searchKeywords: ['azelaic acid', 'حمض الأزيليك', 'أزيليك أسيد'],
        usageScope: ['skin'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'حمض ثنائي الكربوكسيل',
        benefits: { 'تفتيح وتوحيد اللون': 0.9, 'مكافحة حب الشباب': 0.9, 'ملطف ومرمم': 0.92, 'علاج الوردية': 0.88 },
        synergy: { 'niacinamide': 1.08 },
        negativeSynergy: {},
        warnings: [{ level: 'caution', text: 'قد يسبب وخزا أو حكة خفيفة في بداية الاستخدام، وعادة ما يزول مع اعتياد البشرة عليه.' }],
      },
{
        id: 'alpha-arbutin-serum',
        name: 'ألفا أربوتين',
        scientific_name: 'Alpha-Arbutin',
        searchKeywords: ['alpha arbutin', 'arbutin', 'ألفا أربوتين', 'اربوتين'],
        usageScope: ['skin'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'مثبط للتصبغ',
        benefits: { 'تفتيح وتوحيد اللون': 0.92, 'علاج البقع الداكنة': 0.9 },
        synergy: { 'niacinamide': 1.1, 'vitamin-c': 1.08 },
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'يعتبر بديلا آمنا وفعالا للهيدروكينون.' }],
      },
{
        id: 'peptide-serum',
        name: 'الببتيدات',
        scientific_name: 'Peptides',
        searchKeywords: ['peptides', 'peptide', 'ببتيد', 'ببتيدات'],
        usageScope: ['skin'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'ببتيد',
        benefits: { 'تحفيز الكولاجين': 0.9, 'تحسين مرونة الجلد': 0.88, 'تقليل التجاعيد': 0.85 },
        synergy: { 'hyaluronic-acid-serum': 1.1 },
        negativeSynergy: { 'glycolic-acid': { reason: 'الأحماض القوية قد تفكك روابط الببتيد وتقلل من فعاليته.' } },
        warnings: [],
      },
{
        id: 'ascorbyl-glucoside',
        name: 'أسكوربيل غلوكوزيد',
        scientific_name: 'Ascorbyl Glucoside',
        searchKeywords: ['ascorbyl glucoside', 'aa2g', 'أسكوربيل غلوكوزيد'],
        usageScope: ['skin'],
        functionalCategory: 'مضاد أكسدة',
        chemicalType: 'مشتق فيتامين',
        benefits: { 'فيتامين سي مستقر جدا': 0.95, 'تفتيح وتوحيد اللون': 0.88, 'مضاد أكسدة': 0.9 },
        synergy: { 'niacinamide': 1.05 },
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'مشتق مستقر من فيتامين C، يتحول إلى حمض الأسكوربيك في الجلد، مما يجعله فعالا ولكنه لطيف جدا ومناسب للبشرة الحساسة.' }],
      },
{
        id: 'hyaluronic-acid-serum',
        name: 'حمض الهيالورونيك',
        scientific_name: 'Hyaluronic Acid / Sodium Hyaluronate',
        searchKeywords: ['hyaluronic acid', 'ha', 'sodium hyaluronate', 'هيالورونيك أسيد', 'هايلورونيك'],
        usageScope: ['skin', 'hair'],
        functionalCategory: 'مرطب / مطري',
        chemicalType: 'مرطب',
        benefits: { 'ترطيب': 0.98, 'ملء الخطوط الدقيقة': 0.85, 'دعم حاجز البشرة': 0.8 },
        synergy: { 'vitamin-c': 1.1, 'niacinamide': 1.08 },
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'لأفضل النتائج، يجب تطبيقه على بشرة رطبة ثم وضع مرطب فوقه لحبس الرطوبة.' }],
      },
{
        id: 'glycerin',
        name: 'الجلسرين',
        scientific_name: 'Glycerin',
        searchKeywords: ['glycerin', 'glycerol', 'جلسرين', 'غليسرين'],
        usageScope: ['skin', 'hair'],
        functionalCategory: 'مرطب / مطري',
        chemicalType: 'مرطب',
        benefits: { 'ترطيب': 0.98, 'دعم حاجز البشرة': 0.9, 'حماية البشرة': 0.88 },
        synergy: { 'hyaluronic-acid-serum': 1.1 },
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'في الأجواء شديدة الجفاف، قد يسحب الرطوبة من الطبقات العميقة للجلد إلى السطح إذا لم يستخدم مع مكونات عازلة.' }],
      },
{
        id: 'ceramides',
        name: 'السيراميدات',
        scientific_name: 'Ceramide NP, AP, EOP, etc.',
        searchKeywords: ['ceramide', 'ceramides', 'سيراميد', 'سيراميدات'],
        usageScope: ['skin', 'hair'],
        functionalCategory: 'دعم حاجز البشرة',
        chemicalType: 'دهون',
        benefits: { 'دعم حاجز البشرة': 0.98, 'ترطيب': 0.9, 'ملطف ومرمم': 0.95 },
        synergy: { 'cholesterol': 1.1, 'linoleic-acid': 1.1 },
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'المكون الأساسي لحاجز البشرة الصحي. ضروري للبشرة الجافة، الحساسة، أو المعرضة للأكزيما.' }],
      },
{
        id: 'cholesterol',
        name: 'الكوليسترول',
        scientific_name: 'Cholesterol',
        searchKeywords: ['cholesterol', 'كوليسترول'],
        usageScope: ['skin'],
        functionalCategory: 'دعم حاجز البشرة',
        chemicalType: 'دهون',
        benefits: { 'دعم حاجز البشرة': 0.95, 'ترطيب': 0.9, 'إصلاح البشرة الناضجة': 0.92 },
        synergy: { 'ceramides': 1.1, 'linoleic-acid': 1.1 },
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'مكون حيوي (يشكل ~25% من دهون الحاجز) يعمل مع السيراميدات والأحماض الدهنية للحفاظ على صحة الجلد.' }],
      },
{
        id: 'panthenol',
        name: 'البانثينول (برو-فيتامين B5)',
        scientific_name: 'Panthenol',
        searchKeywords: ['panthenol', 'dexpanthenol', 'pro-vitamin b5', 'بانثينول'],
        usageScope: ['skin', 'hair'],
        functionalCategory: 'ملطف ومرمم',
        chemicalType: 'فيتامين',
        benefits: { 'ترطيب': 0.92, 'ملطف ومرمم': 0.9, 'دعم حاجز البشرة': 0.88 },
        synergy: { 'hyaluronic-acid-serum': 1.05 },
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'butylene-glycol',
        name: 'بيوتيلين جلايكول',
        scientific_name: 'Butylene Glycol',
        searchKeywords: ['butylene glycol', 'bg', 'بيوتيلين جلايكول'],
        usageScope: ['hair', 'skin'],
        functionalCategory: 'مذيب',
        chemicalType: 'مرطب',
        benefits: { 'ترطيب': 0.8, 'مذيب': 0.9, 'تحسين قوام المنتج': 0.85 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'peg-ppg-polybutylene-glycol-8-5-3-glycerin',
        name: 'بيج/بي بي جي/بولي بيوتيلين جلايكول-8/5/3 جليسرين',
        scientific_name: 'PEG/PPG/Polybutylene Glycol-8/5/3 Glycerin',
        searchKeywords: ['peg ppg polybutylene glycol', 'PEG/PPG/Polybutylene Glycol-8/5/3 Glycerin', 'PEG/PPG/Polybutylene Glycol-8/5/3', 'بولي إيثيلين جلايكول/ بولي بروبيلين جلايكول/ بولي بيوتيلين جلايكول-8/5/3 جليسرين', 'بيج/بي بي جي/بولي بيوتيلين جليكول-8/5/3 جليسرين'],
        usageScope: ['skin'],
        functionalCategory: 'مرطب / مطري',
        chemicalType: 'بوليمر مرطب',
        benefits: { 'ترطيب فعال للبشرة': 0.85, 'يحسن قوام المنتج': 0.7 },
        synergy: { 'glycerin': 1.05 },
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'isononyl-isononanoate',
        name: 'أيزونونيل أيزونونانوات',
        scientific_name: 'Isononyl Isononanoate',
        searchKeywords: ['isononyl isononanoate', 'أيزونونيل أيزونونانوات'],
        usageScope: ['skin'],
        functionalCategory: 'مرطب / مطري',
        chemicalType: 'مطري',
        benefits: { 'مطري ذو ملمس شبيه بالسيليكون': 0.9, 'خفيف الوزن وغير ساد للمسام': 0.88, 'يعطي انزلاقا ممتازا': 0.92 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: '2-mercaptonicotinoyl-glycine',
        name: '٢-ميركابتونيكوتينويل جلايسين',
        scientific_name: '2-Mercaptonicotinoyl Glycine',
        searchKeywords: ['2-Mercaptonicotinoyl Glycine', '2-MNG'],
        usageScope: ['skin'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'مثبط للتصبغ',
        benefits: { 'يمنع تكون التصبغات': 0.9, 'يوحد لون البشرة': 0.85 },
        synergy: { 'niacinamide': 1.1 },
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'silica',
        name: 'سيليكا',
        scientific_name: 'Silica',
        searchKeywords: ['silica', 'silicon dioxide', 'سيليكا'],
        usageScope: ['skin'],
        functionalCategory: 'محسن قوام',
        chemicalType: 'ممتص للدهون',
        benefits: { 'امتصاص الزيوت والدهون': 0.9, 'تحسين ملمس المنتج': 0.85, 'تأثير مطفي (Matte)': 0.88 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'adenosine',
        name: 'أدينوزين',
        scientific_name: 'Adenosine',
        searchKeywords: ['adenosine', 'أدينوزين'],
        usageScope: ['skin'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'مكون متواصل مع الخلايا',
        benefits: { 'مكافحة الشيخوخة': 0.88, 'ملطف ومرمم': 0.9, 'دعم حاجز البشرة': 0.85 },
        synergy: { 'niacinamide': 1.05 },
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'trisodium-ethylenediamine-disuccinate',
        name: 'ثلاثي صوديوم إيثيلين ديامين ديسكسينات',
        scientific_name: 'Trisodium Ethylenediamine Disuccinate',
        searchKeywords: ['trisodium ethylenediamine disuccinate', 'edds'],
        usageScope: ['skin', 'hair'],
        functionalCategory: 'محسن قوام',
        chemicalType: 'عامل خالب',
        benefits: { 'يحافظ على استقرار المنتج': 0.9, 'قابل للتحلل': 0.88 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'xanthan-gum',
        name: 'صمغ الزانثان',
        scientific_name: 'Xanthan Gum',
        searchKeywords: ['xanthan gum', 'صمغ الزانثان', 'زانثان غوم', 'زانثان جم', 'زانثان', 'xantham gum', 'xantham', 'الكرودان'],
        usageScope: ['skin', 'hair'],
        functionalCategory: 'محسن قوام',
        chemicalType: 'بوليمر طبيعي',
        benefits: { 'مكثف للقوام': 0.9, 'تثبيت المستحلبات': 0.88 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'tocopheryl-acetate',
        name: 'توكوفيريل أسيتات',
        scientific_name: 'Tocopheryl Acetate',
        searchKeywords: ['tocopheryl acetate', 'vitamin e acetate'],
        usageScope: ['skin'],
        functionalCategory: 'مضاد أكسدة',
        chemicalType: 'مشتق فيتامين',
        benefits: { 'مضاد أكسدة (في المنتج)': 0.85, 'ترطيب': 0.8 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'شكل استر ومستقر من فيتامين E. يعمل بشكل أساسي كمضاد أكسدة لحماية التركيبة، وفعاليته على الجلد أقل من التوكوفيرول النقي.' }],
      },
{
        id: 'ci-15510-orange-4',
        name: 'CI 15510 / برتقالي 4',
        scientific_name: 'CI 15510',
        searchKeywords: ['ci 15510', 'orange 4'],
        usageScope: ['skin'],
        functionalCategory: 'محسن جمالي',
        chemicalType: 'ملون صناعي',
        benefits: { 'يعطي لونا برتقاليا': 1.0 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'قد يسبب حساسية لدى بعض الأفراد.' }],
      },
{
        id: 'ci-19140-yellow-5',
        name: 'CI 19140 / أصفر 5',
        scientific_name: 'CI 19140',
        searchKeywords: ['ci 19140', 'yellow 5', 'tartrazine'],
        usageScope: ['skin', 'hair'],
        functionalCategory: 'محسن جمالي',
        chemicalType: 'ملون صناعي',
        benefits: { 'يعطي لونا أصفرا': 1.0 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'caution', text: 'ارتبط بحساسية نادرة لدى بعض الأشخاص، خاصة الذين يعانون من حساسية الأسبرين.' }],
      },
{
        id: 'parfum-fragrance',
        name: 'عطر',
        scientific_name: 'Parfum / Fragrance',
        searchKeywords: ['parfum', 'fragrance', 'aroma', 'scent', 'عطر', 'رائحة'],
        usageScope: ['skin', 'hair'],
        functionalCategory: 'عطر',
        chemicalType: 'مزيج عطري',
        benefits: { 'تحسين رائحة المنتج': 0.98 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'risk', text: 'أحد أكثر المسببات شيوعا لردود الفعل التحسسية والتهيج في مستحضرات التجميل. يمكن أن يخفي عشرات المكونات الكيميائية.' }],
      },
{
        id: 'oud-oil',
        name: 'زيت العود',
        scientific_name: 'Aquilaria Malaccensis Oil',
        searchKeywords: ['oud oil', 'agarwood oil', 'aquilaria malaccensis', 'oudh', 'oud', 'عود', 'دهن العود'],
        usageScope: ['skin'],
        functionalCategory: 'عطر',
        chemicalType: 'زيت عطري',
        benefits: { 'رائحة فاخرة': 0.99, 'مهدئ': 0.85, 'مضاد للميكروبات': 0.7 },
        synergy: { 'sandalwood-essential': 1.1, 'rose-essential': 1.08 },
        negativeSynergy: {},
        warnings: [
          { level: 'caution', text: 'مكلف جدا ونادر، تأكد من نقاء المصدر.' },
          { level: 'info', text: 'يتم استخدامه بتركيزات منخفضة جدا في المنتجات النهائية.' }
        ],
      },
{
        id: 'amber-oil',
        name: 'زيت العنبر',
        scientific_name: 'Pinus Succinifera Oil',
        searchKeywords: ['amber oil', 'pinus succinifera', 'amber', 'عنبر', 'زيت العنبر'],
        usageScope: ['skin', 'hair'],
        functionalCategory: 'عطر',
        chemicalType: 'زيت عطري',
        benefits: { 'رائحة دافئة': 0.95, 'مهدئ': 0.9, 'ملطف ومرمم': 0.8 },
        synergy: { 'frankincense-essential': 1.1 },
        negativeSynergy: {},
        warnings: [
          { level: 'info', text: 'غالبا ما يكون زيتا عطريا مركبا وليس من مصدر واحد. يتم استخدامه بتركيزات منخفضة.' }
        ],
      },
{
        id: 'taif-rose-oil',
        name: 'زيت الورد الطائفي',
        scientific_name: 'Rosa Damascena var. Trigintipetala Oil',
        searchKeywords: ['taif rose oil', 'taif rose', 'ورد طائفي', 'زيت الورد الطائفي'],
        usageScope: ['skin'],
        functionalCategory: 'عطر',
        chemicalType: 'زيت عطري',
        benefits: { 'رائحة فاخرة': 0.99, 'مكافحة الشيخوخة': 0.9, 'ملطف ومرمم': 0.88 },
        synergy: { 'oud-oil': 1.1 },
        negativeSynergy: {},
        warnings: [
          { level: 'caution', text: 'نادر ومكلف جدا. تأكد من نقاء المصدر.' },
          { level: 'info', text: 'يتم استخدامه بتركيزات منخفضة جدا.' }
        ],
      },
{
        id: 'q10-serum',
        name: 'سيروم إنزيم Q10',
        scientific_name: 'Ubiquinone',
        searchKeywords: ['coenzyme q10', 'q10', 'ubiquinone', 'إنزيم Q10'],
        usageScope: ['skin'],
        functionalCategory: 'مضاد أكسدة',
        chemicalType: 'إنزيم مساعد',
        benefits: { 'مضاد أكسدة': 0.9, 'مكافحة الشيخوخة': 0.88, 'إنتاج طاقة الخلايا': 0.85 },
        synergy: { 'tocopherol': 1.08 },
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'growth-factors-serum',
        name: 'سيروم عوامل النمو',
        scientific_name: 'EGF, FGF, etc.',
        searchKeywords: ['growth factors', 'egf', 'fgf', 'عوامل النمو'],
        usageScope: ['skin'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'عامل نمو (ببتيد)',
        benefits: { 'تجديد البشرة': 0.95, 'تحفيز الكولاجين': 0.92, 'علاج الندوب': 0.88 },
        synergy: { 'hyaluronic-acid-serum': 1.1 },
        negativeSynergy: {},
        warnings: [
          { level: 'caution', text: 'مكونات بيولوجية متقدمة، غالبا ما تكون مكلفة.' },
          { level: 'info', text: 'يجب استشارة طبيب الجلدية لتحديد مدى ملاءمتها.' }
        ],
      },
{
        id: 'stem-cell-serum',
        name: 'سيروم الخلايا الجذعية',
        scientific_name: 'Stem Cell Conditioned Media / Plant Stem Cells',
        searchKeywords: ['stem cell', 'خلايا جذعية'],
        usageScope: ['skin'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'مكون حيوي',
        benefits: { 'تجديد عميق للبشرة': 0.92, 'إصلاح التلف': 0.9, 'مكافحة الشيخوخة': 0.88 },
        synergy: { 'peptide-serum': 1.08 },
        negativeSynergy: {},
        warnings: [
          { level: 'caution', text: 'غالبا ما تكون المنتجات مكلفة. الأدلة العلمية على الخلايا الجذعية النباتية لا تزال قيد البحث.' }
        ],
      },
{
        id: 'matrixyl-serum',
        name: 'سيروم ماتريكسيل',
        scientific_name: 'Palmitoyl Pentapeptide-4 / Palmitoyl Tripeptide-1',
        searchKeywords: ['matrixyl', 'matrixyl 3000', 'ماتريكسيل'],
        usageScope: ['skin'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'ببتيد',
        benefits: { 'تقليل التجاعيد': 0.92, 'تحفيز الكولاجين': 0.9, 'تحسين مرونة البشرة': 0.88 },
        synergy: { 'hyaluronic-acid-serum': 1.08 },
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'illuminyl-serum',
        name: 'سيروم إلومينيل',
        scientific_name: 'Diglucosyl Gallic Acid',
        searchKeywords: ['illuminyl', 'diglucosyl gallic acid', 'إلومينيل'],
        usageScope: ['skin'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'مركب نشط حيويا',
        benefits: { 'تفتيح وتوحيد اللون': 0.95, 'تقليل الاحمرار': 0.88, 'مضاد أكسدة': 0.9 },
        synergy: { 'niacinamide': 1.1 },
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'جيل جديد من مكونات التفتيح يتم تنشيطه بواسطة ميكروبيوم الجلد.' }],
      },
{
        id: 'argireline-serum',
        name: 'سيروم الأرجيرلاين',
        scientific_name: 'Acetyl Hexapeptide-8',
        searchKeywords: ['argireline', 'acetyl hexapeptide-8', 'botox in a jar', 'أرجيرلاين'],
        usageScope: ['skin'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'ببتيد',
        benefits: { 'تقليل التجاعيد التعبيرية': 0.9, 'تأثير شبيه بالبوتوكس': 0.88, 'تنعيم الخطوط الدقيقة': 0.85 },
        synergy: { 'matrixyl-serum': 1.1 },
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'يعمل عن طريق إرخاء العضلات السطحية للوجه، مما يقلل من ظهور التجاعيد الناتجة عن الحركة.' }],
      },
{
        id: 'azelaic-acid-suspension',
        name: 'حمض الأزيليك (معلق)',
        scientific_name: 'Azelaic Acid',
        searchKeywords: ['azelaic acid suspension', 'معلق أزيليك'],
        usageScope: ['skin'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'حمض ثنائي الكربوكسيل',
        benefits: { 'مكافحة حب الشباب': 0.9, 'ملطف ومرمم': 0.92, 'تفتيح وتوحيد اللون': 0.9 },
        synergy: { 'niacinamide': 1.08 },
        negativeSynergy: {},
        warnings: [
          { level: 'caution', text: 'قوام المعلق قد يتعارض مع منتجات أخرى (يتكتل أو يتقشر).' },
          { level: 'caution', text: 'قد يسبب وخزا أو حكة خفيفة في بداية الاستخدام.' }
        ],
      },
{
        id: 'selenium',
        name: 'السيلينيوم',
        scientific_name: 'Selenium',
        searchKeywords: ['selenium', 'سيلينيوم'],
        usageScope: ['skin'],
        functionalCategory: 'مضاد أكسدة',
        chemicalType: 'معدن',
        benefits: { 'مضاد أكسدة': 0.85, 'حماية من أضرار الشمس': 0.8, 'مضاد للالتهابات': 0.75 },
        synergy: { 'tocopherol': 1.1 },
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'peg-compounds',
        name: 'مركبات PEG',
        scientific_name: 'Polyethylene Glycol',
        searchKeywords: ['peg', 'polyethylene glycol', 'بي اي جي'],
        usageScope: ['skin', 'hair'],
        functionalCategory: 'محسن قوام',
        chemicalType: 'بوليمر',
        benefits: { 'تحسين امتصاص المكونات': 0.8, 'مستحلب': 0.75, 'مرطب': 0.7 },
        synergy: {},
        negativeSynergy: {},
        warnings: [
          { level: 'caution', text: 'هناك مخاوف نظرية بشأن التلوث بـ 1,4-dioxane أثناء التصنيع، ولكن الهيئات التنظيمية تضع حدودا صارمة لضمان الأمان.' },
          { level: 'risk', text: 'لا ينصح باستخدامه على البشرة المتضررة أو المجروحة.' }
        ],
      },
{
        id: 'parabens',
        name: 'البارابين (عام)',
        scientific_name: 'Parabens',
        searchKeywords: ['paraben', 'parabens', 'بارابين'],
        usageScope: ['skin', 'hair'],
        functionalCategory: 'مكون ضار',
        chemicalType: 'مادة حافظة',
        benefits: { 'حفظ المنتج': 0.9 },
        synergy: {},
        negativeSynergy: {},
        warnings: [
          { level: 'caution', text: 'مادة حافظة مثيرة للجدل. على الرغم من أن الهيئات التنظيمية مثل SCCS و CIR تعتبرها آمنة بالتركيزات المستخدمة، إلا أن هناك مخاوف عامة بشأن تأثيرها الهرموني المحتمل.' }
        ],
      },
{
        id: 'methylparaben',
        name: 'ميثيل بارابين',
        scientific_name: 'Methylparaben',
        searchKeywords: ['methylparaben', 'ميثيل بارابين'],
        usageScope: ['skin', 'hair'],
        functionalCategory: 'مادة حافظة',
        chemicalType: 'بارابين',
        benefits: { 'حفظ المنتج': 0.9 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'caution', text: 'أحد أكثر أنواع البارابين شيوعا وأقلها إثارة للجدل. يعتبر آمنا بالتركيزات المسموح بها.' }],
      },
{
        id: 'propylparaben',
        name: 'بروبيل بارابين',
        scientific_name: 'Propylparaben',
        searchKeywords: ['propylparaben', 'بروبيل بارابين'],
        usageScope: ['skin', 'hair'],
        functionalCategory: 'مادة حافظة',
        chemicalType: 'بارابين',
        benefits: { 'حفظ المنتج': 0.92 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'caution', text: 'بارابين فعال جدا، ولكنه أكثر إثارة للجدل من الميثيل بارابين بسبب نشاطه الهرموني الطفيف في الدراسات المخبرية.' }],
      },
{
        id: 'butylparaben',
        name: 'بوتيل بارابين',
        scientific_name: 'Butylparaben',
        searchKeywords: ['butylparaben', 'بوتيل بارابين'],
        usageScope: ['skin', 'hair'],
        functionalCategory: 'مادة حافظة',
        chemicalType: 'بارابين',
        benefits: { 'حفظ المنتج': 0.95 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'caution', text: 'أقوى وأكثر إثارة للجدل من البروبيل بارابين. تم تقييد استخدامه بشدة في الاتحاد الأوروبي.' }],
      },
{
        id: 'ethylparaben',
        name: 'إيثيل بارابين',
        scientific_name: 'Ethylparaben',
        searchKeywords: ['ethylparaben', 'إيثيل بارابين'],
        usageScope: ['skin', 'hair'],
        functionalCategory: 'مادة حافظة',
        chemicalType: 'بارابين',
        benefits: { 'حفظ المنتج': 0.88 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'caution', text: 'يعتبر من البارابينات الأكثر أمانا نسبيا.' }],
      },
{
        id: 'formaldehyde',
        name: 'الفورمالديهايد',
        scientific_name: 'Formaldehyde',
        searchKeywords: ['formaldehyde', 'formalin', 'فورمالديهايد'],
        usageScope: ['skin', 'hair'],
        functionalCategory: 'مكون ضار',
        chemicalType: 'مادة حافظة (محظورة)',
        benefits: {},
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'risk', text: 'مادة مسرطنة معروفة ومهيجة قوية للجلد والجهاز التنفسي. محظور استخدامه كمكون مباشر في مستحضرات التجميل في العديد من المناطق.' }],
      },
{
        id: 'formaldehyde-releasers',
        name: 'مطلقات الفورمالديهايد (عام)',
        scientific_name: 'Formaldehyde Releasers',
        searchKeywords: ['formaldehyde releasers', 'مطلقات الفورمالديهايد'],
        usageScope: ['skin', 'hair'],
        functionalCategory: 'مكون ضار',
        chemicalType: 'مادة حافظة',
        benefits: { 'حفظ المنتج': 0.8 },
        synergy: {},
        negativeSynergy: {},
        warnings: [
          { level: 'risk', text: 'مجموعة من المواد الحافظة التي تطلق الفورمالديهايد ببطء للحفاظ على المنتج. يمكن أن تسبب حساسية لدى الأشخاص الذين يعانون من حساسية الفورمالديهايد.' }
        ],
      },
{
        id: 'triclosan',
        name: 'تريكلوسان',
        scientific_name: 'Triclosan',
        searchKeywords: ['triclosan', 'تريكلوسان'],
        usageScope: ['skin'],
        functionalCategory: 'مكون ضار',
        chemicalType: 'عامل مضاد للميكروبات',
        benefits: { 'مضاد للميكروبات': 0.9 },
        synergy: {},
        negativeSynergy: {},
        warnings: [
          { level: 'risk', text: 'تم حظره من قبل إدارة الغذاء والدواء الأمريكية في منتجات الغسول المطهرة. مرتبط بمخاوف بشأن مقاومة البكتيريا واضطراب الهرمونات.' }
        ],
      },
{
        id: 'mineral-oil',
        name: 'الزيت المعدني',
        scientific_name: 'Mineral Oil / Paraffinum Liquidum',
        searchKeywords: ['mineral oil', 'paraffinum liquidum', 'زيت معدني'],
        usageScope: ['skin'],
        functionalCategory: 'عازل',
        chemicalType: 'عازل',
        benefits: { 'عازل فعال جدا': 0.95, 'يمنع فقدان الماء': 0.92 },
        synergy: {},
        negativeSynergy: {},
        warnings: [
          { level: 'caution', text: 'فعال جدا في حبس الرطوبة، ولكنه قد يكون كوميدوجينيك (ساد للمسام) للبشرة المعرضة بشدة لحب الشباب.' }
        ],
      },
{
        id: 'petrolatum',
        name: 'الفازلين (بترولاتوم)',
        scientific_name: 'Petrolatum',
        searchKeywords: ['petrolatum', 'petroleum jelly', 'vaseline', 'فازلين'],
        usageScope: ['skin'],
        functionalCategory: 'عازل',
        chemicalType: 'عازل',
        benefits: { 'أقوى مادة عازلة': 0.98, 'يمنع فقدان الماء بنسبة 99%': 0.95, 'حماية فائقة للبشرة': 0.92 },
        synergy: {},
        negativeSynergy: {},
        warnings: [
          { level: 'caution', text: 'يعتبر المعيار الذهبي لحماية حاجز البشرة (slugging)، ولكنه ثقيل جدا وقد يكون كوميدوجينيك للبشرة المعرضة لحب الشباب.' }
        ],
      },
{
        id: 'phthalates',
        name: 'الفثالات',
        scientific_name: 'Phthalates',
        searchKeywords: ['phthalates', 'dbp', 'dep', 'dehp', 'فثالات'],
        usageScope: ['skin', 'hair'],
        functionalCategory: 'مكون ضار',
        chemicalType: 'مذيب (ملدن)',
        benefits: { 'تثبيت الرائحة': 0.8, 'جعل البلاستيك مرنا': 0.9 },
        synergy: {},
        negativeSynergy: {},
        warnings: [
          { level: 'risk', text: 'مجموعة من المواد الكيميائية المرتبطة باضطرابات هرمونية ومشاكل في النمو. غالبا ما تكون مخفية تحت كلمة "Parfum". محظورة في مستحضرات التجميل في الاتحاد الأوروبي.' }
        ],
      },
{
        id: 'polyethylene-glycol',
        name: 'بولي إيثيلين جلايكول (PEG)',
        scientific_name: 'Polyethylene Glycol',
        searchKeywords: ['polyethylene glycol', 'peg'],
        usageScope: ['skin', 'hair'],
        functionalCategory: 'محسن قوام',
        chemicalType: 'بوليمر',
        benefits: { 'تحسين امتصاص المكونات': 0.8, 'مستحلب': 0.75, 'مرطب': 0.7 },
        synergy: {},
        negativeSynergy: {},
        warnings: [
          { level: 'caution', text: 'هناك مخاوف نظرية بشأن التلوث بـ 1,4-dioxane أثناء التصنيع، ولكن الهيئات التنظيمية تضع حدودا صارمة لضمان الأمان.' },
          { level: 'risk', text: 'لا ينصح باستخدامه على البشرة المتضررة أو المجروحة.' }
        ],
      },
{
        id: 'cocamide-dea',
        name: 'كوكاميد DEA',
        scientific_name: 'Cocamide DEA',
        searchKeywords: ['cocamide dea', 'كوكاميد DEA', 'كوكاميد دأأ'],
        usageScope: ['hair'],
        functionalCategory: 'محسن قوام',
        chemicalType: 'أمين دهني',
        benefits: { 'زيادة كثافة الرغوة': 0.88, 'مكثف للقوام': 0.9 },
        synergy: {},
        negativeSynergy: {},
        warnings: [
          { level: 'risk', text: 'مكون مثير للجدل. قد يتفاعل مع مكونات أخرى لتكوين النتروزامينات المسرطنة. تم تقييده في بعض المناطق.' }
        ],
      },
{
        id: '2-bromo-2-nitropropane-1-3-diol',
        name: '2-برومو-2-نيتروبروبان-1،3-ديول (برونوبول)',
        scientific_name: '2-Bromo-2-Nitropropane-1,3-Diol',
        searchKeywords: ['2-bromo-2-nitropropane-1-3-diol', 'bronopol', 'برونوبول'],
        usageScope: ['skin', 'hair'],
        functionalCategory: 'مادة حافظة',
        chemicalType: 'مطلق للفورمالديهايد',
        benefits: { 'حفظ المنتج': 0.85 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'risk', text: 'مادة حافظة تطلق الفورمالديهايد ويمكن أن تتفاعل لتكوين النتروزامينات المسرطنة.' }],
      },
{
        id: 'toluene',
        name: 'تولوين',
        scientific_name: 'Toluene',
        searchKeywords: ['toluene', 'تولوين'],
        usageScope: ['hair'],
        functionalCategory: 'مكون ضار',
        chemicalType: 'مذيب',
        benefits: {},
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'risk', text: 'مذيب سام للأعصاب والجهاز التنفسي. ممنوع في العديد من المناطق، ولكن قد يوجد في طلاء الأظافر وصبغات الشعر.' }],
      },
{
        id: 'resorcinol',
        name: 'ريسورسينول',
        scientific_name: 'Resorcinol',
        searchKeywords: ['resorcinol', 'ريسورسينول'],
        usageScope: ['hair', 'skin'],
        functionalCategory: 'مكون ضار',
        chemicalType: 'مكون فعال (صبغة)',
        benefits: { 'تثبيت الصبغة': 0.8, 'علاج حب الشباب': 0.7 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'risk', text: 'مكون فعال في بعض علاجات حب الشباب وصبغات الشعر، ولكنه مهيج قوي للجلد ومثير للقلق بشأن اضطراب الغدة الدرقية. مقيد بشدة.' }],
      },
{
        id: 'coal-tar',
        name: 'قطران الفحم',
        scientific_name: 'Coal Tar',
        searchKeywords: ['coal tar', 'قطران الفحم'],
        usageScope: ['hair'],
        functionalCategory: 'مكون ضار',
        chemicalType: 'مكون فعال (علاجي)',
        benefits: { 'علاج القشرة': 0.9, 'علاج الصدفية': 0.9},
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'risk', text: 'مادة مسرطنة معروفة، ولكن لا يزال مسموحا بها بتركيزات منخفضة في الشامبوهات العلاجية التي لا تستلزم وصفة طبية لعلاج الصدفية والتهاب الجلد الدهني.' }],
      },
{
        id: 'lead-acetate',
        name: 'خلات الرصاص',
        scientific_name: 'Lead Acetate',
        searchKeywords: ['lead acetate', 'خلات الرصاص'],
        usageScope: ['hair'],
        functionalCategory: 'مكون ضار',
        chemicalType: 'ملون معدني',
        benefits: {},
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'risk', text: 'سم عصبي يتراكم في الجسم. محظور في معظم أنحاء العالم، ولكن لا يزال موجودا في بعض صبغات الشعر "التدريجية" القديمة في الولايات المتحدة.' }],
      },
{
        id: 'methanol',
        name: 'الميثانول',
        scientific_name: 'Methanol',
        searchKeywords: ['methanol', 'methyl alcohol', 'ميثانول', 'كحول ميثيلي'],
        usageScope: ['skin'],
        functionalCategory: 'مكون ضار',
        chemicalType: 'مذيب',
        benefits: {},
        synergy: {},
        negativeSynergy: {},
        warnings: [
          { level: 'risk', text: 'مادة سامة جدا عند الامتصاص عبر الجلد أو الاستنشاق. قد يسبب العمى أو الوفاة. يوجد أحيانا في المنتجات المقلدة أو غير المرخصة.' }
        ],
      },
{
        id: 'azeloyl-diglycinate',
        name: 'أزويل ديجليسينات',
        scientific_name: 'Potassium Azeloyl Diglycinate',
        searchKeywords: ['azeloyl diglycinate', 'potassium azeloyl diglycinate', 'pad'],
        usageScope: ['skin'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'مشتق حمض',
        benefits: { 'تفتيح وتوحيد اللون': 0.9, 'تنظيم الإفرازات الدهنية': 0.9, 'ترطيب': 0.85, 'ألطف من الأزيليك': 0.95 },
        synergy: { 'niacinamide': 1.1 },
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'مشتق من حمض الأزيليك، يوفر فوائد مشابهة (تفتيح، تنظيم الدهون) مع تهيج أقل بكثير وقوام مائي أنيق.' }],
      },
{
        id: 'bis-peg-18-methyl-ether-dimethyl-silane',
        name: 'بس-PEG-18 ميثيل إيثر ثنائي ميثيل سيلان',
        scientific_name: 'Bis-PEG-18 Methyl Ether Dimethyl Silane',
        searchKeywords: ['bis-peg-18 methyl ether dimethyl silane'],
        usageScope: ['skin'],
        functionalCategory: 'مرطب / مطري',
        chemicalType: 'سيليكون قابل للذوبان في الماء',
        benefits: { 'يعطي ملمسا حريريا': 0.9, 'مرطب خفيف': 0.8, 'انزلاق ممتاز': 0.88 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'سيليكون قابل للذوبان في الماء، لا يتراكم ويعطي إحساسا بالنعومة والترطيب.' }],
      },
{
        id: 'vp-eicosene-copolymer',
        name: 'VP/إيكوسين كوبوليمر',
        scientific_name: 'VP/Eicosene Copolymer',
        searchKeywords: ['vp/eicosene copolymer'],
        usageScope: ['skin'],
        functionalCategory: 'محسن قوام',
        chemicalType: 'مكون طبقة رقيقة',
        benefits: { 'مقاومة للماء': 0.9, 'تكوين طبقة رقيقة': 0.88, 'يعزز حماية واقي الشمس': 0.85 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'يستخدم في واقيات الشمس لجعلها مقاومة للماء ولتحسين توزيع فلاتر الشمس على الجلد.' }],
      },
{
        id: 'polymethyl-methacrylate',
        name: 'بولي ميثيل ميثاكريلات (PMMA)',
        scientific_name: 'Polymethyl Methacrylate',
        searchKeywords: ['polymethyl methacrylate', 'pmma'],
        usageScope: ['skin'],
        functionalCategory: 'محسن قوام',
        chemicalType: 'مسحوق',
        benefits: { 'يعطي ملمسا ناعما': 0.9, 'تأثير بصري لتمويه التجاعيد': 0.85, 'مادة حشو': 0.8 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'corn-starch-modified',
        name: 'نشا الذرة المعدل',
        scientific_name: 'Zea Mays (Corn) Starch / Corn Starch Modified',
        searchKeywords: ['corn starch modified', 'zea mays starch'],
        usageScope: ['skin'],
        functionalCategory: 'محسن قوام',
        chemicalType: 'ممتص للدهون',
        benefits: { 'يمتص الزيوت': 0.88, 'يعطي ملمسا مطفيا': 0.85 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'calcium-gluconate',
        name: 'جلوكونات الكالسيوم',
        scientific_name: 'Calcium Gluconate',
        searchKeywords: ['calcium gluconate'],
        usageScope: ['skin'],
        functionalCategory: 'مرطب / مطري',
        chemicalType: 'مرطب',
        benefits: { 'مرطب': 0.85, 'ملطف ومرمم': 0.8 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'sodium-hydroxymethylglycinate',
        name: 'هيدروكسي ميثيل جليسينات الصوديوم',
        scientific_name: 'Sodium Hydroxymethylglycinate',
        searchKeywords: ['sodium hydroxymethylglycinate'],
        usageScope: ['skin'],
        functionalCategory: 'مادة حافظة',
        chemicalType: 'مطلق للفورمالديهايد',
        benefits: { 'حفظ المنتج': 0.8 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'risk', text: 'مادة حافظة تطلق الفورمالديهايد. تعتبر مهيجة للجلد ومسببة للحساسية.' }],
      },
{
        id: 'lanolin-wax',
        name: 'شمع اللانولين',
        scientific_name: 'Lanolin Cera',
        searchKeywords: ['lanolin wax', 'lanolin cera'],
        usageScope: ['skin'],
        functionalCategory: 'عازل',
        chemicalType: 'شمع حيواني',
        benefits: { 'عازل': 0.9, 'مستحلب': 0.8 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'risk', text: 'قد يسبب حساسية لدى الأشخاص الذين يعانون من حساسية الصوف.' }],
      },
{
        id: 'oleyl-alcohol',
        name: 'كحول الأولييل',
        scientific_name: 'Oleyl Alcohol',
        searchKeywords: ['oleyl alcohol'],
        usageScope: ['skin'],
        functionalCategory: 'مرطب / مطري',
        chemicalType: 'كحول دهني',
        benefits: { 'مطري': 0.85, 'مستحلب': 0.8 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'isocetyl-alcohol',
        name: 'كحول الأيزوسيتيل',
        scientific_name: 'Isocetyl Alcohol',
        searchKeywords: ['isocetyl alcohol'],
        usageScope: ['skin'],
        functionalCategory: 'مرطب / مطري',
        chemicalType: 'كحول دهني',
        benefits: { 'مطري': 0.8, 'مذيب': 0.75 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'risk', text: 'يعتبر من المواد شديدة السد للمسام (كوميدوجينيك).' }],
      },
{
        id: 'glyceryl-laurate',
        name: 'لورات الجليسريل',
        scientific_name: 'Glyceryl Laurate',
        searchKeywords: ['glyceryl laurate', 'monolaurin'],
        usageScope: ['skin'],
        functionalCategory: 'محسن قوام',
        chemicalType: 'مستحلب',
        benefits: { 'مستحلب': 0.8, 'مضاد للميكروبات': 0.85, 'مكافحة حب الشباب': 0.82 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'sodium-levulinate',
        name: 'ليفولينات الصوديوم',
        scientific_name: 'Sodium Levulinate',
        searchKeywords: ['sodium levulinate'],
        usageScope: ['skin'],
        functionalCategory: 'مادة حافظة',
        chemicalType: 'مادة حافظة طبيعية',
        benefits: { 'حفظ المنتج': 0.88, 'تكييف البشرة': 0.8 },
        synergy: { 'sodium-anisate': 1.05 },
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'sodium-anisate',
        name: 'أنيسات الصوديوم',
        scientific_name: 'Sodium Anisate',
        searchKeywords: ['sodium anisate'],
        usageScope: ['skin'],
        functionalCategory: 'مادة حافظة',
        chemicalType: 'مادة حافظة طبيعية',
        benefits: { 'حفظ المنتج': 0.88, 'مضاد للميكروبات': 0.85 },
        synergy: { 'sodium-levulinate': 1.05 },
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'glyceryl-caprylate',
        name: 'كابريلات الجليسريل',
        scientific_name: 'Glyceryl Caprylate',
        searchKeywords: ['glyceryl caprylate'],
        usageScope: ['skin'],
        functionalCategory: 'محسن قوام',
        chemicalType: 'معزز للمواد الحافظة',
        benefits: { 'معزز للمواد الحافظة': 0.9, 'مطري ومنعم': 0.85, 'مضاد للميكروبات': 0.88 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'potassium-palmitoyl-hydrolyzed-wheat-protein',
        name: 'بروتين القمح المهدرج بالميتويل البوتاسيوم',
        scientific_name: 'Potassium Palmitoyl Hydrolyzed Wheat Protein',
        searchKeywords: ['potassium palmitoyl hydrolyzed wheat protein'],
        usageScope: ['skin'],
        functionalCategory: 'منظف',
        chemicalType: 'منظف لطيف',
        benefits: { 'منظف لطيف جدا': 0.9, 'مستحلب': 0.85, 'يعطي ملمسا ناعما': 0.8 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'risk', text: 'يجب تجنبه من قبل الأشخاص الذين يعانون من حساسية القمح أو الغلوتين.' }],
      },
{
        id: 'calcium-pca',
        name: 'كالسيوم PCA',
        scientific_name: 'Calcium PCA',
        searchKeywords: ['calcium pca'],
        usageScope: ['skin'],
        functionalCategory: 'مرطب / مطري',
        chemicalType: 'مرطب',
        benefits: { 'يقوي حاجز البشرة': 0.88, 'ترطيب': 0.85 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'magnesium-pca',
        name: 'مغنيسيوم PCA',
        scientific_name: 'Magnesium PCA',
        searchKeywords: ['magnesium pca'],
        usageScope: ['skin'],
        functionalCategory: 'مرطب / مطري',
        chemicalType: 'مرطب',
        benefits: { 'تنشيط طاقة الخلايا': 0.85, 'ترطيب': 0.8 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'manganese-pca',
        name: 'منغنيز PCA',
        scientific_name: 'Manganese PCA',
        searchKeywords: ['manganese pca'],
        usageScope: ['skin'],
        functionalCategory: 'مرطب / مطري',
        chemicalType: 'مرطب',
        benefits: { 'مضاد أكسدة': 0.8, 'ترطيب': 0.75 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'lauryl-laurate',
        name: 'لورات اللوريل',
        scientific_name: 'Lauryl Laurate',
        searchKeywords: ['lauryl laurate'],
        usageScope: ['skin'],
        functionalCategory: 'مرطب / مطري',
        chemicalType: 'مطري شمعي',
        benefits: { 'مطري شمعي يذوب عند ملامسة البشرة': 0.9, 'يعطي ملمسا أنيقا': 0.85 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'hexyldecanol',
        name: 'هيكسيلديكانول',
        scientific_name: 'Hexyldecanol',
        searchKeywords: ['hexyldecanol'],
        usageScope: ['skin'],
        functionalCategory: 'مرطب / مطري',
        chemicalType: 'مطري',
        benefits: { 'مطري ومنعم': 0.9, 'مذيب للمكونات الأخرى': 0.85 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'hexyldecyl-laurate',
        name: 'لورات هيكسيلديسيل',
        scientific_name: 'Hexyldecyl Laurate',
        searchKeywords: ['hexyldecyl laurate'],
        usageScope: ['skin'],
        functionalCategory: 'مرطب / مطري',
        chemicalType: 'مطري',
        benefits: { 'مطري خفيف وغير دهني': 0.88, 'يعطي ملمسا ناعما': 0.85 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'isostearyl-neopentanoate',
        name: 'نيوبنتانوات الأيزوستيريل',
        scientific_name: 'Isostearyl Neopentanoate',
        searchKeywords: ['isostearyl neopentanoate'],
        usageScope: ['skin'],
        functionalCategory: 'مرطب / مطري',
        chemicalType: 'مطري',
        benefits: { 'مطري خفيف جدا وغير دهني': 0.9, 'يعطي ملمسا مخمليا': 0.88 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'propanediol-dicaprylate',
        name: 'ثنائي كابريلات البروبانيديول',
        scientific_name: 'Propanediol Dicaprylate',
        searchKeywords: ['propanediol dicaprylate'],
        usageScope: ['skin'],
        functionalCategory: 'مرطب / مطري',
        chemicalType: 'مطري',
        benefits: { 'مطري خفيف جدا من أصل نباتي': 0.9, 'بديل للسيليكون': 0.88 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'sodium-polyaspartate',
        name: 'بولي أسبارتات الصوديوم',
        scientific_name: 'Sodium Polyaspartate',
        searchKeywords: ['sodium polyaspartate'],
        usageScope: ['skin'],
        functionalCategory: 'مرطب / مطري',
        chemicalType: 'مرطب',
        benefits: { 'ترطيب': 0.8, 'تكوين طبقة رقيقة': 0.75 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'p-anisic-acid',
        name: 'حمض الأنيسيك-p',
        scientific_name: 'p-Anisic Acid',
        searchKeywords: ['p-anisic acid'],
        usageScope: ['skin'],
        functionalCategory: 'مادة حافظة',
        chemicalType: 'مادة حافظة طبيعية',
        benefits: { 'حفظ المنتج': 0.8, 'مضاد للفطريات': 0.85 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'behenoxy-dimethicone',
        name: 'بيهينوكسي دايميثيكون',
        scientific_name: 'Behenoxy Dimethicone',
        searchKeywords: ['behenoxy dimethicone'],
        usageScope: ['skin'],
        functionalCategory: 'محسن قوام',
        chemicalType: 'سيليكون',
        benefits: { 'مكثف شمعي': 0.85, 'مطري': 0.8 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'steareth-10',
        name: 'ستيريث-10',
        scientific_name: 'Steareth-10',
        searchKeywords: ['steareth-10', 'ستيريث-10', 'ستيارث 10', 'ستيارث-10', 'ستيريث 10', 'polyoxyethylene stearate', 'polyoxyethylene (10) stearate', 'ستياريث-10' ],
        usageScope: ['skin'],
        functionalCategory: 'محسن قوام',
        chemicalType: 'مستحلب',
        benefits: { 'مستحلب': 0.8, 'مكثف': 0.75 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'sucrose-cocoate',
        name: 'كوكوات السكروز',
        scientific_name: 'Sucrose Cocoate',
        searchKeywords: ['sucrose cocoate'],
        usageScope: ['skin'],
        functionalCategory: 'محسن قوام',
        chemicalType: 'مستحلب',
        benefits: { 'مستحلب لطيف': 0.9, 'إعادة ترطيب البشرة': 0.85 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'مستحلب طبيعي مشتق من السكر وزيت جوز الهند.' }],
      },
{
        id: 'phytosphingosine',
        name: 'فيتوسفينجوزين',
        scientific_name: 'Phytosphingosine',
        searchKeywords: ['phytosphingosine', 'فيتوسفينجوزين'],
        usageScope: ['skin'],
        functionalCategory: 'دعم حاجز البشرة',
        chemicalType: 'دهون',
        benefits: { 'مضاد للميكروبات': 0.9, 'مضاد للالتهابات': 0.88, 'لبنة بناء للسيراميدات': 0.92 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'دهون طبيعية في البشرة، تساعد على تنظيم الميكروبيوم وتهدئة الالتهابات.' }],
      },
{
        id: 'allantoin',
        name: 'آلانتوين',
        scientific_name: 'Allantoin',
        searchKeywords: ['allantoin', 'آلانتوين'],
        usageScope: ['skin'],
        functionalCategory: 'ملطف ومرمم',
        chemicalType: 'مركب نشط',
        benefits: { 'ملطف ومرمم': 0.95, 'تسريع التئام الجروح': 0.88, 'تقشير لطيف (حال للطبقة القرنية)': 0.8 },
        synergy: { 'panthenol': 1.08 },
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'beta-glucan',
        name: 'بيتا جلوكان',
        scientific_name: 'Beta-Glucan',
        searchKeywords: ['beta-glucan', 'beta glucan', 'بيتا جلوكان'],
        usageScope: ['skin'],
        functionalCategory: 'ملطف ومرمم',
        chemicalType: 'متعدد السكاريد',
        benefits: { 'ملطف ومرمم': 0.92, 'ترطيب عميق': 0.9, 'دعم حاجز البشرة': 0.88, 'مضاد أكسدة': 0.8 },
        synergy: { 'centella-asiatica': 1.1 },
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'متعدد السكاريد مشتق من الشوفان أو الفطر، معروف بقدرته الفائقة على تهدئة البشرة وترطيبها.' }],
      },
{
        id: 'madecassoside',
        name: 'ماديكاسوسيد',
        scientific_name: 'Madecassoside',
        searchKeywords: ['madecassoside', 'cica active', 'ماديكاسوسيد'],
        usageScope: ['skin'],
        functionalCategory: 'ملطف ومرمم',
        chemicalType: 'مركب نشط نباتي',
        benefits: { 'ملطف ومرمم': 0.95, 'مضاد للالتهابات': 0.9, 'تحفيز الكولاجين': 0.85, 'تسريع التئام الجروح': 0.92 },
        synergy: { 'panthenol': 1.05 },
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'أحد المركبات النشطة الرئيسية في عشبة السيكا (Centella Asiatica)، فعال جدا في إصلاح البشرة وتهدئة الاحمرار.' }],
      },
{
        id: 'asiaticoside',
        name: 'آسياتيكوسيد',
        scientific_name: 'Asiaticoside',
        searchKeywords: ['asiaticoside', 'cica active', 'آسياتيكوسيد'],
        usageScope: ['skin'],
        functionalCategory: 'ملطف ومرمم',
        chemicalType: 'مركب نشط نباتي',
        benefits: { 'تحفيز الكولاجين': 0.9, 'تسريع التئام الجروح': 0.92, 'مضاد أكسدة': 0.8 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'أحد المركبات النشطة الرئيسية في عشبة السيكا (Centella Asiatica).' }],
      },
{
        id: 'tranexamic-acid',
        name: 'حمض الترانيكساميك',
        scientific_name: 'Tranexamic Acid',
        searchKeywords: ['tranexamic acid', 'حمض الترانيكساميك'],
        usageScope: ['skin'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'حمض أميني صناعي',
        benefits: { 'تفتيح وتوحيد اللون': 0.92, 'علاج الكلف والبقع الداكنة': 0.9, 'تقليل الاحمرار': 0.85 },
        synergy: { 'niacinamide': 1.1, 'alpha-arbutin-serum': 1.08 },
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'فعال جدا في علاج التصبغات الناتجة عن الالتهابات والشمس.' }],
      },
{
        id: 'kojic-acid',
        name: 'حمض الكوجيك',
        scientific_name: 'Kojic Acid',
        searchKeywords: ['kojic acid', 'حمض الكوجيك'],
        usageScope: ['skin'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'مثبط للتصبغ',
        benefits: { 'تفتيح وتوحيد اللون': 0.88, 'علاج البقع الداكنة': 0.85 },
        synergy: { 'glycolic-acid': 1.05 },
        negativeSynergy: {},
        warnings: [{ level: 'caution', text: 'غير مستقر ويميل للتأكسد وتغيير لونه. قد يسبب حساسية لدى البعض. مشتقاته (Kojic Dipalmitate) أكثر استقرارا.' }],
      },
{
        id: 'bakuchiol',
        name: 'باكوتشيول',
        scientific_name: 'Bakuchiol',
        searchKeywords: ['bakuchiol', 'retinol alternative', 'باكوتشيول'],
        usageScope: ['skin'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'مركب نباتي',
        benefits: { 'بديل طبيعي للريتينول': 0.9, 'مكافحة الشيخوخة': 0.88, 'مكافحة حب الشباب': 0.8, 'لطيف على البشرة': 0.95 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'مستخلص نباتي أظهرت الدراسات أن له تأثيرات مشابهة للريتينول (في تقليل التجاعيد والتصبغات) ولكن مع تهيج أقل بكثير.' }],
      },
{
        id: 'superoxide-dismutase',
        name: 'سوبر أكسيد ديسميوتاز (SOD)',
        scientific_name: 'Superoxide Dismutase',
        searchKeywords: ['superoxide dismutase', 'sod', 'سوبر أكسيد ديسميوتاز'],
        usageScope: ['skin'],
        functionalCategory: 'مضاد أكسدة',
        chemicalType: 'إنزيم',
        benefits: { 'مضاد أكسدة إنزيمي قوي': 0.95, 'حماية من الجذور الحرة': 0.92, 'مضاد للالتهابات': 0.85 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'إنزيم مضاد للأكسدة موجود بشكل طبيعي في الجسم، وهو خط الدفاع الأول ضد الجذور الحرة الأكثر ضررا.' }],
      },
{
        id: 'glutathione',
        name: 'الجلوتاثيون',
        scientific_name: 'Glutathione',
        searchKeywords: ['glutathione', 'gsh', 'جلوتاثيون'],
        usageScope: ['skin'],
        functionalCategory: 'مضاد أكسدة',
        chemicalType: 'ثلاثي الببتيد',
        benefits: { 'مضاد أكسدة رئيسي في الجسم': 0.95, 'تفتيح وتوحيد اللون': 0.85, 'إزالة السموم': 0.9 },
        synergy: { 'vitamin-c-serum': 1.1 },
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'يعمل على تفتيح البشرة عن طريق تحويل إنتاج الميلانين الداكن إلى ميلانين أفتح.' }],
      },
{
        id: 'ergothioneine',
        name: 'إرغوثيونين',
        scientific_name: 'Ergothioneine',
        searchKeywords: ['ergothioneine', 'egh', 'mushroom antioxidant', 'إرغوثيونين'],
        usageScope: ['skin'],
        functionalCategory: 'مضاد أكسدة',
        chemicalType: 'حمض أميني',
        benefits: { 'مضاد أكسدة ذكي': 0.92, 'حماية من أضرار الشمس والتلوث': 0.9, 'مضاد للالتهابات': 0.85 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'مضاد أكسدة قوي يوجد بكثرة في الفطر، له نظام نقل خاص به في خلايا الجلد، مما يشير إلى أهميته البيولوجية.' }],
      },
{
        id: 'alpha-glucan-oligosaccharide',
        name: 'ألفا-جلوكان أوليجوسكاريد',
        scientific_name: 'Alpha-Glucan Oligosaccharide',
        searchKeywords: ['alpha-glucan oligosaccharide', 'prebiotic', 'bioecolia', 'بريبيوتيك'],
        usageScope: ['skin'],
        functionalCategory: 'دعم حاجز البشرة',
        chemicalType: 'بريبيوتيك',
        benefits: { 'دعم ميكروبيوم البشرة': 0.95, 'تقوية دفاعات البشرة': 0.9, 'تقليل التهيج': 0.85 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'بريبيوتيك يغذي البكتيريا المفيدة على سطح الجلد ويساعد على الحفاظ على توازن صحي للميكروبيوم.' }],
      },
{
        id: 'inulin',
        name: 'إينولين',
        scientific_name: 'Inulin',
        searchKeywords: ['inulin', 'prebiotic', 'إينولين'],
        usageScope: ['skin'],
        functionalCategory: 'دعم حاجز البشرة',
        chemicalType: 'بريبيوتيك',
        benefits: { 'دعم ميكروبيوم البشرة': 0.9, 'ترطيب': 0.8 },
        synergy: { 'fructose': 1.05 },
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'بريبيوتيك مشتق من جذور الهندباء، يدعم صحة ميكروبيوم الجلد.' }],
      },
{
        id: 'sodium-ascorbyl-phosphate',
        name: 'فوسفات أسكوربيل الصوديوم (SAP)',
        scientific_name: 'Sodium Ascorbyl Phosphate',
        searchKeywords: ['sodium ascorbyl phosphate', 'sap', 'فيتامين سي للبشرة الدهنية'],
        usageScope: ['skin'],
        functionalCategory: 'مضاد أكسدة',
        chemicalType: 'مشتق فيتامين',
        benefits: { 'مضاد أكسدة': 0.88, 'مكافحة حب الشباب': 0.85, 'تفتيح وتوحيد اللون': 0.8 },
        synergy: { 'niacinamide': 1.1 },
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'مشتق فيتامين C لطيف ومستقر، فعال بشكل خاص للبشرة المعرضة لحب الشباب لخصائصه المضادة للميكروبات.' }],
      },
{
        id: 'magnesium-ascorbyl-phosphate',
        name: 'فوسفات أسكوربيل المغنيسيوم (MAP)',
        scientific_name: 'Magnesium Ascorbyl Phosphate',
        searchKeywords: ['magnesium ascorbyl phosphate', 'map', 'فيتامين سي للبشرة الحساسة'],
        usageScope: ['skin'],
        functionalCategory: 'مضاد أكسدة',
        chemicalType: 'مشتق فيتامين',
        benefits: { 'مضاد أكسدة': 0.9, 'تفتيح وتوحيد اللون': 0.85, 'تحفيز الكولاجين': 0.8, 'مرطب': 0.82 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'أحد ألطف مشتقات فيتامين C وأكثرها ترطيبا، مثالي للبشرة الجافة والحساسة.' }],
      },
{
        id: 'tetrahexyldecyl-ascorbate',
        name: 'رباعي هيكسيل ديسيل أسكوربات (THDA)',
        scientific_name: 'Tetrahexyldecyl Ascorbate',
        searchKeywords: ['tetrahexyldecyl ascorbate', 'thda', 'vitamin c ester', 'ascorbyl tetraisopalmitate'],
        usageScope: ['skin'],
        functionalCategory: 'مضاد أكسدة',
        chemicalType: 'مشتق فيتامين',
        benefits: { 'فيتامين سي قابل للذوبان في الزيت': 0.95, 'مضاد أكسدة': 0.92, 'تحفيز الكولاجين': 0.9, 'تفتيح وتوحيد اللون': 0.88 },
        synergy: { 'tocopherol': 1.1 },
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'مشتق فيتامين C فاخر قابل للذوبان في الزيت، يخترق الجلد بعمق. مستقر جدا ولكنه مكلف.' }],
      },
{
        id: 'ethyl-ascorbic-acid',
        name: 'حمض الأسكوربيك الإيثيلي (3-O-Ethyl Ascorbic Acid)',
        scientific_name: '3-O-Ethyl Ascorbic Acid',
        searchKeywords: ['3-o-ethyl ascorbic acid', 'ethyl ascorbic acid', 'فيتامين سي إيثيلي', 'حمض الأسكوربيك الإيثيلي'],
        usageScope: ['skin'],
        functionalCategory: 'مضاد أكسدة',
        chemicalType: 'مشتق فيتامين',
        benefits: { 'مستقر وفعال جدا': 0.95, 'مضاد أكسدة': 0.92, 'تفتيح وتوحيد اللون': 0.9, 'تحفيز الكولاجين': 0.88 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'مشتق فيتامين C حديث ومستقر للغاية، يعتبر الأقرب في فعاليته لحمض الأسكوربيك النقي ولكن بتهيج أقل بكثير.' }],
      },
{
        id: 'ascorbyl-palmitate',
        name: 'بالميتات الأسكوربيل',
        scientific_name: 'Ascorbyl Palmitate',
        searchKeywords: ['ascorbyl palmitate', 'vitamin c ester'],
        usageScope: ['skin'],
        functionalCategory: 'مضاد أكسدة',
        chemicalType: 'مشتق فيتامين',
        benefits: { 'مضاد أكسدة (في المنتج)': 0.8, 'تفتيح طفيف': 0.6 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'caution', text: 'مشتق فيتامين C قابل للذوبان في الزيت، ولكنه ضعيف جدا مقارنة بالمشتقات الأخرى ويعمل بشكل أساسي كمضاد أكسدة لحماية التركيبة نفسها.' }],
      },
{
        id: 'ascorbyl-tetraisopalmitate',
        name: 'رباعي أيزوبالميتات الأسكوربيل',
        scientific_name: 'Ascorbyl Tetraisopalmitate',
        searchKeywords: ['ascorbyl tetraisopalmitate', 'vc-ip'],
        usageScope: ['skin'],
        functionalCategory: 'مضاد أكسدة',
        chemicalType: 'مشتق فيتامين',
        benefits: { 'فيتامين سي قابل للذوبان في الزيت': 0.95, 'مستقر جدا': 0.92, 'تفتيح وتوحيد اللون': 0.88 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'اسم آخر لـ Tetrahexyldecyl Ascorbate، شائع جدا في مستحضرات التجميل الآسيوية.' }],
      },
{
        id: 'retinyl-palmitate',
        name: 'بالميتات الريتينيل',
        scientific_name: 'Retinyl Palmitate',
        searchKeywords: ['retinyl palmitate', 'vitamin a palmitate', 'بالميتات الريتينيل'],
        usageScope: ['skin'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'ريتينويد',
        benefits: { 'مضاد أكسدة': 0.8, 'مكافحة الشيخوخة (خفيف)': 0.7 },
        synergy: {},
        negativeSynergy: {},
        warnings: [
          { level: 'info', text: 'أضعف أشكال الريتينويد وأكثرها لطفا. يتطلب خطوتين للتحول إلى حمض الريتينويك في الجلد.' },
          { level: 'caution', text: 'هناك جدل حول سلامته في واقيات الشمس، حيث تشير بعض الدراسات إلى أنه قد يزيد من ضرر الجذور الحرة عند تعرضه للشمس.' }
        ],
      },
{
        id: 'retinyl-retinoate',
        name: 'ريتينوات الريتينيل',
        scientific_name: 'Retinyl Retinoate',
        searchKeywords: ['retinyl retinoate'],
        usageScope: ['skin'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'ريتينويد',
        benefits: { 'مكافحة الشيخوخة': 0.92, 'لطيف على البشرة': 0.9, 'مستقر ضوئيا': 0.85 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'ريتينويد هجين يجمع بين الريتينول وحمض الريتينويك، وهو فعال وقوي ولكن ألطف بكثير من حمض الريتينويك النقي.' }],
      },
{
        id: 'hydroxypinacolone-retinoate',
        name: 'هيدروكسي بيناكولون ريتينوات (Granactive Retinoid)',
        scientific_name: 'Hydroxypinacolone Retinoate',
        searchKeywords: ['hydroxypinacolone retinoate', 'hpr', 'granactive retinoid', 'جراناكتيف ريتينويد'],
        usageScope: ['skin'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'ريتينويد',
        benefits: { 'فعالية عالية مع تهيج منخفض': 0.95, 'مكافحة الشيخوخة': 0.92, 'لا يحتاج للتحول': 0.9 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'استر من حمض الريتينويك، يعمل مباشرة على المستقبلات في الجلد دون الحاجة للتحول، مما يوفر فعالية عالية مع تهيج منخفض جدا.' }],
      },
{
        id: 'mandelic-acid',
        name: 'حمض المندليك',
        scientific_name: 'Mandelic Acid',
        searchKeywords: ['mandelic acid', 'aha', 'حمض المندليك'],
        usageScope: ['skin'],
        functionalCategory: 'مقشر',
        chemicalType: 'حمض ألفا هيدروكسي (AHA)',
        benefits: { 'تقشير لطيف': 0.9, 'مكافحة حب الشباب': 0.85, 'تفتيح وتوحيد اللون': 0.8, 'مناسب للبشرة الحساسة': 0.92 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'حمض ألفا هيدروكسي ذو جزيئات كبيرة، يخترق الجلد ببطء مما يجعله ألطف بكثير من حمض الجليكوليك ومناسب للبشرة الحساسة والمعرضة لحب الشباب.' }],
      },
{
        id: 'tartaric-acid',
        name: 'حمض الطرطريك',
        scientific_name: 'Tartaric Acid',
        searchKeywords: ['tartaric acid', 'aha', 'حمض الطرطريك'],
        usageScope: ['skin'],
        functionalCategory: 'محسن قوام',
        chemicalType: 'حمض ألفا هيدروكسي (AHA)',
        benefits: { 'تنظيم درجة الحموضة': 0.9, 'مضاد أكسدة': 0.7, 'تقشير خفيف': 0.65 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'يستخدم بشكل أساسي للمساعدة في الحفاظ على درجة الحموضة المناسبة في تركيبات الأحماض الأخرى.' }],
      },
{
        id: 'malic-acid',
        name: 'حمض الماليك',
        scientific_name: 'Malic Acid',
        searchKeywords: ['malic acid', 'aha', 'حمض الماليك'],
        usageScope: ['skin'],
        functionalCategory: 'مقشر',
        chemicalType: 'حمض ألفا هيدروكسي (AHA)',
        benefits: { 'تقشير لطيف': 0.8, 'ترطيب': 0.75 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'حمض ألفا هيدروكسي ذو جزيئات أكبر من الجليكوليك، يوفر تقشيرا ألطف مع فوائد ترطيبية.' }],
      },
{
        id: 'lactobionic-acid',
        name: 'حمض اللاكتوبيونيك',
        scientific_name: 'Lactobionic Acid',
        searchKeywords: ['lactobionic acid', 'pha', 'حمض اللاكتوبيونيك'],
        usageScope: ['skin'],
        functionalCategory: 'مقشر',
        chemicalType: 'حمض بولي هيدروكسي (PHA)',
        benefits: { 'تقشير لطيف جدا': 0.9, 'ترطيب فائق': 0.92, 'مضاد أكسدة': 0.88 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'حمض PHA من الجيل الثاني، مقشر لطيف جدا يعمل كمرطب قوي ومضاد للأكسدة.' }],
      },
{
        id: 'gluconolactone',
        name: 'جلوكونولاكتون',
        scientific_name: 'Gluconolactone',
        searchKeywords: ['gluconolactone', 'pha', 'جلوكونولاكتون'],
        usageScope: ['skin'],
        functionalCategory: 'مقشر',
        chemicalType: 'حمض بولي هيدروكسي (PHA)',
        benefits: { 'تقشير لطيف جدا': 0.92, 'ترطيب': 0.9, 'دعم حاجز البشرة': 0.85 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'أشهر أحماض PHA، يقشر بلطف شديد ويرطب ويقوي حاجز البشرة. مناسب للبشرة الحساسة.' }],
      },
{
        id: 'ferulic-acid',
        name: 'حمض الفيروليك',
        scientific_name: 'Ferulic Acid',
        searchKeywords: ['ferulic acid', 'حمض الفيروليك'],
        usageScope: ['skin'],
        functionalCategory: 'مضاد أكسدة',
        chemicalType: 'مركب نباتي',
        benefits: { 'مضاد أكسدة': 0.9, 'يعزز فعالية فيتامين C و E': 0.98, 'حماية من أضرار الشمس': 0.85 },
        synergy: { 'vitamin-c-serum': 1.15, 'tocopherol': 1.1 },
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'مضاد أكسدة قوي، عند دمجه مع فيتامين C و E، فإنه يضاعف حمايتهما الضوئية ويجعلهما أكثر استقرارا.' }],
      },
{
        id: 'caffeic-acid',
        name: 'حمض الكافيك',
        scientific_name: 'Caffeic Acid',
        searchKeywords: ['caffeic acid', 'حمض الكافيك'],
        usageScope: ['skin'],
        functionalCategory: 'مضاد أكسدة',
        chemicalType: 'مركب نباتي',
        benefits: { 'مضاد أكسدة': 0.88, 'مضاد للالتهابات': 0.85 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
      id: 'zinc-pyrithione',
      name: 'بيريثيون الزنك',
      scientific_name: 'Zinc Pyrithione',
      searchKeywords: ['zinc pyrithione', 'zpt', 'head and shoulders active', 'بيريثيون الزنك'],
      usageScope: ['hair'],
      functionalCategory: 'مكون فعال',
      chemicalType: 'عامل مضاد للفطريات',
      benefits: { 'علاج القشرة': 0.95, 'مضاد للفطريات والبكتيريا': 0.92, 'علاج التهاب الجلد الدهني': 0.88 },
      synergy: {},
      negativeSynergy: {},
      warnings: [
        { level: 'info', text: 'أحد أشهر المكونات الفعالة لعلاج القشرة في الشامبوهات المتاحة دون وصفة طبية.' },
        { level: 'risk', text: 'تم تقييد استخدامه بشدة في مستحضرات التجميل في الاتحاد الأوروبي بسبب مخاوف تتعلق بالسمية البيئية والسمية الجينية المحتملة.' }
      ],
    },
{
      id: 'selenium-sulfide',
      name: 'كبريتيد السيلينيوم',
      scientific_name: 'Selenium Sulfide',
      searchKeywords: ['selenium sulfide', 'selsun blue active', 'كبريتيد السيلينيوم'],
      usageScope: ['hair'],
      functionalCategory: 'مكون فعال',
      chemicalType: 'عامل مضاد للفطريات',
      benefits: { 'علاج القشرة': 0.92, 'علاج التهاب الجلد الدهني': 0.9, 'مضاد للفطريات': 0.88 },
      synergy: {},
      negativeSynergy: {},
      warnings: [
        { level: 'caution', text: 'علاج قوي، قد يسبب جفافا أو تهيجا. يجب استخدامه حسب التعليمات.' },
        { level: 'caution', text: 'قد يغير لون الشعر المصبوغ أو الفاتح.' }
      ],
    },
{
      id: 'piroctone-olamine',
      name: 'بيروكتون أولامين',
      scientific_name: 'Piroctone Olamine',
      searchKeywords: ['piroctone olamine', 'octopirox', 'بيروكتون أولامين'],
      usageScope: ['hair'],
      functionalCategory: 'مكون فعال',
      chemicalType: 'عامل مضاد للفطريات',
      benefits: { 'علاج القشرة': 0.9, 'لطيف على فروة الرأس': 0.88, 'مضاد للفطريات': 0.85 },
      synergy: { 'salicylic-acid': 1.05 },
      negativeSynergy: {},
      warnings: [{ level: 'info', text: 'بديل حديث ولطيف لبيريثيون الزنك، فعال ضد الفطريات المسببة للقشرة.' }],
    },
{
      id: 'climbazole',
      name: 'كليمبازول',
      scientific_name: 'Climbazole',
      searchKeywords: ['climbazole', 'كليمبازول'],
      usageScope: ['hair'],
      functionalCategory: 'مكون فعال',
      chemicalType: 'عامل مضاد للفطريات',
      benefits: { 'مضاد للقشرة قوي': 0.9, 'مضاد للفطريات': 0.92 },
      synergy: {},
      negativeSynergy: {},
      warnings: [{ level: 'info', text: 'مادة حافظة ومضادة للفطريات تستخدم في الشامبوهات المضادة للقشرة.' }],
    },
{
      id: 'ketoconazole',
      name: 'كيتوكونازول',
      scientific_name: 'Ketoconazole',
      searchKeywords: ['ketoconazole', 'nizoral active', 'كيتوكونازول'],
      usageScope: ['hair'],
      functionalCategory: 'مكون فعال',
      chemicalType: 'عامل مضاد للفطريات (دوائي)',
      benefits: { 'علاج القشرة': 0.95, 'قد يساعد في تقليل تساقط الشعر': 0.75, 'مضاد للفطريات': 0.9},
      synergy: {},
      negativeSynergy: {},
      warnings: [
        { level: 'info', text: 'مكون دوائي فعال جدا، يوجد بتركيزات منخفضة (1%) في الشامبوهات المتاحة دون وصفة طبية.' }
      ],
    },
{
      id: 'salix-alba-bark-extract',
      name: 'مستخلص لحاء الصفصاف الأبيض',
      scientific_name: 'Salix Alba (Willow) Bark Extract',
      searchKeywords: ['willow bark extract', 'salix alba', 'natural salicylic acid', 'مستخلص لحاء الصفصاف'],
      usageScope: ['hair', 'skin'],
      functionalCategory: 'مكون فعال',
      chemicalType: 'مستخلص نباتي',
      benefits: { 'تقشير لطيف للفروة': 0.85, 'مضاد للالتهابات': 0.8, 'تنظيم الإفرازات الدهنية': 0.82 },
      synergy: {},
      negativeSynergy: {},
      warnings: [{ level: 'info', text: 'مصدر طبيعي للساليسين، الذي يتحول إلى حمض الساليسيليك. يعمل على تقشير فروة الرأس بلطف.' }],
    },
{
      id: 'zinc-pca',
      name: 'زنك PCA',
      scientific_name: 'Zinc PCA',
      searchKeywords: ['zinc pca', 'زنك PCA'],
      usageScope: ['hair', 'skin'],
      functionalCategory: 'مكون فعال',
      chemicalType: 'مركب زنك',
      benefits: { 'تنظيم الإفرازات الدهنية': 0.92, 'مضاد للميكروبات': 0.85, 'مضاد للالتهابات': 0.8 },
      synergy: { 'niacinamide': 1.1 },
      negativeSynergy: {},
      warnings: [{ level: 'info', text: 'ممتاز لفروة الرأس الدهنية والمعرضة لحب الشباب.' }],
    },
{
      id: 'juniperus-communis-fruit-extract',
      name: 'مستخلص توت العرعر',
      scientific_name: 'Juniperus Communis Fruit Extract',
      searchKeywords: ['juniper berry extract', 'juniperus communis'],
      usageScope: ['hair'],
      functionalCategory: 'مكون فعال',
      chemicalType: 'مستخلص نباتي',
      benefits: { 'تنقية فروة الرأس': 0.8, 'مضاد للميكروبات': 0.85, 'تنظيم الإفرازات الدهنية': 0.78 },
      synergy: {},
      negativeSynergy: {},
      warnings: [],
    },
{
      id: 'glycol-distearate',
      name: 'جلايكول ثنائي الستيارات',
      scientific_name: 'Glycol Distearate',
      searchKeywords: ['glycol distearate', 'pearlizing agent', 'جلايكول ديستيارات', 'جلايكول ديستيرات' ],
      usageScope: ['hair', 'skin'],
      functionalCategory: 'محسن جمالي',
      chemicalType: 'عامل لؤلؤي',
      benefits: { 'إعطاء مظهر لؤلؤي': 0.95, 'مكثف للقوام': 0.85, 'مطري': 0.8 },
      synergy: {},
      negativeSynergy: {},
      warnings: [{ level: 'info', text: 'المكون المسؤول عن المظهر الأبيض اللؤلؤي اللامع في العديد من الشامبوهات والكريمات.' }],
    },
{
      id: 'glycol-stearate',
      name: 'ستيارات الجلايكول',
      scientific_name: 'Glycol Stearate',
      searchKeywords: ['glycol stearate'],
      usageScope: ['hair', 'skin'],
      functionalCategory: 'محسن جمالي',
      chemicalType: 'عامل لؤلؤي',
      benefits: { 'إعطاء مظهر لؤلؤي': 0.9, 'مكثف': 0.8 },
      synergy: {},
      negativeSynergy: {},
      warnings: [],
    },
{
      id: 'peg-150-distearate',
      name: 'PEG-150 ثنائي الستيارات',
      scientific_name: 'PEG-150 Distearate',
      searchKeywords: ['peg-150 distearate'],
      usageScope: ['hair'],
      functionalCategory: 'محسن قوام',
      chemicalType: 'مكثف',
      benefits: { 'مكثف فعال جدا': 0.95, 'لا يقلل الرغوة': 0.85 },
      synergy: {},
      negativeSynergy: {},
      warnings: [{ level: 'info', text: 'مكثف شائع جدا وفعال للغاية في أنظمة المنظفات.' }],
    },
{
      id: 'cocamide-mea',
      name: 'كوكاميد MEA',
      scientific_name: 'Cocamide MEA',
      searchKeywords: ['cocamide mea', 'كوكاميد MEA'],
      usageScope: ['hair'],
      functionalCategory: 'محسن قوام',
      chemicalType: 'معزز للرغوة',
      benefits: { 'معزز للرغوة': 0.9, 'مكثف للقوام': 0.88 },
      synergy: {},
      negativeSynergy: {},
      warnings: [{ level: 'caution', text: 'مثل Cocamide DEA، قد يكون هناك مخاوف بشأن تكون النتروزامينات المسرطنة إذا لم يتم تصنيعه بشكل صحيح.' }],
    },
{
      id: 'cocamide-mipa',
      name: 'كوكاميد MIPA',
      scientific_name: 'Cocamide MIPA',
      searchKeywords: ['cocamide mipa'],
      usageScope: ['hair'],
      functionalCategory: 'محسن قوام',
      chemicalType: 'معزز للرغوة',
      benefits: { 'معزز للرغوة': 0.88, 'مكثف للقوام': 0.9 },
      synergy: {},
      negativeSynergy: {},
      warnings: [],
    },
{
      id: 'lauryl-hydroxysultaine',
      name: 'لوريل هيدروكسي سولتين',
      scientific_name: 'Lauryl Hydroxysultaine',
      searchKeywords: ['lauryl hydroxysultaine'],
      usageScope: ['hair'],
      functionalCategory: 'منظف',
      chemicalType: 'منظف معتدل',
      benefits: { 'معزز للرغوة': 0.9, 'تنظيف لطيف': 0.85, 'مكثف للقوام': 0.8 },
      synergy: {},
      negativeSynergy: {},
      warnings: [],
    },
{
      id: 'styrene-acrylates-copolymer',
      name: 'ستيرين/أكريلات كوبوليمر',
      scientific_name: 'Styrene/Acrylates Copolymer',
      searchKeywords: ['styrene/acrylates copolymer', 'styreneacrylates copolymer', 'opacifier', 'كوبوليمر أكريلات الستايرين', 'أكريلات/كوبوليمر الستايرين', 'بوليمر الستايرين/أكريلات', 'ستايرين/أكريلات كوبوليمر', ],
      usageScope: ['hair', 'skin'],
      functionalCategory: 'محسن جمالي',
      chemicalType: 'عامل معتم',
      benefits: { 'يجعل المنتج معتما': 0.95, 'يعطي مظهرا كريميا': 0.9 },
      synergy: {},
      negativeSynergy: {},
      warnings: [{ level: 'info', text: 'يستخدم لجعل التركيبات الشفافة تبدو بيضاء حليبية وكريمية.' }],
    },
{
      id: 'acrylates-copolymer',
      name: 'أكريلات كوبوليمر',
      scientific_name: 'Acrylates Copolymer',
      searchKeywords: ['acrylates copolymer'],
      usageScope: ['hair', 'skin'],
      functionalCategory: 'محسن قوام',
      chemicalType: 'بوليمر صناعي',
      benefits: { 'مكثف للقوام': 0.88, 'تكوين طبقة رقيقة': 0.9, 'مقاومة للماء': 0.8 },
      synergy: {},
      negativeSynergy: {},
      warnings: [],
    },
{
      id: 'peg-40-stearate',
      name: 'ستيرات PEG-40',
      scientific_name: 'PEG-40 Stearate',
      searchKeywords: ['peg-40 stearate'],
      usageScope: ['skin', 'hair'],
      functionalCategory: 'محسن قوام',
      chemicalType: 'مستحلب',
      benefits: { 'مستحلب': 0.85, 'عامل منظف': 0.75 },
      synergy: {},
      negativeSynergy: {},
      warnings: [],
    },
{
      id: 'sorbitan-stearate',
      name: 'ستيرات السوربيتان',
      scientific_name: 'Sorbitan Stearate',
      searchKeywords: ['sorbitan stearate'],
      usageScope: ['skin', 'hair'],
      functionalCategory: 'محسن قوام',
      chemicalType: 'مستحلب',
      benefits: { 'مستحلب': 0.8, 'مكثف': 0.82 },
      synergy: {},
      negativeSynergy: {},
      warnings: [],
    },
{
      id: 'hydrolyzed-soy-protein',
      name: 'بروتين الصويا المهدرج',
      scientific_name: 'Hydrolyzed Soy Protein',
      searchKeywords: ['hydrolyzed soy protein', 'soy protein', 'بروتين الصويا'],
      usageScope: ['hair'],
      functionalCategory: 'تقوية الشعر',
      chemicalType: 'بروتين',
      benefits: { 'تقوية الشعر': 0.85, 'إضافة حجم': 0.8, 'تحسين مرونة الشعر': 0.82 },
      synergy: {},
      negativeSynergy: {},
      warnings: [],
    },
{
      id: 'hydrolyzed-rice-protein',
      name: 'بروتين الأرز المهدرج',
      scientific_name: 'Hydrolyzed Rice Protein',
      searchKeywords: ['hydrolyzed rice protein', 'rice protein', 'بروتين الأرز'],
      usageScope: ['hair'],
      functionalCategory: 'تقوية الشعر',
      chemicalType: 'بروتين',
      benefits: { 'إضافة حجم وكثافة': 0.9, 'تقوية الشعر': 0.85, 'لا يسبب تصلبا': 0.88 },
      synergy: {},
      negativeSynergy: {},
      warnings: [{ level: 'info', text: 'بروتين ممتاز للشعر الناعم والرقيق لأنه يقوي ويضيف حجما دون إثقاله.' }],
    },
{
      id: 'hydrolyzed-vegetable-protein',
      name: 'بروتين نباتي مهدرج',
      scientific_name: 'Hydrolyzed Vegetable Protein',
      searchKeywords: ['hydrolyzed vegetable protein', 'vegetable protein', 'بروتين نباتي'],
      usageScope: ['hair'],
      functionalCategory: 'تقوية الشعر',
      chemicalType: 'بروتين',
      benefits: { 'تقوية الشعر': 0.88, 'ترطيب': 0.8, 'تقليل التقصف': 0.85 },
      synergy: {},
      negativeSynergy: {},
      warnings: [],
    },
{
      id: 'hydrolyzed-vegetable-protein-pg-propyl-silanetriol',
      name: 'بروتين نباتي مهدرج PG-بروبيل سيلانتريول',
      scientific_name: 'Hydrolyzed Vegetable Protein PG-Propyl Silanetriol',
      searchKeywords: ['hydrolyzed vegetable protein pg-propyl silanetriol', 'keravis'],
      usageScope: ['hair'],
      functionalCategory: 'تقوية الشعر',
      chemicalType: 'بروتين معدل',
      benefits: { 'تقوية فائقة للشعر': 0.95, 'حماية من التكسر': 0.92, 'تأثير مزدوج (داخلي وخارجي)': 0.9 },
      synergy: {},
      negativeSynergy: {},
      warnings: [{ level: 'info', text: 'مركب حاصل على براءة اختراع (Keravis™)، مصمم لاختراق قشرة الشعر وتقويته من الداخل، مع تكوين طبقة واقية على السطح.' }],
    },
{
      id: 'sodium-pca',
      name: 'صوديوم PCA',
      scientific_name: 'Sodium PCA',
      searchKeywords: ['sodium pca', 'صوديوم pca'],
      usageScope: ['skin', 'hair'],
      functionalCategory: 'مرطب / مطري',
      chemicalType: 'مرطب',
      benefits: { 'مرطب طبيعي قوي': 0.95, 'يجذب الرطوبة للشعر': 0.9, 'مضاد للكهرباء الساكنة': 0.8 },
      synergy: {},
      negativeSynergy: {},
      warnings: [{ level: 'info', text: 'جزء من عامل الترطيب الطبيعي (NMF) للجلد، وهو مرطب ممتاز للشعر أيضا.' }],
    },
{
      id: 'silk-amino-acids',
      name: 'أحماض الحرير الأمينية',
      scientific_name: 'Silk Amino Acids',
      searchKeywords: ['silk amino acids', 'أحماض الحرير الأمينية'],
      usageScope: ['hair'],
      functionalCategory: 'تقوية الشعر',
      chemicalType: 'حمض أميني',
      benefits: { 'اختراق عميق للشعر': 0.9, 'ترطيب وتقوية': 0.88, 'إضافة لمعان': 0.85 },
      synergy: {},
      negativeSynergy: {},
      warnings: [{ level: 'info', text: 'جزيئات صغيرة جدا من بروتين الحرير، قادرة على اختراق جذع الشعرة لترطيبها وتقويتها من الداخل.' }],
    },
{
      id: 'cocoyl-hydrolyzed-collagen',
      name: 'كولاجين مهدرج كوكويل',
      scientific_name: 'Cocoyl Hydrolyzed Collagen',
      searchKeywords: ['cocoyl hydrolyzed collagen'],
      usageScope: ['hair'],
      functionalCategory: 'مكيف للشعر',
      chemicalType: 'بروتين معدل',
      benefits: { 'تكييف خفيف': 0.8, 'تكوين طبقة واقية': 0.82 },
      synergy: {},
      negativeSynergy: {},
      warnings: [],
    },
{
      id: 'hydrolyzed-elastin',
      name: 'إيلاستين مهدرج',
      scientific_name: 'Hydrolyzed Elastin',
      searchKeywords: ['hydrolyzed elastin', 'إيلاستين مهدرج'],
      usageScope: ['hair'],
      functionalCategory: 'تقوية الشعر',
      chemicalType: 'بروتين',
      benefits: { 'تحسين مرونة الشعر': 0.88, 'ترطيب': 0.8 },
      synergy: {},
      negativeSynergy: {},
      warnings: [],
    },
{
      id: 'polyquaternium-80',
      name: 'بولي كواتيرنيوم-80',
      scientific_name: 'Polyquaternium-80',
      searchKeywords: ['polyquaternium-80'],
      usageScope: ['hair'],
      functionalCategory: 'مكيف للشعر',
      chemicalType: 'بوليمر طبيعي',
      benefits: { 'تكييف لطيف': 0.85, 'تقليل الهيشان للشعر المجعد': 0.88 },
      synergy: {},
      negativeSynergy: {},
      warnings: [{ level: 'info', text: 'بوليمر مشتق من النبات، يوفر تكييفا جيدا دون تراكم.' }],
    },
{
      id: 'polyquaternium-68',
      name: 'بولي كواتيرنيوم-68',
      scientific_name: 'Polyquaternium-68',
      searchKeywords: ['polyquaternium-68'],
      usageScope: ['hair'],
      functionalCategory: 'مثبت للشعر',
      chemicalType: 'بوليمر',
      benefits: { 'تثبيت التصفيف': 0.9, 'مقاومة للرطوبة': 0.85 },
      synergy: {},
      negativeSynergy: {},
      warnings: [{ level: 'info', text: 'يستخدم في منتجات التصفيف لتوفير ثبات قوي.' }],
    },
{
      id: 'silicone-quaternium-18',
      name: 'سيليكون كواتيرنيوم-18',
      scientific_name: 'Silicone Quaternium-18',
      searchKeywords: ['silicone quaternium-18'],
      usageScope: ['hair'],
      functionalCategory: 'مكيف للشعر',
      chemicalType: 'سيليكون أميني',
      benefits: { 'تكييف عميق': 0.88, 'حماية من الحرارة': 0.85 },
      synergy: {},
      negativeSynergy: {},
      warnings: [],
    },
{
      id: 'silicone-quaternium-22',
      name: 'سيليكون كواتيرنيوم-22',
      scientific_name: 'Silicone Quaternium-22',
      searchKeywords: ['silicone quaternium-22'],
      usageScope: ['hair'],
      functionalCategory: 'مكيف للشعر',
      chemicalType: 'سيليكون أميني',
      benefits: { 'تكييف عميق دون إثقال الشعر': 0.9, 'حماية اللون': 0.88 },
      synergy: {},
      negativeSynergy: {},
      warnings: [{ level: 'info', text: 'يوفر فوائد التكييف العميق للسيليكونات الأمينية مع تقليل التراكم.' }],
    },
{
      id: 'peg-ppg-15-15-dimethicone',
      name: 'PEG/PPG-15/15 دايميثيكون',
      scientific_name: 'PEG/PPG-15/15 Dimethicone',
      searchKeywords: ['peg/ppg-15/15 dimethicone'],
      usageScope: ['hair'],
      functionalCategory: 'مكيف للشعر',
      chemicalType: 'سيليكون قابل للذوبان في الماء',
      benefits: { 'تكييف خفيف': 0.8, 'إضافة لمعان': 0.85 },
      synergy: {},
      negativeSynergy: {},
      warnings: [],
    },
{
      id: 'phenyl-trimethicone',
      name: 'فينيل ثلاثي الميثيكون',
      scientific_name: 'Phenyl Trimethicone',
      searchKeywords: ['phenyl trimethicone'],
      usageScope: ['hair'],
      functionalCategory: 'مكيف للشعر',
      chemicalType: 'سيليكون',
      benefits: { 'إضافة لمعان فائق': 0.95, 'ملمس غير دهني': 0.88, 'مقاومة للحرارة': 0.8 },
      synergy: {},
      negativeSynergy: {},
      warnings: [{ level: 'info', text: 'سيليكون مصمم خصيصا لزيادة لمعان الشعر وانعكاس الضوء.' }],
    },
{
      id: 'c13-14-isoparaffin',
      name: 'C13-14 إيزوبرافين',
      scientific_name: 'C13-14 Isoparaffin',
      searchKeywords: ['c13-14 isoparaffin'],
      usageScope: ['skin', 'hair'],
      functionalCategory: 'مذيب',
      chemicalType: 'مذيب',
      benefits: { 'مذيب': 0.9, 'مكون في مكثفات البوليمر': 0.85 },
      synergy: {},
      negativeSynergy: {},
      warnings: [{ level: 'info', text: 'غالبا ما يوجد كمذيب في مكثفات البوليمر مثل Sepigel 305.' }],
    },
{
      id: 'laureth-7',
      name: 'لوريث-7',
      scientific_name: 'Laureth-7',
      searchKeywords: ['laureth-7', 'لوريث 7'],
      usageScope: ['skin', 'hair'],
      functionalCategory: 'مستحلب',
      chemicalType: 'مستحلب',
      benefits: { 'مستحلب': 0.8, 'مكون في مكثفات البوليمر': 0.85 },
      synergy: {},
      negativeSynergy: {},
      warnings: [{ level: 'info', text: 'غالبا ما يوجد كمستحلب في مكثفات البوليمر مثل Sepigel 305.' }],
    },
{
      id: 'polyacrylamide',
      name: 'بولي أكريلاميد',
      scientific_name: 'Polyacrylamide',
      searchKeywords: ['polyacrylamide', 'sepigel 305'],
      usageScope: ['skin', 'hair'],
      functionalCategory: 'محسن قوام',
      chemicalType: 'بوليمر صناعي',
      benefits: { 'مكثف للقوام': 0.9, 'تكوين طبقة رقيقة': 0.85 },
      synergy: {},
      negativeSynergy: {},
      warnings: [{ level: 'risk', text: 'مثير للجدل بسبب احتمال تلوثه بمادة الأكريلاميد المونومر، وهي مادة مسرطنة وسامة للأعصاب. الهيئات التنظيمية تضع حدودا صارمة جدا على مستويات الأكريلاميد المتبقية.' }],
    },
{
      id: 'equisetum-arvense-extract',
      name: 'مستخلص ذيل الحصان',
      scientific_name: 'Equisetum Arvense Extract',
      searchKeywords: ['equisetum arvense', 'horsetail extract', 'مستخلص ذيل الحصان'],
      usageScope: ['hair'],
      functionalCategory: 'مكون فعال',
      chemicalType: 'مستخلص نباتي',
      benefits: { 'غني بالسيليكا': 0.9, 'تقوية الشعر': 0.88, 'تحسين مرونة الشعر': 0.85 },
      synergy: {},
      negativeSynergy: {},
      warnings: [],
    },
{
      id: 'serenoa-serrulata-fruit-extract',
      name: 'مستخلص البلميط المنشاري',
      scientific_name: 'Serenoa Serrulata (Saw Palmetto) Fruit Extract',
      searchKeywords: ['saw palmetto', 'serenoa serrulata', 'البلميط المنشاري'],
      usageScope: ['hair'],
      functionalCategory: 'مكون فعال',
      chemicalType: 'مستخلص نباتي',
      benefits: { 'قد يقلل من تساقط الشعر': 0.8, 'مثبط لإنزيم 5-alpha-reductase': 0.82 },
      synergy: {},
      negativeSynergy: {},
      warnings: [{ level: 'info', text: 'يُعتقد أنه يساعد في منع تساقط الشعر الوراثي عن طريق تثبيط الإنزيم الذي يحول التستوستيرون إلى DHT.' }],
    },
{
      id: 'humulus-lupulus-extract',
      name: 'مستخلص الجنجل',
      scientific_name: 'Humulus Lupulus (Hops) Extract',
      searchKeywords: ['humulus lupulus', 'hops extract', 'الجنجل'],
      usageScope: ['hair'],
      functionalCategory: 'مكون فعال',
      chemicalType: 'مستخلص نباتي',
      benefits: { 'مضاد للالتهابات': 0.8, 'مضاد للميكروبات': 0.85, 'إضافة لمعان': 0.75 },
      synergy: {},
      negativeSynergy: {},
      warnings: [],
    },
{
      id: 'achillea-millefolium-extract',
      name: 'مستخلص الأخيليا الألفية',
      scientific_name: 'Achillea Millefolium Extract',
      searchKeywords: ['achillea millefolium', 'yarrow extract', 'الأخيليا'],
      usageScope: ['hair'],
      functionalCategory: 'ملطف ومرمم',
      chemicalType: 'مستخلص نباتي',
      benefits: { 'ملطف ومرمم': 0.85, 'مضاد للالتهابات': 0.8, 'قابض للمسام': 0.78 },
      synergy: {},
      negativeSynergy: {},
      warnings: [],
    },
{
      id: 'tussilago-farfara-leaf-extract',
      name: 'مستخلص أوراق حشيشة السعال',
      scientific_name: 'Tussilago Farfara (Coltsfoot) Leaf Extract',
      searchKeywords: ['tussilago farfara', 'coltsfoot'],
      usageScope: ['hair'],
      functionalCategory: 'مكون فعال',
      chemicalType: 'مستخلص نباتي',
      benefits: { 'تقوية الشعر': 0.8, 'مضاد أكسدة': 0.75 },
      synergy: {},
      negativeSynergy: {},
      warnings: [],
    },
{
      id: 'cinchona-succirubra-bark-extract',
      name: 'مستخلص لحاء الكينا',
      scientific_name: 'Cinchona Succirubra Bark Extract',
      searchKeywords: ['cinchona succirubra', 'quinine', 'كينا'],
      usageScope: ['hair'],
      functionalCategory: 'مكون فعال',
      chemicalType: 'مستخلص نباتي',
      benefits: { 'تحفيز فروة الرأس': 0.85, 'تقوية الشعر': 0.8 },
      synergy: {},
      negativeSynergy: {},
      warnings: [{ level: 'info', text: 'غني بالكينين، الذي يستخدم لتقوية الشعر وتنشيط فروة الرأس.' }],
    },
{
      id: 'biotin',
      name: 'البيوتين (فيتامين B7)',
      scientific_name: 'Biotin',
      searchKeywords: ['biotin', 'vitamin b7', 'vitamin h', 'بيوتين', 'البيوتين'],
      usageScope: ['hair'],
      functionalCategory: 'مكون فعال',
      chemicalType: 'فيتامين',
      benefits: { 'تقوية الشعر': 0.8, 'تحسين بنية الكيراتين': 0.82 },
      synergy: {},
      negativeSynergy: {},
      warnings: [{ level: 'info', text: 'على الرغم من شهرته، إلا أن فعاليته عند التطبيق الموضعي محدودة مقارنة بتناوله كمكمل غذائي.' }],
    },
{
      id: 'menthyl-lactate',
      name: 'لاكتات المنثيل',
      scientific_name: 'Menthyl Lactate',
      searchKeywords: ['menthyl lactate'],
      usageScope: ['hair', 'skin'],
      functionalCategory: 'مكون فعال',
      chemicalType: 'عامل تبريد',
      benefits: { 'إحساس بالبرودة لطيف وطويل الأمد': 0.9, 'أقل تهيجا من المنثول': 0.92 },
      synergy: {},
      negativeSynergy: {},
      warnings: [{ level: 'info', text: 'يوفر إحساسا بالبرودة والانتعاش ولكن بشكل ألطف وبدون الرائحة القوية للمنثول أو النعناع.' }],
    },
{
      id: 'laureth-2',
      name: 'لوريث-2',
      scientific_name: 'Laureth-2',
      searchKeywords: ['laureth-2'],
      usageScope: ['hair'],
      functionalCategory: 'محسن قوام',
      chemicalType: 'مستحلب',
      benefits: { 'مكثف للقوام': 0.8 },
      synergy: {},
      negativeSynergy: {},
      warnings: [],
    },
{
      id: 'peg-3-distearate',
      name: 'PEG-3 ثنائي الستيارات',
      scientific_name: 'PEG-3 Distearate',
      searchKeywords: ['peg-3 distearate'],
      usageScope: ['hair'],
      functionalCategory: 'محسن جمالي',
      chemicalType: 'عامل لؤلؤي',
      benefits: { 'إعطاء مظهر لؤلؤي': 0.85, 'مستحلب': 0.8 },
      synergy: {},
      negativeSynergy: {},
      warnings: [],
    },
{
      id: 'limonene',
      name: 'ليمونين',
      scientific_name: 'Limonene',
      searchKeywords: ['limonene', 'ليمونين'],
      usageScope: ['skin', 'hair'],
      functionalCategory: 'عطر',
      chemicalType: 'مكون عطري (مسبب حساسية)',
      benefits: { 'رائحة حمضية': 0.9, 'مذيب': 0.7 },
      synergy: {},
      negativeSynergy: {},
      warnings: [{ level: 'caution', text: 'مكون عطري طبيعي (من قشور الحمضيات) يجب الإفصاح عنه كمسبب للحساسية. يتأكسد بسهولة عند تعرضه للهواء، ونواتج الأكسدة هي المسبب الرئيسي للحساسية.' }],
    },
{
      id: 'linalool',
      name: 'لينالول',
      scientific_name: 'Linalool',
      searchKeywords: ['linalool', 'لينالول'],
      usageScope: ['skin', 'hair'],
      functionalCategory: 'عطر',
      chemicalType: 'مكون عطري (مسبب حساسية)',
      benefits: { 'رائحة زهرية': 0.9 },
      synergy: {},
      negativeSynergy: {},
      warnings: [{ level: 'risk', text: 'مكون عطري طبيعي (من اللافندر والكزبرة) يجب الإفصاح عنه كمسبب للحساسية. يتأكسد بسهولة عند تعرضه للهواء، مما يجعله أكثر تحسيسا.' }],
    },
{
      id: 'citronellol',
      name: 'سيترونيلول',
      scientific_name: 'Citronellol',
      searchKeywords: ['citronellol', 'سيترونيلول'],
      usageScope: ['skin', 'hair'],
      functionalCategory: 'عطر',
      chemicalType: 'مكون عطري (مسبب حساسية)',
      benefits: { 'رائحة الورد/الليمون': 0.9 },
      synergy: {},
      negativeSynergy: {},
      warnings: [{ level: 'caution', text: 'مكون عطري طبيعي (من الورد والجيرانيوم) يجب الإفصاح عنه كمسبب للحساسية.' }],
    },
{
      id: 'hexyl-cinnamal',
      name: 'هكسيل سينامال',
      scientific_name: 'Hexyl Cinnamal',
      searchKeywords: ['hexyl cinnamal', 'هكسيل سينامال'],
      usageScope: ['skin', 'hair'],
      functionalCategory: 'عطر',
      chemicalType: 'مكون عطري (مسبب حساسية)',
      benefits: { 'رائحة الياسمين': 0.9 },
      synergy: {},
      negativeSynergy: {},
      warnings: [{ level: 'caution', text: 'مكون عطري (غالبا صناعي) يجب الإفصاح عنه كمسبب للحساسية.' }],
    },
{
      id: 'geraniol',
      name: 'جيرانيول',
      scientific_name: 'Geraniol',
      searchKeywords: ['geraniol', 'جيرانيول'],
      usageScope: ['skin', 'hair'],
      functionalCategory: 'عطر',
      chemicalType: 'مكون عطري (مسبب حساسية)',
      benefits: { 'رائحة الورد': 0.9 },
      synergy: {},
      negativeSynergy: {},
      warnings: [{ level: 'caution', text: 'مكون عطري طبيعي (من الورد والجيرانيوم) يجب الإفصاح عنه كمسبب للحساسية.' }],
    },
{
      id: 'benzyl-salicylate',
      name: 'بنزيل ساليسيلات',
      scientific_name: 'Benzyl Salicylate',
      searchKeywords: ['benzyl salicylate', 'بنزيل ساليسيلات'],
      usageScope: ['skin', 'hair'],
      functionalCategory: 'عطر',
      chemicalType: 'مكون عطري (مسبب حساسية)',
      benefits: { 'رائحة زهرية خفيفة': 0.85, 'مذيب للعطور': 0.8, 'امتصاص للأشعة فوق البنفسجية': 0.7 },
      synergy: {},
      negativeSynergy: {},
      warnings: [{ level: 'caution', text: 'مكون عطري يجب الإفصاح عنه كمسبب للحساسية. يجب تجنبه من قبل الأشخاص الذين يعانون من حساسية الأسبرين.' }],
    },
{
      id: 'alpha-isomethyl-ionone',
      name: 'ألفا-أيزوميثيل أيونون',
      scientific_name: 'Alpha-Isomethyl Ionone',
      searchKeywords: ['alpha-isomethyl ionone', 'ألفا-أيزوميثيل أيونون'],
      usageScope: ['skin', 'hair'],
      functionalCategory: 'عطر',
      chemicalType: 'مكون عطري (مسبب حساسية)',
      benefits: { 'رائحة البنفسج البودرية': 0.9 },
      synergy: {},
      negativeSynergy: {},
      warnings: [{ level: 'caution', text: 'مكون عطري صناعي يجب الإفصاح عنه كمسبب للحساسية.' }],
    },
{
      id: 'coumarin',
      name: 'كومارين',
      scientific_name: 'Coumarin',
      searchKeywords: ['coumarin', 'كومارين'],
      usageScope: ['skin', 'hair'],
      functionalCategory: 'عطر',
      chemicalType: 'مكون عطري (مسبب حساسية)',
      benefits: { 'رائحة حلوة (تشبه التبن والفانيليا)': 0.9 },
      synergy: {},
      negativeSynergy: {},
      warnings: [{ level: 'caution', text: 'مكون عطري طبيعي (من حبوب التونكا) يجب الإفصاح عنه كمسبب للحساسية.' }],
    },
{
      id: 'eugenol',
      name: 'يوجينول',
      scientific_name: 'Eugenol',
      searchKeywords: ['eugenol', 'يوجينول'],
      usageScope: ['skin', 'hair'],
      functionalCategory: 'عطر',
      chemicalType: 'مكون عطري (مسبب حساسية)',
      benefits: { 'رائحة القرنفل الحارة': 0.9 },
      synergy: {},
      negativeSynergy: {},
      warnings: [{ level: 'caution', text: 'مكون عطري طبيعي (من القرنفل) يجب الإفصاح عنه كمسبب للحساسية.' }],
    },
{
      id: 'citral',
      name: 'سيترال',
      scientific_name: 'Citral',
      searchKeywords: ['citral', 'سيترال'],
      usageScope: ['skin', 'hair'],
      functionalCategory: 'عطر',
      chemicalType: 'مكون عطري (مسبب حساسية)',
      benefits: { 'رائحة ليمون قوية': 0.9 },
      synergy: {},
      negativeSynergy: {},
      warnings: [{ level: 'risk', text: 'مكون عطري طبيعي (من الليمون وعشب الليمون) يجب الإفصاح عنه كمسبب للحساسية.' }],
    },
{
      id: 'cinnamal',
      name: 'سينامال',
      scientific_name: 'Cinnamal',
      searchKeywords: ['cinnamal', 'cinnamaldehyde', 'سينامال'],
      usageScope: ['skin', 'hair'],
      functionalCategory: 'عطر',
      chemicalType: 'مكون عطري (مسبب حساسية)',
      benefits: { 'رائحة القرفة': 0.9 },
      synergy: {},
      negativeSynergy: {},
      warnings: [{ level: 'risk', text: 'مكون عطري طبيعي (من القرفة) يجب الإفصاح عنه كمسبب للحساسية.' }],
    },
{
      id: 'hydroxycitronellal',
      name: 'هيدروكسي سيترونيلال',
      scientific_name: 'Hydroxycitronellal',
      searchKeywords: ['hydroxycitronellal', 'هيدروكسي سيترونيلال'],
      usageScope: ['skin', 'hair'],
      functionalCategory: 'عطر',
      chemicalType: 'مكون عطري (مسبب حساسية)',
      benefits: { 'رائحة زهرية (زنبق الوادي)': 0.9 },
      synergy: {},
      negativeSynergy: {},
      warnings: [{ level: 'risk', text: 'مكون عطري صناعي يجب الإفصاح عنه كمسبب للحساسية.' }],
    },
{
      id: 'disodium-edta',
      name: 'ثنائي الصوديوم EDTA',
      scientific_name: 'Disodium EDTA',
      searchKeywords: ['disodium edta', 'edta'],
      usageScope: ['hair', 'skin'],
      functionalCategory: 'محسن قوام',
      chemicalType: 'عامل خالب',
      benefits: { 'يحسن استقرار المنتج': 0.92, 'يعزز الرغوة في الماء العسر': 0.9, 'يعزز فعالية المواد الحافظة': 0.88 },
      synergy: {},
      negativeSynergy: {},
      warnings: [
        { level: 'info', text: 'عامل خالب يربط أيونات المعادن (مثل الكالسيوم والمغنيسيوم) في الماء العسر، مما يمنعها من التأثير على استقرار المنتج ورغوته.' },
        { level: 'caution', text: 'غير قابل للتحلل بيولوجيا بسهولة، مما يثير مخاوف بيئية.' }
      ],
    },
{
      id: 'tetrasodium-edta',
      name: 'رباعي الصوديوم EDTA',
      scientific_name: 'Tetrasodium EDTA',
      searchKeywords: ['tetrasodium edta', 'edta'],
      usageScope: ['hair', 'skin'],
      functionalCategory: 'محسن قوام',
      chemicalType: 'عامل خالب',
      benefits: { 'يحسن استقرار المنتج': 0.9, 'يعزز الرغوة في الماء العسر': 0.88 },
      synergy: {},
      negativeSynergy: {},
      warnings: [{ level: 'info', text: 'يعمل بشكل أفضل في التركيبات ذات الرقم الهيدروجيني الأعلى من Disodium EDTA.' }],
    },
{
      id: 'tetrasodium-etidronate',
      name: 'رباعي الصوديوم إيتيدرونات',
      scientific_name: 'Tetrasodium Etidronate',
      searchKeywords: ['tetrasodium etidronate'],
      usageScope: ['hair', 'skin'],
      functionalCategory: 'محسن قوام',
      chemicalType: 'عامل خالب',
      benefits: { 'يحسن استقرار المنتج': 0.88 },
      synergy: {},
      negativeSynergy: {},
      warnings: [],
    },
{
      id: 'aminomethyl-propanol',
      name: 'أمينوميثيل بروبانول',
      scientific_name: 'Aminomethyl Propanol',
      searchKeywords: ['aminomethyl propanol', 'amp'],
      usageScope: ['hair', 'skin'],
      functionalCategory: 'محسن قوام',
      chemicalType: 'معدل الحموضة',
      benefits: { 'ضبط حموضة المنتج (pH)': 0.95 },
      synergy: {},
      negativeSynergy: {},
      warnings: [],
    },
{
      id: 'triethanolamine',
      name: 'ثلاثي إيثانولامين (TEA)',
      scientific_name: 'Triethanolamine',
      searchKeywords: ['triethanolamine'],
      usageScope: ['skin', 'hair'],
      functionalCategory: 'محسن قوام',
      chemicalType: 'معدل الحموضة',
      benefits: { 'ضبط حموضة المنتج (pH)': 0.92, 'مستحلب': 0.8 },
      synergy: {},
      negativeSynergy: {},
      warnings: [{ level: 'risk', text: 'مقيد الاستخدام. قد يتفاعل مع عوامل النترتة لتكوين النتروزامينات المسرطنة. لا ينبغي استخدامه في المنتجات التي تحتوي على هذه العوامل.' }],
    },
{
      id: 'tromethamine',
      name: 'تروميثامين',
      scientific_name: 'Tromethamine',
      searchKeywords: ['tromethamine'],
      usageScope: ['skin', 'hair'],
      functionalCategory: 'محسن قوام',
      chemicalType: 'معدل الحموضة',
      benefits: { 'ضبط حموضة المنتج (pH)': 0.9, 'لطيف على البشرة': 0.85 },
      synergy: {},
      negativeSynergy: {},
      warnings: [],
    },
{
      id: 'sodium-sulfite',
      name: 'كبريتيت الصوديوم',
      scientific_name: 'Sodium Sulfite',
      searchKeywords: ['sodium sulfite'],
      usageScope: ['hair'],
      functionalCategory: 'مضاد أكسدة',
      chemicalType: 'مادة حافظة',
      benefits: { 'مضاد أكسدة (يحمي المنتج)': 0.85, 'مادة حافظة': 0.8 },
      synergy: {},
      negativeSynergy: {},
      warnings: [],
    },
{
      id: 'sodium-metabisulfite',
      name: 'ميتابيسلفيت الصوديوم',
      scientific_name: 'Sodium Metabisulfite',
      searchKeywords: ['sodium metabisulfite'],
      usageScope: ['hair'],
      functionalCategory: 'مضاد أكسدة',
      chemicalType: 'مادة حافظة',
      benefits: { 'مضاد أكسدة (يحمي المنتج)': 0.88, 'مادة حافظة': 0.82 },
      synergy: {},
      negativeSynergy: {},
      warnings: [],
    },
{
      id: 'dehydroacetic-acid',
      name: 'حمض ديهيدروأسيتيك',
      scientific_name: 'Dehydroacetic Acid',
      searchKeywords: ['dehydroacetic acid'],
      usageScope: ['skin', 'hair'],
      functionalCategory: 'مادة حافظة',
      chemicalType: 'مادة حافظة',
      benefits: { 'حفظ المنتج': 0.85, 'مقبول في مستحضرات التجميل الطبيعية': 0.9 },
      synergy: { 'benzyl-alcohol': 1.05 },
      negativeSynergy: {},
      warnings: [{ level: 'info', text: 'غالبا ما يستخدم مع كحول البنزيل في أنظمة الحفظ المعتمدة من Ecocert.' }],
    },
{
      id: 'peg-7m',
      name: 'PEG-7M',
      scientific_name: 'PEG-7M',
      searchKeywords: ['peg-7m'],
      usageScope: ['hair'],
      functionalCategory: 'محسن قوام',
      chemicalType: 'بوليمر',
      benefits: { 'مكثف للقوام': 0.8, 'يعطي انزلاقا': 0.85 },
      synergy: {},
      negativeSynergy: {},
      warnings: [],
    },
{
      id: 'peg-90m',
      name: 'PEG-90M',
      scientific_name: 'PEG-90M',
      searchKeywords: ['peg-90m'],
      usageScope: ['hair'],
      functionalCategory: 'محسن قوام',
      chemicalType: 'بوليمر',
      benefits: { 'مكثف للقوام': 0.85, 'يعطي انزلاقا': 0.9, 'تكوين طبقة رقيقة': 0.8 },
      synergy: {},
      negativeSynergy: {},
      warnings: [],
    },
{
      id: 'peg-180m',
      name: 'PEG-180M',
      scientific_name: 'PEG-180M',
      searchKeywords: ['peg-180m'],
      usageScope: ['hair'],
      functionalCategory: 'محسن قوام',
      chemicalType: 'بوليمر',
      benefits: { 'مكثف للقوام': 0.88, 'يعطي انزلاقا': 0.92 },
      synergy: {},
      negativeSynergy: {},
      warnings: [],
    },
{
      id: 'peg-80-sorbitan-laurate',
      name: 'لورات سوربيتان PEG-80',
      scientific_name: 'PEG-80 Sorbitan Laurate',
      searchKeywords: ['peg-80 sorbitan laurate'],
      usageScope: ['hair', 'skin'],
      functionalCategory: 'منظف',
      chemicalType: 'منظف لطيف',
      benefits: { 'منظف لطيف جدا': 0.9, 'لا يسبب تهيج للعين': 0.95 },
      synergy: {},
      negativeSynergy: {},
      warnings: [{ level: 'info', text: 'مكون أساسي في تركيبات الشامبو "No More Tears" للأطفال.' }],
    },
{
      id: 'ppg-2-hydroxyethyl-coco-isostearamide',
      name: 'PPG-2 هيدروكسي إيثيل كوكو/أيزوستيراميد',
      scientific_name: 'PPG-2-Hydroxyethyl Coco/Isostearamide',
      searchKeywords: ['ppg-2-hydroxyethyl coco/isostearamide'],
      usageScope: ['hair'],
      functionalCategory: 'محسن قوام',
      chemicalType: 'معزز للرغوة',
      benefits: { 'معزز للرغوة': 0.85, 'مكيف خفيف': 0.8 },
      synergy: {},
      negativeSynergy: {},
      warnings: [],
    },
{
      id: 'sodium-c12-13-pareth-sulfate',
      name: 'صوديوم C12-13 باريث سلفات',
      scientific_name: 'Sodium C12-13 Pareth Sulfate',
      searchKeywords: ['sodium c12-13 pareth sulfate'],
      usageScope: ['hair'],
      functionalCategory: 'منظف',
      chemicalType: 'منظف قوي',
      benefits: { 'تنظيف فعال': 0.88, 'رغوة جيدة': 0.85 },
      synergy: {},
      negativeSynergy: {},
      warnings: [{ level: 'caution', text: 'مشابه لـ SLES، قد يكون مجففا.' }],
    },
{
      id: 'potassium-chloride',
      name: 'كلوريد البوتاسيوم',
      scientific_name: 'Potassium Chloride',
      searchKeywords: ['potassium chloride', 'كلوريد البوتاسيوم'],
      usageScope: ['hair'],
      functionalCategory: 'محسن قوام',
      chemicalType: 'ملح',
      benefits: { 'مكثف للقوام': 0.8 },
      synergy: {},
      negativeSynergy: {},
      warnings: [{ level: 'info', text: 'ملح يستخدم لزيادة لزوجة الشامبو.' }],
    },
{
      id: 'zinc-sulfate',
      name: 'كبريتات الزنك',
      scientific_name: 'Zinc Sulfate',
      searchKeywords: ['zinc sulfate', 'كبريتات الزنك'],
      usageScope: ['hair'],
      functionalCategory: 'مكون فعال',
      chemicalType: 'ملح معدني',
      benefits: { 'قابض للمسام': 0.8, 'مضاد للميكروبات': 0.75 },
      synergy: {},
      negativeSynergy: {},
      warnings: [],
    },
{
      id: 'ppg-12',
      name: 'PPG-12',
      scientific_name: 'PPG-12',
      searchKeywords: ['ppg-12'],
      usageScope: ['hair'],
      functionalCategory: 'مكيف للشعر',
      chemicalType: 'بوليمر',
      benefits: { 'يعطي ملمسا ناعما': 0.8, 'مذيب': 0.82 },
      synergy: {},
      negativeSynergy: {},
      warnings: [],
    },
{
      id: 'oleth-20',
      name: 'أوليث-20',
      scientific_name: 'Oleth-20',
      searchKeywords: ['oleth-20'],
      usageScope: ['hair'],
      functionalCategory: 'مستحلب',
      chemicalType: 'مستحلب',
      benefits: { 'مستحلب': 0.85, 'منظف': 0.8 },
      synergy: {},
      negativeSynergy: {},
      warnings: [],
    },
{
      id: 'peg-14m',
      name: 'PEG-14M',
      scientific_name: 'PEG-14M',
      searchKeywords: ['peg-14m'],
      usageScope: ['hair'],
      functionalCategory: 'محسن قوام',
      chemicalType: 'بوليمر',
      benefits: { 'مكثف للقوام': 0.82, 'يعطي انزلاقا': 0.88 },
      synergy: {},
      negativeSynergy: {},
      warnings: [],
    },
{
      id: 'peg-23m',
      name: 'PEG-23M',
      scientific_name: 'PEG-23M',
      searchKeywords: ['peg-23m'],
      usageScope: ['hair'],
      functionalCategory: 'محسن قوام',
      chemicalType: 'بوليمر',
      benefits: { 'مكثف للقوام': 0.85, 'يعطي انزلاقا': 0.9 },
      synergy: {},
      negativeSynergy: {},
      warnings: [],
    },
{
      id: 'peg-45m',
      name: 'PEG-45M',
      scientific_name: 'PEG-45M',
      searchKeywords: ['peg-45m'],
      usageScope: ['hair'],
      functionalCategory: 'محسن قوام',
      chemicalType: 'بوليمر',
      benefits: { 'مكثف للقوام': 0.88, 'يعطي انزلاقا': 0.92 },
      synergy: {},
      negativeSynergy: {},
      warnings: [],
    },
{
      id: 'peg-8-dimethicone',
      name: 'PEG-8 دايميثيكون',
      scientific_name: 'PEG-8 Dimethicone',
      searchKeywords: ['peg-8 dimethicone'],
      usageScope: ['hair'],
      functionalCategory: 'مكيف للشعر',
      chemicalType: 'سيليكون قابل للذوبان في الماء',
      benefits: { 'تكييف خفيف': 0.82, 'ترطيب': 0.8 },
      synergy: {},
      negativeSynergy: {},
      warnings: [],
    },
{
      id: 'laureth-9',
      name: 'لوريث-9',
      scientific_name: 'Laureth-9',
      searchKeywords: ['laureth-9', 'polidocanol'],
      usageScope: ['hair', 'skin'],
      functionalCategory: 'مكون فعال',
      chemicalType: 'عامل مهدئ',
      benefits: { 'مضاد للحكة': 0.9, 'منظف': 0.8 },
      synergy: {},
      negativeSynergy: {},
      warnings: [{ level: 'info', text: 'يستخدم كمخدر موضعي خفيف لتهدئة الحكة في فروة الرأس.' }],
    },
{
      id: 'trideceth-6',
      name: 'ترايديسيث-6',
      scientific_name: 'Trideceth-6',
      searchKeywords: ['trideceth-6'],
      usageScope: ['hair'],
      functionalCategory: 'مستحلب',
      chemicalType: 'مستحلب',
      benefits: { 'مستحلب': 0.8, 'يساعد على توزيع المكونات': 0.85 },
      synergy: {},
      negativeSynergy: {},
      warnings: [],
    },
{
      id: 'trideceth-3',
      name: 'ترايديسيث-3',
      scientific_name: 'Trideceth-3',
      searchKeywords: ['trideceth-3'],
      usageScope: ['hair'],
      functionalCategory: 'مستحلب',
      chemicalType: 'مستحلب',
      benefits: { 'مستحلب': 0.82, 'منظف ثانوي': 0.75 },
      synergy: {},
      negativeSynergy: {},
      warnings: [],
    },
{
      id: 'polysorbate-21',
      name: 'بوليسوربات 21',
      scientific_name: 'Polysorbate 21',
      searchKeywords: ['polysorbate 21'],
      usageScope: ['hair'],
      functionalCategory: 'مستحلب',
      chemicalType: 'مستحلب',
      benefits: { 'مذوب للعطور': 0.88 },
      synergy: {},
      negativeSynergy: {},
      warnings: [],
    },
{
      id: 'poloxamer-338',
      name: 'بولوكسامير 338',
      scientific_name: 'Poloxamer 338',
      searchKeywords: ['poloxamer 338'],
      usageScope: ['hair'],
      functionalCategory: 'منظف',
      chemicalType: 'منظف لطيف',
      benefits: { 'منظف لطيف': 0.8 },
      synergy: {},
      negativeSynergy: {},
      warnings: [],
    },
{
      id: 'sodium-xylenesulfonate',
      name: 'صوديوم زيلين سلفونات',
      scientific_name: 'Sodium Xylenesulfonate',
      searchKeywords: ['sodium xylenesulfonate', 'sodium xylene sulfonate', 'سلفونات زيلين الصوديوم'],
      usageScope: ['hair'],
      functionalCategory: 'محسن قوام',
      chemicalType: 'هيدروتروب',
      benefits: { 'يزيد من قابلية ذوبان المنظفات': 0.9, 'يقلل لزوجة المنتج': 0.85 },
      synergy: {},
      negativeSynergy: {},
      warnings: [{ level: 'info', text: 'يساعد على منع انفصال التركيبة والحفاظ على شفافيتها.' }],
    },
{
      id: 'dipalmitoylethyl-hydroxyethylmonium-methosulfate',
      name: 'ميثوسلفات ثنائي بالميتويل إيثيل هيدروكسي إيثيل مونيوم',
      scientific_name: 'Dipalmitoylethyl Hydroxyethylmonium Methosulfate',
      searchKeywords: ['dipalmitoylethyl hydroxyethylmonium methosulfate'],
      usageScope: ['hair'],
      functionalCategory: 'مكيف للشعر',
      chemicalType: 'مركب استر رباعي',
      benefits: { 'تكييف': 0.85, 'تنعيم': 0.8, 'قابل للتحلل': 0.88 },
      synergy: {},
      negativeSynergy: {},
      warnings: [{ level: 'info', text: 'بديل صديق للبيئة للمكيفات الرباعية التقليدية.' }],
    },
{
      id: 'hydroxypropyltrimonium-hydrolyzed-wheat-protein',
      name: 'بروتين القمح المهدرج هيدروكسي بروبيل تريمونيوم',
      scientific_name: 'Hydroxypropyltrimonium Hydrolyzed Wheat Protein',
      searchKeywords: ['hydroxypropyltrimonium hydrolyzed wheat protein'],
      usageScope: ['hair'],
      functionalCategory: 'مكيف للشعر',
      chemicalType: 'بروتين معدل',
      benefits: { 'تكييف': 0.88, 'تقليل الهيشان': 0.85 },
      synergy: {},
      negativeSynergy: {},
      warnings: [],
    },
{
      id: 'palmitamidopropyltrimonium-chloride',
      name: 'كلوريد بالميتاميدوبروبيل تريمونيوم',
      scientific_name: 'Palmitamidopropyltrimonium Chloride',
      searchKeywords: ['palmitamidopropyltrimonium chloride'],
      usageScope: ['hair'],
      functionalCategory: 'مكيف للشعر',
      chemicalType: 'مركب أمونيوم رباعي',
      benefits: { 'تكييف': 0.85, 'مضاد للكهرباء الساكنة': 0.88 },
      synergy: {},
      negativeSynergy: {},
      warnings: [],
    },
{
      id: 'distearoylethyl-dimonium-chloride',
      name: 'كلوريد ثنائي ستيرويل إيثيل دايمونيوم',
      scientific_name: 'Distearoylethyl Dimonium Chloride',
      searchKeywords: ['distearoylethyl dimonium chloride'],
      usageScope: ['hair'],
      functionalCategory: 'مكيف للشعر',
      chemicalType: 'مركب استر رباعي',
      benefits: { 'تكييف': 0.88, 'فك تشابك': 0.85, 'قابل للتحلل': 0.9 },
      synergy: {},
      negativeSynergy: {},
      warnings: [],
    },
{
      id: 'ppg-1-peg-9-lauryl-glycol-ether',
      name: 'PPG-1-PEG-9 لوريل جلايكول إيثر',
      scientific_name: 'PPG-1-PEG-9 Lauryl Glycol Ether',
      searchKeywords: ['ppg-1-peg-9 lauryl glycol ether'],
      usageScope: ['hair'],
      functionalCategory: 'مذوب',
      chemicalType: 'مستحلب',
      benefits: { 'مذوب للعطور والزيوت': 0.9 },
      synergy: {},
      negativeSynergy: {},
      warnings: [],
    },
{
      id: 'sodium-formate',
      name: 'فورمات الصوديوم',
      scientific_name: 'Sodium Formate',
      searchKeywords: ['sodium formate'],
      usageScope: ['hair'],
      functionalCategory: 'محسن قوام',
      chemicalType: 'معدل الحموضة',
      benefits: { 'تنظيم درجة الحموضة': 0.8 },
      synergy: {},
      negativeSynergy: {},
      warnings: [],
    },
{
      id: 'peg-ppg-120-10-trimethylolpropane-trioleate',
      name: 'PEG/PPG-120/10 ثلاثي ميثيل بروبان ثلاثي الأوليات',
      scientific_name: 'PEG/PPG-120/10 Trimethylolpropane Trioleate',
      searchKeywords: ['peg/ppg-120/10 trimethylolpropane trioleate'],
      usageScope: ['hair'],
      functionalCategory: 'محسن قوام',
      chemicalType: 'مكثف',
      benefits: { 'مكثف فعال': 0.9 },
      synergy: {},
      negativeSynergy: {},
      warnings: [],
    },
{
      id: 'laureth-10',
      name: 'لوريث-10',
      scientific_name: 'Laureth-10',
      searchKeywords: ['laureth-10'],
      usageScope: ['hair'],
      functionalCategory: 'مستحلب',
      chemicalType: 'مستحلب',
      benefits: { 'مستحلب': 0.8, 'مكثف': 0.75 },
      synergy: {},
      negativeSynergy: {},
      warnings: [],
    },
{
      id: 'glyceryl-oleate',
      name: 'أوليات الجليسريل',
      scientific_name: 'Glyceryl Oleate',
      searchKeywords: ['glyceryl oleate'],
      usageScope: ['hair', 'skin'],
      functionalCategory: 'مرطب / مطري',
      chemicalType: 'عامل إعادة ترطيب',
      benefits: { 'إعادة ترطيب الشعر': 0.88, 'مطري': 0.9, 'مستحلب': 0.82 },
      synergy: { 'coco-glucoside': 1.05 },
      negativeSynergy: {},
      warnings: [{ level: 'info', text: 'غالبا ما يستخدم مع Coco-Glucoside لتكوين Lamesoft PO 65، وهو عامل إعادة ترطيب حيوي مطابق للدهون الطبيعية في البشرة.' }],
    },
{
      id: 'hydrogenated-palm-glycerides-citrate',
      name: 'سيترات جليسريدات النخيل المهدرجة',
      scientific_name: 'Hydrogenated Palm Glycerides Citrate',
      searchKeywords: ['hydrogenated palm glycerides citrate'],
      usageScope: ['skin', 'hair'],
      functionalCategory: 'مرطب / مطري',
      chemicalType: 'مطري',
      benefits: { 'مطري': 0.8, 'مستحلب': 0.75 },
      synergy: {},
      negativeSynergy: {},
      warnings: [{ level: 'caution', text: 'قد يثير مخاوف بشأن الاستدامة البيئية لزيت النخيل.' }],
    },
{
      id: 'peg-18-glyceryl-oleate-cocoate',
      name: 'PEG-18 أوليات/كوكوات الجليسريل',
      scientific_name: 'PEG-18 Glyceryl Oleate/Cocoate',
      searchKeywords: ['peg-18 glyceryl oleate/cocoate'],
      usageScope: ['hair'],
      functionalCategory: 'محسن قوام',
      chemicalType: 'مكثف',
      benefits: { 'مكثف': 0.85, 'إعادة ترطيب': 0.8 },
      synergy: {},
      negativeSynergy: {},
      warnings: [],
    },
{
      id: 'peg-200-hydrogenated-glyceryl-palmate',
      name: 'بالميت الجليسريل المهدرج PEG-200',
      scientific_name: 'PEG-200 Hydrogenated Glyceryl Palmate',
      searchKeywords: ['peg-200 hydrogenated glyceryl palmate'],
      usageScope: ['hair'],
      functionalCategory: 'محسن قوام',
      chemicalType: 'مكثف',
      benefits: { 'مكثف فعال': 0.9, 'لطيف جدا': 0.85 },
      synergy: {},
      negativeSynergy: {},
      warnings: [],
    },
{
      id: 'sorbic-acid',
      name: 'حمض السوربيك',
      scientific_name: 'Sorbic Acid',
      searchKeywords: ['sorbic acid', 'حمض السوربيك', 'acid sorbic'],
      usageScope: ['skin', 'hair'],
      functionalCategory: 'مادة حافظة',
      chemicalType: 'مادة حافظة',
      benefits: { 'حفظ المنتج': 0.88, 'مضاد للفطريات والخمائر': 0.9 },
      synergy: {},
      negativeSynergy: {},
      warnings: [],
    },
{
      id: 'polysilicone-29',
      name: 'بولي سيليكون-29',
      scientific_name: 'Polysilicone-29',
      searchKeywords: ['polysilicone-29'],
      usageScope: ['hair'],
      functionalCategory: 'مكيف للشعر',
      chemicalType: 'سيليكون',
      benefits: { 'حماية من الحرارة': 0.9, 'تقليل الهيشان': 0.88, 'مقاومة للرطوبة': 0.85 },
      synergy: {},
      negativeSynergy: {},
      warnings: [{ level: 'info', text: 'سيليكون يتم تنشيطه بالحرارة لتكوين طبقة واقية على الشعر.' }],
    },
{
      id: 'basic-brown-17',
      name: 'بني أساسي 17',
      scientific_name: 'Basic Brown 17',
      searchKeywords: ['basic brown 17', 'ci 12251'],
      usageScope: ['hair'],
      functionalCategory: 'محسن جمالي',
      chemicalType: 'ملون شعر شبه دائم',
      benefits: { 'إعطاء لون بني محمر': 0.9 },
      synergy: {},
      negativeSynergy: {},
      warnings: [],
    },
{
      id: 'basic-yellow-57',
      name: 'أصفر أساسي 57',
      scientific_name: 'Basic Yellow 57',
      searchKeywords: ['basic yellow 57', 'ci 12719'],
      usageScope: ['hair'],
      functionalCategory: 'محسن جمالي',
      chemicalType: 'ملون شعر شبه دائم',
      benefits: { 'إعطاء لون أصفر/ذهبي': 0.9 },
      synergy: {},
      negativeSynergy: {},
      warnings: [],
    },
{
      id: 'acid-violet-43',
      name: 'بنفسجي حمضي 43',
      scientific_name: 'Acid Violet 43',
      searchKeywords: ['acid violet 43', 'ci 60730', 'ext. violet 2'],
      usageScope: ['hair'],
      functionalCategory: 'محسن جمالي',
      chemicalType: 'ملون',
      benefits: { 'معادلة اللون الأصفر': 0.95, 'للشامبو البنفسجي': 0.92 },
      synergy: {},
      negativeSynergy: {},
      warnings: [{ level: 'info', text: 'الصبغة البنفسجية الأساسية المستخدمة في "الشامبو البنفسجي" لمعادلة درجات اللون الأصفر والنحاسي في الشعر الأشقر أو الرمادي.' }],
    },
{
      id: 'basic-blue-99',
      name: 'أزرق أساسي 99',
      scientific_name: 'Basic Blue 99',
      searchKeywords: ['basic blue 99', 'ci 56059'],
      usageScope: ['hair'],
      functionalCategory: 'محسن جمالي',
      chemicalType: 'ملون شعر شبه دائم',
      benefits: { 'إعطاء لون أزرق/معادلة البرتقالي': 0.9 },
      synergy: {},
      negativeSynergy: {},
      warnings: [],
    },
{
      id: 'basic-red-51',
      name: 'أحمر أساسي 51',
      scientific_name: 'Basic Red 51',
      searchKeywords: ['basic red 51'],
      usageScope: ['hair'],
      functionalCategory: 'محسن جمالي',
      chemicalType: 'ملون شعر شبه دائم',
      benefits: { 'إعطاء لون أحمر/وردي': 0.9 },
      synergy: {},
      negativeSynergy: {},
      warnings: [],
    },
{
      id: 'hc-blue-no-2',
      name: 'HC Blue No. 2',
      scientific_name: 'HC Blue No. 2',
      searchKeywords: ['hc blue no. 2'],
      usageScope: ['hair'],
      functionalCategory: 'محسن جمالي',
      chemicalType: 'ملون شعر شبه دائم',
      benefits: { 'إعطاء لون أزرق-بنفسجي': 0.88 },
      synergy: {},
      negativeSynergy: {},
      warnings: [],
    },
{
      id: 'peg-20-glyceryl-laurate',
      name: 'لورات الجليسريل PEG-20',
      scientific_name: 'PEG-20 Glyceryl Laurate',
      searchKeywords: ['peg-20 glyceryl laurate'],
      usageScope: ['hair', 'skin'],
      functionalCategory: 'مستحلب',
      chemicalType: 'مستحلب',
      benefits: { 'مستحلب': 0.8, 'منظف لطيف': 0.75 },
      synergy: {},
      negativeSynergy: {},
      warnings: [],
    },
{
      id: 'linoleamidopropyl-pg-dimonium-chloride-phosphate',
      name: 'فوسفات كلوريد لينولياميدوبروبيل PG-ديمونيوم',
      scientific_name: 'Linoleamidopropyl PG-Dimonium Chloride Phosphate',
      searchKeywords: ['linoleamidopropyl pg-dimonium chloride phosphate'],
      usageScope: ['hair', 'skin'],
      functionalCategory: 'مكيف للشعر',
      chemicalType: 'مركب دهني فسفوري',
      benefits: { 'تكييف لطيف': 0.9, 'مضاد للتهيج': 0.85 },
      synergy: {},
      negativeSynergy: {},
      warnings: [],
    },
{
      id: 'sodium-c12-18-alkyl-sulfate',
      name: 'صوديوم C12-18 ألكيل سلفات',
      scientific_name: 'Sodium C12-18 Alkyl Sulfate',
      searchKeywords: ['sodium c12-18 alkyl sulfate'],
      usageScope: ['hair'],
      functionalCategory: 'منظف',
      chemicalType: 'منظف قوي',
      benefits: { 'تنظيف فعال': 0.85 },
      synergy: {},
      negativeSynergy: {},
      warnings: [{ level: 'caution', text: 'مشابه لـ SLS، وهو منظف قوي.' }],
    },
{
      id: 'persea-gratissima-fruit-extract',
      name: 'مستخلص فاكهة الأفوكادو',
      scientific_name: 'Persea Gratissima (Avocado) Fruit Extract',
      searchKeywords: ['avocado fruit extract', 'مستخلص الأفوكادو'],
      usageScope: ['hair'],
      functionalCategory: 'مرطب / مطري',
      chemicalType: 'مستخلص نباتي',
      benefits: { 'تغذية': 0.8, 'ترطيب': 0.85 },
      synergy: {},
      negativeSynergy: {},
      warnings: [],
    },
{
      id: 'mangifera-indica-fruit-extract',
      name: 'مستخلص فاكهة المانجو',
      scientific_name: 'Mangifera Indica (Mango) Fruit Extract',
      searchKeywords: ['mango fruit extract', 'مستخلص المانجو'],
      usageScope: ['hair'],
      functionalCategory: 'مضاد أكسدة',
      chemicalType: 'مستخلص نباتي',
      benefits: { 'مضاد أكسدة': 0.8, 'ترطيب': 0.82 },
      synergy: {},
      negativeSynergy: {},
      warnings: [],
    },
{
      id: 'passiflora-incarnata-extract',
      name: 'مستخلص زهرة الآلام',
      scientific_name: 'Passiflora Incarnata Extract',
      searchKeywords: ['passion flower extract'],
      usageScope: ['hair'],
      functionalCategory: 'ملطف ومرمم',
      chemicalType: 'مستخلص نباتي',
      benefits: { 'ملطف ومرمم': 0.85, 'مضاد أكسدة': 0.8 },
      synergy: {},
      negativeSynergy: {},
      warnings: [],
    },
{
      id: 'actinidia-chinensis-fruit-extract',
      name: 'مستخلص فاكهة الكيوي',
      scientific_name: 'Actinidia Chinensis (Kiwi) Fruit Extract',
      searchKeywords: ['kiwi fruit extract'],
      usageScope: ['hair'],
      functionalCategory: 'مضاد أكسدة',
      chemicalType: 'مستخلص نباتي',
      benefits: { 'مضاد أكسدة': 0.85, 'غني بفيتامين C': 0.8 },
      synergy: {},
      negativeSynergy: {},
      warnings: [],
    },
{
      id: 'aloe-barbadensis-flower-extract',
      name: 'مستخلص زهرة الألوفيرا',
      scientific_name: 'Aloe Barbadensis Flower Extract',
      searchKeywords: ['aloe flower extract'],
      usageScope: ['hair'],
      functionalCategory: 'ملطف ومرمم',
      chemicalType: 'مستخلص نباتي',
      benefits: { 'مضاد أكسدة': 0.8, 'ملطف ومرمم': 0.85 },
      synergy: {},
      negativeSynergy: {},
      warnings: [],
    },
{
      id: 'coccinia-indica-fruit-extract',
      name: 'مستخلص فاكهة الكوسينيا الهندية',
      scientific_name: 'Coccinia Indica Fruit Extract',
      searchKeywords: ['ivy gourd extract'],
      usageScope: ['hair'],
      functionalCategory: 'مكيف للشعر',
      chemicalType: 'مستخلص نباتي',
      benefits: { 'تكييف': 0.8, 'مضاد أكسدة': 0.75 },
      synergy: {},
      negativeSynergy: {},
      warnings: [],
    },
{
      id: 'solanum-melongena-fruit-extract',
      name: 'مستخلص فاكهة الباذنجان',
      scientific_name: 'Solanum Melongena (Eggplant) Fruit Extract',
      searchKeywords: ['eggplant fruit extract'],
      usageScope: ['hair'],
      functionalCategory: 'مضاد أكسدة',
      chemicalType: 'مستخلص نباتي',
      benefits: { 'مضاد أكسدة': 0.8, 'تكييف': 0.75 },
      synergy: {},
      negativeSynergy: {},
      warnings: [],
    },
{
      id: 'ocimum-sanctum-leaf-extract',
      name: 'مستخلص أوراق الريحان المقدس',
      scientific_name: 'Ocimum Sanctum Leaf Extract',
      searchKeywords: ['holy basil extract', 'tulsi'],
      usageScope: ['hair'],
      functionalCategory: 'مكون فعال',
      chemicalType: 'مستخلص نباتي',
      benefits: { 'مضاد للميكروبات': 0.85, 'مضاد للالتهابات': 0.82 },
      synergy: {},
      negativeSynergy: {},
      warnings: [],
    },
{
      id: 'melia-azadirachta-leaf-extract',
      name: 'مستخلص أوراق النيم',
      scientific_name: 'Melia Azadirachta Leaf Extract',
      searchKeywords: ['neem leaf extract'],
      usageScope: ['hair'],
      functionalCategory: 'مكون فعال',
      chemicalType: 'مستخلص نباتي',
      benefits: { 'مضاد للميكروبات': 0.88, 'صحة فروة الرأس': 0.85 },
      synergy: {},
      negativeSynergy: {},
      warnings: [],
    },
{
      id: 'corallina-officinalis-extract',
      name: 'مستخلص الطحالب المرجانية',
      scientific_name: 'Corallina Officinalis Extract',
      searchKeywords: ['corallina officinalis extract'],
      usageScope: ['hair'],
      functionalCategory: 'مكون فعال',
      chemicalType: 'مستخلص طحالب',
      benefits: { 'غني بالمعادن': 0.8, 'تنشيط فروة الرأس': 0.75 },
      synergy: {},
      negativeSynergy: {},
      warnings: [],
    },
{
      id: 'sodium-c10-16-alkyl-glucoside-crosspolymer',
      name: 'صوديوم C10-16 ألكيل جلوكوسايد كروسبوليمر',
      scientific_name: 'Sodium C10-16 Alkyl Glucoside Crosspolymer',
      searchKeywords: ['sodium c10-16 alkyl glucoside crosspolymer'],
      usageScope: ['hair'],
      functionalCategory: 'منظف',
      chemicalType: 'منظف لطيف جدا',
      benefits: { 'تنظيف لطيف': 0.88 },
      synergy: {},
      negativeSynergy: {},
      warnings: [],
    },
{
      id: 'sodium-lauroyl-lactylate',
      name: 'صوديوم لورويل لاكتيلات',
      scientific_name: 'Sodium Lauroyl Lactylate',
      searchKeywords: ['sodium lauroyl lactylate'],
      usageScope: ['hair'],
      functionalCategory: 'منظف',
      chemicalType: 'منظف لطيف',
      benefits: { 'تنظيف لطيف': 0.85, 'مستحلب': 0.8, 'مضاد للميكروبات': 0.75 },
      synergy: {},
      negativeSynergy: {},
      warnings: [],
    },
{
      id: 'sodium-cocoyl-hydrolyzed-amaranth-protein',
      name: 'بروتين القطيفة المهدرج صوديوم كوكويل',
      scientific_name: 'Sodium Cocoyl Hydrolyzed Amaranth Protein',
      searchKeywords: ['sodium cocoyl hydrolyzed amaranth protein'],
      usageScope: ['hair'],
      functionalCategory: 'منظف',
      chemicalType: 'منظف لطيف',
      benefits: { 'تنظيف لطيف': 0.9, 'إضافة حجم': 0.8 },
      synergy: {},
      negativeSynergy: {},
      warnings: [],
    },
{
      id: 'glyceryl-caprylate-caprate',
      name: 'كابريلات/كبرات الجليسريل',
      scientific_name: 'Glyceryl Caprylate/Caprate',
      searchKeywords: ['glyceryl caprylate/caprate'],
      usageScope: ['hair'],
      functionalCategory: 'مرطب / مطري',
      chemicalType: 'عامل إعادة ترطيب',
      benefits: { 'إعادة ترطيب': 0.8, 'مستحلب': 0.85 },
      synergy: {},
      negativeSynergy: {},
      warnings: [],
    },
{
      id: 'polyglyceryl-10-laurate',
      name: 'بولي جليسريل-10 لوريت',
      scientific_name: 'Polyglyceryl-10 Laurate',
      searchKeywords: ['polyglyceryl-10 laurate'],
      usageScope: ['hair'],
      functionalCategory: 'مذوب',
      chemicalType: 'مستحلب',
      benefits: { 'مذوب لطيف': 0.9, 'منظف ثانوي': 0.8 },
      synergy: {},
      negativeSynergy: {},
      warnings: [],
    },
{
      id: 'sodium-methyl-2-sulfolaurate',
      name: 'صوديوم ميثيل 2-سلفولورات',
      scientific_name: 'Sodium Methyl 2-Sulfolaurate',
      searchKeywords: ['sodium methyl 2-sulfolaurate'],
      usageScope: ['hair'],
      functionalCategory: 'منظف',
      chemicalType: 'منظف لطيف',
      benefits: { 'تنظيف لطيف': 0.88, 'رغوة جيدة': 0.85 },
      synergy: {},
      negativeSynergy: {},
      warnings: [],
    },
{
      id: 'disodium-2-sulfolaurate',
      name: 'ثنائي الصوديوم 2-سلفولورات',
      scientific_name: 'Disodium 2-Sulfolaurate',
      searchKeywords: ['disodium 2-sulfolaurate'],
      usageScope: ['hair'],
      functionalCategory: 'منظف',
      chemicalType: 'منظف لطيف',
      benefits: { 'منظف ثانوي لطيف': 0.85 },
      synergy: {},
      negativeSynergy: {},
      warnings: [],
    },
{
      id: 'menthol',
      name: 'منثول',
      scientific_name: 'Menthol',
      searchKeywords: ['menthol', 'منثول'],
      usageScope: ['hair', 'skin'],
      functionalCategory: 'مكون فعال',
      chemicalType: 'عامل تبريد',
      benefits: { 'إحساس قوي بالبرودة والانتعاش': 0.95, 'تخفيف الحكة': 0.88, 'تنشيط الدورة الدموية': 0.8 },
      synergy: {},
      negativeSynergy: {},
      warnings: [{ level: 'caution', text: 'قد يكون مهيجا للبشرة الحساسة.' }],
    },
{
      id: 'mentha-arvensis-leaf-oil',
      name: 'زيت النعناع الحقلي',
      scientific_name: 'Mentha Arvensis Leaf Oil',
      searchKeywords: ['mentha arvensis', 'corn mint oil', 'wild mint'],
      usageScope: ['hair'],
      functionalCategory: 'مكون فعال',
      chemicalType: 'زيت عطري',
      benefits: { 'إحساس بالبرودة': 0.9, 'تنشيط فروة الرأس': 0.85 },
      synergy: {},
      negativeSynergy: {},
      warnings: [],
    },
{
      id: 'vanillyl-butyl-ether',
      name: 'فانيليل بوتيل إيثر',
      scientific_name: 'Vanillyl Butyl Ether',
      searchKeywords: ['vanillyl butyl ether', 'hotflux'],
      usageScope: ['hair'],
      functionalCategory: 'مكون فعال',
      chemicalType: 'عامل تسخين',
      benefits: { 'إحساس بالدفء': 0.9, 'تنشيط الدورة الدموية': 0.85 },
      synergy: {},
      negativeSynergy: {},
      warnings: [{ level: 'info', text: 'يستخدم في الشامبوهات المنشطة لإعطاء إحساس بالحرارة اللطيفة.' }],
    },
{
      id: 'hydrolyzed-corn-starch',
      name: 'نشا الذرة المهدرج',
      scientific_name: 'Hydrolyzed Corn Starch',
      searchKeywords: ['hydrolyzed corn starch'],
      usageScope: ['hair'],
      functionalCategory: 'مكيف للشعر',
      chemicalType: 'مكيف',
      benefits: { 'ترطيب': 0.8, 'تكييف': 0.82 },
      synergy: {},
      negativeSynergy: {},
      warnings: [],
    },
{
      id: 'tilia-tomentosa-bud-extract',
      name: 'مستخلص براعم الزيزفون',
      scientific_name: 'Tilia Tomentosa Bud Extract',
      searchKeywords: ['tilia tomentosa', 'linden bud extract'],
      usageScope: ['hair'],
      functionalCategory: 'مكيف للشعر',
      chemicalType: 'مستخلص نباتي',
      benefits: { 'تكوين طبقة واقية': 0.88, 'إضافة حجم': 0.85, 'تقليل الكهرباء الساكنة': 0.8 },
      synergy: {},
      negativeSynergy: {},
      warnings: [],
    },
{
      id: 'olea-europaea-leaf-extract',
      name: 'مستخلص أوراق الزيتون',
      scientific_name: 'Olea Europaea (Olive) Leaf Extract',
      searchKeywords: ['olive leaf extract', 'مستخلص أوراق الزيتون'],
      usageScope: ['hair'],
      functionalCategory: 'مضاد أكسدة',
      chemicalType: 'مستخلص نباتي',
      benefits: { 'مضاد أكسدة': 0.9, 'مضاد للالتهابات': 0.85 },
      synergy: {},
      negativeSynergy: {},
      warnings: [],
    },
{
      id: 'apium-graveolens-seed-extract',
      name: 'مستخلص بذور الكرفس',
      scientific_name: 'Apium Graveolens (Celery) Seed Extract',
      searchKeywords: ['celery seed extract', 'apium graveolens'],
      usageScope: ['hair'],
      functionalCategory: 'مكون فعال',
      chemicalType: 'مستخلص نباتي',
      benefits: { 'صحة فروة الرأس': 0.85, 'تقليل الحكة والقشرة': 0.82 },
      synergy: {},
      negativeSynergy: {},
      warnings: [],
    },
{
      id: 'pisum-sativum-sprout-extract',
      name: 'مستخلص براعم البازلاء',
      scientific_name: 'Pisum Sativum (Pea) Sprout Extract',
      searchKeywords: ['pisum sativum sprout extract', 'ana-gain'],
      usageScope: ['hair'],
      functionalCategory: 'مكون فعال',
      chemicalType: 'مستخلص نباتي',
      benefits: { 'تحفيز نمو الشعر': 0.88, 'تقليل تساقط الشعر': 0.85 },
      synergy: {},
      negativeSynergy: {},
      warnings: [{ level: 'info', text: 'يعرف تجاريا باسم AnaGain™، يعمل على تحفيز جزيئات معينة في بصيلات الشعر لإعادة تنشيط نمو الشعر.' }],
    },
{
      id: 'larix-europaea-wood-extract',
      name: 'مستخلص خشب الأرز الأوروبي',
      scientific_name: 'Larix Europaea Wood Extract',
      searchKeywords: ['larix europaea wood extract', 'redensyl component'],
      usageScope: ['hair'],
      functionalCategory: 'مكون فعال',
      chemicalType: 'مستخلص نباتي',
      benefits: { 'تنشيط الخلايا الجذعية للبصيلات': 0.9 },
      synergy: {},
      negativeSynergy: {},
      warnings: [{ level: 'info', text: 'أحد المكونات الرئيسية في مركب Redensyl® الحائز على جوائز.' }],
    },
{
      id: 'zinc-chloride',
      name: 'كلوريد الزنك',
      scientific_name: 'Zinc Chloride',
      searchKeywords: ['zinc chloride', 'كلوريد الزنك'],
      usageScope: ['hair'],
      functionalCategory: 'مكون فعال',
      chemicalType: 'ملح معدني',
      benefits: { 'يدعم صحة فروة الرأس': 0.8 },
      synergy: {},
      negativeSynergy: {},
      warnings: [],
    },
{
      id: 'polyglyceryl-3-diisostearate',
      name: 'بولي جليسريل-3 ثنائي الأيزوستيرات',
      scientific_name: 'Polyglyceryl-3 Diisostearate',
      searchKeywords: ['polyglyceryl-3 diisostearate'],
      usageScope: ['hair'],
      functionalCategory: 'مستحلب',
      chemicalType: 'مستحلب',
      benefits: { 'مستحلب': 0.85 },
      synergy: {},
      negativeSynergy: {},
      warnings: [],
    },
{
      id: 'polyglyceryl-4-caprate',
      name: 'بولي جليسريل-4 كابرات',
      scientific_name: 'Polyglyceryl-4 Caprate',
      searchKeywords: ['polyglyceryl-4 caprate'],
      usageScope: ['hair'],
      functionalCategory: 'مذوب',
      chemicalType: 'مستحلب',
      benefits: { 'مذوب لطيف': 0.9, 'عامل إعادة ترطيب': 0.8 },
      synergy: {},
      negativeSynergy: {},
      warnings: [],
    },
{
      id: 'polyglyceryl-6-caprylate',
      name: 'بولي جليسريل-6 كابريلات',
      scientific_name: 'Polyglyceryl-6 Caprylate',
      searchKeywords: ['polyglyceryl-6 caprylate'],
      usageScope: ['hair'],
      functionalCategory: 'مذوب',
      chemicalType: 'مستحلب',
      benefits: { 'مذوب لطيف جدا': 0.92, 'منظف ثانوي': 0.8 },
      synergy: {},
      negativeSynergy: {},
      warnings: [],
    },
{
      id: 'polyglyceryl-3-cocoate',
      name: 'بولي جليسريل-3 كوكوات',
      scientific_name: 'Polyglyceryl-3 Cocoate',
      searchKeywords: ['polyglyceryl-3 cocoate'],
      usageScope: ['hair'],
      functionalCategory: 'مستحلب',
      chemicalType: 'مستحلب',
      benefits: { 'مستحلب لطيف': 0.88 },
      synergy: {},
      negativeSynergy: {},
      warnings: [],
    },
{
      id: 'hydroxypropyl-oxidized-starch-pg-trimonium-chloride',
      name: 'كلوريد النشا المؤكسد هيدروكسي بروبيل PG-تريمونيوم',
      scientific_name: 'Hydroxypropyl Oxidized Starch PG-Trimonium Chloride',
      searchKeywords: ['hydroxypropyl oxidized starch pg-trimonium chloride'],
      usageScope: ['hair'],
      functionalCategory: 'مكيف للشعر',
      chemicalType: 'بوليمر طبيعي',
      benefits: { 'تكييف': 0.85, 'فك تشابك': 0.82 },
      synergy: {},
      negativeSynergy: {},
      warnings: [],
    },
{
      id: 'starch-hydroxypropyltrimonium-chloride',
      name: 'كلوريد النشا هيدروكسي بروبيل تريمونيوم',
      scientific_name: 'Starch Hydroxypropyltrimonium Chloride',
      searchKeywords: ['starch hydroxypropyltrimonium chloride'],
      usageScope: ['hair'],
      functionalCategory: 'مكيف للشعر',
      chemicalType: 'بوليمر طبيعي',
      benefits: { 'تكييف لطيف': 0.88, 'تقليل الهيشان': 0.85, 'مشتق طبيعي': 0.9 },
      synergy: {},
      negativeSynergy: {},
      warnings: [{ level: 'info', text: 'مكيف طبيعي مشتق من النشا، بديل جيد للكواتس الصناعية.' }],
    },
{
      id: 'peg-15-cocopolyamine',
      name: 'PEG-15 كوكوبوليامين',
      scientific_name: 'PEG-15 Cocopolyamine',
      searchKeywords: ['peg-15 cocopolyamine'],
      usageScope: ['hair'],
      functionalCategory: 'مكيف للشعر',
      chemicalType: 'مضاد للكهرباء الساكنة',
      benefits: { 'مضاد للكهرباء الساكنة': 0.85 },
      synergy: {},
      negativeSynergy: {},
      warnings: [],
    },
{
      id: 'peg-ppg-20-6-dimethicone',
      name: 'PEG/PPG-20/6 دايميثيكون',
      scientific_name: 'PEG/PPG-20/6 Dimethicone',
      searchKeywords: ['peg/ppg-20/6 dimethicone'],
      usageScope: ['hair'],
      functionalCategory: 'مكيف للشعر',
      chemicalType: 'سيليكون قابل للذوبان في الماء',
      benefits: { 'تكييف خفيف': 0.8, 'إضافة لمعان': 0.82 },
      synergy: {},
      negativeSynergy: {},
      warnings: [],
    },
{
      id: 'olea-europaea-fruit-extract',
      name: 'مستخلص فاكهة الزيتون',
      scientific_name: 'Olea Europaea (Olive) Fruit Extract',
      searchKeywords: ['olive fruit extract'],
      usageScope: ['hair'],
      functionalCategory: 'مضاد أكسدة',
      chemicalType: 'مستخلص نباتي',
      benefits: { 'مضاد أكسدة': 0.88, 'تكييف': 0.8 },
      synergy: {},
      negativeSynergy: {},
      warnings: [],
    },
{
      id: 'ci-47005-yellow-10',
      name: 'CI 47005 / أصفر 10',
      scientific_name: 'CI 47005',
      searchKeywords: ['ci 47005', 'yellow 10'],
      usageScope: ['hair'],
      functionalCategory: 'محسن جمالي',
      chemicalType: 'ملون صناعي',
      benefits: { 'يعطي لونا أصفر مخضر': 1.0 },
      synergy: {},
      negativeSynergy: {},
      warnings: [],
    },
{
      id: 'ci-17200-red-33',
      name: 'CI 17200 / أحمر 33',
      scientific_name: 'CI 17200',
      searchKeywords: ['ci 17200', 'red 33'],
      usageScope: ['hair'],
      functionalCategory: 'محسن جمالي',
      chemicalType: 'ملون صناعي',
      benefits: { 'يعطي لونا أحمر': 1.0 },
      synergy: {},
      negativeSynergy: {},
      warnings: [],
    },
{
      id: 'ci-42053-green-3',
      name: 'CI 42053 / أخضر 3',
      scientific_name: 'CI 42053',
      searchKeywords: ['ci 42053', 'green 3'],
      usageScope: ['hair'],
      functionalCategory: 'محسن جمالي',
      chemicalType: 'ملون صناعي',
      benefits: { 'يعطي لونا أخضر': 1.0 },
      synergy: {},
      negativeSynergy: {},
      warnings: [],
    },
{
      id: 'ci-14700-red-4',
      name: 'CI 14700 / أحمر 4',
      scientific_name: 'CI 14700',
      searchKeywords: ['ci 14700', 'red 4'],
      usageScope: ['hair'],
      functionalCategory: 'محسن جمالي',
      chemicalType: 'ملون صناعي',
      benefits: { 'يعطي لونا أحمر': 1.0 },
      synergy: {},
      negativeSynergy: {},
      warnings: [],
    },
{
      id: 'phenylethyl-resorcinol',
      name: 'فينيل إيثيل ريزورسينول',
      scientific_name: 'Phenylethyl Resorcinol',
      searchKeywords: ['phenylethyl resorcinol', 'symwhite 377', 'فينيل إيثيل ريزورسينول', 'فينيل إيثيل ريسورسينول'],
      usageScope: ['skin'],
      functionalCategory: 'مكون فعال',
      chemicalType: 'مثبط للتصبغ',
      benefits: { 'تفتيح وتوحيد اللون': 0.98, 'مثبط قوي للتيروزيناز': 0.95, 'مضاد أكسدة': 0.85 },
      synergy: { 'niacinamide': 1.1 },
      negativeSynergy: {},
      warnings: [
        { level: 'info', text: 'يعرف تجاريا باسم SymWhite 377®. أحد أقوى مثبطات إنتاج الميلانين، مستوحى من جزيئات طبيعية في لحاء الصنوبر.' },
      ],
    },
{
      id: 'n-acetyl-glucosamine',
      name: 'إن-أسيتيل جلوكوزامين (NAG)',
      scientific_name: 'Acetyl Glucosamine',
      searchKeywords: ['n-acetyl glucosamine', 'nag', 'acetyl glucosamine'],
      usageScope: ['skin'],
      functionalCategory: 'مكون فعال',
      chemicalType: 'حمض أميني سكري',
      benefits: { 'تفتيح وتوحيد اللون': 0.85, 'لبنة بناء لحمض الهيالورونيك': 0.9, 'تقشير لطيف': 0.8 },
      synergy: { 'niacinamide': 1.1 },
      negativeSynergy: {},
      warnings: [{ level: 'info', text: 'يعمل بشكل تآزري ممتاز مع النياسيناميد لتفتيح التصبغات.' }],
    },
{
      id: 'kojic-acid-dipalmitate',
      name: 'ثنائي بالميتات حمض الكوجيك',
      scientific_name: 'Kojic Dipalmitate',
      searchKeywords: ['kojic dipalmitate', 'stable kojic acid'],
      usageScope: ['skin'],
      functionalCategory: 'مكون فعال',
      chemicalType: 'مثبط للتصبغ',
      benefits: { 'تفتيح وتوحيد اللون': 0.85, 'أكثر استقرارا من حمض الكوجيك': 0.9 },
      synergy: {},
      negativeSynergy: {},
      warnings: [{ level: 'info', text: 'شكل استر ومستقر من حمض الكوجيك، قابل للذوبان في الزيت وأقل تهيجا.' }],
    },
{
      id: 'dimethylmethoxy-chromanyl-palmitate',
      name: 'بالميتات ثنائي ميثيل ميثوكسي كرومانيل',
      scientific_name: 'Dimethylmethoxy Chromanyl Palmitate',
      searchKeywords: ['dimethylmethoxy chromanyl palmitate', 'chromabright'],
      usageScope: ['skin'],
      functionalCategory: 'مكون فعال',
      chemicalType: 'مثبط للتصبغ',
      benefits: { 'تفتيح وتوحيد اللون': 0.88, 'حماية من أضرار الشمس': 0.8 },
      synergy: {},
      negativeSynergy: {},
      warnings: [{ level: 'info', text: 'يعرف تجاريا باسم Chromabright™. جزيء تفتيح لا يسبب حساسية للضوء.' }],
    },
{
      id: 'dipeptide-diaminobutyroyl-benzylamide-diacetate',
      name: 'ثنائي الببتيد ديامينوبوتيرويل بنزيلاميد ثنائي الأسيتات',
      scientific_name: 'Dipeptide Diaminobutyroyl Benzylamide Diacetate',
      searchKeywords: ['dipeptide diaminobutyroyl benzylamide diacetate', 'syn-ake'],
      usageScope: ['skin'],
      functionalCategory: 'مكون فعال',
      chemicalType: 'ببتيد عصبي',
      benefits: { 'تقليل التجاعيد التعبيرية': 0.9, 'تأثير شبيه بسم الأفعى': 0.88 },
      synergy: {},
      negativeSynergy: {},
      warnings: [{ level: 'info', text: 'يعرف تجاريا باسم SYN®-AKE. ببتيد صغير يحاكي تأثير Waglerin-1، وهو ببتيد موجود في سم أفعى المعبد، لإرخاء العضلات السطحية.' }],
    },
{
      id: 'palmitoyl-tripeptide-5',
      name: 'بالميتويل ثلاثي الببتيد-5',
      scientific_name: 'Palmitoyl Tripeptide-5',
      searchKeywords: ['palmitoyl tripeptide-5', 'syn-coll'],
      usageScope: ['skin'],
      functionalCategory: 'مكون فعال',
      chemicalType: 'ببتيد',
      benefits: { 'تحفيز إنتاج الكولاجين': 0.92, 'تحسين صلابة البشرة': 0.88 },
      synergy: {},
      negativeSynergy: {},
      warnings: [{ level: 'info', text: 'يعرف تجاريا باسم SYN®-COLL. ببتيد صغير يحاكي آلية الجسم لإنتاج الكولاجين عبر TGF-β.' }],
    },
{
      id: 'trifluoroacetyl-tripeptide-2',
      name: 'ثلاثي فلورو أسيتيل ثلاثي الببتيد-2',
      scientific_name: 'Trifluoroacetyl Tripeptide-2',
      searchKeywords: ['trifluoroacetyl tripeptide-2', 'progeline'],
      usageScope: ['skin'],
      functionalCategory: 'مكون فعال',
      chemicalType: 'ببتيد',
      benefits: { 'مقاومة ترهل الجلد': 0.9, 'تحسين صلابة البشرة': 0.88, 'يقلل من البروجيرين': 0.92 },
      synergy: {},
      negativeSynergy: {},
      warnings: [{ level: 'info', text: 'يعرف تجاريا باسم Progeline™. يقلل من إنتاج البروجيرين، وهو بروتين سام يساهم في شيخوخة الخلايا.' }],
    },
{
      id: 'acetyl-tetrapeptide-5',
      name: 'أسيتيل رباعي الببتيد-5',
      scientific_name: 'Acetyl Tetrapeptide-5',
      searchKeywords: ['acetyl tetrapeptide-5', 'eyeseril'],
      usageScope: ['skin'],
      functionalCategory: 'مكون فعال',
      chemicalType: 'ببتيد',
      benefits: { 'تقليل الانتفاخ تحت العين': 0.9, 'تقليل الهالات السوداء': 0.8, 'تحسين مرونة الجلد': 0.85 },
      synergy: {},
      negativeSynergy: {},
      warnings: [{ level: 'info', text: 'يعرف تجاريا باسم Eyeseryl®. ببتيد مصمم خصيصا لمنطقة حول العين.' }],
    },
{
      id: 'palmitoyl-pentapeptide-4',
      name: 'بالميتويل خماسي الببتيد-4',
      scientific_name: 'Palmitoyl Pentapeptide-4',
      searchKeywords: ['palmitoyl pentapeptide-4', 'matrixyl', 'ماتريكسيل'],
      usageScope: ['skin'],
      functionalCategory: 'مكون فعال',
      chemicalType: 'ببتيد',
      benefits: { 'تحفيز الكولاجين': 0.9, 'تقليل التجاعيد': 0.88 },
      synergy: {},
      negativeSynergy: {},
      warnings: [{ level: 'info', text: 'يعرف تجاريا باسم Matrixyl®. أحد أشهر الببتيدات وأكثرها دراسة في مجال مكافحة الشيخوخة.' }],
    },
{
      id: 'bidens-pilosa-extract',
      name: 'مستخلص عشبة البيدنس بيلوسا',
      scientific_name: 'Bidens Pilosa Extract',
      searchKeywords: ['bidens pilosa extract', 'bio-retinol', 'revincage'],
      usageScope: ['skin'],
      functionalCategory: 'مكون فعال',
      chemicalType: 'مركب نباتي',
      benefits: { 'تأثير شبيه بالريتينويد': 0.85, 'مكافحة الشيخوخة': 0.8, 'ملطف ومرمم': 0.82 },
      synergy: {},
      negativeSynergy: {},
      warnings: [{ level: 'info', text: 'يعرف بـ "Bio-Retinol". مستخلص نباتي يتفاعل مع مستقبلات الريتينويد في الجلد دون أن يكون ريتينويدا حقيقيا.' }],
    },
{
      id: 'moth-bean-extract',
      name: 'مستخلص حبوب الموث',
      scientific_name: 'Vigna Aconitifolia Seed Extract',
      searchKeywords: ['vigna aconitifolia', 'moth bean', 'vit-a-like'],
      usageScope: ['skin'],
      functionalCategory: 'مكون فعال',
      chemicalType: 'مركب نباتي',
      benefits: { 'بديل نباتي للريتينول': 0.8, 'تحفيز تجدد الخلايا': 0.82, 'مكافحة الشيخوخة': 0.78 },
      synergy: {},
      negativeSynergy: {},
      warnings: [{ level: 'info', text: 'مستخلص نباتي غني بالفيتوستيرولات التي تظهر نشاطا شبيها بالريتينول.' }],
    },
{
      id: 'euk-134',
      name: 'EUK-134',
      scientific_name: 'Ethylbisiminomethylguaiacol Manganese Chloride',
      searchKeywords: ['euk-134', 'ethylbisiminomethylguaiacol manganese chloride'],
      usageScope: ['skin'],
      functionalCategory: 'مضاد أكسدة',
      chemicalType: 'مركب محاكي للإنزيم',
      benefits: { 'مضاد أكسدة ذاتي التجدد': 0.98, 'يحاكي إنزيمات الجسم': 0.95, 'حماية من أضرار الشمس': 0.9 },
      synergy: {},
      negativeSynergy: { 'glycolic-acid': { reason: 'الأحماض القوية والمباشرة يمكن أن تفكك المركب المعدني.' }, 'vitamin-c': { reason: 'الأحماض القوية والمباشرة يمكن أن تفكك المركب المعدني.' } },
      warnings: [{ level: 'info', text: 'جزيء رائع يحاكي إنزيمي Superoxide Dismutase و Catalase، مما يسمح له بتحييد الجذور الحرة بشكل مستمر دون أن يُستهلك.' }],
    },
{
      id: 'bifida-ferment-lysate',
      name: 'خميرة بيفيدا',
      scientific_name: 'Bifida Ferment Lysate',
      searchKeywords: ['bifida ferment lysate', 'probiotic', 'bifida', 'بيفيدا'],
      usageScope: ['skin'],
      functionalCategory: 'دعم حاجز البشرة',
      chemicalType: 'مكون مخمر (بروبيوتيك)',
      benefits: { 'إصلاح الحمض النووي': 0.9, 'دعم حاجز البشرة': 0.92, 'تقليل حساسية البشرة': 0.88 },
      synergy: {},
      negativeSynergy: {},
      warnings: [{ level: 'info', text: 'مكون بروبيوتيك شهير (المكون الرئيسي في Estée Lauder Advanced Night Repair)، يساعد على حماية البشرة من الأضرار البيئية وتعزيز إصلاحها الذاتي.' }],
    },
{
      id: 'galactomyces-ferment-filtrate',
      name: 'مرشح خميرة الجلاكتوميسيس',
      scientific_name: 'Galactomyces Ferment Filtrate',
      searchKeywords: ['galactomyces ferment filtrate', 'pitera', 'جلاكتوميسيس'],
      usageScope: ['skin'],
      functionalCategory: 'مكون فعال',
      chemicalType: 'مكون مخمر',
      benefits: { 'تفتيح وتوحيد اللون': 0.88, 'تحسين ملمس البشرة': 0.85, 'ترطيب': 0.9 },
      synergy: {},
      negativeSynergy: {},
      warnings: [{ level: 'info', text: 'يعرف تجاريا باسم Pitera™ (SK-II). سائل غني بالفيتامينات والأحماض الأمينية والمعادن، ناتج عن تخمير الخميرة.' }],
    },
{
      id: 'rice-ferment-filtrate',
      name: 'مرشح خميرة الأرز (ساكي)',
      scientific_name: 'Sake / Rice Ferment Filtrate',
      searchKeywords: ['rice ferment filtrate', 'sake', 'ساكي', 'خميرة الأرز'],
      usageScope: ['skin'],
      functionalCategory: 'مكون فعال',
      chemicalType: 'مكون مخمر',
      benefits: { 'ترطيب': 0.9, 'تفتيح وتوحيد اللون': 0.85, 'غني بالأحماض الأمينية': 0.88 },
      synergy: {},
      negativeSynergy: {},
      warnings: [],
    },
{
      id: 'lactobacillus-ferment-lysate',
      name: 'خميرة اللاكتوباسيلس',
      scientific_name: 'Lactobacillus Ferment Lysate',
      searchKeywords: ['lactobacillus ferment lysate', 'probiotic', 'لاكتوباسيلس'],
      usageScope: ['skin'],
      functionalCategory: 'دعم حاجز البشرة',
      chemicalType: 'مكون مخمر (بروبيوتيك)',
      benefits: { 'دعم ميكروبيوم البشرة': 0.92, 'ملطف ومرمم': 0.9, 'مضاد للميكروبات': 0.8 },
      synergy: {},
      negativeSynergy: {},
      warnings: [],
    },
{ id: 'oleuropein', name: 'أوروبيين', scientific_name: 'Oleuropein', searchKeywords: ['oleuropein', 'olive leaf active'], usageScope: ['skin'], functionalCategory: 'مضاد أكسدة', chemicalType: 'مركب نباتي', benefits: { 'مضاد أكسدة': 0.9, 'مضاد للالتهابات': 0.88 }, synergy: {}, negativeSynergy: {}, warnings: [] },
{ id: 'magnolol-honokiol', name: 'ماغنولول/هونوكيول', scientific_name: 'Magnolol, Honokiol', searchKeywords: ['magnolia bark extract actives'], usageScope: ['skin'], functionalCategory: 'مكون فعال', chemicalType: 'مركب نباتي', benefits: { 'مضاد للميكروبات (حب الشباب)': 0.88, 'مضاد للالتهابات': 0.9 }, synergy: {}, negativeSynergy: {}, warnings: [] },
{ id: 'ellagic-acid', name: 'حمض الإيلاجيك', scientific_name: 'Ellagic Acid', searchKeywords: ['ellagic acid', 'pomegranate active'], usageScope: ['skin'], functionalCategory: 'مضاد أكسدة', chemicalType: 'مركب نباتي', benefits: { 'مضاد أكسدة': 0.9, 'تفتيح وتوحيد اللون': 0.8 }, synergy: {}, negativeSynergy: {}, warnings: [] },
{ id: 'carnitine', name: 'كارنيتين', scientific_name: 'Carnitine', searchKeywords: ['carnitine', 'l-carnitine'], usageScope: ['skin'], functionalCategory: 'مكون فعال', chemicalType: 'حمض أميني', benefits: { 'تنظيم الإفرازات الدهنية': 0.8, 'تقشير لطيف': 0.75 }, synergy: {}, negativeSynergy: {}, warnings: [] },
{ id: 'betaine-salicylate', name: 'ساليسيلات البيتين', scientific_name: 'Betaine Salicylate', searchKeywords: ['betaine salicylate'], usageScope: ['skin'], functionalCategory: 'مقشر', chemicalType: 'حمض بيتا هيدروكسي (BHA)', benefits: { 'تقشير لطيف': 0.9, 'مكافحة حب الشباب': 0.85 }, synergy: {}, negativeSynergy: {}, warnings: [] },
{
    id: 'tocopherol',
    name: 'توكوفيرول (فيتامين E النقي)',
    scientific_name: 'Tocopherol',
    searchKeywords: ['tocopherol', 'vitamin e', 'فيتامين هـ', 'توكوفيرول'],
    usageScope: ['skin', 'hair'],
    functionalCategory: 'مضاد أكسدة',
    chemicalType: 'فيتامين',
    benefits: { 'مضاد أكسدة': 0.95, 'دعم حاجز البشرة': 0.88, 'حماية من أضرار الشمس': 0.85, 'ترطيب': 0.8 },
    synergy: { 'vitamin-c': 1.1, 'ferulic-acid': 1.1 },
    negativeSynergy: {},
    warnings: [{ level: 'info', text: 'الشكل النقي والأكثر فعالية بيولوجيا لفيتامين E. مضاد أكسدة قوي قابل للذوبان في الزيت.' }],
  },
{
    id: 'urea',
    name: 'اليوريا',
    scientific_name: 'Urea',
    searchKeywords: ['urea', 'carbamide', 'يوريا', 'كارbamيد'],
    usageScope: ['skin'],
    functionalCategory: 'مرطب / مطري',
    chemicalType: 'مرطب (NMF)',
    benefits: { 'ترطيب مكثف': 0.98, 'دعم حاجز البشرة': 0.9, 'تقشير لطيف (حال للتقرن)': 0.85 },
    synergy: { 'lactic-acid': 1.05 },
    negativeSynergy: {},
    warnings: [{ level: 'info', text: 'جزء من عامل الترطيب الطبيعي للبشرة (NMF). بتركيزات منخفضة (<10%) يعمل كمرطب، وبتركيزات عالية (>10%) يعمل كمقشر فعال.' }],
  },
{
    id: 'propanediol',
    name: 'بروبانيديول',
    scientific_name: 'Propanediol',
    searchKeywords: ['propanediol', 'zemaea', 'corn-derived glycol', 'بروبانيديول'],
    usageScope: ['skin', 'hair'],
    functionalCategory: 'مذيب',
    chemicalType: 'مرطب',
    benefits: { 'مرطب': 0.85, 'مذيب فعال': 0.9, 'تحسين امتصاص المكونات': 0.88, 'بديل طبيعي للبروبيلين جلايكول': 0.92 },
    synergy: {},
    negativeSynergy: {},
    warnings: [{ level: 'info', text: 'غالبا ما يكون مشتقا من الذرة، ويعتبر بديلا أكثر استدامة ولطفا للبروبيلين جلايكول.' }],
  },
{
    id: 'propylene-glycol',
    name: 'بروبيلين جلايكول',
    scientific_name: 'Propylene Glycol',
    searchKeywords: ['propylene glycol', 'pg', 'بروبيلين جلايكول'],
    usageScope: ['skin', 'hair'],
    functionalCategory: 'مذيب',
    chemicalType: 'مرطب',
    benefits: { 'مرطب': 0.8, 'مذيب': 0.95, 'تحسين امتصاص المكونات': 0.9, 'يقلل لزوجة المنتج': 0.85 },
    synergy: {},
    negativeSynergy: {},
    warnings: [{ level: 'caution', text: 'على الرغم من أنه آمن، إلا أن بعض الأشخاص قد يجدونه مهيجا بشكل طفيف.' }],
  },
{
    id: 'betaine',
    name: 'البيتين',
    scientific_name: 'Betaine',
    searchKeywords: ['betaine', 'trimethylglycine', 'بيتين'],
    usageScope: ['skin', 'hair'],
    functionalCategory: 'مرطب / مطري',
    chemicalType: 'حمض أميني',
    benefits: { 'ترطيب': 0.9, 'ملطف ومرمم': 0.92, 'حماية الخلايا من الإجهاد': 0.88 },
    synergy: {},
    negativeSynergy: {},
    warnings: [{ level: 'info', text: 'حمض أميني مشتق من بنجر السكر، يعمل كمرطب ومهدئ ممتاز للبشرة.' }],
  },
{
    id: 'saccharide-isomerate',
    name: 'سكاريد أيزوميرات',
    scientific_name: 'Saccharide Isomerate',
    searchKeywords: ['saccharide isomerate', 'pentavitin', 'بينتافيتين'],
    usageScope: ['skin'],
    functionalCategory: 'مرطب / مطري',
    chemicalType: 'مرطب',
    benefits: { 'ترطيب عميق وطويل الأمد (72 ساعة)': 0.98, 'دعم حاجز البشرة': 0.9, 'تهدئة الحكة': 0.85 },
    synergy: {},
    negativeSynergy: {},
    warnings: [{ level: 'info', text: 'يعرف تجاريا باسم Pentavitin®. مرطب فريد يرتبط بالكيراتين في الجلد مثل المغناطيس، مما يوفر ترطيبا لا يمكن غسله بسهولة.' }],
  },
{
    id: 'caffeine',
    name: 'الكافيين',
    scientific_name: 'Caffeine',
    searchKeywords: ['caffeine', 'كافيين'],
    usageScope: ['skin', 'hair'],
    functionalCategory: 'مكون فعال',
    chemicalType: 'قلويد',
    benefits: { 'مضاد أكسدة': 0.88, 'تقليل الانتفاخ': 0.9, 'تحسين الدورة الدموية': 0.85, 'تحفيز نمو الشعر': 0.8 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'c12-15-alkyl-benzoate',
    name: 'بنزوات ألكيل C12-15',
    scientific_name: 'C12-15 Alkyl Benzoate',
    searchKeywords: ['c12-15 alkyl benzoate'],
    usageScope: ['skin'],
    functionalCategory: 'مرطب / مطري',
    chemicalType: 'مطري',
    benefits: { 'مذيب لفلاتر الشمس': 0.95, 'يعطي ملمسا خفيفا وأنيقا': 0.92, 'مطري': 0.85 },
    synergy: {},
    negativeSynergy: {},
    warnings: [{ level: 'info', text: 'مطري شائع جدا في واقيات الشمس لقدرته على إذابة فلاتر الشمس الصعبة وإعطاء ملمس نهائي جميل.' }],
  },
{
    id: 'dicaprylyl-carbonate',
    name: 'كربونات ثنائي الكابريليل',
    scientific_name: 'Dicaprylyl Carbonate',
    searchKeywords: ['dicaprylyl carbonate', 'cétiol cc'],
    usageScope: ['skin'],
    functionalCategory: 'مرطب / مطري',
    chemicalType: 'مطري',
    benefits: { 'مطري جاف وسريع الامتصاص': 0.95, 'تحسين انتشار المنتج': 0.9, 'مذيب لفلاتر الشمس': 0.88 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'dicaprylyl-ether',
    name: 'إيثر ثنائي الكابريليل',
    scientific_name: 'Dicaprylyl Ether',
    searchKeywords: ['dicaprylyl ether', 'cétiol oe'],
    usageScope: ['skin', 'hair'],
    functionalCategory: 'مرطب / مطري',
    chemicalType: 'مطري',
    benefits: { 'مطري خفيف جدا': 0.9, 'بديل للسيليكون المتطاير': 0.88 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'neopentyl-glycol-diheptanoate',
    name: 'ثنائي هيبتانوات نيوبنتيل جلايكول',
    scientific_name: 'Neopentyl Glycol Diheptanoate',
    searchKeywords: ['neopentyl glycol diheptanoate'],
    usageScope: ['skin'],
    functionalCategory: 'مرطب / مطري',
    chemicalType: 'مطري',
    benefits: { 'يعطي ملمسا مخمليا': 0.92, 'خفيف الوزن': 0.9 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'octyldodecanol',
    name: 'أوكتيل دوديكانول',
    scientific_name: 'Octyldodecanol',
    searchKeywords: ['octyldodecanol'],
    usageScope: ['skin'],
    functionalCategory: 'مرطب / مطري',
    chemicalType: 'كحول دهني',
    benefits: { 'مطري': 0.9, 'مذيب': 0.88, 'مثبت للمستحلبات': 0.8 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'propylheptyl-caprylate',
    name: 'كابريلات بروبيل هبتيل',
    scientific_name: 'Propylheptyl Caprylate',
    searchKeywords: ['propylheptyl caprylate'],
    usageScope: ['skin'],
    functionalCategory: 'مرطب / مطري',
    chemicalType: 'مطري',
    benefits: { 'مطري سريع الانتشار': 0.9, 'ملمس ناعم': 0.85 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'hydrogenated-polyisobutene',
    name: 'بولي أيزوبيوتين مهدرج',
    scientific_name: 'Hydrogenated Polyisobutene',
    searchKeywords: ['hydrogenated polyisobutene'],
    usageScope: ['skin'],
    functionalCategory: 'مرطب / مطري',
    chemicalType: 'مطري',
    benefits: { 'مطري غني': 0.88, 'بديل للزيوت المعدنية': 0.9, 'يعطي لمعانا': 0.85 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'c13-15-alkane',
    name: 'ألكان C13-15',
    scientific_name: 'C13-15 Alkane',
    searchKeywords: ['c13-15 alkane', 'hemisqualane'],
    usageScope: ['skin', 'hair'],
    functionalCategory: 'مرطب / مطري',
    chemicalType: 'مطري',
    benefits: { 'بديل خفيف جدا للسيليكون': 0.95, 'إزالة المكياج': 0.88, 'تقليل الهيشان': 0.85 },
    synergy: {},
    negativeSynergy: {},
    warnings: [{ level: 'info', text: 'يعرف تجاريا باسم "Himisqualane". مطري متطاير وخفيف جدا مشتق من قصب السكر.' }],
  },
{
    id: 'undecane',
    name: 'أونديكان',
    scientific_name: 'Undecane',
    searchKeywords: ['undecane'],
    usageScope: ['skin'],
    functionalCategory: 'مذيب',
    chemicalType: 'مطري',
    benefits: { 'مطري خفيف جدا ومتطاير': 0.9, 'بديل طبيعي للسيليكون': 0.92 },
    synergy: { 'tridecane': 1.05 },
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'tridecane',
    name: 'ترايدكان',
    scientific_name: 'Tridecane',
    searchKeywords: ['tridecane'],
    usageScope: ['skin'],
    functionalCategory: 'مذيب',
    chemicalType: 'مطري',
    benefits: { 'مطري خفيف': 0.88, 'بديل طبيعي للسيليكون': 0.9 },
    synergy: { 'undecane': 1.05 },
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'boron-nitride',
    name: 'نتريد البورون',
    scientific_name: 'Boron Nitride',
    searchKeywords: ['boron nitride'],
    usageScope: ['skin'],
    functionalCategory: 'محسن قوام',
    chemicalType: 'مسحوق',
    benefits: { 'يعطي ملمسا حريريا': 0.95, 'تأثير بصري لتمويه العيوب': 0.88, 'تحسين الالتصاق': 0.85 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'polymethylsilsesquioxane',
    name: 'بولي ميثيل سيلسسكويكسان',
    scientific_name: 'Polymethylsilsesquioxane',
    searchKeywords: ['polymethylsilsesquioxane', 'tospearl'],
    usageScope: ['skin'],
    functionalCategory: 'محسن قوام',
    chemicalType: 'مسحوق سيليكوني',
    benefits: { 'تأثير بصري لتمويه التجاعيد': 0.92, 'ملمس بودري ناعم': 0.9, 'امتصاص الزيوت': 0.85 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'vinyl-dimethicone-methicone-silsesquioxane-crosspolymer',
    name: 'فينيل دايميثيكون/ميثيكون سيلسسكويكسان كروسبوليمر',
    scientific_name: 'Vinyl Dimethicone/Methicone Silsesquioxane Crosspolymer',
    searchKeywords: ['vinyl dimethicone/methicone silsesquioxane crosspolymer'],
    usageScope: ['skin'],
    functionalCategory: 'محسن قوام',
    chemicalType: 'مسحوق سيليكوني',
    benefits: { 'يعطي ملمسا مخمليا': 0.9, 'تأثير بصري لتمويه العيوب': 0.88 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'sodium-chloride',
    name: 'كلوريد الصوديوم',
    scientific_name: 'Sodium Chloride',
    searchKeywords: ['sodium chloride', 'salt', 'ملح', 'كلوريد الصوديوم'],
    usageScope: ['hair', 'skin'],
    functionalCategory: 'محسن قوام',
    chemicalType: 'ملح',
    benefits: { 'مكثف للقوام': 0.9, 'مقشر فيزيائي': 0.7 },
    synergy: {},
    negativeSynergy: {},
    warnings: [
      { level: 'info', text: 'يستخدم كعامل سماكة رخيص في الشامبوهات التي تحتوي على السلفات.' },
      { level: 'caution', text: 'قد يكون مجففا للشعر والجلد بتركيزات عالية.' }
    ],
  },
{
    id: 'citric-acid',
    name: 'حمض السيتريك',
    scientific_name: 'Citric Acid',
    searchKeywords: ['citric acid', 'aha', 'حمض السيتريك', 'حمض الليمون', 'citric acid monohydrate', 'حمض السيتريك مونوهيدرات'],
    usageScope: ['skin', 'hair'],
    functionalCategory: 'محسن قوام',
    chemicalType: 'معدل الحموضة',
    benefits: { 'ضبط حموضة المنتج (pH)': 0.98, 'تقشير خفيف جدا': 0.7, 'عامل خالب': 0.75 },
    synergy: {},
    negativeSynergy: {},
    warnings: [{ level: 'info', text: 'يستخدم بشكل أساسي بكميات صغيرة جدا لخفض درجة حموضة المنتج.' }],
  },
{
    id: 'sodium-hydroxide',
    name: 'هيدروكسيد الصوديوم',
    scientific_name: 'Sodium Hydroxide',
    searchKeywords: ['sodium hydroxide', 'lye', 'caustic soda', 'هيدروكسيد الصوديوم', 'صودا كاوية'],
    usageScope: ['skin'],
    functionalCategory: 'محسن قوام',
    chemicalType: 'معدل الحموضة',
    benefits: { 'ضبط حموضة المنتج (pH)': 0.98, 'عامل تصبن': 0.9 },
    synergy: {},
    negativeSynergy: {},
    warnings: [{ level: 'info', text: 'يستخدم بكميات صغيرة جدا لرفع درجة حموضة المنتج. وهو أساسي في صناعة الصابون التقليدي.' }],
  },
{
    id: 'carbomer',
    name: 'كاربومير',
    scientific_name: 'Carbomer',
    searchKeywords: ['carbomer', 'carbopol', 'كاربومير'],
    usageScope: ['skin', 'hair'],
    functionalCategory: 'محسن قوام',
    chemicalType: 'بوليمر صناعي',
    benefits: { 'مكثف فعال لتكوين الجل': 0.95, 'تثبيت المستحلبات': 0.9 },
    synergy: {},
    negativeSynergy: {},
    warnings: [{ level: 'info', text: 'بوليمر شائع جدا يستخدم لتكوين قوام الجل في العديد من المنتجات.' }],
  },
{
    id: 'kaolin',
    name: 'طين الكاولين',
    scientific_name: 'Kaolin',
    searchKeywords: ['kaolin', 'white clay', 'china clay', 'كاولين', 'طين أبيض'],
    usageScope: ['skin'],
    functionalCategory: 'ممتص للدهون',
    chemicalType: 'طين',
    benefits: { 'امتصاص الزيوت الزائدة': 0.9, 'تنقية البشرة': 0.85, 'تنظيف لطيف': 0.8 },
    synergy: {},
    negativeSynergy: {},
    warnings: [{ level: 'info', text: 'أحد ألطف أنواع الطين، مناسب للبشرة العادية إلى الدهنية والحساسة.' }],
  },
{
    id: 'bentonite',
    name: 'طين البنتونيت',
    scientific_name: 'Bentonite',
    searchKeywords: ['bentonite', 'montmorillonite', 'بنتونيت'],
    usageScope: ['skin'],
    functionalCategory: 'ممتص للدهون',
    chemicalType: 'طين',
    benefits: { 'امتصاص قوي للزيوت والشوائب': 0.95, 'تنقية عميقة للمسام': 0.9 },
    synergy: {},
    negativeSynergy: {},
    warnings: [{ level: 'caution', text: 'طين قوي جدا، قد يكون مجففا للبشرة الجافة أو الحساسة.' }],
  },
{
    id: 'charcoal-powder',
    name: 'مسحوق الفحم',
    scientific_name: 'Charcoal Powder',
    searchKeywords: ['charcoal powder', 'activated charcoal', 'فحم', 'فحم نشط'],
    usageScope: ['skin'],
    functionalCategory: 'ممتص للدهون',
    chemicalType: 'ممتص',
    benefits: { 'امتصاص الشوائب والسموم': 0.92, 'تنقية عميقة للمسام': 0.9, 'التحكم في الدهون': 0.85 },
    synergy: {},
    negativeSynergy: {},
    warnings: [{ level: 'caution', text: 'قد يكون مجففا جدا إذا استخدم بشكل مفرط.' }],
  },
{
    id: 'disteardimonium-hectorite',
    name: 'داي ستير دايمونيوم هيكتوريت',
    scientific_name: 'Disteardimonium Hectorite',
    searchKeywords: ['disteardimonium hectorite'],
    usageScope: ['skin'],
    functionalCategory: 'محسن قوام',
    chemicalType: 'مثبت',
    benefits: { 'مكثف للزيوت والسيليكونات': 0.9, 'تثبيت المستحلبات': 0.92 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'snail-secretion-filtrate',
    name: 'مرشح إفراز الحلزون',
    scientific_name: 'Snail Secretion Filtrate',
    searchKeywords: ['snail secretion filtrate', 'snail mucin', 'هلام الحلزون'],
    usageScope: ['skin'],
    functionalCategory: 'ملطف ومرمم',
    chemicalType: 'مكون حيوي',
    benefits: { 'إصلاح البشرة': 0.95, 'ترطيب': 0.9, 'تقليل الندوب': 0.85, 'مكافحة الشيخوخة': 0.8 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'propolis-extract',
    name: 'مستخلص البروبوليس (العكبر)',
    scientific_name: 'Propolis Extract',
    searchKeywords: ['propolis extract', 'propolis', 'عكبر', 'بروبوليس'],
    usageScope: ['skin'],
    functionalCategory: 'مكون فعال',
    chemicalType: 'مكون حيوي',
    benefits: { 'مضاد للميكروبات': 0.9, 'مضاد للالتهابات': 0.92, 'ملطف ومرمم': 0.88, 'مضاد أكسدة': 0.85 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'artemisia-princeps-leaf-extract',
    name: 'مستخلص الشيح (Mugwort)',
    scientific_name: 'Artemisia Princeps Leaf Extract',
    searchKeywords: ['artemisia princeps', 'mugwort', 'شيح كوري'],
    usageScope: ['skin'],
    functionalCategory: 'ملطف ومرمم',
    chemicalType: 'مستخلص نباتي',
    benefits: { 'ملطف ومرمم': 0.95, 'مضاد للالتهابات': 0.9, 'تقليل الاحمرار': 0.92, 'مضاد أكسدة': 0.8 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'houttuynia-cordata-extract',
    name: 'مستخلص هوتينيا كورداتا',
    scientific_name: 'Houttuynia Cordata Extract',
    searchKeywords: ['houttuynia cordata', 'heartleaf', 'عشبة القلب'],
    usageScope: ['skin'],
    functionalCategory: 'ملطف ومرمم',
    chemicalType: 'مستخلص نباتي',
    benefits: { 'ملطف ومرمم': 0.9, 'مضاد للالتهابات': 0.88, 'مضاد أكسدة': 0.85 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'centella-asiatica-extract',
    name: 'مستخلص السينتيلا أسياتيكا (Cica)',
    scientific_name: 'Centella Asiatica Extract',
    searchKeywords: ['centella asiatica', 'cica', 'gotu kola', 'tiger grass', 'سينتيلا', 'سيكا'],
    usageScope: ['skin'],
    functionalCategory: 'ملطف ومرمم',
    chemicalType: 'مستخلص نباتي',
    benefits: { 'ملطف ومرمم': 0.98, 'تسريع التئام الجروح': 0.95, 'دعم حاجز البشرة': 0.9, 'تحفيز الكولاجين': 0.85 },
    synergy: { 'madecassoside': 1.1, 'panthenol': 1.08 },
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'glycyrrhiza-glabra-root-extract',
    name: 'مستخلص جذور عرق السوس',
    scientific_name: 'Glycyrrhiza Glabra (Licorice) Root Extract',
    searchKeywords: ['licorice root extract', 'glycyrrhiza glabra', 'عرق السوس'],
    usageScope: ['skin'],
    functionalCategory: 'مكون فعال',
    chemicalType: 'مستخلص نباتي',
    benefits: { 'تفتيح وتوحيد اللون': 0.9, 'مضاد للالتهابات': 0.92, 'ملطف ومرمم': 0.88 },
    synergy: { 'niacinamide': 1.1 },
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'tremella-fuciformis-extract',
    name: 'مستخلص فطر الثلج',
    scientific_name: 'Tremella Fuciformis (Mushroom) Extract',
    searchKeywords: ['tremella fuciformis', 'snow mushroom', 'فطر الثلج'],
    usageScope: ['skin'],
    functionalCategory: 'مرطب / مطري',
    chemicalType: 'مستخلص نباتي',
    benefits: { 'ترطيب فائق': 0.95, 'مضاد أكسدة': 0.8, 'بديل طبيعي لحمض الهيالورونيك': 0.9 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'dimethicone-vinyl-dimethicone-crosspolymer',
    name: 'دايميثيكون/فينيل دايميثيكون كروسبوليمر',
    scientific_name: 'Dimethicone/Vinyl Dimethicone Crosspolymer',
    searchKeywords: ['dimethicone/vinyl dimethicone crosspolymer'],
    usageScope: ['skin'],
    functionalCategory: 'محسن قوام',
    chemicalType: 'سيليكون',
    benefits: { 'يعطي ملمسا بودريا ومخمليا': 0.95, 'تأثير بصري لتمويه العيوب': 0.88, 'مكثف للقوام': 0.8 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'cyclohexasiloxane',
    name: 'سيكلوهكساسيلوكسان (D6)',
    scientific_name: 'Cyclohexasiloxane',
    searchKeywords: ['cyclohexasiloxane', 'd6'],
    usageScope: ['skin', 'hair'],
    functionalCategory: 'محسن قوام',
    chemicalType: 'سيليكون متطاير',
    benefits: { 'ملمس حريري': 0.9, 'تبخر بطيء': 0.85, 'تسهيل انتشار المنتج': 0.9 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'sodium-lauroyl-glutamate',
    name: 'صوديوم لورويل جلوتامات',
    scientific_name: 'Sodium Lauroyl Glutamate',
    searchKeywords: ['sodium lauroyl glutamate'],
    usageScope: ['skin', 'hair'],
    functionalCategory: 'منظف',
    chemicalType: 'منظف لطيف',
    benefits: { 'تنظيف لطيف جدا': 0.92, 'مشتق من الأحماض الأمينية': 0.9, 'رغوة ناعمة': 0.85 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'sodium-cocoamphoacetate',
    name: 'صوديوم كوكو أمفو أسيتات',
    scientific_name: 'Sodium Cocoamphoacetate',
    searchKeywords: ['sodium cocoamphoacetate'],
    usageScope: ['skin', 'hair'],
    functionalCategory: 'منظف',
    chemicalType: 'منظف لطيف جدا',
    benefits: { 'تنظيف لطيف جدا': 0.95, 'مناسب للأطفال': 0.9, 'لا يسبب تهيج للعين': 0.92 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'potassium-cocoyl-hydrolyzed-oat-protein',
    name: 'بروتين الشوفان المهدرج بوتاسيوم كوكويل',
    scientific_name: 'Potassium Cocoyl Hydrolyzed Oat Protein',
    searchKeywords: ['potassium cocoyl hydrolyzed oat protein'],
    usageScope: ['skin'],
    functionalCategory: 'منظف',
    chemicalType: 'منظف لطيف',
    benefits: { 'تنظيف لطيف': 0.9, 'ملطف ومرمم': 0.88 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'cellulose',
    name: 'سليلوز',
    scientific_name: 'Cellulose',
    searchKeywords: ['cellulose', 'سليلوز'],
    usageScope: ['skin'],
    functionalCategory: 'محسن قوام',
    chemicalType: 'بوليمر طبيعي',
    benefits: { 'مكثف للقوام': 0.85, 'امتصاص الزيوت': 0.8 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'pectin',
    name: 'بكتين',
    scientific_name: 'Pectin',
    searchKeywords: ['pectin', 'بكتين'],
    usageScope: ['skin'],
    functionalCategory: 'محسن قوام',
    chemicalType: 'بوليمر طبيعي',
    benefits: { 'مكثف للقوام': 0.8, 'تكوين جل': 0.85 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'pullulan',
    name: 'بولولان',
    scientific_name: 'Pullulan',
    searchKeywords: ['pullulan', 'بولولان'],
    usageScope: ['skin'],
    functionalCategory: 'محسن قوام',
    chemicalType: 'بوليمر طبيعي',
    benefits: { 'تكوين طبقة رقيقة': 0.9, 'شد فوري للبشرة': 0.88, 'مضاد أكسدة': 0.8 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'sorbitol',
    name: 'سوربيتول',
    scientific_name: 'Sorbitol',
    searchKeywords: ['sorbitol', 'سوربيتول'],
    usageScope: ['skin', 'hair'],
    functionalCategory: 'مرطب / مطري',
    chemicalType: 'مرطب',
    benefits: { 'ترطيب': 0.88, 'مكثف للقوام': 0.8, 'يمنع جفاف المنتج': 0.85 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'papain',
    name: 'باباين',
    scientific_name: 'Papain',
    searchKeywords: ['papain', 'papaya enzyme', 'باباين'],
    usageScope: ['skin'],
    functionalCategory: 'مقشر',
    chemicalType: 'إنزيم',
    benefits: { 'تقشير إنزيمي': 0.9, 'تفتيح وتوحيد اللون': 0.82, 'تنعيم البشرة': 0.85 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'bromelain',
    name: 'بروميلين',
    scientific_name: 'Bromelain',
    searchKeywords: ['bromelain', 'pineapple enzyme', 'بروميلين'],
    usageScope: ['skin'],
    functionalCategory: 'مقشر',
    chemicalType: 'إنزيم',
    benefits: { 'تقشير إنزيمي': 0.88, 'مضاد للالتهابات': 0.85 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'menadione',
    name: 'ميناديون (فيتامين K3)',
    scientific_name: 'Menadione',
    searchKeywords: ['menadione', 'vitamin k3'],
    usageScope: ['skin'],
    functionalCategory: 'مكون فعال',
    chemicalType: 'فيتامين',
    benefits: { 'تقليل الاحمرار': 0.8 },
    synergy: {},
    negativeSynergy: {},
    warnings: [{ level: 'caution', text: 'الشكل الصناعي لفيتامين K، استخدامه مثير للجدل ومقيد في بعض المناطق.' }],
  },
{
    id: 'phytonadione',
    name: 'فيتوناديون (فيتامين K1)',
    scientific_name: 'Phytonadione',
    searchKeywords: ['phytonadione', 'vitamin k1'],
    usageScope: ['skin'],
    functionalCategory: 'مكون فعال',
    chemicalType: 'فيتامين',
    benefits: { 'تقليل الهالات السوداء': 0.85, 'تقليل الكدمات والاحمرار': 0.88 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'retinyl-acetate',
    name: 'أسيتات الريتينيل',
    scientific_name: 'Retinyl Acetate',
    searchKeywords: ['retinyl acetate', 'vitamin a acetate'],
    usageScope: ['skin'],
    functionalCategory: 'مكون فعال',
    chemicalType: 'ريتينويد',
    benefits: { 'مكافحة الشيخوخة (خفيف)': 0.75, 'مضاد أكسدة': 0.8 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'retinyl-linoleate',
    name: 'لينوليات الريتينيل',
    scientific_name: 'Retinyl Linoleate',
    searchKeywords: ['retinyl linoleate'],
    usageScope: ['skin'],
    functionalCategory: 'مكون فعال',
    chemicalType: 'ريتينويد',
    benefits: { 'مكافحة الشيخوخة': 0.8, 'لطيف على البشرة': 0.85 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'astaxanthin',
    name: 'أستازانتين',
    scientific_name: 'Astaxanthin',
    searchKeywords: ['astaxanthin', 'أستازانتين'],
    usageScope: ['skin'],
    functionalCategory: 'مضاد أكسدة',
    chemicalType: 'كاروتينويد',
    benefits: { 'مضاد أكسدة فائق القوة': 0.98, 'حماية من أضرار الشمس': 0.9, 'مكافحة الشيخوخة': 0.88 },
    synergy: {},
    negativeSynergy: {},
    warnings: [{ level: 'caution', text: 'له لون أحمر/برتقالي قوي جدا وقد يصبغ البشرة والمنتجات.' }],
  },
{
    id: 'cyanocobalamin',
    name: 'سيانوكوبالامين (فيتامين B12)',
    scientific_name: 'Cyanocobalamin',
    searchKeywords: ['cyanocobalamin', 'vitamin b12'],
    usageScope: ['skin'],
    functionalCategory: 'ملطف ومرمم',
    chemicalType: 'فيتامين',
    benefits: { 'تقليل الاحمرار والالتهاب': 0.8, 'ملطف ومرمم': 0.85 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'pyridoxine-hcl',
    name: 'بيريدوكسين هيدروكلورايد (فيتامين B6)',
    scientific_name: 'Pyridoxine HCl',
    searchKeywords: ['pyridoxine hcl', 'vitamin b6'],
    usageScope: ['skin'],
    functionalCategory: 'مكون فعال',
    chemicalType: 'فيتامين',
    benefits: { 'تنظيم الإفرازات الدهنية': 0.8, 'مضاد للالتهابات': 0.75 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'folic-acid',
    name: 'حمض الفوليك (فيتامين B9)',
    scientific_name: 'Folic Acid',
    searchKeywords: ['folic acid', 'vitamin b9', 'حمض الفوليك'],
    usageScope: ['skin'],
    functionalCategory: 'مكون فعال',
    chemicalType: 'فيتامين',
    benefits: { 'تحسين صلابة البشرة': 0.8, 'ترطيب': 0.75 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'dipalmitoyl-hydroxyproline',
    name: 'ثنائي بالميتويل هيدروكسي برولين',
    scientific_name: 'Dipalmitoyl Hydroxyproline',
    searchKeywords: ['dipalmitoyl hydroxyproline', 'sepilift dphp'],
    usageScope: ['skin'],
    functionalCategory: 'مكون فعال',
    chemicalType: 'حمض أميني دهني',
    benefits: { 'شد البشرة': 0.9, 'تحفيز الكولاجين': 0.88, 'مضاد أكسدة': 0.8 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'sodium-ascorbate',
    name: 'أسكوربات الصوديوم',
    scientific_name: 'Sodium Ascorbate',
    searchKeywords: ['sodium ascorbate'],
    usageScope: ['skin'],
    functionalCategory: 'مضاد أكسدة',
    chemicalType: 'مشتق فيتامين',
    benefits: { 'مضاد أكسدة': 0.85, 'يتحول إلى فيتامين سي': 0.8 },
    synergy: {},
    negativeSynergy: {},
    warnings: [{ level: 'info', text: 'شكل ملح من فيتامين C، أقل تهييجا ولكنه أقل استقرارا من المشتقات الأخرى.' }],
  },
{
    id: 'calcium-ascorbate',
    name: 'أسكوربات الكالسيوم',
    scientific_name: 'Calcium Ascorbate',
    searchKeywords: ['calcium ascorbate', 'ester-c'],
    usageScope: ['skin'],
    functionalCategory: 'مضاد أكسدة',
    chemicalType: 'مشتق فيتامين',
    benefits: { 'مضاد أكسدة': 0.85, 'لطيف على البشرة': 0.9 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'hydroxyacetophenone',
    name: 'هيدروكسي أسيتوفينون',
    scientific_name: 'Hydroxyacetophenone',
    searchKeywords: ['hydroxyacetophenone'],
    usageScope: ['skin'],
    functionalCategory: 'مضاد أكسدة',
    chemicalType: 'مضاد أكسدة صناعي',
    benefits: { 'مضاد أكسدة': 0.9, 'معزز للمواد الحافظة': 0.92, 'ملطف ومرمم': 0.85 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'inositol',
    name: 'إينوزيتول',
    scientific_name: 'Inositol',
    searchKeywords: ['inositol'],
    usageScope: ['skin'],
    functionalCategory: 'مرطب / مطري',
    chemicalType: 'مرطب',
    benefits: { 'ترطيب': 0.85, 'تنظيم الإفرازات الدهنية': 0.8 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'phytosterols',
    name: 'فيتوستيرولات',
    scientific_name: 'Phytosterols',
    searchKeywords: ['phytosterols', 'plant sterols'],
    usageScope: ['skin'],
    functionalCategory: 'دعم حاجز البشرة',
    chemicalType: 'دهون نباتية',
    benefits: { 'دعم حاجز البشرة': 0.9, 'ملطف ومرمم': 0.88, 'يشبه الكوليسترول': 0.85 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'ginkgo-biloba-leaf-extract',
    name: 'مستخلص أوراق الجنكة بيلوبا',
    scientific_name: 'Ginkgo Biloba Leaf Extract',
    searchKeywords: ['ginkgo biloba', 'جنكة'],
    usageScope: ['skin'],
    functionalCategory: 'مضاد أكسدة',
    chemicalType: 'مستخلص نباتي',
    benefits: { 'مضاد أكسدة': 0.9, 'تحسين الدورة الدموية': 0.85, 'مضاد للالتهابات': 0.8 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'hamamelis-virginiana-water',
    name: 'ماء بندق الساحرة',
    scientific_name: 'Hamamelis Virginiana (Witch Hazel) Water',
    searchKeywords: ['witch hazel', 'hamamelis virginiana', 'بندق الساحرة'],
    usageScope: ['skin'],
    functionalCategory: 'مكون فعال',
    chemicalType: 'مستخلص نباتي',
    benefits: { 'قابض للمسام': 0.85, 'مضاد للالتهابات': 0.8, 'تهدئة البشرة': 0.82 },
    synergy: {},
    negativeSynergy: {},
    warnings: [{ level: 'caution', text: 'المنتجات المقطرة بالكحول يمكن أن تكون مجففة ومهيجة. ابحث عن الإصدارات الخالية من الكحول.' }],
  },
{
    id: 'aesculus-hippocastanum-extract',
    name: 'مستخلص كستناء الحصان',
    scientific_name: 'Aesculus Hippocastanum (Horse Chestnut) Extract',
    searchKeywords: ['horse chestnut', 'aesculus hippocastanum', 'كستناء الحصان'],
    usageScope: ['skin'],
    functionalCategory: 'مكون فعال',
    chemicalType: 'مستخلص نباتي',
    benefits: { 'تقوية الأوعية الدموية': 0.88, 'مضاد للالتهابات': 0.85, 'تقليل الاحمرار': 0.8 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'hydrolyzed-collagen',
    name: 'الكولاجين المهدرج',
    scientific_name: 'Hydrolyzed Collagen',
    searchKeywords: ['hydrolyzed collagen', 'collagen', 'كولاجين مهدرج'],
    usageScope: ['skin', 'hair'],
    functionalCategory: 'مرطب / مطري',
    chemicalType: 'بروتين',
    benefits: { 'ترطيب': 0.88, 'تكوين طبقة رقيقة': 0.85 },
    synergy: {},
    negativeSynergy: {},
    warnings: [{ level: 'info', text: 'جزيئاته كبيرة جدا لاختراق الجلد وتحفيز إنتاج الكولاجين، ولكنه يعمل كمرطب جيد على السطح.' }],
  },
{
    id: 'hydrolyzed-silk',
    name: 'الحرير المهدرج',
    scientific_name: 'Hydrolyzed Silk',
    searchKeywords: ['hydrolyzed silk', 'silk protein', 'بروتين الحرير'],
    usageScope: ['hair'],
    functionalCategory: 'مكيف للشعر',
    chemicalType: 'بروتين',
    benefits: { 'تنعيم الشعر': 0.9, 'إضافة لمعان': 0.92, 'تحسين المرونة': 0.85 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'pantolactone',
    name: 'بانتولاكتون',
    scientific_name: 'Pantolactone',
    searchKeywords: ['pantolactone'],
    usageScope: ['skin'],
    functionalCategory: 'مرطب / مطري',
    chemicalType: 'مرطب',
    benefits: { 'مرطب': 0.8 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'phospholipids',
    name: 'الفوسفوليبيدات',
    scientific_name: 'Phospholipids',
    searchKeywords: ['phospholipids', 'فوسفوليبيدات'],
    usageScope: ['skin'],
    functionalCategory: 'دعم حاجز البشرة',
    chemicalType: 'دهون',
    benefits: { 'إصلاح حاجز البشرة': 0.9, 'مكون رئيسي لغشاء الخلية': 0.92, 'ترطيب': 0.85 },
    synergy: {},
    negativeSynergy: {},
    warnings: [{ level: 'info', text: 'مكون أساسي في بنية أغشية خلايا الجلد، وغالبا ما يستخدم في أنظمة توصيل الجسيمات الشحمية (liposomes).' }],
  },
{
    id: 'sarcosine',
    name: 'ساركوزين',
    scientific_name: 'Sarcosine',
    searchKeywords: ['sarcosine', 'ساركوزين'],
    usageScope: ['skin'],
    functionalCategory: 'مكون فعال',
    chemicalType: 'حمض أميني',
    benefits: { 'تنظيم الإفرازات الدهنية': 0.85, 'تقليل حجم المسام': 0.8, 'تأثير مطفي': 0.82 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'tribehenin',
    name: 'تراي بيهينين',
    scientific_name: 'Tribehenin',
    searchKeywords: ['tribehenin'],
    usageScope: ['skin'],
    functionalCategory: 'محسن قوام',
    chemicalType: 'مطري',
    benefits: { 'يعطي ملمسا ناعما': 0.88, 'مكثف للقوام': 0.85 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'glyceryl-behenate',
    name: 'بيهينات الجليسريل',
    scientific_name: 'Glyceryl Behenate',
    searchKeywords: ['glyceryl behenate'],
    usageScope: ['skin'],
    functionalCategory: 'محسن قوام',
    chemicalType: 'مستحلب',
    benefits: { 'مكثف للقوام': 0.85, 'مثبت للمستحلبات': 0.8 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'hydrogenated-poly-c6-14-olefin',
    name: 'بولي أوليفين C6-14 مهدرج',
    scientific_name: 'Hydrogenated Poly(C6-14 Olefin)',
    searchKeywords: ['hydrogenated poly(c6-14 olefin)'],
    usageScope: ['skin'],
    functionalCategory: 'مرطب / مطري',
    chemicalType: 'مطري',
    benefits: { 'مطري غني': 0.9, 'بديل للسكوالان': 0.85 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'behenyl-alcohol',
    name: 'كحول البيهينيل',
    scientific_name: 'Behenyl Alcohol',
    searchKeywords: ['behenyl alcohol', 'كحول البيهينيل'],
    usageScope: ['skin', 'hair'],
    functionalCategory: 'محسن قوام',
    chemicalType: 'كحول دهني',
    benefits: { 'مكثف للقوام': 0.9, 'مطري': 0.88, 'مستحلب مساعد': 0.8 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'arachidyl-alcohol',
    name: 'كحول الأراكيديل',
    scientific_name: 'Arachidyl Alcohol',
    searchKeywords: ['arachidyl alcohol'],
    usageScope: ['skin'],
    functionalCategory: 'محسن قوام',
    chemicalType: 'كحول دهني',
    benefits: { 'مكثف للقوام': 0.85, 'مطري': 0.8 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'arachidyl-glucoside',
    name: 'أراكيديل جلوكوزيد',
    scientific_name: 'Arachidyl Glucoside',
    searchKeywords: ['arachidyl glucoside'],
    usageScope: ['skin'],
    functionalCategory: 'محسن قوام',
    chemicalType: 'مستحلب',
    benefits: { 'مستحلب لطيف مشتق من النبات': 0.9, 'يعطي ملمسا خفيفا': 0.88 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'sodium-stearoyl-glutamate',
    name: 'صوديوم ستيرويل جلوتامات',
    scientific_name: 'Sodium Stearoyl Glutamate',
    searchKeywords: ['sodium stearoyl glutamate'],
    usageScope: ['skin'],
    functionalCategory: 'محسن قوام',
    chemicalType: 'مستحلب',
    benefits: { 'مستحلب لطيف مشتق من الأحماض الأمينية': 0.92, 'لطيف جدا على البشرة': 0.9 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'sodium-stearoyl-lactylate',
    name: 'صوديوم ستيرويل لاكتيلات',
    scientific_name: 'Sodium Stearoyl Lactylate',
    searchKeywords: ['sodium stearoyl lactylate'],
    usageScope: ['skin'],
    functionalCategory: 'محسن قوام',
    chemicalType: 'مستحلب',
    benefits: { 'مستحلب طبيعي': 0.9, 'ترطيب خفيف': 0.8 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'glyceryl-oleate-citrate',
    name: 'سيترات أوليات الجليسريل',
    scientific_name: 'Glyceryl Oleate Citrate',
    searchKeywords: ['glyceryl oleate citrate'],
    usageScope: ['skin'],
    functionalCategory: 'محسن قوام',
    chemicalType: 'مستحلب',
    benefits: { 'مستحلب لطيف': 0.88 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'polyglyceryl-2-dipolyhydroxystearate',
    name: 'بولي جليسريل-2 ثنائي بولي هيدروكسي ستيرات',
    scientific_name: 'Polyglyceryl-2 Dipolyhydroxystearate',
    searchKeywords: ['polyglyceryl-2 dipolyhydroxystearate'],
    usageScope: ['skin'],
    functionalCategory: 'محسن قوام',
    chemicalType: 'مستحلب',
    benefits: { 'مستحلب قوي للماء في الزيت': 0.92, 'مثالي لواقيات الشمس المعدنية': 0.9 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'potassium-laureth-phosphate',
    name: 'فوسفات لوريث البوتاسيوم',
    scientific_name: 'Potassium Laureth Phosphate',
    searchKeywords: ['potassium laureth phosphate'],
    usageScope: ['skin'],
    functionalCategory: 'منظف',
    chemicalType: 'منظف لطيف',
    benefits: { 'تنظيف لطيف': 0.9, 'رغوة كريمية': 0.85 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'tea-cocoyl-glutamate',
    name: 'TEA-كوكويل جلوتامات',
    scientific_name: 'TEA-Cocoyl Glutamate',
    searchKeywords: ['tea-cocoyl glutamate'],
    usageScope: ['skin'],
    functionalCategory: 'منظف',
    chemicalType: 'منظف لطيف',
    benefits: { 'تنظيف لطيف': 0.88 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'polysorbate-40',
    name: 'بوليسوربات 40',
    scientific_name: 'Polysorbate 40',
    searchKeywords: ['polysorbate 40'],
    usageScope: ['skin'],
    functionalCategory: 'محسن قوام',
    chemicalType: 'مستحلب',
    benefits: { 'مستحلب': 0.85 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'steareth-21',
    name: 'ستيريث-21',
    scientific_name: 'Steareth-21',
    searchKeywords: ['steareth-21', 'ستياريث-21'],
    usageScope: ['skin'],
    functionalCategory: 'محسن قوام',
    chemicalType: 'مستحلب',
    benefits: { 'مستحلب فعال': 0.9 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'aminomethyl-propanediol',
    name: 'أمينوميثيل بروبانيديول',
    scientific_name: 'Aminomethyl Propanediol',
    searchKeywords: ['aminomethyl propanediol'],
    usageScope: ['skin'],
    functionalCategory: 'محسن قوام',
    chemicalType: 'معدل الحموضة',
    benefits: { 'ضبط حموضة المنتج (pH)': 0.9 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'oleth-10',
    name: 'أوليث-10',
    scientific_name: 'Oleth-10',
    searchKeywords: ['oleth-10'],
    usageScope: ['hair', 'skin'],
    functionalCategory: 'محسن قوام',
    chemicalType: 'مستحلب',
    benefits: { 'مستحلب': 0.88 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'isopentyldiol',
    name: 'إيزوبنتيلديول',
    scientific_name: 'Isopentyldiol',
    searchKeywords: ['isopentyldiol'],
    usageScope: ['skin', 'hair'],
    functionalCategory: 'مذيب',
    chemicalType: 'مرطب',
    benefits: { 'مرطب': 0.85, 'مذيب': 0.9, 'يعطي ملمسا خفيفا': 0.88 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'sodium-polyitaconate',
    name: 'بولي إيتاكونات الصوديوم',
    scientific_name: 'Sodium Polyitaconate',
    searchKeywords: ['sodium polyitaconate'],
    usageScope: ['hair'],
    functionalCategory: 'مثبت للشعر',
    chemicalType: 'بوليمر طبيعي',
    benefits: { 'تثبيت التصفيف': 0.88, 'قابل للتحلل': 0.9 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'ammonium-acryloyldimethyltaurate-vp-copolymer',
    name: 'أمونيوم أكريلويل دايميثيل تورات/VP كوبوليمر',
    scientific_name: 'Ammonium Acryloyldimethyltaurate/VP Copolymer',
    searchKeywords: ['ammonium acryloyldimethyltaurate/vp copolymer', 'aristoflex avs'],
    usageScope: ['skin'],
    functionalCategory: 'محسن قوام',
    chemicalType: 'بوليمر صناعي',
    benefits: { 'مكثف ومثبت': 0.92, 'يعطي ملمسا خفيفا': 0.95 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'sodium-acrylates-crosspolymer-2',
    name: 'صوديوم أكريلات كروسبوليمر-2',
    scientific_name: 'Sodium Acrylates Crosspolymer-2',
    searchKeywords: ['sodium acrylates crosspolymer-2'],
    usageScope: ['skin'],
    functionalCategory: 'محسن قوام',
    chemicalType: 'بوليمر صناعي',
    benefits: { 'مكثف': 0.9, 'ممتص للماء': 0.85 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'acrylates-steareth-20-methacrylate-crosspolymer',
    name: 'أكريلات/ستيريث-20 ميثاكريليت كروسبوليمر',
    scientific_name: 'Acrylates/Steareth-20 Methacrylate Crosspolymer',
    searchKeywords: ['acrylates/steareth-20 methacrylate crosspolymer', 'acremar', 'أكريلات/ستياريث-20 ميثاكريليت كروسبوليمر'],
    usageScope: ['hair'],
    functionalCategory: 'محسن قوام',
    chemicalType: 'بوليمر صناعي',
    benefits: { 'مكثف للقوام': 0.92 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'sodium-cocoyl-amino-acids',
    name: 'صوديوم كوكويل الأحماض الأمينية',
    scientific_name: 'Sodium Cocoyl Amino Acids',
    searchKeywords: ['sodium cocoyl amino acids'],
    usageScope: ['skin'],
    functionalCategory: 'منظف',
    chemicalType: 'منظف لطيف',
    benefits: { 'تنظيف لطيف': 0.9 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'potassium-aspartate',
    name: 'أسبارتات البوتاسيوم',
    scientific_name: 'Potassium Aspartate',
    searchKeywords: ['potassium aspartate'],
    usageScope: ['skin'],
    functionalCategory: 'مرطب / مطري',
    chemicalType: 'حمض أميني',
    benefits: { 'ترطيب': 0.8 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'magnesium-aspartate',
    name: 'أسبارتات المغنيسيوم',
    scientific_name: 'Magnesium Aspartate',
    searchKeywords: ['magnesium aspartate'],
    usageScope: ['skin'],
    functionalCategory: 'مكون فعال',
    chemicalType: 'حمض أميني',
    benefits: { 'تنشيط طاقة الخلايا': 0.85 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'zinc-gluconate',
    name: 'جلوكونات الزنك',
    scientific_name: 'Zinc Gluconate',
    searchKeywords: ['zinc gluconate', 'جلوكونات الزنك'],
    usageScope: ['skin'],
    functionalCategory: 'مكون فعال',
    chemicalType: 'مركب زنك',
    benefits: { 'تنظيم الإفرازات الدهنية': 0.9, 'مضاد للالتهابات': 0.85, 'مضاد للميكروبات': 0.82 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'copper-gluconate',
    name: 'جلوكونات النحاس',
    scientific_name: 'Copper Gluconate',
    searchKeywords: ['copper gluconate', 'جلوكونات النحاس'],
    usageScope: ['skin'],
    functionalCategory: 'مكون فعال',
    chemicalType: 'مركب نحاس',
    benefits: { 'تحفيز الكولاجين': 0.85, 'تسريع التئام الجروح': 0.8, 'مضاد أكسدة': 0.82 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'oleth-3-phosphate',
    name: 'فوسفات أوليث-3',
    scientific_name: 'Oleth-3 Phosphate',
    searchKeywords: ['oleth-3 phosphate'],
    usageScope: ['skin'],
    functionalCategory: 'محسن قوام',
    chemicalType: 'مستحلب',
    benefits: { 'مستحلب': 0.88 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'isosteareth-20',
    name: 'أيزوستيريث-20',
    scientific_name: 'Isosteareth-20',
    searchKeywords: ['isosteareth-20', 'أيزوستياريث-20' ],
    usageScope: ['skin'],
    functionalCategory: 'محسن قوام',
    chemicalType: 'مستحلب',
    benefits: { 'مستحلب': 0.85 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'peg-7-olivate',
    name: 'أوليفات PEG-7',
    scientific_name: 'PEG-7 Olivate',
    searchKeywords: ['peg-7 olivate'],
    usageScope: ['skin'],
    functionalCategory: 'مرطب / مطري',
    chemicalType: 'عامل إعادة ترطيب',
    benefits: { 'إعادة ترطيب': 0.85 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'sorbeth-30-tetraoleate',
    name: 'سوربيث-30 تترا أوليات',
    scientific_name: 'Sorbeth-30 Tetraoleate',
    searchKeywords: ['sorbeth-30 tetraoleate'],
    usageScope: ['skin'],
    functionalCategory: 'مستحلب',
    chemicalType: 'مستحلب',
    benefits: { 'مستحلب فعال في زيوت التنظيف': 0.9 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'c10-18-triglycerides',
    name: 'ثلاثي جليسريد C10-18',
    scientific_name: 'C10-18 Triglycerides',
    searchKeywords: ['c10-18 triglycerides'],
    usageScope: ['skin'],
    functionalCategory: 'محسن قوام',
    chemicalType: 'مطري شمعي',
    benefits: { 'يعطي قواما زبديا': 0.9, 'مطري': 0.85 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'hydrolyzed-corn-protein',
    name: 'بروتين الذرة المهدرج',
    scientific_name: 'Hydrolyzed Corn Protein',
    searchKeywords: ['hydrolyzed corn protein'],
    usageScope: ['hair'],
    functionalCategory: 'تقوية الشعر',
    chemicalType: 'بروتين',
    benefits: { 'تقوية الشعر': 0.8, 'ترطيب': 0.82 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'laurdimonium-hydroxypropyl-hydrolyzed-soy-protein',
    name: 'بروتين الصويا المهدرج لورديمونيوم هيدروكسي بروبيل',
    scientific_name: 'Laurdimonium Hydroxypropyl Hydrolyzed Soy Protein',
    searchKeywords: ['laurdimonium hydroxypropyl hydrolyzed soy protein'],
    usageScope: ['hair'],
    functionalCategory: 'مكيف للشعر',
    chemicalType: 'بروتين معدل',
    benefits: { 'تكييف': 0.88, 'تقوية': 0.85 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'cocodimonium-hydroxypropyl-hydrolyzed-rice-protein',
    name: 'بروتين الأرز المهدرج كوكوديمونيوم هيدروكسي بروبيل',
    scientific_name: 'Cocodimonium Hydroxypropyl Hydrolyzed Rice Protein',
    searchKeywords: ['cocodimonium hydroxypropyl hydrolyzed rice protein'],
    usageScope: ['hair'],
    functionalCategory: 'مكيف للشعر',
    chemicalType: 'بروتين معدل',
    benefits: { 'تكييف': 0.9, 'إضافة حجم': 0.85 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'ppg-26-buteth-26',
    name: 'PPG-26-بوتيث-26',
    scientific_name: 'PPG-26-Buteth-26',
    searchKeywords: ['ppg-26-buteth-26'],
    usageScope: ['hair', 'skin'],
    functionalCategory: 'مذوب',
    chemicalType: 'مستحلب',
    benefits: { 'مذوب للعطور والزيوت': 0.92, 'مكيف خفيف': 0.8 },
    synergy: { 'peg-40-hydrogenated-castor-oil': 1.05 },
    negativeSynergy: {},
    warnings: [{ level: 'info', text: 'غالبا ما يستخدم مع PEG-40 Hydrogenated Castor Oil لإذابة المكونات.' }],
  },
{
    id: 'acrylates-vinyl-neodecanoate-crosspolymer',
    name: 'أكريلات/فينيل نيوديكانوات كروسبوليمر',
    scientific_name: 'Acrylates/Vinyl Neodecanoate Crosspolymer',
    searchKeywords: ['acrylates/vinyl neodecanoate crosspolymer'],
    usageScope: ['skin'],
    functionalCategory: 'محسن قوام',
    chemicalType: 'بوليمر صناعي',
    benefits: { 'مكثف': 0.88 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'acrylates-c12-22-alkyl-methacrylate-copolymer',
    name: 'أكريلات/C12-22 ألكيل ميثاكريليت كوبوليمر',
    scientific_name: 'Acrylates/C12-22 Alkyl Methacrylate Copolymer',
    searchKeywords: ['acrylates/c12-22 alkyl methacrylate copolymer'],
    usageScope: ['skin'],
    functionalCategory: 'محسن قوام',
    chemicalType: 'مكون طبقة رقيقة',
    benefits: { 'تكوين طبقة رقيقة': 0.85, 'مقاومة للماء': 0.88 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'c20-22-alkyl-phosphate',
    name: 'فوسفات ألكيل C20-22',
    scientific_name: 'C20-22 Alkyl Phosphate',
    searchKeywords: ['c20-22 alkyl phosphate'],
    usageScope: ['skin'],
    functionalCategory: 'محسن قوام',
    chemicalType: 'مستحلب',
    benefits: { 'مستحلب': 0.9, 'يعطي ملمسا خفيفا': 0.85 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'c20-22-alcohols',
    name: 'كحولات C20-22',
    scientific_name: 'C20-22 Alcohols',
    searchKeywords: ['c20-22 alcohols'],
    usageScope: ['skin'],
    functionalCategory: 'محسن قوام',
    chemicalType: 'كحول دهني',
    benefits: { 'مستحلب مساعد': 0.88, 'مكثف للقوام': 0.85 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'polyglyceryl-6-distearate',
    name: 'بولي جليسريل-6 ثنائي الستيارات',
    scientific_name: 'Polyglyceryl-6 Distearate',
    searchKeywords: ['polyglyceryl-6 distearate'],
    usageScope: ['skin'],
    functionalCategory: 'محسن قوام',
    chemicalType: 'مستحلب',
    benefits: { 'مستحلب طبيعي': 0.9 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'jojoba-esters',
    name: 'استرات الجوجوبا',
    scientific_name: 'Jojoba Esters',
    searchKeywords: ['jojoba esters'],
    usageScope: ['skin'],
    functionalCategory: 'مرطب / مطري',
    chemicalType: 'مطري',
    benefits: { 'مطري': 0.9, 'مشابه لدهون البشرة': 0.88 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'polyglyceryl-3-beeswax',
    name: 'بولي جليسريل-3 شمع النحل',
    scientific_name: 'Polyglyceryl-3 Beeswax',
    searchKeywords: ['polyglyceryl-3 beeswax'],
    usageScope: ['skin'],
    functionalCategory: 'محسن قوام',
    chemicalType: 'مستحلب',
    benefits: { 'مستحلب طبيعي': 0.88 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'isobutylparaben',
    name: 'أيزوبوتيل بارابين',
    scientific_name: 'Isobutylparaben',
    searchKeywords: ['isobutylparaben'],
    usageScope: ['skin', 'hair'],
    functionalCategory: 'مادة حافظة',
    chemicalType: 'بارابين',
    benefits: { 'حفظ المنتج': 0.9 },
    synergy: {},
    negativeSynergy: {},
    warnings: [{ level: 'risk', text: 'أحد أكثر أنواع البارابين إثارة للجدل، ومحظور في الاتحاد الأوروبي بسبب مخاوف بشأن اضطراب الغدد الصماء.' }],
  },
{
    id: 'sodium-dehydroacetate',
    name: 'ديهيدروأسيتات الصوديوم',
    scientific_name: 'Sodium Dehydroacetate',
    searchKeywords: ['sodium dehydroacetate'],
    usageScope: ['skin', 'hair'],
    functionalCategory: 'مادة حافظة',
    chemicalType: 'مادة حافظة',
    benefits: { 'مضاد للفطريات والخمائر': 0.88 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'caprylhydroxamic-acid',
    name: 'حمض كابريل هيدروكساميك',
    scientific_name: 'Caprylhydroxamic Acid',
    searchKeywords: ['caprylhydroxamic acid'],
    usageScope: ['skin'],
    functionalCategory: 'مادة حافظة',
    chemicalType: 'عامل خالب',
    benefits: { 'مضاد للفطريات': 0.9, 'بديل للمواد الحافظة التقليدية': 0.88 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'peg-ppg-17-6-copolymer',
    name: 'PEG/PPG-17/6 كوبوليمر',
    scientific_name: 'PEG/PPG-17/6 Copolymer',
    searchKeywords: ['peg/ppg-17/6 copolymer'],
    usageScope: ['skin'],
    functionalCategory: 'مذيب',
    chemicalType: 'مرطب',
    benefits: { 'مذيب': 0.85, 'مرطب': 0.8 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'arginine-pca',
    name: 'أرجينين PCA',
    scientific_name: 'Arginine PCA',
    searchKeywords: ['arginine pca'],
    usageScope: ['skin'],
    functionalCategory: 'مرطب / مطري',
    chemicalType: 'حمض أميني',
    benefits: { 'ترطيب': 0.9 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'sorbitan-sesquioleate',
    name: 'سيسكوي أوليات السوربيتان',
    scientific_name: 'Sorbitan Sesquioleate',
    searchKeywords: ['sorbitan sesquioleate'],
    usageScope: ['skin'],
    functionalCategory: 'محسن قوام',
    chemicalType: 'مستحلب',
    benefits: { 'مستحلب للماء في الزيت': 0.85 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'acacia-decurrens-flower-wax',
    name: 'شمع زهرة الأكاسيا',
    scientific_name: 'Acacia Decurrens Flower Wax',
    searchKeywords: ['acacia decurrens', 'mimosa wax'],
    usageScope: ['skin'],
    functionalCategory: 'محسن قوام',
    chemicalType: 'شمع نباتي',
    benefits: { 'عازل': 0.8, 'يعطي قواما ناعما': 0.85 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'helianthus-annuus-seed-wax',
    name: 'شمع بذور دوار الشمس',
    scientific_name: 'Helianthus Annuus (Sunflower) Seed Wax',
    searchKeywords: ['sunflower seed wax'],
    usageScope: ['skin'],
    functionalCategory: 'محسن قوام',
    chemicalType: 'شمع نباتي',
    benefits: { 'مكثف للقوام': 0.88, 'مثبت للمستحلبات': 0.85 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'polyglycerin-3',
    name: 'بولي جليسرين-3',
    scientific_name: 'Polyglycerin-3',
    searchKeywords: ['polyglycerin-3'],
    usageScope: ['skin'],
    functionalCategory: 'مرطب / مطري',
    chemicalType: 'مرطب',
    benefits: { 'ترطيب': 0.9, 'يعطي ملمسا غنيا': 0.85 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'sucrose-stearate',
    name: 'ستيرات السكروز',
    scientific_name: 'Sucrose Stearate',
    searchKeywords: ['sucrose stearate'],
    usageScope: ['skin'],
    functionalCategory: 'محسن قوام',
    chemicalType: 'مستحلب',
    benefits: { 'مستحلب لطيف': 0.9, 'مرطب': 0.8 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'sucrose-distearate',
    name: 'ثنائي ستيرات السكروز',
    scientific_name: 'Sucrose Distearate',
    searchKeywords: ['sucrose distearate'],
    usageScope: ['skin'],
    functionalCategory: 'محسن قوام',
    chemicalType: 'مستحلب',
    benefits: { 'مستحلب': 0.88, 'مكثف': 0.8 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'eryngium-maritimum-callus-culture-filtrate',
    name: 'مرشح مزرعة الكالس من شوك الجمل البحري',
    scientific_name: 'Eryngium Maritimum Callus Culture Filtrate',
    searchKeywords: ['eryngium maritimum', 'sea holly stem cells'],
    usageScope: ['skin'],
    functionalCategory: 'مكون فعال',
    chemicalType: 'خلايا جذعية نباتية',
    benefits: { 'تجديد البشرة': 0.9, 'تحسين صلابة البشرة': 0.88 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'crithmum-maritimum-extract',
    name: 'مستخلص شمر البحر',
    scientific_name: 'Crithmum Maritimum Extract',
    searchKeywords: ['crithmum maritimum', 'sea fennel', 'retinol-like plant'],
    usageScope: ['skin'],
    functionalCategory: 'مكون فعال',
    chemicalType: 'مستخلص نباتي',
    benefits: { 'تأثير شبيه بالريتينول': 0.85, 'تفتيح وتوحيد اللون': 0.8, 'تنظيم الإفرازات الدهنية': 0.82 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'limonium-vulgare-flower-leaf-stem-extract',
    name: 'مستخلص خزامى البحر',
    scientific_name: 'Limonium Vulgare Flower/Leaf/Stem Extract',
    searchKeywords: ['limonium vulgare', 'sea lavender'],
    usageScope: ['skin'],
    functionalCategory: 'مضاد أكسدة',
    chemicalType: 'مستخلص نباتي',
    benefits: { 'مضاد أكسدة': 0.88, 'حماية من أضرار البيئة': 0.85 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'ethyl-linoleate',
    name: 'لينوليات الإيثيل',
    scientific_name: 'Ethyl Linoleate',
    searchKeywords: ['ethyl linoleate'],
    usageScope: ['skin'],
    functionalCategory: 'دعم حاجز البشرة',
    chemicalType: 'مطري',
    benefits: { 'دعم حاجز البشرة': 0.9, 'مضاد للالتهابات': 0.85 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'brassica-campestris-sterols',
    name: 'ستيرولات بذور اللفت',
    scientific_name: 'Brassica Campestris (Rapeseed) Sterols',
    searchKeywords: ['brassica campestris sterols', 'rapeseed sterols'],
    usageScope: ['skin'],
    functionalCategory: 'دعم حاجز البشرة',
    chemicalType: 'دهون نباتية',
    benefits: { 'دعم حاجز البشرة': 0.88, 'ملطف ومرمم': 0.9 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'isostearyl-alcohol',
    name: 'كحول الأيزوستيريل',
    scientific_name: 'Isostearyl Alcohol',
    searchKeywords: ['isostearyl alcohol'],
    usageScope: ['skin'],
    functionalCategory: 'مرطب / مطري',
    chemicalType: 'كحول دهني',
    benefits: { 'مطري': 0.88, 'مثبت للمستحلبات': 0.8 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'butylene-glycol-dicaprylate-dicaprate',
    name: 'ثنائي الكابريلات/الكبرات بيوتيلين جلايكول',
    scientific_name: 'Butylene Glycol Dicaprylate/Dicaprate',
    searchKeywords: ['butylene glycol dicaprylate/dicaprate'],
    usageScope: ['skin'],
    functionalCategory: 'مرطب / مطري',
    chemicalType: 'مطري',
    benefits: { 'مطري خفيف': 0.9 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'polyglyceryl-2-stearate',
    name: 'بولي جليسريل-2 ستيرات',
    scientific_name: 'Polyglyceryl-2 Stearate',
    searchKeywords: ['polyglyceryl-2 stearate'],
    usageScope: ['skin'],
    functionalCategory: 'محسن قوام',
    chemicalType: 'مستحلب',
    benefits: { 'مستحلب': 0.85 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'polyglyceryl-6-stearate',
    name: 'بولي جليسريل-6 ستيرات',
    scientific_name: 'Polyglyceryl-6 Stearate',
    searchKeywords: ['polyglyceryl-6 stearate'],
    usageScope: ['skin'],
    functionalCategory: 'محسن قوام',
    chemicalType: 'مستحلب',
    benefits: { 'مستحلب لطيف': 0.9 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'sodium-stearoyl-caseinate',
    name: 'ستيرويل كازينات الصوديوم',
    scientific_name: 'Sodium Stearoyl Caseinate',
    searchKeywords: ['sodium stearoyl caseinate'],
    usageScope: ['skin'],
    functionalCategory: 'محسن قوام',
    chemicalType: 'مستحلب',
    benefits: { 'مستحلب': 0.8 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'sodium-lauroyl-hydrolyzed-silk',
    name: 'صوديوم لورويل الحرير المهدرج',
    scientific_name: 'Sodium Lauroyl Hydrolyzed Silk',
    searchKeywords: ['sodium lauroyl hydrolyzed silk'],
    usageScope: ['hair'],
    functionalCategory: 'منظف',
    chemicalType: 'منظف لطيف',
    benefits: { 'تنظيف لطيف': 0.88, 'تكييف': 0.85 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'hydrolyzed-casein',
    name: 'الكازين المهدرج',
    scientific_name: 'Hydrolyzed Casein',
    searchKeywords: ['hydrolyzed casein', 'milk protein'],
    usageScope: ['hair'],
    functionalCategory: 'مكيف للشعر',
    chemicalType: 'بروتين',
    benefits: { 'تكييف': 0.8, 'ترطيب': 0.82 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'hydrolyzed-milk-protein',
    name: 'بروتين الحليب المهدرج',
    scientific_name: 'Hydrolyzed Milk Protein',
    searchKeywords: ['hydrolyzed milk protein'],
    usageScope: ['hair', 'skin'],
    functionalCategory: 'مكيف للشعر',
    chemicalType: 'بروتين',
    benefits: { 'تكييف': 0.85, 'ترطيب': 0.88 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'triticum-vulgare-protein',
    name: 'بروتين القمح',
    scientific_name: 'Triticum Vulgare (Wheat) Protein',
    searchKeywords: ['wheat protein', 'triticum vulgare protein'],
    usageScope: ['hair'],
    functionalCategory: 'تقوية الشعر',
    chemicalType: 'بروتين',
    benefits: { 'تقوية الشعر': 0.85, 'إصلاح التلف': 0.8 },
    synergy: {},
    negativeSynergy: {},
    warnings: [{ level: 'risk', text: 'يجب تجنبه من قبل الأشخاص الذين يعانون من حساسية القمح أو الغلوتين.' }],
  },
{
    id: 'keratin-amino-acids',
    name: 'أحماض الكيراتين الأمينية',
    scientific_name: 'Keratin Amino Acids',
    searchKeywords: ['keratin amino acids'],
    usageScope: ['hair'],
    functionalCategory: 'تقوية الشعر',
    chemicalType: 'حمض أميني',
    benefits: { 'اختراق عميق للشعر': 0.9, 'إصلاح وتقوية': 0.92 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'collagen-amino-acids',
    name: 'أحماض الكولاجين الأمينية',
    scientific_name: 'Collagen Amino Acids',
    searchKeywords: ['collagen amino acids'],
    usageScope: ['hair', 'skin'],
    functionalCategory: 'مرطب / مطري',
    chemicalType: 'حمض أميني',
    benefits: { 'ترطيب': 0.9 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'sodium-hyaluronate-dimethylsilanol',
    name: 'صوديوم هيالورونات ثنائي ميثيل سيلانول',
    scientific_name: 'Sodium Hyaluronate Dimethylsilanol',
    searchKeywords: ['sodium hyaluronate dimethylsilanol'],
    usageScope: ['skin'],
    functionalCategory: 'مرطب / مطري',
    chemicalType: 'مرطب',
    benefits: { 'ترطيب': 0.88, 'مكافحة الشيخوخة': 0.8 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'dimethyl-silanol-hyaluronate',
    name: 'هيالورونات ثنائي ميثيل سيلانول',
    scientific_name: 'Dimethyl Silanol Hyaluronate',
    searchKeywords: ['dimethyl silanol hyaluronate'],
    usageScope: ['skin'],
    functionalCategory: 'مرطب / مطري',
    chemicalType: 'مرطب',
    benefits: { 'ترطيب': 0.88, 'مكافحة الشيخوخة': 0.8 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'silanetriol',
    name: 'سيلانتريول',
    scientific_name: 'Silanetriol',
    searchKeywords: ['silanetriol'],
    usageScope: ['skin', 'hair'],
    functionalCategory: 'مكيف للشعر',
    chemicalType: 'سيليكون عضوي',
    benefits: { 'تكييف': 0.8, 'ترطيب': 0.85 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'hydrolyzed-glycosaminoglycans',
    name: 'جليكوزامينوجليكان مهدرج',
    scientific_name: 'Hydrolyzed Glycosaminoglycans',
    searchKeywords: ['hydrolyzed glycosaminoglycans', 'gags'],
    usageScope: ['skin'],
    functionalCategory: 'مرطب / مطري',
    chemicalType: 'مرطب',
    benefits: { 'ترطيب عميق': 0.9, 'تحسين امتصاص المكونات': 0.8 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'palmitoyl-oligopeptide',
    name: 'بالميتويل أوليجوببتيد',
    scientific_name: 'Palmitoyl Oligopeptide',
    searchKeywords: ['palmitoyl oligopeptide', 'biopeptide-cl'],
    usageScope: ['skin'],
    functionalCategory: 'مكون فعال',
    chemicalType: 'ببتيد',
    benefits: { 'تحفيز الكولاجين': 0.88 },
    synergy: {},
    negativeSynergy: {},
    warnings: [{ level: 'info', text: 'اسم قديم لـ Palmitoyl Tripeptide-1.' }],
  },
{
    id: 'acetyl-tetrapeptide-3',
    name: 'أسيتيل رباعي الببتيد-3',
    scientific_name: 'Acetyl Tetrapeptide-3',
    searchKeywords: ['acetyl tetrapeptide-3', 'capixyl'],
    usageScope: ['hair'],
    functionalCategory: 'مكون فعال',
    chemicalType: 'ببتيد',
    benefits: { 'تحفيز نمو الشعر': 0.88, 'تقوية بصيلات الشعر': 0.85 },
    synergy: {},
    negativeSynergy: {},
    warnings: [{ level: 'info', text: 'المكون الببتيدي في مركب Capixyl®.' }],
  },
{
    id: 'hydrolyzed-walnut-extract',
    name: 'مستخلص الجوز المهدرج',
    scientific_name: 'Hydrolyzed Walnut Extract',
    searchKeywords: ['hydrolyzed walnut extract'],
    usageScope: ['hair'],
    functionalCategory: 'مكون فعال',
    chemicalType: 'مستخلص نباتي',
    benefits: { 'حماية اللون': 0.85, 'مضاد أكسدة': 0.8 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'salvia-hispanica-seed-extract',
    name: 'مستخلص بذور الشيا',
    scientific_name: 'Salvia Hispanica Seed Extract',
    searchKeywords: ['chia seed extract', 'salvia hispanica'],
    usageScope: ['skin'],
    functionalCategory: 'مرطب / مطري',
    chemicalType: 'مستخلص نباتي',
    benefits: { 'ترطيب': 0.88, 'تكوين طبقة واقية': 0.85 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'linum-usitatissimum-seed-extract',
    name: 'مستخلص بذور الكتان',
    scientific_name: 'Linum Usitatissimum (Linseed) Seed Extract',
    searchKeywords: ['flaxseed extract', 'linseed extract'],
    usageScope: ['hair'],
    functionalCategory: 'مكيف للشعر',
    chemicalType: 'مستخلص نباتي',
    benefits: { 'تحديد تموجات الشعر': 0.85, 'ترطيب': 0.8 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'polyglyceryl-10-stearate',
    name: 'بولي جليسريل-10 ستيرات',
    scientific_name: 'Polyglyceryl-10 Stearate',
    searchKeywords: ['polyglyceryl-10 stearate'],
    usageScope: ['skin'],
    functionalCategory: 'محسن قوام',
    chemicalType: 'مستحلب',
    benefits: { 'مستحلب لطيف': 0.9 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'sucrose',
    name: 'السكروز (سكر)',
    scientific_name: 'Sucrose',
    searchKeywords: ['sucrose', 'sugar', 'سكر'],
    usageScope: ['skin'],
    functionalCategory: 'مقشر',
    chemicalType: 'مقشر فيزيائي',
    benefits: { 'تقشير فيزيائي': 0.9, 'مرطب طبيعي': 0.7 },
    synergy: {},
    negativeSynergy: {},
    warnings: [{ level: 'caution', text: 'قد تكون الحبيبات حادة جدا على بشرة الوجه الحساسة.' }],
  },
{
    id: 'hydrated-silica',
    name: 'سيليكا رطبة',
    scientific_name: 'Hydrated Silica',
    searchKeywords: ['hydrated silica', 'silica beads', 'سيليكا رطبة'],
    usageScope: ['skin'],
    functionalCategory: 'مقشر',
    chemicalType: 'مقشر فيزيائي',
    benefits: { 'تقشير لطيف': 0.88, 'مكثف للقوام': 0.8 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'bambusa-arundinacea-stem-powder',
    name: 'مسحوق سيقان الخيزران',
    scientific_name: 'Bambusa Arundinacea Stem Powder',
    searchKeywords: ['bamboo powder', 'bambusa arundinacea'],
    usageScope: ['skin'],
    functionalCategory: 'مقشر',
    chemicalType: 'مقشر فيزيائي',
    benefits: { 'تقشير طبيعي': 0.85, 'غني بالسيليكا': 0.8 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'prunus-armeniaca-seed-powder',
    name: 'مسحوق بذور المشمش',
    scientific_name: 'Prunus Armeniaca (Apricot) Seed Powder',
    searchKeywords: ['apricot seed powder', 'مسحوق بذور المشمش'],
    usageScope: ['skin'],
    functionalCategory: 'مقشر',
    chemicalType: 'مقشر فيزيائي',
    benefits: { 'تقشير قوي': 0.8 },
    synergy: {},
    negativeSynergy: {},
    warnings: [{ level: 'risk', text: 'يعتبر قاسيا جدا على بشرة الوجه ويمكن أن يسبب تمزقات دقيقة.' }],
  },
{
    id: 'juglans-regia-shell-powder',
    name: 'مسحوق قشر الجوز',
    scientific_name: 'Juglans Regia (Walnut) Shell Powder',
    searchKeywords: ['walnut shell powder', 'مسحوق قشر الجوز'],
    usageScope: ['skin'],
    functionalCategory: 'مقشر',
    chemicalType: 'مقشر فيزيائي',
    benefits: { 'تقشير قوي جدا': 0.8 },
    synergy: {},
    negativeSynergy: {},
    warnings: [{ level: 'risk', text: 'قاسٍ جدا وغير منتظم الحواف، لا ينصح به أبدا للوجه.' }],
  },
{
    id: 'coffea-arabica-seed-powder',
    name: 'مسحوق بذور القهوة',
    scientific_name: 'Coffea Arabica (Coffee) Seed Powder',
    searchKeywords: ['coffee grounds', 'coffee powder', 'مسحوق القهوة'],
    usageScope: ['skin'],
    functionalCategory: 'مقشر',
    chemicalType: 'مقشر فيزيائي',
    benefits: { 'تقشير وتنشيط': 0.85, 'مضاد أكسدة': 0.8 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'pumice',
    name: 'حجر الخفاف',
    scientific_name: 'Pumice',
    searchKeywords: ['pumice', 'volcanic rock', 'حجر الخفاف'],
    usageScope: ['skin'],
    functionalCategory: 'مقشر',
    chemicalType: 'مقشر فيزيائي',
    benefits: { 'تقشير قوي للقدمين والجسم': 0.9 },
    synergy: {},
    negativeSynergy: {},
    warnings: [{ level: 'risk', text: 'قاسٍ جدا على الوجه والمناطق الحساسة.' }],
  },
{
    id: 'oryza-sativa-powder',
    name: 'مسحوق الأرز',
    scientific_name: 'Oryza Sativa (Rice) Powder',
    searchKeywords: ['rice powder', 'rice starch', 'مسحوق الأرز'],
    usageScope: ['skin'],
    functionalCategory: 'مقشر',
    chemicalType: 'مقشر فيزيائي',
    benefits: { 'تقشير لطيف جدا': 0.88, 'امتصاص الزيوت': 0.8 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'moroccan-lava-clay',
    name: 'الطين المغربي (غاسول)',
    scientific_name: 'Moroccan Lava Clay',
    searchKeywords: ['rhassoul clay', 'ghassoul', 'moroccan lava clay', 'غاسول', 'طين مغربي'],
    usageScope: ['skin', 'hair'],
    functionalCategory: 'ممتص للدهون',
    chemicalType: 'طين',
    benefits: { 'تنظيف وتنقية': 0.92, 'غني بالمعادن': 0.9, 'تنعيم الشعر والبشرة': 0.88 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'illite',
    name: 'طين الإيليت (الطين الأخضر الفرنسي)',
    scientific_name: 'Illite',
    searchKeywords: ['illite', 'french green clay', 'green clay', 'طين أخضر'],
    usageScope: ['skin'],
    functionalCategory: 'ممتص للدهون',
    chemicalType: 'طين',
    benefits: { 'امتصاص قوي للزيوت والشوائب': 0.95, 'إزالة السموم': 0.9, 'تنقية عميقة': 0.92 },
    synergy: {},
    negativeSynergy: {},
    warnings: [{ level: 'caution', text: 'قوي جدا وقد يكون مجففا للبشرة الجافة أو الحساسة.' }],
  },
{
    id: 'montmorillonite',
    name: 'طين المونتموريلونيت',
    scientific_name: 'Montmorillonite',
    searchKeywords: ['montmorillonite', 'مونتموريلونيت'],
    usageScope: ['skin'],
    functionalCategory: 'ممتص للدهون',
    chemicalType: 'طين',
    benefits: { 'إزالة السموم': 0.9, 'تنقية': 0.88 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'fullers-earth',
    name: 'تراب القصار (طين ملتان ميتي)',
    scientific_name: 'Solum Fullonum / Fuller\'s Earth',
    searchKeywords: ['fullers earth', 'multani mitti', 'تراب القصار'],
    usageScope: ['skin'],
    functionalCategory: 'ممتص للدهون',
    chemicalType: 'طين',
    benefits: { 'امتصاص فائق للزيوت': 0.98, 'تفتيح البشرة': 0.8 },
    synergy: {},
    negativeSynergy: {},
    warnings: [{ level: 'risk', text: 'مجفف وقوي جدا، غير مناسب للبشرة الجافة.' }],
  },
{
    id: 'sea-silt',
    name: 'طمي البحر',
    scientific_name: 'Sea Silt / Maris Limus',
    searchKeywords: ['sea silt', 'marine mud', 'dead sea mud', 'طمي البحر', 'طين البحر الميت'],
    usageScope: ['skin'],
    functionalCategory: 'مكون فعال',
    chemicalType: 'طين',
    benefits: { 'غني بالمعادن': 0.95, 'إزالة السموم': 0.9, 'مضاد للالتهابات': 0.85 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'butane',
    name: 'البيوتان',
    scientific_name: 'Butane',
    searchKeywords: ['butane', 'بيوتان'],
    usageScope: ['hair', 'skin'],
    functionalCategory: 'غاز دافع',
    chemicalType: 'غاز دافع',
    benefits: { 'دفع المنتج من العبوة': 0.95 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'propane',
    name: 'البروبان',
    scientific_name: 'Propane',
    searchKeywords: ['propane', 'بروبان'],
    usageScope: ['hair', 'skin'],
    functionalCategory: 'غاز دافع',
    chemicalType: 'غاز دافع',
    benefits: { 'دفع المنتج من العبوة': 0.95 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'isobutane',
    name: 'الأيزوبيوتان',
    scientific_name: 'Isobutane',
    searchKeywords: ['isobutane', 'أيزوبيوتان'],
    usageScope: ['hair', 'skin'],
    functionalCategory: 'غاز دافع',
    chemicalType: 'غاز دافع',
    benefits: { 'دفع المنتج من العبوة': 0.95 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'hydrofluorocarbon-152a',
    name: 'هيدروفلوروكربون 152a',
    scientific_name: 'Hydrofluorocarbon 152a',
    searchKeywords: ['hydrofluorocarbon 152a'],
    usageScope: ['hair'],
    functionalCategory: 'غاز دافع',
    chemicalType: 'غاز دافع',
    benefits: { 'دفع المنتج من العبوة': 0.95 },
    synergy: {},
    negativeSynergy: {},
    warnings: [{ level: 'caution', text: 'له تأثير أقل على طبقة الأوزون من مركبات الكربون الكلورية فلورية القديمة، ولكنه لا يزال من غازات الدفيئة.' }],
  },
{
    id: 'dimethyl-ether',
    name: 'ثنائي ميثيل الإيثر',
    scientific_name: 'Dimethyl Ether',
    searchKeywords: ['dimethyl ether', 'dme'],
    usageScope: ['hair'],
    functionalCategory: 'غاز دافع',
    chemicalType: 'مذيب/دافع',
    benefits: { 'دفع المنتج من العبوة': 0.92, 'مذيب فعال': 0.85 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'ci-15850-red-7',
    name: 'CI 15850 (Red 7 Lake)',
    scientific_name: 'CI 15850',
    searchKeywords: ['ci 15850', 'red 7 lake', 'd&c red no. 7'],
    usageScope: ['skin'],
    functionalCategory: 'محسن جمالي',
    chemicalType: 'ملون عضوي',
    benefits: { 'يعطي لونا أحمر زاهي': 0.95 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'ci-77491-red-iron-oxide',
    name: 'CI 77491 (أكسيد الحديد الأحمر)',
    scientific_name: 'Iron Oxides (CI 77491)',
    searchKeywords: ['ci 77491', 'red iron oxide', 'أكسيد الحديد الأحمر'],
    usageScope: ['skin'],
    functionalCategory: 'محسن جمالي',
    chemicalType: 'ملون معدني',
    benefits: { 'يعطي لونا أحمر/بني': 0.95 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'ci-77492-yellow-iron-oxide',
    name: 'CI 77492 (أكسيد الحديد الأصفر)',
    scientific_name: 'Iron Oxides (CI 77492)',
    searchKeywords: ['ci 77492', 'yellow iron oxide', 'أكسيد الحديد الأصفر'],
    usageScope: ['skin'],
    functionalCategory: 'محسن جمالي',
    chemicalType: 'ملون معدني',
    benefits: { 'يعطي لونا أصفر': 0.95 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'ci-77499-black-iron-oxide',
    name: 'CI 77499 (أكسيد الحديد الأسود)',
    scientific_name: 'Iron Oxides (CI 77499)',
    searchKeywords: ['ci 77499', 'black iron oxide', 'أكسيد الحديد الأسود'],
    usageScope: ['skin'],
    functionalCategory: 'محسن جمالي',
    chemicalType: 'ملون معدني',
    benefits: { 'يعطي لونا أسود': 0.95 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'ci-77288-chromium-oxide-green',
    name: 'CI 77288 (أخضر أكسيد الكروم)',
    scientific_name: 'Chromium Oxide Greens (CI 77288)',
    searchKeywords: ['ci 77288', 'chromium oxide green'],
    usageScope: ['skin'],
    functionalCategory: 'محسن جمالي',
    chemicalType: 'ملون معدني',
    benefits: { 'يعطي لونا أخضر': 0.95 },
    synergy: {},
    negativeSynergy: {},
    warnings: [{ level: 'risk', text: 'غير مسموح به في منتجات الشفاه في الولايات المتحدة.' }],
  },
{
    id: 'ci-77742-manganese-violet',
    name: 'CI 77742 (بنفسجي المنغنيز)',
    scientific_name: 'Manganese Violet (CI 77742)',
    searchKeywords: ['ci 77742', 'manganese violet'],
    usageScope: ['skin'],
    functionalCategory: 'محسن جمالي',
    chemicalType: 'ملون معدني',
    benefits: { 'يعطي لونا بنفسجيا': 0.95 },
    synergy: {},
    negativeSynergy: {},
    warnings: [{ level: 'risk', text: 'غير مسموح به في منتجات الشفاه في الولايات المتحدة.' }],
  },
{
    id: 'ci-77007-ultramarines',
    name: 'CI 77007 (ألترامارين)',
    scientific_name: 'Ultramarines (CI 77007)',
    searchKeywords: ['ci 77007', 'ultramarines'],
    usageScope: ['skin'],
    functionalCategory: 'محسن جمالي',
    chemicalType: 'ملون معدني',
    benefits: { 'يعطي لونا أزرق أو بنفسجي': 0.95 },
    synergy: {},
    negativeSynergy: {},
    warnings: [{ level: 'risk', text: 'غير مسموح به في منتجات الشفاه في الولايات المتحدة.' }],
  },
{
    id: 'ci-77163-bismuth-oxychloride',
    name: 'CI 77163 (أوكسي كلوريد البزموت)',
    scientific_name: 'Bismuth Oxychloride (CI 77163)',
    searchKeywords: ['ci 77163', 'bismuth oxychloride'],
    usageScope: ['skin'],
    functionalCategory: 'محسن جمالي',
    chemicalType: 'ملون معدني',
    benefits: { 'يعطي لمعانا لؤلؤيا': 0.92 },
    synergy: {},
    negativeSynergy: {},
    warnings: [{ level: 'caution', text: 'قد يسبب تهيجا وحكة لدى البعض.' }],
  },
{
    id: 'ci-77510-ferric-ferrocyanide',
    name: 'CI 77510 (أزرق بروسي)',
    scientific_name: 'Ferric Ferrocyanide (CI 77510)',
    searchKeywords: ['ci 77510', 'ferric ferrocyanide', 'prussian blue'],
    usageScope: ['skin'],
    functionalCategory: 'محسن جمالي',
    chemicalType: 'ملون معدني',
    benefits: { 'يعطي لونا أزرق داكن': 0.95 },
    synergy: {},
    negativeSynergy: {},
    warnings: [{ level: 'risk', text: 'غير مسموح به في منتجات الشفاه في الولايات المتحدة.' }],
  },
{
    id: 'ci-16035-red-40',
    name: 'CI 16035 (Red 40 Lake)',
    scientific_name: 'CI 16035',
    searchKeywords: ['ci 16035', 'red 40 lake'],
    usageScope: ['skin'],
    functionalCategory: 'محسن جمالي',
    chemicalType: 'ملون عضوي',
    benefits: { 'يعطي لونا أحمر': 0.9 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'ci-42090-blue-1',
    name: 'CI 42090 (Blue 1 Lake)',
    scientific_name: 'CI 42090',
    searchKeywords: ['ci 42090', 'blue 1 lake'],
    usageScope: ['skin'],
    functionalCategory: 'محسن جمالي',
    chemicalType: 'ملون عضوي',
    benefits: { 'يعطي لونا أزرق': 0.95 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'ci-45410-red-28',
    name: 'CI 45410 (Red 28 Lake)',
    scientific_name: 'CI 45410',
    searchKeywords: ['ci 45410', 'red 28 lake'],
    usageScope: ['skin'],
    functionalCategory: 'محسن جمالي',
    chemicalType: 'ملون عضوي',
    benefits: { 'يعطي لونا ورديا زاهيا': 0.95 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'carmine-ci-75470',
    name: 'كارمين (CI 75470)',
    scientific_name: 'Carmine (CI 75470)',
    searchKeywords: ['carmine', 'ci 75470', 'cochineal extract', 'كارمين'],
    usageScope: ['skin'],
    functionalCategory: 'محسن جمالي',
    chemicalType: 'ملون طبيعي',
    benefits: { 'يعطي لونا أحمر قرمزي': 0.95 },
    synergy: {},
    negativeSynergy: {},
    warnings: [{ level: 'risk', text: 'مستخلص من الحشرات (الدودة القرمزية)، لذا فهو ليس نباتيا. قد يسبب ردود فعل تحسسية شديدة لدى البعض.' }],
  },
{
    id: 'polyvinyl-alcohol',
    name: 'كحول البوليفينيل (PVA)',
    scientific_name: 'Polyvinyl Alcohol',
    searchKeywords: ['polyvinyl alcohol', 'pva'],
    usageScope: ['skin'],
    functionalCategory: 'مكون طبقة رقيقة',
    chemicalType: 'بوليمر صناعي',
    benefits: { 'تكوين طبقة قابلة للنزع': 0.95 },
    synergy: {},
    negativeSynergy: {},
    warnings: [{ level: 'info', text: 'المكون الرئيسي في الأقنعة القابلة للنزع (peel-off masks).' }],
  },
{
    id: 'nitrocellulose',
    name: 'نيتروسليلوز',
    scientific_name: 'Nitrocellulose',
    searchKeywords: ['nitrocellulose'],
    usageScope: ['skin'],
    functionalCategory: 'مكون طبقة رقيقة',
    chemicalType: 'بوليمر',
    benefits: { 'تكوين طبقة صلبة ولامعة': 0.95 },
    synergy: {},
    negativeSynergy: {},
    warnings: [{ level: 'info', text: 'المكون الرئيسي لتشكيل الطبقة في طلاء الأظافر.' }],
  },
{
    id: 'shellac',
    name: 'شيلاك',
    scientific_name: 'Shellac',
    searchKeywords: ['shellac', 'شيلاك'],
    usageScope: ['hair'],
    functionalCategory: 'مكون طبقة رقيقة',
    chemicalType: 'راتنج طبيعي',
    benefits: { 'تثبيت الشعر': 0.85, 'إضافة لمعان': 0.8 },
    synergy: {},
    negativeSynergy: {},
    warnings: [{ level: 'info', text: 'راتنج طبيعي من الحشرات، يستخدم في بعض مثبتات الشعر.' }],
  },
{
    id: 'tosylamide-formaldehyde-resin',
    name: 'راتنج توسيلاميد/فورمالديهايد',
    scientific_name: 'Tosylamide/Formaldehyde Resin',
    searchKeywords: ['tosylamide/formaldehyde resin'],
    usageScope: ['skin'],
    functionalCategory: 'مكون طبقة رقيقة',
    chemicalType: 'بوليمر',
    benefits: { 'تحسين متانة طلاء الأظافر': 0.9 },
    synergy: {},
    negativeSynergy: {},
    warnings: [{ level: 'risk', text: 'قد يسبب حساسية تلامسية. العديد من العلامات التجارية تتجنبه الآن (3-Free).' }],
  },
{
    id: 'acrylates-ethylhexyl-acrylate-copolymer',
    name: 'أكريلات/إيثيل هكسيل أكريلات كوبوليمر',
    scientific_name: 'Acrylates/Ethylhexyl Acrylate Copolymer',
    searchKeywords: ['acrylates/ethylhexyl acrylate copolymer'],
    usageScope: ['skin'],
    functionalCategory: 'مكون طبقة رقيقة',
    chemicalType: 'بوليمر',
    benefits: { 'مقاومة للماء': 0.9, 'تثبيت المكياج': 0.88 },
    synergy: {},
    negativeSynergy: {},
    warnings: [{ level: 'info', text: 'يستخدم في محدد العيون السائل والماسكارا المقاومة للماء.' }],
  },
{
    id: 'copper-peptides',
    name: 'ببتيدات النحاس (GHK-Cu)',
    scientific_name: 'Copper Tripeptide-1',
    searchKeywords: ['copper peptides', 'ghk-cu', 'copper tripeptide-1', 'ببتيدات النحاس', 'جي إتش كي'],
    usageScope: ['skin', 'hair'],
    functionalCategory: 'مكون فعال',
    chemicalType: 'ببتيد',
    benefits: { 'إصلاح البشرة': 0.95, 'تحفيز الكولاجين': 0.9, 'مضاد للالتهابات': 0.88, 'صحة فروة الرأس': 0.85, 'تسريع التئام الجروح': 0.92 },
    synergy: { 'hyaluronic-acid-serum': 1.1 },
    negativeSynergy: {
      'vitamin-c': { reason: 'قد يتأكسد كل منهما الآخر ويقلل من فعاليتهما، خاصة مع حمض الأسكوربيك النقي.' },
      'glycolic-acid': { reason: 'الأحماض القوية ذات درجة الحموضة المنخفضة جدا قد تفكك روابط الببتيد.' }
    },
    warnings: [
      { level: 'caution', text: 'لا ينصح بخلطه مباشرة مع الأحماض القوية (AHA/BHA) أو فيتامين C النقي (L-Ascorbic Acid) في نفس الخطوة من الروتين.' },
      { level: 'info', text: 'مكون قوي لإصلاح البشرة وتجديدها، ويعزز صحة فروة الرأس لتحفيز نمو الشعر.' }
    ],
  },
{
    id: 'minoxidil',
    name: 'مينوكسيديل',
    scientific_name: 'Minoxidil',
    searchKeywords: ['minoxidil', 'rogaine', 'regaine', 'مينوكسيديل', 'روجين', 'ريجين'],
    usageScope: ['hair'],
    functionalCategory: 'مكون فعال',
    chemicalType: 'مكون دوائي',
    benefits: { 'علاج تساقط الشعر الوراثي': 0.98, 'تحفيز نمو الشعر': 0.95, 'زيادة كثافة الشعر': 0.9 },
    synergy: { 'azelaic-acid': 1.05, 'retinoid-serum': 1.08 },
    negativeSynergy: {},
    warnings: [
      { level: 'risk', text: 'مكون دوائي وليس تجميليا. يجب استخدامه تحت إشراف طبي.' },
      { level: 'caution', text: 'قد يسبب تساقط شعر مؤقت في بداية الاستخدام (shedding). قد تشمل الآثار الجانبية تهيج الفروة، جفاف، أو نمو شعر غير مرغوب فيه في مناطق أخرى.' },
      { level: 'risk', text: 'غير آمن للاستخدام أثناء الحمل أو الرضاعة.' }
    ],
  },
{
    id: 'polysilicone-11',
    name: 'بولي سيليكون-11',
    scientific_name: 'Polysilicone-11',
    searchKeywords: ['polysilicone-11', 'gransil', 'silicone elastomer', 'بولي سيليكون 11'],
    usageScope: ['skin'],
    functionalCategory: 'محسن قوام',
    chemicalType: 'سيليكون (إلاستومر)',
    benefits: { 'ملمس مخملي ومطفي': 0.95, 'تأثير بصري لتمويه التجاعيد': 0.9, 'امتصاص الزيوت': 0.85, 'يعطي ملمسا ناعما': 0.92 },
    synergy: { 'isododecane': 1.05 },
    negativeSynergy: {},
    warnings: [
      { level: 'info', text: 'سيليكون إلاستومري يشكل شبكة على الجلد، مما يعطي تأثير "soft-focus" لتمويه الخطوط الدقيقة والمسام ويمنح ملمسا بودريا أنيقا.' }
    ],
  },
{
    id: 'sidr-leaf-powder',
    name: 'مسحوق أوراق السدر',
    scientific_name: 'Ziziphus Spina-Christi Leaf Powder',
    searchKeywords: ['sidr powder', 'ziziphus spina-christi', 'سدر مطحون', 'مسحوق السدر', 'السدر'],
    usageScope: ['hair'],
    functionalCategory: 'منظف',
    chemicalType: 'منظف طبيعي',
    benefits: { 'تنظيف لطيف': 0.9, 'تقوية الشعر': 0.88, 'تكثيف الشعر': 0.85, 'علاج القشرة': 0.8 },
    synergy: {},
    negativeSynergy: {},
    warnings: [{ level: 'info', text: 'منظف طبيعي تقليدي غني بالصابونين، يستخدم كبديل للشامبو (غسول للشعر).' }],
  },
{
    id: 'myrrh-essential-oil',
    name: 'زيت المر العطري',
    scientific_name: 'Commiphora Myrrha Oil',
    searchKeywords: ['myrrh oil', 'commiphora myrrha', 'المر', 'زيت المر'],
    usageScope: ['skin'],
    functionalCategory: 'ملطف ومرمم',
    chemicalType: 'زيت عطري',
    benefits: { 'ملطف ومرمم': 0.9, 'مضاد للميكروبات': 0.88, 'مضاد للالتهابات': 0.85, 'مكافحة الشيخوخة': 0.8 },
    synergy: { 'frankincense-essential': 1.1 },
    negativeSynergy: {},
    warnings: [{ level: 'info', text: 'يستخدم بتركيزات منخفضة في المنتجات النهائية.' }],
  },
{
    id: 'liquorice-root-extract',
    name: 'مستخلص جذور عرق السوس',
    scientific_name: 'Glycyrrhiza Glabra (Licorice) Root Extract',
    searchKeywords: ['licorice root extract', 'glycyrrhiza glabra', 'عرق السوس', 'مستخلص عرق السوس'],
    usageScope: ['skin'],
    functionalCategory: 'مكون فعال',
    chemicalType: 'مستخلص نباتي',
    benefits: { 'تفتيح وتوحيد اللون': 0.9, 'مضاد للالتهابات': 0.92, 'ملطف ومرمم': 0.88 },
    synergy: { 'niacinamide': 1.1 },
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'sandalwood-essential-oil',
    name: 'زيت خشب الصندل العطري',
    scientific_name: 'Santalum Album Oil',
    searchKeywords: ['sandalwood oil', 'santalum album', 'خشب الصندل', 'زيت الصندل'],
    usageScope: ['skin'],
    functionalCategory: 'ملطف ومرمم',
    chemicalType: 'زيت عطري',
    benefits: { 'ملطف ومرمم': 0.9, 'مضاد للالتهابات': 0.85, 'رائحة مهدئة': 0.92 },
    synergy: {},
    negativeSynergy: {},
    warnings: [{ level: 'caution', text: 'تأكد من أنه من مصادر مستدامة حيث أن خشب الصندل مهدد بالانقراض.' }],
  },
{
    id: 'turmeric-root-extract',
    name: 'مستخلص جذور الكركم',
    scientific_name: 'Curcuma Longa (Turmeric) Root Extract',
    searchKeywords: ['turmeric extract', 'curcuma longa', 'curcumin', 'كركم', 'مستخلص الكركم'],
    usageScope: ['skin'],
    functionalCategory: 'مضاد أكسدة',
    chemicalType: 'مستخلص نباتي',
    benefits: { 'مضاد للالتهابات': 0.95, 'مضاد أكسدة': 0.9, 'تفتيح وتوحيد اللون': 0.8 },
    synergy: {},
    negativeSynergy: {},
    warnings: [{ level: 'caution', text: 'قد يصبغ البشرة باللون الأصفر مؤقتا.' }],
  },
{
    id: 'saffron-stigma-extract',
    name: 'مستخلص الزعفران',
    scientific_name: 'Crocus Sativus Stigma Extract',
    searchKeywords: ['saffron extract', 'crocus sativus', 'زعفران', 'مستخلص الزعفران'],
    usageScope: ['skin'],
    functionalCategory: 'مضاد أكسدة',
    chemicalType: 'مستخلص نباتي',
    benefits: { 'مضاد أكسدة': 0.92, 'تفتيح وتوحيد اللون': 0.88, 'مضاد للالتهابات': 0.85 },
    synergy: {},
    negativeSynergy: {},
    warnings: [{ level: 'info', text: 'مكون فاخر ومكلف.' }],
  },
{
    id: 'damask-rose-flower-water',
    name: 'ماء الورد الدمشقي',
    scientific_name: 'Rosa Damascena Flower Water',
    searchKeywords: ['rose water', 'rosa damascena', 'ماء ورد', 'ماء الورد'],
    usageScope: ['skin', 'hair'],
    functionalCategory: 'ملطف ومرمم',
    chemicalType: 'مستخلص مائي',
    benefits: { 'ملطف ومرمم': 0.9, 'ترطيب': 0.85, 'قابض للمسام خفيف': 0.8 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'orange-blossom-water',
    name: 'ماء زهر البرتقال',
    scientific_name: 'Citrus Aurantium Amara (Bitter Orange) Flower Water',
    searchKeywords: ['orange blossom water', 'neroli water', 'ماء زهر', 'ماء زهر البرتقال'],
    usageScope: ['skin'],
    functionalCategory: 'ملطف ومرمم',
    chemicalType: 'مستخلص مائي',
    benefits: { 'ملطف ومرمم': 0.88, 'تنشيط البشرة': 0.85, 'تنقية': 0.8 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'alum-powder',
    name: 'مسحوق الشبة',
    scientific_name: 'Potassium Alum',
    searchKeywords: ['alum powder', 'potassium alum', 'شبة', 'بودرة الشبة'],
    usageScope: ['skin'],
    functionalCategory: 'مكون فعال',
    chemicalType: 'ملح معدني',
    benefits: { 'مضاد للتعرق طبيعي': 0.9, 'قابض للمسام': 0.88, 'مضاد للميكروبات': 0.85 },
    synergy: {},
    negativeSynergy: {},
    warnings: [{ level: 'caution', text: 'قد يسبب تهيجا للبشرة الحساسة جدا.' }],
  },
{
    id: 'tragacanth-gum',
    name: 'صمغ الكثيراء',
    scientific_name: 'Astragalus Gummifer Gum',
    searchKeywords: ['tragacanth gum', 'kateera', 'كثيراء', 'صمغ الكثيرة'],
    usageScope: ['hair'],
    functionalCategory: 'محسن قوام',
    chemicalType: 'بوليمر طبيعي',
    benefits: { 'مكثف للقوام': 0.9, 'تثبيت خفيف للشعر': 0.8 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'illipe-butter',
    name: 'زبدة الإيليبي',
    scientific_name: 'Shorea Stenoptera Seed Butter',
    searchKeywords: ['illipe butter', 'shorea stenoptera'],
    usageScope: ['skin'],
    functionalCategory: 'مرطب / مطري',
    chemicalType: 'زبدة',
    benefits: { 'ترطيب عميق': 0.9, 'استعادة مرونة الجلد': 0.88 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'sal-butter',
    name: 'زبدة السال',
    scientific_name: 'Shorea Robusta Seed Butter',
    searchKeywords: ['sal butter', 'shorea robusta'],
    usageScope: ['skin'],
    functionalCategory: 'مرطب / مطري',
    chemicalType: 'زبدة',
    benefits: { 'مقاومة للأكسدة': 0.85, 'ترطيب': 0.88 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'pracaxi-oil',
    name: 'زيت البراكاكسي',
    scientific_name: 'Pentaclethra Macroloba Seed Oil',
    searchKeywords: ['pracaxi oil', 'pentaclethra macroloba'],
    usageScope: ['hair', 'skin'],
    functionalCategory: 'مكيف للشعر',
    chemicalType: 'زيت ناقل',
    benefits: { 'تكييف عميق للشعر': 0.9, 'علاج علامات التمدد': 0.8, 'إضافة لمعان': 0.88 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'sacha-inchi-oil',
    name: 'زيت ساشا إنشي',
    scientific_name: 'Plukenetia Volubilis Seed Oil',
    searchKeywords: ['sacha inchi oil', 'plukenetia volubilis'],
    usageScope: ['skin'],
    functionalCategory: 'ملطف ومرمم',
    chemicalType: 'زيت ناقل',
    benefits: { 'مصدر غني بالأوميغا 3': 0.95, 'مضاد للالتهابات': 0.9, 'مكافحة الشيخوخة': 0.85 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'camelina-oil',
    name: 'زيت الكاميلينا',
    scientific_name: 'Camelina Sativa Seed Oil',
    searchKeywords: ['camelina oil', 'camelina sativa', 'false flax'],
    usageScope: ['skin'],
    functionalCategory: 'ملطف ومرمم',
    chemicalType: 'زيت ناقل',
    benefits: { 'غني بالأوميغا 3': 0.9, 'دعم حاجز البشرة': 0.88 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'tucuma-butter',
    name: 'زبدة التوكوما',
    scientific_name: 'Astrocaryum Tucuma Seed Butter',
    searchKeywords: ['tucuma butter', 'astrocaryum tucuma'],
    usageScope: ['hair', 'skin'],
    functionalCategory: 'مرطب / مطري',
    chemicalType: 'زبدة',
    benefits: { 'بديل للسيليكون': 0.88, 'ترطيب': 0.9, 'إضافة لمعان': 0.85 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'ucuuba-butter',
    name: 'زبدة الأوكوبا',
    scientific_name: 'Virola Surinamensis Seed Butter',
    searchKeywords: ['ucuuba butter', 'virola surinamensis'],
    usageScope: ['skin'],
    functionalCategory: 'مكون فعال',
    chemicalType: 'زبدة',
    benefits: { 'مضاد للالتهابات': 0.9, 'مضاد للميكروبات': 0.85 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'zinc-ricinoleate',
    name: 'ريسينولات الزنك',
    scientific_name: 'Zinc Ricinoleate',
    searchKeywords: ['zinc ricinoleate', 'deodorant active'],
    usageScope: ['skin'],
    functionalCategory: 'مكون فعال',
    chemicalType: 'مزيل روائح',
    benefits: { 'امتصاص الروائح الكريهة': 0.95, 'لا يمنع التعرق': 0.9 },
    synergy: {},
    negativeSynergy: {},
    warnings: [{ level: 'info', text: 'لا يسد المسام أو يمنع عملية التعرق الطبيعية، بل يحبس جزيئات الرائحة.' }],
  },
{
    id: 'saccharomyces-ferment-filtrate',
    name: 'مرشح خميرة السكاروميسيس',
    scientific_name: 'Saccharomyces Ferment Filtrate',
    searchKeywords: ['saccharomyces ferment filtrate', 'deodorant active'],
    usageScope: ['skin'],
    functionalCategory: 'مكون فعال',
    chemicalType: 'مزيل روائح',
    benefits: { 'تحييد الروائح': 0.9, 'غني بالمعادن': 0.8 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'diatomaceous-earth',
    name: 'التراب الدياتومي',
    scientific_name: 'Diatomaceous Earth',
    searchKeywords: ['diatomaceous earth'],
    usageScope: ['skin'],
    functionalCategory: 'ممتص للدهون',
    chemicalType: 'ممتص',
    benefits: { 'امتصاص الرطوبة والزيوت': 0.88, 'مقشر لطيف': 0.8 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'arrowroot-powder',
    name: 'مسحوق الأروروت',
    scientific_name: 'Maranta Arundinacea Root Powder',
    searchKeywords: ['arrowroot powder', 'maranta arundinacea'],
    usageScope: ['skin'],
    functionalCategory: 'ممتص للدهون',
    chemicalType: 'ممتص',
    benefits: { 'امتصاص الرطوبة': 0.85, 'يعطي ملمسا حريريا': 0.8 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'tapioca-starch',
    name: 'نشا التابيوكا',
    scientific_name: 'Tapioca Starch',
    searchKeywords: ['tapioca starch'],
    usageScope: ['skin'],
    functionalCategory: 'ممتص للدهون',
    chemicalType: 'ممتص',
    benefits: { 'امتصاص الزيوت': 0.85, 'تقليل الملمس الدهني': 0.88 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'aluminum-chlorohydrate',
    name: 'كلوروهيدرات الألومنيوم',
    scientific_name: 'Aluminum Chlorohydrate',
    searchKeywords: ['aluminum chlorohydrate', 'antiperspirant'],
    usageScope: ['skin'],
    functionalCategory: 'مكون فعال',
    chemicalType: 'مضاد للتعرق',
    benefits: { 'مضاد للتعرق فعال': 0.95 },
    synergy: {},
    negativeSynergy: {},
    warnings: [{ level: 'caution', text: 'مكون مثير للجدل بسبب ارتباطه المحتمل بمشاكل صحية، على الرغم من أن الهيئات التنظيمية تعتبره آمنا.' }],
  },
{
    id: 'aluminum-zirconium-tetrachlorohydrex-gly',
    name: 'ألومنيوم زركونيوم رباعي كلورو هيدروكسي جلايسين',
    scientific_name: 'Aluminum Zirconium Tetrachlorohydrex Gly',
    searchKeywords: ['aluminum zirconium'],
    usageScope: ['skin'],
    functionalCategory: 'مكون فعال',
    chemicalType: 'مضاد للتعرق',
    benefits: { 'مضاد للتعرق قوي جدا': 0.98 },
    synergy: {},
    negativeSynergy: {},
    warnings: [{ level: 'caution', text: 'أحد أقوى مضادات التعرق المتاحة.' }],
  },
{
    id: 'magnesium-hydroxide',
    name: 'هيدروكسيد المغنيسيوم',
    scientific_name: 'Magnesium Hydroxide',
    searchKeywords: ['magnesium hydroxide', 'milk of magnesia'],
    usageScope: ['skin'],
    functionalCategory: 'مكون فعال',
    chemicalType: 'مزيل روائح',
    benefits: { 'تحييد البكتيريا المسببة للرائحة': 0.9, 'لطيف على البشرة': 0.85 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'sodium-bicarbonate',
    name: 'بيكربونات الصوديوم',
    scientific_name: 'Sodium Bicarbonate',
    searchKeywords: ['sodium bicarbonate', 'baking soda'],
    usageScope: ['skin'],
    functionalCategory: 'مكون فعال',
    chemicalType: 'مزيل روائح',
    benefits: { 'تحييد الروائح': 0.88 },
    synergy: {},
    negativeSynergy: {},
    warnings: [{ level: 'caution', text: 'قاعدي جدا (pH عالي) وقد يسبب تهيجا واضطرابا لحاجز البشرة مع الاستخدام المتكرر.' }],
  },
{
    id: 'lauryl-alcohol',
    name: 'كحول لوريل',
    scientific_name: 'Lauryl Alcohol',
    searchKeywords: ['lauryl alcohol'],
    usageScope: ['skin'],
    functionalCategory: 'محسن قوام',
    chemicalType: 'كحول دهني',
    benefits: { 'مطري': 0.8, 'مكثف': 0.85 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'myristyl-alcohol',
    name: 'كحول الميريستيل',
    scientific_name: 'Myristyl Alcohol',
    searchKeywords: ['myristyl alcohol'],
    usageScope: ['skin'],
    functionalCategory: 'محسن قوام',
    chemicalType: 'كحول دهني',
    benefits: { 'مطري': 0.85, 'مكثف': 0.88 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'isopropyl-lanolate',
    name: 'لانولات الآيزوبروبيل',
    scientific_name: 'Isopropyl Lanolate',
    searchKeywords: ['isopropyl lanolate'],
    usageScope: ['skin'],
    functionalCategory: 'مرطب / مطري',
    chemicalType: 'مطري',
    benefits: { 'مطري': 0.8 },
    synergy: {},
    negativeSynergy: {},
    warnings: [{ level: 'risk', text: 'قد يكون كوميدوجينيك ومسببا للحساسية.' }],
  },
{
    id: 'lanolin-oil',
    name: 'زيت اللانولين',
    scientific_name: 'Lanolin Oil',
    searchKeywords: ['lanolin oil'],
    usageScope: ['skin'],
    functionalCategory: 'مرطب / مطري',
    chemicalType: 'مطري',
    benefits: { 'مطري فعال': 0.9 },
    synergy: {},
    negativeSynergy: {},
    warnings: [{ level: 'risk', text: 'قد يسبب حساسية.' }],
  },
{
    id: 'peg-8-beeswax',
    name: 'شمع النحل PEG-8',
    scientific_name: 'PEG-8 Beeswax',
    searchKeywords: ['peg-8 beeswax'],
    usageScope: ['skin'],
    functionalCategory: 'محسن قوام',
    chemicalType: 'مستحلب',
    benefits: { 'مستحلب': 0.85 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'laureth-1',
    name: 'لوريث-1',
    scientific_name: 'Laureth-1',
    searchKeywords: ['laureth-1'],
    usageScope: ['skin'],
    functionalCategory: 'محسن قوام',
    chemicalType: 'مستحلب',
    benefits: { 'مستحلب': 0.7 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'laureth-3',
    name: 'لوريث-3',
    scientific_name: 'Laureth-3',
    searchKeywords: ['laureth-3'],
    usageScope: ['skin'],
    functionalCategory: 'محسن قوام',
    chemicalType: 'مستحلب',
    benefits: { 'مستحلب': 0.8 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'isosteareth-2',
    name: 'أيزوستيريث-2',
    scientific_name: 'Isosteareth-2',
    searchKeywords: ['isosteareth-2'],
    usageScope: ['skin'],
    functionalCategory: 'محسن قوام',
    chemicalType: 'مستحلب',
    benefits: { 'مستحلب': 0.8 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'peg-2-stearate',
    name: 'ستيرات PEG-2',
    scientific_name: 'PEG-2 Stearate',
    searchKeywords: ['peg-2 stearate'],
    usageScope: ['skin'],
    functionalCategory: 'محسن قوام',
    chemicalType: 'مستحلب',
    benefits: { 'مستحلب': 0.85 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'glyceryl-isostearate',
    name: 'أيزوستيرات الجليسريل',
    scientific_name: 'Glyceryl Isostearate',
    searchKeywords: ['glyceryl isostearate'],
    usageScope: ['skin'],
    functionalCategory: 'محسن قوام',
    chemicalType: 'مستحلب',
    benefits: { 'مستحلب': 0.85, 'مطري': 0.8 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'polyglyceryl-10-oleate',
    name: 'بولي جليسريل-10 أوليات',
    scientific_name: 'Polyglyceryl-10 Oleate',
    searchKeywords: ['polyglyceryl-10 oleate'],
    usageScope: ['skin'],
    functionalCategory: 'محسن قوام',
    chemicalType: 'مستحلب',
    benefits: { 'مستحلب لطيف': 0.9 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'sodium-borate',
    name: 'بورات الصوديوم (البوراكس)',
    scientific_name: 'Sodium Borate',
    searchKeywords: ['sodium borate', 'borax', 'بوراكس'],
    usageScope: ['skin'],
    functionalCategory: 'محسن قوام',
    chemicalType: 'معدل الحموضة',
    benefits: { 'مستحلب': 0.8, 'ضبط الحموضة': 0.85 },
    synergy: {},
    negativeSynergy: {},
    warnings: [{ level: 'risk', text: 'مكون محظور أو مقيد بشدة في أوروبا وكندا بسبب مخاوف تتعلق بالسمية التناسلية.' }],
  },
{
    id: 'benzoic-acid',
    name: 'حمض البنزويك',
    scientific_name: 'Benzoic Acid',
    searchKeywords: ['benzoic acid', 'حمض البنزويك'],
    usageScope: ['skin'],
    functionalCategory: 'مادة حافظة',
    chemicalType: 'مادة حافظة',
    benefits: { 'حفظ المنتج': 0.9 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'capryloyl-keratin-amino-acids',
    name: 'أحماض أمينية كيراتين كابريلويل',
    scientific_name: 'Capryloyl Keratin Amino Acids',
    searchKeywords: ['capryloyl keratin amino acids'],
    usageScope: ['hair'],
    functionalCategory: 'مكون فعال',
    chemicalType: 'بروتين معدل',
    benefits: { 'تنظيم دهون الفروة': 0.85, 'تقوية الشعر': 0.8 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'sodium-dna',
    name: 'صوديوم DNA',
    scientific_name: 'Sodium DNA',
    searchKeywords: ['sodium dna', 'dna repair'],
    usageScope: ['skin'],
    functionalCategory: 'مكون فعال',
    chemicalType: 'مكون حيوي',
    benefits: { 'إصلاح البشرة': 0.85, 'ترطيب': 0.8 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'rna',
    name: 'RNA',
    scientific_name: 'RNA',
    searchKeywords: ['rna'],
    usageScope: ['skin'],
    functionalCategory: 'مكون فعال',
    chemicalType: 'مكون حيوي',
    benefits: { 'تحفيز تجديد الخلايا': 0.8 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'glycogen',
    name: 'جليكوجين',
    scientific_name: 'Glycogen',
    searchKeywords: ['glycogen', 'جليكوجين'],
    usageScope: ['skin', 'hair'],
    functionalCategory: 'مرطب / مطري',
    chemicalType: 'مرطب',
    benefits: { 'مصدر طاقة للخلايا': 0.85, 'ترطيب': 0.88 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'mannose',
    name: 'مانوز',
    scientific_name: 'Mannose',
    searchKeywords: ['mannose', 'مانوز'],
    usageScope: ['skin'],
    functionalCategory: 'مرطب / مطري',
    chemicalType: 'مرطب',
    benefits: { 'ترطيب': 0.9, 'دعم حاجز البشرة': 0.85 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'hydrolyzed-algae-extract',
    name: 'مستخلص الطحالب المهدرج',
    scientific_name: 'Hydrolyzed Algae Extract',
    searchKeywords: ['hydrolyzed algae extract'],
    usageScope: ['skin'],
    functionalCategory: 'مكون فعال',
    chemicalType: 'مستخلص طحالب',
    benefits: { 'مكافحة الشيخوخة': 0.8, 'ترطيب': 0.85 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'pinus-pinaster-bark-extract',
    name: 'مستخلص لحاء الصنوبر',
    scientific_name: 'Pinus Pinaster Bark Extract',
    searchKeywords: ['pycnogenol', 'pinus pinaster', 'pine bark extract'],
    usageScope: ['skin'],
    functionalCategory: 'مضاد أكسدة',
    chemicalType: 'مستخلص نباتي',
    benefits: { 'مضاد أكسدة قوي جدا': 0.98, 'تحفيز الكولاجين': 0.88, 'مضاد للالتهابات': 0.9 },
    synergy: { 'vitamin-c': 1.1 },
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'hydrolyzed-conchiolin-protein',
    name: 'بروتين الكونكيولين المهدرج (بروتين اللؤلؤ)',
    scientific_name: 'Hydrolyzed Conchiolin Protein',
    searchKeywords: ['hydrolyzed conchiolin protein', 'pearl protein', 'بروتين اللؤلؤ'],
    usageScope: ['skin'],
    functionalCategory: 'مرطب / مطري',
    chemicalType: 'بروتين',
    benefits: { 'ترطيب': 0.88, 'تفتيح وتوحيد اللون': 0.8 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'hydrolyzed-roe',
    name: 'بطارخ مهدرجة (كافيار)',
    scientific_name: 'Hydrolyzed Roe',
    searchKeywords: ['hydrolyzed roe', 'caviar extract'],
    usageScope: ['skin'],
    functionalCategory: 'مكون فعال',
    chemicalType: 'مكون حيوي',
    benefits: { 'تغذية': 0.9, 'مكافحة الشيخوخة': 0.88 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'royal-jelly-extract',
    name: 'مستخلص غذاء ملكات النحل',
    scientific_name: 'Royal Jelly Extract',
    searchKeywords: ['royal jelly extract', 'غذاء ملكات النحل'],
    usageScope: ['skin'],
    functionalCategory: 'مكون فعال',
    chemicalType: 'مكون حيوي',
    benefits: { 'مكافحة الشيخوخة': 0.85, 'مضاد للميكروبات': 0.8, 'تغذية': 0.9 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'bee-venom',
    name: 'سم النحل',
    scientific_name: 'Bee Venom',
    searchKeywords: ['bee venom', 'apitoxin', 'سم النحل'],
    usageScope: ['skin'],
    functionalCategory: 'مكون فعال',
    chemicalType: 'مكون حيوي',
    benefits: { 'تحفيز الكولاجين (تأثير شبيه بالبوتوكس)': 0.88, 'مضاد للالتهابات': 0.8 },
    synergy: {},
    negativeSynergy: {},
    warnings: [{ level: 'risk', text: 'لا يستخدم من قبل الأشخاص الذين يعانون من حساسية لسعات النحل.' }],
  },
{
    id: 'colloidal-oatmeal',
    name: 'دقيق الشوفان الغروي',
    scientific_name: 'Avena Sativa (Oat) Kernel Flour',
    searchKeywords: ['colloidal oatmeal', 'avena sativa kernel flour'],
    usageScope: ['skin'],
    functionalCategory: 'ملطف ومرمم',
    chemicalType: 'مستخلص نباتي',
    benefits: { 'ملطف ومرمم': 0.98, 'مضاد للحكة': 0.95, 'دعم حاجز البشرة': 0.9 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'colloidal-sulfur',
    name: 'الكبريت الغروي',
    scientific_name: 'Sulfur',
    searchKeywords: ['colloidal sulfur', 'sulfur', 'كبريت'],
    usageScope: ['skin'],
    functionalCategory: 'مكون فعال',
    chemicalType: 'مكون علاجي',
    benefits: { 'مكافحة حب الشباب': 0.9, 'مضاد للميكروبات': 0.88, 'تنظيم الإفرازات الدهنية': 0.85 },
    synergy: {},
    negativeSynergy: {},
    warnings: [{ level: 'caution', text: 'له رائحة مميزة وقد يكون مجففا للبشرة.' }],
  },
{
    id: 'colloidal-silver',
    name: 'الفضة الغروية',
    scientific_name: 'Colloidal Silver',
    searchKeywords: ['colloidal silver', 'فضة غروية'],
    usageScope: ['skin'],
    functionalCategory: 'مكون فعال',
    chemicalType: 'معدن',
    benefits: { 'مضاد للميكروبات': 0.88, 'مضاد للالتهابات': 0.8 },
    synergy: {},
    negativeSynergy: {},
    warnings: [{ level: 'caution', text: 'مكون مثير للجدل، استخدامه على المدى الطويل قد يسبب حالة نادرة تسمى "argyria" (تصبغ الجلد باللون الأزرق الرمادي).' }],
  },
{
    id: 'colloidal-gold',
    name: 'الذهب الغروي',
    scientific_name: 'Colloidal Gold',
    searchKeywords: ['colloidal gold', 'ذهب غروي'],
    usageScope: ['skin'],
    functionalCategory: 'مكون فعال',
    chemicalType: 'معدن',
    benefits: { 'مضاد للالتهابات': 0.8, 'توصيل المكونات النشطة': 0.85 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'diamond-powder',
    name: 'مسحوق الألماس',
    scientific_name: 'Diamond Powder',
    searchKeywords: ['diamond powder', 'مسحوق الألماس'],
    usageScope: ['skin'],
    functionalCategory: 'مقشر',
    chemicalType: 'مقشر فيزيائي',
    benefits: { 'تأثير بصري للإشراقة': 0.9, 'تقشير لطيف': 0.8 },
    synergy: {},
    negativeSynergy: {},
    warnings: [{ level: 'info', text: 'مكون فاخر يستخدم لإعطاء إشراقة فورية للبشرة.' }],
  },
{
    id: 'pearl-powder',
    name: 'مسحوق اللؤلؤ',
    scientific_name: 'Pearl Powder',
    searchKeywords: ['pearl powder', 'مسحوق اللؤلؤ'],
    usageScope: ['skin'],
    functionalCategory: 'مكون فعال',
    chemicalType: 'مكون حيوي',
    benefits: { 'تفتيح وتوحيد اللون': 0.85, 'غني بالمعادن والأحماض الأمينية': 0.9 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'conchiolin-powder',
    name: 'مسحوق الكونكيولين',
    scientific_name: 'Conchiolin Powder',
    searchKeywords: ['conchiolin powder'],
    usageScope: ['skin'],
    functionalCategory: 'مرطب / مطري',
    chemicalType: 'بروتين',
    benefits: { 'ترطيب': 0.85 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'silk-powder',
    name: 'مسحوق الحرير',
    scientific_name: 'Silk Powder / Serica Powder',
    searchKeywords: ['silk powder', 'serica powder', 'مسحوق الحرير'],
    usageScope: ['skin'],
    functionalCategory: 'محسن قوام',
    chemicalType: 'بروتين',
    benefits: { 'يعطي ملمسا حريريا': 0.92, 'امتصاص الزيوت': 0.8 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'amber-powder',
    name: 'مسحوق العنبر',
    scientific_name: 'Amber Powder',
    searchKeywords: ['amber powder', 'مسحوق العنبر'],
    usageScope: ['skin'],
    functionalCategory: 'مقشر',
    chemicalType: 'مقشر فيزيائي',
    benefits: { 'تقشير لطيف': 0.8, 'تنشيط البشرة': 0.82 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'nephrite-powder',
    name: 'مسحوق اليشم',
    scientific_name: 'Nephrite Powder',
    searchKeywords: ['nephrite powder', 'jade powder', 'مسحوق اليشم'],
    usageScope: ['skin'],
    functionalCategory: 'مقشر',
    chemicalType: 'مقشر فيزيائي',
    benefits: { 'تقشير لطيف': 0.8, 'تحسين الدورة الدموية': 0.82 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'tourmaline',
    name: 'التورمالين',
    scientific_name: 'Tourmaline',
    searchKeywords: ['tourmaline', 'تورمالين'],
    usageScope: ['skin'],
    functionalCategory: 'مكون فعال',
    chemicalType: 'معدن',
    benefits: { 'تنشيط البشرة': 0.8 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'smithsonite-extract',
    name: 'مستخلص السميثسونايت',
    scientific_name: 'Smithsonite Extract',
    searchKeywords: ['smithsonite extract'],
    usageScope: ['skin'],
    functionalCategory: 'مكون فعال',
    chemicalType: 'معدن',
    benefits: { 'حماية من أضرار الشمس': 0.8 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'malachite-extract',
    name: 'مستخلص الملاكيت',
    scientific_name: 'Malachite Extract',
    searchKeywords: ['malachite extract', 'ملاكيت'],
    usageScope: ['skin'],
    functionalCategory: 'مضاد أكسدة',
    chemicalType: 'معدن',
    benefits: { 'مضاد أكسدة': 0.88, 'إزالة السموم': 0.85 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'hematite-extract',
    name: 'مستخلص الهيماتيت',
    scientific_name: 'Hematite Extract',
    searchKeywords: ['hematite extract', 'هيماتيت'],
    usageScope: ['skin'],
    functionalCategory: 'مكون فعال',
    chemicalType: 'معدن',
    benefits: { 'تحفيز الكولاجين': 0.85, 'غني بالحديد': 0.8 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'squalane-olivate',
    name: 'أوليفات السكوالان',
    scientific_name: 'Squalane Olivate',
    searchKeywords: ['squalane olivate'],
    usageScope: ['skin'],
    functionalCategory: 'مرطب / مطري',
    chemicalType: 'مطري',
    benefits: { 'مطري': 0.8, 'مرطب': 0.85 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'peg-5-glyceryl-stearate',
    name: 'ستيرات الجليسريل PEG-5',
    scientific_name: 'PEG-5 Glyceryl Stearate',
    searchKeywords: ['peg-5 glyceryl stearate'],
    usageScope: ['skin'],
    functionalCategory: 'محسن قوام',
    chemicalType: 'مستحلب',
    benefits: { 'مستحلب': 0.8 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'peg-6-isostearate',
    name: 'أيزوستيرات PEG-6',
    scientific_name: 'PEG-6 Isostearate',
    searchKeywords: ['peg-6 isostearate'],
    usageScope: ['skin'],
    functionalCategory: 'محسن قوام',
    chemicalType: 'مستحلب',
    benefits: { 'مستحلب': 0.8 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'peg-8-laurate',
    name: 'لوريت PEG-8',
    scientific_name: 'PEG-8 Laurate',
    searchKeywords: ['peg-8 laurate'],
    usageScope: ['skin'],
    functionalCategory: 'محسن قوام',
    chemicalType: 'مستحلب',
    benefits: { 'مستحلب': 0.8 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'peg-30-glyceryl-stearate',
    name: 'ستيرات الجليسريل PEG-30',
    scientific_name: 'PEG-30 Glyceryl Stearate',
    searchKeywords: ['peg-30 glyceryl stearate'],
    usageScope: ['skin'],
    functionalCategory: 'محسن قوام',
    chemicalType: 'مستحلب',
    benefits: { 'مستحلب': 0.85 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'peg-40-sorbitan-peroleate',
    name: 'بيروليات السوربيتان PEG-40',
    scientific_name: 'PEG-40 Sorbitan Peroleate',
    searchKeywords: ['peg-40 sorbitan peroleate'],
    usageScope: ['skin'],
    functionalCategory: 'محسن قوام',
    chemicalType: 'مستحلب',
    benefits: { 'مستحلب': 0.85 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'peg-60-glyceryl-isostearate',
    name: 'أيزوستيرات الجليسريل PEG-60',
    scientific_name: 'PEG-60 Glyceryl Isostearate',
    searchKeywords: ['peg-60 glyceryl isostearate'],
    usageScope: ['skin'],
    functionalCategory: 'محسن قوام',
    chemicalType: 'مستحلب',
    benefits: { 'مستحلب': 0.9 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'sodium-trideceth-sulfate',
    name: 'صوديوم ترايديسيث سلفات',
    scientific_name: 'Sodium Trideceth Sulfate',
    searchKeywords: ['sodium trideceth sulfate'],
    usageScope: ['hair'],
    functionalCategory: 'منظف',
    chemicalType: 'منظف قوي',
    benefits: { 'تنظيف فعال': 0.85, 'ألطف من SLS/SLES': 0.8 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'sodium-cocoyl-alaninate',
    name: 'صوديوم كوكويل ألانينات',
    scientific_name: 'Sodium Cocoyl Alaninate',
    searchKeywords: ['sodium cocoyl alaninate'],
    usageScope: ['skin', 'hair'],
    functionalCategory: 'منظف',
    chemicalType: 'منظف لطيف جدا',
    benefits: { 'تنظيف لطيف': 0.92, 'رغوة جيدة': 0.85 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'tea-cocoyl-alaninate',
    name: 'TEA-كوكويل ألانينات',
    scientific_name: 'TEA-Cocoyl Alaninate',
    searchKeywords: ['tea-cocoyl alaninate'],
    usageScope: ['hair'],
    functionalCategory: 'منظف',
    chemicalType: 'منظف لطيف',
    benefits: { 'تنظيف لطيف': 0.88 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'potassium-cocoyl-glutamate',
    name: 'بوتاسيوم كوكويل جلوتامات',
    scientific_name: 'Potassium Cocoyl Glutamate',
    searchKeywords: ['potassium cocoyl glutamate'],
    usageScope: ['skin'],
    functionalCategory: 'منظف',
    chemicalType: 'منظف لطيف',
    benefits: { 'تنظيف لطيف': 0.9 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'sodium-lauroyl-aspartate',
    name: 'صوديوم لورويل أسبارتات',
    scientific_name: 'Sodium Lauroyl Aspartate',
    searchKeywords: ['sodium lauroyl aspartate'],
    usageScope: ['skin'],
    functionalCategory: 'منظف',
    chemicalType: 'منظف لطيف',
    benefits: { 'تنظيف لطيف': 0.88 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'sodium-methyl-oleoyl-taurate',
    name: 'صوديوم ميثيل أولييل توريت',
    scientific_name: 'Sodium Methyl Oleoyl Taurate',
    searchKeywords: ['sodium methyl oleoyl taurate'],
    usageScope: ['hair'],
    functionalCategory: 'منظف',
    chemicalType: 'منظف لطيف',
    benefits: { 'تنظيف لطيف': 0.9, 'رغوة كثيفة': 0.92 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'sodium-cocoyl-taurate',
    name: 'صوديوم كوكويل توريت',
    scientific_name: 'Sodium Cocoyl Taurate',
    searchKeywords: ['sodium cocoyl taurate'],
    usageScope: ['hair'],
    functionalCategory: 'منظف',
    chemicalType: 'منظف لطيف',
    benefits: { 'تنظيف لطيف': 0.85 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'disodium-oleamido-mea-sulfosuccinate',
    name: 'ثنائي الصوديوم أولياميدو MEA-سلفوساكسينات',
    scientific_name: 'Disodium Oleamido MEA-Sulfosuccinate',
    searchKeywords: ['disodium oleamido mea-sulfosuccinate'],
    usageScope: ['hair'],
    functionalCategory: 'منظف',
    chemicalType: 'منظف لطيف',
    benefits: { 'تنظيف لطيف': 0.9 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'sodium-lauryl-sulfoacetate',
    name: 'صوديوم لوريل سلفوأسيتات (SLSA)',
    scientific_name: 'Sodium Lauryl Sulfoacetate',
    searchKeywords: ['sodium lauryl sulfoacetate', 'slsa'],
    usageScope: ['hair'],
    functionalCategory: 'منظف',
    chemicalType: 'منظف لطيف',
    benefits: { 'رغوة غنية': 0.9, 'تنظيف لطيف': 0.88, 'بديل للسلفات': 0.9 },
    synergy: {},
    negativeSynergy: {},
    warnings: [{ level: 'info', text: 'ليس من السلفات. جزيئاته كبيرة ولا تخترق الجلد، مما يجعله ألطف بكثير من SLS.' }],
  },
{
    id: 'hydroxyethylcellulose',
    name: 'هيدروكسي إيثيل السليلوز',
    scientific_name: 'Hydroxyethylcellulose',
    searchKeywords: ['hydroxyethylcellulose', 'hec'],
    usageScope: ['skin', 'hair'],
    functionalCategory: 'محسن قوام',
    chemicalType: 'بوليمر طبيعي',
    benefits: { 'مكثف للقوام': 0.92, 'تكوين طبقة رقيقة': 0.8 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'hydroxypropylcellulose',
    name: 'هيدروكسي بروبيل السليلوز',
    scientific_name: 'Hydroxypropylcellulose',
    searchKeywords: ['hydroxypropylcellulose', 'hpc'],
    usageScope: ['skin'],
    functionalCategory: 'محسن قوام',
    chemicalType: 'بوليمر طبيعي',
    benefits: { 'مكثف للقوام': 0.88, 'تكوين طبقة رقيقة': 0.85 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'hydroxypropyl-starch-phosphate',
    name: 'فوسفات النشا هيدروكسي بروبيل',
    scientific_name: 'Hydroxypropyl Starch Phosphate',
    searchKeywords: ['hydroxypropyl starch phosphate'],
    usageScope: ['skin'],
    functionalCategory: 'محسن قوام',
    chemicalType: 'بوليمر طبيعي',
    benefits: { 'مكثف طبيعي': 0.9, 'يعطي ملمسا مخمليا': 0.88 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'aluminum-starch-octenylsuccinate',
    name: 'ألومنيوم نشا أوكتنيل سكسينات',
    scientific_name: 'Aluminum Starch Octenylsuccinate',
    searchKeywords: ['aluminum starch octenylsuccinate', 'dry-flo'],
    usageScope: ['skin'],
    functionalCategory: 'ممتص للدهون',
    chemicalType: 'ممتص',
    benefits: { 'امتصاص الزيوت': 0.92, 'تقليل الملمس الدهني': 0.9, 'يعطي ملمسا بودريا': 0.88 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'sucrose-laurate',
    name: 'لوريت السكروز',
    scientific_name: 'Sucrose Laurate',
    searchKeywords: ['sucrose laurate'],
    usageScope: ['skin'],
    functionalCategory: 'محسن قوام',
    chemicalType: 'مستحلب',
    benefits: { 'مستحلب لطيف': 0.9, 'منظف خفيف': 0.8 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'sucrose-palmitate',
    name: 'بالميتات السكروز',
    scientific_name: 'Sucrose Palmitate',
    searchKeywords: ['sucrose palmitate'],
    usageScope: ['skin'],
    functionalCategory: 'محسن قوام',
    chemicalType: 'مستحلب',
    benefits: { 'مستحلب لطيف': 0.88, 'مطري': 0.8 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'sorbitan-isostearate',
    name: 'أيزوستيرات السوربيتان',
    scientific_name: 'Sorbitan Isostearate',
    searchKeywords: ['sorbitan isostearate'],
    usageScope: ['skin'],
    functionalCategory: 'محسن قوام',
    chemicalType: 'مستحلب',
    benefits: { 'مستحلب': 0.9 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'glyceryl-stearate-se',
    name: 'جليسريل ستيرات SE',
    scientific_name: 'Glyceryl Stearate SE',
    searchKeywords: ['glyceryl stearate se'],
    usageScope: ['skin'],
    functionalCategory: 'محسن قوام',
    chemicalType: 'مستحلب',
    benefits: { 'مستحلب ذاتي': 0.92, 'يعطي قواما كريميا': 0.88 },
    synergy: {},
    negativeSynergy: {},
    warnings: [{ level: 'info', text: 'SE تعني "Self-Emulsifying"، مما يجعله أسهل في الاستخدام.' }],
  },
{
    id: 'lysolecithin',
    name: 'ليزوليسيثين',
    scientific_name: 'Lysolecithin',
    searchKeywords: ['lysolecithin'],
    usageScope: ['skin'],
    functionalCategory: 'محسن قوام',
    chemicalType: 'مستحلب',
    benefits: { 'مستحلب': 0.85, 'تحسين امتصاص المكونات': 0.88 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'sorbitan-caprylate',
    name: 'كابريلات السوربيتان',
    scientific_name: 'Sorbitan Caprylate',
    searchKeywords: ['sorbitan caprylate'],
    usageScope: ['skin'],
    functionalCategory: 'مادة حافظة',
    chemicalType: 'معزز للمواد الحافظة',
    benefits: { 'معزز للمواد الحافظة': 0.88, 'مضاد للفطريات': 0.85 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'ppg-10-methyl-glucose-ether',
    name: 'PPG-10 ميثيل جلوكوز إيثر',
    scientific_name: 'PPG-10 Methyl Glucose Ether',
    searchKeywords: ['ppg-10 methyl glucose ether'],
    usageScope: ['skin'],
    functionalCategory: 'مرطب / مطري',
    chemicalType: 'مرطب',
    benefits: { 'مرطب': 0.85, 'يقلل اللزوجة': 0.8 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'ppg-20-methyl-glucose-ether',
    name: 'PPG-20 ميثيل جلوكوز إيثر',
    scientific_name: 'PPG-20 Methyl Glucose Ether',
    searchKeywords: ['ppg-20 methyl glucose ether'],
    usageScope: ['skin'],
    functionalCategory: 'مرطب / مطري',
    chemicalType: 'مطري',
    benefits: { 'مطري خفيف': 0.9, 'يعطي ملمسا ناعما': 0.85 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'tilia-cordata-flower-extract',
    name: 'مستخلص زهرة الزيزفون',
    scientific_name: 'Tilia Cordata Flower Extract',
    searchKeywords: ['linden flower extract', 'tilia cordata'],
    usageScope: ['skin'],
    functionalCategory: 'ملطف ومرمم',
    chemicalType: 'مستخلص نباتي',
    benefits: { 'ملطف ومرمم': 0.9, 'مضاد للالتهابات': 0.85 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'verbena-officinalis-extract',
    name: 'مستخلص رعي الحمام',
    scientific_name: 'Verbena Officinalis Extract',
    searchKeywords: ['verbena extract', 'vervain'],
    usageScope: ['skin'],
    functionalCategory: 'ملطف ومرمم',
    chemicalType: 'مستخلص نباتي',
    benefits: { 'ملطف ومرمم': 0.88, 'مضاد للميكروبات': 0.8 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'thymus-vulgaris-extract',
    name: 'مستخلص الزعتر',
    scientific_name: 'Thymus Vulgaris (Thyme) Extract',
    searchKeywords: ['thyme extract', 'thymus vulgaris', 'زعتر'],
    usageScope: ['skin'],
    functionalCategory: 'مكون فعال',
    chemicalType: 'مستخلص نباتي',
    benefits: { 'مضاد للميكروبات': 0.9, 'مضاد أكسدة': 0.85 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'origanum-vulgare-leaf-extract',
    name: 'مستخلص أوراق الأوريجانو',
    scientific_name: 'Origanum Vulgare Leaf Extract',
    searchKeywords: ['oregano extract', 'origanum vulgare', 'أوريجانو'],
    usageScope: ['skin'],
    functionalCategory: 'مكون فعال',
    chemicalType: 'مستخلص نباتي',
    benefits: { 'مضاد للميكروبات قوي': 0.92, 'مضاد أكسدة': 0.88 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'sambucus-nigra-flower-extract',
    name: 'مستخلص زهرة البلسان الأسود',
    scientific_name: 'Sambucus Nigra Flower Extract',
    searchKeywords: ['elderflower extract', 'sambucus nigra'],
    usageScope: ['skin'],
    functionalCategory: 'ملطف ومرمم',
    chemicalType: 'مستخلص نباتي',
    benefits: { 'ملطف ومرمم': 0.88, 'مضاد أكسدة': 0.85 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'hedera-helix-extract',
    name: 'مستخلص اللبلاب',
    scientific_name: 'Hedera Helix (Ivy) Extract',
    searchKeywords: ['ivy extract', 'hedera helix', 'لبلاب'],
    usageScope: ['skin'],
    functionalCategory: 'مكون فعال',
    chemicalType: 'مستخلص نباتي',
    benefits: { 'مضاد للالتهابات': 0.8, 'قابض للمسام': 0.82 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'arnica-montana-flower-extract',
    name: 'مستخلص زهرة العطاس',
    scientific_name: 'Arnica Montana Flower Extract',
    searchKeywords: ['arnica extract', 'arnica montana', 'زهرة العطاس'],
    usageScope: ['skin'],
    functionalCategory: 'ملطف ومرمم',
    chemicalType: 'مستخلص نباتي',
    benefits: { 'مضاد للالتهابات': 0.9, 'علاج الكدمات': 0.88 },
    synergy: {},
    negativeSynergy: {},
    warnings: [{ level: 'risk', text: 'لا يستخدم على الجروح المفتوحة.' }],
  },
{
    id: 'cucumis-sativus-fruit-extract',
    name: 'مستخلص فاكهة الخيار',
    scientific_name: 'Cucumis Sativus (Cucumber) Fruit Extract',
    searchKeywords: ['cucumber extract', 'cucumis sativus', 'خيار'],
    usageScope: ['skin'],
    functionalCategory: 'ملطف ومرمم',
    chemicalType: 'مستخلص نباتي',
    benefits: { 'ملطف ومرمم': 0.92, 'ترطيب': 0.9, 'تقليل الانتفاخ': 0.85 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'dipeptide-2',
    name: 'ثنائي الببتيد-2',
    scientific_name: 'Dipeptide-2',
    searchKeywords: ['dipeptide-2', 'eyeliss'],
    usageScope: ['skin'],
    functionalCategory: 'مكون فعال',
    chemicalType: 'ببتيد',
    benefits: { 'تحسين التصريف اللمفاوي': 0.88, 'تقليل الانتفاخ تحت العين': 0.9 },
    synergy: {},
    negativeSynergy: {},
    warnings: [{ level: 'info', text: 'أحد مكونات مركب "Eyeliss" الشهير لعلاج انتفاخ العين.' }],
  },
{
    id: 'palmitoyl-tetrapeptide-10',
    name: 'بالميتويل رباعي الببتيد-10',
    scientific_name: 'Palmitoyl Tetrapeptide-10',
    searchKeywords: ['palmitoyl tetrapeptide-10', 'crystalide'],
    usageScope: ['skin'],
    functionalCategory: 'مكون فعال',
    chemicalType: 'ببتيد',
    benefits: { 'تحسين إشراقة البشرة': 0.88, 'توحيد لون البشرة': 0.85 },
    synergy: {},
    negativeSynergy: {},
    warnings: [{ level: 'info', text: 'يعرف تجاريا باسم Crystalide™. يساعد على الحصول على بشرة صافية وعاكسة للضوء.' }],
  },
{
    id: 'acetyl-heptapeptide-4',
    name: 'أسيتيل سباعي الببتيد-4',
    scientific_name: 'Acetyl Heptapeptide-4',
    searchKeywords: ['acetyl heptapeptide-4', 'royal epigen'],
    usageScope: ['skin'],
    functionalCategory: 'مكون فعال',
    chemicalType: 'ببتيد',
    benefits: { 'تسريع تجدد البشرة': 0.88, 'مكافحة الشيخوخة': 0.85 },
    synergy: {},
    negativeSynergy: {},
    warnings: [{ level: 'info', text: 'يحاكي تأثير بروتين "Royalactin" الموجود في غذاء ملكات النحل لتسريع تجدد البشرة.' }],
  },
{
    id: 'acetyl-hexapeptide-38',
    name: 'أسيتيل سداسي الببتيد-38',
    scientific_name: 'Acetyl Hexapeptide-38',
    searchKeywords: ['acetyl hexapeptide-38', 'adifyline'],
    usageScope: ['skin'],
    functionalCategory: 'مكون فعال',
    chemicalType: 'ببتيد',
    benefits: { 'زيادة حجم الدهون الموضعي': 0.85, 'تأثير ملء': 0.88 },
    synergy: {},
    negativeSynergy: {},
    warnings: [{ level: 'info', text: 'يعرف تجاريا باسم Adifyline™. يزيد من حجم الأنسجة الدهنية في المناطق التي يتم تطبيقه عليها، مما يعطي تأثير "ملء" للخدود أو الشفاه.' }],
  },
{
    id: 'palmitoyl-tripeptide-28',
    name: 'بالميتويل ثلاثي الببتيد-28',
    scientific_name: 'Palmitoyl Tripeptide-28',
    searchKeywords: ['palmitoyl tripeptide-28'],
    usageScope: ['skin'],
    functionalCategory: 'مكون فعال',
    chemicalType: 'ببتيد',
    benefits: { 'تحفيز الكولاجين': 0.88 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'undecylenoyl-glycine',
    name: 'أنديسيلينويل جلايسين',
    scientific_name: 'Undecylenoyl Glycine',
    searchKeywords: ['undecylenoyl glycine'],
    usageScope: ['skin'],
    functionalCategory: 'مكون فعال',
    chemicalType: 'حمض أميني دهني',
    benefits: { 'مضاد للفطريات': 0.88, 'تنظيم الإفرازات الدهنية': 0.85 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'undecylenic-acid',
    name: 'حمض الأنديسيلينيك',
    scientific_name: 'Undecylenic Acid',
    searchKeywords: ['undecylenic acid'],
    usageScope: ['skin'],
    functionalCategory: 'مادة حافظة',
    chemicalType: 'حمض دهني',
    benefits: { 'مضاد للفطريات': 0.9 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'sodium-trideceth-carboxylate',
    name: 'صوديوم ترايديسيث كاربوكسيلات',
    scientific_name: 'Sodium Trideceth Carboxylate',
    searchKeywords: ['sodium trideceth carboxylate'],
    usageScope: ['hair'],
    functionalCategory: 'منظف',
    chemicalType: 'منظف لطيف',
    benefits: { 'تنظيف لطيف': 0.85 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'disodium-trideceth-sulfosuccinate',
    name: 'ثنائي الصوديوم ترايديسيث سلفوساكسينات',
    scientific_name: 'Disodium Trideceth Sulfosuccinate',
    searchKeywords: ['disodium trideceth sulfosuccinate'],
    usageScope: ['hair'],
    functionalCategory: 'منظف',
    chemicalType: 'منظف لطيف',
    benefits: { 'تنظيف لطيف': 0.9 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'trideceth-5',
    name: 'ترايديسيث-5',
    scientific_name: 'Trideceth-5',
    searchKeywords: ['trideceth-5'],
    usageScope: ['hair'],
    functionalCategory: 'مستحلب',
    chemicalType: 'مستحلب',
    benefits: { 'مستحلب': 0.8 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'c12-13-pareth-3',
    name: 'C12-13 باريث-3',
    scientific_name: 'C12-13 Pareth-3',
    searchKeywords: ['c12-13 pareth-3'],
    usageScope: ['hair'],
    functionalCategory: 'مستحلب',
    chemicalType: 'مستحلب',
    benefits: { 'مستحلب': 0.8 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'ci-77266-black-2',
    name: 'CI 77266 (أسود 2)',
    scientific_name: 'CI 77266 [nano]',
    searchKeywords: ['ci 77266', 'black 2', 'carbon black', 'أسود كربوني'],
    usageScope: ['skin'],
    functionalCategory: 'محسن جمالي',
    chemicalType: 'ملون عضوي',
    benefits: { 'يعطي لونا أسودا كثيفا': 0.98 },
    synergy: {},
    negativeSynergy: {},
    warnings: [{ level: 'info', text: 'الملون الأساسي في معظم محددات العيون السائلة السوداء.' }],
  },
{
    id: 'ci-77000-aluminum-powder',
    name: 'CI 77000 (مسحوق الألومنيوم)',
    scientific_name: 'CI 77000',
    searchKeywords: ['ci 77000', 'aluminum powder'],
    usageScope: ['skin'],
    functionalCategory: 'محسن جمالي',
    chemicalType: 'ملون معدني',
    benefits: { 'يعطي لونا فضيا معدنيا': 0.95 },
    synergy: {},
    negativeSynergy: {},
    warnings: [{ level: 'risk', text: 'غير مسموح به في منتجات الشفاه.' }],
  },
{
    id: 'ci-77400-copper-powder',
    name: 'CI 77400 (مسحوق النحاس)',
    scientific_name: 'CI 77400',
    searchKeywords: ['ci 77400', 'copper powder'],
    usageScope: ['skin'],
    functionalCategory: 'محسن جمالي',
    chemicalType: 'ملون معدني',
    benefits: { 'يعطي لونا نحاسيا معدنيا': 0.95 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'ci-75120-annatto',
    name: 'CI 75120 (أناتو)',
    scientific_name: 'CI 75120',
    searchKeywords: ['ci 75120', 'annatto'],
    usageScope: ['skin'],
    functionalCategory: 'محسن جمالي',
    chemicalType: 'ملون طبيعي',
    benefits: { 'يعطي لونا برتقاليا-أصفر': 0.9 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'ci-73360-red-30-lake',
    name: 'CI 73360 (Red 30 Lake)',
    scientific_name: 'CI 73360',
    searchKeywords: ['ci 73360', 'red 30 lake'],
    usageScope: ['skin'],
    functionalCategory: 'محسن جمالي',
    chemicalType: 'ملون عضوي',
    benefits: { 'يعطي لونا ورديا-أحمر': 0.9 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'alaria-esculenta-extract',
    name: 'مستخلص طحلب الألاريا',
    scientific_name: 'Alaria Esculenta Extract',
    searchKeywords: ['alaria esculenta', 'winged kelp'],
    usageScope: ['skin'],
    functionalCategory: 'مكون فعال',
    chemicalType: 'مستخلص طحالب',
    benefits: { 'مكافحة الشيخوخة': 0.88, 'تحسين صلابة البشرة': 0.85 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'hydrolyzed-rhodophyceae-extract',
    name: 'مستخلص الطحالب الحمراء المهدرج',
    scientific_name: 'Hydrolyzed Rhodophyceae Extract',
    searchKeywords: ['hydrolyzed rhodophyceae extract', 'red algae'],
    usageScope: ['skin'],
    functionalCategory: 'مكون فعال',
    chemicalType: 'مستخلص طحالب',
    benefits: { 'حماية من التلوث': 0.85, 'دعم حاجز البشرة': 0.88 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'jania-rubens-extract',
    name: 'مستخلص جانيا روبنز',
    scientific_name: 'Jania Rubens Extract',
    searchKeywords: ['jania rubens extract', 'taurine-rich algae'],
    usageScope: ['skin'],
    functionalCategory: 'مكون فعال',
    chemicalType: 'مستخلص طحالب',
    benefits: { 'تنشيط طاقة الخلايا': 0.88, 'ترطيب': 0.85 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'nasturtium-officinale-extract',
    name: 'مستخلص الجرجير المائي',
    scientific_name: 'Nasturtium Officinale Extract',
    searchKeywords: ['watercress extract', 'nasturtium officinale'],
    usageScope: ['skin'],
    functionalCategory: 'مكون فعال',
    chemicalType: 'مستخلص نباتي',
    benefits: { 'تنقية البشرة': 0.8, 'مضاد أكسدة': 0.85 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'tropaeolum-majus-extract',
    name: 'مستخلص الكبوسين',
    scientific_name: 'Tropaeolum Majus Flower/Leaf/Stem Extract',
    searchKeywords: ['tropaeolum majus', 'nasturtium'],
    usageScope: ['skin'],
    functionalCategory: 'مكون فعال',
    chemicalType: 'مستخلص نباتي',
    benefits: { 'تحسين أكسجة البشرة': 0.88, 'حماية من الضوء الأزرق': 0.8 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'leontopodium-alpinum-callus-culture-extract',
    name: 'مستخلص مزرعة الكالس من زهرة إديلويس',
    scientific_name: 'Leontopodium Alpinum Callus Culture Extract',
    searchKeywords: ['edelweiss stem cells'],
    usageScope: ['skin'],
    functionalCategory: 'مكون فعال',
    chemicalType: 'خلايا جذعية نباتية',
    benefits: { 'تأثير شد': 0.88, 'مضاد أكسدة': 0.9 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'crithmum-maritimum-callus-culture-filtrate',
    name: 'مرشح مزرعة الكالس من شمر البحر',
    scientific_name: 'Crithmum Maritimum Callus Culture Filtrate',
    searchKeywords: ['sea fennel stem cells'],
    usageScope: ['skin'],
    functionalCategory: 'مكون فعال',
    chemicalType: 'خلايا جذعية نباتية',
    benefits: { 'تفتيح وتوحيد اللون': 0.85, 'تجديد البشرة': 0.88 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'silybum-marianum-fruit-extract',
    name: 'مستخلص فاكهة حليب الشوك',
    scientific_name: 'Silybum Marianum Fruit Extract',
    searchKeywords: ['milk thistle extract', 'silymarin'],
    usageScope: ['skin'],
    functionalCategory: 'مضاد أكسدة',
    chemicalType: 'مستخلص نباتي',
    benefits: { 'مضاد أكسدة': 0.92, 'حماية من أكسدة الدهون': 0.88 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'palmitoyl-dipeptide-10',
    name: 'بالميتويل ثنائي الببتيد-10',
    scientific_name: 'Palmitoyl Dipeptide-10',
    searchKeywords: ['palmitoyl dipeptide-10'],
    usageScope: ['skin'],
    functionalCategory: 'مكون فعال',
    chemicalType: 'ببتيد',
    benefits: { 'دعم بنية الجلد': 0.8 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'tripeptide-29',
    name: 'ثلاثي الببتيد-29',
    scientific_name: 'Tripeptide-29',
    searchKeywords: ['tripeptide-29', 'collagen peptide'],
    usageScope: ['skin'],
    functionalCategory: 'مكون فعال',
    chemicalType: 'ببتيد',
    benefits: { 'تحفيز الكولاجين': 0.85 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'hydrolyzed-aca-catechu-seed-extract',
    name: 'مستخلص بذور الكاتشو المهدرج',
    scientific_name: 'Hydrolyzed Acacia Catechu Seed Extract',
    searchKeywords: ['acacia catechu'],
    usageScope: ['skin'],
    functionalCategory: 'مكون فعال',
    chemicalType: 'مستخلص نباتي',
    benefits: { 'تحفيز الكولاجين': 0.8 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'creatine',
    name: 'كرياتين',
    scientific_name: 'Creatine',
    searchKeywords: ['creatine', 'كرياتين'],
    usageScope: ['skin'],
    functionalCategory: 'مكون فعال',
    chemicalType: 'حمض أميني',
    benefits: { 'تنشيط طاقة الخلايا': 0.85, 'إصلاح البشرة': 0.8 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'calcium-hydroxide',
    name: 'هيدروكسيد الكالسيوم',
    scientific_name: 'Calcium Hydroxide',
    searchKeywords: ['calcium hydroxide', 'slaked lime'],
    usageScope: ['hair'],
    functionalCategory: 'مكون فعال',
    chemicalType: 'معدل الحموضة',
    benefits: { 'فرد الشعر (في كريمات الفرد)': 0.9 },
    synergy: {},
    negativeSynergy: {},
    warnings: [{ level: 'risk', text: 'مادة قلوية قوية جدا تستخدم في كريمات فرد الشعر الكيميائية. مهيجة وحارقة للجلد.' }],
  },
{
    id: 'guanidine-carbonate',
    name: 'كربونات الغوانيدين',
    scientific_name: 'Guanidine Carbonate',
    searchKeywords: ['guanidine carbonate'],
    usageScope: ['hair'],
    functionalCategory: 'مكون فعال',
    chemicalType: 'عامل فرد',
    benefits: { 'فرد الشعر (في كريمات الفرد)': 0.88 },
    synergy: {},
    negativeSynergy: {},
    warnings: [{ level: 'risk', text: 'يستخدم مع هيدروكسيد الكالسيوم لتكوين هيدروكسيد الغوانيدين، وهو عامل فرد "خالٍ من الصودا الكاوية" ولكنه لا يزال قاسيا جدا.' }],
  },
{
    id: 'thioglycolic-acid',
    name: 'حمض الثيوجليكوليك',
    scientific_name: 'Thioglycolic Acid',
    searchKeywords: ['thioglycolic acid', 'hair removal active'],
    usageScope: ['hair'],
    functionalCategory: 'مكون فعال',
    chemicalType: 'عامل مختزل',
    benefits: { 'إزالة الشعر': 0.95 },
    synergy: {},
    negativeSynergy: {},
    warnings: [{ level: 'risk', text: 'المكون النشط في معظم كريمات إزالة الشعر. يكسر روابط ثاني كبريتيد في الكيراتين. مهيج وله رائحة قوية.' }],
  },
{
    id: 'p-phenylenediamine',
    name: 'بارا-فينيلين ديامين (PPD)',
    scientific_name: 'p-Phenylenediamine',
    searchKeywords: ['p-phenylenediamine', 'ppd', 'hair dye'],
    usageScope: ['hair'],
    functionalCategory: 'مكون ضار',
    chemicalType: 'ملون شعر دائم',
    benefits: { 'صبغ الشعر بلون داكن': 0.98 },
    synergy: {},
    negativeSynergy: {},
    warnings: [{ level: 'risk', text: 'مسبب قوي جدا للحساسية وقد يسبب ردود فعل جلدية شديدة. محظور في مستحضرات التجميل التي تلامس الجلد في أوروبا.' }],
  },
{
    id: 'ammonia',
    name: 'الأمونيا',
    scientific_name: 'Ammonia',
    searchKeywords: ['ammonia', 'ammonium hydroxide', 'أمونيا'],
    usageScope: ['hair'],
    functionalCategory: 'مكون ضار',
    chemicalType: 'معدل الحموضة',
    benefits: { 'يفتح قشرة الشعر لدخول الصبغة': 0.95 },
    synergy: {},
    negativeSynergy: {},
    warnings: [{ level: 'risk', text: 'مادة قلوية قاسية تستخدم في صبغات الشعر الدائمة. مهيجة للجهاز التنفسي وتتلف الشعر.' }],
  },
{
    id: 'ethanolamine',
    name: 'إيثانولامين (MEA)',
    scientific_name: 'Ethanolamine',
    searchKeywords: ['ethanolamine', 'mea', 'ammonia-free dye'],
    usageScope: ['hair'],
    functionalCategory: 'مكون فعال',
    chemicalType: 'معدل الحموضة',
    benefits: { 'بديل للأمونيا في الصبغات': 0.9 },
    synergy: {},
    negativeSynergy: {},
    warnings: [{ level: 'caution', text: 'على الرغم من تسويقه كبديل "لطيف"، إلا أنه قد يكون أكثر ضررا للشعر من الأمونيا على المدى الطويل لأنه يسبب تضخما أكبر للشعرة.' }],
  },
{
    id: 'hydrogen-peroxide',
    name: 'بيروكسيد الهيدروجين',
    scientific_name: 'Hydrogen Peroxide',
    searchKeywords: ['hydrogen peroxide', 'developer', 'oxidant', 'ماء أكسجين'],
    usageScope: ['hair'],
    functionalCategory: 'مكون فعال',
    chemicalType: 'عامل مؤكسد',
    benefits: { 'تفتيح لون الشعر': 0.95, 'تثبيت الصبغة': 0.9 },
    synergy: {},
    negativeSynergy: {},
    warnings: [{ level: 'risk', text: 'عامل مؤكسد قوي يسبب ضررا للشعر ويجرده من لونه الطبيعي.' }],
  },
{
    id: 'persulfates',
    name: 'البيرسلفات',
    scientific_name: 'Potassium Persulfate, Ammonium Persulfate',
    searchKeywords: ['persulfate', 'bleach powder', 'بودرة تفتيح'],
    usageScope: ['hair'],
    functionalCategory: 'مكون فعال',
    chemicalType: 'عامل مؤكسد',
    benefits: { 'تفتيح قوي للشعر (سحب اللون)': 0.98 },
    synergy: {},
    negativeSynergy: {},
    warnings: [{ level: 'risk', text: 'المكونات النشطة في بودرة تفتيح الشعر. قاسية جدا وتسبب تلفا شديدا للشعر. مهيجة للجهاز التنفسي.' }],
  },
{
    id: 'magnesium-carbonate',
    name: 'كربونات المغنيسيوم',
    scientific_name: 'Magnesium Carbonate',
    searchKeywords: ['magnesium carbonate'],
    usageScope: ['skin'],
    functionalCategory: 'ممتص للدهون',
    chemicalType: 'ممتص',
    benefits: { 'امتصاص الزيوت': 0.88, 'عامل مضاد للتكتل': 0.85 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'magnesium-sulfate',
    name: 'كبريتات المغنيسيوم (ملح إبسوم)',
    scientific_name: 'Magnesium Sulfate',
    searchKeywords: ['magnesium sulfate', 'epsom salt', 'ملح إبسوم'],
    usageScope: ['skin'],
    functionalCategory: 'مكون فعال',
    chemicalType: 'ملح',
    benefits: { 'تهدئة آلام العضلات': 0.85, 'مكثف للقوام': 0.8 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'magnesium-silicate',
    name: 'سيليكات المغنيسيوم',
    scientific_name: 'Magnesium Silicate',
    searchKeywords: ['magnesium silicate'],
    usageScope: ['skin'],
    functionalCategory: 'ممتص للدهون',
    chemicalType: 'ممتص',
    benefits: { 'امتصاص الزيوت': 0.8, 'مضاد للتكتل': 0.85 },
    synergy: {},
    negativeSynergy: {},
    warnings: [{ level: 'info', text: 'شكل من أشكال التلك.' }],
  },
{
    id: 'calcium-carbonate',
    name: 'كربونات الكالسيوم',
    scientific_name: 'Calcium Carbonate',
    searchKeywords: ['calcium carbonate', 'chalk'],
    usageScope: ['skin'],
    functionalCategory: 'ممتص للدهون',
    chemicalType: 'ممتص',
    benefits: { 'امتصاص الزيوت': 0.8, 'عامل معتم': 0.85 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'calcium-thioglycolate',
    name: 'ثيوجليكولات الكالسيوم',
    scientific_name: 'Calcium Thioglycolate',
    searchKeywords: ['calcium thioglycolate'],
    usageScope: ['hair'],
    functionalCategory: 'مكون فعال',
    chemicalType: 'عامل مختزل',
    benefits: { 'إزالة الشعر': 0.9 },
    synergy: {},
    negativeSynergy: {},
    warnings: [{ level: 'caution', text: 'مكون آخر يستخدم في كريمات إزالة الشعر.' }],
  },
{
    id: 'sodium-thioglycolate',
    name: 'ثيوجليكولات الصوديوم',
    scientific_name: 'Sodium Thioglycolate',
    searchKeywords: ['sodium thioglycolate'],
    usageScope: ['hair'],
    functionalCategory: 'مكون فعال',
    chemicalType: 'عامل مختزل',
    benefits: { 'إزالة الشعر': 0.88 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'potassium-thioglycolate',
    name: 'ثيوجليكولات البوتاسيوم',
    scientific_name: 'Potassium Thioglycolate',
    searchKeywords: ['potassium thioglycolate'],
    usageScope: ['hair'],
    functionalCategory: 'مكون فعال',
    chemicalType: 'عامل مختزل',
    benefits: { 'إزالة الشعر': 0.92 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'sodium-sulfate-c10-16-alkyl',
    name: 'كبريتات ألكيل الصوديوم C10-16',
    scientific_name: 'Sodium C10-16 Alkyl Sulfate',
    searchKeywords: ['sodium c10-16 alkyl sulfate'],
    usageScope: ['hair'],
    functionalCategory: 'منظف',
    chemicalType: 'منظف قاسي',
    benefits: { 'تنظيف عميق': 0.88 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'ppg-5-ceteth-10-phosphate',
    name: 'PPG-5 سيتيث-10 فوسفات',
    scientific_name: 'PPG-5-Ceteth-10 Phosphate',
    searchKeywords: ['ppg-5-ceteth-10 phosphate'],
    usageScope: ['hair'],
    functionalCategory: 'مستحلب',
    chemicalType: 'مستحلب',
    benefits: { 'مستحلب': 0.8, 'منظف لطيف': 0.75 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'coceth-7',
    name: 'كوسيث-7',
    scientific_name: 'Coceth-7',
    searchKeywords: ['coceth-7'],
    usageScope: ['hair'],
    functionalCategory: 'مستحلب',
    chemicalType: 'مستحلب',
    benefits: { 'مستحلب': 0.8 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'hc-yellow-no-2',
    name: 'HC Yellow No. 2',
    scientific_name: 'HC Yellow No. 2',
    searchKeywords: ['hc yellow no. 2'],
    usageScope: ['hair'],
    functionalCategory: 'محسن جمالي',
    chemicalType: 'ملون شعر شبه دائم',
    benefits: { 'إعطاء لون أصفر': 0.9 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: '2-amino-6-chloro-4-nitrophenol',
    name: '2-أمينو-6-كلورو-4-نيتروفينول',
    scientific_name: '2-Amino-6-Chloro-4-Nitrophenol',
    searchKeywords: ['2-amino-6-chloro-4-nitrophenol'],
    usageScope: ['hair'],
    functionalCategory: 'محسن جمالي',
    chemicalType: 'ملون شعر شبه دائم',
    benefits: { 'إعطاء لون': 0.85 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'hydroxyethyl-2-nitro-p-toluidine',
    name: 'هيدروكسي إيثيل-2-نيترو-ب-تولويدين',
    scientific_name: 'Hydroxyethyl-2-Nitro-p-Toluidine',
    searchKeywords: ['hydroxyethyl-2-nitro-p-toluidine'],
    usageScope: ['hair'],
    functionalCategory: 'محسن جمالي',
    chemicalType: 'ملون شعر شبه دائم',
    benefits: { 'إعطاء لون': 0.85 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'toluene-2-5-diamine-sulfate',
    name: 'كبريتات تولوين-2،5-ديامين',
    scientific_name: 'Toluene-2,5-Diamine Sulfate',
    searchKeywords: ['toluene-2,5-diamine sulfate'],
    usageScope: ['hair'],
    functionalCategory: 'محسن جمالي',
    chemicalType: 'ملون شعر دائم',
    benefits: { 'صبغ الشعر': 0.95 },
    synergy: {},
    negativeSynergy: {},
    warnings: [{ level: 'risk', text: 'بديل لـ PPD ولكنه لا يزال مسببا قويا للحساسية.' }],
  },
{
    id: 'm-aminophenol',
    name: 'ميتا-أمينوفينول',
    scientific_name: 'm-Aminophenol',
    searchKeywords: ['m-aminophenol'],
    usageScope: ['hair'],
    functionalCategory: 'محسن جمالي',
    chemicalType: 'ملون شعر دائم',
    benefits: { 'مكون في صبغات الشعر': 0.9 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'p-aminophenol',
    name: 'بارا-أمينوفينول',
    scientific_name: 'p-Aminophenol',
    searchKeywords: ['p-aminophenol'],
    usageScope: ['hair'],
    functionalCategory: 'محسن جمالي',
    chemicalType: 'ملون شعر دائم',
    benefits: { 'مكون في صبغات الشعر': 0.9 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: '4-amino-2-hydroxytoluene',
    name: '4-أمينو-2-هيدروكسي تولوين',
    scientific_name: '4-Amino-2-Hydroxytoluene',
    searchKeywords: ['4-amino-2-hydroxytoluene'],
    usageScope: ['hair'],
    functionalCategory: 'محسن جمالي',
    chemicalType: 'ملون شعر دائم',
    benefits: { 'مكون في صبغات الشعر': 0.9 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: '2-methylresorcinol',
    name: '2-ميثيل ريزورسينول',
    scientific_name: '2-Methylresorcinol',
    searchKeywords: ['2-methylresorcinol'],
    usageScope: ['hair'],
    functionalCategory: 'محسن جمالي',
    chemicalType: 'ملون شعر دائم',
    benefits: { 'مكون في صبغات الشعر': 0.9 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: '1-naphthol',
    name: '1-نافثول',
    scientific_name: '1-Naphthol',
    searchKeywords: ['1-naphthol'],
    usageScope: ['hair'],
    functionalCategory: 'محسن جمالي',
    chemicalType: 'ملون شعر دائم',
    benefits: { 'مكون في صبغات الشعر': 0.9 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'glyceryl-undecylenate',
    name: 'أنديسيلينات الجليسريل',
    scientific_name: 'Glyceryl Undecylenate',
    searchKeywords: ['glyceryl undecylenate'],
    usageScope: ['skin'],
    functionalCategory: 'مادة حافظة',
    chemicalType: 'معزز للمواد الحافظة',
    benefits: { 'مضاد للفطريات': 0.88, 'معزز للمواد الحافظة': 0.85 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'capryl-capramidopropyl-betaine',
    name: 'كابريل/كابر أميدوبروبيل بيتين',
    scientific_name: 'Capryl/Capramidopropyl Betaine',
    searchKeywords: ['capryl/capramidopropyl betaine'],
    usageScope: ['hair'],
    functionalCategory: 'منظف',
    chemicalType: 'منظف معتدل',
    benefits: { 'تنظيف لطيف': 0.85, 'رغوة جيدة': 0.8 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'lauryl-betaine',
    name: 'لوريل بيتين',
    scientific_name: 'Lauryl Betaine',
    searchKeywords: ['lauryl betaine'],
    usageScope: ['hair'],
    functionalCategory: 'منظف',
    chemicalType: 'منظف معتدل',
    benefits: { 'تنظيف لطيف': 0.8, 'معزز للرغوة': 0.85 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'oleamidopropyl-betaine',
    name: 'أولياميدوبروبيل بيتين',
    scientific_name: 'Oleamidopropyl Betaine',
    searchKeywords: ['oleamidopropyl betaine'],
    usageScope: ['hair'],
    functionalCategory: 'منظف',
    chemicalType: 'منظف معتدل',
    benefits: { 'تنظيف لطيف': 0.8, 'مكيف خفيف': 0.82 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'sodium-trideceth-7-carboxylate',
    name: 'صوديوم ترايديسيث-7 كاربوكسيلات',
    scientific_name: 'Sodium Trideceth-7 Carboxylate',
    searchKeywords: ['sodium trideceth-7 carboxylate'],
    usageScope: ['hair'],
    functionalCategory: 'منظف',
    chemicalType: 'منظف لطيف',
    benefits: { 'تنظيف لطيف': 0.88 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'lauramine-oxide',
    name: 'أكسيد لورامين',
    scientific_name: 'Lauramine Oxide',
    searchKeywords: ['lauramine oxide'],
    usageScope: ['hair'],
    functionalCategory: 'منظف',
    chemicalType: 'منظف معتدل',
    benefits: { 'معزز للرغوة': 0.92, 'تنظيف': 0.8 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'oleamine-oxide',
    name: 'أكسيد أوليامين',
    scientific_name: 'Oleamine Oxide',
    searchKeywords: ['oleamine oxide'],
    usageScope: ['hair'],
    functionalCategory: 'مكيف للشعر',
    chemicalType: 'مكيف',
    benefits: { 'مكيف': 0.8, 'معزز للرغوة': 0.85 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'cocamine-oxide',
    name: 'أكسيد كوكامين',
    scientific_name: 'Cocamine Oxide',
    searchKeywords: ['cocamine oxide'],
    usageScope: ['hair'],
    functionalCategory: 'منظف',
    chemicalType: 'منظف معتدل',
    benefits: { 'معزز للرغوة': 0.9 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'cocamidopropylamine-oxide',
    name: 'أكسيد كوكاميدوبروبيل أمين',
    scientific_name: 'Cocamidopropylamine Oxide',
    searchKeywords: ['cocamidopropylamine oxide'],
    usageScope: ['hair'],
    functionalCategory: 'منظف',
    chemicalType: 'منظف معتدل',
    benefits: { 'معزز للرغوة': 0.92, 'لطيف': 0.85 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'behentrimonium-propyltrimonium-chloride',
    name: 'كلوريد بيهينتريمونيوم بروبيل تريمونيوم',
    scientific_name: 'Behentrimonium Propyltrimonium Chloride',
    searchKeywords: ['behentrimonium propyltrimonium chloride'],
    usageScope: ['hair'],
    functionalCategory: 'مكيف للشعر',
    chemicalType: 'مركب أمونيوم رباعي',
    benefits: { 'تكييف': 0.8 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'babassu-oil-polyglyceryl-4-esters',
    name: 'استرات بولي جليسريل-4 لزيت الباباسو',
    scientific_name: 'Babassu Oil Polyglyceryl-4 Esters',
    searchKeywords: ['babassu oil polyglyceryl-4 esters'],
    usageScope: ['skin'],
    functionalCategory: 'مرطب / مطري',
    chemicalType: 'عامل إعادة ترطيب',
    benefits: { 'إعادة ترطيب': 0.88, 'لطيف': 0.85 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'ppg-3-caprylyl-ether',
    name: 'PPG-3 كابريليل إيثر',
    scientific_name: 'PPG-3 Caprylyl Ether',
    searchKeywords: ['ppg-3 caprylyl ether'],
    usageScope: ['hair'],
    functionalCategory: 'مكيف للشعر',
    chemicalType: 'مطري',
    benefits: { 'بديل للسيليكون': 0.9, 'تنعيم الشعر': 0.88 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'pca-glyceryl-oleate',
    name: 'أوليات جليسريل PCA',
    scientific_name: 'PCA Glyceryl Oleate',
    searchKeywords: ['pca glyceryl oleate'],
    usageScope: ['hair'],
    functionalCategory: 'مكيف للشعر',
    chemicalType: 'عامل إعادة ترطيب',
    benefits: { 'إعادة ترطيب': 0.85, 'حماية اللون': 0.8 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'dioleoylethyl-hydroxyethylmonium-methosulfate',
    name: 'ميثوسلفات ثنائي أوليويل إيثيل هيدروكسي إيثيل مونيوم',
    scientific_name: 'Dioleoylethyl Hydroxyethylmonium Methosulfate',
    searchKeywords: ['dioleoylethyl hydroxyethylmonium methosulfate'],
    usageScope: ['hair'],
    functionalCategory: 'مكيف للشعر',
    chemicalType: 'مركب استر رباعي',
    benefits: { 'تكييف': 0.85, 'قابل للتحلل': 0.9 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'quaternium-87',
    name: 'كواتيرنيوم-87',
    scientific_name: 'Quaternium-87',
    searchKeywords: ['quaternium-87'],
    usageScope: ['hair'],
    functionalCategory: 'مكيف للشعر',
    chemicalType: 'مركب استر رباعي',
    benefits: { 'تكييف': 0.88, 'قابل للتحلل': 0.9 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'quaternium-91',
    name: 'كواتيرنيوم-91',
    scientific_name: 'Quaternium-91',
    searchKeywords: ['quaternium-91'],
    usageScope: ['hair'],
    functionalCategory: 'مكيف للشعر',
    chemicalType: 'مركب أمونيوم رباعي',
    benefits: { 'تكييف عميق': 0.9, 'فك تشابك': 0.92 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'cetrimonium-methosulfate',
    name: 'ميثوسلفات السيتريمونيوم',
    scientific_name: 'Cetrimonium Methosulfate',
    searchKeywords: ['cetrimonium methosulfate'],
    usageScope: ['hair'],
    functionalCategory: 'مكيف للشعر',
    chemicalType: 'مركب أمونيوم رباعي',
    benefits: { 'مكيف': 0.8 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'hydroxypropyltrimonium-hydrolyzed-corn-starch',
    name: 'نشا الذرة المهدرج هيدروكسي بروبيل تريمونيوم',
    scientific_name: 'Hydroxypropyltrimonium Hydrolyzed Corn Starch',
    searchKeywords: ['hydroxypropyltrimonium hydrolyzed corn starch'],
    usageScope: ['hair'],
    functionalCategory: 'مكيف للشعر',
    chemicalType: 'بوليمر طبيعي',
    benefits: { 'مكافحة الهيشان': 0.88, 'تحديد التموجات': 0.85 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'hydrolyzed-jojoba-esters',
    name: 'استرات الجوجوبا المهدرجة',
    scientific_name: 'Hydrolyzed Jojoba Esters',
    searchKeywords: ['hydrolyzed jojoba esters'],
    usageScope: ['hair'],
    functionalCategory: 'مكيف للشعر',
    chemicalType: 'مطري',
    benefits: { 'ترطيب': 0.85, 'تكوين طبقة رقيقة': 0.8 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'silica-silylate',
    name: 'سيليلات السيليكا',
    scientific_name: 'Silica Silylate',
    searchKeywords: ['silica silylate'],
    usageScope: ['hair'],
    functionalCategory: 'محسن قوام',
    chemicalType: 'سيليكون',
    benefits: { 'إضافة حجم وكثافة': 0.9, 'تأثير مطفي': 0.88 },
    synergy: {},
    negativeSynergy: {},
    warnings: [{ level: 'info', text: 'يستخدم في مساحيق الشعر لإعطاء حجم وامتصاص الزيوت.' }],
  },
{
    id: 'silica-dimethyl-silylate',
    name: 'سيليلات ثنائي ميثيل السيليكا',
    scientific_name: 'Silica Dimethyl Silylate',
    searchKeywords: ['silica dimethyl silylate'],
    usageScope: ['hair', 'skin'],
    functionalCategory: 'محسن قوام',
    chemicalType: 'سيليكون',
    benefits: { 'مكثف للزيوت': 0.9, 'إضافة حجم': 0.85 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'calcium-silicate',
    name: 'سيليكات الكالسيوم',
    scientific_name: 'Calcium Silicate',
    searchKeywords: ['calcium silicate'],
    usageScope: ['skin'],
    functionalCategory: 'ممتص للدهون',
    chemicalType: 'ممتص',
    benefits: { 'امتصاص الزيوت': 0.85 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'sodium-silicate',
    name: 'سيليكات الصوديوم',
    scientific_name: 'Sodium Silicate',
    searchKeywords: ['sodium silicate', 'liquid glass'],
    usageScope: ['skin'],
    functionalCategory: 'مكون فعال',
    chemicalType: 'معدن',
    benefits: { 'شد فوري للبشرة (مؤقت)': 0.9 },
    synergy: {},
    negativeSynergy: {},
    warnings: [{ level: 'info', text: 'يشكل طبقة رقيقة على الجلد تتقلص عند جفافها، مما يعطي تأثير شد فوري ومؤقت للتجاعيد.' }],
  },
{
    id: 'perlite',
    name: 'البيرلايت',
    scientific_name: 'Perlite',
    searchKeywords: ['perlite', 'volcanic glass', 'بيرلايت'],
    usageScope: ['skin'],
    functionalCategory: 'ممتص للدهون',
    chemicalType: 'معدن',
    benefits: { 'امتصاص فائق للماء والزيوت': 0.92, 'خفيف الوزن': 0.9 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'ricinoleamidopropyltrimonium-chloride',
    name: 'كلوريد ريسينولياميدوبروبيل تريمونيوم',
    scientific_name: 'Ricinoleamidopropyltrimonium Chloride',
    searchKeywords: ['ricinoleamidopropyltrimonium chloride'],
    usageScope: ['hair'],
    functionalCategory: 'مكيف للشعر',
    chemicalType: 'مركب أمونيوم رباعي',
    benefits: { 'مكيف': 0.85, 'مشتق من زيت الخروع': 0.8 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'cocotrimonium-chloride',
    name: 'كلوريد كوكوتريمونيوم',
    scientific_name: 'Cocotrimonium Chloride',
    searchKeywords: ['cocotrimonium chloride'],
    usageScope: ['hair'],
    functionalCategory: 'مكيف للشعر',
    chemicalType: 'مركب أمونيوم رباعي',
    benefits: { 'مضاد للكهرباء الساكنة': 0.8 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'laurtrimonium-chloride',
    name: 'كلوريد لوريتريمونيوم',
    scientific_name: 'Laurtrimonium Chloride',
    searchKeywords: ['laurtrimonium chloride'],
    usageScope: ['hair'],
    functionalCategory: 'مكيف للشعر',
    chemicalType: 'مركب أمونيوم رباعي',
    benefits: { 'مضاد للكهرباء الساكنة': 0.85 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'dicocodimonium-chloride',
    name: 'كلوريد دياكوكوديمونيوم',
    scientific_name: 'Dicocodimonium Chloride',
    searchKeywords: ['dicocodimonium chloride'],
    usageScope: ['hair'],
    functionalCategory: 'مكيف للشعر',
    chemicalType: 'مركب أمونيوم رباعي',
    benefits: { 'مكيف': 0.8 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'dihydroxypropyltrimonium-chloride',
    name: 'كلوريد داي هيدروكسي بروبيل تريمونيوم',
    scientific_name: 'Dihydroxypropyltrimonium Chloride',
    searchKeywords: ['dihydroxypropyltrimonium chloride'],
    usageScope: ['hair'],
    functionalCategory: 'مكيف للشعر',
    chemicalType: 'مركب أمونيوم رباعي',
    benefits: { 'مكيف': 0.8 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'hydroxypropyl-guar-hydroxypropyltrimonium-chloride',
    name: 'كلوريد هيدروكسي بروبيل غوار هيدروكسي بروبيل تريمونيوم',
    scientific_name: 'Hydroxypropyl Guar Hydroxypropyltrimonium Chloride',
    searchKeywords: ['hydroxypropyl guar hydroxypropyltrimonium chloride', 'jaguar c-17'],
    usageScope: ['hair'],
    functionalCategory: 'مكيف للشعر',
    chemicalType: 'بوليمر طبيعي',
    benefits: { 'تكييف': 0.88, 'مكثف': 0.85 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'sodium-lauryl-ether-sulfate',
    name: 'صوديوم لوريل إيثر سلفات',
    scientific_name: 'Sodium Lauryl Ether Sulfate',
    searchKeywords: ['sodium lauryl ether sulfate', 'SLES'],
    usageScope: ['hair', 'skin'],
    functionalCategory: 'منظف',
    chemicalType: 'منظف قوي',
    benefits: { 'تنظيف فعال': 0.9, 'رغوة غنية': 0.95 },
    synergy: {},
    negativeSynergy: {},
    warnings: [{ level: 'info', text: 'اسم آخر شائع لـ Sodium Laureth Sulfate.' }],
  },
{
    id: 'tridecyl-stearate',
    name: 'ستيرات الترايديسيل',
    scientific_name: 'Tridecyl Stearate',
    searchKeywords: ['tridecyl stearate'],
    usageScope: ['skin'],
    functionalCategory: 'مرطب / مطري',
    chemicalType: 'مطري',
    benefits: { 'مطري': 0.85 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'tridecyl-trimellitate',
    name: 'تريميليتات الترايديسيل',
    scientific_name: 'Tridecyl Trimellitate',
    searchKeywords: ['tridecyl trimellitate'],
    usageScope: ['skin'],
    functionalCategory: 'مرطب / مطري',
    chemicalType: 'مطري',
    benefits: { 'مطري غني': 0.9, 'يعطي لمعانا': 0.85 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'neopentyl-glycol-dicaprylate-dicaprate',
    name: 'ثنائي الكابريلات/الكبرات نيوبنتيل جلايكول',
    scientific_name: 'Neopentyl Glycol Dicaprylate/Dicaprate',
    searchKeywords: ['neopentyl glycol dicaprylate/dicaprate'],
    usageScope: ['skin'],
    functionalCategory: 'مرطب / مطري',
    chemicalType: 'مطري',
    benefits: { 'مطري خفيف': 0.88 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'c12-15-pareth-3',
    name: 'C12-15 باريث-3',
    scientific_name: 'C12-15 Pareth-3',
    searchKeywords: ['c12-15 pareth-3'],
    usageScope: ['hair'],
    functionalCategory: 'مستحلب',
    chemicalType: 'مستحلب',
    benefits: { 'مستحلب': 0.8 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'laureth-5-carboxylic-acid',
    name: 'حمض لوريث-5 الكربوكسيلي',
    scientific_name: 'Laureth-5 Carboxylic Acid',
    searchKeywords: ['laureth-5 carboxylic acid'],
    usageScope: ['hair'],
    functionalCategory: 'منظف',
    chemicalType: 'منظف لطيف',
    benefits: { 'تنظيف لطيف': 0.88 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'peg-15-glyceryl-isostearate',
    name: 'أيزوستيرات الجليسريل PEG-15',
    scientific_name: 'PEG-15 Glyceryl Isostearate',
    searchKeywords: ['peg-15 glyceryl isostearate'],
    usageScope: ['skin'],
    functionalCategory: 'مستحلب',
    chemicalType: 'مستحلب',
    benefits: { 'مستحلب': 0.8 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'steareth-25',
    name: 'ستيريث-25',
    scientific_name: 'Steareth-25',
    searchKeywords: ['steareth-25', 'ستياريث-25'],
    usageScope: ['hair'],
    functionalCategory: 'مستحلب',
    chemicalType: 'مستحلب',
    benefits: { 'مستحلب': 0.85 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'ceteareth-12',
    name: 'سيتاريث-12',
    scientific_name: 'Ceteareth-12',
    searchKeywords: ['ceteareth-12'],
    usageScope: ['skin'],
    functionalCategory: 'محسن قوام',
    chemicalType: 'مستحلب',
    benefits: { 'مستحلب': 0.88 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'ceteareth-30',
    name: 'سيتاريث-30',
    scientific_name: 'Ceteareth-30',
    searchKeywords: ['ceteareth-30'],
    usageScope: ['hair'],
    functionalCategory: 'محسن قوام',
    chemicalType: 'مستحلب',
    benefits: { 'مستحلب': 0.9 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'peg-6',
    name: 'PEG-6',
    scientific_name: 'PEG-6',
    searchKeywords: ['peg-6'],
    usageScope: ['skin'],
    functionalCategory: 'مرطب / مطري',
    chemicalType: 'مرطب',
    benefits: { 'مرطب': 0.8, 'مذيب': 0.82 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'peg-32',
    name: 'PEG-32',
    scientific_name: 'PEG-32',
    searchKeywords: ['peg-32'],
    usageScope: ['skin'],
    functionalCategory: 'مرطب / مطري',
    chemicalType: 'مرطب',
    benefits: { 'مرطب': 0.88, 'مذيب': 0.85 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'peg-100',
    name: 'PEG-100',
    scientific_name: 'PEG-100',
    searchKeywords: ['peg-100'],
    usageScope: ['skin'],
    functionalCategory: 'مرطب / مطري',
    chemicalType: 'مرطب',
    benefits: { 'مرطب': 0.85 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'laureth-12',
    name: 'لوريث-12',
    scientific_name: 'Laureth-12',
    searchKeywords: ['laureth-12'],
    usageScope: ['hair'],
    functionalCategory: 'مستحلب',
    chemicalType: 'مستحلب',
    benefits: { 'مستحلب': 0.85 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'c12-20-acid-peg-8-ester',
    name: 'استر PEG-8 لحمض C12-20',
    scientific_name: 'C12-20 Acid PEG-8 Ester',
    searchKeywords: ['c12-20 acid peg-8 ester'],
    usageScope: ['skin'],
    functionalCategory: 'محسن قوام',
    chemicalType: 'مستحلب',
    benefits: { 'مستحلب': 0.88 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'peg-8-stearate',
    name: 'ستيرات PEG-8',
    scientific_name: 'PEG-8 Stearate',
    searchKeywords: ['peg-8 stearate'],
    usageScope: ['skin'],
    functionalCategory: 'محسن قوام',
    chemicalType: 'مستحلب',
    benefits: { 'مستحلب': 0.85 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'steareth-10-allyl-ether-acrylates-copolymer',
    name: 'ستيريث-10 أليل إيثر/أكريلات كوبوليمر',
    scientific_name: 'Steareth-10 Allyl Ether/Acrylates Copolymer',
    searchKeywords: ['steareth-10 allyl ether/acrylates copolymer'],
    usageScope: ['hair'],
    functionalCategory: 'مثبت للشعر',
    chemicalType: 'بوليمر صناعي',
    benefits: { 'تثبيت التصفيف': 0.85 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'ammonium-sulfate',
    name: 'كبريتات الأمونيوم',
    scientific_name: 'Ammonium Sulfate',
    searchKeywords: ['ammonium sulfate'],
    usageScope: ['hair'],
    functionalCategory: 'محسن قوام',
    chemicalType: 'ملح',
    benefits: { 'مكثف للقوام': 0.75 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'sodium-polystyrene-sulfonate',
    name: 'صوديوم بوليسترين سلفونات',
    scientific_name: 'Sodium Polystyrene Sulfonate',
    searchKeywords: ['sodium polystyrene sulfonate'],
    usageScope: ['hair', 'skin'],
    functionalCategory: 'مكون طبقة رقيقة',
    chemicalType: 'بوليمر صناعي',
    benefits: { 'شد فوري للبشرة': 0.85, 'تثبيت خفيف للشعر': 0.8 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'poloxamer-188',
    name: 'بولوكسامير 188',
    scientific_name: 'Poloxamer 188',
    searchKeywords: ['poloxamer 188'],
    usageScope: ['skin'],
    functionalCategory: 'منظف',
    chemicalType: 'منظف لطيف',
    benefits: { 'منظف لطيف': 0.88 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'poloxamer-235',
    name: 'بولوكسامير 235',
    scientific_name: 'Poloxamer 235',
    searchKeywords: ['poloxamer 235'],
    usageScope: ['skin'],
    functionalCategory: 'مستحلب',
    chemicalType: 'مستحلب',
    benefits: { 'مستحلب': 0.8 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'polyglyceryl-2-oleate',
    name: 'بولي جليسريل-2 أوليات',
    scientific_name: 'Polyglyceryl-2 Oleate',
    searchKeywords: ['polyglyceryl-2 oleate'],
    usageScope: ['skin'],
    functionalCategory: 'محسن قوام',
    chemicalType: 'مستحلب',
    benefits: { 'مستحلب': 0.8 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'polyglyceryl-10-myristate',
    name: 'بولي جليسريل-10 ميريستات',
    scientific_name: 'Polyglyceryl-10 Myristate',
    searchKeywords: ['polyglyceryl-10 myristate'],
    usageScope: ['skin'],
    functionalCategory: 'محسن قوام',
    chemicalType: 'مستحلب',
    benefits: { 'مستحلب لطيف': 0.9 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'hydrolyzed-psyllium-seed',
    name: 'بذور القطوناء المهدرجة',
    scientific_name: 'Hydrolyzed Psyllium Seed',
    searchKeywords: ['psyllium seed', 'ispaghula'],
    usageScope: ['hair'],
    functionalCategory: 'مكيف للشعر',
    chemicalType: 'بوليمر طبيعي',
    benefits: { 'تحديد التموجات': 0.8, 'ترطيب': 0.85 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'triticum-vulgare-bran-extract',
    name: 'مستخلص نخالة القمح',
    scientific_name: 'Triticum Vulgare (Wheat) Bran Extract',
    searchKeywords: ['wheat bran extract'],
    usageScope: ['skin'],
    functionalCategory: 'مكون فعال',
    chemicalType: 'مستخلص نباتي',
    benefits: { 'مضاد أكسدة': 0.8, 'تفتيح': 0.75 },
    synergy: {},
    negativeSynergy: {},
    warnings: [{ level: 'risk', text: 'تجنب إذا كان لديك حساسية من القمح.' }],
  },
{
    id: 'hydrolyzed-sweet-almond-protein',
    name: 'بروتين اللوز الحلو المهدرج',
    scientific_name: 'Hydrolyzed Sweet Almond Protein',
    searchKeywords: ['hydrolyzed sweet almond protein'],
    usageScope: ['hair'],
    functionalCategory: 'مكيف للشعر',
    chemicalType: 'بروتين',
    benefits: { 'تكييف': 0.85, 'إصلاح': 0.8 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'hydrolyzed-algin-and-zinc-sulfate',
    name: 'ألجين مهدرج وكبريتات الزنك',
    scientific_name: 'Hydrolyzed Algin & Zinc Sulfate',
    searchKeywords: ['hydrolyzed algin and zinc sulfate'],
    usageScope: ['skin'],
    functionalCategory: 'مكون فعال',
    chemicalType: 'مركب نشط',
    benefits: { 'تنظيم الإفرازات الدهنية': 0.88, 'مضاد للالتهابات': 0.85 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'perfluorodecalin',
    name: 'بيرفلوروديكالين',
    scientific_name: 'Perfluorodecalin',
    searchKeywords: ['perfluorodecalin', 'oxygen-carrying'],
    usageScope: ['skin'],
    functionalCategory: 'مكون فعال',
    chemicalType: 'مذيب',
    benefits: { 'توصيل الأكسجين للبشرة': 0.85, 'تنشيط البشرة': 0.8 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'pinus-sylvestris-cone-extract',
    name: 'مستخلص مخروط الصنوبر',
    scientific_name: 'Pinus Sylvestris Cone Extract',
    searchKeywords: ['pine cone extract'],
    usageScope: ['skin'],
    functionalCategory: 'مكون فعال',
    chemicalType: 'مستخلص نباتي',
    benefits: { 'مضاد أكسدة': 0.8 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'avena-strigosa-seed-extract',
    name: 'مستخلص بذور الشوفان الأسود',
    scientific_name: 'Avena Strigosa Seed Extract',
    searchKeywords: ['avena strigosa', 'black oat extract'],
    usageScope: ['skin', 'hair'],
    functionalCategory: 'مرطب / مطري',
    chemicalType: 'مستخلص نباتي',
    benefits: { 'ترطيب عميق': 0.9, 'دعم حاجز البشرة': 0.88 },
    synergy: { 'lecithin': 1.05 },
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'sodium-dilauramidoglutamide-lysine',
    name: 'صوديوم ديلوراميدوجلوتاميد ليسين',
    scientific_name: 'Sodium Dilauramidoglutamide Lysine',
    searchKeywords: ['sodium dilauramidoglutamide lysine', 'pellicer'],
    usageScope: ['hair'],
    functionalCategory: 'مكيف للشعر',
    chemicalType: 'مركب دهني فسفوري',
    benefits: { 'إصلاح الشعر التالف': 0.92, 'ترطيب': 0.88 },
    synergy: {},
    negativeSynergy: {},
    warnings: [{ level: 'info', text: 'يعمل كـ "سيراميد صناعي" للشعر، يخترق ويصلح التلف الداخلي.' }],
  },
{
    id: 'diethyl-sebacate',
    name: 'سيباكات ثنائي الإيثيل',
    scientific_name: 'Diethyl Sebacate',
    searchKeywords: ['diethyl sebacate'],
    usageScope: ['skin'],
    functionalCategory: 'مرطب / مطري',
    chemicalType: 'مطري',
    benefits: { 'مطري خفيف': 0.85, 'مذيب': 0.8 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'diethylene-glycol-monoethyl-ether',
    name: 'إيثر أحادي الإيثيل لثنائي إيثيلين جلايكول',
    scientific_name: 'Diethylene Glycol Monoethyl Ether',
    searchKeywords: ['diethylene glycol monoethyl ether', 'transcutol'],
    usageScope: ['skin'],
    functionalCategory: 'مذيب',
    chemicalType: 'معزز امتصاص',
    benefits: { 'معزز امتصاص قوي': 0.92, 'مذيب': 0.9 },
    synergy: {},
    negativeSynergy: {},
    warnings: [{ level: 'info', text: 'اسم آخر لـ Ethoxydiglycol.' }],
  },
{
    id: 'ppg-2-myristyl-ether-propionate',
    name: 'بروبيونات إيثر الميريستيل PPG-2',
    scientific_name: 'PPG-2 Myristyl Ether Propionate',
    searchKeywords: ['ppg-2 myristyl ether propionate'],
    usageScope: ['skin'],
    functionalCategory: 'مرطب / مطري',
    chemicalType: 'مطري',
    benefits: { 'مطري خفيف وغير كوميدوجينيك': 0.9 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'c12-13-pareth-9',
    name: 'C12-13 باريث-9',
    scientific_name: 'C12-13 Pareth-9',
    searchKeywords: ['c12-13 pareth-9'],
    usageScope: ['hair'],
    functionalCategory: 'مستحلب',
    chemicalType: 'مستحلب',
    benefits: { 'مستحلب': 0.85 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'ammonium-polyacrylatedimethyl-tauramide',
    name: 'أمونيوم بولي أكريلات دايميثيل توراميد',
    scientific_name: 'Ammonium Polyacrylatedimethyl Tauramide',
    searchKeywords: ['ammonium polyacrylatedimethyl tauramide'],
    usageScope: ['skin'],
    functionalCategory: 'محسن قوام',
    chemicalType: 'بوليمر صناعي',
    benefits: { 'مكثف': 0.88 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'sodium-polyacrylate-starch',
    name: 'نشا بولي أكريلات الصوديوم',
    scientific_name: 'Sodium Polyacrylate Starch',
    searchKeywords: ['sodium polyacrylate starch'],
    usageScope: ['skin'],
    functionalCategory: 'محسن قوام',
    chemicalType: 'بوليمر',
    benefits: { 'مكثف': 0.9, 'مثبت': 0.85 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'acrylates-c10-30-alkyl-acrylate-crosspolymer-1',
    name: 'أكريلات/C10-30 ألكيل أكريليت كروسبوليمر-1',
    scientific_name: 'Acrylates/C10-30 Alkyl Acrylate Crosspolymer',
    searchKeywords: ['pemulen tr-1'],
    usageScope: ['skin'],
    functionalCategory: 'محسن قوام',
    chemicalType: 'بوليمر صناعي',
    benefits: { 'مستحلب بوليمري': 0.92, 'يثبت الزيوت': 0.9 },
    synergy: {},
    negativeSynergy: {},
    warnings: [{ level: 'info', text: 'غالبا ما يستخدم كاسم عام، ولكن هناك أنواع مختلفة منه مثل Pemulen™ TR-1 و TR-2.' }],
  },
{
    id: 'ceteth-20',
    name: 'سيتيث-20',
    scientific_name: 'Ceteth-20',
    searchKeywords: ['ceteth-20'],
    usageScope: ['skin'],
    functionalCategory: 'محسن قوام',
    chemicalType: 'مستحلب',
    benefits: { 'مستحلب': 0.9 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'steareth-100',
    name: 'ستيريث-100',
    scientific_name: 'Steareth-100',
    searchKeywords: ['steareth-100', 'ستياريث-100'],
    usageScope: ['hair'],
    functionalCategory: 'محسن قوام',
    chemicalType: 'مكثف',
    benefits: { 'مكثف': 0.88 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'peg-4-dilaurate',
    name: 'ثنائي لوريت PEG-4',
    scientific_name: 'PEG-4 Dilaurate',
    searchKeywords: ['peg-4 dilaurate'],
    usageScope: ['skin'],
    functionalCategory: 'محسن قوام',
    chemicalType: 'مستحلب',
    benefits: { 'مستحلب': 0.8 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'glyceryl-laurate-se',
    name: 'لوريت الجليسريل SE',
    scientific_name: 'Glyceryl Laurate SE',
    searchKeywords: ['glyceryl laurate se'],
    usageScope: ['skin'],
    functionalCategory: 'محسن قوام',
    chemicalType: 'مستحلب',
    benefits: { 'مستحلب ذاتي': 0.85 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'polyglyceryl-4-oleate',
    name: 'بولي جليسريل-4 أوليات',
    scientific_name: 'Polyglyceryl-4 Oleate',
    searchKeywords: ['polyglyceryl-4 oleate'],
    usageScope: ['skin'],
    functionalCategory: 'محسن قوام',
    chemicalType: 'مستحلب',
    benefits: { 'مستحلب': 0.88 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'polyglyceryl-10-isostearate',
    name: 'بولي جليسريل-10 أيزوستيرات',
    scientific_name: 'Polyglyceryl-10 Isostearate',
    searchKeywords: ['polyglyceryl-10 isostearate'],
    usageScope: ['skin'],
    functionalCategory: 'محسن قوام',
    chemicalType: 'مستحلب',
    benefits: { 'مستحلب': 0.85 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'hydroxypropyltrimonium-honey',
    name: 'عسل هيدروكسي بروبيل تريمونيوم',
    scientific_name: 'Hydroxypropyltrimonium Honey',
    searchKeywords: ['hydroxypropyltrimonium honey', 'honeyquat'],
    usageScope: ['hair'],
    functionalCategory: 'مكيف للشعر',
    chemicalType: 'مرطب',
    benefits: { 'ترطيب فائق': 0.92, 'مضاد للكهرباء الساكنة': 0.85 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'hydrolyzed-linseed-extract',
    name: 'مستخلص بذور الكتان المهدرج',
    scientific_name: 'Hydrolyzed Linseed Extract',
    searchKeywords: ['hydrolyzed linseed extract'],
    usageScope: ['skin'],
    functionalCategory: 'مكون فعال',
    chemicalType: 'مستخلص نباتي',
    benefits: { 'تحسين صلابة البشرة': 0.8, 'ترطيب': 0.82 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'hydrolyzed-almond-protein',
    name: 'بروتين اللوز المهدرج',
    scientific_name: 'Hydrolyzed Almond Protein',
    searchKeywords: ['hydrolyzed almond protein'],
    usageScope: ['hair'],
    functionalCategory: 'مكيف للشعر',
    chemicalType: 'بروتين',
    benefits: { 'تكييف': 0.8 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'soy-amino-acids',
    name: 'أحماض الصويا الأمينية',
    scientific_name: 'Soy Amino Acids',
    searchKeywords: ['soy amino acids'],
    usageScope: ['hair'],
    functionalCategory: 'تقوية الشعر',
    chemicalType: 'حمض أميني',
    benefits: { 'تقوية': 0.8, 'ترطيب': 0.82 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'wheat-amino-acids',
    name: 'أحماض القمح الأمينية',
    scientific_name: 'Wheat Amino Acids',
    searchKeywords: ['wheat amino acids'],
    usageScope: ['hair'],
    functionalCategory: 'تقوية الشعر',
    chemicalType: 'حمض أميني',
    benefits: { 'تقوية': 0.85, 'إصلاح': 0.88 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'corn-amino-acids',
    name: 'أحماض الذرة الأمينية',
    scientific_name: 'Corn Amino Acids',
    searchKeywords: ['corn amino acids'],
    usageScope: ['hair'],
    functionalCategory: 'تقوية الشعر',
    chemicalType: 'حمض أميني',
    benefits: { 'تقوية': 0.8 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'sh-polypeptide-9',
    name: 'sh-Polypeptide-9 (VEGF)',
    scientific_name: 'sh-Polypeptide-9',
    searchKeywords: ['sh-polypeptide-9', 'vegf'],
    usageScope: ['hair'],
    functionalCategory: 'مكون فعال',
    chemicalType: 'عامل نمو (ببتيد)',
    benefits: { 'تحفيز نمو الشعر': 0.88 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'sh-polypeptide-1',
    name: 'sh-Polypeptide-1 (bFGF)',
    scientific_name: 'sh-Polypeptide-1',
    searchKeywords: ['sh-polypeptide-1', 'bfgf'],
    usageScope: ['hair'],
    functionalCategory: 'مكون فعال',
    chemicalType: 'عامل نمو (ببتيد)',
    benefits: { 'تحفيز نمو الشعر': 0.85 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'sh-oligopeptide-2',
    name: 'sh-Oligopeptide-2 (IGF-1)',
    scientific_name: 'sh-Oligopeptide-2',
    searchKeywords: ['sh-oligopeptide-2', 'igf-1'],
    usageScope: ['hair'],
    functionalCategory: 'مكون فعال',
    chemicalType: 'عامل نمو (ببتيد)',
    benefits: { 'تقوية بصيلات الشعر': 0.88 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'polyquaternium-51',
    name: 'بولي كواتيرنيوم-51',
    scientific_name: 'Polyquaternium-51',
    searchKeywords: ['polyquaternium-51', 'lipidure'],
    usageScope: ['skin'],
    functionalCategory: 'مرطب / مطري',
    chemicalType: 'مرطب',
    benefits: { 'ترطيب فائق': 0.9, 'دعم حاجز البشرة': 0.88 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'polyquaternium-59',
    name: 'بولي كواتيرنيوم-59',
    scientific_name: 'Polyquaternium-59',
    searchKeywords: ['polyquaternium-59'],
    usageScope: ['hair'],
    functionalCategory: 'مكون فعال',
    chemicalType: 'بوليمر',
    benefits: { 'حماية من أضرار الشمس': 0.85 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'polyquaternium-61',
    name: 'بولي كواتيرنيوم-61',
    scientific_name: 'Polyquaternium-61',
    searchKeywords: ['polyquaternium-61'],
    usageScope: ['skin'],
    functionalCategory: 'مرطب / مطري',
    chemicalType: 'مرطب',
    benefits: { 'ترطيب': 0.85 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'quaternium-18',
    name: 'كواتيرنيوم-18',
    scientific_name: 'Quaternium-18',
    searchKeywords: ['quaternium-18'],
    usageScope: ['hair'],
    functionalCategory: 'مكيف للشعر',
    chemicalType: 'مركب أمونيوم رباعي',
    benefits: { 'مكيف': 0.8 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'quaternium-22',
    name: 'كواتيرنيوم-22',
    scientific_name: 'Quaternium-22',
    searchKeywords: ['quaternium-22'],
    usageScope: ['hair'],
    functionalCategory: 'مكيف للشعر',
    chemicalType: 'مركب أمونيوم رباعي',
    benefits: { 'مكيف': 0.8 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'polyglyceryl-10-caprylate-caprate',
    name: 'بولي جليسريل-10 كابريلات/كبرات',
    scientific_name: 'Polyglyceryl-10 Caprylate/Caprate',
    searchKeywords: ['polyglyceryl-10 caprylate/caprate'],
    usageScope: ['skin'],
    functionalCategory: 'مذوب',
    chemicalType: 'مستحلب',
    benefits: { 'مذوب': 0.85 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'polyglyceryl-6-oleate',
    name: 'بولي جليسريل-6 أوليات',
    scientific_name: 'Polyglyceryl-6 Oleate',
    searchKeywords: ['polyglyceryl-6 oleate'],
    usageScope: ['skin'],
    functionalCategory: 'مستحلب',
    chemicalType: 'مستحلب',
    benefits: { 'مستحلب': 0.8 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'polyglyceryl-10-dipalmitate',
    name: 'بولي جليسريل-10 ثنائي البالميتات',
    scientific_name: 'Polyglyceryl-10 Dipalmitate',
    searchKeywords: ['polyglyceryl-10 dipalmitate'],
    usageScope: ['skin'],
    functionalCategory: 'مستحلب',
    chemicalType: 'مستحلب',
    benefits: { 'مستحلب': 0.8 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'peg-7-glyceryl-laurate',
    name: 'لوريت الجليسريل PEG-7',
    scientific_name: 'PEG-7 Glyceryl Laurate',
    searchKeywords: ['peg-7 glyceryl laurate'],
    usageScope: ['skin'],
    functionalCategory: 'مستحلب',
    chemicalType: 'مستحلب',
    benefits: { 'مستحلب': 0.8 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'peg-20-methyl-glucose-sesquistearate',
    name: 'سيسكوي ستيرات ميثيل جلوكوز PEG-20',
    scientific_name: 'PEG-20 Methyl Glucose Sesquistearate',
    searchKeywords: ['peg-20 methyl glucose sesquistearate'],
    usageScope: ['skin'],
    functionalCategory: 'مستحلب',
    chemicalType: 'مستحلب',
    benefits: { 'مستحلب لطيف': 0.9 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'methyl-glucose-sesquistearate',
    name: 'سيسكوي ستيرات ميثيل جلوكوز',
    scientific_name: 'Methyl Glucose Sesquistearate',
    searchKeywords: ['methyl glucose sesquistearate'],
    usageScope: ['skin'],
    functionalCategory: 'مستحلب',
    chemicalType: 'مستحلب',
    benefits: { 'مستحلب لطيف': 0.88 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'peg-4-polyglyceryl-2-stearate',
    name: 'ستيرات بولي جليسريل-2 PEG-4',
    scientific_name: 'PEG-4 Polyglyceryl-2 Stearate',
    searchKeywords: ['peg-4 polyglyceryl-2 stearate'],
    usageScope: ['skin'],
    functionalCategory: 'مستحلب',
    chemicalType: 'مستحلب',
    benefits: { 'مستحلب': 0.8 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'ceteareth-25',
    name: 'سيتاريث-25',
    scientific_name: 'Ceteareth-25',
    searchKeywords: ['ceteareth-25'],
    usageScope: ['hair'],
    functionalCategory: 'مستحلب',
    chemicalType: 'مستحلب',
    benefits: { 'مستحلب قوي': 0.92, 'مثالي لمنتجات تصفيف الشعر': 0.9 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'ceteareth-60-myristyl-glycol',
    name: 'سيتاريث-60 ميريستيل جلايكول',
    scientific_name: 'Ceteareth-60 Myristyl Glycol',
    searchKeywords: ['ceteareth-60 myristyl glycol'],
    usageScope: ['hair'],
    functionalCategory: 'محسن قوام',
    chemicalType: 'مكثف',
    benefits: { 'مكثف فعال': 0.9 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'isoparaffin-c13-16',
    name: 'إيزوبرافين C13-16',
    scientific_name: 'C13-16 Isoparaffin',
    searchKeywords: ['c13-16 isoparaffin'],
    usageScope: ['skin'],
    functionalCategory: 'مذيب',
    chemicalType: 'مطري',
    benefits: { 'مطري خفيف': 0.85 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'laureth-21',
    name: 'لوريث-21',
    scientific_name: 'Laureth-21',
    searchKeywords: ['laureth-21'],
    usageScope: ['skin'],
    functionalCategory: 'مستحلب',
    chemicalType: 'مستحلب',
    benefits: { 'مستحلب': 0.8 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'peg-40-glyceryl-cocoate',
    name: 'كوكوات الجليسريل PEG-40',
    scientific_name: 'PEG-40 Glyceryl Cocoate',
    searchKeywords: ['peg-40 glyceryl cocoate'],
    usageScope: ['hair'],
    functionalCategory: 'مستحلب',
    chemicalType: 'مستحلب',
    benefits: { 'مستحلب': 0.8 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'sodium-coceth-sulfate',
    name: 'صوديوم كوسيث سلفات',
    scientific_name: 'Sodium Coceth Sulfate',
    searchKeywords: ['sodium coceth sulfate'],
    usageScope: ['hair'],
    functionalCategory: 'منظف',
    chemicalType: 'منظف قوي',
    benefits: { 'تنظيف': 0.88 },
    synergy: {},
    negativeSynergy: {},
    warnings: [{ level: 'caution', text: 'ألطف قليلا من SLES ولكنه لا يزال من السلفات.' }],
  },
{
    id: 'magnesium-laureth-sulfate',
    name: 'كبريتات لوريث المغنيسيوم',
    scientific_name: 'Magnesium Laureth Sulfate',
    searchKeywords: ['magnesium laureth sulfate'],
    usageScope: ['hair'],
    functionalCategory: 'منظف',
    chemicalType: 'منظف قوي',
    benefits: { 'تنظيف لطيف نسبيا': 0.85 },
    synergy: {},
    negativeSynergy: {},
    warnings: [{ level: 'info', text: 'يعتبر ألطف من نظائره الصوديوم والأمونيوم بسبب حجم جزيئاته الأكبر.' }],
  },
{
    id: 'disodium-lauryl-sulfosuccinate',
    name: 'ثنائي الصوديوم لوريل سلفوساكسينات',
    scientific_name: 'Disodium Lauryl Sulfosuccinate',
    searchKeywords: ['disodium lauryl sulfosuccinate'],
    usageScope: ['hair'],
    functionalCategory: 'منظف',
    chemicalType: 'منظف لطيف',
    benefits: { 'تنظيف لطيف': 0.9 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'sodium-lauryl-sarcosinate',
    name: 'صوديوم لوريل ساركوسينات',
    scientific_name: 'Sodium Lauryl Sarcosinate',
    searchKeywords: ['sodium lauryl sarcosinate'],
    usageScope: ['hair'],
    functionalCategory: 'منظف',
    chemicalType: 'منظف لطيف',
    benefits: { 'تنظيف لطيف': 0.88 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'c14-22-alcohols',
    name: 'كحولات C14-22',
    scientific_name: 'C14-22 Alcohols',
    searchKeywords: ['c14-22 alcohols'],
    usageScope: ['skin'],
    functionalCategory: 'محسن قوام',
    chemicalType: 'كحول دهني',
    benefits: { 'مستحلب مساعد': 0.85 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'c12-20-alkyl-glucoside',
    name: 'ألكيل جلوكوزيد C12-20',
    scientific_name: 'C12-20 Alkyl Glucoside',
    searchKeywords: ['c12-20 alkyl glucoside'],
    usageScope: ['skin'],
    functionalCategory: 'محسن قوام',
    chemicalType: 'مستحلب',
    benefits: { 'مستحلب لطيف': 0.9 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'isostearyl-palmitate',
    name: 'بالميتات الأيزوستيريل',
    scientific_name: 'Isostearyl Palmitate',
    searchKeywords: ['isostearyl palmitate'],
    usageScope: ['skin'],
    functionalCategory: 'مرطب / مطري',
    chemicalType: 'مطري',
    benefits: { 'مطري': 0.8 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'myristyl-propionate',
    name: 'بروبيونات الميريستيل',
    scientific_name: 'Myristyl Propionate',
    searchKeywords: ['myristyl propionate'],
    usageScope: ['skin'],
    functionalCategory: 'مرطب / مطري',
    chemicalType: 'مطري',
    benefits: { 'مطري': 0.8 },
    synergy: {},
    negativeSynergy: {},
    warnings: [{ level: 'caution', text: 'قد يكون كوميدوجينيك.' }],
  },
{
    id: 'peg-10-rapeseed-sterol',
    name: 'ستيرول بذور اللفت PEG-10',
    scientific_name: 'PEG-10 Rapeseed Sterol',
    searchKeywords: ['peg-10 rapeseed sterol'],
    usageScope: ['skin'],
    functionalCategory: 'مستحلب',
    chemicalType: 'مستحلب',
    benefits: { 'مستحلب': 0.8 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'potassium-caseinate',
    name: 'كازينات البوتاسيوم',
    scientific_name: 'Potassium Caseinate',
    searchKeywords: ['potassium caseinate'],
    usageScope: ['hair'],
    functionalCategory: 'مكيف للشعر',
    chemicalType: 'بروتين',
    benefits: { 'مكيف': 0.8 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'sesamum-indicum-unsaponifiables',
    name: 'غير قابل للتصبن من زيت السمسم',
    scientific_name: 'Sesamum Indicum (Sesame) Oil Unsaponifiables',
    searchKeywords: ['sesame oil unsaponifiables'],
    usageScope: ['skin'],
    functionalCategory: 'ملطف ومرمم',
    chemicalType: 'مطري',
    benefits: { 'ملطف ومرمم': 0.8 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'soya-sterol',
    name: 'ستيرول الصويا',
    scientific_name: 'Glycine Soja (Soybean) Sterols',
    searchKeywords: ['soya sterol', 'soybean sterols'],
    usageScope: ['skin'],
    functionalCategory: 'دعم حاجز البشرة',
    chemicalType: 'دهون نباتية',
    benefits: { 'دعم حاجز البشرة': 0.85 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'sodium-cetearyl-sulfate',
    name: 'كبريتات سيتريل الصوديوم',
    scientific_name: 'Sodium Cetearyl Sulfate',
    searchKeywords: ['sodium cetearyl sulfate'],
    usageScope: ['hair'],
    functionalCategory: 'منظف',
    chemicalType: 'منظف قوي',
    benefits: { 'تنظيف': 0.85, 'مستحلب': 0.8 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'sodium-stearate',
    name: 'ستيرات الصوديوم',
    scientific_name: 'Sodium Stearate',
    searchKeywords: ['sodium stearate'],
    usageScope: ['skin'],
    functionalCategory: 'منظف',
    chemicalType: 'صابون',
    benefits: { 'منظف': 0.8, 'مكثف': 0.9 },
    synergy: {},
    negativeSynergy: {},
    warnings: [{ level: 'info', text: 'أحد المكونات الرئيسية في الصابون الصلب.' }],
  },
{
    id: 'sodium-palmitate',
    name: 'بالميتات الصوديوم',
    scientific_name: 'Sodium Palmitate',
    searchKeywords: ['sodium palmitate'],
    usageScope: ['skin'],
    functionalCategory: 'منظف',
    chemicalType: 'صابون',
    benefits: { 'منظف': 0.8, 'مكثف': 0.85 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'sodium-cocoate',
    name: 'كوكوات الصوديوم',
    scientific_name: 'Sodium Cocoate',
    searchKeywords: ['sodium cocoate'],
    usageScope: ['skin'],
    functionalCategory: 'منظف',
    chemicalType: 'صابون',
    benefits: { 'منظف': 0.85, 'رغوة': 0.88 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'sodium-tallowate',
    name: 'تالوات الصوديوم',
    scientific_name: 'Sodium Tallowate',
    searchKeywords: ['sodium tallowate'],
    usageScope: ['skin'],
    functionalCategory: 'منظف',
    chemicalType: 'صابون',
    benefits: { 'منظف': 0.8 },
    synergy: {},
    negativeSynergy: {},
    warnings: [{ level: 'info', text: 'صابون مشتق من دهون حيوانية.' }],
  },
{
    id: 'sodium-stearoyl-hydrolyzed-protein',
    name: 'بروتين مهدرج ستيرويل الصوديوم',
    scientific_name: 'Sodium Stearoyl Hydrolyzed Protein',
    searchKeywords: ['sodium stearoyl hydrolyzed protein'],
    usageScope: ['hair'],
    functionalCategory: 'مكيف للشعر',
    chemicalType: 'بروتين معدل',
    benefits: { 'مكيف': 0.8 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'sodium-laneth-40-maleate-styrene-sulfonate-copolymer',
    name: 'صوديوم لانيث-40 ماليات/ستيرين سلفونات كوبوليمر',
    scientific_name: 'Sodium Laneth-40 Maleate/Styrene Sulfonate Copolymer',
    searchKeywords: ['sodium laneth-40 maleate/styrene sulfonate copolymer'],
    usageScope: ['hair'],
    functionalCategory: 'مثبت للشعر',
    chemicalType: 'بوليمر صناعي',
    benefits: { 'تثبيت التصفيف': 0.88 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'steareth-200',
    name: 'ستيريث-200',
    scientific_name: 'Steareth-200',
    searchKeywords: ['steareth-200', 'steareth200', 'steareth 200', 'ستياريث-200'],
    usageScope: ['hair'],
    functionalCategory: 'مستحلب',
    chemicalType: 'مستحلب',
    benefits: { 'مستحلب': 0.8 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'stearyl-citrate',
    name: 'سيترات الستيريل',
    scientific_name: 'Stearyl Citrate',
    searchKeywords: ['stearyl citrate'],
    usageScope: ['skin'],
    functionalCategory: 'مذيب',
    chemicalType: 'مذيب',
    benefits: { 'مذيب': 0.8 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'sucrose-polystearate',
    name: 'بولي ستيرات السكروز',
    scientific_name: 'Sucrose Polystearate',
    searchKeywords: ['sucrose polystearate'],
    usageScope: ['skin'],
    functionalCategory: 'مستحلب',
    chemicalType: 'مستحلب',
    benefits: { 'مستحلب': 0.8 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'tridecyl-salicylate',
    name: 'ساليسيلات الترايديسيل',
    scientific_name: 'Tridecyl Salicylate',
    searchKeywords: ['tridecyl salicylate'],
    usageScope: ['skin'],
    functionalCategory: 'مكون فعال',
    chemicalType: 'مقشر',
    benefits: { 'مقشر لطيف': 0.8 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'laureth-8',
    name: 'لوريث-8',
    scientific_name: 'Laureth-8',
    searchKeywords: ['laureth-8'],
    usageScope: ['hair'],
    functionalCategory: 'مستحلب',
    chemicalType: 'مستحلب',
    benefits: { 'مستحلب': 0.8 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'trilaureth-4-phosphate',
    name: 'فوسفات ثلاثي لوريث-4',
    scientific_name: 'Trilaureth-4 Phosphate',
    searchKeywords: ['trilaureth-4 phosphate'],
    usageScope: ['hair'],
    functionalCategory: 'مستحلب',
    chemicalType: 'مستحلب',
    benefits: { 'مستحلب': 0.9, 'مكيف': 0.8 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'tri-c12-13-alkyl-citrate',
    name: 'سيترات ثلاثي ألكيل C12-13',
    scientific_name: 'Tri-C12-13 Alkyl Citrate',
    searchKeywords: ['tri-c12-13 alkyl citrate'],
    usageScope: ['skin'],
    functionalCategory: 'مرطب / مطري',
    chemicalType: 'مطري',
    benefits: { 'مطري': 0.85 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'tricaprylin',
    name: 'تريكابريلين',
    scientific_name: 'Tricaprylin',
    searchKeywords: ['tricaprylin'],
    usageScope: ['skin'],
    functionalCategory: 'مرطب / مطري',
    chemicalType: 'مطري',
    benefits: { 'مطري': 0.88 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'triheptanoin',
    name: 'تريهيبتانوين',
    scientific_name: 'Triheptanoin',
    searchKeywords: ['triheptanoin'],
    usageScope: ['skin'],
    functionalCategory: 'مرطب / مطري',
    chemicalType: 'مطري',
    benefits: { 'مطري خفيف': 0.9 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'triolein',
    name: 'تريولين',
    scientific_name: 'Triolein',
    searchKeywords: ['triolein'],
    usageScope: ['skin'],
    functionalCategory: 'مرطب / مطري',
    chemicalType: 'مطري',
    benefits: { 'مطري غني': 0.85 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'zea-mays-germ-oil',
    name: 'زيت جنين الذرة',
    scientific_name: 'Zea Mays (Corn) Germ Oil',
    searchKeywords: ['corn oil', 'zea mays oil', 'زيت الذرة'],
    usageScope: ['skin'],
    functionalCategory: 'مرطب / مطري',
    chemicalType: 'زيت ناقل',
    benefits: { 'ترطيب': 0.75 },
    synergy: {},
    negativeSynergy: {},
    warnings: [{ level: 'caution', text: 'قد يكون كوميدوجينيك.' }],
  },
{
    id: 'ethyl-oleate',
    name: 'أوليات الإيثيل',
    scientific_name: 'Ethyl Oleate',
    searchKeywords: ['ethyl oleate'],
    usageScope: ['skin'],
    functionalCategory: 'مرطب / مطري',
    chemicalType: 'مطري',
    benefits: { 'مطري': 0.8 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'isopropyl-isostearate',
    name: 'أيزوستيرات الآيزوبروبيل',
    scientific_name: 'Isopropyl Isostearate',
    searchKeywords: ['isopropyl isostearate'],
    usageScope: ['skin'],
    functionalCategory: 'مرطب / مطري',
    chemicalType: 'مطري',
    benefits: { 'مطري': 0.85 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'lauryl-pca',
    name: 'لوريل PCA',
    scientific_name: 'Lauryl PCA',
    searchKeywords: ['lauryl pca'],
    usageScope: ['hair'],
    functionalCategory: 'مكيف للشعر',
    chemicalType: 'مكيف',
    benefits: { 'مكيف': 0.8 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'myristyl-3-glyceryl-ascorbate',
    name: 'أسكوربات ميريستيل-3-جليسريل',
    scientific_name: 'Myristyl 3-Glyceryl Ascorbate',
    searchKeywords: ['myristyl 3-glyceryl ascorbate', 'vitamin c derivative'],
    usageScope: ['skin'],
    functionalCategory: 'مضاد أكسدة',
    chemicalType: 'مشتق فيتامين',
    benefits: { 'فيتامين سي مستقر': 0.88, 'ترطيب': 0.8 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'disodium-laureth-5-carboxylate',
    name: 'ثنائي الصوديوم لوريث-5 كاربوكسيلات',
    scientific_name: 'Disodium Laureth-5 Carboxylate',
    searchKeywords: ['disodium laureth-5 carboxylate'],
    usageScope: ['hair'],
    functionalCategory: 'منظف',
    chemicalType: 'منظف لطيف',
    benefits: { 'تنظيف لطيف': 0.85 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'sodium-c14-18-olefin-sulfonate',
    name: 'صوديوم C14-18 أوليفين سلفونات',
    scientific_name: 'Sodium C14-18 Olefin Sulfonate',
    searchKeywords: ['sodium c14-18 olefin sulfonate'],
    usageScope: ['hair'],
    functionalCategory: 'منظف',
    chemicalType: 'منظف قوي',
    benefits: { 'تنظيف عميق': 0.88 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'sodium-methyl-stearoyl-taurate',
    name: 'صوديوم ميثيل ستيرويل توريت',
    scientific_name: 'Sodium Methyl Stearoyl Taurate',
    searchKeywords: ['sodium methyl stearoyl taurate'],
    usageScope: ['skin'],
    functionalCategory: 'مستحلب',
    chemicalType: 'مستحلب',
    benefits: { 'مستحلب لطيف': 0.9 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'sodium-stearoyl-glutamate-1',
    name: 'صوديوم ستيرويل جلوتامات',
    scientific_name: 'Sodium Stearoyl Glutamate',
    searchKeywords: ['sodium stearoyl glutamate', 'eumulgin sg'],
    usageScope: ['skin'],
    functionalCategory: 'محسن قوام',
    chemicalType: 'مستحلب',
    benefits: { 'مستحلب لطيف مشتق من الأحماض الأمينية': 0.92, 'لطيف جدا على البشرة': 0.9 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'steareth-10-allyl-ether-acrylates-copolymer-1',
    name: 'ستيريث-10 أليل إيثر/أكريلات كوبوليمر',
    scientific_name: 'Steareth-10 Allyl Ether/Acrylates Copolymer',
    searchKeywords: ['steareth-10 allyl ether/acrylates copolymer', 'salcare sc90'],
    usageScope: ['hair'],
    functionalCategory: 'مثبت للشعر',
    chemicalType: 'بوليمر صناعي',
    benefits: { 'تثبيت التصفيف': 0.85 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'polysilicone-13',
    name: 'بولي سيليكون-13',
    scientific_name: 'Polysilicone-13',
    searchKeywords: ['polysilicone-13'],
    usageScope: ['hair'],
    functionalCategory: 'مكيف للشعر',
    chemicalType: 'سيليكون',
    benefits: { 'مكيف': 0.8 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'peg-4-laurate',
    name: 'لوريت PEG-4',
    scientific_name: 'PEG-4 Laurate',
    searchKeywords: ['peg-4 laurate'],
    usageScope: ['skin'],
    functionalCategory: 'مستحلب',
    chemicalType: 'مستحلب',
    benefits: { 'مستحلب': 0.8 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'peg-20-stearate',
    name: 'ستيرات PEG-20',
    scientific_name: 'PEG-20 Stearate',
    searchKeywords: ['peg-20 stearate'],
    usageScope: ['skin'],
    functionalCategory: 'مستحلب',
    chemicalType: 'مستحلب',
    benefits: { 'مستحلب': 0.85 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'peg-30-stearate',
    name: 'ستيرات PEG-30',
    scientific_name: 'PEG-30 Stearate',
    searchKeywords: ['peg-30 stearate'],
    usageScope: ['skin'],
    functionalCategory: 'مستحلب',
    chemicalType: 'مستحلب',
    benefits: { 'مستحلب': 0.8 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'peg-40-laurate',
    name: 'لوريت PEG-40',
    scientific_name: 'PEG-40 Laurate',
    searchKeywords: ['peg-40 laurate'],
    usageScope: ['skin'],
    functionalCategory: 'مستحلب',
    chemicalType: 'مستحلب',
    benefits: { 'مستحلب': 0.8 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'peg-70-mango-glycerides',
    name: 'جليسريدات المانجو PEG-70',
    scientific_name: 'PEG-70 Mango Glycerides',
    searchKeywords: ['peg-70 mango glycerides'],
    usageScope: ['skin'],
    functionalCategory: 'مستحلب',
    chemicalType: 'مستحلب',
    benefits: { 'مستحلب': 0.8 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'peg-120-jojoba-acid',
    name: 'حمض الجوجوبا PEG-120',
    scientific_name: 'PEG-120 Jojoba Acid',
    searchKeywords: ['peg-120 jojoba acid'],
    usageScope: ['skin'],
    functionalCategory: 'مستحلب',
    chemicalType: 'مستحلب',
    benefits: { 'مستحلب': 0.8 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'polyglyceryl-4-laurate',
    name: 'بولي جليسريل-4 لوريت',
    scientific_name: 'Polyglyceryl-4 Laurate',
    searchKeywords: ['polyglyceryl-4 laurate'],
    usageScope: ['skin'],
    functionalCategory: 'مستحلب',
    chemicalType: 'مستحلب',
    benefits: { 'مستحلب': 0.85 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'polyglyceryl-5-oleate',
    name: 'بولي جليسريل-5 أوليات',
    scientific_name: 'Polyglyceryl-5 Oleate',
    searchKeywords: ['polyglyceryl-5 oleate'],
    usageScope: ['skin'],
    functionalCategory: 'مستحلب',
    chemicalType: 'مستحلب',
    benefits: { 'مستحلب': 0.8 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'polyglyceryl-6-isostearate',
    name: 'بولي جليسريل-6 أيزوستيرات',
    scientific_name: 'Polyglyceryl-6 Isostearate',
    searchKeywords: ['polyglyceryl-6 isostearate'],
    usageScope: ['skin'],
    functionalCategory: 'مستحلب',
    chemicalType: 'مستحلب',
    benefits: { 'مستحلب': 0.8 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'polyglyceryl-10-behenate-eicosadioate',
    name: 'بولي جليسريل-10 بيهينات/إيكوساديويت',
    scientific_name: 'Polyglyceryl-10 Behenate/Eicosadioate',
    searchKeywords: ['polyglyceryl-10 behenate/eicosadioate'],
    usageScope: ['skin'],
    functionalCategory: 'مستحلب',
    chemicalType: 'مستحلب',
    benefits: { 'مستحلب': 0.85 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'polyglyceryl-10-pentastearate',
    name: 'بولي جليسريل-10 خماسي الستيارات',
    scientific_name: 'Polyglyceryl-10 Pentastearate',
    searchKeywords: ['polyglyceryl-10 pentastearate'],
    usageScope: ['skin'],
    functionalCategory: 'مستحلب',
    chemicalType: 'مستحلب',
    benefits: { 'مستحلب': 0.9 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'quaternium-90-bentonite',
    name: 'كواتيرنيوم-90 بنتونيت',
    scientific_name: 'Quaternium-90 Bentonite',
    searchKeywords: ['quaternium-90 bentonite'],
    usageScope: ['skin'],
    functionalCategory: 'محسن قوام',
    chemicalType: 'مثبت',
    benefits: { 'مثبت': 0.88 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'steareth-20-methacrylate-copolymer',
    name: 'ستيريث-20 ميثاكريليت كوبوليمر',
    scientific_name: 'Steareth-20 Methacrylate Copolymer',
    searchKeywords: ['steareth-20 methacrylate copolymer'],
    usageScope: ['hair'],
    functionalCategory: 'مثبت للشعر',
    chemicalType: 'بوليمر صناعي',
    benefits: { 'تثبيت': 0.8 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'steartrimonium-bromide',
    name: 'بروميد ستيرتريمونيوم',
    scientific_name: 'Steartrimonium Bromide',
    searchKeywords: ['steartrimonium bromide'],
    usageScope: ['hair'],
    functionalCategory: 'مكيف للشعر',
    chemicalType: 'مركب أمونيوم رباعي',
    benefits: { 'مكيف': 0.8 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'stearyl-behenate',
    name: 'بيهينات الستيريل',
    scientific_name: 'Stearyl Behenate',
    searchKeywords: ['stearyl behenate'],
    usageScope: ['skin'],
    functionalCategory: 'محسن قوام',
    chemicalType: 'مطري شمعي',
    benefits: { 'مكثف': 0.8 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'sunflower-seed-oil-glyceride',
    name: 'جليسريد زيت بذور دوار الشمس',
    scientific_name: 'Sunflower Seed Oil Glyceride',
    searchKeywords: ['sunflower seed oil glyceride'],
    usageScope: ['skin'],
    functionalCategory: 'مرطب / مطري',
    chemicalType: 'مطري',
    benefits: { 'مطري': 0.8 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'tall-oil-acid',
    name: 'حمض زيت طويل',
    scientific_name: 'Tall Oil Acid',
    searchKeywords: ['tall oil acid'],
    usageScope: ['skin'],
    functionalCategory: 'منظف',
    chemicalType: 'حمض دهني',
    benefits: { 'منظف': 0.7 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'tetramethyl-acetyloctahydronaphthalenes',
    name: 'رباعي ميثيل أسيتيل أوكتاهيدرونافثالين',
    scientific_name: 'Tetramethyl Acetyloctahydronaphthalenes',
    searchKeywords: ['iso e super', 'otne'],
    usageScope: ['skin'],
    functionalCategory: 'عطر',
    chemicalType: 'مكون عطري',
    benefits: { 'رائحة خشبية-عنبرية': 0.95 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'trimethyl-pentanyl-diisobutyrate',
    name: 'ثنائي أيزوبيوتيرات ثلاثي ميثيل البنتانيل',
    scientific_name: 'Trimethyl Pentanyl Diisobutyrate',
    searchKeywords: ['trimethyl pentanyl diisobutyrate'],
    usageScope: ['skin'],
    functionalCategory: 'مكون طبقة رقيقة',
    chemicalType: 'مكون طبقة رقيقة',
    benefits: { 'تحسين متانة طلاء الأظافر': 0.8 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'tris-bht-mesitylene',
    name: 'تريس(BHT)ميسيتيلين',
    scientific_name: 'Tris(BHT)Mesitylene',
    searchKeywords: ['tris(bht)mesitylene'],
    usageScope: ['skin'],
    functionalCategory: 'مضاد أكسدة',
    chemicalType: 'مضاد أكسدة',
    benefits: { 'مضاد أكسدة': 0.8 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'undeceth-11',
    name: 'أونديسيث-11',
    scientific_name: 'Undeceth-11',
    searchKeywords: ['undeceth-11'],
    usageScope: ['hair'],
    functionalCategory: 'مستحلب',
    chemicalType: 'مستحلب',
    benefits: { 'مستحلب': 0.8 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'undeceth-5',
    name: 'أونديسيث-5',
    scientific_name: 'Undeceth-5',
    searchKeywords: ['undeceth-5'],
    usageScope: ['hair'],
    functionalCategory: 'مستحلب',
    chemicalType: 'مستحلب',
    benefits: { 'مستحلب': 0.8 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'laureth-11',
    name: 'لوريث-11',
    scientific_name: 'Laureth-11',
    searchKeywords: ['laureth-11'],
    usageScope: ['hair'],
    functionalCategory: 'مستحلب',
    chemicalType: 'مستحلب',
    benefits: { 'مستحلب': 0.8 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'peg-2-oleamine',
    name: 'PEG-2 أوليامين',
    scientific_name: 'PEG-2 Oleamine',
    searchKeywords: ['peg-2 oleamine'],
    usageScope: ['hair'],
    functionalCategory: 'مستحلب',
    chemicalType: 'مستحلب',
    benefits: { 'مستحلب': 0.8 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'peg-9-dimethicone',
    name: 'PEG-9 دايميثيكون',
    scientific_name: 'PEG-9 Dimethicone',
    searchKeywords: ['peg-9 dimethicone'],
    usageScope: ['skin'],
    functionalCategory: 'مستحلب',
    chemicalType: 'سيليكون قابل للذوبان في الماء',
    benefits: { 'مستحلب': 0.8 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'peg-10-dimethicone',
    name: 'PEG-10 دايميثيكون',
    scientific_name: 'PEG-10 Dimethicone',
    searchKeywords: ['peg-10 dimethicone'],
    usageScope: ['skin'],
    functionalCategory: 'مستحلب',
    chemicalType: 'سيليكون قابل للذوبان في الماء',
    benefits: { 'مستحلب ماء في سيليكون': 0.9 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'potassium-oleate',
    name: 'أوليات البوتاسيوم',
    scientific_name: 'Potassium Oleate',
    searchKeywords: ['potassium oleate'],
    usageScope: ['skin'],
    functionalCategory: 'منظف',
    chemicalType: 'صابون',
    benefits: { 'منظف': 0.8 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'potassium-stearate',
    name: 'ستيرات البوتاسيوم',
    scientific_name: 'Potassium Stearate',
    searchKeywords: ['potassium stearate'],
    usageScope: ['skin'],
    functionalCategory: 'منظف',
    chemicalType: 'صابون',
    benefits: { 'منظف': 0.8, 'مستحلب': 0.85 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'sodium-cetyl-sulfate',
    name: 'كبريتات سيتيل الصوديوم',
    scientific_name: 'Sodium Cetyl Sulfate',
    searchKeywords: ['sodium cetyl sulfate'],
    usageScope: ['hair'],
    functionalCategory: 'منظف',
    chemicalType: 'منظف قوي',
    benefits: { 'تنظيف': 0.85 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'magnesium-nitrate',
    name: 'نترات المغنيسيوم',
    scientific_name: 'Magnesium Nitrate',
    searchKeywords: ['magnesium nitrate'],
    usageScope: ['hair'],
    functionalCategory: 'مادة حافظة',
    chemicalType: 'مادة حافظة',
    benefits: { 'مادة حافظة مساعدة': 0.8 },
    synergy: {},
    negativeSynergy: {},
    warnings: [{ level: 'info', text: 'غالبا ما يوجد مع MCI/MIT كمثبت.' }],
  },
{
    id: 'magnesium-chloride',
    name: 'كلوريد المغنيسيوم',
    scientific_name: 'Magnesium Chloride',
    searchKeywords: ['magnesium chloride'],
    usageScope: ['skin'],
    functionalCategory: 'محسن قوام',
    chemicalType: 'ملح',
    benefits: { 'مكثف': 0.75 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'polyglyceryl-4-diisostearate-polyhydroxystearate-sebacate',
    name: 'بولي جليسريل-4 ثنائي أيزوستيرات/بولي هيدروكسي ستيرات/سيباكات',
    scientific_name: 'Polyglyceryl-4 Diisostearate/Polyhydroxystearate/Sebacate',
    searchKeywords: ['polyglyceryl-4 diisostearate/polyhydroxystearate/sebacate'],
    usageScope: ['skin'],
    functionalCategory: 'مستحلب',
    chemicalType: 'مستحلب',
    benefits: { 'مستحلب ماء في زيت': 0.92 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'lauryl-peg-9-polydimethylsiloxyethyl-dimethicone',
    name: 'لوريل PEG-9 بولي داي ميثيل سيلوكسي إيثيل دايميثيكون',
    scientific_name: 'Lauryl PEG-9 Polydimethylsiloxyethyl Dimethicone',
    searchKeywords: ['lauryl peg-9 polydimethylsiloxyethyl dimethicone'],
    usageScope: ['skin'],
    functionalCategory: 'مستحلب',
    chemicalType: 'سيليكون',
    benefits: { 'مستحلب ماء في سيليكون': 0.9 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'sorbitan-sesquiisostearate',
    name: 'سيسكوي أيزوستيرات السوربيتان',
    scientific_name: 'Sorbitan Sesquiisostearate',
    searchKeywords: ['sorbitan sesquiisostearate'],
    usageScope: ['skin'],
    functionalCategory: 'مستحلب',
    chemicalType: 'مستحلب',
    benefits: { 'مستحلب': 0.8 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'peg-ppg-19-19-dimethicone',
    name: 'PEG/PPG-19/19 دايميثيكون',
    scientific_name: 'PEG/PPG-19/19 Dimethicone',
    searchKeywords: ['peg/ppg-19/19 dimethicone'],
    usageScope: ['skin'],
    functionalCategory: 'مستحلب',
    chemicalType: 'سيليكون',
    benefits: { 'مستحلب': 0.85 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'polyglyceryl-3-polydimethylsiloxyethyl-dimethicone',
    name: 'بولي جليسريل-3 بولي داي ميثيل سيلوكسي إيثيل دايميثيكون',
    scientific_name: 'Polyglyceryl-3 Polydimethylsiloxyethyl Dimethicone',
    searchKeywords: ['polyglyceryl-3 polydimethylsiloxyethyl dimethicone'],
    usageScope: ['skin'],
    functionalCategory: 'مستحلب',
    chemicalType: 'سيليكون',
    benefits: { 'مستحلب': 0.88 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'hydrogenated-vegetable-glycerides',
    name: 'جليسريدات نباتية مهدرجة',
    scientific_name: 'Hydrogenated Vegetable Glycerides',
    searchKeywords: ['hydrogenated vegetable glycerides'],
    usageScope: ['skin'],
    functionalCategory: 'مرطب / مطري',
    chemicalType: 'مطري',
    benefits: { 'مطري': 0.8 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'oryzanol',
    name: 'أوريزانول',
    scientific_name: 'Oryzanol',
    searchKeywords: ['oryzanol', 'gamma oryzanol'],
    usageScope: ['skin'],
    functionalCategory: 'مضاد أكسدة',
    chemicalType: 'مركب نباتي',
    benefits: { 'مضاد أكسدة': 0.9, 'حماية من أضرار الشمس': 0.8 },
    synergy: {},
    negativeSynergy: {},
    warnings: [{ level: 'info', text: 'المكون النشط في زيت نخالة الأرز.' }],
  },
{
    id: 'oleth-3',
    name: 'أوليث-3',
    scientific_name: 'Oleth-3',
    searchKeywords: ['oleth-3'],
    usageScope: ['skin'],
    functionalCategory: 'مستحلب',
    chemicalType: 'مستحلب',
    benefits: { 'مستحلب': 0.8 },
    synergy: {},
    negativeSynergy: {},
    warnings: [{ level: 'caution', text: 'قد يكون كوميدوجينيك.' }],
  },
{
    id: 'oleth-5',
    name: 'أوليث-5',
    scientific_name: 'Oleth-5',
    searchKeywords: ['oleth-5'],
    usageScope: ['skin'],
    functionalCategory: 'مستحلب',
    chemicalType: 'مستحلب',
    benefits: { 'مستحلب': 0.8 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'oleth-15',
    name: 'أوليث-15',
    scientific_name: 'Oleth-15',
    searchKeywords: ['oleth-15'],
    usageScope: ['skin'],
    functionalCategory: 'مستحلب',
    chemicalType: 'مستحلب',
    benefits: { 'مستحلب': 0.8 },
    synergy: {},
    negativeSynergy: {},
    warnings: [],
  },
{
    id: 'caprylic-capric-triglyceride',
    name: 'ثلاثي جليسريد الكابريليك/الكابريك',
    scientific_name: 'Caprylic/Capric Triglyceride',
    searchKeywords: ['caprylic/capric triglyceride', 'fractionated coconut oil', 'mct oil', 'زيت جوز الهند المجزأ', 'كبريليك/كبريك ثلاثي جليسريد', 'كبريليك/كبريك ثلاثي غليسيد', 'زيت MCT', 'كبريليك/كبريك ثلاثي جليسيد'],
    usageScope: ['skin', 'hair'],
    functionalCategory: 'مرطب / مطري',
    chemicalType: 'مطري',
    benefits: { 'مطري خفيف جدا': 0.95, 'غير كوميدوجينيك': 0.9, 'تحسين انتشار المنتج': 0.92 },
    synergy: {},
    negativeSynergy: {},
    warnings: [{ level: 'info', text: 'زيت خفيف جدا مشتق من جوز الهند، لا يسد المسام ويعطي ملمسا حريريا.' }],
  },
{
        id: 'batana-oil',
        name: 'زيت الباتانا',
        scientific_name: 'Elaeis Oleifera Kernel Oil',
        searchKeywords: ['batana oil', 'elaeis oleifera', 'american palm oil', 'زيت الباتانا'],
        usageScope: ['hair'],
        functionalCategory: 'مرطب / مطري',
        chemicalType: 'زيت ناقل',
        benefits: { 'تقوية الشعر': 0.88, 'تغذية عميقة': 0.9, 'إعادة الحيوية للشعر التالف': 0.85, 'إضافة لمعان': 0.82 },
        synergy: { 'castor': 1.05 },
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'زيت تقليدي من أمريكا الوسطى، معروف بخصائصه المرممة للشعر.' }],
      },
{
        id: 'soybean-oil',
        name: 'زيت فول الصويا',
        scientific_name: 'Glycine Soja (Soybean) Oil',
        searchKeywords: ['soybean oil', 'glycine soja oil', 'زيت الصويا', 'زيت الصوجا', 'سوجا'],
        usageScope: ['skin', 'hair'],
        functionalCategory: 'مرطب / مطري',
        chemicalType: 'زيت ناقل',
        benefits: { 'ترطيب': 0.8, 'مضاد أكسدة': 0.75, 'دعم حاجز البشرة': 0.82 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'risk', text: 'يجب تجنبه من قبل الأشخاص الذين يعانون من حساسية الصويا.' }],
      },
{
        id: 'onion-seed-oil',
        name: 'زيت بذور البصل',
        scientific_name: 'Allium Cepa Seed Oil',
        searchKeywords: ['onion seed oil', 'allium cepa', 'زيت بذور البصل', 'زيت البصل'],
        usageScope: ['hair'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'زيت ناقل',
        benefits: { 'تحفيز نمو الشعر': 0.88, 'مضاد للميكروبات': 0.8, 'تقوية بصيلات الشعر': 0.85 },
        synergy: { 'black-seed': 1.1, 'rosemary-essential': 1.05 },
        negativeSynergy: {},
        warnings: [{ level: 'caution', text: 'له رائحة بصل قوية قد تبقى في الشعر.' }],
      },
{
        id: 'sweet-orange-flower-oil',
        name: 'زيت زهر البرتقال الحلو',
        scientific_name: 'Citrus Aurantium Dulcis (Orange) Flower Oil',
        searchKeywords: ['sweet orange flower oil', 'citrus aurantium dulcis'],
        usageScope: ['skin'],
        functionalCategory: 'عطر',
        chemicalType: 'زيت عطري',
        benefits: { 'رائحة منعشة': 0.9, 'مهدئ': 0.8 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'sodium-thiosulfate',
        name: 'ثيوكبريتات الصوديوم',
        scientific_name: 'Sodium Thiosulfate',
        searchKeywords: ['sodium thiosulfate'],
        usageScope: ['skin'],
        functionalCategory: 'مضاد أكسدة',
        chemicalType: 'مثبت',
        benefits: { 'مضاد أكسدة': 0.8, 'يحافظ على استقرار المنتج': 0.85 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'palm-kernelamide-dea',
        name: 'بالم كيرنل أميد DEA',
        scientific_name: 'Palm Kernelamide DEA',
        searchKeywords: ['palm kernelamide dea'],
        usageScope: ['hair'],
        functionalCategory: 'محسن قوام',
        chemicalType: 'معزز للرغوة',
        benefits: { 'زيادة كثافة الرغوة': 0.85, 'مكثف للقوام': 0.88 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'risk', text: 'مثل Cocamide DEA، قد يكون هناك مخاوف بشأن تكوين النتروزامينات.' }],
      },
{
        id: 'bergamot-essential-oil',
        name: 'زيت البرغموت العطري',
        scientific_name: 'Citrus Aurantium Bergamia (Bergamot) Fruit Oil',
        searchKeywords: ['bergamot oil', 'citrus bergamia', 'برغموت', 'زيت البرغموت'],
        usageScope: ['skin'],
        functionalCategory: 'عطر',
        chemicalType: 'زيت عطري',
        benefits: { 'رائحة منعشة': 0.95, 'تنظيم الإفرازات الدهنية': 0.8 },
        synergy: {},
        negativeSynergy: {},
        warnings: [
          { level: 'risk', text: 'يسبب حساسية شديدة للضوء (Phototoxic) ما لم يكن "FCF" (خالٍ من الفورانوكومارين). يجب تجنب التعرض للشمس بعد استخدامه.' }
        ],
      },
{
        id: 'jasmine-absolute',
        name: 'زيت الياسمين (مطلق)',
        scientific_name: 'Jasminum Officinale (Jasmine) Oil',
        searchKeywords: ['jasmine absolute', 'jasminum officinale', 'ياسمين', 'زيت الياسمين'],
        usageScope: ['skin'],
        functionalCategory: 'عطر',
        chemicalType: 'زيت عطري (مطلق)',
        benefits: { 'رائحة زهرية فاخرة': 0.98, 'مهدئ': 0.8 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'patchouli-essential-oil',
        name: 'زيت الباتشولي العطري',
        scientific_name: 'Pogostemon Cablin Oil',
        searchKeywords: ['patchouli oil', 'pogostemon cablin', 'باتشولي', 'زيت الباتشولي'],
        usageScope: ['skin'],
        functionalCategory: 'عطر',
        chemicalType: 'زيت عطري',
        benefits: { 'رائحة ترابية': 0.9, 'مضاد للالتهابات': 0.8, 'ملطف ومرمم': 0.82 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'camphor-essential-oil',
        name: 'زيت الكافور العطري',
        scientific_name: 'Cinnamomum Camphora (Camphor) Bark Oil',
        searchKeywords: ['camphor oil', 'cinnamomum camphora', 'كافور', 'زيت الكافور'],
        usageScope: ['skin'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'زيت عطري',
        benefits: { 'تخفيف الألم الموضعي': 0.88, 'مضاد للميكروبات': 0.8 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'risk', text: 'قوي جدا وقد يكون ساما بتركيزات عالية. يستخدم بحذر شديد.' }],
      },
{
        id: 'grapefruit-essential-oil',
        name: 'زيت الجريب فروت العطري',
        scientific_name: 'Citrus Paradisi (Grapefruit) Peel Oil',
        searchKeywords: ['grapefruit oil', 'citrus paradisi', 'جريب فروت', 'زيت الجريب فروت'],
        usageScope: ['skin'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'زيت عطري',
        benefits: { 'تنقية البشرة': 0.85, 'رائحة منعشة': 0.9 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'caution', text: 'قد يسبب حساسية للضوء.' }],
      },
{
        id: 'tomato-fruit-extract',
        name: 'مستخلص فاكهة الطماطم',
        scientific_name: 'Solanum Lycopersicum (Tomato) Fruit Extract',
        searchKeywords: ['tomato extract', 'solanum lycopersicum'],
        usageScope: ['skin'],
        functionalCategory: 'مضاد أكسدة',
        chemicalType: 'مستخلص نباتي',
        benefits: { 'غني بالليكوبين': 0.9, 'مضاد أكسدة': 0.92, 'حماية من أضرار الشمس': 0.75 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'carrot-root-extract',
        name: 'مستخلص جذور الجزر',
        scientific_name: 'Daucus Carota Sativa (Carrot) Root Extract',
        searchKeywords: ['carrot extract', 'daucus carota sativa', 'مستخلص الجزر'],
        usageScope: ['skin'],
        functionalCategory: 'مضاد أكسدة',
        chemicalType: 'مستخلص نباتي',
        benefits: { 'غني بالبيتا كاروتين': 0.9, 'مضاد أكسدة': 0.88, 'تجديد البشرة': 0.8 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'carrot-seed-oil',
        name: 'زيت بذور الجزر',
        scientific_name: 'Daucus Carota Sativa (Carrot) Seed Oil',
        searchKeywords: ['carrot seed oil', 'daucus carota sativa seed oil', 'زيت بذور الجزر'],
        usageScope: ['skin'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'زيت عطري',
        benefits: { 'تجديد خلايا البشرة': 0.9, 'توحيد لون البشرة': 0.85, 'مكافحة الشيخوخة': 0.88 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'oat-kernel-extract',
        name: 'مستخلص نواة الشوفان',
        scientific_name: 'Avena Sativa (Oat) Kernel Extract',
        searchKeywords: ['oat extract', 'avena sativa kernel extract', 'مستخلص الشوفان'],
        usageScope: ['skin'],
        functionalCategory: 'ملطف ومرمم',
        chemicalType: 'مستخلص نباتي',
        benefits: { 'ملطف ومرمم فائق': 0.95, 'مضاد للحكة': 0.92, 'مضاد للالتهابات': 0.9 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'steareth-4',
        name: 'ستيريث-4',
        scientific_name: 'Steareth-4',
        searchKeywords: ['steareth-4', 'ستياريث-4'],
        usageScope: ['skin', 'hair'],
        functionalCategory: 'محسن قوام',
        chemicalType: 'مستحلب',
        benefits: { 'مستحلب': 0.85 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'decylene-glycol',
        name: 'ديسيلين جلايكول',
        scientific_name: 'Decylene Glycol',
        searchKeywords: ['decylene glycol'],
        usageScope: ['skin'],
        functionalCategory: 'مرطب / مطري',
        chemicalType: 'مرطب',
        benefits: { 'ترطيب': 0.8, 'مضاد للميكروبات': 0.85, 'يعزز امتصاص المكونات': 0.82 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'rosemary-leaf-water',
        name: 'ماء إكليل الجبل',
        scientific_name: 'Rosmarinus Officinalis (Rosemary) Leaf Water',
        searchKeywords: ['rosemary water', 'rosmarinus officinalis water', 'ماء إكليل الجبل'],
        usageScope: ['hair'],
        functionalCategory: 'ملطف ومرمم',
        chemicalType: 'مستخلص مائي',
        benefits: { 'صحة فروة الرأس': 0.9, 'تقليل القشرة': 0.8, 'تحفيز نمو الشعر': 0.85 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'peppermint-leaf-water',
        name: 'ماء النعناع الفلفلي',
        scientific_name: 'Mentha Piperita (Peppermint) Leaf Water',
        searchKeywords: ['peppermint water', 'mentha piperita water', 'ماء النعناع'],
        usageScope: ['hair', 'skin'],
        functionalCategory: 'ملطف ومرمم',
        chemicalType: 'مستخلص مائي',
        benefits: { 'تنشيط فروة الرأس': 0.9, 'إحساس بالبرودة': 0.88, 'تنقية البشرة': 0.8 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'dioxybenzone',
        name: 'ديوكسي بنزون',
        scientific_name: 'Benzophenone-8',
        searchKeywords: ['dioxybenzone', 'benzophenone-8'],
        usageScope: ['skin'],
        functionalCategory: 'حماية من الشمس',
        chemicalType: 'فلتر كيميائي',
        benefits: { 'حماية من UVB و UVA2': 0.8 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'caution', text: 'فلتر شمسي قديم، لم يعد شائع الاستخدام.' }],
      },
{
        id: 'menthyl-anthranilate',
        name: 'أنثرانيلات المنثيل',
        scientific_name: 'Menthyl Anthranilate',
        searchKeywords: ['menthyl anthranilate'],
        usageScope: ['skin'],
        functionalCategory: 'حماية من الشمس',
        chemicalType: 'فلتر كيميائي (UVA)',
        benefits: { 'حماية من UVA2': 0.75 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'caution', text: 'فلتر UVA ضعيف، لم يعد شائع الاستخدام في العديد من المناطق.' }],
      },
{
        id: 'sulisobenzone',
        name: 'سوليسوبنزون',
        scientific_name: 'Benzophenone-4',
        searchKeywords: ['sulisobenzone', 'benzophenone-4'],
        usageScope: ['skin'],
        functionalCategory: 'حماية من الشمس',
        chemicalType: 'فلتر كيميائي',
        benefits: { 'حماية من UVB و UVA2': 0.85, 'مثبت للتركيبة': 0.8 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'قابل للذوبان في الماء، يستخدم لحماية المنتج نفسه من التلف الضوئي وكمساعد فلتر شمسي.' }],
      },
{
        id: 'trolamine-salicylate',
        name: 'ساليسيلات الترولامين',
        scientific_name: 'Trolamine Salicylate',
        searchKeywords: ['trolamine salicylate', 'tea-salicylate'],
        usageScope: ['skin'],
        functionalCategory: 'حماية من الشمس',
        chemicalType: 'فلتر كيميائي (UVB)',
        benefits: { 'حماية من UVB': 0.7, 'تخفيف الألم الموضعي': 0.8 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'caution', text: 'فلتر UVB ضعيف، يستخدم بشكل أساسي في منتجات تخفيف آلام العضلات.' }],
      },
{
        id: 'indigo-powder',
        name: 'مسحوق النيلة (إنديجو)',
        scientific_name: 'Indigofera Tinctoria Leaf Powder',
        searchKeywords: ['indigo powder', 'indigofera tinctoria', 'النيلة الزرقاء', 'مسحوق النيلة'],
        usageScope: ['hair', 'skin'],
        functionalCategory: 'محسن جمالي',
        chemicalType: 'ملون طبيعي',
        benefits: { 'صبغ الشعر باللون الأزرق/الأسود': 0.95, 'تفتيح البشرة (تقليدي)': 0.7 },
        synergy: { 'henna-oil': { reason: 'يستخدم مع الحناء لصبغ الشعر باللون البني أو الأسود.' } },
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'يستخدم تقليديا في المغرب لتفتيح البشرة وكصبغة طبيعية للشعر.' }],
      },
{
        id: 'damask-rose-absolute',
        name: 'زيت الورد الدمشقي (مطلق)',
        scientific_name: 'Rosa Damascena Flower Extract',
        searchKeywords: ['rose absolute', 'rosa damascena absolute'],
        usageScope: ['skin'],
        functionalCategory: 'عطر',
        chemicalType: 'زيت عطري (مطلق)',
        benefits: { 'رائحة ورد غنية وعميقة': 0.98, 'مكافحة الشيخوخة': 0.85 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'frankincense-water',
        name: 'ماء اللبان (هيدروسول)',
        scientific_name: 'Boswellia Carterii Water',
        searchKeywords: ['frankincense hydrosol', 'frankincense water', 'ماء اللبان'],
        usageScope: ['skin'],
        functionalCategory: 'ملطف ومرمم',
        chemicalType: 'مستخلص مائي',
        benefits: { 'ملطف ومرمم': 0.88, 'توحيد لون البشرة': 0.85 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'myrrh-water',
        name: 'ماء المر (هيدروسول)',
        scientific_name: 'Commiphora Myrrha Water',
        searchKeywords: ['myrrh hydrosol', 'myrrh water', 'ماء المر'],
        usageScope: ['skin'],
        functionalCategory: 'ملطف ومرمم',
        chemicalType: 'مستخلص مائي',
        benefits: { 'تطهير': 0.85, 'ملطف ومرمم': 0.88 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'cornflower-water',
        name: 'ماء وردة الذرة (القنطريون)',
        scientific_name: 'Centaurea Cyanus Flower Water',
        searchKeywords: ['cornflower water', 'centaurea cyanus water', 'ماء القنطريون'],
        usageScope: ['skin'],
        functionalCategory: 'ملطف ومرمم',
        chemicalType: 'مستخلص مائي',
        benefits: { 'تهدئة منطقة العين': 0.9, 'تقليل الانتفاخ': 0.85 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'chamomile-water',
        name: 'ماء البابونج (هيدروسول)',
        scientific_name: 'Chamomilla Recutita Flower Water',
        searchKeywords: ['chamomile water', 'chamomile hydrosol', 'ماء البابونج'],
        usageScope: ['skin'],
        functionalCategory: 'ملطف ومرمم',
        chemicalType: 'مستخلص مائي',
        benefits: { 'ملطف ومرمم فائق': 0.95, 'تقليل الاحمرار': 0.9 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'witch-hazel-extract',
        name: 'مستخلص بندق الساحرة',
        scientific_name: 'Hamamelis Virginiana (Witch Hazel) Extract',
        searchKeywords: ['witch hazel extract', 'hamamelis virginiana extract'],
        usageScope: ['skin'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'مستخلص نباتي',
        benefits: { 'قابض للمسام': 0.88, 'مضاد للالتهابات': 0.82 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'المستخلص الخالي من الكحول يحتوي على التانينات المفيدة.' }],
      },
{
        id: 'konjac-root-powder',
        name: 'مسحوق جذور الكونجاك',
        scientific_name: 'Amorphophallus Konjac Root Powder',
        searchKeywords: ['konjac powder', 'glucomannan', 'مسحوق الكونجاك'],
        usageScope: ['skin'],
        functionalCategory: 'محسن قوام',
        chemicalType: 'بوليمر طبيعي',
        benefits: { 'مكثف طبيعي لتكوين الجل': 0.9, 'ترطيب': 0.8 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'gellan-gum',
        name: 'صمغ الجيلان',
        scientific_name: 'Gellan Gum',
        searchKeywords: ['gellan gum'],
        usageScope: ['skin'],
        functionalCategory: 'محسن قوام',
        chemicalType: 'بوليمر طبيعي',
        benefits: { 'مكثف فعال': 0.9, 'تكوين جل سائل': 0.88 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'rice-bran-wax',
        name: 'شمع نخالة الأرز',
        scientific_name: 'Oryza Sativa (Rice) Bran Wax',
        searchKeywords: ['rice bran wax', 'oryza sativa cera'],
        usageScope: ['skin'],
        functionalCategory: 'محسن قوام',
        chemicalType: 'شمع نباتي',
        benefits: { 'مكثف للقوام': 0.9, 'مثبت للمستحلبات': 0.85 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'berry-wax',
        name: 'شمع التوت',
        scientific_name: 'Rhus Verniciflua Peel Cera',
        searchKeywords: ['berry wax', 'rhus verniciflua'],
        usageScope: ['skin'],
        functionalCategory: 'محسن قوام',
        chemicalType: 'شمع نباتي',
        benefits: { 'يعطي ملمسا ناعما': 0.9, 'مكثف': 0.8 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'brassicyl-isoleucinate-esylate',
        name: 'براسيسيل أيزوليوسينات إيسيلات',
        scientific_name: 'Brassicyl Isoleucinate Esylate',
        searchKeywords: ['brassicyl isoleucinate esylate', 'emulsense hc'],
        usageScope: ['hair'],
        functionalCategory: 'مكيف للشعر',
        chemicalType: 'مكيف طبيعي',
        benefits: { 'تكييف طبيعي': 0.92, 'قابل للتحلل': 0.95, 'مضاد للكهرباء الساكنة': 0.88 },
        synergy: { 'brassica-alcohol': 1.1 },
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'brassica-alcohol',
        name: 'كحول البراسيكا',
        scientific_name: 'Brassica Alcohol',
        searchKeywords: ['brassica alcohol'],
        usageScope: ['hair', 'skin'],
        functionalCategory: 'محسن قوام',
        chemicalType: 'كحول دهني',
        benefits: { 'مكثف طبيعي': 0.9, 'مستحلب مساعد': 0.85 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'pca-ethyl-cocoyl-arginate',
        name: 'PCA إيثيل كوكويل أرجينات',
        scientific_name: 'PCA Ethyl Cocoyl Arginate',
        searchKeywords: ['pca ethyl cocoyl arginate', 'aminat-g'],
        usageScope: ['hair', 'skin'],
        functionalCategory: 'مكيف للشعر',
        chemicalType: 'مكيف طبيعي',
        benefits: { 'مضاد للميكروبات': 0.88, 'تكييف لطيف': 0.85, 'مادة حافظة مساعدة': 0.8 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'sodium-shea-butterate',
        name: 'شيا باتريت الصوديوم',
        scientific_name: 'Sodium Shea Butterate',
        searchKeywords: ['sodium shea butterate', 'saponified shea butter'],
        usageScope: ['skin'],
        functionalCategory: 'منظف',
        chemicalType: 'صابون',
        benefits: { 'تنظيف كريمي': 0.9, 'ترطيب': 0.85 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'زبدة الشيا المتصبنة، مكون في الصابون الطبيعي.' }],
      },
{
        id: 'sodium-olivate',
        name: 'أوليفات الصوديوم',
        scientific_name: 'Sodium Olivate',
        searchKeywords: ['sodium olivate', 'saponified olive oil'],
        usageScope: ['skin'],
        functionalCategory: 'منظف',
        chemicalType: 'صابون',
        benefits: { 'تنظيف لطيف': 0.92, 'ترطيب': 0.88 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'زيت الزيتون المتصبن، المكون الرئيسي في صابون قشتالة.' }],
      },
{
        id: 'sodium-castorate',
        name: 'كاستورات الصوديوم',
        scientific_name: 'Sodium Castorate',
        searchKeywords: ['sodium castorate', 'saponified castor oil'],
        usageScope: ['skin'],
        functionalCategory: 'منظف',
        chemicalType: 'صابون',
        benefits: { 'رغوة كريمية': 0.9, 'ترطيب': 0.8 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'sodium-sunflowerseedate',
        name: 'زيت بذور دوار الشمس المتصبن',
        scientific_name: 'Sodium Sunflowerseedate',
        searchKeywords: ['sodium sunflowerseedate', 'saponified sunflower oil'],
        usageScope: ['skin'],
        functionalCategory: 'منظف',
        chemicalType: 'صابون',
        benefits: { 'تنظيف لطيف': 0.85 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'potassium-hydroxide-saponified',
        name: 'هيدروكسيد البوتاسيوم (في الصابون السائل)',
        scientific_name: 'Potassium Hydroxide',
        searchKeywords: ['saponified oils with potassium hydroxide'],
        usageScope: ['skin'],
        functionalCategory: 'منظف',
        chemicalType: 'صابون',
        benefits: { 'مكون أساسي للصابون السائل': 0.95 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'لا يبقى هيدروكسيد البوتاسيوم الحر في المنتج النهائي بعد عملية التصبن.' }],
      },
{
        id: 'hydrolyzed-gardenia-florida-extract',
        name: 'مستخلص الجاردينيا المهدرج',
        scientific_name: 'Hydrolyzed Gardenia Florida Extract',
        searchKeywords: ['hydrolyzed gardenia florida extract'],
        usageScope: ['skin'],
        functionalCategory: 'مضاد أكسدة',
        chemicalType: 'مستخلص نباتي',
        benefits: { 'مضاد أكسدة': 0.8 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'maltodextrin',
        name: 'مالتوديكسترين',
        scientific_name: 'Maltodextrin',
        searchKeywords: ['maltodextrin', 'مالتوديكسترين'],
        usageScope: ['skin'],
        functionalCategory: 'محسن قوام',
        chemicalType: 'مثبت',
        benefits: { 'مثبت للمستحلبات': 0.85, 'عامل سماكة': 0.8, 'ترطيب خفيف': 0.7 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'dextran',
        name: 'ديكستران',
        scientific_name: 'Dextran',
        searchKeywords: ['dextran', 'ديكستران'],
        usageScope: ['skin'],
        functionalCategory: 'محسن قوام',
        chemicalType: 'مكثف',
        benefits: { 'مكثف': 0.8, 'مرطب': 0.75 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'trifolium-pratense-flower-extract',
        name: 'مستخلص زهرة البرسيم الأحمر',
        scientific_name: 'Trifolium Pratense (Clover) Flower Extract',
        searchKeywords: ['red clover extract', 'trifolium pratense'],
        usageScope: ['hair'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'مستخلص نباتي',
        benefits: { 'تقليل تساقط الشعر': 0.88, 'مضاد للالتهابات': 0.8 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'أحد المكونات الرئيسية في مركب Capixyl®.' }],
      },
{
        id: 'citrus-aurantium-flower-extract',
        name: 'مستخلص زهرة البرتقال المر',
        scientific_name: 'Citrus Aurantium Amara (Bitter Orange) Flower Extract',
        searchKeywords: ['neroli extract', 'bitter orange flower extract'],
        usageScope: ['skin'],
        functionalCategory: 'ملطف ومرمم',
        chemicalType: 'مستخلص نباتي',
        benefits: { 'ملطف': 0.85, 'عطري': 0.9 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'aloe-barbadensis-leaf-polysaccharides',
        name: 'سكريات أوراق الألوفيرا',
        scientific_name: 'Aloe Barbadensis Leaf Polysaccharides',
        searchKeywords: ['aloe polysaccharides'],
        usageScope: ['skin'],
        functionalCategory: 'مرطب / مطري',
        chemicalType: 'مرطب',
        benefits: { 'ترطيب': 0.9, 'ملطف ومرمم': 0.88 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'leuconostoc-radish-root-ferment-filtrate',
        name: 'مرشح خميرة جذور الفجل',
        scientific_name: 'Leuconostoc/Radish Root Ferment Filtrate',
        searchKeywords: ['leuconostoc/radish root ferment filtrate', 'leucidal'],
        usageScope: ['skin'],
        functionalCategory: 'مادة حافظة',
        chemicalType: 'مادة حافظة طبيعية',
        benefits: { 'مادة حافظة طبيعية': 0.9, 'مضاد للميكروبات': 0.88, 'ترطيب': 0.75 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'aspen-bark-extract',
        name: 'مستخلص لحاء الحور',
        scientific_name: 'Populus Tremuloides Bark Extract',
        searchKeywords: ['aspen bark extract', 'populus tremuloides'],
        usageScope: ['skin'],
        functionalCategory: 'مادة حافظة',
        chemicalType: 'مادة حافظة طبيعية',
        benefits: { 'مادة حافظة طبيعية': 0.88, 'مصدر طبيعي للساليسيلات': 0.8, 'مضاد للالتهابات': 0.82 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'caprylyl-capryl-wheat-bran-straw-glycosides',
        name: 'جليكوسيدات قش/نخالة القمح كابريليل/كابريل',
        scientific_name: 'Caprylyl/Capryl Wheat Bran/Straw Glycosides',
        searchKeywords: ['caprylyl/capryl wheat bran/straw glycosides'],
        usageScope: ['skin'],
        functionalCategory: 'مذوب',
        chemicalType: 'مستحلب',
        benefits: { 'مذوب لطيف': 0.85 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'fusel-wheat-bran-straw-glycosides',
        name: 'جليكوسيدات قش/نخالة القمح فيوزل',
        scientific_name: 'Fusel Wheat Bran/Straw Glycosides',
        searchKeywords: ['fusel wheat bran/straw glycosides'],
        usageScope: ['skin'],
        functionalCategory: 'مذوب',
        chemicalType: 'مستحلب',
        benefits: { 'مذوب': 0.8 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'polyglyceryl-5-oleate-1',
        name: 'بولي جليسريل-5 أوليات',
        scientific_name: 'Polyglyceryl-5 Oleate',
        searchKeywords: ['polyglyceryl-5 oleate'],
        usageScope: ['skin'],
        functionalCategory: 'مستحلب',
        chemicalType: 'مستحلب',
        benefits: { 'مستحلب': 0.8 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'sodium-olivoyl-glutamate',
        name: 'صوديوم أوليفويل جلوتامات',
        scientific_name: 'Sodium Olivoyl Glutamate',
        searchKeywords: ['sodium olivoyl glutamate'],
        usageScope: ['skin'],
        functionalCategory: 'مستحلب',
        chemicalType: 'مستحلب',
        benefits: { 'مستحلب لطيف': 0.9 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'glyceryl-caprate',
        name: 'كبرات الجليسريل',
        scientific_name: 'Glyceryl Caprate',
        searchKeywords: ['glyceryl caprate'],
        usageScope: ['skin'],
        functionalCategory: 'مستحلب',
        chemicalType: 'مستحلب',
        benefits: { 'مستحلب': 0.8, 'معزز للمواد الحافظة': 0.85 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'hectorite-clay',
        name: 'طين الهيكتوريت',
        scientific_name: 'Hectorite',
        searchKeywords: ['hectorite clay', 'هيكتوريت'],
        usageScope: ['skin'],
        functionalCategory: 'محسن قوام',
        chemicalType: 'طين طبيعي',
        benefits: { 'مكثف طبيعي من الطين': 0.9, 'يثبت المستحلبات': 0.88, 'يعطي قواما كريميا': 0.85 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'olea-europaea-unsaponifiables',
        name: 'غير قابل للتصبن من زيت الزيتون',
        scientific_name: 'Olea Europaea (Olive) Oil Unsaponifiables',
        searchKeywords: ['olive oil unsaponifiables'],
        usageScope: ['skin'],
        functionalCategory: 'ملطف ومرمم',
        chemicalType: 'مطري',
        benefits: { 'إصلاح حاجز البشرة': 0.9, 'غني بالسكوالين والفيتوستيرول': 0.92 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'glycine-soja-unsaponifiables',
        name: 'غير قابل للتصبن من زيت الصويا',
        scientific_name: 'Glycine Soja (Soybean) Oil Unsaponifiables',
        searchKeywords: ['soybean oil unsaponifiables'],
        usageScope: ['skin'],
        functionalCategory: 'ملطف ومرمم',
        chemicalType: 'مطري',
        benefits: { 'إصلاح حاجز البشرة': 0.88 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'triticum-vulgare-germ-unsaponifiables',
        name: 'غير قابل للتصبن من زيت جنين القمح',
        scientific_name: 'Triticum Vulgare (Wheat) Germ Oil Unsaponifiables',
        searchKeywords: ['wheat germ oil unsaponifiables'],
        usageScope: ['skin'],
        functionalCategory: 'ملطف ومرمم',
        chemicalType: 'مطري',
        benefits: { 'إصلاح حاجز البشرة': 0.85 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'pollulan-serine-algin-trehalose-urea-complex',
        name: 'مركب (بولولان، سيرين، ألجين، تريهالوز، يوريا)',
        scientific_name: 'Pullulan, Serine, Algin, Trehalose, Urea',
        searchKeywords: ['pullulan serine algin trehalose urea complex', 'patch2o'],
        usageScope: ['skin'],
        functionalCategory: 'مرطب / مطري',
        chemicalType: 'مرطب',
        benefits: { 'ترطيب فوري وطويل الأمد': 0.95, 'تكوين طبقة مرطبة': 0.9 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'مركب ترطيب متقدم يشكل شبكة جزيئية تطلق المرطبات ببطء مع مرور الوقت.' }],
      },
{
        id: 'hydrolyzed-caesalpinia-spinosa-gum',
        name: 'صمغ تارا المهدرج',
        scientific_name: 'Hydrolyzed Caesalpinia Spinosa Gum',
        searchKeywords: ['hydrolyzed tara gum'],
        usageScope: ['skin'],
        functionalCategory: 'مرطب / مطري',
        chemicalType: 'مرطب',
        benefits: { 'ترطيب': 0.85 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'glyceryl-polyacrylate',
        name: 'بولي أكريلات الجليسريل',
        scientific_name: 'Glyceryl Polyacrylate',
        searchKeywords: ['glyceryl polyacrylate'],
        usageScope: ['skin'],
        functionalCategory: 'مرطب / مطري',
        chemicalType: 'مرطب',
        benefits: { 'تكوين طبقة مرطبة': 0.9, 'ترطيب': 0.85 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'cichorium-intybus-root-oligosaccharides',
        name: 'أوليجوسكاريدات جذور الهندباء',
        scientific_name: 'Cichorium Intybus (Chicory) Root Oligosaccharides',
        searchKeywords: ['chicory root oligosaccharides'],
        usageScope: ['skin'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'مركب نباتي',
        benefits: { 'شد فوري للبشرة (مؤقت)': 0.88 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'pyrus-communis-fruit-extract',
        name: 'مستخلص فاكهة الكمثرى',
        scientific_name: 'Pyrus Communis (Pear) Fruit Extract',
        searchKeywords: ['pear fruit extract'],
        usageScope: ['skin'],
        functionalCategory: 'مضاد أكسدة',
        chemicalType: 'مستخلص نباتي',
        benefits: { 'مضاد أكسدة': 0.8 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'prunus-persica-fruit-extract',
        name: 'مستخلص فاكهة الخوخ',
        scientific_name: 'Prunus Persica (Peach) Fruit Extract',
        searchKeywords: ['peach fruit extract'],
        usageScope: ['skin'],
        functionalCategory: 'مضاد أكسدة',
        chemicalType: 'مستخلص نباتي',
        benefits: { 'مضاد أكسدة': 0.8, 'ترطيب': 0.82 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'vanilla-planifolia-fruit-extract',
        name: 'مستخلص فاكهة الفانيليا',
        scientific_name: 'Vanilla Planifolia Fruit Extract',
        searchKeywords: ['vanilla extract'],
        usageScope: ['skin'],
        functionalCategory: 'مضاد أكسدة',
        chemicalType: 'مستخلص نباتي',
        benefits: { 'مضاد أكسدة': 0.85, 'ملطف ومرمم': 0.8, 'عطري': 0.9 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'zingiber-officinale-root-extract',
        name: 'مستخلص جذور الزنجبيل',
        scientific_name: 'Zingiber Officinale (Ginger) Root Extract',
        searchKeywords: ['ginger root extract'],
        usageScope: ['skin'],
        functionalCategory: 'مضاد أكسدة',
        chemicalType: 'مستخلص نباتي',
        benefits: { 'مضاد أكسدة': 0.9, 'مضاد للالتهابات': 0.88 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'citrus-grandis-peel-extract',
        name: 'مستخلص قشر البوميلو',
        scientific_name: 'Citrus Grandis (Grapefruit) Peel Extract',
        searchKeywords: ['pomelo peel extract', 'citrus grandis'],
        usageScope: ['skin'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'مستخلص نباتي',
        benefits: { 'تنقية البشرة': 0.8 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'ribose',
        name: 'ريبوز',
        scientific_name: 'Ribose',
        searchKeywords: ['ribose'],
        usageScope: ['skin'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'سكر',
        benefits: { 'تنشيط طاقة الخلايا': 0.88, 'مكافحة الشيخوخة': 0.85 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'sodium-dna-1',
        name: 'صوديوم DNA',
        scientific_name: 'Sodium DNA',
        searchKeywords: ['sodium dna'],
        usageScope: ['skin'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'مكون حيوي',
        benefits: { 'إصلاح البشرة': 0.85, 'ترطيب': 0.8 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'disodium-acetyl-glucosamine-phosphate',
        name: 'فوسفات أسيتيل الجلوكوزامين ثنائي الصوديوم',
        scientific_name: 'Disodium Acetyl Glucosamine Phosphate',
        searchKeywords: ['disodium acetyl glucosamine phosphate'],
        usageScope: ['skin'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'مركب سكري',
        benefits: { 'تحفيز إنتاج حمض الهيالورونيك': 0.92 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'algae-oligosaccharides',
        name: 'أوليجوسكاريدات الطحالب',
        scientific_name: 'Algae Oligosaccharides',
        searchKeywords: ['algae oligosaccharides'],
        usageScope: ['skin'],
        functionalCategory: 'مرطب / مطري',
        chemicalType: 'مرطب',
        benefits: { 'ترطيب': 0.8 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'fucus-serratus-extract',
        name: 'مستخلص الفوقس المسنن',
        scientific_name: 'Fucus Serratus Extract',
        searchKeywords: ['fucus serratus', 'serrated wrack'],
        usageScope: ['skin'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'مستخلص طحالب',
        benefits: { 'مكافحة الشيخوخة': 0.8, 'ترطيب': 0.82 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'ascophyllum-nodosum-extract',
        name: 'مستخلص أسكوفيلوم نودوسوم',
        scientific_name: 'Ascophyllum Nodosum Extract',
        searchKeywords: ['ascophyllum nodosum', 'knotted wrack'],
        usageScope: ['skin'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'مستخلص طحالب',
        benefits: { 'تفتيح وتوحيد اللون': 0.85, 'مضاد أكسدة': 0.88 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'pelvetia-canaliculata-extract',
        name: 'مستخلص بيلفيتيا كاناليكولاتا',
        scientific_name: 'Pelvetia Canaliculata Extract',
        searchKeywords: ['pelvetia canaliculata', 'channelled wrack'],
        usageScope: ['skin'],
        functionalCategory: 'مرطب / مطري',
        chemicalType: 'مستخلص طحالب',
        benefits: { 'ترطيب': 0.88 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'laminaria-hyperborea-extract',
        name: 'مستخلص لاميناريا هايبربوريا',
        scientific_name: 'Laminaria Hyperborea Extract',
        searchKeywords: ['laminaria hyperborea'],
        usageScope: ['skin'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'مستخلص طحالب',
        benefits: { 'مكافحة الشيخوخة': 0.8 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'hydrolyzed-corallina-officinalis-extract',
        name: 'مستخلص الطحالب المرجانية المهدرج',
        scientific_name: 'Hydrolyzed Corallina Officinalis Extract',
        searchKeywords: ['hydrolyzed corallina officinalis extract'],
        usageScope: ['skin'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'مستخلص طحالب',
        benefits: { 'غني بالكالسيوم': 0.8 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'palmaria-palmata-extract',
        name: 'مستخلص بالميارا بالماتا',
        scientific_name: 'Palmaria Palmata Extract',
        searchKeywords: ['palmaria palmata', 'dulse'],
        usageScope: ['skin'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'مستخلص طحالب',
        benefits: { 'تفتيح وتوحيد اللون': 0.8, 'تحسين الدورة الدموية': 0.82 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'ulva-lactuca-extract',
        name: 'مستخلص خس البحر',
        scientific_name: 'Ulva Lactuca Extract',
        searchKeywords: ['ulva lactuca', 'sea lettuce'],
        usageScope: ['skin'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'مستخلص طحالب',
        benefits: { 'تحسين مرونة الجلد': 0.85 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'enteromorpha-compressa-extract',
        name: 'مستخلص إنتيرومورفا كومبريسا',
        scientific_name: 'Enteromorpha Compressa Extract',
        searchKeywords: ['enteromorpha compressa'],
        usageScope: ['skin'],
        functionalCategory: 'ملطف ومرمم',
        chemicalType: 'مستخلص طحالب',
        benefits: { 'ملطف ومرمم': 0.85 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'himanthalia-elongata-extract',
        name: 'مستخلص هيمانثاليا إيلونجاتا',
        scientific_name: 'Himanthalia Elongata Extract',
        searchKeywords: ['himanthalia elongata', 'sea spaghetti'],
        usageScope: ['skin'],
        functionalCategory: 'مضاد أكسدة',
        chemicalType: 'مستخلص طحالب',
        benefits: { 'مضاد أكسدة': 0.88, 'حماية من الضوء الأزرق': 0.8 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'padina-pavonica-thallus-extract',
        name: 'مستخلص بادينا بافونيكا',
        scientific_name: 'Padina Pavonica Thallus Extract',
        searchKeywords: ['padina pavonica', 'peacock\'s tail'],
        usageScope: ['skin'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'مستخلص طحالب',
        benefits: { 'تحفيز إنتاج GAGs': 0.9, 'ترطيب': 0.88 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'sargassum-muticum-extract',
        name: 'مستخلص سرجسوم',
        scientific_name: 'Sargassum Muticum Extract',
        searchKeywords: ['sargassum muticum', 'japanese wireweed'],
        usageScope: ['skin'],
        functionalCategory: 'مضاد أكسدة',
        chemicalType: 'مستخلص طحالب',
        benefits: { 'مضاد أكسدة': 0.85 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'porphyra-umbilicalis-extract',
        name: 'مستخلص بورفيرا أومبيليكاليس',
        scientific_name: 'Porphyra Umbilicalis Extract',
        searchKeywords: ['porphyra umbilicalis', 'nori'],
        usageScope: ['skin'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'مستخلص طحالب',
        benefits: { 'حماية من أشعة UVA': 0.85 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'plankton-extract',
        name: 'مستخلص البلانكتون',
        scientific_name: 'Plankton Extract',
        searchKeywords: ['plankton extract'],
        usageScope: ['skin'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'مستخلص بحري',
        benefits: { 'إصلاح الحمض النووي': 0.88, 'حماية من أضرار الشمس': 0.9 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'marine-collagen',
        name: 'الكولاجين البحري',
        scientific_name: 'Soluble Collagen',
        searchKeywords: ['marine collagen', 'fish collagen'],
        usageScope: ['skin'],
        functionalCategory: 'مرطب / مطري',
        chemicalType: 'بروتين',
        benefits: { 'ترطيب': 0.9, 'تكوين طبقة رقيقة': 0.88 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'hydrolyzed-actin',
        name: 'الأكتين المهدرج',
        scientific_name: 'Hydrolyzed Actin',
        searchKeywords: ['hydrolyzed actin'],
        usageScope: ['skin'],
        functionalCategory: 'مرطب / مطري',
        chemicalType: 'بروتين',
        benefits: { 'ترطيب': 0.8 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'hydrolyzed-fibronectin',
        name: 'الفيبرونكتين المهدرج',
        scientific_name: 'Hydrolyzed Fibronectin',
        searchKeywords: ['hydrolyzed fibronectin'],
        usageScope: ['skin'],
        functionalCategory: 'مرطب / مطري',
        chemicalType: 'بروتين',
        benefits: { 'ترطيب': 0.8 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'laurdimonium-hydroxypropyl-hydrolyzed-keratin',
        name: 'كيراتين مهدرج لورديمونيوم هيدروكسي بروبيل',
        scientific_name: 'Laurdimonium Hydroxypropyl Hydrolyzed Keratin',
        searchKeywords: ['laurdimonium hydroxypropyl hydrolyzed keratin'],
        usageScope: ['hair'],
        functionalCategory: 'تقوية الشعر',
        chemicalType: 'بروتين معدل',
        benefits: { 'إصلاح الشعر التالف': 0.92, 'تكييف': 0.88 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'peg-35-castor-oil',
        name: 'زيت الخروع PEG-35',
        scientific_name: 'PEG-35 Castor Oil',
        searchKeywords: ['peg-35 castor oil'],
        usageScope: ['hair'],
        functionalCategory: 'مذوب',
        chemicalType: 'مستحلب',
        benefits: { 'مذوب': 0.85 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'peg-5-cocamide',
        name: 'كوكاميد PEG-5',
        scientific_name: 'PEG-5 Cocamide',
        searchKeywords: ['peg-5 cocamide'],
        usageScope: ['hair'],
        functionalCategory: 'مستحلب',
        chemicalType: 'مستحلب',
        benefits: { 'مستحلب': 0.8 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'peg-60-almond-glycerides',
        name: 'جليسريدات اللوز PEG-60',
        scientific_name: 'PEG-60 Almond Glycerides',
        searchKeywords: ['peg-60 almond glycerides'],
        usageScope: ['skin'],
        functionalCategory: 'مرطب / مطري',
        chemicalType: 'عامل إعادة ترطيب',
        benefits: { 'إعادة ترطيب': 0.85 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'ceteth-10',
        name: 'سيتيث-10',
        scientific_name: 'Ceteth-10',
        searchKeywords: ['ceteth-10'],
        usageScope: ['skin'],
        functionalCategory: 'مستحلب',
        chemicalType: 'مستحلب',
        benefits: { 'مستحلب': 0.8 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'steareth-30',
        name: 'ستيريث-30',
        scientific_name: 'Steareth-30',
        searchKeywords: ['steareth-30', 'ستياريث-30'],
        usageScope: ['hair'],
        functionalCategory: 'مستحلب',
        chemicalType: 'مستحلب',
        benefits: { 'مستحلب': 0.8 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'glycol',
        name: 'جلايكول',
        scientific_name: 'Glycol',
        searchKeywords: ['glycol', 'جليكول'],
        usageScope: ['skin'],
        functionalCategory: 'مذيب',
        chemicalType: 'مرطب',
        benefits: { 'مذيب': 0.8, 'مرطب': 0.75 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'dipotassium-glycyrrhizate',
        name: 'ثنائي بوتاسيوم غليسيريزات',
        scientific_name: 'Dipotassium Glycyrrhizate',
        searchKeywords: ['dipotassium glycyrrhizate', 'licorice root active'],
        usageScope: ['skin'],
        functionalCategory: 'ملطف ومرمم',
        chemicalType: 'مركب نشط نباتي',
        benefits: { 'مضاد للالتهابات': 0.92, 'ملطف ومرمم': 0.9, 'تقليل الاحمرار': 0.88 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'مكون نشط رئيسي آخر من جذر عرق السوس، معروف بخصائصه المهدئة القوية.' }],
      },
{
        id: 'hydrolyzed-hyaluronic-acid-sodium-salt',
        name: 'ملح الصوديوم لحمض الهيالورونيك المهدرج',
        scientific_name: 'Hydrolyzed Sodium Hyaluronate',
        searchKeywords: ['hydrolyzed sodium hyaluronate', 'mini hyaluronic acid'],
        usageScope: ['skin'],
        functionalCategory: 'مرطب / مطري',
        chemicalType: 'مرطب',
        benefits: { 'اختراق عميق جدا': 0.92, 'ترطيب من الداخل': 0.9, 'إصلاح البشرة': 0.88 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'sodium-polyglutamate-crosspolymer',
        name: 'بولي جلوتامات الصوديوم كروسبوليمر',
        scientific_name: 'Sodium Polyglutamate Crosspolymer',
        searchKeywords: ['sodium polyglutamate crosspolymer'],
        usageScope: ['skin'],
        functionalCategory: 'مرطب / مطري',
        chemicalType: 'مرطب',
        benefits: { 'ترطيب طويل الأمد': 0.9 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'ethyl-linolenate',
        name: 'لينولينات الإيثيل',
        scientific_name: 'Ethyl Linolenate',
        searchKeywords: ['ethyl linolenate'],
        usageScope: ['skin'],
        functionalCategory: 'ملطف ومرمم',
        chemicalType: 'مطري',
        benefits: { 'مضاد للالتهابات': 0.85 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'ethyl-oleate-1',
        name: 'أوليات الإيثيل',
        scientific_name: 'Ethyl Oleate',
        searchKeywords: ['ethyl oleate'],
        usageScope: ['skin'],
        functionalCategory: 'مرطب / مطري',
        chemicalType: 'مطري',
        benefits: { 'مطري': 0.8 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'glyceryl-linoleate',
        name: 'لينوليات الجليسريل',
        scientific_name: 'Glyceryl Linoleate',
        searchKeywords: ['glyceryl linoleate'],
        usageScope: ['skin'],
        functionalCategory: 'دعم حاجز البشرة',
        chemicalType: 'مستحلب',
        benefits: { 'دعم حاجز البشرة': 0.9 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'glyceryl-linolenate',
        name: 'لينولينات الجليسريل',
        scientific_name: 'Glyceryl Linolenate',
        searchKeywords: ['glyceryl linolenate'],
        usageScope: ['skin'],
        functionalCategory: 'ملطف ومرمم',
        chemicalType: 'مستحلب',
        benefits: { 'مضاد للالتهابات': 0.88 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'saccharomyces-silicon-ferment',
        name: 'خميرة سيليكون السكاروميسيس',
        scientific_name: 'Saccharomyces/Silicon Ferment',
        searchKeywords: ['saccharomyces/silicon ferment'],
        usageScope: ['skin'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'مكون مخمر',
        benefits: { 'سيليكون حيوي متاح': 0.85 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'saccharomyces-magnesium-ferment',
        name: 'خميرة مغنيسيوم السكاروميسيس',
        scientific_name: 'Saccharomyces/Magnesium Ferment',
        searchKeywords: ['saccharomyces/magnesium ferment'],
        usageScope: ['skin'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'مكون مخمر',
        benefits: { 'مغنيسيوم حيوي متاح': 0.88 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'saccharomyces-copper-ferment',
        name: 'خميرة نحاس السكاروميسيس',
        scientific_name: 'Saccharomyces/Copper Ferment',
        searchKeywords: ['saccharomyces/copper ferment'],
        usageScope: ['skin'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'مكون مخمر',
        benefits: { 'نحاس حيوي متاح': 0.9, 'تحفيز الكولاجين': 0.85 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'saccharomyces-iron-ferment',
        name: 'خميرة حديد السكاروميسيس',
        scientific_name: 'Saccharomyces/Iron Ferment',
        searchKeywords: ['saccharomyces/iron ferment'],
        usageScope: ['skin'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'مكون مخمر',
        benefits: { 'حديد حيوي متاح': 0.8 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'saccharomyces-zinc-ferment',
        name: 'خميرة زنك السكاروميسيس',
        scientific_name: 'Saccharomyces/Zinc Ferment',
        searchKeywords: ['saccharomyces/zinc ferment'],
        usageScope: ['skin'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'مكون مخمر',
        benefits: { 'زنك حيوي متاح': 0.9, 'تنظيم الإفرازات الدهنية': 0.88 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'banana-fruit-juice',
        name: 'عصير فاكهة الموز',
        scientific_name: 'Musa Paradisiaca (Banana) Fruit Juice',
        searchKeywords: ['banana fruit juice', 'musa paradisiaca', 'عصير الموز'],
        usageScope: ['hair', 'skin'],
        functionalCategory: 'مرطب / مطري',
        chemicalType: 'مستخلص نباتي',
        benefits: { 'ترطيب': 0.9, 'غني بالبوتاسيوم': 0.88, 'تنعيم الشعر': 0.85 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'butylphenyl-methylpropional',
        name: 'بوتيل فينيل ميثيل بروبيونال (ليليال)',
        scientific_name: 'Butylphenyl Methylpropional',
        searchKeywords: ['butylphenyl methylpropional', 'lilial', 'ليليال'],
        usageScope: ['skin', 'hair'],
        functionalCategory: 'مكون ضار',
        chemicalType: 'مكون عطري (مسبب حساسية)',
        benefits: { 'رائحة زهرية (زنبق الوادي)': 0.9 },
        synergy: {},
        negativeSynergy: {},
        warnings: [
          { level: 'risk', text: 'مكون عطري تم حظره في الاتحاد الأوروبي منذ مارس 2022 بسبب مخاوف تتعلق بالسمية التناسلية.' },
          { level: 'risk', text: 'مسبب شائع للحساسية.' }
        ],
      },
{
        id: 'vegetable-collagen',
        name: 'كولاجين نباتي',
        scientific_name: 'Hydrolyzed Soy Protein, Rice Amino Acids, Hydrolyzed Adansonia Digitata Seed Extract, etc.',
        searchKeywords: ['vegetable collagen', 'plant collagen', 'vegan collagen', 'كولاجين نباتي'],
        usageScope: ['skin'],
        functionalCategory: 'مرطب / مطري',
        chemicalType: 'بروتين',
        benefits: { 'ترطيب': 0.9, 'تكوين طبقة رقيقة': 0.88, 'تحسين مرونة الجلد': 0.8 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'مزيج من البروتينات والسكريات النباتية التي تحاكي وظيفة الكولاجين الحيواني في الترطيب.' }],
      },
{
        id: 'rose-essential-oil',
        name: 'زيت الورد العطري (الدمشقي)',
        scientific_name: 'Rosa Damascena Flower Oil',
        searchKeywords: ['rose otto', 'rose essential oil', 'rosa damascena oil', 'زيت الورد', 'زيت الورد الدمشقي'],
        usageScope: ['skin'],
        functionalCategory: 'عطر',
        chemicalType: 'زيت عطري',
        benefits: { 'مكافحة الشيخوخة': 0.9, 'ملطف ومرمم': 0.88, 'رائحة فاخرة': 0.98 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'يتم استخراجه بالتقطير بالبخار وهو مختلف عن "مستخلص الورد المطلق".' }],
      },
{
        id: 'sodium-hydrolyzed-potato-starch-dodecenylsuccinate',
        name: 'دوديسينيل سكسينات نشا البطاطس المهدرج',
        scientific_name: 'Sodium Hydrolyzed Potato Starch Dodecenylsuccinate',
        searchKeywords: ['sodium hydrolyzed potato starch dodecenylsuccinate'],
        usageScope: ['skin'],
        functionalCategory: 'محسن قوام',
        chemicalType: 'بوليمر',
        benefits: { 'تثبيت المستحلبات': 0.85, 'يعطي ملمسا ناعما': 0.8 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'potassium-acrylates-copolymer',
        name: 'كوبوليمر أكريلات البوتاسيوم',
        scientific_name: 'Potassium Acrylates Copolymer',
        searchKeywords: ['potassium acrylates copolymer'],
        usageScope: ['skin'],
        functionalCategory: 'محسن قوام',
        chemicalType: 'بوليمر صناعي',
        benefits: { 'تكوين طبقة رقيقة': 0.88, 'مكثف للقوام': 0.8 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'disodium-tetrapropenyl-succinate',
        name: 'ثنائي الصوديوم تترابروبينيل سكسينات',
        scientific_name: 'Disodium Tetrapropenyl Succinate',
        searchKeywords: ['disodium tetrapropenyl succinate'],
        usageScope: ['skin'],
        functionalCategory: 'منظف',
        chemicalType: 'منظف لطيف',
        benefits: { 'تنظيف لطيف': 0.85 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'ci-15985-yellow-6',
        name: 'CI 15985 / أصفر 6',
        scientific_name: 'CI 15985',
        searchKeywords: ['ci 15985', 'yellow 6', 'sunset yellow fcf'],
        usageScope: ['skin', 'hair'],
        functionalCategory: 'محسن جمالي',
        chemicalType: 'ملون صناعي',
        benefits: { 'يعطي لونا برتقاليا-أصفر': 1.0 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'capryloyl-caproyl-methyl-glucamide',
        name: 'كابريلويل/كابرويل ميثيل جلوكاميد',
        scientific_name: 'Capryloyl/Caproyl Methyl Glucamide',
        searchKeywords: ['capryloyl/caproyl methyl glucamide'],
        usageScope: ['hair'],
        functionalCategory: 'منظف',
        chemicalType: 'منظف لطيف جدا',
        benefits: { 'مذوب فعال للزيوت': 0.9, 'رغوة جيدة': 0.85, 'لطيف جدا': 0.92 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'منظف حديث مشتق من السكر، فعال ولطيف.' }],
      },
{
        id: 'lauroyl-myristoyl-methyl-glucamide',
        name: 'لورويل/ميريستويل ميثيل جلوكاميد',
        scientific_name: 'Lauroyl/Myristoyl Methyl Glucamide',
        searchKeywords: ['lauroyl/myristoyl methyl glucamide'],
        usageScope: ['hair'],
        functionalCategory: 'منظف',
        chemicalType: 'منظف لطيف جدا',
        benefits: { 'معزز للرغوة': 0.9, 'لطيف جدا': 0.92 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'arbutin',
        name: 'أربوتين',
        scientific_name: 'Arbutin',
        searchKeywords: ['arbutin', 'beta-arbutin', 'bearberry extract', 'أربوتين', 'بيتا أربوتين'],
        usageScope: ['skin'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'مثبط للتصبغ',
        benefits: { 'تفتيح وتوحيد اللون': 0.8, 'علاج البقع الداكنة': 0.75 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'الشكل الطبيعي (بيتا-أربوتين) أقل استقرارا وفعالية من ألفا-أربوتين.' }],
      },
{
        id: 'phenethyl-alcohol',
        name: 'كحول فينيثيل',
        scientific_name: 'Phenethyl Alcohol',
        searchKeywords: ['phenethyl alcohol'],
        usageScope: ['skin'],
        functionalCategory: 'مادة حافظة',
        chemicalType: 'كحول عطري',
        benefits: { 'مادة حافظة مساعدة': 0.9, 'مضاد للميكروبات': 0.88, 'رائحة ورد خفيفة': 0.8 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'mandarin-essential-oil',
        name: 'زيت المندرين العطري',
        scientific_name: 'Citrus Nobilis (Mandarin Orange) Peel Oil',
        searchKeywords: ['mandarin oil', 'citrus nobilis', 'ماندرين', 'يوسفي', 'زيت المندرين'],
        usageScope: ['skin'],
        functionalCategory: 'عطر',
        chemicalType: 'زيت عطري',
        benefits: { 'رائحة حلوة ومنعشة': 0.9, 'مهدئ': 0.8 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'caution', text: 'قد يسبب حساسية للضوء.' }],
      },
{
        id: 'kumkumadi-oil',
        name: 'زيت كومكومادي',
        scientific_name: 'Kumkumadi Tailam',
        searchKeywords: ['kumkumadi tailam', 'kumkumadi oil', 'كومكومادي'],
        usageScope: ['skin'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'زيت أيورفيدي',
        benefits: { 'تفتيح وتوحيد اللون': 0.9, 'تقليل التصبغات': 0.88, 'مكافحة الشيخوخة': 0.85 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'زيت عشبي أيورفيدي تقليدي يتكون من مزيج من الزعفران، خشب الصندل، وعرق السوس والعديد من الأعشاب الأخرى.' }],
      },
{
        id: 'yeast-extract',
        name: 'مستخلص الخميرة',
        scientific_name: 'Yeast Extract / Faex Extract',
        searchKeywords: ['yeast extract', 'faex extract', 'مستخلص الخميرة'],
        usageScope: ['skin'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'مكون مخمر',
        benefits: { 'غني بالأحماض الأمينية': 0.9, 'ترطيب': 0.88, 'دعم حاجز البشرة': 0.85 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'lactobacillus',
        name: 'لاكتوباسيلس (بروبيوتيك)',
        scientific_name: 'Lactobacillus',
        searchKeywords: ['lactobacillus', 'probiotic', 'لاكتوباسيلس'],
        usageScope: ['skin'],
        functionalCategory: 'دعم حاجز البشرة',
        chemicalType: 'بروبيوتيك',
        benefits: { 'دعم ميكروبيوم البشرة': 0.92, 'ملطف ومرمم': 0.9, 'تقليل الالتهابات': 0.85 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'rose-hydrosol',
        name: 'ماء الورد المقطر (هيدروسول)',
        scientific_name: 'Rosa Damascena Flower Water',
        searchKeywords: ['rose hydrosol', 'rose water', 'ماء الورد المقطر'],
        usageScope: ['skin', 'hair'],
        functionalCategory: 'ملطف ومرمم',
        chemicalType: 'مستخلص مائي',
        benefits: { 'ملطف ومرمم': 0.9, 'ترطيب': 0.85, 'توازن الحموضة': 0.8 },
        synergy: {},
        negativeSynergy: {},
        warnings: [{ level: 'info', text: 'الناتج المائي العطري لعملية تقطير زيت الورد بالبخار.' }],
      },
{
        id: 'bearberry-extract',
        name: 'مستخلص عنب الدب',
        scientific_name: 'Arctostaphylos Uva-Ursi Leaf Extract',
        searchKeywords: ['bearberry extract', 'uva-ursi', 'عنب الدب'],
        usageScope: ['skin'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'مستخلص نباتي',
        benefits: { 'مصدر طبيعي للأربوتين': 0.88, 'تفتيح وتوحيد اللون': 0.85, 'مضاد أكسدة': 0.8 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: 'ci-15985',
        name: 'CI 15985',
        scientific_name: 'CI 15985 / Yellow 6',
        searchKeywords: ['ci 15985', 'yellow 6'],
        usageScope: ['skin'],
        functionalCategory: 'محسن جمالي',
        chemicalType: 'ملون صناعي',
        benefits: { 'يعطي لونا برتقاليا-أصفر': 1.0 },
        synergy: {},
        negativeSynergy: {},
        warnings: [],
      },
{
        id: "yemen-honey-mask",
        name: "ماسك العسل اليمني للوجه",
        searchKeywords: ["yemeni honey mask", "sidr honey mask", "honey face mask", "ماسك العسل اليمني", "ماسك عسل السدر"],
        usageScope: ["skin"],
        functionalCategory: "ماسك وجه",
        chemicalType: "ماسك طبيعي",
        components: ["honey", "black-seed"],
        benefits: { "مضاد للميكروبات": 0.9, "ملطف ومرمم": 0.88, "ترطيب": 0.85, "تفتيح وتوحيد اللون": 0.75 },
        synergy: { "black-seed": 1.1, "turmeric-root-extract": 1.05 },
        negativeSynergy: {},
        warnings: [
          { level: 'info', text: "يستخدم العسل اليمني، وخصوصا عسل السدر، لخصائصه العلاجية والمضادة للبكتيريا القوية." }
        ]
      },
{
        id: "ghassoul-clay-mask",
        name: "ماسك الغاسول المغربي",
        searchKeywords: ["ghassoul clay mask", "rhassoul clay", "moroccan clay mask", "ماسك الغاسول", "ماسك الطين المغربي"],
        usageScope: ["skin", "hair"],
        functionalCategory: "ماسك وجه",
        chemicalType: "ماسك طيني",
        components: ["moroccan-lava-clay", "damask-rose-flower-water"],
        benefits: { "تنقية البشرة": 0.95, "امتصاص الزيوت": 0.92, "تقليل الرؤوس السوداء": 0.85, "تنعيم الشعر": 0.8 },
        synergy: { "argan": 1.05, "damask-rose-flower-water": 1.1 },
        negativeSynergy: {},
        warnings: [
          { level: "caution", text: "قد يكون مجففا للبشرة الجافة إذا ترك ليجف تماما. ينصح بشطفه وهو لا يزال رطبا." }
        ]
      },
{
        id: "egyptian-yogurt-mask",
        name: "ماسك الزبادي المصري (ماسك كليوباترا)",
        scientific_name: "Yogurt & Mel",
        searchKeywords: ["egyptian yogurt mask", "cleopatra mask", "yogurt and honey mask", "ماسك الزبادي والعسل", "ماسك كليوباترا"],
        usageScope: ["skin"],
        functionalCategory: "ماسك وجه",
        chemicalType: "ماسك طبيعي",
        components: ["yogurt", "honey"],
        preparation: "امزجي ملعقتين من الزبادي العادي مع ملعقة من العسل. طبقي المزيج على وجه نظيف لمدة 15-20 دقيقة.",
        benefits: { "ترطيب": 0.9, "تقشير لطيف": 0.85, "ملطف ومرمم": 0.88, "تفتيح وتوحيد اللون": 0.8 },
        synergy: { "honey": 1.1, "oat-kernel-extract": 1.05 },
        negativeSynergy: {},
        warnings: [
          { level: 'info', text: "يعرف بـ 'ماسك كليوباترا'، حيث يقال إنها كانت تستخدم الزبادي (أو الحليب الحامض) للحفاظ على نضارة بشرتها." }
        ]
      },
{
        id: "saudi-sidr-hair-mask",
        name: "ماسك السدر السعودي للشعر",
        scientific_name: "Ziziphus Spina-Christi & Yogurt",
        searchKeywords: ["sidr hair mask", "saudi hair mask", "ziziphus spina-christi", "ماسك السدر للشعر", "خلطة السدر"],
        usageScope: ["hair"],
        functionalCategory: "ماسك شعر",
        chemicalType: "ماسك عشبي مقوي",
        components: ["sidr-leaf-powder", "yogurt"],
        preparation: "اخلطي كمية من مسحوق السدر مع الزبادي والماء الدافئ لتكوين عجينة. طبقيها من الجذور حتى الأطراف، واتركيها لمدة ساعة إلى ساعتين.",
        benefits: { "تقوية الشعر": 0.92, "تنقية فروة الرأس": 0.9, "تكثيف الشعر": 0.88, "علاج القشرة": 0.8 },
        synergy: { "yemen-honey-mask": 1.05, "laurel-berry-oil": 1.1 },
        negativeSynergy: {},
        warnings: [
          { level: 'info', text: "يستخدم مسحوق أوراق السدر تقليديا في السعودية والخليج كغسول طبيعي ومنظف للشعر." },
          { level: "info", text: "قد يسبب جفافا طفيفا للشعر، لذا يفضل إضافة مكونات مرطبة للخلطة مثل الزبادي أو الزيوت." }
        ]
      },
{
        id: "gulf-hammam-zeit",
        name: "حمام الزيت الخليجي",
        scientific_name: "مزيج من الزيوت النباتية",
        searchKeywords: ["hammam zait", "hot oil treatment", "arabic hair oiling", "حمام زيت", "تزييت الشعر"],
        usageScope: ["hair"],
        functionalCategory: "ماسك شعر",
        chemicalType: "ماسك زيتي",
        components: ["coconut", "castor", "arugula-seed-oil", "sweet-almond"],
        preparation: "امزجي كميات متساوية من الزيوت المختارة وقومي بتدفئتها قليلا. دلكي الزيت الدافئ في فروة الرأس ووزعيه على طول الشعر. غطي الشعر واتركيه لمدة ساعة على الأقل.",
        benefits: { "تغذية عميقة": 0.95, "ترطيب": 0.92, "تنعيم وإعطاء لمعان": 0.9, "تحفيز نمو الشعر": 0.85 },
        synergy: { "coconut": 1.1, "castor": 1.08, "arugula-seed-oil": 1.05 },
        negativeSynergy: {},
        warnings: [
          { level: 'info', text: "علاج تقليدي يعتمد على تدفئة مزيج من الزيوت وتدليكها في فروة الرأس والشعر لترطيبه بعمق." }
        ]
      },
{
        id: "maghrebi-henna-mask",
        name: "ماسك الحناء المغربي للشعر",
        scientific_name: "Lawsonia Inermis & additives",
        searchKeywords: ["moroccan henna mask", "henna hair treatment", "ماسك الحناء للشعر", "الحنة المغربية"],
        usageScope: ["hair"],
        functionalCategory: "ماسك شعر",
        chemicalType: "ماسك عشبي مقوي",
        components: ["henna-oil", "amla-oil", "argan", "clove-essential-oil"],
        preparation: "اعجني مسحوق الحناء بالماء الدافئ وشاي البابونج أو الكركديه. أضيفي الزيوت والمكونات الأخرى واتركي المزيج ليتخمر لبضع ساعات قبل تطبيقه على الشعر.",
        benefits: { "تقوية الشعر": 0.9, "تنعيم وإعطاء لمعان": 0.88, "صحة فروة الرأس": 0.85, "إضافة لون نحاسي": 0.8 },
        synergy: { "amla-oil": 1.1, "argan": 1.05, "clove-essential-oil": 1.05 },
        negativeSynergy: {},
        warnings: [
          { level: "caution", text: "قد يضفي لونا نحاسيا على الشعر. لا ينصح باستخدامه على الشعر المصبوغ كيميائيا." }
        ]
      },
{
        id: "algerian-ghassoul-rose-mask",
        name: "ماسك الغاسول وماء الورد الجزائري",
        scientific_name: "Moroccan Lava Clay & Rosa Damascena Water",
        searchKeywords: ["algerian ghassoul mask", "rhassoul clay and rose water", "ماسك الطين الجزائري", "الغاسول وماء الورد"],
        usageScope: ["skin"],
        functionalCategory: "ماسك وجه",
        chemicalType: "ماسك طيني",
        components: ["moroccan-lava-clay", "damask-rose-flower-water"],
        preparation: "اخلطي طين الغاسول مع ماء الورد للحصول على عجينة ناعمة. طبقيها على الوجه وتجنبي منطقة العينين، اتركيها 10 دقائق ثم اشطفيها.",
        benefits: { "تنقية عميقة": 0.9, "امتصاص الزيوت": 0.88, "تنعيم البشرة": 0.85, "تقليل حجم المسام": 0.8 },
        synergy: { "damask-rose-flower-water": 1.1, "argan": 1.05 },
        negativeSynergy: {},
        warnings: [
          { level: 'info', text: "الغاسول، المستخرج من جبال الأطلس، هو جزء أساسي من طقوس الحمام في الجزائر لتنقية البشرة." }
        ]
      },
{
        id: "algerian-coffee-honey-mask",
        name: "ماسك القهوة والعسل الجزائري",
        scientific_name: "Coffea Arabica & Mel",
        searchKeywords: ["algerian coffee mask", "coffee and honey face mask", "ماسك القهوة للوجه", "ماسك القهوة والعسل"],
        usageScope: ["skin"],
        functionalCategory: "ماسك وجه",
        chemicalType: "ماسك مقشر طبيعي",
        components: ["coffee-bean-oil", "honey"],
        preparation: "امزجي ملعقة من تفل القهوة مع ملعقة من العسل. دلكي المزيج بلطف على بشرة رطبة بحركات دائرية لمدة دقيقة، ثم اتركيه لمدة 10 دقائق قبل الشطف.",
        benefits: { "تقشير وتنشيط": 0.9, "تفتيح وتوحيد اللون": 0.85, "مضاد أكسدة": 0.88, "تنعيم البشرة": 0.82 },
        synergy: { "honey": 1.1, "lemon-essential": 1.05 },
        negativeSynergy: {},
        warnings: [
          { level: 'info', text: "يستخدم تفل القهوة كمقشر طبيعي لتنشيط الدورة الدموية وإزالة الجلد الميت." },
          { level: "caution", text: "الفرك القوي قد يكون قاسيا على البشرة الحساسة." }
        ]
      },
{
        id: "algerian-lentisk-oil-mask",
        name: "ماسك زيت الضرو للشعر",
        scientific_name: "Pistacia Lentiscus & Olea Europaea Oil",
        searchKeywords: ["lentisk oil hair mask", "pistacia lentiscus for hair", "algerian hair mask", "ماسك زيت الضرو", "زيت المستكة للشعر"],
        usageScope: ["hair"],
        functionalCategory: "ماسك شعر",
        chemicalType: "ماسك زيتي علاجي",
        components: ["lentisk-oil", "egg-yolk", "olive"],
        preparation: "امزجي بضع قطرات من زيت الضرو مع صفار بيضة وملعقة من زيت الزيتون. طبقي المزيج على فروة الرأس والشعر، واتركيه لمدة ساعة.",
        benefits: { "ملطف ومرمم": 0.92, "علاج مشاكل الفروة": 0.9, "تقوية الشعر": 0.88, "مضاد للميكروبات": 0.85 },
        synergy: { "egg-yolk": 1.1, "olive": 1.05 },
        negativeSynergy: {},
        warnings: [
          { level: 'info', text: "زيت الضرو (المستكة) هو زيت ثمين في الجزائر، ويستخدم تقليديا لخصائصه العلاجية القوية للحروق، الأكزيما، ومشاكل فروة الرأس." }
        ]
      },
{
        id: "mena-turmeric-yogurt-mask",
        name: "ماسك الكركم والزبادي",
        scientific_name: "Curcuma Longa & Yogurt",
        searchKeywords: ["turmeric and yogurt mask", "brightening face mask", "ماسك الكركم والزبادي", "ماسك لتفتيح الوجه"],
        usageScope: ["skin"],
        functionalCategory: "ماسك وجه",
        chemicalType: "ماسك طبيعي مفتح",
        components: ["turmeric-root-extract", "yogurt"],
        preparation: "اخلطي نصف ملعقة صغيرة من مسحوق الكركم مع ملعقتين كبيرتين من الزبادي. طبقي الخليط على الوجه لمدة 15 دقيقة ثم اشطفيه.",
        benefits: { "تفتيح وتوحيد اللون": 0.88, "مضاد للالتهابات": 0.85, "مضاد أكسدة": 0.82, "تقشير لطيف": 0.75 },
        synergy: { "honey": 1.1 },
        negativeSynergy: {},
        warnings: [
          { level: "info", text: "قد يترك الكركم لونا أصفر مؤقتا على البشرة، خاصة الفاتحة منها. يمكن إزالته باستخدام زيت منظف." }
        ]
      },
{
        id: "mena-olive-oil-lemon-hair-mask",
        name: "ماسك زيت الزيتون والليمون للشعر",
        scientific_name: "Olea Europaea Oil & Citrus Limon Juice",
        searchKeywords: ["olive oil and lemon hair mask", "dandruff scalp treatment", "ماسك زيت الزيتون والليمون", "ماسك للقشرة"],
        usageScope: ["hair"],
        functionalCategory: "ماسك شعر",
        chemicalType: "ماسك زيتي منظف",
        components: ["olive", "lemon-essential"],
        preparation: "امزجي ملعقتين من زيت الزيتون مع ملعقتين من عصير الليمون والماء. دلكي المزيج في فروة الرأس واتركيه لمدة 20 دقيقة.",
        benefits: { "علاج القشرة": 0.85, "ترطيب": 0.8, "إضافة لمعان": 0.88, "صحة فروة الرأس": 0.82 },
        synergy: {},
        negativeSynergy: {},
        warnings: [
          { level: "caution", text: "قد يسبب الليمون جفافا للشعر إذا استخدم بكميات كبيرة أو بشكل متكرر. قد يفتح لون الشعر قليلا مع التعرض للشمس." }
        ]
      },
{
        id: "mena-egg-olive-oil-hair-mask",
        name: "ماسك البيض وزيت الزيتون للشعر",
        scientific_name: "Egg & Olea Europaea Oil",
        searchKeywords: ["egg and olive oil hair mask", "protein hair mask", "ماسك البيض للشعر", "ماسك بروتين طبيعي"],
        usageScope: ["hair"],
        functionalCategory: "ماسك شعر",
        chemicalType: "ماسك بروتيني مقوي",
        components: ["egg-yolk", "olive"],
        preparation: "اخفقي بيضة واحدة مع ملعقة كبيرة من زيت الزيتون. طبقي الخليط على الشعر واتركيه لمدة 20 دقيقة قبل شطفه بالماء البارد.",
        benefits: { "تقوية الشعر": 0.9, "تغذية عميقة": 0.88, "إصلاح الشعر التالف": 0.85, "تنعيم وإعطاء لمعان": 0.82 },
        synergy: {},
        negativeSynergy: {},
        warnings: [
          { level: "caution", text: "يجب شطف الماسك بالماء البارد أو الفاتر لتجنب 'طبخ' البيض في الشعر." }
        ]
      },
{
        id: "mena-starch-rosewater-mask",
        name: "ماسك النشا وماء الورد",
        scientific_name: "Corn Starch & Rosa Damascena Water",
        searchKeywords: ["starch and rose water mask", "soothing face mask", "ماسك النشا وماء الورد", "ماسك لتهدئة البشرة"],
        usageScope: ["skin"],
        functionalCategory: "ماسك وجه",
        chemicalType: "ماسك طبيعي مهدئ",
        components: ["corn-starch-modified", "damask-rose-flower-water"],
        preparation: "اخلطي ملعقة كبيرة من النشا مع كمية كافية من ماء الورد لتكوين عجينة. يمكن تسخينها برفق على النار لتكثيفها. طبقيها على الوجه بعد أن تبرد.",
        benefits: { "ملطف ومرمم": 0.9, "تقليل الاحمرار": 0.85, "امتصاص الزيوت": 0.8, "شد البشرة (مؤقت)": 0.75 },
        synergy: {},
        negativeSynergy: {},
        warnings: [
          { level: "info", text: "ماسك بسيط جدا يستخدم لتهدئة البشرة المتهيجة وتقليل الاحمرار بشكل فوري." }
        ]
      },
{
        id: "mena-cucumber-yogurt-mask",
        name: "ماسك الخيار والزبادي",
        scientific_name: "Cucumis Sativus & Yogurt",
        searchKeywords: ["cucumber and yogurt mask", "cooling face mask", "ماسك الخيار والزبادي", "ماسك لترطيب الوجه"],
        usageScope: ["skin"],
        functionalCategory: "ماسك وجه",
        chemicalType: "ماسك طبيعي مرطب",
        components: ["cucumber-seed-oil", "yogurt"],
        preparation: "ابشري نصف خيارة واخلطيها مع ملعقتين كبيرتين من الزبادي. طبقي المزيج على الوجه لمدة 15 دقيقة.",
        benefits: { "ترطيب": 0.92, "ملطف ومرمم": 0.9, "تقليل الانتفاخ": 0.85, "إحساس بالبرودة": 0.88 },
        synergy: {},
        negativeSynergy: {},
        warnings: [
          { level: "info", text: "ماسك منعش ومثالي لتهدئة البشرة بعد التعرض للشمس أو لتخفيف مظهر العيون المتعبة." }
        ]
      },
{
        id: "mena-tomato-honey-mask",
        name: "ماسك الطماطم والعسل",
        scientific_name: "Solanum Lycopersicum & Mel",
        searchKeywords: ["tomato and honey mask", "brightening tomato mask", "ماسك الطماطم والعسل", "ماسك للبشرة الدهنية"],
        usageScope: ["skin"],
        functionalCategory: "ماسك وجه",
        chemicalType: "ماسك طبيعي منقي",
        components: ["tomato-fruit-extract", "honey"],
        preparation: "اهرسي نصف حبة طماطم وامزجيها مع ملعقة من العسل. طبقيها على الوجه لمدة 15 دقيقة.",
        benefits: { "مضاد أكسدة": 0.88, "تفتيح وتوحيد اللون": 0.85, "تنظيم الإفرازات الدهنية": 0.82, "تنقية البشرة": 0.8 },
        synergy: {},
        negativeSynergy: {},
        warnings: [
          { level: "caution", text: "حموضة الطماطم قد تسبب وخزا خفيفا أو تهيجا للبشرة شديدة الحساسية." }
        ]
      },
{
        id: "mena-avocado-olive-oil-mask",
        name: "ماسك الأفوكادو وزيت الزيتون للشعر",
        scientific_name: "Persea Gratissima & Olea Europaea Oil",
        searchKeywords: ["avocado and olive oil hair mask", "deep conditioning mask", "ماسك الأفوكادو للشعر", "ماسك للشعر الجاف"],
        usageScope: ["hair"],
        functionalCategory: "ماسك شعر",
        chemicalType: "ماسك مرطب ومغذي",
        components: ["avocado", "olive"],
        preparation: "اهرسي نصف حبة أفوكادو ناضجة مع ملعقتين من زيت الزيتون. طبقي المزيج على الشعر من الجذور حتى الأطراف واتركيه لمدة 30 دقيقة.",
        benefits: { "تغذية عميقة": 0.95, "ترطيب فائق": 0.92, "إصلاح الشعر التالف": 0.88, "تنعيم وإعطاء لمعان": 0.9 },
        synergy: { "honey": 1.05 },
        negativeSynergy: {},
        warnings: [
          { level: "info", text: "علاج غني ومغذي مثالي للشعر الجاف، المتقصف، أو التالف كيميائيا." }
        ]
      },
{
        id: "mena-potato-lemon-mask",
        name: "ماسك البطاطس والليمون",
        scientific_name: "Solanum Tuberosum & Citrus Limon Juice",
        searchKeywords: ["potato and lemon mask", "dark spot remedy", "ماسك البطاطس والليمون", "ماسك للبقع الداكنة"],
        usageScope: ["skin"],
        functionalCategory: "ماسك وجه",
        chemicalType: "ماسك طبيعي مفتح",
        components: ["potato-juice", "lemon-essential"],
        preparation: "استخدمي عصير حبة بطاطس نيئة وامزجيه مع بضع قطرات فقط من عصير الليمون. طبقيه على المناطق الداكنة باستخدام قطعة قطن.",
        benefits: { "تفتيح وتوحيد اللون": 0.85, "تقليل البقع الداكنة": 0.82, "تقليل الانتفاخ": 0.75 },
        synergy: {},
        negativeSynergy: {},
        warnings: [
          { level: "info", text: "يستخدم عصير البطاطس النيئة تقليديا للمساعدة في تفتيح التصبغات والهالات السوداء بفضل احتوائه على إنزيم الكاتيكولاز." },
          { level: "caution", text: "استخدم الليمون بحذر شديد وبكميات قليلة جدا لأنه قد يكون مهيجا ويزيد من حساسية البشرة للضوء." }
        ]
      },
{
        id: "mena-black-seed-honey-mask",
        name: "ماسك حبة البركة والعسل",
        scientific_name: "Nigella Sativa & Mel",
        searchKeywords: ["black seed and honey mask", "nigella sativa mask", "ماسك حبة البركة والعسل", "ماسك الحبة السوداء"],
        usageScope: ["skin"],
        functionalCategory: "ماسك وجه",
        chemicalType: "ماسك طبيعي منقي",
        components: ["black-seed", "honey"],
        preparation: "امزجي ملعقة من مسحوق حبة البركة مع ملعقتين من العسل للحصول على عجينة. طبقيها على الوجه لمدة 15 دقيقة ثم اشطفيها.",
        benefits: { "مضاد للميكروبات": 0.92, "مضاد للالتهابات": 0.9, "تنقية البشرة": 0.88, "ملطف ومرمم": 0.85 },
        synergy: { "yemen-honey-mask": 1.1 },
        negativeSynergy: {},
        warnings: [
          { level: "info", text: "حبة البركة (الحبة السوداء) لها مكانة مرموقة في الطب التقليدي في المنطقة لخصائصها العلاجية القوية." }
        ]
      },
{
        id: "mena-fenugreek-yogurt-mask",
        name: "ماسك الحلبة والزبادي",
        scientific_name: "Trigonella Foenum-Graecum & Yogurt",
        searchKeywords: ["fenugreek and yogurt mask", "methi face pack", "ماسك الحلبة والزبادي", "ماسك لنضارة الوجه"],
        usageScope: ["skin"],
        functionalCategory: "ماسك وجه",
        chemicalType: "ماسك طبيعي منشط",
        components: ["fenugreek-oil", "yogurt"],
        preparation: "استخدمي مسحوق الحلبة أو انقعي بذورها ثم اطحنيها. امزجيها مع الزبادي وطبقيها على الوجه حتى تجف ثم اشطفيها.",
        benefits: { "إعطاء نضارة": 0.88, "مضاد للالتهابات": 0.85, "تنقية البشرة": 0.8, "ترطيب": 0.82 },
        synergy: {},
        negativeSynergy: {},
        warnings: [
          { level: "info", text: "للحلبة رائحة قوية ونفاذة قد لا تكون مقبولة للجميع." }
        ]
      },
{
        id: "mena-lupine-rosewater-mask",
        name: "ماسك الترمس وماء الورد",
        scientific_name: "Lupinus Albus & Rosa Damascena Water",
        searchKeywords: ["lupine mask", "tirmis scrub", "ماسك الترمس المر", "مقشر الترمس"],
        usageScope: ["skin"],
        functionalCategory: "ماسك وجه",
        chemicalType: "ماسك مقشر طبيعي",
        components: ["lupine-oil", "damask-rose-flower-water"],
        preparation: "امزجي مسحوق الترمس المر المطحون مع كمية كافية من ماء الورد لتكوين عجينة. طبقيها على الوجه لمدة 20 دقيقة، ثم افركيها بلطف بحركات دائرية قبل شطفها.",
        benefits: { "تفتيح وتوحيد اللون": 0.88, "تقشير لطيف": 0.9, "إزالة البقع الداكنة": 0.85, "تنقية البشرة": 0.8 },
        synergy: { "damask-rose-flower-water": 1.05 },
        negativeSynergy: {},
        warnings: [
          { level: "info", text: "يستخدم الترمس المطحون كمقشر ومفتح طبيعي للبشرة في مصر وبلاد الشام منذ القدم." },
          { level: "caution", text: "يجب تجنبه من قبل الأشخاص الذين يعانون من حساسية الترمس أو البقوليات." }
        ]
      },
{
        id: "mena-garlic-olive-oil-mask",
        name: "ماسك الثوم وزيت الزيتون للشعر",
        scientific_name: "Allium Sativum & Olea Europaea Oil",
        searchKeywords: ["garlic and olive oil hair mask", "hair growth mask", "ماسك الثوم للشعر", "خلطة الثوم وزيت الزيتون"],
        usageScope: ["hair"],
        functionalCategory: "ماسك شعر",
        chemicalType: "ماسك زيتي علاجي",
        components: ["garlic-oil", "olive"],
        preparation: "اهرسي فصوص الثوم وانقعيها في زيت زيتون دافئ لمدة ساعة على الأقل. صفي الزيت ودلكي به فروة الرأس جيدا. اتركيه لمدة ساعة على الأقل قبل غسله.",
        benefits: { "تحفيز نمو الشعر": 0.9, "تقليل تساقط الشعر": 0.88, "مضاد للميكروبات": 0.85, "صحة فروة الرأس": 0.82 },
        synergy: { "castor": 1.05, "rosemary-essential": 1.1 },
        negativeSynergy: {},
        warnings: [
          { level: "info", text: "له رائحة قوية جدا ونفاذة قد تبقى في الشعر حتى بعد الغسيل." },
          { level: "caution", text: "قد يسبب تهيجا للفروة الحساسة. يجب إجراء اختبار حساسية أولا." }
        ]
      },
{
        id: "gulf-arugula-hair-mask",
        name: "ماسك الجرجير للشعر",
        scientific_name: "Eruca Sativa",
        searchKeywords: ["arugula hair mask", "jarjeer hair treatment", "ماسك الجرجير للشعر", "خلطة الجرجير"],
        usageScope: ["hair"],
        functionalCategory: "ماسك شعر",
        chemicalType: "ماسك عشبي مقوي",
        components: ["arugula-seed-oil", "coconut"],
        preparation: "اخلطي حزمة من الجرجير الطازج في الخلاط مع قليل من زيت جوز الهند أو الماء للحصول على عجينة ناعمة. طبقيها على فروة الرأس والشعر واتركيها لمدة ساعتين قبل غسلها.",
        benefits: { "تحفيز نمو الشعر": 0.92, "تقوية الشعر": 0.9, "تقليل تساقط الشعر": 0.88, "تغذية": 0.85 },
        synergy: { "arugula-seed-oil": 1.1, "coconut": 1.05 },
        negativeSynergy: {},
        warnings: [
          { level: "info", text: "يستخدم الجرجير الطازج المهروس أو زيته بكثرة في منطقة الخليج كوصفة شعبية لتعزيز كثافة الشعر ونموه." }
        ]
      },
{
        id: 'yogurt',
        name: 'الزبادي',
        scientific_name: 'Yogurt',
        searchKeywords: ['yogurt', 'zabadi', 'laban', 'yaourt', 'زبادي', 'لبن', 'ياغورت'],
        usageScope: ['skin', 'hair'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'مكون طبيعي',
        benefits: { 'تقشير لطيف': 0.88, 'ترطيب': 0.85, 'ملطف ومرمم': 0.8, 'تفتيح وتوحيد اللون': 0.75 },
        synergy: { 'honey': 1.1, 'oat-kernel-extract': 1.05 },
        negativeSynergy: {},
        warnings: [
          { level: 'info', text: 'غني بحمض اللاكتيك الذي يقشر البشرة بلطف ويرطبها.' }
        ],
      },
{
        id: 'egg-yolk',
        name: 'صفار البيض',
        scientific_name: 'Egg Yolk',
        searchKeywords: ['egg yolk', 'egg mask', 'صفار البيض', 'ماسك البيض'],
        usageScope: ['hair', 'skin'],
        functionalCategory: 'تقوية الشعر',
        chemicalType: 'بروتين طبيعي',
        benefits: { 'تقوية الشعر': 0.9, 'تغذية عميقة': 0.88, 'إضافة لمعان': 0.8, 'ترطيب': 0.75 },
        synergy: { 'olive': 1.05, 'castor': 1.08 },
        negativeSynergy: {},
        warnings: [
          { level: 'caution', text: 'عند استخدامه للشعر، يجب شطفه بالماء البارد أو الفاتر لتجنب "طبخه".' }
        ]
      },
{
        id: 'potato-juice',
        name: 'عصير البطاطس',
        scientific_name: 'Solanum Tuberosum Juice',
        searchKeywords: ['potato juice', 'potato for dark spots', 'عصير البطاطس', 'ماء البطاطا'],
        usageScope: ['skin'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'مستخلص نباتي',
        benefits: { 'تفتيح وتوحيد اللون': 0.85, 'تقليل البقع الداكنة': 0.82, 'تقليل الانتفاخ': 0.8, 'مضاد أكسدة': 0.7 },
        synergy: { 'lemon-essential': 1.05 },
        negativeSynergy: {},
        warnings: [
          { level: 'info', text: 'يستخدم تقليديا للمساعدة في تفتيح التصبغات والهالات السوداء بفضل احتوائه على إنزيم الكاتيكولاز.' }
        ]
      },
{
        id: 'clove-essential-oil',
        name: 'زيت القرنفل العطري',
        scientific_name: 'Eugenia Caryophyllus Bud Oil',
        searchKeywords: ['clove oil', 'eugenia caryophyllus', 'clove bud oil', 'قرنفل', 'زيت القرنفل'],
        usageScope: ['hair', 'skin'],
        functionalCategory: 'مكون فعال',
        chemicalType: 'زيت عطري',
        benefits: { 'مضاد للميكروبات': 0.92, 'تحفيز فروة الرأس': 0.85, 'مضاد للالتهابات': 0.8, 'تسكين الألم الموضعي': 0.75 },
        synergy: { 'henna-oil': 1.05 },
        negativeSynergy: {},
        warnings: [
          { level: 'info', text: 'زيت عطري قوي جدا وقد يسبب تهيجا شديدا للجلد. يجب تخفيفه بتركيز منخفض جدا (أقل من 0.5%).' },
          { level: 'risk', text: 'لا ينصح باستخدامه أثناء الحمل.' }
        ]
      },
{
        id: "hydrolyzed-keratin",
        name: "كيراتين مهدرج",
        scientific_name: "Hydrolyzed Keratin",
        searchKeywords: ["hydrolyzed keratin", "keratin hydrolysates", "keratin peptide", "كيراتين مهدرج", "كيراتين متحلل", "بروتين الكيراتين"],
        usageScope: ["hair", "skin"],
        functionalCategory: "تقوية الشعر",
        chemicalType: "بروتين",
        benefits: { "إصلاح الشعر التالف": 0.95, "تقوية الشعر": 0.92, "تقليل الهيشان": 0.88, "ترطيب": 0.85, "إضافة لمعان ونعومة": 0.9 },
        synergy: {},
        negativeSynergy: {},
        warnings: [
            { level: 'info', text: "بروتين كيراتين تم تكسيره إلى جزيئات أصغر قادرة على اختراق جذع الشعرة لإصلاحها وتقويتها من الداخل." },
            { level: 'caution', text: "الاستخدام المفرط لمنتجات البروتين قد يؤدي إلى تصلب الشعر وجعله أكثر عرضة للتكسر." }
        ]
    },
{
        id: "keratin",
        name: "كيراتين",
        scientific_name: "Keratin",
        searchKeywords: ["keratin", "كيراتين"],
        usageScope: ["hair"],
        functionalCategory: "تقوية الشعر",
        chemicalType: "بروتين",
        benefits: { "تكوين طبقة حماية": 0.9, "تنعيم الشعر": 0.88, "تقوية الشعر": 0.85 },
        synergy: {},
        negativeSynergy: {},
        warnings: [
            { level: 'info', text: "بروتين ليفي يشكل البنية الأساسية للشعر. في مستحضرات التجميل، يعمل على تكوين طبقة واقية على سطح الشعرة لتنعيمها وحمايتها." }
        ]
    },
{
        id: "hydrolyzed-pearl",
        name: "بروتين اللؤلؤ المهدرج",
        scientific_name: "Hydrolyzed Pearl",
        searchKeywords: ["hydrolyzed pearl", "pearl extract", "pearl protein", "بروتين اللؤلؤ", "مستخلص اللؤلؤ"],
        usageScope: ["skin", "hair"],
        functionalCategory: "مكون فعال",
        chemicalType: "بروتين",
        benefits: { "إضافة لمعان وإشراقة": 0.92, "تقوية الشعر": 0.88, "ترطيب": 0.85, "مكافحة الشيخوخة": 0.8 },
        synergy: {},
        negativeSynergy: {},
        warnings: [
            { level: 'info', text: "مشتق من اللؤلؤ الطبيعي، غني بالأحماض الأمينية والمعادن التي تغذي الشعر والبشرة، تعزز اللمعان وتحفز تكوين الكيراتين." }
        ]
    },
{
        id: "laminaria-saccharina-extract",
        name: "مستخلص لاميناريا ساكارينا",
        scientific_name: "Laminaria Saccharina Extract",
        searchKeywords: ["laminaria saccharina extract", "sugar kelp", "مستخلص عشب البحر السكري"],
        usageScope: ["skin", "hair"],
        functionalCategory: "مكون فعال",
        chemicalType: "مستخلص طحالب",
        benefits: { "تنظيم الإفرازات الدهنية": 0.9, "ترطيب": 0.85, "ملطف ومرمم": 0.88, "مضاد للميكروبات": 0.8 },
        synergy: {},
        negativeSynergy: {},
        warnings: [
            { level: 'info', text: "نوع من الطحالب البنية (عشب البحر) يساعد على التحكم في إفراز الدهون، وهو مثالي لفروة الرأس الدهنية، كما يوفر الترطيب والحماية." }
        ]
    },
{
        id: "amodimethicone-morpholinomethyl-silsesquioxane-copolymer",
        name: "أمodimethicone/Morpholinomethyl Silsesquioxane Copolymer",
        scientific_name: "Amodimethicone/Morpholinomethyl Silsesquioxane Copolymer",
        searchKeywords: ["amodimethicone/morpholinomethyl silsesquioxane copolymer"],
        usageScope: ["hair"],
        functionalCategory: "مكيف للشعر",
        chemicalType: "سيليكون",
        benefits: { "تكييف ذكي للشعر التالف": 0.92, "حماية اللون": 0.9, "تنعيم الشعر": 0.88 },
        synergy: { "trideceth-5": 1.1 },
        negativeSynergy: {},
        warnings: [
            { level: 'info', text: "سيليكون أميني متقدم يوفر تكييفا مستهدفا للمناطق الأكثر تلفا في الشعر دون أن يسبب تراكمات ثقيلة." }
        ]
    },
{
        id: "lysine-hcl",
        name: "ليسين هيدروكلورايد",
        scientific_name: "Lysine HCl",
        searchKeywords: ["lysine hcl", "lysine hydrochloride", "ليسين"],
        usageScope: ["skin", "hair"],
        functionalCategory: "مكيف للشعر",
        chemicalType: "حمض أميني",
        benefits: { "تكييف الشعر والبشرة": 0.88, "تقوية بنية الشعر": 0.85, "ترطيب": 0.8 },
        synergy: {},
        negativeSynergy: {},
        warnings: [
            { level: 'info', text: "ملح الحمض الأميني \"ليسين\"، يعمل كعامل تكييف للشعر والبشرة ويساهم في تقوية بنية البروتينات مثل الكيراتين." }
        ]
    },
{
        id: "guar-gum",
        name: "صمغ الغوار",
        scientific_name: "Cyamopsis Tetragonoloba (Guar) Gum",
        searchKeywords: ["guar gum", "cyamopsis tetragonoloba gum", "gomme de guar", "صمغ الغوار"],
        usageScope: ["skin", "hair"],
        functionalCategory: "محسن قوام",
        chemicalType: "بوليمر طبيعي",
        benefits: { "مكثف للقوام": 0.92, "تثبيت التركيبة": 0.88, "تكييف خفيف": 0.75 },
        synergy: {},
        negativeSynergy: {},
        warnings: [
            { level: 'info', text: "مكثف طبيعي مشتق من حبوب الغوار، يستخدم لزيادة لزوجة الشامبو والبلسم." }
        ]
    },
{
        id: "ci-14720",
        name: "CI 14720",
        scientific_name: "CI 14720",
        searchKeywords: ["ci 14720", "azorubine", "carmoisine", "acid red 14"],
        usageScope: ["skin", "hair"],
        functionalCategory: "محسن جمالي",
        chemicalType: "ملون صناعي",
        benefits: { "يعطي لونا أحمر": 1.0 },
        synergy: {},
        negativeSynergy: {},
        warnings: [
            { level: 'caution', text: "ملون أحمر صناعي. قد يسبب حساسية جلدية لدى بعض الأفراد." },
            { level: 'risk', text: "غير معتمد للاستخدام في مستحضرات التجميل في الولايات المتحدة." }
        ]
    },
{
        id: "ci-28440",
        name: "CI 28440",
        scientific_name: "CI 28440",
        searchKeywords: ["ci 28440", "brilliant black 1", "black pn", "food black 1"],
        usageScope: ["hair"],
        functionalCategory: "محسن جمالي",
        chemicalType: "ملون صناعي",
        benefits: { "يعطي لونا أسود": 1.0 },
        synergy: {},
        negativeSynergy: {},
        warnings: [
            { level: 'risk', text: "ملون أسود صناعي، استخدامه مقيد في بعض البلدان وغير مسموح به في الولايات المتحدة وكندا." }
        ]
    },
{
        id: "trideceth-7",
        name: "ترايديسيث-7",
        scientific_name: "Trideceth-7",
        searchKeywords: ["trideceth-7", "ترايديسيث 7"],
        usageScope: ["skin", "hair"],
        functionalCategory: "مستحلب",
        chemicalType: "مستحلب",
        benefits: { "مستحلب": 0.9, "عامل منظف": 0.85, "معزز للرغوة": 0.8 },
        synergy: {},
        negativeSynergy: {},
        warnings: [
            { level: 'info', text: "مكون متعدد الوظائف يعمل كمستحلب لخلط الزيت والماء، وكعامل منظف ثانوي." }
        ]
    },
{
        id: "hydrolyzed-wheat-protein",
        name: "بروتين القمح المهدرج",
        scientific_name: "Hydrolyzed Wheat Protein",
        searchKeywords: ["hydrolyzed wheat protein", "phytokeratin", "بروتين القمح المتحلل", "فيتوكيراتين"],
        usageScope: ["hair", "skin"],
        functionalCategory: "تقوية الشعر",
        chemicalType: "بروتين",
        benefits: { "تقوية الشعر": 0.9, "إضافة حجم": 0.88, "تقليل التقصف": 0.85, "ترطيب": 0.8 },
        synergy: {},
        negativeSynergy: {},
        warnings: [
            { level: 'info', text: "بروتين نباتي تم تكسيره ليتغلغل في الشعر بسهولة، مما يساعد على تقويته وإضافة حجم ولمعان." },
            { level: 'risk', text: "يجب تجنبه من قبل الأشخاص الذين يعانون من حساسية القمح أو الغلوتين." }
        ]
    },
{
        id: "distearyl-ether",
        name: "داي ستيريل إيثر",
        scientific_name: "Distearyl Ether",
        searchKeywords: ["distearyl ether", "داي ستيريل إيثر"],
        usageScope: ["skin", "hair"],
        functionalCategory: "مكيف للشعر",
        chemicalType: "مطري",
        benefits: { "تنعيم الشعر والبشرة": 0.9, "تكييف": 0.88 },
        synergy: {},
        negativeSynergy: {},
        warnings: [
            { level: 'info', text: "مطري يعمل على تنعيم الشعر والبشرة ويترك ملمسا خفيفا." }
        ]
    },
{
        id: "peg-4-distearyl-ether",
        name: "داي ستيريل إيثر PEG-4",
        scientific_name: "PEG-4 Distearyl Ether",
        searchKeywords: ["peg-4 distearyl ether"],
        usageScope: ["skin", "hair"],
        functionalCategory: "مكيف للشعر",
        chemicalType: "مستحلب",
        benefits: { "تكييف الشعر": 0.88, "تنعيم البشرة": 0.85 },
        synergy: {},
        negativeSynergy: {},
        warnings: [
            { level: 'info', text: "يعمل كعامل تكييف ومطري للشعر والبشرة." }
        ]
    },
{
        id: "acrylamide-sodium-acryloyldimethyltaurate-copolymer",
        name: "أكريلاميد/صوديوم أكريلويل دايميثيل توريت كوبوليمر",
        scientific_name: "Acrylamide/Sodium Acryloyldimethyltaurate Copolymer",
        searchKeywords: ["acrylamide/sodium acryloyldimethyltaurate copolymer", "sepigel 305"],
        usageScope: ["skin", "hair"],
        functionalCategory: "محسن قوام",
        chemicalType: "بوليمر صناعي",
        benefits: { "مكثف ومثبت": 0.95, "يعطي ملمسا ناعما": 0.9 },
        synergy: {},
        negativeSynergy: {},
        warnings: [
            { level: 'info', text: "غالبا ما يأتي كجزء من مزيج تجاري (مثل Sepigel 305) مع C13-14 Isoparaffin و Laureth-7، ويعمل كمكثف ومثبت فعال جدا." },
            { level: 'caution', text: "هناك مخاوف تنظيمية بشأن احتمال وجود بقايا من مونومر الأكريلاميد، وهي مادة سامة. ومع ذلك، فإن مستوياتها في المنتجات النهائية تخضع لرقابة صارمة لتكون آمنة." }
        ]
    },
{
          id: "c12-15-pareth-2",
          name: "C12-15 باريث-2",
          scientific_name: "C12-15 Pareth-2",
          searchKeywords: ["c12-15 pareth-2"],
          usageScope: ["skin", "hair"],
          functionalCategory: "مستحلب",
          chemicalType: "مستحلب",
          benefits: { "مستحلب": 0.85, "منظف ثانوي": 0.8 },
          synergy: {},
          negativeSynergy: {},
          warnings: []
    },
{
          id: "4-aminobutanoic-acid",
          name: "حمض 4-أمينوبيوتانويك (GABA)",
          scientific_name: "4-Aminobutanoic Acid",
          searchKeywords: ["4-aminobutanoic acid", "gaba", "aminobutyric acid", "جابا"],
          usageScope: ["skin"],
          functionalCategory: "مكون فعال",
          chemicalType: "حمض أميني",
          benefits: { "استرخاء التجاعيد التعبيرية": 0.8, "إصلاح حاجز البشرة": 0.85 },
          synergy: {},
          negativeSynergy: {},
          warnings: [
            {"level": "info", "text": "يعرف أيضا باسم GABA، وهو ناقل عصبي. عند تطبيقه موضعيا، يعتقد أنه يساعد على استرخاء العضلات الدقيقة في الجلد وتقوية حاجز البشرة."}
          ]
    },
{
          id: "hydrolyzed-pea",
          name: "بازلاء مهدرجة",
          scientific_name: "Hydrolyzed Pea",
          searchKeywords: ["hydrolyzed pea"],
          usageScope: ["hair"],
          functionalCategory: "مكيف للشعر",
          chemicalType: "بروتين",
          benefits: { "ترطيب": 0.8, "تكييف": 0.82 },
          synergy: {},
          negativeSynergy: {},
          warnings: []
    },
{
      id: 'bht',
      name: 'بيوتيل هيدروكسي تولوين (BHT)',
      scientific_name: 'Butylated Hydroxytoluene',
      searchKeywords: ['bht', 'butylated hydroxytoluene', 'butylhydroxytoluene', 'بيوتيل هيدروكسي تولوين', 'بوتيل هيدروكسي تولوين'],
      usageScope: ['skin', 'hair'],
      functionalCategory: 'مضاد أكسدة',
      chemicalType: 'مادة حافظة',
      benefits: { 'حماية المنتج من الأكسدة': 0.95, 'إطالة عمر المنتج': 0.9 },
      synergy: {},
      negativeSynergy: {},
      warnings: [
        { level: 'info', text: 'مضاد أكسدة صناعي يستخدم بكميات صغيرة جدا لمنع فساد الزيوت والدهون في التركيبة والحفاظ على استقرارها.' },
        { level: 'caution', text: 'مكون مثير للجدل. هناك بعض المخاوف العامة بشأن احتمالية تسببه في تهيج الجلد أو تأثيره كعامل معطل للغدد الصماء عند تناوله بجرعات عالية.' },
        { level: 'risk', text: 'استخدامه مقيد بتركيزات منخفضة في العديد من البلدان. تعتبره الهيئات التنظيمية آمناً بالتركيزات المستخدمة في مستحضرات التجميل.]' }
      ],
    },
{
      id: 'vitamin-b-complex',
      name: 'مركب فيتامين ب',
      scientific_name: 'Vitamin B Complex (e.g., Niacinamide, Panthenol, Biotin)',
      searchKeywords: ['vitamin b complex', 'b-complex', 'b vitamins', 'مركب فيتامين ب', 'فيتامينات ب'],
      usageScope: ['hair', 'skin'],
      functionalCategory: 'مكون فعال',
      chemicalType: 'مركب فيتامينات',
      benefits: { 
        'صحة فروة الرأس': 0.9, 
        'تقوية الشعر': 0.88, 
        'دعم حاجز البشرة': 0.85, 
        'ترطيب': 0.82 
      },
      synergy: {},
      negativeSynergy: {},
      warnings: [
        { level: 'info', text: 'مصطلح عام يشير إلى مزيج من عدة فيتامينات B. تختلف مكوناته الدقيقة وفوائده باختلاف المنتج.' }
      ],
    },
{
      id: 'ci-77891',
      name: 'CI 77891 (ثاني أكسيد التيتانيوم)',
      scientific_name: 'CI 77891 / Titanium Dioxide',
      searchKeywords: ['ci 77891', 'titanium dioxide', 'ملون أبيض'],
      usageScope: ['skin', 'hair'],
      functionalCategory: 'محسن جمالي',
      chemicalType: 'ملون معدني',
      benefits: { 'يعطي لونا أبيضا': 0.95, 'يجعل التركيبة معتمة': 0.9, 'فلتر شمسي معدني': 0.98 },
      synergy: {},
      negativeSynergy: {},
      warnings: [
        { level: 'info', text: 'عندما يدرج كـ CI 77891، فإنه يستخدم كملون. عندما يدرج كـ Titanium Dioxide، يستخدم عادة كفلتر شمسي.' }
      ],
    },
{
      id: 'ci-42053',
      name: 'CI 42053',
      scientific_name: 'CI 42053 / Green 3',
      searchKeywords: ['ci 42053', 'green 3', 'fast green fcf'],
      usageScope: ['skin', 'hair'],
      functionalCategory: 'محسن جمالي',
      chemicalType: 'ملون صناعي',
      benefits: { 'يعطي لونا أخضر': 1.0 },
      synergy: {},
      negativeSynergy: {},
      warnings: [],
    },
{
      id: 'ci-47005',
      name: 'CI 47005',
      scientific_name: 'CI 47005 / Yellow 10',
      searchKeywords: ['ci 47005', 'yellow 10'],
      usageScope: ['hair'],
      functionalCategory: 'محسن جمالي',
      chemicalType: 'ملون صناعي',
      benefits: { 'يعطي لونا أصفر مخضر': 1.0 },
      synergy: {},
      negativeSynergy: {},
      warnings: [],
    },
{
      id: 'polyglyceryl-6-behenate',
      name: 'بولي جليسريل-6 بيهينات',
      scientific_name: 'Polyglyceryl-6 Behenate',
      searchKeywords: ['polyglyceryl-6 behenate'],
      usageScope: ['skin'],
      functionalCategory: 'مستحلب',
      chemicalType: 'مستحلب',
      benefits: { 'مستحلب': 0.85 },
      synergy: {},
      negativeSynergy: {},
      warnings: [],
    },
{
      id: 'ci-19140',
      name: 'CI 19140',
      scientific_name: 'CI 19140 / Yellow 5',
      searchKeywords: ['ci 19140', 'yellow 5', 'tartrazine'],
      usageScope: ['skin', 'hair'],
      functionalCategory: 'محسن جمالي',
      chemicalType: 'ملون صناعي',
      benefits: { 'يعطي لونا أصفرا': 1.0 },
      synergy: {},
      negativeSynergy: {},
      warnings: [{ level: 'caution', text: 'ارتبط بحساسية نادرة لدى بعض الأشخاص، خاصة الذين يعانون من حساسية الأسبرين.' }],
    },
{
      id: 'ci-15510',
      name: 'CI 15510',
      scientific_name: 'CI 15510 / Orange 4',
      searchKeywords: ['ci 15510', 'orange 4'],
      usageScope: ['skin'],
      functionalCategory: 'محسن جمالي',
      chemicalType: 'ملون صناعي',
      benefits: { 'يعطي لونا برتقاليا': 1.0 },
      synergy: {},
      negativeSynergy: {},
      warnings: [{ level: 'info', text: 'قد يسبب حساسية لدى بعض الأفراد.' }],
    },
{
      id: 'polysorbate-85',
      name: 'بوليسوربات 85',
      scientific_name: 'Polysorbate 85',
      searchKeywords: ['polysorbate 85', 'tween 85'],
      usageScope: ['skin'],
      functionalCategory: 'محسن قوام',
      chemicalType: 'مستحلب',
      benefits: { 'مستحلب (ماء في زيت)': 0.9 },
      synergy: {},
      negativeSynergy: {},
      warnings: []
    },
{
      id: 'peg-75-lanolin',
      name: 'لانولين PEG-75',
      scientific_name: 'PEG-75 Lanolin',
      searchKeywords: ['peg-75 lanolin', 'lanolin ethoxylate'],
      usageScope: ['skin', 'hair'],
      functionalCategory: 'مرطب / مطري',
      chemicalType: 'مستحلب',
      benefits: { 'مطري ومنعم': 0.9, 'مستحلب': 0.85, 'تكييف': 0.82 },
      synergy: {},
      negativeSynergy: {},
      warnings: [
        { level: 'info', text: 'شكل من اللانولين قابل للذوبان في الماء، مما يجعله أقل دهنية وأسهل في الاستخدام في الشامبو والمنظفات.' },
        { level: 'risk', text: 'يجب تجنبه من قبل الأشخاص الذين يعانون من حساسية الصوف.' }
      ]
    },
{
      id: 'triticum-vulgare-gluten',
      name: 'جلوتين القمح',
      scientific_name: 'Triticum Vulgare (Wheat) Gluten',
      searchKeywords: ['wheat gluten', 'triticum vulgare gluten', 'جلوتين القمح'],
      usageScope: ['hair', 'skin'],
      functionalCategory: 'مكيف للشعر',
      chemicalType: 'بروتين',
      benefits: { 'تكييف الشعر': 0.85, 'تكوين طبقة رقيقة': 0.8, 'إصلاح الشعر': 0.82 },
      synergy: {},
      negativeSynergy: {},
      warnings: [
        { level: 'risk', text: 'يجب تجنبه تمامًا من قبل الأشخاص الذين يعانون من حساسية الغلوتين أو مرض الاضطرابات الهضمية.' }
      ]
    },
{
      id: 'peg-12-oleate',
      name: 'أوليات PEG-12',
      scientific_name: 'PEG-12 Oleate',
      searchKeywords: ['peg-12 oleate'],
      usageScope: ['skin', 'hair'],
      functionalCategory: 'محسن قوام',
      chemicalType: 'مستحلب',
      benefits: { 'مستحلب': 0.85, 'مكثف للقوام': 0.8 },
      synergy: {},
      negativeSynergy: {},
      warnings: []
    },
{
      id: 'peg-75-stearate',
      name: 'ستيرات PEG-75',
      scientific_name: 'PEG-75 Stearate',
      searchKeywords: ['peg-75 stearate'],
      usageScope: ['skin'],
      functionalCategory: 'محسن قوام',
      chemicalType: 'مستحلب',
      benefits: { 'مستحلب': 0.88, 'منظف لطيف': 0.8 },
      synergy: {},
      negativeSynergy: {},
      warnings: []
    },
{
      id: 'silicone-general',
      name: 'السيليكون (فئة عامة)',
      scientific_name: 'silicones',
      searchKeywords: ['silicone', 'silicones', 'سيليكون', 'سليكون', 'سيليكونات', 'السيليكون'],
      usageScope: ['hair', 'skin'],
      functionalCategory: 'مكيف للشعر',
      chemicalType: 'بوليمر صناعي',
      benefits: { 'تنعيم وإعطاء لمعان': 0.95, 'فك تشابك الشعر': 0.9, 'تكوين طبقة حماية': 0.92, 'تقليل الهيشان': 0.9, 'حماية من الحرارة': 0.85 },
      synergy: {},
      negativeSynergy: {},
      warnings: [
        { level: 'info', text: 'السيليكون هو فئة من البوليمرات الصناعية المستخدمة على نطاق واسع لإعطاء ملمس ناعم وحريري، لمعان، وحماية للشعر والبشرة.' },
        { level: 'caution', text: 'بعض أنواع السيليكون (مثل Dimethicone) غير قابلة للذوبان في الماء وقد تتراكم على الشعر بمرور الوقت، مما يتطلب استخدام شامبو منظف (clarifying) لإزالتها.' },
        { level: 'info', text: 'السيليكونات المتطايرة (مثل Cyclopentasiloxane) تتبخر بعد التطبيق ولا تتراكم. السيليكونات القابلة للذوبان في الماء (مثل PEG-12 Dimethicone) تُغسل بسهولة.' }
      ],
    },
{
      id: 'water',
      name: 'ماء',
      scientific_name: 'Water (Aqua)',
      searchKeywords: ['water', 'aqua', 'ماء'],
      usageScope: ['hair', 'face', 'body'],
      functionalCategory: 'مذيب',
      chemicalType: 'مذيب أساسي',
      benefits: { 'ترطيب': 0.5, 'إذابة المكونات': 1.0 },
      synergy: {},
      negativeSynergy: {},
      warnings: [{ level: 'info', text: 'يشكل القاعدة الأساسية للمنتج وهو آمن تماماً.' }],
    },
{
      id: 'coco-glycerin-propyl',
      name: 'كوكو غليسيرين بروبيل',
      scientific_name: 'Coco-Glycerin Propyl (Likely PEG-7 Glyceryl Cocoate)',
      searchKeywords: ['coco glycerin', 'peg-7', 'كوكو غليسيرين'],
      usageScope: ['hair', 'body'],
      functionalCategory: 'مطرّي (Emollient)',
      chemicalType: 'إستر دهني',
      benefits: { 'إعادة الدهون للشعر': 0.8, 'تنعيم': 0.75 },
      synergy: {},
      negativeSynergy: {},
      warnings: [{ level: 'info', text: 'يعمل كعامل "إعادة تشحيم" لتعويض الزيوت التي يزيلها الشامبو.' }],
    },
{
      id: 'glycosyl-oleate',
      name: 'غليكوسيل أوليات',
      scientific_name: 'Glycosyl Oleate (Likely Glyceryl Oleate)',
      searchKeywords: ['glyceryl oleate', 'glycosyl', 'أوليات'],
      usageScope: ['hair'],
      functionalCategory: 'مطرّي ومحسّن للدهون',
      chemicalType: 'دهون',
      benefits: { 'حماية الطبقة الدهنية': 0.85, 'نعومة': 0.8 },
      synergy: {},
      negativeSynergy: {},
      warnings: [{ level: 'info', text: 'مكون حيوي مشابه لدهون الجلد الطبيعية، يحمي الفروة من الجفاف.' }],
    },
{
      id: 'gum',
      name: 'صمغ',
      scientific_name: 'Gum (Generic, likely Xanthan or Cellulose)',
      searchKeywords: ['gum', 'xanthan', 'صمغ'],
      usageScope: ['hair', 'face'],
      functionalCategory: 'مغلظ للقوام',
      chemicalType: 'سكريات متعددة',
      benefits: { 'تحسين القوام': 0.8, 'تثبيت المستحلب': 0.7 },
      synergy: {},
      negativeSynergy: {},
      warnings: [{ level: 'info', text: 'مصطلح عام، عادة ما يكون من أصل طبيعي وآمن للاستخدام.' }],
    },
{
      id: 'carotene',
      name: 'كاروتين',
      scientific_name: 'Beta-Carotene',
      searchKeywords: ['carotene', 'vitamin a', 'كرروتان'],
      usageScope: ['hair', 'skin'],
      functionalCategory: 'مضاد أكسدة / ملون',
      chemicalType: 'فيتامين/صبغة',
      benefits: { 'مضاد أكسدة': 0.85, 'لون برتقالي طبيعي': 0.9 },
      synergy: {},
      negativeSynergy: {},
      warnings: [{ level: 'info', text: 'مصدر لفيتامين A، مفيد لصحة الشعر ويعطي لوناً للمنتج.' }],
    },
{
      id: 'fig-extract',
      name: 'مستخلص التين',
      scientific_name: 'Ficus Carica (Fig) Fruit Extract',
      searchKeywords: ['fig extract', 'ficus carica', 'تين'],
      usageScope: ['hair'],
      functionalCategory: 'مكون طبيعي فعال',
      chemicalType: 'مستخلص نباتي',
      benefits: { 'ترطيب': 0.6, 'مضاد أكسدة': 0.6 },
      synergy: {},
      negativeSynergy: {},
      warnings: [{ level: 'info', text: 'يحتوي على فيتامينات وسكريات طبيعية تفيد الشعر.' }],
    },
{
      id: 'cmit',
      name: 'كلورو ميثيل ايزوثيازولينون',
      scientific_name: 'Methylchloroisothiazolinone (CMIT)',
      searchKeywords: ['cmit', 'kathon', 'كلورو ميثيل'],
      usageScope: ['hair (rinse-off only)'],
      functionalCategory: 'مادة حافظة',
      chemicalType: 'Isothiazolinone',
      benefits: { 'حماية من البكتيريا': 0.99 },
      synergy: { 'methylisothiazolinone': 'يستخدمان معاً دائماً لفعالية قصوى (Kathon CG)' },
      negativeSynergy: {},
      warnings: [{ level: 'risk', text: 'مسبب قوي للحساسية (Allergen). يمنع استخدامه في المنتجات التي تترك على الجلد (Leave-on). آمن فقط في المنتجات التي تشطف (Rinse-off) بتركيزات محددة.' }],
    },
{
      id: 'mit',
      name: 'ميثيل ايزوثيازولينون',
      scientific_name: 'Methylisothiazolinone (MIT)',
      searchKeywords: ['mit', 'ميثيل ايزوثيازولينون'],
      usageScope: ['hair (rinse-off only)'],
      functionalCategory: 'مادة حافظة',
      chemicalType: 'Isothiazolinone',
      benefits: { 'حماية من البكتيريا': 0.95 },
      synergy: {},
      negativeSynergy: {},
      warnings: [{ level: 'risk', text: 'مخاطر عالية للتحسس والاكزيما التلامسية. محظور في أوروبا في الكريمات التي لا تغسل.' }],
    },
{
      id: 'melon-extract',
      name: 'مستخلص البطيخ (مالون)',
      scientific_name: 'Cucumis Melo (Melon) Fruit Extract',
      searchKeywords: ['melon', 'malon', 'بطيخ'],
      usageScope: ['hair'],
      functionalCategory: 'مكون طبيعي',
      chemicalType: 'مستخلص نباتي',
      benefits: { 'انتعاش': 0.7, 'ترطيب': 0.6 },
      synergy: {},
      negativeSynergy: {},
      warnings: [{ level: 'info', text: 'الاسم "Malon" في القائمة يرجح أنه خطأ مطبعي لـ Melon (بطيخ/شمام)، وهو مصدر للفيتامينات.' }],
    },
{
      id: 'parfum',
      name: 'عطر',
      scientific_name: 'Parfum (Fragrance)',
      searchKeywords: ['parfum', 'fragrance', 'عطر'],
      usageScope: ['hair', 'body'],
      functionalCategory: 'رائحة',
      chemicalType: 'مزيج عطري',
      benefits: { 'رائحة زكية': 1.0 },
      synergy: {},
      negativeSynergy: {},
      warnings: [{ level: 'caution', text: 'المصطلح "عطر" قد يخفي تحته عشرات المواد الكيميائية التي قد تسبب الحساسية للبعض دون الإفصاح عنها.' }],
    },
    {
  id: 'rapeseed-oil',
  name: 'زيت بذور اللفت',
  scientific_name: 'Brassica Campestris (Rapeseed) Seed Oil',
  searchKeywords: ['rapeseed oil', 'canola oil', 'brassica campestris', 'زيت اللفت', 'كانولا'],
  usageScope: ['hair', 'body', 'face'],
  functionalCategory: 'مطرّي',
  chemicalType: 'زيت نباتي',
  benefits: { 'ترطيب عميق': 0.88, 'فك تشابك الشعر': 0.8, 'حماية من الأكسدة': 0.75 },
  synergy: { 'glycerin':1.0, 'vitamin e': 0.9},
  negativeSynergy: {},
  warnings: [
    { level: 'info', text: 'يُعرف أيضاً باسم "زيت الكانولا" (النسخة منخفضة الحمض). غني جداً بأحماض أوميغا 3 و 6 وفيتامين E، مما يجعله ممتازاً للشعر الجاف.' },
    { level: 'caution', text: 'له قوام دهني غني، وقد يسبب انسداد المسام (Comedogenic) لأصحاب البشرة الدهنية جداً أو المعرضة لحب الشباب عند استخدامه على الوجه.' }
  ],
},
{
  id: 'benzoyl-peroxide',
  name: 'بنزويل بيروكسايد',
  scientific_name: 'Benzoyl Peroxide',
  searchKeywords: ['benzoyl peroxide', 'bpo', 'بنزويل'],
  usageScope: ['face', 'body'],
  functionalCategory: 'مضاد لحب الشباب / مضاد بكتيري',
  chemicalType: 'بيروكسيد عضوي',
  benefits: { 'مكافحة حب الشباب': 0.98, 'تنظيف المسام': 0.9 },
  synergy: { 'adapalene': 1.1 },
  negativeSynergy: { 'vitamin-c': 0.7, 'retinol': 0.8 },
  warnings: [
    { level: 'caution', text: 'يملك خاصية "تبييض" (Bleaching) قوية جداً؛ قد يتلف ألوان المناشف، الملابس، وأغطية الوسائد، ويمكن أن يفتح لون الشعر عند التلامس.' },
    { level: 'caution', text: 'يسبب جفافاً، تقشراً، وتهيجاً في الجلد، خاصة في الأسابيع الأولى من الاستخدام، ويزيد الحساسية تجاه الشمس.' },
    { level: 'info', text: 'يعمل عن طريق إطلاق الأكسجين داخل المسام، مما يقتل البكتيريا المسببة لحب الشباب (C. acnes) لأنها بكتيريا لا هوائية.' }
  ],
},
{
  id: 'adapalene',
  name: 'أدابالين',
  scientific_name: 'Adapalene',
  searchKeywords: ['adapalene', 'differin', 'أدابالين'],
  usageScope: ['face'],
  functionalCategory: 'مقشر / مضاد لحب الشباب',
  chemicalType: 'ريتينويد (جيل ثالث)',
  benefits: { 'تجديد الخلايا': 0.95, 'تنظيف المسام': 0.92, 'مضاد للالتهاب': 0.85 },
  synergy: { 'benzoyl-peroxide': 'مزيج قياسي ذهبي لعلاج حب الشباب، حيث يعمل الأدابالين على فتح المسام والبيروكسايد على قتل البكتيريا.' },
  negativeSynergy: { 'aha-bha': 'استخدامه مع أحماض التقشير (مثل الغليكوليك أو الساليسيليك) في نفس الوقت يزيد بشدة من خطر تهيج والتهاب الحاجز الجلدي.' },
  warnings: [
    { level: 'caution', text: 'يُمنع استخدامه تماماً أثناء الحمل والرضاعة؛ كونه من مشتقات فيتامين A التي قد تشكل خطراً على الجنين.' },
    { level: 'caution', text: 'يزيد حساسية البشرة للشمس بشكل كبير، لذا فإن استخدام واقي الشمس إجباري أثناء فترة العلاج.' },
    { level: 'info', text: 'يتميز عن الريتينول والريتين-أ (Tretinoin) بأنه أكثر استقراراً كيميائياً وأقل تهيجاً للبشرة بشكل عام.' }
  ],
},
{
  id: 'olive-extract',
  name: 'مستخلص الزيتون',
  scientific_name: 'Olea Europaea (Olive) Leaf/Fruit Extract',
  searchKeywords: ['olive extract', 'olea europaea', 'olive leaf', 'مستخلص الزيتون', 'ورق الزيتون'],
  usageScope: ['hair', 'face', 'body'],
  functionalCategory: 'مضاد أكسدة / ملطف',
  chemicalType: 'مستخلص نباتي',
  benefits: { 'حماية من الأكسدة': 0.9, 'تهدئة الجلد': 0.75, 'حماية لون الشعر': 0.8 },
  synergy: { 'vitamin-e': 'يعزز كلاهما قدرة الآخر على محاربة الجذور الحرة وحماية الخلايا من التلف.' },
  negativeSynergy: {},
  warnings: [
    { level: 'info', text: 'غني جداً بمركبات البوليفينول (مثل Oleuropein و Hydroxytyrosol) التي تعتبر من أقوى مضادات الأكسدة الطبيعية لمحاربة الشيخوخة.' },
    { level: 'info', text: 'مكون آمن جداً ومناسب لجميع أنواع البشرة، ويساعد في تهدئة الاحمرار الناتج عن التعرض للشمس.' }
  ],
}

  ]
};