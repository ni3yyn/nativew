...
// Safe bridge sync helper
const syncNativeWidgetTheme = (id) => {
    try {
        if (Platform.OS === 'android' && NativeModules?.SpfWidgetModule?.setAppTheme) {
            NativeModules.SpfWidgetModule.setAppTheme(id);
        }
    } catch (e) {
        console.warn('Native widget theme sync error:', e);
    }
};
...
export const ThemeProvider = ({ children }) => {
    ...
    const changeTheme = async (id) => {
        ...
        // 🌟 1. Save to AsyncStorage FIRST (Never blocked)
        try {
        ...
        // 🌟 2. Sync to Android homescreen widget safely
        syncNativeWidgetTheme(id);
    };
